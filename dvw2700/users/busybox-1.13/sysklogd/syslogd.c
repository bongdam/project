...
static void timestamp_and_log(int pri, char *msg, int len)
{
	...
	IF_FEATURE_VOLATILE_LOG(volatile_log_locally(G.tempLogPath, G.logFileSize << 1, G.printbuf);)
}
...
#define recvbuf (G.recvbuf)
	...
	(void)ntpstate();
#endif
	IF_FEATURE_VOLATILE_LOG(if ((!lstat(G.logFilePath, &statf) && S_ISREG(statf.st_mode)) &&
