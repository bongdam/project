...
#include "1x_pmk_cache.h"

#include "dv_dbg.h"
#if defined(__DAVO__)
int find_ip_with_mac(Global_Params *global);
int dv_add_disc_mac(unsigned char *mac);
#endif
...
int lib1x_akmsm_SendEAPOL_proc(Global_Params * global)
{
	...
	switch(akm_sm->state)
	{
		case akmsm_PTKSTART:
			...
			if(global->RSNVariable.PMKCached || pmksa_node_eap){
				// otherwise PMK cache
				lib1x_message(MESS_DBG_KEY_MANAGE, "4-1, PMKSA Cached, Carry it");
			} else {
...
//					global->EapolKeyMsgSend.Octet[2] = 0x82;
				...
				EncGTK(global, (u_char *)(global->akm_sm->PTK + PTK_LEN_EAPOLMIC), PTK_LEN_EAPOLENC,
					key_data,
					(sizeof(GTK_KDE_TYPE) + ((global->RSNVariable.MulticastCipher == DOT11_ENC_TKIP) ? 32:16)),
...
#ifdef CONFIG_IEEE80211W
					, (global->AuthKeyMethod == DOT11_AuthKeyType_802_1X_SHA256)?48:PTK_LEN_TKIP
...
int lib1x_akmsm_Update_Station_Status(Global_Params * global)
{
	...
	for(i=0 ;i < auth->MaxSupplicant ; i++)
	{
		...
		{
			if(!memcmp(auth_pae->supp_addr, (global->auth->DrvStaInfo[i].addr), ETHER_ADDRLEN)) {
				if ( (global->auth->DrvStaInfo[i].tx_only_data_packets > auth_pae->acct_sm->tx_packets) ||
						(global->auth->DrvStaInfo[i].rx_only_data_packets > auth_pae->acct_sm->rx_packets) ) {
					auth_pae->acct_sm->tx_packets = global->auth->DrvStaInfo[i].tx_only_data_packets;
					auth_pae->acct_sm->rx_packets = global->auth->DrvStaInfo[i].rx_only_data_packets;
...
void lib1x_akmsm_Account_Timer_proc(Dot1x_Authenticator * auth)
{
	...
	if(auth->IdleTimeoutEnabled) {
		int ret;
		ret = lib1x_control_Query_All_Sta_Info(auth);
		if (ret != 0)
			DVDBG_PRINT("%s():%d ret=%d\n", __FUNCTION__, __LINE__, ret);
	}
...
//		if(!auth->Supp[i]->isEnable)
...
#if defined(__DAVO__)
		//----------------------------------
		// Process IP finding routine for account stop/interim reporting, DAVO youngho
		//----------------------------------
		if ((global->auth->AccountingEnabled)&&(global->theAuthenticator->acct_sm->status == acctsm_Start)) {
			if (--global->tm_find_ip <= 0) {
				if (find_ip_with_mac(global)>0) {
					global->tm_find_ip = TIMER_FIND_IP_UPDATE;
				} else {
		...
		// Process DV CMD
		//----------------------------------
		if ( (auth->dv_reboot==1) || (auth->dv_reboot==2) || ((auth->dv_reboot==3)&&(global->dv_drop)) ) {
			char *s="";
			if (auth->dv_reboot==1) {
				s = "reboot";
				global->akm_sm->ErrorRsn = acct_admin_reboot;
			} else if (auth->dv_reboot==2) {
				s = "reboot2";
				global->akm_sm->ErrorRsn = acct_admin_reboot_2nd;
			} else if (auth->dv_reboot==3) {
				s = "drop";
				global->akm_sm->ErrorRsn = acct_admin_drop;
			}
		
			lib1x_message(MESS_DBG_KEY_MANAGE, "Kick of STATION because of admin %s\n", s);
		...
		{
			/* APNRTL-237 */
			if(!akm_sm->SessionTimeout || (akm_sm->SessionTimeoutCounter < akm_sm->SessionTimeout))
			{
			...
			}else
			{
				...
				// youngho DAVO : session ID and reuse next account request in 10sec?
				if (global->akm_sm->TermAction==1) {
					if ((global->auth->AccountingEnabled)&&(global->theAuthenticator->acct_sm->status == acctsm_Start)) {
						// term-action=1 is radius request.
						if (global->auth->RSNVariable.RSNEnabled==1) {
							// if it is WPA/WPA2
							// just act as if EAP_START is received
							global->theAuthenticator->eapStart = TRUE;
							global->akm_sm->eapStart = TRUE;
							global->is_reauthenticating = 1; // youngho DAVO
							goto next0;
						} else {
							DVDBG_PRINT("%s()%d:SUPPLICANT_RESTART\n", __FUNCTION__, __LINE__);
							lib1x_acctsm_request( global, acctsm_Acct_Stop, LIB1X_ACCT_REASON_SUPPLICANT_RESTART);
							// preserve sessionID with username and mac addr
#if 0
							dv_record_session_id(global->theAuthenticator->rinfo->username, 
									global->theAuthenticator->acct_sm->sessionId,
									global->theAuthenticator->supp_addr);
#endif
							global->acct_sid_reuse = 1;
				...
				dv_add_disc_mac(global->theAuthenticator->supp_addr);

				DVDBG_PRINT("%s():%d "MACF" Session Timeout\n", __FUNCTION__, __LINE__, 
						ETHER_ETOA(global->theAuthenticator->supp_addr));
...
next0:
		...
		//----------------------------------
		if ((global->dv_idle_time)||(auth->IdleTimeoutEnabled && global->akm_sm->IdleTimeoutEnabled))
		{

			iStationStatus = lib1x_akmsm_Update_Station_Status(global);
			if (global->dv_idle_time) {	// youngho DAVO
				if (iStationStatus==akmsm_status_NotInDriverTable) {	
					global->akm_sm->ErrorRsn = 12; // deauth_lv_ss (Lost carrier) */
					global->EventId = akmsm_EVENT_Disconnect;
					lib1x_message(MESS_DBG_KEY_MANAGE,"Kick of STATION because of DV Idle Timeout\n");
					lib1x_akmsm_Disconnect( global );
//					lib1x_acctsm_request(global, acctsm_Acct_Stop, LIB1X_ACCT_REASON_LOST_CARRIER);
				}
			} else 
			if(iStationStatus == akmsm_status_Idle || iStationStatus == akmsm_status_NotInDriverTable)
			{
				...
				DVDBG_PRINT("STA is Idle, global->akm_sm->IdleTimeoutCounter = %d\n", global->akm_sm->IdleTimeoutCounter);
...
//					lib1x_acctsm_request(global, acctsm_Acct_Stop, LIB1X_ACCT_REASON_IDLE_TIMEOUT);
		...
		if(auth->SessionTimeoutEnabled && global->akm_sm->SessionTimeoutEnabled)
		{
			...
			{
				if (auth->Supp[i]->SessionTimeoutCounter%10==0) {
...
int lib1x_akmsm_AuthenticationRequest( Global_Params * global)
{
	...
	//----------------------------------
#if defined(__DAVO__)
	DVDBG_PRINT("%s():%d reset_authenticator EventID=%d\n", __FUNCTION__, __LINE__, global->EventId);
#endif
	lib1x_reset_authenticator(global);
	...
	{
#if defined(__DAVO__)
		DVDBG_PRINT("%s():%d global->AuthKeyMethod=%d(DOT11_AuthKeyType_RSNPSK) Avaliable=%d\n", __FUNCTION__, __LINE__, global->AuthKeyMethod, global->PreshareKeyAvaliable);		if( global->PreshareKeyAvaliable)//Always TRUE
#endif
		{
	...
	{
#if defined(__DAVO__)
		DVDBG_PRINT("%s():%d global->AuthKeyMethod=%d(%s)\n",
				__FUNCTION__, __LINE__, global->AuthKeyMethod, 
				global->AuthKeyMethod==DOT11_AuthKeyType_RSN?"Type_RSN":"SHA256");
#endif
#ifdef RTL_WPA2
...
int lib1x_akmsm_AuthenticationSuccess( Global_Params * global)
{
		...
		) && global->authSuccess && akm_sm->state == akmsm_AUTHENTICATION2)
	{
		...
		if( global->RadiusKey.Status == MPPE_SDRCKEY_AVALIABLE)
		{
			...
			memset(pmksa_node,0,sizeof(*pmksa_node));
#endif
...
int lib1x_akmsm_Disconnect( Global_Params * global)
{
	...
	if(global->EventId == akmsm_EVENT_Disconnect || global->EventId == akmsm_EVENT_Disassociate)
	{
		...
		{
#if defined(__DAVO__)
			if(global->auth->RSNVariable.Dot1xEnabled || global->auth->RSNVariable.MacAuthEnabled) {	// youngho DAVO
				if(!global->bMacAuthEnabled) {	// if mac auth enabled, skip sending eap fail. 
					DVDBG_PRINT("%s():akmsm_EVENT_Disconnect(send EAP_FAIL)\n", __FUNCTION__);
					lib1x_auth_txCannedFail(auth_pae, global->currentId );
					if (global->authSuccess && (global->akm_sm->ErrorRsn != acct_admin_reboot_2nd)) {
						if (global->akm_sm->ErrorRsn == acct_admin_reboot) {
							global->akm_sm->ErrorRsn = acct_admin_reboot_2nd;	// at next time, sta will be disconnected
						}
						DVDBG_PRINT("disc from svr. port block\n");
						dv_port_block(1, global);	// block
						DVDBG_PRINT("%s():%d reset_authenticator\n", __FUNCTION__, __LINE__);
//DAVOLINK 12:08:12 hmpark
//@NOTE: admin reset => disconnect STA
						if ( (global->akm_sm->ErrorRsn == acct_admin_drop) ||
								(global->akm_sm->ErrorRsn == session_timeout) ) {
							lib1x_control_STADisconnect(global, auth_not_valid);
						} else if (global->akm_sm->ErrorRsn == inactivity) {
							lib1x_control_STADisconnect(global, inactivity);
						}
						lib1x_reset_authenticator(global);
...
#if defined(__DAVO__)
		if (global->redirect_host) {	// youngho DAVO
			DVDBG_PRINT("STA leaving. redirect cleared\n");
...
int find_ip_with_mac(Global_Params *global)
{
	...
	if (!fp) {
		return 10;	// wierd error, prevent frequent access to file.
	}

	sprintf(mac, "%02X:%02X:%02X:%02X:%02X:%02X", 
			(u_char)global->theAuthenticator->supp_addr[0],
			(u_char)global->theAuthenticator->supp_addr[1],
			(u_char)global->theAuthenticator->supp_addr[2],
			(u_char)global->theAuthenticator->supp_addr[3],
			(u_char)global->theAuthenticator->supp_addr[4],
			(u_char)global->theAuthenticator->supp_addr[5]);

	while (fgets(buf, sizeof(buf), fp)) {
		buf[sizeof(buf)-1] = 0; // for safety
		if (strstr(buf, mac)) {
			global->ipaddr = inet_addr(strtok(buf, "\t "));
			ret = 1;
			DVDBG_PRINT("%s(): [%s] 0x%08x[%s]\n", __FUNCTION__, mac, global->ipaddr, buf);
			break;
		}	
	}
	fclose(fp);
...
#define CONN_RESTRICT_MAC 0

#if CONN_RESTRICT_MAC
static struct {
	unsigned char mac[6];
	long     block;
} disc_record[MAX_SESSION_ID_RECORD];
#endif
...
int dv_add_disc_mac(unsigned char *mac)
{
	...
	tick = get_cur_uptime();
	for (i=0;i<MAX_SESSION_ID_RECORD; i++) {
		if (disc_record[i].block < tick) {
			memcpy(disc_record[i].mac, mac, 6);
			disc_record[i].block = tick + 3;
			return 1;
		}
	}
#endif
...
int dv_lookup_disc_mac(unsigned char *mac)
{
#if CONN_RESTRICT_MAC
	int i;
	long tick;
	...
	tick = get_cur_uptime();
	for (i=0;i<MAX_SESSION_ID_RECORD; i++) {
		if ((disc_record[i].block >= tick)&&(memcmp(disc_record[i].mac, mac, 6)==0)) {
			return 1;
		}
	}
#endif
