...
#define RAMDOM_SESSION_ID 1

#if RAMDOM_SESSION_ID
u_int get_random(int gen_sid);
#endif
extern int dv_lookup_disc_mac(unsigned char *mac);
...
#if defined(__DAVO__)
char lib1x_nas_id[MAX_NAS_ID_LEN]=SYS_NAME;	/* It would be overwritten at lib1x_init_authenticator() */
#else
...
TxRx_Params * lib1x_init_txrx(Dot1x_Authenticator * auth, u_char * oursvr_addr, u_char * svr_addr,
				u_char * oursupp_addr, char * ourip_addr,
				char * svrip_addr, u_short udp_ourport, u_short udp_svrport,
				char * acctip_addr, u_short udp_acctport,
#ifdef RTL_RADIUS_2SET
				char * svr_ip2, u_short udp_svrport2,
				char * acct_ip2, u_short udp_acctport2,
#endif
#if defined(__DAVO__)
				char * svr_domain, char * acct_domain, 
#ifdef RTL_RADIUS_2SET
				char * svr_domain2, char * acct_domain2, 
#endif
#endif
				char * dev_svr, char * dev_supp)
{
...
#if defined(__DAVO__)
/* resolve radius domain */
#ifdef RTL_RADIUS_2SET
#define RS_DOMAIN_CNT 4
#else
#define RS_DOMAIN_CNT 2
#endif
	{
		pid_t pid[RS_DOMAIN_CNT];
		int i;
		char nm[RS_DOMAIN_CNT][40];
		char *nvnm[RS_DOMAIN_CNT];
		char *nvnm_resolve[RS_DOMAIN_CNT];
		char ip_str[40];
		...
		nvnm[0] = "rs_domain";
		nvnm[1] = "acct_domain";
		strcpy(nm[0], svr_domain);
		strcpy(nm[1], acct_domain);
		nvnm_resolve[0] = svrip_addr;
		nvnm_resolve[1] = acctip_addr;
#ifdef RTL_RADIUS_2SET
		nvnm[2] = "rs_domain2";
		nvnm[3] = "acct_domain2";
		strcpy(nm[2], svr_domain2);
		strcpy(nm[3], acct_domain2);
		nvnm_resolve[2] = svr_ip2;
		nvnm_resolve[3] = acct_ip2;
#endif
		for (i=0; i<RS_DOMAIN_CNT; i++) {
			if (nm[i][0]) {
				pid[i] = dv_resolver_exec(nm[i]);
			}
		}
		...
		usleep(100000);
		for (i=0; i<RS_DOMAIN_CNT; i++) {
			if (pid[i]>=0) {    // from child or cache
				save_results(dev_txrx->device_supp, nm[i], nvnm[i]);
			} else {
				save_results(dev_txrx->device_supp, NULL, nvnm[i]);
			}
		}

		for (i=0; i<RS_DOMAIN_CNT; i++) {
			memset(ip_str, 0, sizeof(ip_str));
			if(get_addr_from_file(dev_txrx->device_supp, nvnm[i], ip_str, sizeof(ip_str)) >= 0) {
				strcpy(nvnm_resolve[i], ip_str);
			}
		}
	}
	DVDBG_PRINT("%s():%d svrip_addr=%s acctip_addr=%s\n", __FUNCTION__, __LINE__, svrip_addr, acctip_addr);
	...
	//----endif
	{
		dev_txrx->device_svr = (char*)malloc(IFNAMSIZ + 10);
		...
		//---------------------------------
		dev_txrx->device_supp = (char *)malloc(IFNAMSIZ + 10);
	...
	dev_txrx->network_supp =  lib1x_nal_initialize( dev_txrx->device_supp, oursupp_addr, LIB1X_IT_PKTSOCK);
#else
// move to lib1x_init_driver_interface
...
int lib1x_init_auth(Dot1x_Authenticator * auth)
{
	...
	//Accounting-on Packet Identifier should be different whenever AP turn on
#if RAMDOM_SESSION_ID
	auth->GlobalSessionId = get_random(1);
	auth->GlobalSessionId = ((auth->GlobalSessionId)%100)*10000000;
#else
	gettimeofday(&tv, &tz);
	tv.tv_sec ^= getpid();
...
#if RAMDOM_SESSION_ID
		ulUserName = get_random(0);
		ulUTF8Len = sprintf(szUTF8, "%lu", ulUserName);
#else
...
#if !defined(__DAVO__) // donot send ACCT_ON
...
Global_Params *  lib1x_init_authenticator(Dot1x_Authenticator *auth, TxRx_Params * dev_txrx)
{
	...
	lib1x_acctsm_init(NULL, acct_sm, auth->accountRsMaxReq, auth->accountRsAWhile);
...
#if defined(__DAVO__)
	global->redirect_host = 0; // youngho DAVO
	global->dv_drop = 0; // youngho DAVO
	global->dv_idle_time = 0; // youngho DAVO
	global->wait_eap_start = 0; // youngho DAVO
	global->acct_sid_reuse = 0; // youngho DAVO
	global->ipaddr = 0;
	global->tm_find_ip = TIMER_FIND_IP_1ST;

	global->is_reauthenticating = 0; // youngho DAVO
#endif
...
#ifdef __DAVO__
	gethostname(lib1x_nas_id, sizeof(lib1x_nas_id));
#endif
	strcpy( (char *)auth_pae->rinfo->nas_identifier, lib1x_nas_id); //sc_yang
#if defined(__DAVO__)
    /* 2011-05-19 young */
	strcpy( auth_pae->rinfo->connectinfo, "CONNECT 802.11 AC");
	sprintf(buffer, "/proc/%s/mib_operation", g_wlan_name);
    f = fopen(buffer, "r");
    if (f) {
        int len;
        while (fgets(buffer, sizeof(buffer), f)) {
            if (strstr(buffer, "net_work_type:")) {
				char *p;
				p = strchr(buffer, ':');
		        p++;
		        while(isspace(*p)) {
		        ...
		        if (len>2) {
		            int i = 1;
		            while(p[len-i]==' ' || p[len-i]=='\n' || p[len-i]=='\t') {
		                p[len-i]='\0';
		                i++;
		            }
		        }
				sprintf(auth_pae->rinfo->connectinfo, "CONNECT 802.11 %s", p);
                break;
            }
        }
        fclose(f);
    }
DVDBG_PRINT("%s():%d connectinfo=%s\n", __FUNCTION__, __LINE__, auth_pae->rinfo->connectinfo);
#else
...
#if defined(__DAVO__)
	global->akm_sm->TermAction = 0; // youngho DAVO
#endif
...
void lib1x_reset_authenticator(Global_Params * global)
{
	...
	// 7. Accounting state machine
	lib1x_acctsm_init( global, auth_pae->acct_sm, auth->accountRsMaxReq, auth->accountRsAWhile);
...
int lib1x_timer_authenticator(int signum)
{
	...
	block_timer();
#if 0// defined(__DAVO__)
	/* TODO web redirection */
	if (_redirect_used == REDIR_RESOLV) {
		DVDBG_PRINT("ifname=%s redir_host=%s wlist_file=%s\n", 
				auth->GlobalTxRx->device_wlan0, auth->redir_host, auth->wlist_file);
		dns_query_work(auth->GlobalTxRx->device_wlan0, auth->wlist_file, auth->redir_host);
	}
#endif
	// kenny
...
#if defined(__DAVO__)
					auth->dv_reboot ||   // youngho DAVO
#endif
					(auth->AccountingEnabled && auth->UpdateInterimEnabled))
				{
					lib1x_akmsm_Account_Timer_proc(auth);
#if defined(__DAVO__)
					if (auth->dv_reboot==3)			// youngho DAVO
						auth->dv_reboot = 0;
					else if (auth->dv_reboot==1)		// youngho DAVO. reboot, move to next stage.
						auth->dv_reboot = 2;
					else if ((auth->dv_reboot==2)&&(auth->dv_sigterm==1)) {
						lib1x_remove_fifo();
						printf("auth(%d) terminated\n", getpid());
						exit(0);
					}
#endif
				}
	...
	switch( eapol->packet_type )
	{
		case	LIB1X_EAPOL_LOGOFF :
			...
			if(global->auth->AccountingEnabled)
			{
				...
				DVDBG_PRINT("logoff. port block\n");
				dv_port_block(1, global); // block
				global->wait_eap_start = TRUE;		// logoff  // youngho DAVO
#endif
			}
...
#if defined(__DAVO__)
			DVDBG_PRINT("%s():EAPOL_START\n", __FUNCTION__);
			if (global->wait_eap_start) {
				if (auth_pae->eapLogoff==TRUE) {
					DVDBG_PRINT("%s():%d reset_authenticator\n", __FUNCTION__, __LINE__);
					lib1x_reset_authenticator(global);
				}
				global->wait_eap_start = FALSE;
			}
			if (global->theAuthenticator->eapStart==FALSE) {
...
int lib1x_capture_control(Global_Params * global, struct lib1x_nal_intfdesc * nal, struct lib1x_packet * spkt)
{
	...
	char	wpaIETag[] = {0x00, 0x50, 0xf2, 0x01};
#ifdef RTL_WPA2
	char	wpa2IETag[] = {0x01, 0x00};
	BOOLEAN bWPA2 = FALSE;
#endif
#ifdef HS2_SUPPORT
...
//			bWPA2 = strncmp(wpa2IETag, pAssoInd->RSNIE, sizeof (wpa2IETag)) == 0? TRUE:FALSE;
			bWPA2 = (pAssoInd->RSNIE[0] == WPA2_ELEMENT_ID) && !strncmp((char *)wpa2IETag, (char *)(pAssoInd->RSNIE + 2), sizeof (wpa2IETag))? TRUE:FALSE;
            ...
            #endif
#endif
			if(usLength && ( (!strncmp((char *)wpaIETag, (char *)(pAssoInd->RSNIE + 2), sizeof (wpaIETag))) || bWPA2 ))
...
//				PMFDEBUG("lib1x_control_AssociationRsp\n");
			...
			{
#if defined(__DAVO__)
				if (dv_lookup_disc_mac((unsigned char *)pAssoInd->MACAddr)) {
					/// disconnect .... do not enter here
					lib1x_control_AuthDisconnect(global->auth, (u_char *)pAssoInd->MACAddr, auth_not_valid);
					DVDBG_PRINT("disconnet sta\n");
					goto lib1x_capture_control_END;
				}
#endif
				global->AuthKeyMethod = DOT11_AuthKeyType_PRERSN;
				...
				if(global->auth->AccountingEnabled)
				{
					DVDBG_PRINT("%s()%d "MACF" try REASSOC_USER_REQUEST\n", 
							__FUNCTION__, __LINE__, ETHER_ETOA(pAssoInd->MACAddr));
					lib1x_acctsm_request(global, acctsm_Acct_Stop, LIB1X_ACCT_REASON_USER_REQUEST);
				}
				...
				DVDBG_PRINT("%s():%d "MACF" reset_authenticator\n", 
						__FUNCTION__, __LINE__, ETHER_ETOA(pAssoInd->MACAddr));
#endif
				lib1x_reset_authenticator(global);
				...
				DVDBG_PRINT(""MACF" associated. port block\n", ETHER_ETOA(pAssoInd->MACAddr));
#if defined(__DAVO__)
				dv_port_block(1, global); // block
#endif
				...
				DVDBG_PRINT2("%s() %d "MACF" RADIUS authentication in progress...\n", 
						__FUNCTION__, __LINE__, ETHER_ETOA(pAssoInd->MACAddr));
			}
			break;

	case DOT11_EVENT_AUTHENTICATION_IND:
			DVDBG_PRINT("%s() AUTHENTICATION\n", __FUNCTION__);
			goto lib1x_capture_control_END;
	    	break;
	case DOT11_EVENT_DEAUTHENTICATION_IND:
			DVDBG_PRINT("%s() DEAUTHENTICATION\n", __FUNCTION__);
	 		//global->akm_sm->DeauthenticationRequest = TRUE;
	 		break;
	case DOT11_EVENT_DISASSOCIATION_IND:
			pDisAssoInd = (DOT11_DISASSOCIATION_IND *)msg;
			DVDBG_PRINT("%s() "MACF" DISASSOC\n", __FUNCTION__, ETHER_ETOA(pDisAssoInd->MACAddr));
			//printf("\n---------------------------------------------------------------------\n");
			//printf("---------------DOT11_EVENT_DISASSOCIATION_IND------------------------\n");
			//printf("---------------------------------------------------------------------\n");
#if defined(__DAVO__)
			/* APNRTL-238 */
			memcpy(&global->theAuthenticator->acct_sm->tx_packets, (void *)((unsigned int)pDisAssoInd + (int)(&((DOT11_DISASSOCIATION_IND *)0)->tx_only_data_packets)), sizeof(unsigned long));       // == transmited packets
	 		memcpy(&global->theAuthenticator->acct_sm->rx_packets, (void *)((unsigned int)pDisAssoInd + (int)(&((DOT11_DISASSOCIATION_IND *)0)->rx_only_data_packets)), sizeof(unsigned long));       // == received packets
	 		memcpy(&used_bytes, (void *)((unsigned int)pDisAssoInd + (int)(&((DOT11_DISASSOCIATION_IND *)0)->tx_only_data_bytes)), sizeof(unsigned long long));         // == transmited bytes
			if (used_bytes >= OVER_4G) {
				global->theAuthenticator->acct_sm->tx_bytes_h = used_bytes / OVER_4G;
				global->theAuthenticator->acct_sm->tx_bytes = used_bytes % OVER_4G;
			} else {
				global->theAuthenticator->acct_sm->tx_bytes_h = 0;
				global->theAuthenticator->acct_sm->tx_bytes = used_bytes;
			}
	 		memcpy(&used_bytes, (void *)((unsigned int)pDisAssoInd + (int)(&((DOT11_DISASSOCIATION_IND *)0)->rx_only_data_bytes)), sizeof(unsigned long long));         // == received bytes
			if (used_bytes >= OVER_4G) {
				global->theAuthenticator->acct_sm->rx_bytes_h = used_bytes / OVER_4G;
				global->theAuthenticator->acct_sm->rx_bytes = used_bytes % OVER_4G;
			} else {
				global->theAuthenticator->acct_sm->rx_bytes_h = 0;
				global->theAuthenticator->acct_sm->rx_bytes = used_bytes;
			}
#else
...
#if defined(__DAVO__)
			DVDBG_PRINT("%s(): reason=%d, tx_packets= %u, rx_packets= %u, tx_bytes= %u(%u), rx_bytes= %u(%u)\n",
					__FUNCTION__,
					global->akm_sm->ErrorRsn,
					global->theAuthenticator->acct_sm->tx_packets,
					global->theAuthenticator->acct_sm->rx_packets,
					global->theAuthenticator->acct_sm->tx_bytes,
					global->theAuthenticator->acct_sm->tx_bytes_h,
					global->theAuthenticator->acct_sm->rx_bytes,
					global->theAuthenticator->acct_sm->rx_bytes_h);
#endif
	...
	case DOT11_EVENT_EAPOLSTART_PREAUTH:
			DVDBG_PRINT("%s():%d EAPOLSTART_PREAUTH\n", __FUNCTION__, __LINE__);
...
void lib1x_authsm_capture_svr( Global_Params * global, struct lib1x_nal_intfdesc * nal, struct lib1x_packet * pkt )
{
	...
	switch( rhdr->code  )
	{
		case	LIB1X_RAD_ACCACT:
						...
						DVDBG_PRINT("%s() success. port opened\n", __FUNCTION__);
#if defined(__DAVO__)
						dv_port_block(0, global); // unblock
#endif
						break;
		case	LIB1X_RAD_ACCREJ:
						DVDBG_PRINT("%s():RAD_ACCREJ\n", __FUNCTION__);
...
#if defined(__DAVO__)
							lib1x_acctsm_request(global, acctsm_Acct_Stop, LIB1X_ACCT_REASON_REAUTH_FAIL);
#endif
							global->EventId = akmsm_EVENT_Disconnect;
							global->akm_sm->Disconnect = TRUE;
...
#if defined(__DAVO__)
	global->dv_idle_time = 1;	// youngho DAVO
#endif
...
#if defined(__DAVO__)
			global->dv_idle_time = 0;
#endif
		}
#if defined(__DAVO__)
		if( rattr->type == LIB1X_RAD_TERMINATE_ACTION) {	// youngho DAVO
			lib1x_rad_term_action(global, ((u_char *)rattr), rattr->length);
		}
#endif
...
/* TEST CONFIGURE */
{
	char buf[256];
	u_char		szAttr[4];
	struct lib1x_radiusattr *tmp_rattr;
	tmp_rattr = (struct lib1x_radiusattr *)buf;

/* SESSION TIMEOUT */
	memset(buf, 0, sizeof(buf));
	tmp_rattr->type = LIB1X_RAD_SESSION_TIMEOUT;
	tmp_rattr->length = 0x6;
	memset(szAttr, 0, sizeof(szAttr));
	lib1x_L2N(180, szAttr);
	memcpy ( ( ( u_char* ) tmp_rattr) + LIB1X_RADATTRLEN , szAttr, 4  );

	lib1x_rad_session_timeout(global, ((u_char *)buf), tmp_rattr->length);
printf("%s():%d session_timeout=%d\n", __FUNCTION__, __LINE__, global->akm_sm->SessionTimeout);

/* INTERIM */
	memset(buf, 0, sizeof(buf));
	tmp_rattr->type = LIB1X_RAD_ACCT_INTERIM_TIMEOUT;
	tmp_rattr->length = 0x6;
	memset(szAttr, 0, sizeof(szAttr));
	lib1x_L2N(40, szAttr);
	memcpy ( ( ( u_char* ) tmp_rattr) + LIB1X_RADATTRLEN , szAttr, 4  );
	lib1x_rad_interim_timeout(global, ((u_char *)buf), tmp_rattr->length);
printf("%s():%d intrim=%d\n", __FUNCTION__, __LINE__, global->akm_sm->InterimTimeout);

#if 0
/* IDLE TIMEOUT */
	memset(buf, 0, sizeof(buf));
	tmp_rattr->type = LIB1X_RAD_IDLE_TIMEOUT;
	tmp_rattr->length = 0x6;
	memset(szAttr, 0, sizeof(szAttr));
	lib1x_L2N(5, szAttr);
	memcpy ( ( ( u_char* ) tmp_rattr) + LIB1X_RADATTRLEN , szAttr, 4  );
	lib1x_rad_idle_timeout(global, ((u_char *)buf), tmp_rattr->length);
printf("%s():%d idle_timeout=%d\n", __FUNCTION__, __LINE__, global->akm_sm->IdleTimeout);
	global->dv_idle_time = 0;
#endif
...
/* TERM ACTION */
	memset(buf, 0, sizeof(buf));
	tmp_rattr->type = LIB1X_RAD_TERMINATE_ACTION;
	tmp_rattr->length = 0x6;
	memset(szAttr, 0, sizeof(szAttr));
	lib1x_L2N(0, szAttr);
	memcpy ( ( ( u_char* ) tmp_rattr) + LIB1X_RADATTRLEN , szAttr, 4  );
	lib1x_rad_term_action(global, ((u_char *)buf), tmp_rattr->length);
printf("%s():%d TermAction=%d\n", __FUNCTION__, __LINE__, global->akm_sm->TermAction);
#endif
...
#endif
#if defined(__DAVO__)
				if(global->auth->RSNVariable.Dot1xEnabled || global->auth->RSNVariable.MacAuthEnabled) {
					// youngho DAVO stop if reauth couter over
					DVDBG_PRINT("%s():%d reset_authenticator\n", __FUNCTION__, __LINE__);
					lib1x_reset_authenticator(global);
					global->wait_eap_start = TRUE;
				} else
#endif
				{
				global->EventId = akmsm_EVENT_Disconnect;
...
#endif
#if defined(__DAVO__)
				DVDBG_PRINT("%s():%d Authentication failed:%02x-%02x-%02x-%02x-%02x-%02x\n", __FUNCTION__, __LINE__, 
						global->theAuthenticator->supp_addr[0],
						global->theAuthenticator->supp_addr[1],
						global->theAuthenticator->supp_addr[2],
						global->theAuthenticator->supp_addr[3],
...
#endif
#if defined(__DAVO__)
	eapol->protocol_version = LIB1X_EAPOL_VER_FAKE;
#else
...
int	lib1x_search_supp(Dot1x_Authenticator * auth , struct lib1x_packet * spkt, u_char inttype)
{
	...
	if( inttype == LIB1X_IT_PKTSOCK)
	{
		...
		case    LIB1X_EAPOL_LOGOFF :
			//lib1x_message( MESS_DBG_AUTHNET, " EAPOL LOGOFF");
#if defined(__DAVO__)
			//break; // youngho DAVO DISABLE break; do not ignore LOGOFF
#else
			break;
#endif
...
#if defined(__DAVO__)
	if ((auth->dv_reboot==1) || (auth->dv_reboot==2))
		return -1;	// youngho DAVO, do not accept any request, because it will be rebooted few seconds later.
#endif
...
#if defined(__DAVO__)
			DVDBG_PRINT("%s():%d Supp[%d] reset_authenticator\n", __FUNCTION__, __LINE__, i);
#endif
			lib1x_reset_authenticator(global);
#ifdef CONFIG_RTL_ETH_802DOT1X_SUPPORT
			if(auth->currentRole == role_eth)
...
#if defined(__DAVO__)
TxRx_Params * lib1x_init_driver_interface(Dot1x_Authenticator * auth, char * dev_supp)
{
	TxRx_Params	*	dev_txrx;
	...
	// wireless device initialization
	//---------------------------------
	dev_txrx->device_supp = malloc(IFNAMSIZ + 10);
	memset( dev_txrx->device_supp, 0, sizeof(dev_txrx->device_supp));
	strcpy( dev_txrx->device_supp, dev_supp );
	...
	// wireless device initialization
	//---------------------------------
	dev_txrx->device_wlan0 = malloc(IFNAMSIZ + 10);
	memset( dev_txrx->device_wlan0, 0, sizeof(dev_txrx->device_wlan0));
	//sc_yang
	//strncpy( dev_txrx->device_wlan0, "wlan0", sizeof("wlan0") );
	strcpy( dev_txrx->device_wlan0, dev_txrx->device_supp);
...
u_int get_random(int gen_sid)
{
	...
	if (gen_sid) {
		...
		if (!fp) 
			return 0;
		fscanf(fp, "%u.%u %u.%u", &a[0], &a[1], &a[2], &a[3]);
		fclose(fp);
		a[0] = a[0]*0xff + a[1];
...
//		printf("%u", a[0]);
		...
		if (fp) {
			...
			while (fgets(buf, sizeof(buf), fp)!=NULL) {
				if (sscanf(buf, "%*d: %d %*s %s", &a[1], str)==2) {
					if ((strstr(str, "timer")||(strstr(str, "wlan0")))) {
						//printf(" + %u", a[1]);
						a[0] = a[0]*0xff + a[1];
					}
				}
