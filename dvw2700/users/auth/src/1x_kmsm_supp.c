...
#if defined(CONFIG_RTL865X_KLD) || defined(__DAVO__)
	...
	case DOT11_EVENT_ASSOCIATION_IND:
		...
		if (pGlobal->auth->UsePassphrase) {
			...
			if (strcmp((char *)pGlobal->auth->RSNVariable.ssid, (char *)ssid)) {
				...
				PasswordHash((char *)pGlobal->auth->RSNVariable.PassPhrase, strlen((char *)pGlobal->auth->RSNVariable.PassPhrase),
					pGlobal->auth->RSNVariable.ssid, strlen((char *)pGlobal->auth->RSNVariable.ssid), pGlobal->auth->RSNVariable.PassPhraseKey);
				memcpy(pGlobal->supp_kmsm->PMK,
		...
		if ( pGlobal->auth->RSNVariable.WPA2Enabled) {
			...
			// david+2006-03-31, add event to syslog
			{
				...
				DVDBG_PRINT2("%s():%d Authenticating...... : %02x-%02x-%02x-%02x-%02x-%02x\n", __FUNCTION__, __LINE__, 
...
#if defined(CONFIG_RTL865X_KLD) || defined(__DAVO__)
				DVDBG_PRINT2("%s():%d Authentication failed (4-3: ERROR_AESKEYWRAP_MIC_FAIL) : %02x-%02x-%02x-%02x-%02x-%02x\n", __FUNCTION__, __LINE__, 
					ap_mac[0],ap_mac[1],ap_mac[2],ap_mac[3],ap_mac[4],ap_mac[5]);	
#endif											
...
#if defined(CONFIG_RTL865X_KLD) || defined(__DAVO__)
			DVDBG_PRINT2("%s():%d Authentication Success : %02x-%02x-%02x-%02x-%02x-%02x\n", __FUNCTION__, __LINE__, 
					ap_mac[0],ap_mac[1],ap_mac[2],ap_mac[3],ap_mac[4],ap_mac[5]);	
#endif			
		}
	}
