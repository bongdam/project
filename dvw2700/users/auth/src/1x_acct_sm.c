...
void lib1x_acctsm_init(Global_Params * global, Acct_SM * acct_sm, int maxReq, int aWhile)
{
#if defined(__DAVO__)
	int backup_elapsedSessionTime = -1;

	if (global && (global->auth->AccountingEnabled)&&(acct_sm->status == acctsm_Start)) {
		// acct already enabled
		if (global->is_reauthenticating == 1) { // youngho DAVO
			backup_elapsedSessionTime = acct_sm->elapsedSessionTime;
		} else {
			DVDBG_PRINT2("lib1x_acctsm_init(): acct_already enabled\n");
...
#if defined(__DAVO__)
	if (backup_elapsedSessionTime != -1) {
	    acct_sm->elapsedSessionTime = backup_elapsedSessionTime;
	...
	switch(iAction)
	{
		...
		case	acctsm_Acct_Start:
			...
			if(acct_sm->status == acctsm_Stop)
			{
				...
				acct_sm->status = acctsm_Start;
#if defined(__DAVO__)
				if (global->acct_sid_reuse) {
					DVDBG_PRINT("reuse_sid:%s, %u\n", auth_pae->rinfo->username, acct_sm->sessionId);
				} else
#endif
				{
					acct_sm->sessionId = global->auth->GlobalSessionId;
					g_sid_inc = 1;
					DVDBG_PRINT("session_id allocated:%s, %u\n", auth_pae->rinfo->username, acct_sm->sessionId);
				}
				lib1x_message(MESS_DBG_ACCT, "==============================================\n");
		...
		case	acctsm_Acct_Stop:
			...
			if(acct_sm->status == acctsm_Start)
			{
				...
				acct_sm->status = acctsm_Stop;
#if defined(__DAVO__)
				lib1x_control_ClearSTA(global);	// clear stats of STA, youngho DAVO
				// clear stats of acct_sm
				acct_sm->tx_packets = 0;
				acct_sm->rx_packets = 0;
				acct_sm->tx_bytes   = 0;
				acct_sm->tx_bytes_h = 0;
				acct_sm->rx_bytes   = 0;
				acct_sm->rx_bytes_h = 0;
#endif
			}
			break;
		case	acctsm_Acct_On:
...
int lib1x_acct_request( Auth_Pae * auth_pae, unsigned int msg_type, int iTerminateCause)
{
	...
	/* 11/05/16 uyoung3 */
	ulUTF8Len = sprintf((char *)szUTF8, "%09u", (unsigned int)auth_pae->acct_sm->sessionId);
	...
	switch(msg_type)
	{
		...
		case LIB1X_RADACCT_ACTION_ACCOUNT_STOP:
			...
			DVDBG_PRINT("%s(): TerminateCause=%d tx_packets= %u, rx_packets= %u, tx_bytes= %u(%u), rx_bytes= %u(%u)\n",
					__FUNCTION__, iTerminateCause, 
					auth_pae->global->theAuthenticator->acct_sm->tx_packets,
					auth_pae->global->theAuthenticator->acct_sm->rx_packets,
					auth_pae->global->theAuthenticator->acct_sm->tx_bytes,
					auth_pae->global->theAuthenticator->acct_sm->tx_bytes_h,
					auth_pae->global->theAuthenticator->acct_sm->rx_bytes,
					auth_pae->global->theAuthenticator->acct_sm->rx_bytes_h);
#endif
			...
			lib1x_L2N(acct_sm->rx_bytes_h, szAttr);
			lib1x_radconst_addattr( rconst, LIB1X_RAD_ACCT_INPUT_GIGAWORDS, 4, szAttr);

			lib1x_L2N(acct_sm->tx_bytes_h, szAttr);
		...
		case LIB1X_RADACCT_ACTION_ACCOUNT_ON:
			...
			tv.tv_sec ^= getpid();
#if !defined(__DAVO__)
			ulOutput = sprintf(szOutput, "%u", (unsigned int)tv.tv_sec);
#else
	...
	{
#if defined(__DAVO__)
		lib1x_radconst_addattr( rconst, LIB1X_RAD_NAS_IDENTIFIER, strlen( rinfo->nas_identifier), (u_char *)rinfo->nas_identifier );
...
#if defined(__DAVO__)
		case	acct_admin_drop:		// youngho DAVO
			retVal = LIB1X_ACCT_REASON_ADMIN_RESET;
			break;

		case	acct_admin_reboot:		// youngho DAVO
			retVal = LIB1X_ACCT_REASON_ADMIN_REBOOT;
			break;
