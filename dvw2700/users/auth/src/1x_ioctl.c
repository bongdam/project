...
#endif
{
	...
	lib1x_message(MESS_DBG_CONTROL, "lib1x_control_AssociationRsp(1, ret=%d)\n", result);
...
#if defined(__DAVO__)
			/* APNRTL-238 */
			global->theAuthenticator->acct_sm->tx_packets = pQuery_Stat->tx_only_data_packets;       // == transmited packets
			global->theAuthenticator->acct_sm->rx_packets = pQuery_Stat->rx_only_data_packets;       // == received packets
			if (pQuery_Stat->tx_only_data_bytes >= OVER_4G) {
				global->theAuthenticator->acct_sm->tx_bytes_h = pQuery_Stat->tx_only_data_bytes / OVER_4G;         // == transmited bytes
				global->theAuthenticator->acct_sm->tx_bytes = pQuery_Stat->tx_only_data_bytes % OVER_4G;         // == transmited bytes youngho DAVO
			} else {
				global->theAuthenticator->acct_sm->tx_bytes_h = 0;         // == transmited bytes
				global->theAuthenticator->acct_sm->tx_bytes = pQuery_Stat->tx_only_data_bytes;         // == transmited bytes
			}
			if (pQuery_Stat->rx_only_data_bytes >= OVER_4G) {
				global->theAuthenticator->acct_sm->rx_bytes_h = pQuery_Stat->rx_only_data_bytes / OVER_4G;         // == received bytes youngho DAVO
				global->theAuthenticator->acct_sm->rx_bytes = pQuery_Stat->rx_only_data_bytes % OVER_4G;         // == received bytes
			} else {
				global->theAuthenticator->acct_sm->rx_bytes_h = 0;         // == received bytes youngho DAVO
				global->theAuthenticator->acct_sm->rx_bytes = pQuery_Stat->rx_only_data_bytes;         // == received bytes
			}
#else
...
#if defined(__DAVO__)
int lib1x_control_ClearSTA(Global_Params * global)	// youngho DAVO
{
	int retVal = 0;
	struct iwreq wrq;
	DOT11_QUERY_STATS	Query_Stat;

	lib1x_message(MESS_DBG_CONTROL, "lib1x_control_ClearSTA(1)\n");

	strncpy(wrq.ifr_name, global->auth->GlobalTxRx->device_wlan0, IFNAMSIZ);
	wrq.u.data.pointer = (caddr_t)&Query_Stat;
	wrq.u.data.length = sizeof(DOT11_QUERY_STATS);

	Query_Stat.EventId = DOT11_EVENT_ACC_CLEAR_STATS;
	Query_Stat.IsMoreEvent = FALSE;
	//memcpy(&Query_Stat.MACAddr, global->theAuthenticator->supp_addr, MacAddrLen);
	memcpy(Query_Stat.MACAddr, global->theAuthenticator->supp_addr, MacAddrLen);
	...
	if(ioctl(global->auth->GlobalTxRx->fd_control, SIOCGIWIND, &wrq) < 0) {
		// If no wireless name : no wireless extensions
		retVal = -1;
		lib1x_message(MESS_DBG_CONTROL, "lib1x_control_ClearSTA(1) retun -1\n");
	} else {
		lib1x_message(MESS_DBG_CONTROL, "lib1x_control_ClearSTA(1) retun success\n");
		retVal = 0;
	}
...
int lib1x_control_redirect_setting(u_char status, Global_Params *global)
{
	int retVal = 0;
	struct iwreq wrq;
	DOT11_SETPORT	Set_Port;
	...
	Set_Port.EventId = DOT11_EVENT_SET_PORT_WEBRD;
