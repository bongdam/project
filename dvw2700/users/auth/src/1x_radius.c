...
void lib1x_rad_eapresp_supp( Auth_Pae * auth_pae, struct lib1x_packet * pkt)
{
	...
	int		framed_mtu, val;
#if defined(_ABOCOM) || defined(__DAVO__)
	u_long	ulOutput;
#endif
	char	*src;
	...
	if ( eaprr->type ==  LIB1X_EAP_RRIDENTITY )	/* we have to handle response / identity packets differently */
	{
...
#if defined(__DAVO__)
		memset(rinfo->username, 0, sizeof(rinfo->username));
#endif
...
#if defined(__DAVO__)	// DAVO youngho
		...
		lib1x_acct_MAC_TO_DASH_ASCII(auth_pae->supp_addr, MacAddrLen, etheraddr, &ulOutput);
		...
		lib1x_radconst_addattr( rconst, LIB1X_RAD_CONNECTINFO, strlen( rinfo->connectinfo), (u_char *)rinfo->connectinfo );
#else //#if defined(__DAVO__)
		...
		if ( !auth_pae->global->bMacAuthEnabled ) {
			...
			lib1x_radconst_addattr( rconst, LIB1X_RAD_CONNECTINFO, strlen((char *)rinfo->connectinfo), (u_char *)rinfo->connectinfo );
		}
	...
	else
	{
...
#if defined(__DAVO__)	// DAVO youngho
		lib1x_acct_MAC_TO_DASH_ASCII(auth_pae->supp_addr, MacAddrLen, etheraddr, &ulOutput);
		lib1x_radconst_addattr( rconst, LIB1X_RAD_CALLING_STID, ulOutput,  etheraddr );
#else
#ifndef _ABOCOM
		lib1x_print_etheraddr( (char *)etheraddr, auth_pae->supp_addr );
		lib1x_radconst_addattr( rconst, LIB1X_RAD_CALLING_STID, strlen((char *)etheraddr),  etheraddr );
		...
		lib1x_radconst_addattr( rconst, LIB1X_RAD_NAS_IDENTIFIER, strlen((char *)rinfo->nas_identifier), (u_char *)rinfo->nas_identifier );
		val = htonl( nas_port_type );
...
void lib1x_rad_special_type( Auth_Pae * auth_pae, u_long ulRequestType)
{
	...
	char		szPassword[20];	/* to store ether addresses */
	char		szOutput[64];
	u_long		ulOutput;
	...
	switch(ulRequestType)
	{
		case LIB1X_RAD_AUTH_MAC_AUTHENTICATION:
			...
			lib1x_print_etheraddr( (char *)szOutput, (u_char *)auth_pae->supp_addr );
			lib1x_radconst_addattr( rconst, LIB1X_RAD_USER_NAME, 2*MacAddrLen, (u_char*)szOutput);
#if defined(__DAVO__)
            /* 2011-07-05 12:01:31 young */
            rinfo->username_len = snprintf((char *)rinfo->username, sizeof(rinfo->username), "%s", szOutput);
#endif
			...
			lib1x_radconst_addattr( rconst, LIB1X_RAD_PASSWORD, 16,  (u_char *)szPassword );


			lib1x_acct_MAC_TO_DASH_ASCII(auth_pae->global->TxRx->oursvr_addr, MacAddrLen, (u_char*)szOutput, &ulOutput);
			lib1x_radconst_addattr( rconst, LIB1X_RAD_CALLED_STID, ulOutput, (u_char*)szOutput);

			lib1x_acct_MAC_TO_DASH_ASCII(auth_pae->supp_addr, MacAddrLen, (u_char *)szOutput, &ulOutput);
		...
		case LIB1X_RAD_ACCT_STATUS_ON:
			...
			lib1x_radconst_addattr( rconst, LIB1X_RAD_ACCT_SESSION_ID, ulOutput, (u_char *)szOutput);
...
)
{
	...
	lib1x_message(MESS_DBG_RAD, "STA[%d], Session Timeout : %d\n",global->index, global->auth->Supp[global->index]->SessionTimeoutCounter);
...
)
{
	...
	//atribute value is number of consecutive seconds
	pucValue = ( ( u_char *  )rattr ) + 2;
	lib1x_N2L(pucValue, global->akm_sm->TermAction);
	//global->akm_sm->SessionTimeout = 80;
	lib1x_message(MESS_DBG_RAD, "STA[%d], Term Action : %d\n",global->index, global->akm_sm->TermAction);
