...
#ifdef __DAVO__
int inet_fget(const char *path, char *dst, size_t len)
{
	FILE *f = NULL;
	char tmp[24], buf[24] = { [0] = '\0', };
	struct in_addr ip;
	...
	fscanf(f, "%23s", tmp);
	fclose(f);
	sscanf(tmp, "%23[0-9.]", buf);

	if (inet_pton(AF_INET, buf, &ip) > 0 && ip.s_addr && ip.s_addr != INADDR_NONE) {
