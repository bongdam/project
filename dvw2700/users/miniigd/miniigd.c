...
static void AddToChain(const char *if_name, const char *ip)
{
...
#endif	
	sprintf(tmp, "iptables -t nat -I PREROUTING -d %s -i %s -j MINIUPNPD", ip, if_name);
	system(tmp);	
#if defined(CONFIG_RTL8186_GR) || defined(CONFIG_RTL8186_GW)
	...
	sprintf(tmp, "iptables -t filter -I FORWARD -i %s -o ! %s -j MINIUPNPD", if_name, if_name);
	system(tmp);
#if defined(CONFIG_RTL8186_GR) || defined(CONFIG_RTL8186_GW)
	sprintf(tmpBuf,"echo -n \"%s,\" >> %s",tmp,UPNP_INIT);
	system(tmpBuf);
...
#ifdef __DAVO__
			&& inet_fget(ext_ip_from_file, ext_ip_addr_tmp, INET_ADDRSTRLEN)
#endif
			) {
