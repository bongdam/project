...
//Brad add sync system time for dhcpd 2008/01/28
static int getPid(const char *filename)
{
	FILE *f;
	int pid = 0;
...
static void Inform_DHCPD(void)
{
	int pid = getPid("/var/run/udhcpd.pid");
	if (pid > 1) {
		kill(pid, SIGUSR2);
		usleep(1000);
	}
...
//-----------------------------------
static int yecho(const char *path, const char *fmt, ...)
{
	int fd, n = -1;
	va_list ap;
