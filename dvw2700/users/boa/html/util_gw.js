...
function get_data_size_converter(data_h, data_l) {
  var unit = new Array("", "K", "M", "G", "T");
  var upperQuotient = parseInt(data_h);
  var upperRemainder;
  var bottomQuotient = parseInt(data_l);
  ...
  fraction = "000" + Math.floor(bottomRemainder * 100 / 1024);
  return "" + bottomQuotient + "." + fraction.substring(fraction.length - 2) + " " + unit[i] + "B";
}
  ...
  if (form.name == "wlanSetup") {
    ...
    if (mode.selectedIndex == 0 || mode.selectedIndex == 3) {
      if (hiddenSSIDEnabled.selectedIndex == 0) {
        if (!confirm("hiddenSSID를 활성화 하면 WPS2.0이 비활성화됩니다.")) {
          return false;
        }
      }
    }
...
/*
    var tx_restrict = form.elements["tx_restrict" + wlan_id];
    var rx_restrict = form.elements["rx_restrict" + wlan_id];

    if (!IsDigit(tx_restrict.value) || !IsDigit(rx_restrict.value)) {
    	alert('TX/RX 제한 값은 숫자만 입력 가능합니다!');
    	return false;
    }
*/
  }
  ...
  MaxConn = form.elements["wlan" + wlan_id + "_max_conn"];
  if (!IsDigit(MaxConn.value) || MaxConn.value < 1 || MaxConn.value > 127) {
    alert('동시접속 제한은 1~127까지 설정 가능합니다!');
    MaxConn.focus();
    return false;
  }
  /* END DAVO */
...
function wanShowDiv(pptp_bool, dns_bool, dnsMode_bool, pppoe_bool, static_bool, l2tp_bool, USB3G_bool) {
  ...
  if (pptp_bool == 0 && pppoe_bool == 0 && static_bool == 0 && dns_bool == 0 && l2tp_bool == 0 && USB3G_bool == 0) /*-- keith: add l2tp support. 20080515  */
    show_div(1, "dhcp_div");
  else
    show_div(0, "dhcp_div");
}
...
*/
  ...
  if (form == document.wlanSetup) {
    ...
    enableTextField(form.elements["wlan" + wlan_id + "_max_conn"]);
    //enableTextField(form.elements["tx_restrict" + wlan_id]);
    //enableTextField(form.elements["rx_restrict" + wlan_id]);
  }
}
...
function disableWLAN(form, wlan_id) {
  ...
  if (form == document.wlanSetup) {
    ...
    if (form.elements["multipleAP" + wlan_id] != null)
    	disableButton(form.elements["multipleAP" + wlan_id]);
    disableTextField(form.elements["wlan" + wlan_id + "_max_conn"]);
    //disableTextField(form.elements["tx_restrict" + wlan_id]);
    //disableTextField(form.elements["rx_restrict" + wlan_id]);
  }
  //disableCheckBox(form.elements["wlanMacClone"+wlan_id]);
...
function showChannel5G(form, wlan_id) {
  ...
  var dfs_enable = form.elements["dfs_enable"].value;
  var idx = 0;
...
function setDefaultKeyValue2(form, wlan_id, key_index) {
  if (form.elements["length" + wlan_id].selectedIndex == 0) {
    if (form.elements["format" + wlan_id].selectedIndex == 0) {
      form.elements["key" + key_index].maxLength = 5;
      form.elements["key" + key_index].value = "*****";
    } else {
      form.elements["key" + key_index].maxLength = 10;
      form.elements["key" + key_index].value = "**********";
    }
  } else {
    if (form.elements["format" + wlan_id].selectedIndex == 0) {
      form.elements["key" + key_index].maxLength = 13;
      form.elements["key" + key_index].value = "*************";
    } else {
      form.elements["key" + key_index].maxLength = 26;
      form.elements["key" + key_index].value = "**************************";
    }
  }
}
  ...
  var ac_mode = 0;
	...
	if ( wlan_id == 0 ) {
		...
		*/
		...
		form.elements["channelbound" + wlan_id].options[idx++] = new Option("20+40MHz", "7", false, false);
		form.elements["channelbound" + wlan_id].length = idx;
		if ( backup_chanbond >= 0)
...
function updateChan_selectedIndex(form, wlan_id) {
  ...
  if ((parseInt(wlan_id) == 0) && (parseInt(chan_number) >= 52 && parseInt(chan_number) <= 124)) {
    alert("선택채널로 변경시 DFS에 의해 채널이 변경될 수 있습니다.");
  }
  /*
    ...
    disableTextField(form.elements["controlsideband" + wlan_id]);
  	...
  	if ( wlan_id == 0 ) {
		  ...
		  enableTextField(form.elements["controlsideband" + wlan_id]);
  		  ...
  		  form.elements["controlsideband" + wlan_id].options[0] = new Option("Upper", "0", false, false);
		  form.elements["controlsideband" + wlan_id].options[1] = new Option("Lower", "1", false, false);
		  if (chan_number ==  36 || chan_number == 44 || chan_number == 52 ||
		  	chan_number == 60 || chan_number == 100 || chan_number == 108 ||
		  	chan_number == 116 || chan_number == 149 || chan_number == 157 ) {
		  	form.elements["controlsideband" + wlan_id].selectedIndex=1;
		  } else {
		  	form.elements["controlsideband" + wlan_id].selectedIndex=0;
		  }
	    }
  	} else {
  		if (form.elements["channelbound_manual" + wlan_id].selectedIndex == "0") {
	      disableTextField(form.elements["controlsideband" + wlan_id]);
	    } else {
		  if (chan_number >= 5 && chan_number <= 9) {
		     enableTextField(form.elements["controlsideband" + wlan_id]);
		  } else {
		     if ( chan_number < 5 ) {	//Lower
		     	form.elements["controlsideband" + wlan_id].selectedIndex=1;
		    	} else {	//Upper
		    		form.elements["controlsideband" + wlan_id].selectedIndex=0;
		    	}
  ...
  //ac2g
  if (wlan_id == 1 && (((wlan_support_8812e == 1) && (chan_number > 14)) || band_value == 74)) { //8812
    disableTextField(form.elements["controlsideband" + wlan_id]);
  }

  showchannelbound_updated(form, band_value, wlan_id, -1, chan_number);
  */
}
...
if (top.frames["menufrm"] && top.frames["basefrm"])
        top.frames["menufrm"].change_memu(top.frames["basefrm"].location.href);
document.oncontextmenu=new Function("return false")
}

function ip_range_check(start, end, cur)
{
	start_d = inet_aton(start);
	cur_d = inet_aton(cur);
	end_d = inet_aton(end);

	if ( (start_d <= cur_d) && (cur_d<=end_d) ) {
		return true;
	} else {
		return false;
	}
...
function ip_class_check(ip_addr)
{
	if (ip_range_check("1.0.0.1","126.255.255.254",ip_addr)) { // Class A
		return true;
	} else if (ip_range_check("128.0.0.1","191.255.255.254",ip_addr)) { //Class B
		return true;
	} else if (ip_range_check("192.0.0.1","223.255.255.254",ip_addr)) { //Class C
		return true;
	}
	return false;
}
