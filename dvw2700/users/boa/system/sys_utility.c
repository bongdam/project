...
static int nofork_fwrite(int argc, char *argv[], const char *path)
{
	char **p;
	int len = 0;
...
static int nofork_kill(int argc, char *argv[])
{
	char *p;
	int sig, pid;
	...
	sig = strtol(argv[1], &p, 10);
	if (sig < 0)
		sig = 0 - sig;

	if (sig >= _NSIG || *p || p == argv[1])
		return -3;
	if (argv[0][sizeof("kill") - 1] == '\0') {
		pid = strtol(argv[2], &p, 0);
		if (*p || p == argv[2])
			return -4;

		return kill(pid, sig);
	} else
		return killall(sig, argv[2]);
}
#endif
...
int RunSystemCmd(char *filepath, ...)
{
	...
	else if ((argno + 1) < ARRAY_SIZE(argv)) {
		argv[argno + 1] = NULL;

		switch (argv[0][0]) {
#ifdef __CONFIG_LIB_YTOOL__
		case 'e':
			if (filepath && !strcmp(argv[0], "echo"))
				return nofork_fwrite(argno, argv, filepath) > 0 ? 0 : -1;
			break;
#endif
#ifdef __CONFIG_LIB_SHARED__
		case 'k':
			if (!strcmp(argv[0], "kill") ||
			    !strcmp(argv[0], "killall")) {
				if ((rc = nofork_kill(argno, argv)) > -2)
					return rc;
			}
			break;
#endif
...
void Create_script(char *script_path, char *iface, int network, char *ipaddr, char *mask, char *gateway)
{
	...
	fd = open(script_path, O_RDWR | O_CREAT | O_TRUNC, S_IRWXO | S_IRWXG);
	...
	if (network == LAN_NETWORK) {
		...
		fprintf(fp, "while route del default dev %s\n", iface);
		fprintf(fp, "%s\n", "do :");
		fprintf(fp, "%s\n", "done");
		fprintf(fp, "route add -net default gw %s dev %s\n", gateway, iface);
		fprintf(fp, "%s\n", "init.sh ap wlan_app");
	}
	if (network == WAN_NETWORK) {
#ifdef __CONFIG_APP_LABORER__
		fprintf(fp, "preq gwka -- -q\n");
#endif
#ifdef CONFIG_NF_NAT_TWINIP
		fprintf(fp, "if [ -f /var/ntwk_sdmz ] ; then\n"
		            "\tsysconf sdmz deconfig %s\n"
		            "fi\n", iface);
#endif
		fprintf(fp, "ifconfig %s 0.0.0.0\n", iface);
		fprintf(fp, "echo 0.0.0.0 > /var/wan_ip\n");
		fprintf(fp, "read gw < /var/gateway\n");		
		fprintf(fp, "if [ \"${gw:-0.0.0.0}\" != \"0.0.0.0\" ]; then\n");
		fprintf(fp, "\troute del default gw ${gw}\nfi\n");
		fprintf(fp, "echo 0.0.0.0 > /var/gateway\n");
	}
	fclose(fp);
