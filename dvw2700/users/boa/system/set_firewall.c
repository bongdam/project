...
int run_ipt_restore(char *pathname, char *opt, int try);
...
static int flt_fd = -1;
static int nat_fd = -1;
static int flt_chain_fd = -1;

static void ipt_chain_policy(int opmode, unsigned int drop_mask)
{
	ftruncate(flt_chain_fd, 0);
	lseek(flt_chain_fd, 0, SEEK_SET);
	dprintf(flt_chain_fd,
	        "*filter\n"
	        ":INPUT %s [0:0]\n"
	        ":FORWARD %s [0:0]\n"
	        ":OUTPUT %s [0:0]\n"
	        ...
	        ":PKTFILTER - [0:0]\n" /* APACRTL-159 */
	        ":ACL - [0:0]\n%s%s",
	        (drop_mask & IPT_INPUT) ? "DROP" : "ACCEPT",
	        (drop_mask & IPT_FORWARD) ? "DROP" : "ACCEPT",
	        (drop_mask & IPT_OUTPUT) ? "DROP" : "ACCEPT",
	        (opmode == GATEWAY_MODE && nvram_atoi("UPNP_ENABLED", 0))
	        ? ":MINIUPNPD - [0:0]\n" : "",
	        (opmode == GATEWAY_MODE)
	        ? ":TWEB - [0:0]\n" : ""	// APACRTL-154
	       );
...
static ssize_t fconcatenate(int dst, int src)
{
	char buf[128];
	ssize_t n, count, bytes;
...
{
#ifdef __CONFIG_APP_DVQOS__
	if (!nvram_atoi("x_DV_QOS_DISABLE", 0))
		yexecl(NULL, "dvqos --apply");
#else	/* __CONFIG_APP_DVQOS__ */
#ifdef   HOME_GATEWAY
...
//	apmib_get(MIB_SUBNET_MASK, (void *)&mask);
	...
	if (enabled & 1) {
		if (portscan) {
			...
			add_fcommand("/var/run/ipt_dos_drop", 0, NULL, tmp);
			dprintf(flt_fd, "-I %s\n", tmp);
			snprintf(tmp, sizeof(tmp), "INPUT %s-p %s -m state --state NEW "
			         "-m recent --update --seconds 5 --hitcount %d --blockoff %d -j DROP",
		...
		if (synflood && rate > 0) {
			...
			add_fcommand("/var/run/ipt_dos_drop", 0, NULL, tmp);
			dprintf(flt_fd, "-I %s\n", tmp);
			add_fcommand("/var/run/ipt_dos_drop", 0, NULL, "INPUT -p tcp --tcp-flags SYN,FIN,ACK SYN -m recent --set");
			dprintf(flt_fd, "-I INPUT -p tcp --tcp-flags SYN,FIN,ACK SYN -m recent --set\n");
		}
		...
		if (nvram_atoi("x_noreply_tracert", 0)) {
			add_fcommand("/var/run/ipt_dos_drop", 0, NULL, "INPUT -p udp -m ttl --ttl-lt 31 -j DROP");
			add_fcommand("/var/run/ipt_dos_drop", 0, NULL, "INPUT -p icmp -m ttl --ttl-lt 31 -j DROP");
			add_fcommand("/var/run/ipt_dos_drop", 0, NULL, "FORWARD -p icmp --icmp-type 11 -j DROP");
			add_fcommand("/var/run/ipt_dos_drop", 0, NULL, "OUTPUT -p icmp --icmp-type 11 -j DROP");
			dprintf(flt_fd, "-I INPUT -p udp -m ttl --ttl-lt 31 -j DROP\n");
			dprintf(flt_fd, "-I INPUT -p icmp -m ttl --ttl-lt 31 -j DROP\n");
			dprintf(flt_fd, "-I FORWARD -p icmp --icmp-type 11 -j DROP\n");
			dprintf(flt_fd, "-I OUTPUT -p icmp --icmp-type 11 -j DROP\n");
		}
#if 0
...
int setIpFilter(void)
{
	...
	const char **proto;
#ifdef __DAVO__
	unsigned long lan_addr, lan_mask, ipft_addr;
	int delete_event = 0, delete_entry = 0;
#endif
	...
	for (index = 1; index <= entryNum ; index++) {
		...
		*((char *)&entry) = (char)index;
		...
		tmpStr = inet_ntoa(to_in_addr(entry.ipAddr));
		sprintf(ipAddr, "%s", tmpStr);
#ifdef __DAVO__
		ipft_addr = inet_addr(ipAddr);
		if ( (ipft_addr & lan_mask) != (lan_addr & lan_mask) ) {
			printf("delete ipft_addr wrong ip(%s) rule.\n", ipAddr);
			delete_entry |= (1 << (index - 1));
			delete_event = 1;
			continue;
		}
#endif
		...
		if (entry.ipVer == IPv4) {
#endif
			for (proto = proto_list(entry.protoType); *proto; proto++) {
				dprintf(flt_fd, "-A FORWARD -p %s -s %s -j DROP\n", *proto, ipAddr);
				if (!strcmp(*proto, "udp")) {
					dprintf(flt_fd, "-A INPUT -p udp --dport 53:53 -s %s -j DROP\n", ipAddr);
				}
			}
#ifdef CONFIG_IPV6
...
#ifdef __DAVO__
	if (delete_event) {
		for ( index = entryNum; index > 0 ; index--) {
			if ( delete_entry & (1 << (index - 1)) ) {
				*((char *)&entry) = (char)index;
				apmib_get(MIB_IPFILTER_TBL, (void *)&entry);
				apmib_set(MIB_IPFILTER_DEL, (void *)&entry);
			}
		}
...
{
#if defined(__DAVO__)
	int tbl_num = 0, lan_port, p_port, LAN = 0, i;
	char opmode[4][32], *p_opmode;
	char buf[128], tmp[128];
	char *args[5], *p;
	int active_port = 0;
	/* 0:wan, 1:LAN1, 2:LAN2 3:LAN3 4:LAN4 */
	int port_info[PRTNR_MAX] = {PRTNR_WAN0, PRTNR_LAN1, PRTNR_LAN2, PRTNR_LAN3, PRTNR_LAN4};

	apmib_get(MIB_MACFILTER_TBL_NUM, (void *)&tbl_num);
	/*	if (tbl_num == 0)
			return 0;
	*/

	for (i = 0; i < 4; i++) {
		snprintf(buf, sizeof(buf), "x_MACFILTER_OPMODE%d", i + 1);
		memset(opmode[i], 0, sizeof(opmode[i]));
		nvram_get_r(buf, opmode[i], sizeof(opmode[i]));
		if (strcmp(opmode[i], "drop") && strcmp(opmode[i], "permit")) {
			snprintf(opmode[i], sizeof(opmode[i]), "%s", "drop");
			nvram_set(buf, "drop");
		}
	}
	...
	for (i = 0; i < 4; i++) {
		if (strcmp(opmode[i], "permit") == 0) {
			snprintf(buf, sizeof(buf), "aclwrite del br0 -a drop -r sfilter -o 7 -3 -4 -P %d", 0x1 << port_info[i + 1]);
			yexecl(NULL, buf);
		}
	}
		...
		if (ystrargs(p, args, 5, ",", 1) >= 2) {
			...
			if (LAN != 0) {
				yexecl(NULL, "aclwrite add br0 -a %s -r sfilter -o 7 -m %s -P %d -3 -4",
				       p_opmode, args[0], p_port);
			}
		}
	}
	for (i = 0; i < 4; i++) {
		//if ((active_port & (1<<i)) && (strcmp(opmode[i], "permit") == 0)) {
		if (strcmp(opmode[i], "permit") == 0) {
			snprintf(buf, sizeof(buf), "aclwrite add br0 -a drop -r sfilter -o 7 -3 -4 -P %d", 0x1 << port_info[i + 1]);
			yexecl(NULL, buf);
		}
	}
...
int setPortFilter(void)
{
	...
	//apmib_get(MIB_IPFILTER_TBL_NUM, (void *)&entryNum);
	entryNum = nvram_atoi("PORTFILTER_TBL_NUM", 0);
	...
	for (index = 1; index <= entryNum; index++) {
		...
		for (; *proto; proto++) {
			if (!drop_dns && !strcmp(*proto, "udp") && atoi(fromPort) <= 53 && atoi(toPort) >= 53) {
				dprintf(flt_fd, "-A FORWARD -p udp --dport 53:53 -j DROP\n");
				drop_dns = 1;
			}
			dprintf(flt_fd, "-A FORWARD -p %s --dport %s -j DROP\n", *proto, PortRange);
		}
	}
#else // defined(__DAVO__)
	char PortRange[30];
	//int DNS_Filter=0;
	int entryNum = 0, index;
	PORTFILTER_T entry;
...
int setPortForward(char *pIfaceWan, char *pIpaddrWan)
{
...
#endif
	char rmtip[30];
	char extPortRange[60];
	char tmpbuff[120];
	const char **proto;
	struct in_addr tmp_inaddr;
	unsigned long *pl;
#ifdef __DAVO__
	unsigned long lan_addr, lan_mask, portfw_addr;
	int delete_event = 0, delete_entry = 0;
#endif
	...
	for (index = 1; index <= entryNum; index++) {
		...
		*((char *)&entry) = (char)index;
		...
		pl = (unsigned long *)entry.ipAddr;
		tmp_inaddr.s_addr = *pl;
		tmpStr =	inet_ntoa(tmp_inaddr);
		snprintf(ip, sizeof(ip), "%s", tmpStr);

#ifdef __DAVO__
		portfw_addr = inet_addr(ip);
		if ( (portfw_addr & lan_mask) != (lan_addr & lan_mask) ) {
			printf("delete portfw wrong ip(%s) rule.\n", ip);
			delete_entry |= (1 << (index - 1));
			delete_event = 1;
			continue;
		}
		...
		pl = (unsigned long *)entry.rmtipAddr;
		tmp_inaddr.s_addr = *pl;
		tmpStr =	inet_ntoa(tmp_inaddr);

		if (strcmp(tmpStr, "0.0.0.0")) {
			snprintf(rmtip, sizeof(rmtip), "-s %s", tmpStr);
		} else {
			snprintf(rmtip, sizeof(rmtip), "%s", "");
		}

		if (entry.externelFromPort == entry.externelToPort) {
			snprintf(extPortRange, sizeof(extPortRange), "%d", entry.externelFromPort);
		} else {
			snprintf(extPortRange, sizeof(extPortRange), "%d:%d", entry.externelFromPort, entry.externelToPort);
		}

		if (entry.fromPort < 80 && entry.toPort > 80)
			dprintf(flt_fd, "-D INPUT -p tcp --dport 80:80 -i %s -d %s -j DROP\n", pIfaceWan, pIpaddrWan);

		for (proto = proto_list(entry.protoType); *proto; proto++) {
			dprintf(nat_fd, "-A PREROUTING %s -p %s --dport %s -d %s -j DNAT --to %s:%s\n",
			        rmtip, *proto, extPortRange, pIpaddrWan, ip, PortRange);
			dprintf(flt_fd, " -A FORWARD -i %s -d %s -p %s --dport %s -j ACCEPT\n", pIfaceWan, ip, *proto, PortRange);
		}
...
#if defined(CONFIG_RTL_SUPPORT_ACCESS_PORT_FORWARD_FROM_LAN)
					dprintf(nat_fd, "-A POSTROUTING -p 47 -s %s -d %s -j SNAT --to %s\n", Br0NetSectAddr, ip, pIpaddrWan);
#endif
					dprintf(nat_fd, "-A PREROUTING -i %s -p gre -d %s -j DNAT --to %s\n", pIfaceWan, pIpaddrWan, ip);
					dprintf(flt_fd, "-A FORWARD -p gre -i %s -j ACCEPT\n", pIfaceWan);
				}
			}
		...
		if (ipsec_vpn == 0) {
			if (entry.fromPort <= 500 &&  entry.toPort >= 500) {
				if (entry.protoType == PROTO_BOTH || (entry.protoType == PROTO_UDP)) {
#if defined(CONFIG_RTL_SUPPORT_ACCESS_PORT_FORWARD_FROM_LAN)
					dprintf(nat_fd, "-A POSTROUTING -p esp -s %s -d %s -j SNAT --to %s\n", Br0NetSectAddr, ip, pIpaddrWan);
					dprintf(nat_fd, "-A POSTROUTING -p udp -s %s -d %s --dport 4500 -j SNAT --to %s\n", Br0NetSectAddr, ip, pIpaddrWan);
#endif
					dprintf(nat_fd, "-A PREROUTING -p esp -d %s -j DNAT --to %s\n", pIpaddrWan, ip);
					dprintf(nat_fd, "-A PREROUTING -p udp --dport 4500 -d %s -j DNAT --to %s\n", pIpaddrWan, ip);
					dprintf(flt_fd, "-A FORWARD -p udp --dport 4500 -j ACCEPT\n");
					dprintf(nat_fd, "-A FORWARD -p esp -i %s -j ACCEPT\n", pIfaceWan);
				}
			}
			ipsec_vpn = 1;
		}
...
void setRulesWithOutDevice(int opmode, int wan_dhcp, char* pInterface_wanPhy, char* Interface_wanPhy)
{
	...
	int my_wan_type = 0;
	unsigned long	dos_enabled = 0;
	unsigned char drop_mask = 0;
	char wan_type[8];
#if 0 //def CONFIG_RTL_HW_NAPT
	int ivalue = 0;
#endif
...
#else
	if (opmode != BRIDGE_MODE) {
		if (nvram_atoi("x_input_policy_accept", 0))
			drop_mask &= ~IPT_INPUT;
		else
			drop_mask |= IPT_INPUT;
	} else
...
#else
			drop_mask &= ~(IPT_INPUT | IPT_FORWARD);
			ipt_chain_policy(opmode, drop_mask);
#endif
			return;
		}
	}
	...
	ipt_chain_policy(opmode, drop_mask);
	intVal = nvram_atoi("MACFILTER_ENABLED", 0);

	intVal_num = nvram_atoi("MACFILTER_TBL_NUM", 0);
...
//		RunSystemCmd("/proc/url_filter", "echo", "0", NULL_STR);//disable url filter
		...
		RunSystemCmd("/proc/filter_table", "echo", "flush", NULL_STR);
#endif
#if 0 //defined(CONFIG_RTL_HW_NAPT)
		if (opmode == 0) {
			apmib_get(MIB_SUBNET_MASK, (void*)&ivalue);
			if ((ivalue & HW_NAT_LIMIT_NETMASK) != HW_NAT_LIMIT_NETMASK) {
				RunSystemCmd("/proc/hw_nat", "echo", "0", NULL_STR);
	...
	if (intVal == 0) {
		dprintf(flt_fd, "-A FORWARD -p udp --dport 1701 -j DROP\n");
	} else if (intVal == 1) {
		dprintf(flt_fd, "-A FORWARD -p udp --sport 1701 -j ACCEPT\n");
		dprintf(flt_fd, "-A FORWARD -p udp --dport 1701 -j ACCEPT\n");
	}
	apmib_get(MIB_VPN_PASSTHRU_PPTP_ENABLED, (void *)&intVal2);
	if (intVal2 == 0) {
		dprintf(flt_fd, "-A FORWARD -p tcp --dport 1723 -j DROP\n");
	} else if (intVal2 == 1) {
		dprintf(flt_fd, "-A FORWARD -p tcp --dport 1723 -j ACCEPT\n");
		dprintf(flt_fd, "-A FORWARD -p tcp --sport 1723 -j ACCEPT\n");
		dprintf(flt_fd, "-A FORWARD -p 47 -j ACCEPT\n");
	}

	intVal = 0;
	apmib_get(MIB_VPN_PASSTHRU_IPSEC_ENABLED, (void *)&intVal);
	if (intVal == 0) {
		dprintf(flt_fd, "-A FORWARD -p udp --dport 500 -j DROP\n");
		/*When ipsec enable->disable, ipsec may not send udp port 500 packet,
		so drop udp port 4500 too when ipsec passthru disabled.*/
		dprintf(flt_fd, "-A FORWARD -p udp --dport 4500 -j DROP\n");
	}
	...
	*/
	///////////////////////////////////////////////////////////
	dprintf(flt_fd, "-A FORWARD -p udp -m udp -i %s --destination 224.0.0.0/4 -j ACCEPT\n", Interface_wanPhy);
	dprintf(flt_fd, "-A FORWARD -p 114 -i %s --destination 224.0.0.0/4 -j ACCEPT\n", Interface_wanPhy);
	///////////////////////////////////////////////////////////
	dprintf(flt_fd, "-A INPUT -m state --state RELATED,ESTABLISHED -j ACCEPT\n");
	//iptables -I FORWARD -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
#if defined(CONFIG_REFINE_BR_FW_RULE)
	if (br_rule_refine) {
		dprintf(flt_fd, "-I FORWARD -i br0 -j ACCEPT\n");
		dprintf(flt_fd, "-I FORWARD -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu\n");
		///////////////////////////////////////////////////////////
		dprintf(flt_fd, "-A FORWARD -m state --state INVALID -j DROP\n");
	} else
#endif
	{
		dprintf(flt_fd, "-I FORWARD -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu\n");
		///////////////////////////////////////////////////////////
		dprintf(flt_fd, "-A FORWARD -m state --state INVALID -j DROP\n");
		dprintf(flt_fd, "-A FORWARD -i br0 -j ACCEPT\n");
	}
	///////////////////////////////////////////////////////////
	if (wan_dhcp == 4) {
		dprintf(flt_fd, "-A FORWARD -i %s -m state --state RELATED,ESTABLISHED -j ACCEPT\n", pInterface_wanPhy);
	}
	///////////////////////////////////////////////////////////
	...
	if (wan_dhcp == PPTP) {
		dprintf(nat_fd, "-A POSTROUTING -o %s -j MASQUERADE\n", pInterface_wanPhy);
	}
...
int setFirewallIptablesRules(int argc, char** argv)
{
	...
	char ipaddr_str[16];
#endif
#if 0 //defined(CONFIG_RTL_WEB_WAN_ACCESS_PORT)
	int web_access_port = 0;
	unsigned char port_buff[16] = {'\0'};
	unsigned char ip_buff[64] = {'\0'};
#endif
	char fltnam[] = "/var/XXXXXX";
	char natnam[] = "/var/XXXXXX";
	char fltchainnam[] = "/var/XXXXXX";
	char lan_nip[30];
	char lan_nmask[30];
	int sdmz = 0;
...
wait_lable:
	...
	nvram_get_r("SUBNET_MASK", lan_nmask, sizeof(lan_nmask));
	...
	flt_chain_fd = mkstemp(fltchainnam);
	if (flt_chain_fd < 0)
		return -1;
	flt_fd = mkstemp(fltnam);
	if (flt_fd < 0) {
		safe_close(&flt_chain_fd);
		return -1;
	}
	nat_fd = mkstemp(natnam);
	if (nat_fd < 0) {
		safe_close(&flt_chain_fd);
		safe_close(&flt_fd);
		return -1;
	...
	ipt_chain_policy(opmode, 0);
	acl_init_rule(pInterface, opmode, flt_fd, nat_fd);

	get_wanip = getInAddr(pInterface, IP_ADDR_T, (void *)&wanaddr);
	if (get_wanip == 0) { //get wan ip fail

		dprintf(flt_fd, "-A TELNMS -j DROP\n");     // TELNMS default drop
		dprintf(flt_fd, "COMMIT\n");
		dprintf(nat_fd, "COMMIT\n");

		fconcatenate(nat_fd, flt_chain_fd);
		fconcatenate(nat_fd, flt_fd);
		safe_close(&flt_chain_fd);
		safe_close(&flt_fd);
		safe_close(&nat_fd);
		intVal = run_ipt_restore(natnam, NULL, 10);
		if (intVal)
			fprintf(stderr, "iptables-restore failed with %d code\n", intVal);
...
#ifdef __DAVO__
	snmp_make_rules(pInterface, opmode, flt_fd, nat_fd);
	telnet_make_rules(pInterface, opmode, flt_fd, nat_fd);
	web_remote_access(pInterface, opmode, flt_fd, nat_fd);
	if (opmode == BRIDGE_MODE) {
		goto out;
	}
#endif
	...
	//run_ipt_restore
	dprintf(flt_fd, "-A TELNMS -j DROP\n");		// TELNMS default drop
	dprintf(flt_fd, "-A INPUT -p udp --dport 1900:1900 -i %s -d %s -j DROP\n", pInterface, WanIpAddr);

#ifdef _ALPHA_DUAL_WAN_SUPPORT_
	if (ipaddr.s_addr > 0) {
		dprintf(flt_fd, "-A INPUT -p tcp --dport ! 80:80 -i %s -d %s -j ACCEPT\n", Interface_wanPhy, ipaddr_str);
		dprintf(flt_fd, "-A INPUT -p udp --dport ! 1900:1900 -i %s -d %s -j ACCEPT\n", Interface_wanPhy, ipaddr_str);
	}
#endif
	...
	if (!apmib_get(MIB_IPV6_ORIGIN_TYPE, &val)) {
		...
		if (val == IPV6_ORIGIN_6RD)
			dprintf(flt_fd, "-A INPUT -i %s -p 41 -j ACCEPT\n", pInterface);
	...
	if (wan_type == PPTP) {
		dprintf(flt_fd, "-A INPUT -p 47 -i %s -j ACCEPT\n", pInterface_wanPhy);
		dprintf(flt_fd, "-A INPUT -p tcp --sport 1723 -i %s -j ACCEPT\n", pInterface_wanPhy);
	} else if (wan_type == L2TP) {
		dprintf(flt_fd, "-A INPUT -p udp --sport 1701 -i %s -j ACCEPT\n", pInterface_wanPhy);
	}
	apmib_get(MIB_VPN_PASSTHRU_IPSEC_ENABLED, (void *)&intVal);
	if (intVal == 0) {
#ifdef MULTI_PPPOE
		...
		dprintf(flt_fd, "-A FORWARD -p udp --dport 500 -i %s -o br0 -j ACCEPT\n", pInterface);
	}

	if ((wan_dhcp == DHCP_CLIENT && !sdmz_configured(NULL, 0)) || wan_dhcp == PPTP || wan_dhcp == L2TP) {
		dprintf(flt_fd, "-A INPUT -p udp --dport 68 -i %s -j ACCEPT\n", pInterface);
	}
	//add for DMZ
	get_br0ip = getInAddr(bInterface, IP_ADDR_T, (void *)&br0addr);
	if (get_br0ip == 0) {
	...
	if (intVal == 1) {
		...
		strIp = inet_ntoa(to_in_addr((unsigned char *)IpAddrBuf));
		if (strcmp(strIp, "0.0.0.0")) {
			strcpy(IpAddr, strIp);
			dprintf(nat_fd, "-A PREROUTING -p ALL -d %s -j DNAT --to %s\n", WanIpAddr, IpAddr);
			dprintf(flt_fd, "-D INPUT -p tcp --dport 80:80 -i %s -d %s -j DROP\n", pInterface, WanIpAddr);
			dprintf(flt_fd, "-A FORWARD -i %s -d %s -p ALL -j ACCEPT\n", pInterface, IpAddr);
			//add for DMZ
			dprintf(nat_fd, "-A POSTROUTING -s %s -d %s -j ACCEPT\n", LanIpAddr, IpAddr);
			dprintf(nat_fd, "-A POSTROUTING -s %s -d %s -j SNAT --to %s\n", Br0NetSectAddr, IpAddr, WanIpAddr);

			/*jwj: To avoid pakcets whose dip is lan ip forward to lan pc.*/
			dprintf(flt_fd, "-A PREROUTING -i %s -d %s -j DROP\n", pInterface, Br0NetSectAddr);
		}
	}
	...
	if (intVal == 1) {
		dprintf(flt_fd, "-A INPUT -p icmp --icmp-type echo-request -i %s -d %s -j ACCEPT\n", pInterface, WanIpAddr);
#ifdef _ALPHA_DUAL_WAN_SUPPORT_
		if (ipaddr.s_addr > 0)
			dprintf(flt_fd, "-A INPUT -p icmp --icmp-type echo-request -i %s -d %s -j ACCEPT\n", Interface_wanPhy, ipaddr_str);
#endif
	} else {
		dprintf(flt_fd, "-A INPUT -p icmp --icmp-type echo-request -i %s -d %s -j DROP\n", pInterface, WanIpAddr);
	}
	...
	if (intVal == 0) {
#ifdef SUPPORT_ZIONCOM_RUSSIA
		dprintf(flt_fd, "-A INPUT -p 2 -i %s -j ACCEPT\n", Interface_wanPhy);
		dprintf(flt_fd, "-A FORWARD -i %s -j ACCEPT\n", Interface_wanPhy);
#else
		dprintf(flt_fd, "-A INPUT -p 2 -i %s -j ACCEPT\n", pInterface);
#endif
		if (!sdmz) {
			dprintf(flt_fd, "-I OUTPUT -p 2 -o %s --source %s -j DROP\n", pInterface, lan_nip);
		}
		dprintf(flt_fd, "-A FORWARD -p udp -m udp -i %s --destination 224.0.0.0/4 -j ACCEPT\n", pInterface);
...
//modify
	dprintf(flt_fd, "-A INPUT -i br0 -j ACCEPT\n");
	dprintf(flt_fd, "-A INPUT -i eth0 -j ACCEPT\n");
	dprintf(flt_fd, "-A INPUT -i lo -j ACCEPT\n");
//	RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, in, NOT, pInterface, jump, ACCEPT, NULL_STR);
	...
	if (br_num > 0 && br_num <= MAX_VLAN_CONFIG_NUM) {
		for (br_idx = 1; br_idx < br_num; br_idx++) {
			...
			dprintf(flt_fd, "-A INPUT -i %s -j ACCEPT\n", br_iface);
		}
	}
#endif
	...
	if (intVal != 0) {
		dprintf(flt_fd, "-A INPUT -i br1 -j ACCEPT\n");
	}
#endif

	dprintf(flt_fd, "-A FORWARD -p 50 -i %s -o br0 -j ACCEPT\n", pInterface);
	dprintf(flt_fd, "-A FORWARD -i %s -m state --state RELATED,ESTABLISHED -j ACCEPT\n", pInterface);
	...
	if (dyn_rt_support == 1) {
		...
		if (natEnabled == 1 && intVal == 1) {
			if (intVal1 == 1) {
				dprintf(flt_fd, "-A INPUT -i %s -p udp --dport 520 -j ACCEPT\n", pInterface);
			}
		}
	}
	...
	if (opmode != BRIDGE_MODE) {
		//dprintf(nat_fd, "-A POSTROUTING -o %s -j MASQUERADE\n", pInterface);
		dprintf(nat_fd, "-A POSTROUTING -o %s -s %s/%s -j MASQUERADE\n", pInterface, lan_nip, lan_nmask);
	}
...
EXIT_setFirewallIptablesRules:
	yecho("/proc/sys/net/ipv4/ip_forward", "%d\n", (opmode == GATEWAY_MODE)? 1 : 0);
	return 0;
...
static int comment_out(const char *pathname, int line)
{
	FILE *f;
	int i, c;
...
int run_ipt_restore(char *pathname, char *opt, int try)
{
	const char default_path[] = "/bin:/usr/sbin:/usr/bin:/sbin";
	int i, pid, fdpair[2];
	int line, status = -1;
	char *p;
	char errmsg[128];
	char *const argv[] = { "iptables-restore", pathname, opt, NULL };
	int tmpenv = -1;
	char *pth;

	/* safe multiprocess in multithreaded programming */
	pth = getenv("PATH");
	if (!pth || (strcmp(default_path, pth) && ({ pth = strcpy(alloca(strlen(pth + 1)), pth); 1; })))
		tmpenv = setenv("PATH", default_path, 1);
	...
	do {
		...
		default:
			...
			if (status >= 1) {
				if ((i = read(fdpair[0], errmsg, sizeof(errmsg) - 1)) > 0) {
					errmsg[i] = '\0';
					if ((p = strstr(errmsg, "line")) != NULL)
						line = strtol(&p[5], NULL, 10);
				}
			}
			break;
		}
		...
		if (fdpair[1] > -1)
			close(fdpair[1]);
	} while (--try > 0 && line > 0 && !comment_out(pathname, line - 1));

	if (!tmpenv) {
		if (pth)
			setenv("PATH", pth, 1);	/* replace with original */
		else
			unsetenv("PATH");
	}
	return status;
