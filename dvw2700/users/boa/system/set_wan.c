...
#endif
#ifndef __DAVO__//AVOID_CONFLICTION_IP
...
static int permit_dns_server(char *dns_server)
{
	char *white_dns[5] = {"61.41.153.2", "1.214.68.2", "180.182.54.1", "180.182.54.2", "210.220.163.82"};
	int permit = 0;
	int i;
	int list = sizeof(white_dns) / sizeof(white_dns[0]);

	ydespaces(dns_server);

	for (i = 0; i < list; i++) {
		if (strcmp(white_dns[i], dns_server) == 0) {
			permit = 1;
			break;
		}
	}
...
#ifdef __DAVO__
		if (dns_count == 0 || block_dns) {	/* SERVER DNS NONE CONFIG */
			snprintf(dns_server[0], sizeof(dns_server[0]), "180.182.54.1");
			snprintf(dns_server[1], sizeof(dns_server[1]), "210.220.163.82");
		} else if (dns_count == 1) {	/* SERVER DNS CONFIG ONLY 1 */
			snprintf(dns_server[1], sizeof(dns_server[1]), "210.220.163.82");
		}
#endif
...
//#ifdef TR181_V6_SUPPORT
	...
	if (op_mode == GATEWAY_MODE)
		start_upnp_igd(DHCP_CLIENT, op_mode, -1, "br0");
#else
	int tmp_wisp_id;
...
//#ifdef TR181_V6_SUPPORT
...
#if defined(__DAVO__)
		if (op_mode == 0 && !sdmz) {
			/* APACRTL-309 workaround */
			if (killall(SIGTERM, "udhcpd") > 0) {
				sleep(1);
			}
			n = 0;
			apmib_get(MIB_DHCP, (void *)&n);
			if (n == DHCP_LAN_SERVER)
				start_lan_dhcpd("br0");
		}
#endif // defined(__DAVO__)
	...
	if (sdmz) {
		yecho(PRIVATE_SYSFS_DIR "twin_hwaddr", "%s\n", sdmzHost);
		enable_sdmz(interface);
		/* APACRTL-309 workaround */
		if (killall(SIGTERM, "udhcpd") > 0) {
			sleep(1);
		}
...
//			entry[1].type = 0; //Unknown
	...
	if(intValue == 1) {
		...
		snprintf(tmp1, sizeof(tmp1), "%s", strtmp);
	...
	if(intValue==1){
		...
		else
		{
			...
			if (nvram_atoi("x_sdmz_enable", 0) == 1)
				RunSystemCmd(NULL_FILE, "miniigd", "-e", tmp1, "-i", lan_interface, "-x", "/var/wan_ip", NULL_STR);
			else
				RunSystemCmd(NULL_FILE, "miniigd", "-e", tmp1, "-i", lan_interface,NULL_STR);
#else
			RunSystemCmd(NULL_FILE, "miniigd", "-e", tmp1, "-i", lan_interface,NULL_STR);
...
unsigned char *gettoken(char *str, unsigned int index, unsigned char symbol)
{
	static char tmp[50];
	unsigned char tk[50]; //save symbol index
	int i, j=0, cnt=1, start, end;
...
void start_ntp(void)
{
	...
	if (ntp_onoff == 1) {
		...
		buffer[0] = 0;
		nvram_get_r_def("ntp_server_ip1", ntp_server1, sizeof(ntp_server1), "180.182.38.254");
		nvram_get_r_def("ntp_server_ip2", ntp_server2, sizeof(ntp_server2), "kr.pool.ntp.org");
		...
		RunSystemCmd(NULL_FILE, "ntp_inet", "-x", (!ntp_server_id) ? ntp_server1 : ntp_server2, command,
				daylight_save_str, (!ntp_server_id) ? ntp_server2 : ntp_server1, NULL_STR);
	}
#else
	unsigned int ntp_onoff=0;
...
void start_wan_dhcp_client(char *iface)
{
	...
	init_abuffer(&m, 256);
	/* pid path */
	snprintf(tmp, sizeof(tmp), "/etc/udhcpc/udhcpc-%s.pid", iface);
	killDaemonByPidFile(tmp);

	aprintf(&m, "udhcpc -i %s -p %s -s /usr/share/udhcpc/%s.sh",
	        iface, tmp, iface);

	/* deconfig script path */
	snprintf(tmp, sizeof(tmp), "/usr/share/udhcpc/%s.deconfig", iface);
	Create_script(tmp, iface, WAN_NETWORK, 0, 0, 0);

	tmp[0] = '\0';
	apmib_get(MIB_HOST_NAME, (void *)tmp);
	if (tmp[0])
		aprintf(&m, " -h %s", tmp);
	...
	aprintf(&m, " preq link_watcher add dhcpc \"/etc/udhcpc/udhcpc-%s.pid\"", iface);
#endif
	yexecl(NULL, "sh -c \"%s\"", m.buf);
	fini_abuffer(&m);
}
...
#if defined(__DAVO__)
	yfecho(VAR_WAN_IP_FILE, O_WRONLY|O_CREAT|O_TRUNC, 0644, "%s", Ip);
	yfecho(VAR_NETMASK_FILE, O_WRONLY|O_CREAT|O_TRUNC, 0644, "%s", Mask);
	yfecho(VAR_GATEWAY_FILE, O_WRONLY|O_CREAT|O_TRUNC, 0644, "%s", Gateway);
#endif
...
//	start_upnp_igd(DHCP_DISABLED, sys_op, wisp_id, lan_iface);
	...
	if (sys_op == GATEWAY_MODE) {
		...
		if (sdmz) {
			...
			intValue = 0;
			apmib_get(MIB_DHCP, (void *)&intValue);
			if (intValue == DHCP_LAN_SERVER)
				start_lan_dhcpd("br0");
		}
	}
#endif // CONFIG_NF_NAT_TWINIP
