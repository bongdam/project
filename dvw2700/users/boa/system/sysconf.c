...
//#ifdef RTK_CAPWAP
...
#endif
	{
		...
		if (wan_type == DHCP_CLIENT && (!strcmp(argv[2], "dhcp"))) {
			in_addr_t rt = inet_addr(argv[6]);
			start_gateway_keepalive(argv[3],
				(rt && rt != -1U) ? NULL : getenv("serverid"));
		}
...
//add for start tr069 when system is boot
	...
	else if (argv[1] && !strcmp(argv[1], "sdmz")) {
		if (argc == 4 && !strcmp(argv[2], "deconfig")) {
			disable_sdmz(argv[3]);
		}
	}
#endif
...
static int boundsave(int argc, char **argv)
{
	...
	for (i = 0; i < argc && F[i].path; i++) {
		if (F[i].line == 0) {
			...
			if (strcmp(F[i].path, VAR_WAN_IP_FILE) == 0)
				check_change_wanip(argv[i]);
		} else {
