...
int wlanHandler(request *wp, char *tmpBuf, int *mode, int wlan_id)
{
  char *strSSID, *strChan, *strDisabled, *strVal, strtmp[80];
	...
	if (wlan_id == 1) {
		sprintf(varName, "wlan%d_sel_auto_ch", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			nvram_set(varName, strVal);
		}
	}
...
//
#if defined(__DAVO__)
	snprintf(varName, sizeof(varName), "wlan%d_max_conn", wlan_id);
	strVal = req_get_cstream_var(wp, varName, "");
	if (strVal[0]) {
		val = atoi(strVal);
		if (val < 1 || val > 127) {
	...
	for (i=1; i<5; i++) {
		...
		if (wepKey[0]) {
			...
			if (!isAllStar(wepKey) ) {
				...
				if (wep == WEP64)
					ret = apmib_set((MIB_WLAN_WEP64_KEY1 + i - 1), (void *)key);
				else
					ret = apmib_set((MIB_WLAN_WEP128_KEY1 + i - 1), (void *)key);
				if (ret == 0) {
...
// get cipher suite from user setting, for wpa-aes -------------------
		...
		//if ((encrypt == ENCRYPT_WPA) || (encrypt == ENCRYPT_WPA2_MIXED))
		{
			intVal = 0;
			for (i=1; i<=2; i++) {
				sprintf(varName, "ciphersuite%d_%d", i, wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
...
//			printf("wpa2EnableSHA256, strVar=%s\n",strVal);
		...
		//if ((encrypt == ENCRYPT_WPA2) || (encrypt == ENCRYPT_WPA2_MIXED))
		{
			...
			intVal = 0;
			for (i=1; i<=2; i++) {
				sprintf(varName, "wpa2ciphersuite%d_%d", i, wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
					if ( strstr(strVal, ("tkip")))
						intVal |= WPA_CIPHER_TKIP;
...
void formWlAc(request *wp, char *path, char *query)
{
	...
	memset(&macEntrytmp, 0, sizeof(MACFILTER_T));
...
//			strcpy(tmpBuf, ("Error! No mac address to set."));
		...
		if ( (entryNum + 1) > MAX_WLAN_AC_NUM) {
			strcpy(tmpBuf, ("테이블이 모두 차서 더이상 추가할 수 업습니다!"));
			goto setErr_ac;
		}
...
#endif
		strValue = req_get_cstream_var(wp, ("dfs_enable_"), "");
		if (strValue[0]) {
			if (!strcmp(strValue, ("enable")))
				val = 1;
			else
				val = 0;
			nvram_set("x_dfs_enable", val ? "1" : "0");
		}
...
#if defined(__DAVO__)
	strValue = req_get_cstream_var(wp, "wmm_ps", "");
	if (strValue[0]) {
		if (!strcmp(strValue, ("on")))
			val = 1;
		else
			val = 0;
		apmib_set(MIB_WLAN_UAPSD_ENABLED, (void *)&val);
	}

	snprintf(varName, sizeof(varName), "wlan%d_wlweb_permit", wlan_idx);
	strValue = req_get_cstream_var(wp, "wlweb_permit", "");
	if (strValue[0]) {
		if (!strcmp(strValue, ("on")))
			val = 1;
		else
			val = 0;
		nvram_set(varName, val ? "1" : "0");
	}
#endif
...
/////////////////////////////////////////////////////////////////////////////
enum { search_sta= 0, macSelect, disClient };
int wirelessClientList(request *wp, int argc, char **argv)
{
#if defined(__DAVO__)
	int nBytesSent=0, i, found=0, search_flag=0, link_time_len;
	...
	if( argc>=1 && strcmp(argv[0],"disClient")==0 )	{
		search_flag = disClient;
	} else if( argc>=1 && strcmp(argv[0],"wlMacSelect")==0 ) {
		search_flag = macSelect;
	} else {
		search_flag = search_sta;
	}
	...
	if (search_flag == disClient) { // display to connect to auth(radius)
		char auth_pid_path[32];
		snprintf(auth_pid_path, sizeof(auth_pid_path), "/var/run/auth=%s.pid", WLAN_IF);
		if (access(auth_pid_path, F_OK)) {
#ifdef MBSSID
			if (argc == 2)
	...
	for (i=1; i<=MAX_STA_NUM; i++) {
		...
		if (pInfo->aid && (pInfo->flags & STA_INFO_FLAG_ASOC)) {
			...
			if (search_flag == disClient) { // display to connect to auth(radius)
				...
				("<tr class=\"tbl_body\" align=\"center\"><td><font size=2>%02x:%02x:%02x:%02x:%02x:%02x</td>"
				"<td><font size=2>%s</td>"
				"<td><font size=2><script>document.write(get_data_size_converter(%u, %u));</script></td>"
				"<td><font size=2><script>document.write(get_data_size_converter(%u, %u));</script></td>"
				"<td><font size=2>%s</td>"
				"<td><font size=2>%s</td>"
				"<td><font size=2>%ds</td>"
				"<td><font size=2>%s</td>"
				"<td><font size=2> <input type='submit' value='차단' name='disc_sta' onClick=\"return selectMac('%02x:%02x:%02x:%02x:%02x:%02x')\"></td>"
				 "</tr>"),
				pInfo->addr[0],pInfo->addr[1],pInfo->addr[2],pInfo->addr[3],pInfo->addr[4],pInfo->addr[5],
				mode_buf,
				(unsigned int)(pInfo->tx_only_data_bytes >> 32), (unsigned int)(pInfo->tx_only_data_bytes),
				(unsigned int)(pInfo->rx_only_data_bytes >> 32), (unsigned int)(pInfo->rx_only_data_bytes),
				txrate,
				...
				calc_use_data(tx_data_buf, (unsigned long)(pInfo->tx_only_data_bytes >> 32), (unsigned long)(pInfo->tx_only_data_bytes));
				calc_use_data(rx_data_buf, (unsigned long)(pInfo->rx_only_data_bytes >> 32), (unsigned long)(pInfo->rx_only_data_bytes));
	...
	if (found == 0) {
		nBytesSent += req_format_write(wp,
	   		("<tr class=\"tbl_body\" align=\"center\"><td><font size=2>없음</td>"
			"<td><font size=2>---</td>"
	     	"<td><font size=2>---</td>"
			"<td><font size=2>---</td>"
			"<td><font size=2>---</td>"
			...
			if (search_flag != macSelect) {
				nBytesSent += req_format_write(wp,"<td><font size=2>---</td>");
			}
		nBytesSent += req_format_write(wp,"</tr>");
	}
...
#if defined(__DAVO__)
		snprintf(varName, sizeof(varName), "wlan%d_vap%d_max_conn", wlan_idx, idx-1);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if (1 <= val && val <= 127) {
				nvram_set(varName, strVal);
...
//displayPostDate(wp->post_data);
	...
	if ( refresh[0] ) {
		// issue scan request
		if (strcmp(submitUrl, "/wlap_search.asp") == 0 && wlan_idx == 0) {	//auto channel algorism 5G iwpriv carry out
			yexecl(NULL, "iwpriv wlan0 request_autoch [%d] request_autoch", RTL8192CD_IOCTL_REQ_AOTUCH);
		} else {
			wait_time = 0;
			while (1) {
				...
				switch(getWlSiteSurveyRequest(WLAN_IF, &status))
				{
					case -2:
						printf("-2\n");
						strcpy(tmpBuf, ("자동 검색중!! 잠시 기다려 주십시오..."));
						//goto ss_err;
						break;
					case -1:
						printf("-2\n");
						strcpy(tmpBuf, ("검색 요청 실패!"));
						//goto ss_err;
						break;
					default:
						break;
...
const int influence_table_2g[13][13]= {
...
static int calculate_best_chan(int *sel_auto_ch, const int *pD, int *pS, int end_chan)
{
	int i, ii;
	int min= -1;
	...
	for ( i = 0; i < chan_cnt; i++) {
		for ( ii = 0; ii < chan_cnt; ii++) {
			pS[i] += (pD[ii] * influence_table_2g[i][ii]);
		}

		printf("DVKLOG_Channel %d[detect:%d], score %u\n", i + 1, pD[i], pS[i]);
...
int wlSiteSurveyTbl(request *wp, int argc, char **argv)
{
	...
	WLAN_MODE_T mode;
	bss_info bss;
	int display_channel_info=0;

	if (argc >= 1) {
		display_channel_info = atoi(argv[0]);
	}
...
//#ifdef CONFIG_RTK_MESH
	...
	if (display_channel_info == 1) {
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
					...
					"<td align=center width=\"20%%\" ><font size=\"2\"><b>모드</b></font></td>\n"
					"<td align=center width=\"10%%\" ><font size=\"2\"><b>신호세기</b></font></td>\n"
					"</tr>\n"));
	} else {
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
					"<td align=center width=\"30%%\" ><font size=\"2\"><b>SSID</b></font></td>\n"
					...
					"<td style=\"display:none\" align=center width=\"10%%\" ><font size=\"2\"><b>암호화</b></font></td>\n"
					"<td align=center width=\"10%%\" ><font size=\"2\"><b>신호세기</b></font></td>\n"));
		if( (mode == CLIENT_MODE )
#if defined(CONFIG_RTL_ULINKER)
	...
	for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) {
		...
		if (display_channel_info == 1) {
			...
			"<td align=center width=\"10%%\" ><font size=\"2\">%d</td>\n"
			"</tr>\n"),
			tmp3Buf, tmpBuf, pBss->ChannelNumber, tmp1Buf, CONV_TO_RSSI(pBss->rssi));
			...
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
						...
						"<td align=center width=\"20%%\" ><font size=\"2\">%s</td>\n"
						"<td style=\"display:none\" align=center width=\"10%%\" ><font size=\"2\">%s</td>\n"
						"<td align=center width=\"10%%\" ><font size=\"2\">%d</td>\n"),
	...
	if( pStatus->number == 0 )
	{
		if (display_channel_info == 1) {
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
			"<td align=center width=\"30%%\" ><font size=\"2\">없음</td>\n"
			...
			{
				nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
							"<td align=center width=\"20%%\" ><font size=\"2\">없음</td>\n"
							"<td align=center width=\"20%%\" ><font size=\"2\"></td>\n"
							"<td align=center width=\"10%%\" ><font size=\"2\"></td>\n"
							"<td align=center width=\"20%%\" ><font size=\"2\"></td>\n"
							"<td style=\"display:none\" align=center width=\"10%%\" ><font size=\"2\"></td>\n"
							"<td align=center width=\"10%%\" ><font size=\"2\"></td>\n"
							"<td align=center width=\"20%%\" ><font size=\"2\"></td>\n"
							"</tr>\n"));
			}
...
//DAVOLINK 11:11:20 hmpark
//@NOTE: channel_scan data in webpages (graph data)
		memset(chan_walking_num, 0, sizeof(chan_walking_num));
		memset(sel_auto_ch, 0, sizeof(sel_auto_ch));
		memset(chan_score, 0, sizeof(chan_score));

		if (wlan_idx == 1) {	// 2.4g best Channel algorism use CJHV algorism
			snprintf(path, sizeof(path), "/proc/dv_wlan1/auto_ch_info");
			fp = fopen(path, "r");
			...
			for (i = 0; i < pStatus->number && pStatus->number != 0xff; i++) {
				pBss = &pStatus->bssdb[i];
				if (( pBss->ChannelNumber > 0 && pBss->ChannelNumber < 14 && CONV_TO_RSSI(pBss->rssi) > -80)) {	//rssi < -80 ignore
					chan_walking_num[pBss->ChannelNumber-1]++;
				}
			}

			best_chan = calculate_best_chan(&sel_auto_ch[0], &chan_walking_num[0], &chan_score[0], 11);

			if ( best_chan < 1 ) {
				printf("Error: cant find best channel\n");
				best_chan = 1;
			}
		} else {	// 5g best Channel algorism use realtek sdk algorism
			snprintf(path, sizeof(path), "cat /proc/wlan0/mib_all | grep request_autoch");
			fp = popen(path, "r");
			if (fp) {
				fgets(value, sizeof(value), fp);
				if (ystrargs(value, argv, _countof(argv), " :\n", 0) > 1) {
					best_chan = strtol(argv[1], NULL, 10);
				}
				pclose(fp);
			}
		...
		nBytesSent += req_format_write(wp, "<script>\n"
				"best_ch=%d;\n"
				"</script>\n", best_chan);
	}
...
void formWlWds(request *wp, char *path, char *query)
{
	...
	char tmpBuf[MAX_MSG_BUFFER_SIZE] = {0,};
	char buf[32],buffer[32], list[64];
	int entryNum, i, enabled, val, j, count = 0;
		...
		for(j = 1; j <= entryNum; j++)
		{
			...
			*((char *)&macEntrytmp) = (char)j;
			if ( apmib_get(MIB_WLAN_WDS, (void *)&macEntrytmp))
			{
				if (!memcmp(macEntrytmp.macAddr, macEntry.macAddr, 6))
				{
					strcpy(tmpBuf, ("오류! 해당 MAC 주소가 이미 존재합니다."));
					goto setErr_wds;
				}
	...
	if (strDelMac[0]) {
...
#ifdef __DAVO__
				count++;
				snprintf(buffer, sizeof(buffer), "WLAN%d_WDS%d", wlan_idx, i);
				nvram_unset(buffer);
#else
				*((char *)&macEntry) = (char)i;
				if ( !apmib_get(MIB_WLAN_WDS, (void *)&macEntry)) {
		...
		for (i = 0; i < entryNum; i++) {
			snprintf(buffer, sizeof(buffer), "WLAN%d_WDS%d", wlan_idx, (i + 1));
			if (nvram_get_r(buffer, list, sizeof(list))) {
				snprintf(buf, sizeof(buf), "WLAN%d_WDS%d", wlan_idx, j);
				nvram_set(buf, list);
				j++;
				if ((i + 1) != (j - 1))
					nvram_unset(buffer);
		...
		entryNum -= count;
		snprintf(buf, sizeof(buf), "WLAN%d_WDS_NUM", wlan_idx);
		snprintf(buffer, sizeof(buffer), "%d", entryNum);
		nvram_set(buf, buffer);
#endif
	}
...
int wlWdsList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"35%%\" ><font size=\"2\"><b>MAC 주소</b></font></td>\n"
      	"<td align=center width=\"25%%\" ><font size=\"2\"><b>전송률 (Mbps)</b></font></td>\n"
      	"<td align=center width=\"30%%\" ><font size=\"2\"><b>설명</b></font></td>\n"
      	"<td align=center width=\"10%%\" ><font size=\"2\"><b>선택</b></font></td></tr>\n"));
	...
	for (i = 1; i <= entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		if(entry.fixedTxRate == 0) {
			snprintf(txrate, sizeof(txrate), "%s","Auto");
		} else {
			for(rateid = 0; rateid < 48; rateid++){
				if(tx_fixed_rate[rateid].id == entry.fixedTxRate){
					snprintf(txrate, sizeof(txrate), "%s", tx_fixed_rate[rateid].rate);
					break;
				}
			}
		}
#ifdef __DAVO__
		translate_control_code(entry.comment);
#endif
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
      			"<td align=center width=\"35%%\" ><font size=\"2\">%s</td>\n"
...
void formWdsEncrypt(request *wp, char *path, char *query)
{
	...
	if (encrypt == WDS_ENCRYPT_WEP64 || encrypt == WDS_ENCRYPT_WEP128) {
		...
		if (strVal[0]) {
			if (strVal[0] != '0' && strVal[0] != '1') {
				strcpy(tmpBuf, ("wep key 형식이 올바르지 않습니다!"));
				goto setErr_wdsEncrypt;
			}
			intVal = strVal[0] - '0';
		...
		if  (strVal[0]) {
			if (strlen(strVal) != keyLen) {
				strcpy(tmpBuf, ("wep key 길이가 올바르지 않습니다!"));
				goto setErr_wdsEncrypt;
		}
			if ( !isAllStar(strVal) ) {
				if (intVal == 0) { // ascii
	...
	if (encrypt == WDS_ENCRYPT_TKIP || encrypt == WDS_ENCRYPT_AES) {
		...
		if (strVal[0]) {
			if (strVal[0] != '0' && strVal[0] != '1') {
				strcpy(tmpBuf, ("psk key 형식이 올바르지 않습니다!"));
				goto setErr_wdsEncrypt;
			}
			intVal = strVal[0] - '0';
		} else
...
int wdsList(request *wp, int argc, char **argv)
{
	...
	for (i = 0; i < MAX_WDS_NUM; i++) {
		...
		nBytesSent += req_format_write(wp,
	   		"<tr class=\"tbl_body\">"
	   			"<td><font size=2>%02x:%02x:%02x:%02x:%02x:%02x</td>"
				"<td><font size=2>%d</td>"
	     		"<td><font size=2>%d</td>"
				"<td><font size=2>%d</td>"
				"<td><font size=2>%s</td>"
			"</tr>",
			pInfo->addr[0],pInfo->addr[1],pInfo->addr[2],pInfo->addr[3],pInfo->addr[4],pInfo->addr[5],
			pInfo->tx_packets, pInfo->tx_errors, pInfo->rx_packets,	txrate);
	}
...
{
	char *strVal, *submitUrl, *strResetUnCfg;
...
int getWlProfileInfo(request *wp, int argc, char **argv)
{
#if defined(WLAN_PROFILE)
	char	*name;
	int idx;
