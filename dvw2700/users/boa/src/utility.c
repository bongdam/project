...
int write_line_to_file(char *filename, int mode, char *line_data)
{
    ...
    write(fh, (line_data) ? : "", (line_data) ? strlen(line_data) : 0);
    close(fh);
    return 1;
}
...
int get_clone_mac_by_ip(char *ip, char *clone_mac)
{
	...
	p = arpent_dump();
	if (p) {
		in_addr_t n = inet_addr(ip);
		for (t = p; t->netdev[0]; t++) {
			if (!strcmp(t->netdev, "br0") && (n == t->ip.s_addr)) {
				sprintf(clone_mac, "%02x%02x%02x%02x%02x%02x",
...
struct in_addr to_in_addr(unsigned char *a)
{
	union {
		unsigned char b[4];
		struct in_addr in;
...
int get_ipaddr_file(char *file, unsigned long *ipaddr, char *strIpaddr)
{
	char buf[32];
	struct in_addr addr;

	if (yfcat(file, "%31s", buf) > 0 && inet_aton(buf, &addr)) {
		*ipaddr = addr.s_addr;
		if (strIpaddr)
...
int sdmz_enable(void)
{
	...
	if (sys_opmode == 0) {
		nvram_get_r_def("x_sdmz_enable", sdmz_enable, sizeof(sdmz_enable), "0");
		enable = strtol(sdmz_enable, NULL, 10);
	}
	return enable;
}
...
int autochan_get_bandwidth(void)
{
	...
	f = fopen("/proc/wlan1/mib_11n", "r");
	if (f) {
		while (fgets(buf, sizeof(buf), f)) {
			if ((p = strstr(buf, "currBW:"))) {
				if (strstr(p + 7, "40M"))
					bandwidth = 1;
				break;
			}
...
int send_ping_test(char *test_ip)
{
	char ping_cmd[128] = {0,};
	FILE *pp = NULL;
	int result = 0;
	...
	if (strstr(test_ip, "reboot"))
		return result;

	snprintf(ping_cmd, sizeof(ping_cmd), "mping -q -p -c 1 -w 1000 %s", test_ip);
	pp = popen(ping_cmd, "r");
...
int getPortStats(int port, char *dest)
{
	FILE *f;
	char token[16], buf[16] = { [15] = '\0' };
	unsigned long long octet, uc, mc, bc, stat[2][2];
	int pos = 0;

	f = fopen("/proc/asicCounter", "r");
	if (!f)
		return 0;
	...
	while (pos < 2 && fscanf(f, "%15s ", buf) != EOF) {
		if (!strcmp(token, buf)) {
			if (fscanf(f, "%llu %llu %llu %llu %*[^\n]\n",
			           &octet, &uc, &mc, &bc) == 4) {
				stat[pos][0] = octet;
				stat[pos++][1] = uc + mc + bc;
			}
	...
	sprintf(dest, "%llu %llu %llu %llu",
		stat[1][1], stat[0][1], stat[1][0], stat[0][0]);
	return 1;
}

#define IS_SPECIAL(b) ((b=='\"') || (b=='\'') || (b=='\\'))
void escape_special(char *dst, char *src, int dstlen)
{
	int i, j;
...
struct arpent *arpent_dump(void)
{
	FILE *f;
	struct arpent *p = NULL;

	f = fopen("/proc/net/arp", "r");
	if (f) {
		int n = 0, size = 16;

		p = malloc(sizeof(struct arpent) * size);
		fscanf(f, "%*[^\n]");
		while (fscanf(f, "%hhu.%hhu.%hhu.%hhu %*s %hx %hhx:%hhx:%hhx:%hhx:%hhx:%hhx %*s %15s",
		              ((unsigned char *)&p[n].ip) + 0, ((unsigned char *)&p[n].ip) + 1,
		              ((unsigned char *)&p[n].ip) + 2, ((unsigned char *)&p[n].ip) + 3,
		              &p[n].flag,
		              &p[n].mac[0], &p[n].mac[1], &p[n].mac[2],
		              &p[n].mac[3], &p[n].mac[4], &p[n].mac[5],
		              p[n].netdev) >= 4) {
			if (++n >= size) {
				struct arpent *q = realloc(p, sizeof(struct arpent) * (size + 1));
				if (q == NULL)
					break;
				size += 1;
				p = q;
		...
		fclose(f);
		memset(&p[n], 0, sizeof(struct arpent));
	}
	return p;
}
#endif
