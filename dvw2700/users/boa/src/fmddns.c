...
void formDdns(request *wp, char *path, char *query)
{
	...
	if (apmib_set( MIB_DDNS_ENABLED, (void *)&enabled) == 0) {
		strcpy(tmpBuf, "DDNS 사용 설정 실패!");
		goto setErr_ddns;
	}

	if (enabled) {
		tmpStr = req_get_cstream_var(wp, "ddnsType", "0");
		if (tmpStr[0]) {
		ddnsType = tmpStr[0] - '0' ;
	 		if (apmib_set(MIB_DDNS_TYPE, (void *)&ddnsType) == 0) {
					strcpy(tmpBuf, "DDNS 서비스 공급자 설정 실패!");
					goto setErr_ddns;
			}
		}
		...
		if (tmpStr[0]) {
			if (apmib_set(MIB_DDNS_DOMAIN_NAME, (void *)tmpStr) == 0) {
					strcpy(tmpBuf, "DDNS 도메인 이름 설정 실패!");
					goto setErr_ddns;
			}
		}
	}
