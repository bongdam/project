...
#define DNS1_CJHV       	0xb4b63601  //180.182.54.1
#define DNS2_CJHV2			0xb4b63602  //180.182.54.2
#define DNS1_KT				0xa87e3f01  //168.126.63.1
#define DNS2_KT				0xa87e3f02  //168.126.63.2
#define DNS1_LG				0xa47c6b09  //164.124.107.9
#define DNS2_LG				0xa47c6502  //164.124.101.2
#define DNS3_LG				0xcbf8fc02  //203.248.252.2
#define DNS4_LG				0xcbf8f01f  //203.248.240.31
#define DNS5_LG				0x3d299902  //61.41.153.2
#define DNS6_LG				0x01d64402  //1.214.68.2
#define DNS1_SKB			0xd2dca352  //210.220.163.82
#define DNS2_SKB			0xdbfa2482  //219.250.36.130
#define DNS3_SKB			0xd25e0049  //210.94.0.73
#define DNS4_SKB			0xdd8b0d82  //221.139.13.130
#define DNS5_SKB			0xd2b4624a  //210.180.98.74
#define DNS6_SKB			0xd33afc3e  //211.58.252.62
#define DNS7_SKB			0xd33afc5e  //211.58.252.94
#define DNS8_SKB			0xd2b50129  //210.181.1.41
#define DNS9_SKB			0xd2b50433  //210.181.4.51
#define DNS1_DREAMLINE		0xd2b50118  //210.181.1.24
#define DNS2_DREAMLINE		0xd2b50419  //210.181.4.25
#define DNS1_ONSETELECOM	0xca1e8f0b  //202.30.143.11
#define DNS1_OPEN			0xd043dede  //208.67.222.222
#define DNS2_OPEN			0xd043dcdc  //208.67.220.220
#define DNS1_GOOGLE			0x08080808  //8.8.8.8
#define DNS2_GOOGLE			0x08080404  //8.8.4.4
...
static int check_prevent_dnsaddr(struct in_addr *pin)
{
	int i;

	for (i=0; i < ARRAY_SIZE(dns_list); i++) {
		if ( pin->s_addr == htonl(dns_list[i]) )
			return 0;
	}
	return 1;
}
...
static int is_dup_check(char *target)
{
	...
	if ( (fp = fopen("/var/sys_op", "r")) ) {
		if (fgets(line_str, sizeof(line_str), fp))
			bride_mode = atoi(line_str);
		fclose(fp);
	}

	if (bride_mode)	{
		apmib_get(MIB_HW_NIC0_ADDR, (void *)s_mac);
		snprintf(intf, sizeof(intf), "%s", "br0");
	}
	else {
		apmib_get(MIB_HW_NIC1_ADDR, (void *)s_mac);
		snprintf(intf, sizeof(intf), "%s", "eth1");
	}
...
//////////////////////////////////////////////////////////////////////////////
int getOneDhcpClient(char **ppStart, unsigned long *size, char *ip, char *mac, char *liveTime, time_t now)
{
	...
	*size = *size - sizeof(entry);
	...
	if (entry.expires == 0xffffffff)
		...
		snprintf(liveTime, 12, "%d", (int)(entry.expires - now));
...
int tcpipLanHandler(request *wp, char *tmpBuf)
{
	...
	if ( dhcp == DHCP_DISABLED || dhcp == DHCP_SERVER || dhcp == DHCP_AUTO || DHCP_AUTO_WAN) {
		...
		if ( strIp[0] ) {
			char *user_ip = nvram_get("IP_ADDR");
			...
			if (user_ip != NULL && strcmp(strIp, user_ip)) // user change
				nvram_set("user_ip", strIp);
		}
		...
		if ( dhcp == DHCP_SERVER|| dhcp == DHCP_AUTO || DHCP_AUTO_WAN) {
			...
			if ( strdhcpRangeStart[0] ) {
				char *user_dhcp_start = nvram_get("DHCP_CLIENT_START");
				...
				if (user_dhcp_start != NULL && strcmp(strdhcpRangeStart, user_dhcp_start)) // user change
					nvram_set("user_dhcp_start", strdhcpRangeStart);
			}
			strdhcpRangeEnd = req_get_cstream_var(wp, ("dhcpRangeEnd"), "");
			if ( strdhcpRangeEnd[0] ) {
				char *user_dhcp_end = nvram_get("DHCP_CLIENT_END");
				...
				if (user_dhcp_end != NULL && strcmp(strdhcpRangeEnd, user_dhcp_end)) // user change
					nvram_set("user_dhcp_end", strdhcpRangeEnd);
			}
...
void rearrange_staticdhcp(void)
{
	int i, j = 0;
	...
	for (i = 0; i < MAX_DHCP_RSVD_IP_NUM; i++) {
		snprintf(query, sizeof(query), "DHCPRSVDIP_TBL%d", (i + 1));
		p = nvram_get(query);
		if (p) {
			snprintf(value, sizeof(value), "%s", p);
			nvram_unset(query);
			snprintf(rewrite, sizeof(rewrite), "DHCPRSVDIP_TBL%d", (j + 1));
	...
	nvram_set("DHCPRSVDIP_TBL_NUM", tableNum);
	if (!j)
		nvram_set("DHCPRSVDIP_ENABLED", "0");
}

static void checkDmz_PortFw(void)
{
	int entryNum = 0;
	PORTFW_T entry;
	unsigned long lan_addr, lan_mask, ip_addr;
	int index;
	char query[32] = {0,}, value[64] = {0,};
	char ipaddr[32] = {0,};
	char *args[4] = {NULL,};
	int n;
	struct in_addr tmp;
	...
	for (index = 1; index <= entryNum; index++) {
		...
		*((char *)&entry) = (char)index;
		...
		tmp.s_addr = *pl;
		tmpStr = inet_ntoa(tmp);
		snprintf(ipaddr, sizeof(ipaddr), "%s", tmpStr);

		ip_addr = inet_addr(ipaddr);
		if ( (ip_addr & lan_mask) != (lan_addr & lan_mask) ) {
	...
	nvram_get_r_def("DMZ_HOST", value, sizeof(value), "");
	if (strcmp(value, "0.0.0.0")) {
		ip_addr = inet_addr(value);
		if ( (ip_addr & lan_mask) != (lan_addr & lan_mask) ) {
			printf("delete dmz wrong ip.\n");
	...
	entryNum = nvram_atoi("DHCPRSVDIP_TBL_NUM", 0);
	for (index = 0; index < entryNum; index++) {
		snprintf(query, sizeof(query), "DHCPRSVDIP_TBL%d", index + 1);
		nvram_get_r_def(query, value, sizeof(value), "");
		n = ystrargs(value, args, _countof(args), ",\n", 0);
		if (n >= 2) {
			ip_addr = inet_addr(args[1]);
			if ( (ip_addr & lan_mask) != (lan_addr & lan_mask) ) {
				printf("delete staticdhcp wrong ip.\n");
				nvram_unset(query);
				delete_event = 1;
			}
		}
...
int tcpipWanHandler(request *wp, char * tmpBuf, int *dns_changed)
{
	...
	strVal = req_get_cstream_var(wp, ("macCloneEnable"), "");
 	if (strVal[0]) {
 		...
 		nvram_set("mac_clone_enable", "0");
 	}
#endif

	strMac = req_get_cstream_var(wp, ("wan_macAddr"), "000000000000");
	...
	strMode = req_get_cstream_var(wp, ("staticDNS"), "");
		...
		if ( dns == DNS_MANUAL ) {
			...
			if ( strDNS[0] ) {
...
#ifdef __DAVO__
				{
					...
					fp = fopen("/tmp/.attack_ip", "w");
					if (fp) {
						time(&c);
						strftime(t, sizeof(t), "%F %T", localtime(&c));
						fprintf(fp, "%s %s", wp->remote_ip_addr, t);
						fclose(fp);
					}
				}
#endif
...
//				printf("%s:%d wan_if=%s\n",__FUNCTION__,__LINE__,WAN_IF);
	...
	if ( dhcp == DHCP_DISABLED ) {
		...
		if ( strIp[0] ) {
			if ( !inet_aton(strIp, &inIp) ) {
				strcpy(tmpBuf, ("IP 주소가 올바르지 않습니다!"));
				goto setErr_tcpip;
			}
#ifdef __DAVO__
			if ( (res = is_dup_check(strIp) > 0) ) {
				snprintf(tmpBuf, sizeof(tmpBuf), "%s", (res == 1) ? "IP 주소가 충돌했습니다." : "IP 주소가 올바르지 않습니다!");
				goto setErr_tcpip;
			}
#endif
			if ( !apmib_set(MIB_WAN_IP_ADDR, (void *)&inIp)) {
				strcpy(tmpBuf, ("IP 주소 설정 오류!"));
				goto setErr_tcpip;
			}
		}
		...
		if ( strMask[0] ) {
			if ( !inet_aton(strMask, &inMask) ) {
				strcpy(tmpBuf, ("서브넷 마스크가 올바르지 않습니다!"));
				goto setErr_tcpip;
			}
			if ( !apmib_set(MIB_WAN_SUBNET_MASK, (void *)&inMask)) {
				strcpy(tmpBuf, ("서브넷 마스크 설정 오류!"));
				goto setErr_tcpip;
			}
		}
		...
		if ( strGateway[0] ) {
			if ( !inet_aton(strGateway, &inGateway) ) {
				strcpy(tmpBuf, ("게이트웨이가 올바르지 않습니다!"));
				goto setErr_tcpip;
			}
			if ( !apmib_set(MIB_WAN_DEFAULT_GATEWAY, (void *)&inGateway)) {
				strcpy(tmpBuf, ("게이트웨이 설정 오류!"));
				goto setErr_tcpip;
			}
		}
		...
		if ( strVal[0] ) {
			...
			if ( apmib_set(MIB_FIXED_IP_MTU_SIZE, (void *)&mtuSize) == 0) {
				strcpy(tmpBuf, ("고정 IP mtu 크기 MIB 설정 오류!"));
				goto setErr_tcpip;
			}
		}
	}
		...
		if (dhcp == DHCP_CLIENT) {
			...
			if ( strVal ) {
				...
				if ( apmib_set(MIB_DHCP_MTU_SIZE, (void *)&mtuSize) == 0) {
					strcpy(tmpBuf, ("DHCP mtu 크기 MIB 설정 오류!"));
					goto setErr_tcpip;
				}
			}
...
#ifdef __DAVO__
			if (wp->superUser == 1) {
				strVal = req_get_cstream_var(wp, ("dhcp_retry"), "");
				if (strVal[0]) {
					nvram_set("dhcpc_discover_retry", strVal);
				}

				strVal = req_get_cstream_var(wp, ("dhcp_wait"), "");
				if (strVal[0]) {
					nvram_set("dhcpc_decline_time", strVal);
				}

				strVal = req_get_cstream_var(wp, ("dhcp_ping_wait"), "");
				if (strVal[0]) {
					nvram_set("dhcpc_watching_time", strVal);
				}

				strVal = req_get_cstream_var(wp, ("network_check"), "");
				if (strcmp(strVal, "ON") == 0) {
					nvram_set("dhcpc_watching_probe", "1");
				} else {
					nvram_set("dhcpc_watching_probe", "0");
				}
			}
#endif
		}
...
//Brad add end
		...
		if ( !apmib_set(MIB_VPN_PASSTHRU_L2TP_ENABLED, (void *)&intVal)) {
			strcpy(tmpBuf, ("VPN PASSTHRU L2TP 사용 설정 오류!"));
			goto setErr_tcpip;
		}
		...
		if (wp->superUser == 1) {
			...
			if ( !strcmp(strVal, "ON"))
				nvram_set("telnet_enable", "1");
			else
				nvram_set("telnet_enable", "0");
			start_telnetd();
#ifndef NO_ACTION
			{
				int pid;
		...
		if (nvram_match("OP_MODE", "0") && !strcmp(strVal, "ON"))
			intVal = 1;
		else
			intVal = 0;
...
int dhcpRsvdIp_List(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
				"<td align=center width=\"25%%\" ><font size=\"2\"><b>IP 주소</b></font></td>\n"
				"<td align=center width=\"45%%\" ><font size=\"2\"><b>MAC 주소</b></font></td>\n"
				"<td align=center width=\"20%%\" ><font size=\"2\"><b>설명</b></font></td>\n"
	...
	for (i = 1; i <= entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		if (!memcmp(entry.macAddr, "\x0\x0\x0\x0\x0\x0", 6))
			...
			sprintf(macaddr, " %02x-%02x-%02x-%02x-%02x-%02x", entry.macAddr[0], entry.macAddr[1], entry.macAddr[2],
		...
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
				...
				"<input type=\"submit\" value=\"삭제\" id=\"deleteSelRsvIP%d\" name=\"deleteSelRsvIP\" onClick=\"return deleteClick('%d')\"></td></tr>\n"),
				  inet_ntoa(*((struct in_addr *)entry.ipAddr)), macaddr, hostName, i, i);
	}
/* KDH DAVO 10/03/19 */
/*
	if (entryNum == 0) {
		nBytesSent += req_format_write(wp, ("<tr>"
						  "<td align=center width=\"30%%\" class='mn24' bgcolor=\"#F0F0F0\">없음</td>\n"
						  "<td align=center width=\"30%%\" class='mn24' bgcolor=\"#F0F0F0\">---</td>\n"
						  "<td align=center width=\"30%%\" class='mn24' bgcolor=\"#F0F0F0\">---</td>\n"
						  "<td align=center width=\"10%%\" class='mn24' bgcolor=\"#F0F0F0\">---</td></tr>\n"));
	}
*/
/* DAVO END */
	return 0;
...
int dhcpClientList(request *wp, int argc, char **argv)
{
	...
	int nBytesSent = 0;
	int element = 0, ret;
	char ipAddr[20], macAddr[20], liveTime[12], *buf = NULL, *ptr;
	struct stat status;
	struct timespec ts;
#ifndef LEASE_WRITE_THRU
	int pid;
	...
	pid = getPid(tmpBuf);
	snprintf(tmpBuf, sizeof(tmpBuf), "kill -SIGUSR1 %d\n", pid);
...
#endif
	while (1) {
		ret = getOneDhcpClient(&ptr, &status.st_size, ipAddr, macAddr, liveTime, ts.tv_sec);
		...
		if (argc == 1 && (strcmp(argv[0], "macSelect") == 0)) {
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\" align=center><td><font size=2>%s</td><td><font size=2>%s</td><td><font size=2>%s</td><td><input type='radio' name='macChecked' value='%s' onClick=macInput('%s')></td></tr>\n"),
					ipAddr, macAddr, liveTime, macAddr, macAddr);
		} else if (argc == 1 && (strcmp(argv[0], "stbMac") == 0)) {	/* KDH DAVO, APNRTL-175, search STB mac address */
			nBytesSent += req_format_write(wp, ("%s,%s|"), ipAddr, macAddr);
		} else {
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\" align=center><td><font size=2>%s</td><td><font size=2>%s</td><td><font size=2>%s</td></tr>\n"),
					ipAddr, macAddr, liveTime);
		}
		element++;
	}

	if (argc == 1 && (strcmp(argv[0], "stbMac") == 0)) /* KDH DAVO, APNRTL-175, search STB mac address */
		nBytesSent += req_format_write(wp, ("_%d"), element);
err:
	if (element == 0) {
		if (argc == 1) {
			if (argc == 1 && (strcmp(argv[0], "macSelect") == 0)) {
				nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\" align=center><td><font size=2>None</td><td><font size=2>----</td><td><font size=2>----</td><td>&nbsp;</td></tr>\n"));
			} else if (argc == 1 && (strcmp(argv[0], "stbMac") == 0)) {	/* KDH DAVO, APNRTL-175, search STB mac address */
				nBytesSent += req_format_write(wp, ("null"));
			}
		} else {
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\" align=center><td><font size=2>없음</td><td><font size=2>----</td><td><font size=2>----</td></tr>\n"));
		}
	}
...
/* APNRTL-259 */
//Static DHCP
int check_dup_static_info(unsigned char *webmac, struct in_addr webIp)
{
	DHCPRSVDIP_T entry;
	int entryNum, i;
	apmib_get(MIB_DHCPRSVDIP_TBL_NUM, (void *)&entryNum);
	...
	for (i = 1; i <= entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		if (!memcmp(entry.macAddr, webmac, 6))
			return 1;

		if (!memcmp(entry.ipAddr, &webIp, sizeof(struct in_addr)))
			return 2;
	}
