...
static COUNTRY_IE_ELEMENT countryIEArray[] =
{
	...
	*/
	...
	{410,"KR ", 3, 3, "KOREA REPUBLIC"},
	{408,"KP ", 3, 3, "NORTH KOREA"},
	{414,"KW ", 3, 3, "KUWAIT"},
	{428,"LV ", 3, 3, "LATVIA"},
	{422,"LB ", 3, 3, "LEBANON"},
...
#if defined(__DAVO__)
static int getInfofast(request *wp, int argc, char **argv);
static int getIndexfast(request *wp, int argc, char **argv);
#endif
...
int getInfo(request *wp, int argc, char **argv)
{
...
#if defined(__DAVO__)
	if ((i = getInfofast(wp, argc, argv)) >= 0)
		return i;
#endif
	...
	else if(!strcmp(name, "wlEnableProfile"))
	{
		...
		return req_format_write(wp, "%d일 %d시간 %d분 %d초",
							day, hr, mn, sec);
	}
...
//printf("\r\n buffer[%s],__[%s-%u]\r\n",buffer,__FILE__,__LINE__);
	...
	else if(!strcmp(argv[0],("mesh_jscomment_end")))
	{
		...
		if ( !apmib_get( MIB_IP_ADDR,  (void *)buffer) )
			return -1;
   		return req_format_write(wp, "%s", inet_ntoa(to_in_addr((u_char *)buffer)) );
	}
	...
	*/
	...
	/* Advance setting stuffs */
		...
		if (state == 4 || state == 6)
			sprintf(buffer, "%s &nbsp;<font size=2 color='red'>재부팅 대기중</font>", buffer);
		else
			sprintf(buffer, "%s &nbsp;<font size=2 color='blue'>(%s)</font>", buffer, showApmsState());
...
// for ap package of new ui , end
	...
	isWanPhy_Link = switch_port_status(PRTNR_WAN0);
	...
	if ( dhcp == DHCP_CLIENT) {
		...
		if (nvram_get_r("x_sdmz_enable", buffer, sizeof(buffer))) {
			if( !strcmp(buffer,"1")) {
		...
		if(isWanPhy_Link < 0)
			return req_format_write(wp, "고정 IP 연결안됨");
		else
	...
	else if ( !strcmp(name, "wan-ip"))
  {
	...
	if (get_network_info("wan_ip", strWanIP) == 0)
	...
	char strWanIP[40];
  	...
  	char strWanIP[16];
		...
		if(argc >=2 && argv[1])
			checkwan(argv[1]);
#endif
	if (get_network_info("netmask", strWanMask) == 0)
		getWanInfo(strWanIP, strWanMask, strWanDefIP, strWanHWAddr);
	...
	char strWanIP[40];
  	...
  	char strWanIP[16];
		...
		if(argc >=2 && argv[1])
			checkwan(argv[1]);
#endif
	if (get_network_info("gateway", strWanDefIP) == 0)
		getWanInfo(strWanIP, strWanMask, strWanDefIP, strWanHWAddr);
	...
	else if(!strcmp(name, "vlan_wan_wifi_vap3_pri"))
	{
	  ...
	  sprintf(buffer, "%d", intVal );
	    ...
	    translate_control_code(buffer);
            ...
            req_format_write(wp, "%s", inet_ntoa(to_in_addr((u_char *)buffer)));
            return 0;
    }
...
//		req_format_write(wp,"<tr><hr size=1 noshade align=top></tr>");
	...
	{
		if(strlen(lastUrl) == 0)
			req_format_write(wp, "%s", "/home.htm");
		else
...
static char *inner_getInxxx(request *wp, char *name,
			int (*getInxxx)(request *, int, char **))
{
	char *argv[] = { name , NULL };

	if (wp->buffer) {
		wp->buffer[0] = '\0';
		wp->buffer_start = wp->buffer_end = 0;
		getInxxx(wp, 1, argv);
	}
	return (wp->buffer && wp->buffer[0]) ? wp->buffer : "\"\"";
}

static char *inner_getIndex(request *wp, char *name)
{
	return inner_getInxxx(wp, name, getIndex);
}

static char *inner_getInfo(request *wp, char *name)
{
	return inner_getInxxx(wp, name, getInfo);
}
#else	/* REFINE_ASP */
request inner_req;
...
#ifdef __DAVO__
# define __deprecated ({ 0; }) &&
# define __ejdefined __deprecated
#else
# define __deprecated
#endif
...
#if defined(__DAVO__)
	if ((val = getIndexfast(wp, argc, argv)) >= 0)
		return val;
#endif
...
#ifdef WIFI_SIMPLE_CONFIG
#if 0
	else if (__ejdefined !strcmp(name, "wscDisable")) {
...
//for 11N
	...
	else if (__ejdefined !strcmp(name, "shortGIEnabled")) {
...
#endif
	else if ( !strcmp(name, "wmm_ps")) {
		if ( !apmib_get( MIB_WLAN_UAPSD_ENABLED, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
...
//### end
    ...
    else if(!strcmp(argv[0], "dfs_enable"))
    {
     ...
     else if(!strcmp(name,"set_wlanindex"))
     {
   	...
   	else if (__deprecated !strcmp(name, "vlan_val_init")) {
	...
	else if (__deprecated !strcmp(name, "wizard_wlband_init")) {
#ifdef REFINE_ASP
		int i, val, wlan_idx_ori, vwlan_idx_ori;
		char fbuf[offsetof(request, buffer) - offsetof(request, buffer_end) + sizeof(((request *)(0))->buffer)];
		request *fakewp;

		fakewp = (request *)(fbuf - offsetof(request, buffer_end));
		fakewp->buffer_end = 0;
		fakewp->buffer = (char *)calloc(128, 1);
		fakewp->max_buffer_size = 128;
		...
		for (i = 0; i < wlan_num; i++) {
			...
			req_format_write(wp, "%s[%d]=%s;\n", "wlanDisabled", wlan_idx, inner_getIndex(fakewp, "wlanDisabled"));
			req_format_write(wp, "%s[%d]=%s;\n", "RFType", wlan_idx, inner_getIndex(fakewp, "RFType"));
			req_format_write(wp, "%s[%d]=%s;\n", "APMode", wlan_idx, inner_getIndex(fakewp, "wlanMode"));
			val = atoi(inner_getIndex(fakewp, "band"));
			if (val > 0)
				val = val - 1;
			req_format_write(wp, "%s[%d]=%d;\n", "bandIdx", wlan_idx, val);
			req_format_write(wp, "%s[%d]=%d;\n", "bandIdxClient", wlan_idx, val);
			req_format_write(wp, "%s[%d]=%s;\n", "networkType", wlan_idx, inner_getIndex(fakewp, "networkType"));
			req_format_write(wp, "%s[%d]=%s;\n", "regDomain", wlan_idx, inner_getIndex(fakewp, "regDomain"));
			req_format_write(wp, "%s[%d]=%s;\n", "defaultChan", wlan_idx, inner_getIndex(fakewp, "channel"));
			req_format_write(wp, "%s[%d]=%s;\n", "usedBand", wlan_idx, inner_getIndex(fakewp, "band"));
			req_format_write(wp, "%s[%d]='%s';\n", "ssid", wlan_idx, inner_getInfo(fakewp, "ssid"));
			req_format_write(wp, "%s[%d]=%s;\n", "encrypt", wlan_idx, inner_getIndex(fakewp, "encrypt"));
			req_format_write(wp, "%s[%d]=%s;\n", "wep", wlan_idx, inner_getIndex(fakewp, "wep"));
			req_format_write(wp, "%s[%d]=%s;\n", "defaultKeyId", wlan_idx, inner_getIndex(fakewp, "defaultKeyId"));
			req_format_write(wp, "%s[%d]=%s;\n", "defPskFormat", wlan_idx, inner_getIndex(fakewp, "pskFormat"));
			req_format_write(wp, "%s[%d]=%s;\n", "macClone", wlan_idx, inner_getIndex(fakewp, "wlanMacClone"));
			req_format_write(wp, "%s[%d]='%s';\n", "wpaCipher", wlan_idx, inner_getIndex(fakewp, "wpaCipher"));
			req_format_write(wp, "%s[%d]='%s';\n", "wpa2Cipher", wlan_idx, inner_getIndex(fakewp, "wpa2Cipher"));
			req_format_write(wp, "%s[%d]='%s';\n", "pskValue", wlan_idx, inner_getInfo(fakewp, "pskValue"));
			req_format_write(wp, "%s[%d]=%s;\n", "keyType", wlan_idx, inner_getIndex(fakewp, "keyType"));
			req_format_write(wp, "%s[%d]=%s;\n", "defWapiAuth", wlan_idx, inner_getIndex(fakewp, "wapiAuth"));
			req_format_write(wp, "%s[%d]=%s;\n", "defWapiPskFormat", wlan_idx, inner_getIndex(fakewp, "wapiPskFormat"));
			req_format_write(wp, "%s[%d]=%s;\n", "defWapiPskValue", wlan_idx, inner_getInfo(fakewp, "wapiPskValue"));
			req_format_write(wp, "%s[%d]=%s;\n", "defWapiASIP", wlan_idx, inner_getInfo(fakewp, "wapiASIp"));
			req_format_write(wp, "%s[%d]=%s;\n", "defWapiCertSel", wlan_idx, inner_getInfo(fakewp, "wapiCertSel"));
			req_format_write(wp, "%s[%d]=%s;\n", "init_bound", wlan_idx, inner_getIndex(fakewp, "ChannelBonding"));
			req_format_write(wp, "%s[%d]=%s;\n", "init_sideband", wlan_idx, inner_getIndex(fakewp, "ControlSideBand"));
			req_format_write(wp, "%s[%d]=%s;\n", "wlanBand2G5G", wlan_idx, inner_getIndex(fakewp, "Band2G5GSupport"));
		}
		free(fakewp->buffer);
		wlan_idx  = wlan_idx_ori;
		vwlan_idx = vwlan_idx_ori;
...
#else
		strcat(tmp, "<option value=\"3\">AP+WDS</option>");
#endif
#else
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_WLAN_CLIENT_MODE)// keith. disabled if no this mode in 96c
...
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_WLAN_WDS_SUPPORT)// keith. disabled if no this mode in 96c
#else
		  strcat(tmp, "<option selected value=\"3\">AP+WDS</option>");
#endif
#else	//DAVO
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_WLAN_CLIENT_MODE)// keith. disabled if no this mode in 96c
#else
...
static int getInfofast(request *wp, int argc, char **argv)
{
	struct ej_handler *handler = ej_find_handler(argv[0]);
	if (handler)
		return handler->output(wp, argc, argv, handler->data);
	return -1;
}

static int getIndexfast(request *wp, int argc, char **argv)
{
	struct ej_handler *handler = ej_find_index(argv[0]);
	if (handler)
		return handler->output(wp, argc, argv, handler->data);
	return -1;
}

static int ejx_apmib_getint32(request *wp, int argc, char **argv, void *mib_id)
{
	int n;
	apmib_get((int)mib_id, (void *)&n);
	req_format_write(wp, "%d", n);
	return 0;
}

EJX_ENTRY_DATA(shortGIEnabled, ejx_apmib_getint32, MIB_WLAN_SHORT_GI);
#endif
