...
{
	return get_clone_mac_by_ip(ip, clone_mac);
}
...
static int arplookup(int fd, struct in_addr tina, u_char thwa[6], const char *ifname)
{
	struct arpreq ar;
	...
	rc = ioctl(fd, SIOCGARP, (caddr_t)&ar);
	if (!rc)
...
static int brif_get_portno(const char *bridge, u_char *mac)
{
	...
	if (fd != -1) {
		while (!port_no) {
			do {
				n = read(fd, &fdb, sizeof(fdb));
			} while (n < 0 && errno == EINTR);
			if (n != sizeof(fdb))
				break;
			if (!fdb.is_local && !memcmp(fdb.mac_addr, mac, 6))
				port_no = fdb.port_no;
		}
		close(fd);
	}
...
static int brif_getbymac(const char *bridge, u_char *mac, char name[IFNAMSIZ])
{
	char path[SYSFS_PATH_MAX];
	glob_t globbuf;
	char *p;
	...
	port_no = brif_get_portno(bridge, mac);
	if (!port_no)
		return -1;

	snprintf(path, sizeof(path), SYSFS_CLASS_NET "%s/brif/*", bridge);
	...
	for (i = 0; (i < globbuf.gl_pathc); i++) {
		...
		if (!p)
			continue;
		snprintf(path, sizeof(path), "%s/port_no", globbuf.gl_pathv[i]);
		yfcat(path, "%x\n", &n);
		if (port_no == n) {
			strlcpy(name, strrchr(globbuf.gl_pathv[i], '/') + 1, IFNAMSIZ);
			break;
		}
	}
	globfree(&globbuf);
	return (port_no == n) ? 0 : -1;
}

static int accept_access_by_dot11sta(int fd, struct in_addr ip)
{
	const char *suffix = "\x00\x31\x30";
	const char *br = "br0";
	u_char mac[6];
	char interface[IFNAMSIZ], stem[IFNAMSIZ];
	int perm;

	perm = ((!!nvram_atoi("wlan1_wlweb_permit", 0)) << 1) | !!nvram_atoi("wlan0_wlweb_permit", 0);
	if (perm != 3) {
		snprintf(stem, sizeof(stem), "wlan%c%c", suffix[perm], '\0');
		if (!arplookup(fd, ip, mac, br) && !brif_getbymac(br, mac, interface))
			return (!strncmp(stem, interface, strlen(stem))) ? -1 : 0;
	}
	return 0;
}
#endif
...
#ifdef __DAVO__
	if (accept_access_by_dot11sta(fd, remote_addr.sin_addr)) {
		close(fd);
...
# endif
	...
	if ((ret = mbedtls_net_accept(&mbedtls_listen_fd, &(req->mbedtls_client_fd), NULL, 0, NULL)) != 0) {
...
# else
		fprintf(stderr, "free upload data..............................%p with %d length\n", req->upload_data,
			req->upload_len);
		fflush(stderr);
...
# endif
...
#ifdef __DAVO__
		/* deep stack caused increase in VmSize? lessen stack usage anyway */
		read(req->fd, req->client_stream, sizeof(req->client_stream));
#else
		char buf[32768];
	...
	total_connections--;
#ifdef __DAVO__
	free(req);	/* To keep small memory footprint */
#else
...
#  ifdef WEB_ONLY_ONE_CLIENT
			if (req->auth_flag == 1) {
				if (webClientStatus[idx].status == NOT_LOGIN) {
					req->auth_flag = 0;
				} else if (webClientStatus[idx].status == TOBE_LOGIN) {
				...
				if (cookie && !strncasecmp(cookie, "acookie", 7)) {
					...
					if (parse_cookie(cookie, &cs) == 0) {
						if (inet_addr(req->remote_ip_addr) == cs.host.s_addr) {
							...
							if (req->userName && !strcmp(req->userName, superName))
								req->superUser = 1;
							else {
								nvram_get_r_def("factory_user", facName, sizeof(facName), "");
								if (req->userName && !strcmp(req->userName, facName))
									req->superUser = 1;
								else
									req->superUser = 0;
							}
