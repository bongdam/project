...
///////////////////////////////////////////////////////////////////////////////
/* rewritten in private.c - 2018-10-22 young */
#ifndef __DAVO__
...
{
	char *strUser, *strPassword, *strConfpass;
	char tmpBuf[100];
	char pre_account[65] = {0,};
	char sha256_user[65];
	char sha256_pass[65];
	char facName[65] = {0,};
	...
	nvram_get_r_def("factory_user", facName, sizeof(facName), "");
	if (wp->userName && !strcmp(wp->userName, facName)) {
		strcpy(tmpBuf, ("변경할 수 없습니다!."));
		goto setErr_pass;
	}

	if (!strUser[0])
	{
		strcpy(tmpBuf, ("오류: 사용자 계정이 비어있습니다."));
		goto setErr_pass;
	}
	...
	if (wp->superUser == 1) {	/* SuperUser Account */
		nvram_get_r_def("USER_NAME", pre_account, sizeof(pre_account), "");
	} else {					/* User Account */
		nvram_get_r_def("SUPER_NAME", pre_account, sizeof(pre_account), "");
	}

	if (!pre_account[0]) {
		strcpy(tmpBuf, ("오류: 계정 MIB 읽기 실패!"));
		goto setErr_pass;
	}

	if (!strcmp(pre_account, sha256_user)) {
		strcpy(tmpBuf, ("오류: 사용자 이름과 관리자 이름이 같습니다."));
		goto setErr_pass;
	}

	if (!strcmp(facName, sha256_user)) {
		strcpy(tmpBuf, ("오류: 사용자 이름과 관리자 이름이 같습니다."));
		goto setErr_pass;
	}

	if (!free_from_login_list(wp)) {
		strcpy(tmpBuf, ("오류: 설정 실패!"));
		goto setErr_pass;
	}

	if (wp->superUser == 1) {	/* SuperUser Account */
		nvram_set("SUPER_NAME", sha256_user);
		nvram_set("SUPER_PASSWORD", sha256_pass);
		nvram_set("touch_superUser", strUser);
		nvram_set("web_rootaccount_touch", "1");
	} else {					/* User Account */
		nvram_set("USER_NAME", sha256_user);
		nvram_set("USER_PASSWORD", sha256_pass);
		nvram_set("touch_User", strUser);
		nvram_set("web_useraccount_touch", "1");
	}
...
//Brad add end
	...
	if (enabled){
		...
		if (tmpStr[0])
			nvram_set("ntp_server_ip1", tmpStr);
		tmpStr = req_get_cstream_var(wp, ("ntpServerIp2"), "");
		if (tmpStr[0])
			nvram_set("ntp_server_ip2", tmpStr);
	}
	return 0;
setErr_ntp:
	return -1;
...
//	int pid;
	...
	if(strVal[0]){
		...
		if (!enabled) {
			...
			tmpStr = req_get_cstream_var(wp, ("cur_date"), "");
			if(tmpStr[0]){
				tm_time.tm_year = atoi(strtok(tmpStr, "/")) - 1900; // year
				tm_time.tm_mon = atoi(strtok(NULL, "/")) - 1; // month
				tm_time.tm_mday = atoi(strtok(NULL, "\n")); // day
			}
			tmpStr = req_get_cstream_var(wp, ("hour"), "");
			if(tmpStr[0]){
				tm_time.tm_hour = atoi(strtok(tmpStr, ":")); // hour
				tm_time.tm_min = atoi(strtok(NULL, ":")); // minute
				tm_time.tm_sec = atoi(strtok(NULL, "\n")); // second
			}

			tm = mktime(&tm_time);
			if(tm < 0){
				snprintf(tmpBuf, sizeof(tmpBuf), "시간 설정 설정하지 못했습니다!\n");
				goto setErr_end;
			}
			if(stime(&tm) < 0){
				snprintf(tmpBuf, sizeof(tmpBuf), "시간 설정 설정하지 못했습니다!!\n");
...
void formLogout(request *wp, char *path, char *query)
{
	...
	return_url = req_get_cstream_var(wp, ("return-url"), "/login.htm");
...
static int WriteBlock(request *req, char *buf, int nChars)
{
#ifdef REFINE_ASP
	return rqst_write(req, buf, nChars);
#else
	int bob = nChars;
#ifndef SUPPORT_ASP
...
static char *get_nowtime(char *val, int sz)
{
	struct tm *t;
	time_t now;
	...
	snprintf(val, sz, "%d_%02d_%02d_%02dh%02dm%02ds", t->tm_year + 1900,
 ...
 */
	...
	if (tmpStr[0]) {
		...
		if (fp == NULL) {
			strcpy(tmpBuf, "System Log not exists!");
			goto setErr;
		}
		...
		req_format_write(wp, "Content-Disposition: attachment;filename=\"dvw2700_log_%s.txt\" \n", get_nowtime(now, sizeof(now)));
...
void formDosCfg(request *wp, char *path, char *query)
{
	...
	if (!strcmp(tmpStr, "ON")) {
		nvram_set("x_pingSecEnabled", "1");
		tmpStr = req_get_cstream_var(wp, ("pingSecCount"), "");
		string_to_dec(tmpStr, &floodCount);
		if (floodCount < 0)
			floodCount = 0;
		sprintf(tmpBuf, "%d", floodCount);
		nvram_set("x_icmp_reply_rate", tmpBuf);
	} else {
		nvram_set("x_pingSecEnabled", "0");
		nvram_set("x_icmp_reply_rate", "0");
	}

	tmpStr = req_get_cstream_var(wp, ("TraceRtEnabled"), "");
	if (!strcmp(tmpStr, "ON")) {
		nvram_set("x_noreply_tracert", "1");
	} else {
		nvram_set("x_noreply_tracert", "0");
	}
...
{
	char buf[strlen(img) + sizeof("/tmp/img/") + sizeof(".gif")];
	snprintf(buf, sizeof(buf), "/tmp/img/%s.gif", img);
	unlink(buf);
}
...
static int is_exist_captcha_img(char *img)
{
	...
	snprintf(buf, sizeof(buf), "/tmp/img/%s.gif", img);
	return (stat(buf, &sb) == 0 && S_ISREG(sb.st_mode)) ? 1 : 0;
}
...
void formLogin(request *wp, char *path, char *query)
{
	...
	char userName[65], userPasswd[65];
	char superName[65], superPasswd[65];
	char facName[65], facPasswd[65];
	char hash_captcha[65], sha256_user[65], sha256_pass[65];
	struct user_info *pUser_info;
	int denied = 1, j;
	int login_session_num = 0;
	char buf[1024], captcha[128];
	char *cookie = NULL;
	struct in_addr peer;
	...
	if (strcmp(userName, sha256_user) == 0) {
		nvram_get_r_def("USER_PASSWORD", userPasswd, sizeof(userPasswd), "");
		if (!userPasswd[0]) {
			denied = 10;
			goto login_err;
		}

		if (strcmp(sha256_pass, userPasswd)) {
			denied = 1;
			goto login_err;
		}
		denied = 0;
	...
	nvram_get_r_def("SUPER_NAME", superName, sizeof(superName), "");
	if (!superName[0]) {
		denied = 10;
		goto login_err;
	}
	...
	if (strcmp(sha256_user, superName) == 0) {
		nvram_get_r_def("SUPER_PASSWORD", superPasswd, sizeof(superPasswd), "");
		if (!superPasswd[0]) {
			denied = 10;
			goto login_err;
		}
	...
	if (strcmp(sha256_user, facName) == 0) {
		nvram_get_r_def("factory_pw", facPasswd, sizeof(facPasswd), "");
		if (!facPasswd[0]) {
			denied = 10;
			goto login_err;
		}

		if (strcmp(sha256_pass, facPasswd)) {
			denied = 1;
			goto login_err;
		}
...
pass_check:
	...
	syslog(LOG_INFO, "HTTP login successfully from %s", wp->remote_ip_addr);
	return;
...
login_err:
	...
	syslog(LOG_INFO, "HTTP login failed from %s", wp->remote_ip_addr);
}
...
//wlan0
	...
	{
		if(wlanStaList[i].aid && (wlanStaList[i].flags & STA_INFO_FLAG_ASOC))
		{
