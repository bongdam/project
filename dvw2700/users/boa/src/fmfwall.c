...
void check_to_dhcplist(char *set_ip);
int check_dup_static_ipaddr(struct in_addr strIp);
void check_to_delete_static_lease(PORTFW_T strIp);
...
void formPortFw(request *wp, char *path, char *query)
{
	...
	if (strAddPort[0]) {
		...
		if ( *((unsigned long *)entry.ipAddr) == (unsigned long)curIpAddr.s_addr) {
		...
		if ( !string_to_dec(strT_from, &intVal) || intVal<1 || intVal>65535) {
			strcpy(tmpBuf, ("오류! 포트 범위가 올바르지 않습니다.."));
			goto setErr_portfw;
		}
		entry.externelFromPort = (unsigned short)fromPort;
		entry.externelToPort =  (unsigned short)toPort;
...
void formFilter(request *wp, char *path, char *query)
{
	...
	if (strAddPort[0]) {
		...
		if (!strFrom[0]) { // if port-forwarding, from port must exist
			strcpy(tmpBuf, ("오류! 포트 범위를 설정해야 합니다."));
			goto setErr_filter;
		}
		if ( !string_to_dec(strFrom, &intVal) || intVal<1 || intVal>65535) {
			strcpy(tmpBuf, ("오류! 포트 범위가 올바르지 않습니다."));
	...
	if (strAddMac[0]) {
		...
		if ( !strVal[0] ) {
			strcpy(tmpBuf, ("오류! mac 주소를 설정해야 합니다."));
			goto setErr_filter;
		}
	...
	if (strAddUrl[0]) {
		...
		if(strUrlMode){
			...
			if ( apmib_set(MIB_URLFILTER_MODE, (void *)&mode) == 0) {
		    	strcpy(tmpBuf, ("오류! URL 필터 사용 설정해야 합니다."));
			    goto setErr_filter;
		    }
        }
		...
		if ( !strVal[0] ) {
			strcpy(tmpBuf, ("오류! url을 설정해야 합니다."));
			goto setErr_filter;
		}
		else
		{
		...
		for(j=1;j<=entryNum;j++)
		{
			...
			*((char *)&urlEntrytmp) = (char)j;
			if ( apmib_get(MIB_URLFILTER_TBL, (void *)&urlEntrytmp))
			{
				...
				if (strlen((char *)urlEntry.urlAddr) == strlen((char *)urlEntrytmp.urlAddr))
				{
					if (!memcmp(urlEntrytmp.urlAddr, urlEntry.urlAddr, strlen((char *)urlEntry.urlAddr)))
					{
						strcpy(tmpBuf, ("동일한 설정이 존재합니다."));
						goto setErr_filter;
					}
			...
			*((char *)&ipentrytmp) = (char)j;
			if ( apmib_get(MIB_IPFILTER_TBL, (void *)&ipentrytmp))
			{
				...
				if (strEndIpAddr[0])
				{
					...
					{
						strcpy(tmpBuf, ("오류! IP 주소가 중복되었습니다."));
						goto setErr_filter;
					}
				}
				else
	...
	if (strAddIp[0] || strAddPort[0] || strAddMac[0] || strAddUrl[0]) {
		...
		if (strAddUrl[0])
		{
			if ( (entryNum + 1) > MAX_URLFILTER_NUM) {
				strcpy(tmpBuf, ("테이블이 모두 차서 더이상 URL을 추가할 수 없습니다!"));
				goto setErr_filter;
			}
		}
		else
...
void formDMZ(request *wp, char *path, char *query)
{
	...
	char tmpBuf[100], dmz_ip[32], tmp[32];
	int intVal = 0, res = 0, stp, entryNum;
	struct in_addr ipAddr;
#ifndef NO_ACTION
	int pid;
#endif
	char buf[128], touch[52];
	...
	if (strSave[0]) {
		...
		if ( !strcmp(strVal, ("dmz"))) {
			intVal = 1;
			tmpBuf[0] = 0;
			nvram_get_r("x_sdmz_enable", tmpBuf, sizeof(tmpBuf));
			if (tmpBuf[0] == '1') {
				nvram_set("x_sdmz_enable", "0");
				syslog(LOG_INFO, "SDMZ Turn off");
			}
...
/* END DAVO */
		if ( apmib_set(MIB_DMZ_ENABLED, (void *)&intVal) == 0) {
			snprintf(tmpBuf, sizeof(tmpBuf), "%s", ("사용 설정 실패!"));
			goto setErr_dmz;
		}

		strVal = req_get_cstream_var(wp, ("ip"), (""));
		if (!strVal[0]) {
			if ( apmib_get(MIB_DMZ_HOST, (void *)tmp) == 0)
				goto setOk_dmz;

			snprintf(dmz_ip, sizeof(dmz_ip), "%s", inet_ntoa(*((struct in_addr *)tmp)));
			inet_aton(dmz_ip, &ipAddr);
			if ( (res = check_dup_static_ipaddr(ipAddr)) != 0 ) {
				DHCPRSVDIP_T delEntry;
				/* find static lease ip. */
				snprintf(buf, sizeof(buf), "webtouch_static_lease%d", res);
				if (!nvram_get_r(buf, touch, sizeof(touch))) {
					*((char *)&delEntry) = (char)res;
					if (!apmib_get(MIB_DHCPRSVDIP_TBL, (void *)&delEntry)) {
						printf("err! static lease tbl read.\n");
						goto setOk_dmz;
					}

					if (!apmib_set(MIB_DHCPRSVDIP_DEL, (void *)&delEntry)) {
						printf("err! static lease tbl delete.\n");
						goto setOk_dmz;
					}

					if (!apmib_get(MIB_DHCPRSVDIP_TBL_NUM, (void *)&entryNum)) {
						printf("err! static lease tbl num read.\n");
						goto setOk_dmz;
					}
					...
					if (entryNum == 0) {
						...
						if (!apmib_set(MIB_DHCPRSVDIP_ENABLED, (void *)&stp)) {
							printf("err! static lease enable.\n");
							goto setOk_dmz;
						}
					}
				}
		...
		if ( apmib_set(MIB_DMZ_HOST, (void *)&ipAddr) == 0) {
			snprintf(tmpBuf, sizeof(tmpBuf), "%s", ("DMZ 설정 실패!"));
			goto setErr_dmz;
		}

		if (intVal == 1)
			check_to_dhcplist(strVal);
	}
setOk_dmz:
	apmib_update_web(CURRENT_SETTING);
	OK_MSG("/dmz.htm");
...
int portFwList(request *wp, int argc, char **argv)
{
	...
	char t_portRange[20], comment[32];
	...
	for (i=1; i<=entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		if ( entry.externelToPort == 0)
			strcpy(portRange, "----");
		else if ( entry.externelFromPort == entry.externelToPort )
			snprintf(portRange, 20, "%d", entry.externelFromPort);
		else
			snprintf(portRange, 20, "%d-%d", entry.externelFromPort, entry.externelToPort);

		if ( entry.fromPort == 0)
			strcpy(t_portRange, "----");
		else if ( entry.fromPort == entry.toPort )
			snprintf(t_portRange, 20, "%d", entry.fromPort);
		else
			snprintf(t_portRange, 20, "%d-%d", entry.fromPort, entry.toPort);
        snprintf(comment, sizeof(comment), "%s", entry.comment);
		if (comment[0])
...
int portFilterList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"30%%\"><font size=\"2\"><b>포트 범위</b></font></td>\n"
      	"<td align=center width=\"25%%\"><font size=\"2\"><b>프로토콜</b></font></td>\n"
		"<td align=center width=\"30%%\" ><font size=\"2\"><b>설명</b></font></td>\n"
      	"<td align=center width=\"15%%\" ><font size=\"2\"><b>삭제</b></font></td></tr>\n"));
	...
	for (i = 1; i <= entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
			...
			"<input type=\"submit\" value=\"삭제\" id=\"deleteSelFilterPort%d\" name=\"deleteSelFilterPort\" onClick=\"return deleteClick(%d);\"></td>\n"
			"</tr>\n"),
			portRange, type, entry.comment, i, i);
	}
...
int ipFilterList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"30%%\" ><font size=\"2\"><b>로컬 IP 주소</b></font></td>\n"
      	"<td align=center width=\"25%%\" ><font size=\"2\"><b>프로토콜</b></font></td>\n"
      	"<td align=center width=\"25%%\" ><font size=\"2\"><b>설명</b></font></td>\n"
	...
	for (i = 1; i <= entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
				...
				"<input type=\"submit\" value=\"삭제\" id=\"deleteSelFilterIp%d\" name=\"deleteSelFilterIp\" onClick=\"return deleteClick(%d)\"></td>\n"
				"</tr>\n"),
				ip, type, entry.comment, i, i);
	}
...
void add_static_dhcp_lease(char *ipaddr, char *mac)
{
	int entryNum, stp, res = 0;
	struct in_addr inIp;
	...
	stp = 1;
	if (!apmib_set(MIB_DHCPRSVDIP_ENABLED, (void *)&stp)) {
		printf("err! static lease using.\n");
		goto setac_ret;
	}

	memset(&staticIPEntry, '\0', sizeof(staticIPEntry));

	snprintf(stamac, sizeof(stamac), "%c%c%c%c%c%c%c%c%c%c%c%c", mac[0], mac[1], mac[3], mac[4],
		mac[6], mac[7], mac[9], mac[10], mac[12], mac[13], mac[15], mac[16]);

	inet_aton(ipaddr, &inIp);
	memcpy(staticIPEntry.ipAddr, &inIp, 4);

	if (strlen(stamac) != 12 || !string_to_hex(stamac, staticIPEntry.macAddr, 12))
		goto setac_ret;

	if ( (res = check_dup_static_info(staticIPEntry.macAddr, inIp))!= 0 ) {
		if (res == 1)
			printf("error! duplication station mac.\n");
		else
			printf("error! duplication station ip.\n");

		goto setac_ret;
	}

	if (!apmib_get(MIB_DHCPRSVDIP_TBL_NUM, (void *)&entryNum)) {
		printf("entryNum Read Fail.\n");
		goto setac_ret;
	}
...
void check_to_dhcplist(char *set_ip)
{
	...
	while (1) {
		ret = getOneDhcpClient(&ptr, (unsigned long*)&status.st_size, ipAddr, macAddr, liveTime, ts.tv_sec);
		...
		if (strcmp(set_ip, ipAddr) == 0)
			add_static_dhcp_lease(set_ip, macAddr);
	}
...
int check_dup_static_ipaddr(struct in_addr strIp)
{
	...
	for (i = 1; i <= entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		if (!memcmp(entry.ipAddr, &strIp, sizeof(struct in_addr)))
			return i;
	}
...
void check_to_delete_static_lease(PORTFW_T strIp)
{
	...
	if ( !apmib_get(MIB_PORTFW_TBL_NUM, (void *)&entryNum)) {
		printf("err! portfw tbl num read fail.\n");
		goto setErr_delstp;
	}
	...
	for (i = entryNum; i > 0; i--) {
		...
		*((char *)&entry) = (char)i;
		if ( !apmib_get(MIB_PORTFW_TBL, (void *)&entry)) {
			printf("err! portfw tbl read fail.\n");
			goto setErr_delstp;
		}

		if (!memcmp(strIp.ipAddr, entry.ipAddr, 4))
			exist_ip = 1;
	}

	if (exist_ip == 0) {
		if ( (res = check_dup_static_ipaddr(*((struct in_addr*)strIp.ipAddr))) != 0 ) {
			/* find static lease ip. */
			snprintf(buf, sizeof(buf), "webtouch_static_lease%d", res);
			if (!nvram_get_r(buf, touch, sizeof(touch))) {
				*((char *)&delEntry) = (char)res;
