...
//		if( !strcmp(req->request_uri, WEB_PAGE_FWUPGRADE)) {
...
#ifdef __DAVO__
    n = readlink(req->pathname, buffer, sizeof(buffer) - 1);
    if (n > 0) {
        char *dname, *pathc;

        buffer[n] = '\0';
        pathc = (buffer[0] != '/') ? strdup(req->pathname) : NULL;

        free(req->pathname);
        if (pathc) {
            dname = dirname(pathc);
            asprintf(&req->pathname, "%s%s%s",
                     dname, (dname[0] == '/' && dname[1] != '\0') ? "/" : "" , buffer);
            free(pathc);
        } else
            req->pathname = strdup(buffer);

        pathc = strdup(req->request_uri);
        dname = dirname(pathc);
        snprintf(req->request_uri, sizeof(req->request_uri), "%s%s%s",
            dname, (dname[0] == '/' && dname[1] != '\0') ? "/" : "", basename(buffer));
        free(pathc);
    }
#endif
    ...
    if (strcmp(CGI_MIME_TYPE, get_mime_type(req->pathname)) == 0) { /* cgi */
		...
		if (req->request_uri && strstr(req->request_uri, "mfgtst.cgi")) {
			...
			snprintf(&request_uri_tmp[0], (MAX_HEADER_LENGTH + 1), "/boafrm/formMfgTest?%s", &req->query_string[0]);
