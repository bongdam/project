...
static const char* user_deny_weblist[] = {
	"/dos.htm",
	"/qosacl.htm",
	"/qosque.htm",
	"/qosremark.htm",
	...
	"/snmp.htm",
	"/sub_menu_qos.htm",
	"/connect_VoIP_Client.htm"
	"/navigation.js",
	NULL
};

static void
log_boaform_act(const void *nodep, const VISIT which, const int depth)
{
	if (which == postorder || which == leaf) {
		char *p = *(char **)nodep;
		log_boaform(p, (request *)strtoul(p + strlen(p) + 1, NULL, 0));
	}
}
...
int init_get2(request * req)
{
	...
	if (S_ISDIR(statbuf.st_mode)) {
		...
		if (req->pathname[strlen(req->pathname) - 1] != '/') {
			...
			if (server_port != 80)
				asprintf(&buffer, "http://%s:%d%s/", req->host ? req->host : server_name, server_port,
				        req->request_uri);
			else
				asprintf(&buffer, "http://%s%s/", req->host ? req->host : server_name, req->request_uri);
...
//	req->last_modified = statbuf.st_mtime;
	...
	if (!req->cookie || (!pUser_info) || (strcmp(pUser_info->uniq_cookie, req->cookie))) {
		if (strcmp(req->request_uri, "/login.htm") &&
		    strcmp(req->request_uri, "/formLogin") &&
		    strcmp(req->request_uri, "/util_gw.js")) {
			send_redirect_perm(req, "/login.htm");
			return 0;
		}
	}
	...
	body_start = req->buffer_end;
	//parse and send asp page
	last_right = req->data_mem;

	while (1) {
		left = memmem(last_right, req->filesize - (last_right - req->data_mem), "<%", 2);
		if (left)
			right = memmem(left, req->filesize - (left - req->data_mem), "%>", 2);
		if (left && right) {
			i = rqst_write(req, last_right, left - last_right);
			if (i < 0)
				break;
			last_right = right + 2;
			req->filepos = last_right - req->data_mem;
			handleScript(req, left, right);
		} else {
			i = rqst_write(req, last_right, req->filesize - (last_right - req->data_mem));
			if (i >= 0)
				req->filepos += i;
			break;
		}
	...
	do {
		...
		for (p = req->buffer + body_start;
		     (p = memmem(p, req->buffer_end - (p - req->buffer), "boafrm/", 7));
		     p += strlen(name)) {
			p += 7;
			if (sscanf(p, "%63[^\?\'\"\x20\x09]", name) > 0) {
				asprintf(&q, "%s%c0x%lx", name, '\0', (unsigned long)req);
				val = tsearch(q, &root, (void *)strcmp);
				if (*(char **)val != q)
					free(q);
			}
		}
		twalk(root, log_boaform_act);
		tdestroy(root, free);
	} while (0);
#endif
...
int init_get(request * req)
{
	...
	if (S_ISDIR(statbuf.st_mode)) { /* directory */
		...
		if (req->pathname[strlen(req->pathname) - 1] != '/') {
			unsigned int len;
			char *buffer = alloca(3 * MAX_PATH_LENGTH + 128);
