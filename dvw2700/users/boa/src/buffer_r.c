...
{
	if (!msg || !msg[0] || req->status > DONE)
		return req->buffer_end;
	req_format_write(req, "%s", msg);
	return req->buffer_end;
}
...
int req_write_escape_http(request *req, const char *msg)
{
	...
	while ((c = *msg++) && rc > -1) {
		if (needs_escape((unsigned int)c)) {
			buf[1] = INT_TO_HEX((c >> 4) & 0xf);
			buf[2] = INT_TO_HEX(c & 0xf);
			rc = rqst_write(req, buf, 3);
		} else
			rc = rqst_putc(req, c);
	}

	return (rc > -1) ? req->buffer_end : ({ req->status = DEAD; -1; });
}
	...
	if (bytes_to_write) {
...
#endif
		{
			...
			if (req->mbedtls_client_fd.fd != -1) {
				while ((bytes_written = mbedtls_ssl_write(&(req->mbedtls_ssl_ctx),
				                        req->buffer + req->buffer_start, bytes_to_write)) <= 0) {
