...
#ifdef __DAVO__
extern int init_nvram(unsigned int flash_size);
extern int nvram_commit(void);
extern int nvram_backup(void);
extern char *nvram_get(const char *name);
extern off_t p_nvram_offset, b_nvram_offset;
extern const int max_nvram_space;
...
//-------------------------------------------------------
	...
	/* set pin mux as gpio */
	RTL_W32(BSP_RESET_BTN_PIN_MUX_SEL, (RTL_R32(BSP_RESET_BTN_PIN_MUX_SEL)
					    & ~(BSP_RESET_BTN_PIN_MASK) | (BSP_RESET_BTN_PIN_MUX)));
	/* turn on gpio pin */
	RTL_W32(BSP_GPIO_CNR_REG(BSP_RESET_BTN_PIN), (RTL_R32(BSP_GPIO_CNR_REG(BSP_RESET_BTN_PIN))
						      & (~(1 << BSP_GPIO_BIT(BSP_RESET_BTN_PIN)))));
	/* set gpio as input */
	RTL_W32(BSP_GPIO_DIR_REG(BSP_RESET_BTN_PIN), (RTL_R32(BSP_GPIO_DIR_REG(BSP_RESET_BTN_PIN))
						      & (~(1 << BSP_GPIO_BIT(BSP_RESET_BTN_PIN)))));
#endif
...
#endif
#ifdef __DAVO__
# if defined(CONFIG_RTL_FLASH_DUAL_IMAGE_ENABLE) || defined(CONFIG_RTK_BOOTINFO_SUPPORT)
#error Image-dualizing MUST be revised!
# endif
	memset(&setting_header, 0, sizeof(setting_header));
	if (!flash_capacity || init_nvram(flash_capacity))
		dprintf("Could not find NVRAM.\n");
	else if ((p_nvram_offset && (p_nvram_offset != (flash_capacity - max_nvram_space))) ||
		 (!p_nvram_offset && b_nvram_offset)) {
		if (p_nvram_offset)
			dprintf("Relocating NVRAM...");
		else
			dprintf("Recovering NVRAM from the Secondary...");
		nvram_commit();
		dprintf("done\n");
	} else if (p_nvram_offset && !nvram_backup())
		dprintf("NVRAM's been copied to the Secondary!\n");

	parse_bootline(&setting_header.kern_offset, &setting_header.root_offset);
#endif
	return_addr = 0;
...
# endif
	...
	REG32(0xb8000010) = REG32(0xb8000010) | (1 << 11);
	prom_printf("%s", ((*(volatile unsigned int *)(0xb8000008)) & (0x1 << 22))
	...
	REG32(0xb8000010) = REG32(0xb8000010) | (1 << 11);
#ifdef BT_MAJOR
	prom_printf("\n---Realtek RTL8197F boot code at %s v%c.%c%c (%dMHz)\n", BOOT_CODE_TIME, BT_MAJOR, BT_MINOR, BT_BUILD, cpu_speed);
#else
