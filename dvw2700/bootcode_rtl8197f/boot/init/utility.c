...
#    ifndef CONFIG_NAND_FLASH_BOOTING
		memcpy((void *)(&rootfs_padding) + sizeof(rootfs_padding.zero_pad),
		       (void *)(mips_io_port_base + addr + length - SIZE_OF_CHECKSUM),
		       sizeof(struct _rootfs_padding) - sizeof(rootfs_padding.zero_pad));
#    else
		/* nand */
		goto SKIP_CHECK_BURN_SERIAL;
#    endif
...
#  ifndef CONFIG_NAND_FLASH_BOOTING
		...
		if (ret == 0)
			ret = check_rootfs_image((unsigned long)FLASH_BASE + ROOT_FS_OFFSET + ROOT_FS_OFFSET_OP1 +
					       ROOT_FS_OFFSET_OP2 + bank_offset);
#  else
		ret = 0;
#  endif
#  ifdef CONFIG_RTL_FLASH_MAPPING_ENABLE
...
#   ifndef CONFIG_NAND_FLASH_BOOTING
			...
			if (ret == 0)
				ret = check_webpage_image((unsigned long)FLASH_BASE + WEBPAGE_OFFSET + WEBPAGE_OFFSET_OP1 +
							bank_offset);
			if (ret == 0)
				ret = check_webpage_image((unsigned long)FLASH_BASE + WEBPAGE_OFFSET + WEBPAGE_OFFSET_OP1 +
							WEBPAGE_OFFSET_OP2 + bank_offset);
#   endif
...
unsigned long sel_burnbank_offset()
{
	...
	if (((boot_bank == BANK1_BOOT) && (bank_mark != FORCEBOOT_BANK_MARK)) || ((boot_bank == BANK2_BOOT) && (bank_mark == FORCEBOOT_BANK_MARK)))	//burn to bank2
...
#   ifndef CONFIG_NAND_FLASH_BOOTING
#    define DST_BUFFER_ADDR	0x80800000
					// place to put flash read image
#   else
#    define DST_BUFFER_ADDR	0xa0800000
					// place to put flash read image
#   endif
...
#   ifndef CONFIG_NAND_FLASH_BOOTING
			...
			printf("Flash Program from %X to %X with %X bytes ?\n", DST_BUFFER_ADDR,
			       src_addr - CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET, length);
...
#   endif
		...
		if (Get_GPIO_SW_IN()) {	// button pressed
#   if defined(UTILITY_DEBUG)
...
void sdcard_booting(void)
{
	...
	unsigned long image_header =
	    (int)(addr[0]) * 0x1000000 + (int)(addr[1]) * 0x10000 + (int)(addr[2]) * 0x100 + (int)(addr[3]);
	if (image_header == LINUX_HEADER) {
		unsigned long image_mem =
		    (int)(addr[4]) * 0x1000000 + (int)(addr[5]) * 0x10000 + (int)(addr[6]) * 0x100 + (int)(addr[7]);
...
# if !defined(CONFIG_NONE_FLASH)
#  ifdef CONFIG_NAND_FLASH_BOOTING
	ret = nflashread(pheader->startAddr | 0x20000000, (unsigned int)(addr - FLASH_BASE + sizeof(IMG_HEADER_T)),
		       pheader->len - 2, 0);
#  else
	ret = flashread(pheader->startAddr | 0x20000000, (unsigned int)(addr - FLASH_BASE + sizeof(IMG_HEADER_T)),
		      pheader->len - 2);
#  endif
# endif
