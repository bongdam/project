...
//dprintf("send TFTP-RRQ request\n");
	...
	prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_SERVER].node, (unsigned char *)&tftp_tx,
		      (unsigned short)sizeof(struct iphdr) + sizeof(struct udphdr) + length + 2);
}
...
# endif
		...
		prepare_txpkt(0, FRAME_ARP, arptable_tftp[TFTP_CLIENT].node, (unsigned char *)&arpreply,
...
int imageFileValid(unsigned long startAddr, int len)
{
	...
	while ((head_offset + sizeof(IMG_HEADER_T)) < len) {
		...
		if (skip_check_signature || memcmp(Header.signature, WEB_SIGNATURE, 3)) {
			//calculate checksum
...
# ifdef __DAVO__
	unsigned int kern_off = 0, fs_off = 0;
# endif
...
# endif				// defined(RTL8198)
		...
		} else {	//web page use different checksum algorimth
		...
		if ((burnLen % 0x1000) == 0)	//mean 4k alignemnt
		{
			...
			if (((*((unsigned char *)(startAddr + burnLen))) == 0xde) && ((*((unsigned char *)(startAddr + burnLen + 1))) == 0xad) && ((*((unsigned char *)(startAddr + burnLen + 2))) == 0xc0) && ((*((unsigned char *)(startAddr + burnLen + 3))) == 0xde))	//wrt jffs2 endof mark
			{
...
# if CHECK_BURN_SERIAL
		if (!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) ||
			...
			BDBG_BSN("[%s:%d] burn_serial=0x%08x, linux_imghdr.burnAddr=0x%08x\n", __FUNCTION__, __LINE__,
				 burn_serial, linux_imghdr.burnAddr);
			...
			if (!memcmp(Header.signature, ROOT_SIGNATURE, SIG_LEN)) {
				...
				if (sblk.s_magic != SQUASHFS_MAGIC && sblk.s_magic != SQUASHFS_MAGIC_SWAP) {
					...
					memcpy((void *)(&rootfs_padding.chksum), (void *)(srcAddr + burnLen - SIZE_OF_CHECKSUM),
					       SIZE_OF_CHECKSUM);
					...
					rootfs_padding.len = sblk.mkfs_time = burn_serial =
					    board_rootfs_length - SIZE_OF_SQFS_SUPER_BLOCK - SIZE_OF_CHECKSUM;
...
#  ifdef SUPPORT_SPI_MIO_8198_8196C
		if (Header.burnAddr + burn_offset + burnLen > spi_flash_info[0].chip_size) {
			if (spi_flw_image_mio_8198(0, Header.burnAddr + burn_offset, srcAddr,
			     spi_flash_info[0].chip_size - (Header.burnAddr + burn_offset))
			    && spi_flw_image_mio_8198(1, 0,
						      srcAddr + spi_flash_info[0].chip_size - (Header.burnAddr + burn_offset),
						      Header.burnAddr + burn_offset + burnLen - spi_flash_info[0].chip_size))
				trueorfalse = 1;
		} else {
...
#   if CHECK_BURN_SERIAL
				if (burn_serial == 0)
					...
					if (!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) ||
					    !memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN)) {
						if (spi_flw_image_mio_8198(0, Header.burnAddr + burn_offset + burnLen, &burn_serial,
						     sizeof(burn_serial)))
							trueorfalse = 1;
					} else if (!memcmp(Header.signature, ROOT_SIGNATURE, SIG_LEN)) {
						if (padding_len) {
							BDBG_BSN("[%s:%d] pad\n", __FUNCTION__, __LINE__);
							if (spi_flw_image_mio_8198(0, Header.burnAddr + burn_offset + burnLen - SIZE_OF_CHECKSUM,
							     (void *)(&rootfs_padding), sizeof(rootfs_padding)))
								trueorfalse = 1;
						} else {
							BDBG_BSN("[%s:%d] nopad\n", __FUNCTION__, __LINE__);
							if (spi_flw_image_mio_8198(0, Header.burnAddr + burn_offset + burnLen - SIZE_OF_CHECKSUM,
							     (void *)(&rootfs_padding) + sizeof(rootfs_padding.zero_pad),
							     sizeof(rootfs_padding) - sizeof(rootfs_padding.zero_pad)))
								trueorfalse = 1;
						}
					}
				}
...
#  if CHECK_BURN_SERIAL
		...
		if (!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) ||
			...
			if (memcmp(Header.burnAddr + burn_offset + burnLen + (unsigned long)FLASH_BASE, &burn_serial,
			     sizeof(burn_serial)) != 0) {
				printf("[%s]%d: linux image burn fail!\n", __func__, __LINE__);
				memset(Header.signature, 0, SIG_LEN);
				spi_flw_image_mio_8198(0, Header.burnAddr, &Header, sizeof(Header));
				trueorfalse = 0;
			...
			if (memcmp(Header.burnAddr + burn_offset + (unsigned long)FLASH_BASE, srcAddr,
			     burnLen - SIZE_OF_CHECKSUM) != 0) {
			...
			if (padding_len) {
				BDBG_BSN("[%s:%d] pad\n", __FUNCTION__, __LINE__);
				if (memcmp(Header.burnAddr + burn_offset + burnLen - SIZE_OF_CHECKSUM + (unsigned long)FLASH_BASE,
				     (void *)(&rootfs_padding), sizeof(rootfs_padding)) != 0) {
					printf("[%s]%d: root image burn fail!\n", __func__, __LINE__);
					memset(Header.signature, 0, SIG_LEN);
					spi_flw_image_mio_8198(0, Header.burnAddr, &Header, sizeof(Header));
					trueorfalse = 0;
...
# ifdef __DAVO__
			if (!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) ||
			    !memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN))
				kern_off = Header.burnAddr + burn_offset;
			else if (!memcmp(Header.signature, ROOT_SIGNATURE, SIG_LEN))
				fs_off = Header.burnAddr + burn_offset;
# endif
...
# ifdef __DAVO__
	if (kern_off && fs_off) {
		...
		SprintF(bootline, "x_sys_bootm=0x%x,0x%x", kern_off, fs_off);
		nvram_put(bootline);
		nvram_commit();
	}
# endif
...
char *get_command_one(char *buffer, unsigned long total_length, unsigned int *i)
{
	...
	while (*i < total_length) {
		...
		if (!first && (*i > 0) && ((*(buffer + *i) == '\n') || (*i == (total_length - 1)))) {
			if (!memcmp(command_one, (char *)sign_tbl_command[0].signature, sign_tbl[0].sig_len)
			    && j > sign_tbl[0].sig_len) {
...
void tftpd_send_ack(unsigned short number)
{
	...
	if (tftp_client_enabled)
		prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_SERVER].node, (unsigned char *)&tftp_tx,
			      (unsigned short)sizeof(struct iphdr) + sizeof(struct udphdr) + 4);
	else
#endif
		prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_CLIENT].node, (unsigned char *)&tftp_tx,
			      (unsigned short)sizeof(struct iphdr) + sizeof(struct udphdr) + 4);
}
...
void tftpd_send_data(char *filename, unsigned short block_number)
{
 ...
 /********************************************/
	...
	prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_CLIENT].node, (unsigned char *)&tftp_tx,
		      (unsigned short)sizeof(struct iphdr) + sizeof(struct udphdr) + length + 4);
}
