/* $Id: telnetd.c,v 1.4 2012-02-09 00:48:08 hmpark Exp $
 *
...
#define TELNET_CONF "/var/telnetd.conf"
...
#if !defined(CONFIG_FEATURE_TELNETD_INETD) && defined(__DAVO__)
	long rdtimeo;		/* APNRTL-240 */
#endif
};
...
#define TELNET_PID_PATH "/var/run/telnetd.pid"

static unsigned short login_peer(int fd, char *str, size_t siz)
{
	...
	*str = '\0';
	su.s = (struct sockaddr *)&addr;
	if (!getpeername(fd, su.s, &len)) {
		switch (addr.ss_family) {
		case AF_INET:
			inet_ntop(AF_INET, &su.sin->sin_addr, str, siz);
			return su.sin->sin_port;
		case AF_INET6:
			inet_ntop(AF_INET6, &su.sin6->sin6_addr, str, siz);
...
static void hup(int signum)
{
	(void)signum;

#if defined(CONFIG_OEM_CJHV)
	syslog(LOG_INFO, "telnet 멈춤");
#endif
	exit(EXIT_SUCCESS);
}
...
//APNRTL-240 
//@note: add timeout
make_new_session(int sockfd, int timeout, char *login_ip)
#endif /* CONFIG_FEATURE_TELNETD_INETD */
{
	...
	if (session_count >= MAX_SESSION_COUNT) {
		syslog(LOG_ERR, "Max session count reached!");
		return 0;
	}
	...
	//yfecho("/dev/console", O_WRONLY, 0644, "%s():%d   pass:%d\n", __FUNCTION__, __LINE__, nvram_get_r_def("SUPER_PASSWORD", root_pw, sizeof(root_pw), "nobody"));
	if ((pid = fork()) < 0) {
...
main(int argc, char **argv)
{
...
#if defined(CONFIG_OEM_CJHV)
	char tmp_port[64];
	int portnbr = atoi(nvram_get_r_def("telnet_port", tmp_port, sizeof(tmp_port), "2323"));
#else
	int portnbr = atoi(nvram_get_r_def("telnet_port", tmp_port, sizeof(tmp_port), "6000"));
#endif
#endif /* CONFIG_FEATURE_TELNETD_INETD */
	int c;
#if !defined(CONFIG_FEATURE_TELNETD_INETD) && defined(__DAVO__)
	int timeout = 0, opmode;
#endif
	int pid;
	...
	if ((pid = test_pid(TELNET_PID_PATH)) > 0) {
		kill(pid, SIGKILL);
		unlink(TELNET_PID_PATH);
		fprintf(stderr, "kill existed telnetd(%d)\n", pid);
	}
	for (;;) {
...
#if defined(__DAVO__)
	if (yfcat("/var/sys_op", "%d", &opmode) == 1 && opmode == 0)
		sa.sin_addr.s_addr = inet_addr(nvram_get("IP_ADDR") ? : "0.0.0.0");
#endif
...
#endif
	ywrite_pid(TELNET_PID_PATH);
	maxfd = master_fd;
#endif				/* CONFIG_FEATURE_TELNETD_INETD */

#if !defined(CONFIG_FEATURE_TELNETD_INETD) && defined(__DAVO__)
	yfcat(TELNET_CONF, "%*s %d", &timeout);
	if (timeout <= 60)
		timeout = 60;
#endif

#if defined(CONFIG_OEM_CJHV)
	syslog(LOG_INFO, "telnet 시작");
#endif
	...
	do {
		...
		long diff, min_sec = INT_MAX;
		long current = dtimeup();
#endif
		FD_ZERO(&rdfdset);
		FD_ZERO(&wrfdset);
		...
		if (FD_ISSET(master_fd, &rdfdset)) {
			...
			if ((fd = accept(master_fd, (struct sockaddr *)&sa,
				...
				/* Create a new session and link it into
				   our active list.  */
	//yfecho("/dev/console", O_WRONLY, 0644, "%s():%d   pass:%d\n", __FUNCTION__, __LINE__, nvram_get_r_def("SUPER_PASSWORD", root_pw, sizeof(root_pw), "nobody"));
				struct tsession *new_ts = make_new_session(fd, timeout, inet_ntoa((struct in_addr)sa.sin_addr));
				if (new_ts) {
					...
					/*
					   if (session_count >= MAX_SESSION_COUNT) {
					   #define  MAX_SESSION_REACHED "\007\007\007Max session count reached!\r\n"
					   write(fd, MAX_SESSION_REACHED, strlen(MAX_SESSION_REACHED));
					   }
					 */
					close(fd);
				}
