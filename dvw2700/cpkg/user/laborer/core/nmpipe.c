...
static ssize_t readchar(int fd, char *ptr)
{
 ...
 again:
		...
		default:
			read_cnt = safe_read(fd, read_buf, sizeof(read_buf));
			if (read_cnt > 0)
				break;
			else if (read_cnt == 0)
				return 0;
...
int fifo_sendreq(const char *command)
{
	int rc, server = open(Laborer, O_WRONLY | O_NONBLOCK);
...
struct nmpipe *prequest(const char *command)
{
	...
	snprintf(p->path, sizeof(p->path), "/var/served_%d", getpid());
