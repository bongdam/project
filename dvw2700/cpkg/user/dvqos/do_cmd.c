...
/* rtl865x_asicL2.c: rtl8651_setAsicDot1qAbsolutelyPriority */
...
int patch_wfq_rate(int port, unsigned int val)
{
	...
	phystat = switch_port_status(port);

	if (!val) {
		val = 0xfffff;		/* full speed */
		if (phystat & PHF_LINKUP) {
			if (phystat & PHF_10M)
				val = 1 KB * 10;
	...
	write_qos_reg(QOS_WFQRCRP0 + (12 * port), val);
	return 1;
}
...
int qos_port_rate(int port, char *cmd, int rate_1k)
{
	...
	if (cmd[0] == 'i') {
		...
		val = (unit & 0x0000ffff) << ((port & 1) * 16);	// val==0 use line rate
		set_qos_reg32(reg, ~mask, val);
#ifdef CONFIG_RTL_8197F
		do {
			int rate_high = unit >> 16;

			mask = 0xf << (port << 2);
			val = (rate_high & 0xf) << (port << 2);
			set_qos_reg32(QOS_IBCR3, ~mask, val);
		} while (0);
#endif
	} else {
		// egress (WFQ rate control)
		patch_wfq_rate(port, unit);
	}
	return 1;
}
...
static int qos_port_q(int port, int q, int type, int avg_64k, int weight, int peak, int l1)
{
	...
	q = conv_sys_q(q);
	/* rtl865x_asicL2.c: rtl8651_setAsicQueueRate */
	if (avg_64k == 0)
		avg_64k = 0xfffff;
	v = (peak & 0x7) << 28;
	v |= (l1 & 0xff) << 20;
	...
	/* rtl865x_asicL2.c: rtl8651_setAsicQueueWeight */
...
int do_port(int argc, char *argv[])
{
	...
	if ((strcmp(argv[1], "enable") == 0) || (strcmp(argv[1], "disable") == 0)) {
		...
		if (argc < 3)
			return 0;
		rate = strtoul(argv[2], NULL, 0);	// unit 1k
...
static int qos_port_remark(int port_bits, char *cmd, int mval[8])
{
	...
	if (cmd[0] == '1') {
		/* rtl865x_asicL2.c: rtl8651_setAsicVlanRemark */
		m = 0x07;
		v = (port_bits ? 0x07 : 0);
		...
		/* rtl865x_asicL2.c: rtl8651_setAsicDscpRemark */
		m = 0x07 << 3;
...
void qos_port_apply(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (strcmp(v, "0")) {	//0 = disable, 1=enable
			...
			nvram_get_r_def(n, v, sizeof(v), "0");
			qos_port_rate(i, "inrate", atoi(v));

			sprintf(n, "x_QOS_RATE_O_%d", i);
			nvram_get_r_def(n, v, sizeof(v), "0");
			qos_port_rate(i, "outrate", atoi(v));
		} else {
