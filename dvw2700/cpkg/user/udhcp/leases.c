				  ...
				  unsigned long lease)
{
	...
	oldest = find_lease_by_chaddr(chaddr);
	if (oldest)
		VoIP_Device = oldest->VoIP_Device;
	...
	oldest->VoIP_Device = VoIP_Device;
...
#else
	addr = ntohl(server_config.start)
		+ (RAND() % (1 + ntohl(server_config.end) - ntohl(server_config.start)));
#endif
	stop = addr;
...
int check_ip(u_int32_t addr, const u_int8_t *excluded_chaddr)
{
	...
	if (arpping(addr, M, server_config.server,
		...
		if (excluded_chaddr && !memcmp(excluded_chaddr, M, 6))
			return 0;
		LOG(LOG_INFO, "DHCPD IP " NQF " ", NIPQUAD(addr));
