...
#if !defined(CONFIG_OPENWRT_SDK) && !defined(CONFIG_OEM_CJHV)
...
static bool in_mcast_reserved(const __be32 daddr);
static bool test_mcast_reserved(__u8 *p);
...
int  rtl_MulticastRxCheck(struct sk_buff *skb,rtl_nicRx_info *info,int mCastFlag)
{
	...
	int pid=info->pid;
#ifdef __DAVO__
	u32 forbidden_pmask = 0;
#endif

#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
#ifdef __DAVO__
	if (info->vid == 0 && info->passthru_pkt)
		vid = info->vid = PASSTHRU_VLAN_ID;
#endif
...
#ifdef IFF_IPV6_PASSTHRU
	if (skb->dev->priv_flags & IFF_IPV6_PASSTHRU)
#else
...
//#endif
		...
		if(iph!=NULL)
		{
			...
			{
#ifdef __DAVO__
				if (rtl865x_curOpMode == BRIDGE_MODE && in_mcast_reserved(iph->daddr)) {
					if (pid == PRTNR_WAN0)
						return -100;
					else
						forbidden_pmask = (1 << PRTNR_WAN0);
				}
#endif
                ...
                #endif
                #endif
				re865x_relayTrappedMCast( skb, vid, vlanRelayPortMask & ~forbidden_pmask, TRUE);
...
#endif
	{
		...
		if(ipv6h!=NULL)
		{
			...
			{
#ifdef __DAVO__
				if ((rtl865x_curOpMode == BRIDGE_MODE || info->passthru_pkt) && in6_mcast_reserved(&ipv6h->daddr)) {
					if (pid == PRTNR_WAN0)
						return -100;
					else
						forbidden_pmask = (1 << PRTNR_WAN0);
		...
		if(ret == 1)
        {
			if(skb1)
			{
				...
				#endif
				{
					...
					#if defined (CONFIG_RTL_IGMP_SNOOPING)||defined(CONFIG_BRIDGE_IGMP_SNOOPING)
#ifdef __DAVO__
					if (rtl_MulticastRxCheck(skb1, &info1, mCastFlag) == -100) {
						dev_kfree_skb_any(skb);
						return;
					}
#else
...
#else
	do {
		unsigned int portmask = reset_hw_mib_counter(dev);
		int i;
		for (i = 0; i < 5; i++)
			if ((1 << i) & portmask)
				reset_stats22(i);
...
static inline int rtl_fill_txInfo(rtl_nicTx_info *txInfo)
{
	...
	#endif
#ifdef __DAVO__
	if ((rtl865x_curOpMode == BRIDGE_MODE) && (txInfo->portlist & (1 << PRTNR_WAN0)) &&
	    ((skb->data[0] == 0x01) || (skb->data[0] == 0x33))) {
		if (test_mcast_reserved(skb->data))
			txInfo->portlist &= ~(1 << PRTNR_WAN0);
	}
#endif
...
static int re865x_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	cp = NETDRV_PRIV(dev);
#ifdef __DAVO__
	if (dev->priv_flags & IFF_IPV6_PASSTHRU)
		;	/* do nothing */
	else
#endif
	if (
...
//call extract_magicNum() in rtl_fp_dev_queue_xmit_check()
	...
	if (
	    ...
	    (skb->srcPhyPort > PHY4) &&
	    (skb->srcPhyPort != 0xff) &&
	    (rtl_isWanDev(NETDRV_PRIV(skb->dev))) &&
	    (SUCCESS == rtl_isPassthruFrame(skb->data)))
	{
...
#endif
#ifdef IFF_IPV6_PASSTHRU
		dev->priv_flags |= IFF_IPV6_PASSTHRU;
#endif
#ifdef CP_VLAN_TAG_USED
		dev->features |= NETIF_F_HW_VLAN_TX | NETIF_F_HW_VLAN_RX;
#endif
		      ...
		      unsigned long count, void *data)
{
	...
	if (strtrim_from_user(tmpBuf, sizeof(tmpBuf), buffer, count) > 0) {
		sscanf(tmpBuf, "%d %d %d %d", &port, &enableStormCtrl, &limit_bps, &stormCtrlType);
...
#define IN4(a, b, c, d) __constant_htonl(((__u32)a << 24) | ((__u32)b << 16) | ((__u32)c << 8) | ((__u32)d))

static bool in_mcast_reserved(const __be32 daddr)
{
	switch (daddr) {
	case IN4(239, 255, 255, 250):	/* UPnP */
	case IN4(224, 0, 0, 251):	/* mDNS */
	case IN4(224, 0, 0, 252):	/* LLMNR */
	case IN4(239, 255, 255, 246):	/* UPnP# */
	case IN4(224, 0, 1, 65):	/* iapp */
	case IN4(224, 0, 1, 76):	/* IAPP */
	case IN4(224, 0, 1, 178):	/* IEEE IAPP */
		return true;
	default:
		return false;
	}
...
bool in6_mcast_reserved(const struct in6_addr *daddr)
{
	static const struct in6_addr reserved6[] = {
		[0] = { .s6_addr16[0] = __constant_htons(0xff02),
			.s6_addr16[7] = __constant_htons(0xc) },
		[1] = { .s6_addr16[0] = __constant_htons(0xff02),
			.s6_addr16[7] = __constant_htons(0xf) },
		[2] = { .s6_addr16[0] = __constant_htons(0xff02),
			.s6_addr16[7] = __constant_htons(0xfb) },
		[3] = { .s6_addr16[0] = __constant_htons(0xff02),
			.s6_addr16[6] = __constant_htons(0x1),
			.s6_addr16[7] = __constant_htons(0x3) },
	};
	int i;

	for (i = 0; i < ARRAY_SIZE(reserved6); i++)
		if (ipv6_addr_equal(daddr, reserved6 + i))
			return true;
	return false;
}
EXPORT_SYMBOL(in6_mcast_reserved);
...
static bool test_mcast_reserved(__u8 *p)
{
	...
	u.b += offsetof(struct ethhdr, h_proto);
	if (u.w[0] == __constant_htons(ETH_P_8021Q))
		u.b += 4;	/* move to h_vlan_encapsulated_proto */

	if (u.w[0] == __constant_htons(ETH_P_IP) ||
	    u.w[0] == __constant_htons(ETH_P_IPV6)) {
		u.w += 1;
		switch (u.ih->version) {
		case 4:
			return in_mcast_reserved(u.ih->daddr);
		case 6:
			return in6_mcast_reserved(&u.i6h->daddr);
		default:
			break;
		}
	}
