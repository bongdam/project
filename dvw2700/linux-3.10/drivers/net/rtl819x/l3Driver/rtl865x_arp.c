...
#ifdef __DAVO__
/* APACRTL-87 */
int rtl_ll_lookup(const u8 *addr, rtl865x_tblAsicDrv_l2Param_t *l2, u16 fid, u32 *mat)
{
	u32 i, row;

	row = rtl8651_filterDbIndex((ether_addr_t *)addr, fid);
...
int rtl865x_isEthArp(ipaddr_t ip)
{
	rtl865x_route_t rt;
	rtl865x_vlan_entry_t *vlan;
	...
	if (rtl865x_getRouteEntry(ip, &rt) != SUCCESS)
		return FALSE;

	if ((rt.valid != 1) || (rt.process != 2) || (rt.dstNetif == NULL))
		return FALSE;

	if (rtl865x_arp_hash(ip, &h) != SUCCESS)
		return FALSE;

	if (arpTables.mappings[h].ip != ip)
		return FALSE;

	vlan = _rtl8651_getVlanTableEntry(rt.dstNetif->vid);
	if (rtl_ll_lookup(arpTables.mappings[h].mac.octet, &l2, vlan ? vlan->fid : 0, NULL))
		return FALSE;

	return (l2.memberPortMask & ((1 << RTL8651_PHY_NUMBER) - 1)) ? TRUE : FALSE;
}
#else /* __DAVO__ */
