...
extern long l2_proc_ioctl(struct file *file, unsigned int command, unsigned long arg);
...
static int32 ip_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (strtrim_from_user(buf, sizeof(buf), buff, len) > 0) {
		i = strargs(buf, argv, 8, " \t\r\n");
		if (i < 2)
			goto ip_proc_out;

		if (!strcmp(argv[0], "add")) {
			if (i == 4) {
				intip = in_aton(argv[1]);
				extip = in_aton(argv[2]);
				nat = !strcmp(argv[3], "napt") ? IP_TYPE_NAPT : IP_TYPE_NAT;
				rtl865x_addIp(ntohl(intip), ntohl(extip), nat);
			}
		} else if (!strcmp(argv[0], "del")) {
			if (strcmp(argv[1], "external")) {
				extip = in_aton(argv[1]);
				rtl865x_delIp(ntohl(extip));
			} else {
				for (i = 0; i < RTL8651_IPTABLE_SIZE; i++) {
					if (rtl8651_getAsicExtIntIpTable(i, &ip) != SUCCESS)
						continue;
					if (ip.nat == 0 && ip.localPublic == 0)
						rtl865x_delIp(ntohl(ip.extIpAddr));
				}
			}
		}
	}
...
//}
#ifdef __DAVO__
extern ssize_t acl_write(struct file *filp, const char __user *buffer, size_t count, loff_t *off);
#endif
