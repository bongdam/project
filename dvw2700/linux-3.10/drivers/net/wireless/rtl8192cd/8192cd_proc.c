...
#endif
#ifdef __DAVO__
	PRINT_SINGL_ARG("    request_autoch: ", priv->pmib->dot11RFEntry.request_autochan_rst, "%u");
#endif
	return pos;
}
...
#endif
{
	...
	CHECK_LEN_B;
#if defined(__DAVO__)
	PRINT_SINGL_ARG("    tx_only_data_bytes: ", pstat->tx_only_data_bytes, "%llu");
	CHECK_LEN_B;
#endif
	PRINT_SINGL_ARG("    rx_bytes: ", pstat->rx_bytes, "%u");
	CHECK_LEN_B;
#if defined(__DAVO__)
	PRINT_SINGL_ARG("    rx_only_data_bytes: ", pstat->rx_only_data_bytes, "%llu");
	CHECK_LEN_B;
#endif
	...
	CHECK_LEN_B;
#if defined(__DAVO__)
	PRINT_SINGL_ARG("    tx_only_data_packets: ", pstat->tx_only_data_packets, "%u");
	CHECK_LEN_B;
#endif
	PRINT_SINGL_ARG("    rx_pkts: ", pstat->rx_pkts, "%u");
	CHECK_LEN_B;
#if defined(__DAVO__)
	PRINT_SINGL_ARG("    rx_only_data_packets: ", pstat->rx_only_data_packets, "%u");
	CHECK_LEN_B;
#endif
...
#if defined(__DAVO__)
	PRINT_SINGL_ARG("    tx_only_data_bytes:", priv->tx_only_data_bytes, "%llu");
#endif
...
#if defined(__DAVO__)
	PRINT_SINGL_ARG("    rx_only_data_bytes:", priv->rx_only_data_bytes, "%llu");
#endif
#if defined(CONFIG_RTL8672) || defined(CONFIG_WLAN_STATS_EXTENTION)
...
#if defined(__DAVO__)
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_dv_max_count_read(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_dv_max_count_read(char *buf, char **start, off_t offset,
		int length, int *eof, void *data)
#endif
{
	struct net_device *dev = PROC_GET_DEV();
	...
	memcpy(ssid, priv->pmib->dot11StationConfigEntry.dot11DesiredSSID,
			priv->pmib->dot11StationConfigEntry.dot11DesiredSSIDLen);

#ifdef CONFIG_RTL_PROC_NEW
	seq_printf(s, "%s cur:%d max:%d\n", ssid, priv->assoc_num, priv->max_count);
#else
	pos += sprintf(&buf[pos], "%s cur:%d max:%d\n", ssid, priv->assoc_num, priv->max_count);
#endif

	return pos;
}

static int rtl8192cd_proc_dv_max_count_write(struct file *file, const char *buffer,
				unsigned long count, void *data)
{
	struct net_device *dev = (struct net_device *)data;
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
static int dv_proc_read_rfi_hist(struct seq_file *s, void *v)
{
	...
	if (dv_get_rf_env_log(priv, &rf_env)) {
		PRINT_ONE("idx      fa     cca  mac-rx  tx-Mbps  rx-Mbps  IGI", "%s", 1);
		PRINT_ONE("--------------------------------------------------", "%s", 1);
		i=rf_env->index;
		for (j=0; j<MAX_LOG_HIST; j++) {
			...
			PRINT_ONE(calc_mbps(rf_env->cnt[i].rx_bytes, buff, sizeof(buff)), "%8s", 0);
			PRINT_ONE(rf_env->cnt[i].CurIGValue, "   0x%x", 1);
		}
	} else {
		PRINT_ONE("N/A", "%s", 1);
	}
...
#if defined(__DAVO__)
	RTK_DECLARE_READ_WRITE_PROC_FOPS(rtl8192cd_proc_dv_max_count_read, rtl8192cd_proc_dv_max_count_write);
...
#if defined(__DAVO__)
    RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "dv_max_count", rtl8192cd_proc_dv_max_count_read, rtl8192cd_proc_dv_max_count_write);
...
#if defined(__DAVO__)
        remove_proc_entry( "dv_max_count", rtl8192cd_proc_root );
#endif
