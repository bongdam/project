...
#if defined(__DAVO__)
static __inline__ void tx_onlydata_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *pskb)
{
	if (!(pskb->data[0] & 1) && pskb->data[12] == 0x08 && pskb->data[13] == 0x00) {
		int n = pskb->len - ETHER_HDRLEN;
		priv->tx_only_data_bytes += n;
		priv->tx_only_data_packets++;
		if (pstat) {
			pstat->tx_only_data_bytes += n;
			pstat->tx_only_data_packets++;
		}
	}
}

static __inline__ void rx_onlydata_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *pskb)
{
	if (!(pskb->data[0] & 1) && pskb->data[12] == 0x08 && pskb->data[13] == 0x00) {
		int n = pskb->len - ETHER_HDRLEN;
		priv->rx_only_data_bytes += n;
		priv->rx_only_data_packets++;
		if (pstat) {
			pstat->rx_only_data_bytes += n;
