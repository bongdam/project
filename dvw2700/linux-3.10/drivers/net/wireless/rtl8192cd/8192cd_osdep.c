	...
	// construct operation and basic rates set
	{
		...
		if (priv->pmib->dot11RFEntry.dot11channel != 0) {
...
#endif
			{
				...
				if ( priv->pmib->dot11nConfigEntry.dot11nUse40M == 7 ) {
					priv->auto_channel = 1;
					priv->auto_bandwidth = 1;
				} else {
					priv->auto_channel = 0;
				}
...
//------------------------------ david+2008-01-11
		...
		if ((priv->pmib->dot11nConfigEntry.dot11nUse40M == 1 || priv->pmib->dot11nConfigEntry.dot11nUse40M == 7 )&&
			(!(priv->pmib->dot11BssType.net_work_type & (WIRELESS_11N | WIRELESS_11AC)))) {
...
#endif
#ifdef IFF_802_11_LAN
	dev->priv_flags |= IFF_802_11_LAN;
#endif
