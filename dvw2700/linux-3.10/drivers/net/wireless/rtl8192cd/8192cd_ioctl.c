...
#ifdef __DAVO__
	{ SIOCIWCUSTOM, IW_PRIV_TYPE_CHAR | 128, 0, "request_autoch" },
#endif
...
#if defined(__DAVO__)
			pInfo->tx_only_data_bytes = pstat->tx_only_data_bytes;
			pInfo->rx_only_data_bytes = pstat->rx_only_data_bytes;
			pInfo->tx_only_data_packets = pstat->tx_only_data_packets;
			pInfo->rx_only_data_packets = pstat->rx_only_data_packets;
#endif
...
#ifdef __DAVO__
int rtl8192cd_autochannel_request(struct rtl8192cd_priv *priv)
{
	INT8 ret = 0;
	...
	if (!ret)	// now, let's start site survey
	{
		...
		DEBUG_INFO("start_clnt_ss, trigger by %s, ss_ssidlen=0\n", (char *)__FUNCTION__);
		start_clnt_ss(priv);
		priv->pmib->dot11RFEntry.request_autochan = 1;
		priv->ss_req_ongoing = 1;
		priv->auto_channel = 1;
	}
	return ret;
...
*/
	...
	case SIOCIWCUSTOM:
		switch (wrq->u.data.flags) {
			...
			case RTL8192CD_IOCTL_REQ_AOTUCH:
				if (!(OPMODE & WIFI_AP_STATE)){
					DEBUG_ERR("can't do auto-channel select for non-AP mode!\n");
					break;
				}
				...
				printk("SIOCIWCUSTOM %s command\n", tmpbuf);
				ret = rtl8192cd_autochannel_request(priv);
				break;
			default:
