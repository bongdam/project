...
extern uint8_t twin_haddr[ETH_ALEN];

static unsigned int ipv4_ct_spoof_igmp(unsigned int, struct sk_buff *,
...
#ifdef CONFIG_NF_NAT_TWINIP
static unsigned int ipv4_ct_spoof_igmp(unsigned int hooknum,
				       struct sk_buff *skb,
				       const struct net_device *in,
				       const struct net_device *out,
				       int (*okfn)(struct sk_buff *))
{
	...
	if (twin_inaddr.s_addr &&
	    ({ iph = ip_hdr(skb); 1; }) &&
	    iph->protocol == IPPROTO_IGMP &&
	    iph->saddr == twin_inaddr.s_addr &&
		...
		if (del_timer(&ct->timeout)) {
			/* shrink timer to 100ms */
			ct->timeout.expires = jiffies + (HZ / 10);
			add_timer(&ct->timeout);
		}
		spin_unlock_bh(&nf_conntrack_lock);

		kdmsg(KDMSG_NET_NETFILTER, "spoofing igmp: %pI4 %pI4\n",
