...
void rtl_addnaptconn(struct nf_conn *, struct nf_conntrack_tuple *,
		     struct nf_conntrack_tuple *, struct sk_buff *);
#endif
...
#if defined(__DAVO__)
int nf_nat_preprocess_packet(struct nf_conn *, unsigned int, struct sk_buff *);
#endif
			   ...
			   struct sk_buff *skb)
{
...
#if defined(__DAVO__)
	fpath = nf_nat_preprocess_packet(ct, hooknum, skb);
#endif
	...
	if (ct->status & statusbit) {
		...
		struct nf_conntrack_tuple *org, *rep;
		int ptow;	/* from private network to WAN */

		if ((ct->status & (IPS_FASTPATH|IPS_SDMZCONN)) == IPS_SDMZCONN &&
		    ({ org = &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple; 1; }) &&
		    ((ptow = (org->src.u3.ip == twin_inaddr.s_addr)) || (org->dst.u3.ip == twin_inaddr.s_addr)) &&
		    !nfct_help(ct)) {
		    	if (test_all_bits(IPS_SRC_NAT_DONE|IPS_DST_NAT_DONE, ct->status) && 
			    ((ip_hdr(skb)->protocol == IPPROTO_UDP) ||
			     (ct->proto.tcp.state == TCP_CONNTRACK_ESTABLISHED))) {
			     	rep = &ct->tuplehash[IP_CT_DIR_REPLY].tuple;
				if ((org->dst.u3.ip == rep->src.u3.ip) && (org->src.u3.ip == rep->dst.u3.ip)) {
					if (ptow == TRUE)
						rtl_addnaptconn(ct, org, rep, skb);
					else
						rtl_addnaptconn(ct, rep, org, skb);
				}
			}
		}
	}
#endif //defined(CONFIG_NF_NAT_TWINIP)
	return NF_ACCEPT;
}
