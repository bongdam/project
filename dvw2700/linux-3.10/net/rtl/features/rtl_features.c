...
#if defined(CONFIG_NF_NAT_TWINIP)
extern struct in_addr twin_inaddr;
extern int is_ifa_local(__be32 address);

#define mark_fp_bit(expr) do { if ((expr) == LR_SUCCESS) set_bit(IPS_FASTPATH_BIT, &ct->status); } while (0)
#else
#define mark_fp_bit(expr) expr
#endif
...
#if defined(CONFIG_NF_NAT_TWINIP)
static inline int ip_local(__be32 ip)
{
	return (is_ifa_local(ip) || (twin_inaddr.s_addr && twin_inaddr.s_addr == ip));
}

void rtl_addnaptconn(struct nf_conn *ct,
		     struct nf_conntrack_tuple *t1,
		     struct nf_conntrack_tuple *t2,
		     struct sk_buff *skb)
{
	rtl_fp_napt_entry rtlFpNaptEntry;

	rtlFpNaptEntry.protocol = (ip_hdr(skb)->protocol == IPPROTO_UDP) ? NP_UDP : NP_TCP;
	rtlFpNaptEntry.intIp = t1->src.u3.ip;
	rtlFpNaptEntry.intPort = ntohs(t1->src.u.all);
	rtlFpNaptEntry.extIp = t2->dst.u3.ip;
	rtlFpNaptEntry.extPort = ntohs(t2->dst.u.all);
	rtlFpNaptEntry.remIp = t2->src.u3.ip;
	rtlFpNaptEntry.remPort = ntohs(t2->src.u.all);

	mark_fp_bit(rtk_addNaptConnection(&rtlFpNaptEntry, skb, ct, NP_NONE));
}
#endif
...
#if defined(CONFIG_NF_NAT_TWINIP)
		if ((ip_local(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip) &&
					ip_local(ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip)) ||
		    (ip_local(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip) && 
			 ip_local(ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3.ip))) {
			return;
		}
#endif
		if(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip
		      ...
		      unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(&gHwNatSetting, buffer, 8)) {
			...
			||(gHwNatSetting[1] == '4') /* PPTP */))
		{
			...
			get_br0_ip_mask();
#if !defined(CONFIG_NF_NAT_TWINIP) //todo : add acl
