...
BROADCAST_STORM_ENABLE=`nvram get BROADCASTSTORM_CTRL_ENABLE`
if [ "$BROADCAST_STORM_ENABLE" == "" ]; then
	exit 1
fi

BROADCAST_STORM_BPS=`nvram get BROADCASTSTORM_BPS`
...
while [ "$num" -lt 5 ]
do
	BROADCAST_STORM_PORT_ENABLE=`nvram get 'BROADCASTSTORM_PORT'$num'_ENABLE'`
