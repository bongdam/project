...
extern size_t strtrim_from_user(char *dst, size_t dlen,
			const char __user *src, size_t count);

const unsigned int cpu_clksel_table[] = {
...
static int read_watchdog_cmd_proc(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < sizeof(wtdog_tbl) / sizeof(WTDOG_REGTBL_T); i++) {
		if (wtdog_tbl[i].oversel_h == higl_oversel && wtdog_tbl[i].oversel_l == low_oversel) {
					...
					size_t count, loff_t * off)
{
	...
	if (strtrim_from_user(flag, sizeof(flag), userbuf, count) > 0) {
		int i;
		...
		if (enable == 0) {
			...
			for (i = 0; i < sizeof(wtdog_tbl) / sizeof(WTDOG_REGTBL_T); i++) {
				...
				if (interval >= wtdog_intervel0 && interval <= wtdog_intervel)
					goto END;
...
static ssize_t write_watchdog_reboot(struct file *file, const char *buffer, size_t count, loff_t * data)
{
	char tmp[16];
...
#ifdef  CONFIG_RTL_USERSPACE_WTDOG
	proc_create_data("watchdog_cmd", 0, &proc_root, &watchdog_cmd_proc_fops, NULL);
