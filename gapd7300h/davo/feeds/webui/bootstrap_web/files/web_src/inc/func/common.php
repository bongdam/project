...
//					$tmp = str_ireplace("'", "\'", $tmp);
	...
	*/
	...
	function dv_set_cookie($key_, $value_, $expire_ = 86400){
		setcookie($key_, $value_, time()+($expire_));
	}
	/*
	* 제작자 : take99
	* 제작일 : 2015/02/13
	...
	*/
	...
	function getTimestamp()
	{
		...
		$millisecond_ = substr($rounded."000",0,3);
...
//			print_r($break_);
	...
	function getFromUrl($url, $method = 'GET')
	{
		...
		$res = curl_exec($ch);
		curl_close($ch);
		$res = str_replace("Current IP Address: ","",$res);
		return $res;
	}
?>