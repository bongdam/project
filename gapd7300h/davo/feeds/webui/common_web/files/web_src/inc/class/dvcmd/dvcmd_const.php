<?php
	DEFINE("SYSCALL",[
		...
		"dhcpr_reload"		=> "/etc/init.d/dhcpr restart",
		...
		"iwconfig"			=> "iwconfig %s",
		"mount"				=> "/usr/bin/mount %s",
		"umount"			=> "/usr/bin/umount %s",
		"nginx_restart"		=> "/etc/init.d/nginx restart",
		"channel_info"		=> "ath0=$(iwinfo ath0 freqlist | grep \* | sed 's/* //' 2&> /dev/null) && ath1=$(iwinfo ath1 freqlist | grep \* | sed 's/* //' 2&> /dev/null) && echo  \$ath0,\$ath1 ",
		"kill_proc"			=> "kill -9 %s",
		"ps_info"			=> "ps %s",
		"ddns_restart"		=> "/usr/lib/ddns/dynamic_dns_updater.sh myddns_ipv4 0 > /dev/null 2>&1 &",
		"fw_printenv"		=> "/usr/sbin/fw_printenv %s",
		"clearcaldata"		=> "/usr/sbin/clearcaldata",
		"file_copy"			=> "/bin/cp %s",
		"file_remove"		=> "/bin/rm %s",
		"child_guard"		=> "/usr/sbin/wpa_cli -g /var/run/hostapd/global raw INIT_CHILD_GUARD OK",
		"fw_setenv"			=> "/usr/sbin/fw_setenv %s"
	]);
	DEFINE("SYSCALL_P",[
		"free"				=> false,
		"getpasswd"			=> true,
