...
String.prototype.ip_check = function(b_){
	var ip = this;
	var ip2  = b_;
	var ip_arr = ip.split('.');
	var ip_int = 0;
...
var CreateDummy = function(){
	var DummyVal ="";
	var now = new Date();
	DummyVal = now.getFullYear().toString()+now.getMonth().toString()+now.getDate().toString()+now.getHours().toString()+now.getMinutes().toString()+now.getSeconds().toString()+now.getMilliseconds().toString();
	return DummyVal;
};
...
if(jQuery){
	...
	$(document).ready( function() {
		if($("frameset").length == 0){
			if(parent.location.pathname != "/" && parent.location.pathname != "/login.php" && parent.location.pathname != "/index.php" && location.hash != "#form"){
				top.parent.window.location.href="/";
			}
			if((top.parent.window.location.pathname == "/index.php" || top.parent.window.location.pathname == "/") && location.pathname == "/login.php"){
				top.parent.window.location.href="/";
			}
		}
	});
