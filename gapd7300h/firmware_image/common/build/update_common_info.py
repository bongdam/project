...
-------------------------------------------------------------------------------
  ...
  $Header: //depot/asic/ipq8066/IPQ4019.ILQ.5.0_P/meta/main/latest/common/build/update_common_info.py#2 $
  $DateTime: 2017/03/29 06:37:28 $
...
if FIT_tool:
   ...
   if not os.path.exists(ipq_p_gcc5_dir):
	...
	#ipq_allconf_dir = './ipq_allconf' # SDK_ORI
   ipq_allconf_dir = './ipq'
   if not os.path.exists(ipq_allconf_dir):
      os.makedirs(ipq_allconf_dir,mode = 0755)
   ...
   for flash_file in mi.get_files(attr='flash_file'):
      shutil.copy(flash_file, ipq_allconf_dir)
   ...
   for flash_file in mi.get_files(attr='flash_file_allconf'):
      shutil.copy(flash_file, ipq_allconf_dir)
  ...
  # copy all files has attribute QPST_file to uppermost meta folder
   ...
   # DAVO custom NAND partition regen
   pytool_dir = os.getcwd()+'/ipq/tools'
   regen_out_dir = os.getcwd()+'/out'
   regen_dest_dir = pytool_dir+'/../'
   ret_nand_partition = lg.log_exec(['python', pytool_dir+'/'+'genimg.py', '--partition_tool='+tools_dir+'/'+'partition_tool', '--mbn_gen='+tools_dir+'/'+'nand_mbn_generator.py', '--configdir='+tools_dir+'/'+'config/', '--skip_export', '--image_name=NOR_PLUS_NAND_IMAGES'], verbose=0)
   print regen_out_dir
   print regen_dest_dir
   os.system('cp '+regen_out_dir+'/*'+' ' + regen_dest_dir)
   # DAVO custom End
   ...
   ret_nand_allconf_single = 0
   ret_emmc_allconfig_single = 0
...
#   print "ret_norplusemmc_single %d." % ret_norplusemmc_single
   ...
   ret_nand_allconf_single = lg.log_exec(['python',pack_tool,'-t','nand','-B','-F','boardconfig_premium', '-o', bin_dir +'/'+'nand-ipq40xx-allconfig-single.img',ipq_allconf_dir],verbose=0)
   print "ret_nand_allconf_single %d." % ret_nand_allconf_single
#   ret_emmc_allconfig_single = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'emmc-ipq40xx-allconfig-single.img',ipq_allconf_dir],verbose=0)
#   print "ret_emmc_allconfig_single %d." % ret_emmc_allconfig_single
...
#   print "ret_noremmc_apps %d." % ret_noremmc_apps
   ...
   ret_streamboost_nor = 0
   ret_streamboost_nand = 0
   ret_streamboost_nornand = 0

   if os.path.isfile("./ipq/openwrt-ipq806x-streamboost3-squashfs-rootfs.img"):
		os.rename("./ipq/openwrt-ipq806x-streamboost3-squashfs-rootfs.img", "./ipq/openwrt-ipq806x-squashfs-root.img")
		os.rename("./ipq/openwrt-ipq806x-ipq40xx-ubi-root-streamboost3-enabled.img", "./ipq/openwrt-ipq806x-ipq40xx-ubi-root.img")
		ret_streamboost_nor = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'nor-ipq40xx-single-streamboost.img',ipq_dir],verbose=0)
		print "ret_streamboost_nor %d." % ret_streamboost_nor
		ret_streamboost_nand = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'nand-ipq40xx-single-streamboost.img',ipq_dir],verbose=0)
		print "ret_streamboost_nand %d." % ret_streamboost_nand
		ret_streamboost_nornand = lg.log_exec(['python',pack_tool,'-t','norplusnand','-B', '-F','boardconfig_premium','-o', bin_dir +'/'+'nornand-ipq40xx-single-streamboost.img',ipq_dir],verbose=0)
		print "ret_streamboost_nornand %d." % ret_streamboost_nornand
