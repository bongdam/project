...
guest_link_override=0 guest_vid=2
        ...
        if [ "$rate_num" -lt "$rate_samples" ]; then
            ...
            if [ "$restarted" -eq 0 ]; then
                if [ -n "$sta_iface_24g" ]; then
                    ...
                    if [ "$my_uplink_rate_median" -lt "$rate_pref2G" ]; then
...
__repacd_wifimon_config_best_ap() {
    ...
    local config_status=0 vap_restarted=0
       ...
       if [ -n "$new_bestap" -a "$new_bestap" != "00:00:00:00:00:00" ]; then
           if [ -n "$otherband_sta_iface" -a -n "$new_bestap_otherband" \
               ...
               __repacd_wifimon_check_and_config_bssid $sta_iface \
                   $sta_iface_config_name $new_bestap restart config_changed
               if [ "$config_changed" -gt 0 ]; then
                   __repacd_wifimon_wpa_config_bssid $sta_iface $new_bestap $restart
               fi
               if [ "$restart" -gt 0 ]; then
                   vap_restarted=$restart
               fi
               ...
               if [ -n "$otherband_sta_iface" ]; then
                   ...
                   __repacd_wifimon_check_and_config_bssid $otherband_sta_iface \
                       $otherband_sta_iface_config_name $new_bestap_otherband restart config_changed
                   if [ "$config_changed" -gt 0 ]; then
                       __repacd_wifimon_wpa_config_bssid $otherband_sta_iface $new_bestap_otherband $restart
                   fi
                   if [ "$restart" -gt 0 ]; then
                       vap_restarted=$((vap_restarted + 1))
                   fi
               fi

               eval "$3='$vap_restarted'"
               config_status=1
           fi
       else
           config_status=0
