...
ifacemgr_bringup_apvaps_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if(gptr->retry_count >= 3 || allDone) {
        ...
        eloop_register_timeout(8, 0, ifacemgr_bringup_apvaps_timer, gptr, NULL);
    }
}
...
ifacemgr_stavap_connection_status(struct iface_daemon *iptr, struct sta_vap *stavap)
{
    ...
    if (stavap_status) {
        ifacemgr_printf("initial stavap(%s) connection: %d", stavap->ifname,stavap_status);
    }
...
/* When both STAVAPs are in down state, periodic poll from driver
   wil give correct STAVAP status, if iface-mgr missed event from
   wpa_supplicant*/

static void
ifacemgr_stavap_periodic_poll(void *eloop_ctx, void *timeout_ctx)
{
    ...
    eloop_register_timeout(PERIODIC_POLL_TIME, 0, ifacemgr_stavap_periodic_poll, iptr, NULL);
    return;
}
    ...
    if (os_strncmp(msg + WPA_S_MSG_ADDR_OFF, "CTRL-EVENT-DISCONNECTED ", 24) == 0) {
        ...
        if (gptr->num_sta_vaps_up == 1) {
            if(IFACE_MODE_FASTLANE == (iptr->mode)) {
                ...
                eloop_cancel_timeout(ifacemgr_stavap_periodic_poll, iptr, NULL);
            } else {
