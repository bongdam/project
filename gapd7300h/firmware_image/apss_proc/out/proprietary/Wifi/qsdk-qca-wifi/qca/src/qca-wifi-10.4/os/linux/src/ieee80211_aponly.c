...
ath_rx_removebuffer_aponly(struct ath_softc *sc, HAL_RX_QUEUE qtype)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        if (wbuf) {
            ...
            if (!bf) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] PANIC wbuf %p Index %d\n", __func__, __LINE__, wbuf, i);
            } else {
        ...
        ieee80211_rx_status_t *rx_status, u_int8_t *chainreset)
{
     ...
     */
    {
        ...
        if (ic->ic_green_ap_get_print_level(ic)) {
            if (!(print_now & 0xff)) {
                  QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Rx rssi0 %d rssi1 %d rssi2 %d\n",
