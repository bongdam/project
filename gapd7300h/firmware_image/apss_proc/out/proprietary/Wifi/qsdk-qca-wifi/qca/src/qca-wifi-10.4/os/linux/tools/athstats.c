...
/* PHY statistics */
    ...
    fprintf(fd," mcs 0- mcs 7 STATS:");
    for (i = 0; i< 8; i++)
        fprintf(fd,"%#6d,",stats->ast_rx_rate_packets[i]);
    fprintf(fd,"\n mcs 8- mcs15 STATS:");
    for (i = 0;i < 8; i++)
        fprintf(fd,"%#6d,",stats->ast_rx_rate_packets[i+8]);
    fprintf(fd,"\n mcs16- mcs23 STATS:");
    for (i = 0; i < 8; i++)
        fprintf(fd,"%#6d,",stats->ast_rx_rate_packets[i+16]);
    fprintf(fd,"\n");
...
ol_radiostats_print(const struct ol_ath_radiostats *radio_stats, const struct wlan_dbg_stats *wlan_pdev_stats, FILE *fd)
{
    ...
    fprintf(fd,"ast_rx_data_bytes   :\t%lu\n",radio_stats->rx_data_bytes);
