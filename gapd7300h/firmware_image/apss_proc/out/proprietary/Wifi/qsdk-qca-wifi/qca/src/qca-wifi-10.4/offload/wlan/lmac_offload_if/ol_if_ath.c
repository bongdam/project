...
extern int whal_ratecode_to_kbps(int, int, int);
...
static void ieee80211_unauthorized_sta_timeout(struct ieee80211_node_table *nt)
{
    struct ieee80211_node *ni;
    u_int gen;
    systime_t now;
    ...
    TAILQ_FOREACH(ni, &nt->nt_node, ni_list) {
        ...
        if (ni->ni_inact <= 0) {
            ...
            if ( ni->ni_vap->iv_opmode == IEEE80211_M_IBSS || ni->ni_vap->iv_opmode == IEEE80211_M_STA) {
                ...
                if( ni->ni_last_auth_rx_time != 0 && (CONVERT_SYSTEM_TIME_TO_MS(now - ni->ni_last_auth_rx_time) > 1500)) {
...
static OS_TIMER_FUNC(ol_ath_timeout)
{
    struct ieee80211com *ic;
    OS_GET_TIMER_ARG(ic, struct ieee80211com *);
    ieee80211_unauthorized_sta_timeout(&ic->ic_sta);
