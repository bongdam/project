                                  ...
                                  union iwreq_data *wrqu, char *extra)
{
    ...
    if (info->cmd >= SIOCIWFIRSTPRIV) {
        ...
        if (cmd_id < 0 || cmd_id >= TABLE_SIZE(ath_iw_priv_handlers)) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : #### wrong private ioctl 0x%x\n", __func__, info->cmd);
            return -EINVAL;
        }
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : #### no registered private ioctl function for cmd 0x%x\n", __func__, info->cmd);
    }

    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : #### unknown command 0x%x\n", __func__, info->cmd);
    ...
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            ...
            if (tid) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   ac_q=%d, ac=0x%p, hw_q=%d, node=%p, an_flags=0x%x, sched=%d, hwqcnt=%d, ",
                    ac->qnum, ac, i, ni, tid->an->an_flags, ac->sched, ac->hwqcnt);
            } else {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   ac_q=%d, ac=0x%p, hw_q=%d, node=<null>, sched=%d, hwqcnt=%d, ",
    ...
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            TAILQ_FOREACH_SAFE(tid, &ac->tid_q, tid_qelem, next_tid) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   vap=%d, tidno=%d, tid=0x%p, node=%p, an_flags=0x%x, ac_q=%d, ",
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "depth=%d, num_mcast_buf=%d, sched=%d, paused=%d, min_depth=%d\n",
                    depth, mcast_cnt, tid->sched, tid->paused, tid->min_depth);
            }
        }
        ATH_TXQ_UNLOCK(txq);
    }
