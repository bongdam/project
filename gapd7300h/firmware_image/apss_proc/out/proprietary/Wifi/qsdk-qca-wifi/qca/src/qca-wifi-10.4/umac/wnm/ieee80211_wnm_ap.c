                            ...
                            struct ieee80211_bstm_reqinfo *bstm_reqinfo)
{
    ...
    if (ieee80211_vap_mbo_is_set(vap)) {
        info.regclass = regdmn_get_opclass (vap->iv_ic);
    } else {
...
void ieee80211_wnm_bssmax_updaterx(struct ieee80211_node *ni, int update_timer)
{
    ...
    if (vap->wnm->wnm_bss_idle_option & WNM_BSS_IDLE_PROTECTED_KEEPALIVE) {
        if (update_timer) {
...
u_int8_t * ieee80211_add_bssmax(u_int8_t *frm, struct ieee80211vap *vap)
{
    ...
    *(u_int16_t *)frm = htole16(vap->wnm->wnm_bss_max_idle_period);
    frm += 2;
    *(u_int8_t *)frm = vap->wnm->wnm_bss_idle_option;
    frm += 1;
