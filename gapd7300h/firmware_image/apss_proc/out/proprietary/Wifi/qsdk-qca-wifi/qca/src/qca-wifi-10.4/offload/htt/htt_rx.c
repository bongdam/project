    ...
    if (!msdu_done) {
            ...
            if (pdev->htt_rx_donebit_assertflag == 0) {
                if ((pdev->g_htt_status != htt_rx_status_ok) && ((*npackets) == 0)) {
                    ...
                    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "rx_desc %p msdu %p rx_ind_msg %p \n", rx_desc, msdu, rx_ind_msg);
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "next_rx_desc %p next_msdu %p \n", rx_desc, next_msdu);
            ...
            if (pdev->htt_rx_donebit_assertflag == 1) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "rx_desc %p msdu %p \n", rx_desc, msdu);
