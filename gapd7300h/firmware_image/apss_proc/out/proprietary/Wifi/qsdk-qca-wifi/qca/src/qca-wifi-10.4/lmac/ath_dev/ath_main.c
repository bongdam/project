...
ath_node_cleanup(ath_dev_t dev, ath_node_t node)
{
    ...
    if (atomic_read(&an->an_active_tx_cnt)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "skipping lmac node %p cleanup due to tx activity\n", an);
        return 1;
    }
...
static int ath_rng_kthread(void *data)
{
    ...
    while (!kthread_should_stop()) {
        ...
        if(last_mode != mode)
        {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "mode changed to %d\n",ic->random_gen_mode);
            last_mode = mode;
        }
        if (mode ==0) {
            rssi_const_count  = 0;
        ...
        if ((rssi_const_count  > ATH_RSSI_CONST_COUNT_LIMIT) || mode == 2)
        {
            ...
            {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ERROR \n");
