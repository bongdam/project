...
TARGET_CFLAGS += -DIPQ4019 -D__DAVO__
else
TARGET_CFLAGS += -DIPQ806X -D__DAVO__
endif

ifeq ($(CONFIG_COMPANY_LGU), y)
TARGET_CFLAGS += -D__LGUPLUS__
endif
...
# DAVO add
ifneq ("$(wildcard $(PKG_BUILD_DIR)/.static_used)","")
PKG_STATIC:=1
else
PKG_STATIC:=
endif
...
# DAVO modified
  ...
  $(STAMP_PREPARED):
	@(\
		if [ ! -f $(PKG_BUILD_DIR)/.static_used ]; then \
				rm -rf $(PKG_BUILD_DIR); \
				mkdir -p $(PKG_BUILD_DIR); \
		fi; \
	)
	$(if $(PKG_STATIC),,$(foreach hook,$(Hooks/Prepare/Pre),$(call $(hook))$(sep)))
	$(if $(PKG_STATIC),,$(Build/Prepare))
	$(if $(PKG_STATIC),,$(foreach hook,$(Hooks/Prepare/Post),$(call $(hook))$(sep)))
	touch $$@
