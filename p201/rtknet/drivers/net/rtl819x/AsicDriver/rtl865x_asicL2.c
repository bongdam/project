...
#if defined(CONFIG_RTL_8197F_VG_V672) || defined(CONFIG_RTL_8197F_VG_V355I)
#define CONFIG_RTL_MDC_D1_MDIO_D7	1
#endif
...
#ifdef IP04438A
	/* Configure RTL8211F LED */
	if (gpio_simulate_mdc_mdio) {
		rtl_mdio_write(PORT0_RGMII_PHYID, 31, 0xd04); // extended-page 0xd04
		rtl_mdio_read(PORT0_RGMII_PHYID, 16, &data);
		rtl_mdio_write(PORT0_RGMII_PHYID, 16, (data & ~0x6f7b) | 0x207b);
		rtl_mdio_read(PORT0_RGMII_PHYID, 18, &data);
		rtl_mdio_write(PORT0_RGMII_PHYID, 18, (data & ~0xfc00) | data);
		rtl_mdio_write(PORT0_RGMII_PHYID, 31, 0); // reset page to 0

		if (!eee_enabled) {
			rtl_mdio_write(PORT0_RGMII_PHYID, 13, 7); // MMD DEV:7
			rtl_mdio_write(PORT0_RGMII_PHYID, 14, 0x3c); // EEE Adv Reg
			rtl_mdio_write(PORT0_RGMII_PHYID, 13, 0x4007); // OP:01 DEVAD:7
			rtl_mdio_read(PORT0_RGMII_PHYID, 14, &data);
			rtl_mdio_write(PORT0_RGMII_PHYID, 14, data & ~6); // Off both 1000EEE and 100EEE cap
		}
	}
#endif
	return 0;
