...
{
	static unsigned int regData, prelink;
	int rp;
...
#ifdef CONFIG_DVFLAG
	if (prelink != regData)
		dvflag_set((regData & (1 << 2)) ? DF_WANLINK : 0, DF_WANLINK);
#endif
	prelink = regData;
}	
...
inline int rtl_runout_resume(void)
{
	...
	{
		#if defined(CONFIG_RTL_ETH_PRIV_SKB)
		buf_pool_num = rx_skb_queue.qlen + eth_skb_free_num;
		#else
		buf_pool_num = rx_skb_queue.qlen;
		#endif
		...
		if (rx_desc_full(0)==1) {
			...
			if (buf_pool_num > freeSkbThreshold) {
