...
static struct rtm_rx_handler *rtm_rx_chain;

static int __rtm_rx_handler_register(struct rtm_rx_handler **hl,
				     struct rtm_rx_handler *h)
{
	while ((*hl) != NULL) {
		/* the bigger is the higher */
		if (h->priority > (*hl)->priority)
			break;
		hl = &((*hl)->next);
	}
	h->next = *hl;
...
static int __rtm_rx_handler_deregister(struct rtm_rx_handler **hl,
				       struct rtm_rx_handler *h)
{
	if (h == NULL)
		return -1;
...
int rtm_rx_handler_register(struct rtm_rx_handler *h, u_int n)
{
	...
	for (i = 0; i < n; i++) {
		err = __rtm_rx_handler_register(&rtm_rx_chain, &h[i]);
		if (err)
			goto err;
	}
	return err;
...
{
	while (n-- > 0)
		__rtm_rx_handler_deregister(&rtm_rx_chain, &h[n]);
}

void rtm_rx_func_chain(struct nlmsghdr *nlh, u_int len, u_int mtype)
{
	struct rtm_rx_handler *h, *next_h;

	for (h = rtm_rx_chain; h; h = next_h) {
		next_h = h->next;
		if (h->mtype != mtype)
			continue;
		h->rx_handler(h, nlh, len);
	}
}
