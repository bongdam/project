...
static int rtgateway(struct nlmsghdr *n, const char *interface)
{
	struct rtmsg *r;
...
static int
netif_rtm_link(struct rtm_rx_handler *rh, struct nlmsghdr *nlh, u_int len)
{
	struct ifinfomsg *imsg = (struct ifinfomsg *)NLMSG_DATA(nlh);
	const char *nif = (const char *)rh->data;
...
static int
netif_rtm_addr(struct rtm_rx_handler *rh, struct nlmsghdr *nlh, u_int len)
{
...
static int
netif_rtm_route(struct rtm_rx_handler *rh, struct nlmsghdr *nlh, u_int len)
{
	return rtgateway(nlh, (const char *)rh->data);
}

#ifdef __CONFIG_APP_DHCPV6__
static int
netif_rtm_prefix(struct rtm_rx_handler *rh, struct nlmsghdr *nlh, u_int len)
{
	return in6_prefix(nlh, (unsigned int *)&rh->data);
}
#endif

static struct rtm_rx_handler netif_rx_ops[] = {
	{
	.rx_handler = netif_rtm_link,
	.mtype = RTM_NEWLINK,
	.data = (long)nif_wan,
	},
	{
...
static int netif_event_read(struct select_event_base *base, int fd)
{
	...
	strlcpy(nif_wan, conf_ifwan(), IFNAMSIZ);

	err = len;
	for (n = (struct nlmsghdr *)buf; NLMSG_OK(n, err); n = NLMSG_NEXT(n, err))
	     	rtm_rx_func_chain(n, n->nlmsg_len, n->nlmsg_type);
	return len;
}
...
static void __attribute__((constructor)) register_netif_event(void)
{
	...
	if (fd > -1) {
		...
		addr.nl_pid = (__u32)(now.tv_sec * 1000000 + now.tv_usec);
		...
		if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)))
			perror("bind");
		           ...
		           NULL, "socket://rtnetlink:%u", addr.nl_pid))
			close(fd);
		else {
			rtm_rx_handler_register(netif_rx_ops, _countof(netif_rx_ops));
