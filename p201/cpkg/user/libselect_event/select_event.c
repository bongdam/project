...
{
	struct epoll_event events[maxevts ? : 1];
	int n, nfds;
	struct select_event_base *base;

	nfds = epoll_wait(fd_poll, events, maxevts ? : 1, timeout);
...
{
	return ((long)base->fd == (long)data) ? 0 : -1;
}

struct select_event_base *select_event_getbyfd(int fd)
{
	return select_event_iterate(fdcompar, (void *)(long)fd);
}
