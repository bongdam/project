...
{
	return (pthread_create) ? pthread_mutex_lock(&itimerlist_lock) : 0;
}

static inline int unlock(void)
{
	return (pthread_create) ? pthread_mutex_unlock(&itimerlist_lock) : 0;
}
...
int itimer_cancel(long tid, int (*function)(unsigned long))
{
	...
	unsigned long uninitialized_var(data);
