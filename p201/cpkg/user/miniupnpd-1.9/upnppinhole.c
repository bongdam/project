...
#if 0
int upnp_check_outbound_pinhole(int proto, int *timeout)
{
 ...
 */
int upnp_add_inboundpinhole(const char *raddr, unsigned short rport, const char *iaddr,
		unsigned short iport, int proto, char *desc, unsigned int leasetime, int *uid)
{
	int r;
	time_t current;
	...
	if (r <= 0) {
		LOG(LOG_ERR, "inet_pton(%d, %s, %p) FAILED",
				AF_INET6, iaddr, &address);
		return -4;
	}
	current = getuptime(NULL);
	timestamp = current + leasetime;
	r = 0;

#if 0
	if (r == 1 && strcmp(iaddr, iaddr_old) == 0 && iport == iport_old) {
		LOG(LOG_INFO, "Pinhole for inbound traffic from [%s]:%hu to [%s]:%hu with protocol %s already done. Updating it.", raddr, rport, iaddr_old, iport_old, protocol);
	...
	*uid = add_pinhole (0/*ext_if_name*/, raddr, rport, iaddr, iport, proto, desc, timestamp);
...
#if 0
int upnp_add_inboundpinhole_internal(const char *raddr, unsigned short rport,
		const char *iaddr, unsigned short iport, const char *proto, int *uid)
{
	int c = 9999;
...
#ifdef DEBUG
	LOG(LOG_INFO, "Adding following ip6tables rule:");
	LOG(LOG_INFO, "  -> %s", cmd);
	LOG(LOG_INFO, "  -> %s", cmd_raw);
#endif
 ...
 *  -42  Not implemented
 */
int upnp_get_pinhole_info(unsigned short uid, char *raddr, int raddrlen, unsigned short *rport, char *iaddr, int iaddrlen,
		unsigned short *iport, int *proto, char *desc, int desclen, unsigned int *leasetime, unsigned int *packets)
{
	...
	r = get_pinhole_info(uid, raddr, raddrlen, rport,
			iaddr, iaddrlen, iport, proto, desc, desclen,
...
int upnp_update_inboundpinhole(unsigned short uid, unsigned int leasetime)
{
	...
	timestamp = getuptime(NULL) + leasetime;
...
*/
int upnp_check_pinhole_working(const char *uid, char *eaddr, char *iaddr, unsigned short *eport,
		unsigned short *iport, char *protocol, int *rulenum_used)
{
	...
	fd = fopen(filename, "r");
	if (fd == NULL) {
		LOG(LOG_ERR, "Get_rule: could not open file: %s", filename);
		return -1;
	}

	LOG(LOG_INFO, "Get_rule: Starting getting info in file %s for %s\n", filename, uid);
	buf[sizeof(buf)-1] = 0;
