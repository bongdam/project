...
/* return uid */
static int add_to_pinhole_list(struct in6_addr *saddr, unsigned short sport,
		...
		int proto, const char *desc, unsigned int timestamp)
{
	...
	if (!p) {
		LOG(LOG_ERR, "add_to_pinhole_list calloc() error");
		return -1;
	}
	strcpy(p->desc, desc);
...
static struct ip6t_entry_match *new_match(int proto, unsigned short sport, unsigned short dport)
{
	...
	if (name)
		...
		LOG(LOG_WARNING, "no name for protocol %d", proto);
...
static struct ip6t_entry_target *get_accept_target(void)
{
...
static int ip6tc_init_verify_append(const char *table,
		const char *chain,
		struct ip6t_entry *e)
{
	IP6TC_HANDLE h;
	...
	h = ip6tc_init(table);
	if (!h) {
		LOG(LOG_ERR, "ip6tc_init error : %s", ip6tc_strerror(errno));
		return -1;
	}
	if (!ip6tc_is_chain(chain, h)) {
		LOG(LOG_ERR, "chain %s not found", chain);
		goto error;
	}
	if (!ip6tc_append_entry(chain, e, h)) {
		LOG(LOG_ERR, "ip6tc_append_entry() error : %s", ip6tc_strerror(errno));
		goto error;
	}
	if (!ip6tc_commit(h)) {
		LOG(LOG_ERR, "ip6tc_commit() error : %s", ip6tc_strerror(errno));
		goto error;
	}
	return 0;	/* ok */
error:
		...
		int proto, const char *desc, unsigned int timestamp)
{
	...
	if (!tmp) {
		LOG(LOG_ERR, "%s: realloc(%d) failed",
...
int delete_pinhole(unsigned short uid)
{
	...
	if (!ip6tc_is_chain(miniupnpd_v6_filter_chain, h)) {
		LOG(LOG_ERR, "chain %s not found", miniupnpd_v6_filter_chain);
		goto error;
	}
	index = 0;
	for (e = ip6tc_first_rule(miniupnpd_v6_filter_chain, h);
	    e; e = ip6tc_next_rule(e, h)) {
		if ((e->ipv6.proto == p->proto) &&
			...
			if ((info->spts[0] == p->sport) && (info->dpts[0] == p->dport)) {
				if (!ip6tc_delete_num_entry(miniupnpd_v6_filter_chain, index, h)) {
					LOG(LOG_ERR, "ip6tc_delete_num_entry(%s,%d,...): %s",
							miniupnpd_v6_filter_chain, index, ip6tc_strerror(errno));
					goto error;
				}
				if (!ip6tc_commit(h)) {
					LOG(LOG_ERR, "ip6tc_commit(): %s",
	...
	ip6tc_free(h);
	LOG(LOG_WARNING, "delete_pinhole() rule with PID=%hu not found", uid);
	return -2;	/* not found */
error:
	ip6tc_free(h);
	return -1;
...
int clean_pinhole_list(unsigned int *next_timestamp)
{
	...
	while (p != NULL) {
		if (p->timestamp <= (unsigned int)current_time) {
			...
			LOG(LOG_INFO, "removing expired pinhole with uid=%hu", uid);
			p = p->entries.le_next;
