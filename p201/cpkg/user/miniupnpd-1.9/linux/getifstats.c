...
int getifstats(const char *ifname, struct ifdata *data)
{
	...
	if (current_time == ((time_t)-1)) {
		LOG(LOG_ERR, "getifstats() : time() error : %m");
	} else {
	...
	if (!f) {
		LOG(LOG_ERR, "getifstats() : cannot open /proc/net/dev : %m");
		return -1;
	}
	/* discard the two header lines */
	if (!fgets(line, sizeof(line), f) || !fgets(line, sizeof(line), f)) {
		LOG(LOG_ERR, "getifstats() : error reading /proc/net/dev : %m");
	}
	...
	if (f) {
		...
		LOG(LOG_WARNING, "cannot read %s file : %m", fname);
	}
#ifdef GET_WIRELESS_STATS
