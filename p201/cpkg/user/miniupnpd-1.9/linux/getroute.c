...
int get_src_for_route_to(const struct sockaddr *dst, void *src, size_t *src_len, int *index)
{
...

	{
		...
		DEBUG("get_src_for_route_to (%s)", dst_str);
	}
	/* add address */
	...
	if (fd < 0) {
		LOG(LOG_ERR, "socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE) : %m");
		return -1;
	}
	...
	if (status < 0) {
		LOG(LOG_ERR, "sendmsg(rtnetlink) : %m");
		goto error;
	}
	...
	for (;;) {
		...
		if (status < 0) {
			if (errno == EINTR || errno == EAGAIN)
				continue;
			LOG(LOG_ERR, "recvmsg(rtnetlink) %m");
			goto error;
		}
		if (status == 0) {
			LOG(LOG_ERR, "recvmsg(rtnetlink) EOF");
			goto error;
		}
		for (h = (struct nlmsghdr *)&req.n; status >= (int)sizeof(*h); ) {
			...
			if (l < 0 || len > status) {
				if (msg.msg_flags & MSG_TRUNC) {
					LOG(LOG_ERR, "Truncated message");
				}
				LOG(LOG_ERR, "malformed message: len=%d", len);
				goto error;
			}

			if (nladdr.nl_pid != 0 || h->nlmsg_seq != 1 /*seq*/) {
				LOG(LOG_ERR, "wrong seq = %d\n", h->nlmsg_seq);
			...
			if (h->nlmsg_type == NLMSG_ERROR) {
				...
				LOG(LOG_ERR, "NLMSG_ERROR %d : %s", err->error, strerror(-err->error));
				goto error;
			}
					...
					if (rta->rta_type == RTA_PREFSRC) {
						if (src_len && src) {
							if (*src_len < RTA_PAYLOAD(rta)) {
								LOG(LOG_WARNING, "cannot copy src: %u<%lu",
	...
	LOG(LOG_WARNING, "get_src_for_route_to() : src not found");
error:
	if (fd >= 0)
		close(fd);
	return -1;
