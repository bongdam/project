...
int OpenAndConfNATPMPSockets(int *sockets)
{
	...
	for (i = 0, lan_addr = lan_addrs.lh_first; lan_addr != NULL; lan_addr = lan_addr->list.le_next) {
...
static void FillPublicAddressResponse(unsigned char *resp, in_addr_t senderaddr)
{
	...
	if (use_ext_ip_addr) {
		...
		if (!ext_if_name || ext_if_name[0]=='\0') {
			...
			LOG(LOG_ERR, "Failed to get IP for interface %s", ext_if_name);
			resp[3] = 3;	/* Network Failure (e.g. NAT box itself
			                 * has not obtained a DHCP lease) */
		} else {
	...
	for (lan_addr = lan_addrs.lh_first; lan_addr != NULL; lan_addr = lan_addr->list.le_next) {
		if((senderaddr & lan_addr->mask.s_addr) == (lan_addr->addr.s_addr & lan_addr->mask.s_addr)) {
			memcpy(resp+8, &lan_addr->ext_ip_addr, sizeof(lan_addr->ext_ip_addr));
			break;
		}
	}
#endif
                             ...
                             unsigned char *msg_buff, size_t msg_buff_size)
{
	...
	if ((msg.msg_flags & MSG_TRUNC) || (msg.msg_flags & MSG_CTRUNC)) {
		LOG(LOG_WARNING, "%s: truncated message",
	...
	for (h = CMSG_FIRSTHDR(&msg); h; h = CMSG_NXTHDR(&msg, h)) {
		if (h->cmsg_level == IPPROTO_IPV6 && h->cmsg_type == IPV6_PKTINFO) {
			...
			DEBUG("%s: packet destination: %s scope_id=%u",
			       "ReceiveNATPMPOrPCPPacket",
	...
	n = recvfrom(s, msg_buff, msg_buff_size, 0, senderaddr, senderaddrlen);

	if (n < 0) {
		/* EAGAIN, EWOULDBLOCK and EINTR : silently ignore (retry next time)
		 * other errors : log to LOG_ERR */
		if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) {
			LOG(LOG_ERR, "recvfrom(natpmp): %m");
		}
		return n;
	}
#endif /* IPV6_PKTINFO */
...
void ProcessIncomingNATPMPPacket(int s, unsigned char *msg_buff, int len, struct sockaddr_in *senderaddr)
{
	...
	if (!inet_ntop(AF_INET, &senderaddr->sin_addr, senderaddrstr, sizeof(senderaddrstr))) {
		LOG(LOG_ERR, "inet_ntop(natpmp): %m");
	}

	LOG(LOG_INFO, "NAT-PMP request received from %s:%hu %dbytes",
			senderaddrstr, ntohs(senderaddr->sin_port), n);

	if (n < 2 || ((((req[1]-1) &~ 1) == 0) && n < 12)) {
		LOG(LOG_WARNING, "discarding NAT-PMP request (too short) %dBytes", n);
		return;
	}
	if (req[1] & 128) {
	...
	if (req[0] > 0) {
		/* invalid version */
		LOG(LOG_WARNING, "unsupported NAT-PMP version : %u",
		       (unsigned)req[0]);
		resp[3] = 1;	/* unsupported version */
	} else switch(req[1]) {
		case 0:	/* Public address request */
			LOG(LOG_INFO, "NAT-PMP public address request");
		...
		case 2:	/* TCP port mapping request */
			{
				...
				LOG(LOG_INFO, "NAT-PMP port mapping request : "
				...
				if (lifetime == 0) {
					...
					while (get_redirect_rule_by_index(index, 0,
								...
								desc, sizeof(desc),
								0, 0, &timestamp, 0, 0) >= 0) {
						DEBUG("%d %d %hu->'%s':%hu '%s'", index, proto2, eport2, iaddr2, iport2, desc);
						if (!strcmp(iaddr2, senderaddrstr) && 0 == memcmp(desc, "NAT-PMP", 7)) {
							/* (iport == 0) => remove all the mappings for this client */
							if ((iport == 0) || ((iport == iport2) && (proto == proto2))) {
								r = _upnp_delete_redir(eport2, proto2);
								if (r < 0) {
									LOG(LOG_ERR, "Failed to remove NAT-PMP mapping eport %hu, protocol %s",
											eport2, (proto2==IPPROTO_TCP)?"TCP":"UDP");
									resp[3] = 2;	/* Not Authorized/Refused */
									break;
								} else {
									LOG(LOG_INFO, "NAT-PMP %s port %hu mapping removed",
					...
					while (resp[3] == 0) {
						if (eport_first == 0) { /* first time in loop */
							...
							if (any_eport_allowed == 0) { /* all eports rejected by permissions */
								LOG(LOG_ERR, "No allowed eport for NAT-PMP %hu %s->%s:%hu",
										eport, (proto==IPPROTO_TCP)?"tcp":"udp", senderaddrstr, iport);
								resp[3] = 2;	/* Not Authorized/Refused */
							} else { /* at least one eport allowed (but none available) */
								LOG(LOG_ERR, "Failed to find available eport for NAT-PMP %hu %s->%s:%hu",
										eport, (proto==IPPROTO_TCP)?"tcp":"udp", senderaddrstr, iport);
						...
						r = get_redirect_rule(ext_if_name, eport, proto,
								...
								&timestamp, 0, 0);
						if (r == 0) {
							if (strcmp(senderaddrstr, iaddr_old)==0 && iport==iport_old) {
								/* redirection allready existing */
								LOG(LOG_INFO, "port %hu %s already redirected to %s:%hu, replacing",
										eport, (proto==IPPROTO_TCP)?"tcp":"udp", iaddr_old, iport_old);
								/* remove and then add again */
								if (_upnp_delete_redir(eport, proto) < 0) {
									LOG(LOG_ERR, "failed to remove port mapping");
									break;
								}
							} else {
								eport++;
						...
						if (upnp_redirect_internal(NULL, eport, senderaddrstr, iport, proto, desc, timestamp) < 0) {
							LOG(LOG_ERR, "Failed to add NAT-PMP %hu %s->%s:%hu '%s'",
	...
	if (n < 0) {
		LOG(LOG_ERR, "sendto(natpmp): %m");
	} else if (n < resplen) {
		LOG(LOG_ERR, "sendto(natpmp): sent only %d bytes out of %d", n, resplen);
	}
}
...
void SendNATPMPPublicAddressChangeNotification(int *sockets, int n_sockets)
{
	...
	if (notif[3]) {
		LOG(LOG_WARNING, "%s: cannot get public IP address, stopping",
				"SendNATPMPPublicAddressChangeNotification");
		return;
	}
#endif
	...
	for (j=0; j<n_sockets; j++) {
		...
		if (n < 0) {
			LOG(LOG_ERR, "%s: sendto(s_udp=%d): %m",
