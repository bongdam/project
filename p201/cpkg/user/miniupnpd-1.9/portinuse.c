/* $Id: portinuse.c,v 1.4 2014/04/20 16:46:42 nanard Exp $ */
/* MiniUPnP project
...
int port_in_use(const char *if_name, unsigned eport, int proto, const char *iaddr, unsigned iport)
{
	...
	DEBUG("Check protocol %s for port %d on ext_if %s %s, %08X",
	...
	if (!f) {
		LOG(LOG_ERR, "cannot open %s", (proto==IPPROTO_TCP)?tcpfile:udpfile);
		return -1;
	}
...
#ifdef DEBUG
			DEBUG("port_in_use check port %d and address %s", tmp_port, eaddr);
#endif
			if (tmp_port == eport) {
				char tmp_addr[4];
	...
	if (!kd) {
		LOG(LOG_ERR, "%s: kvm_openfiles(): %s", "portinuse()", errstr);
		return -1;
	}
	if (kvm_nlist(kd, list) < 0) {
		LOG(LOG_ERR, "%s: kvm_nlist(): %s", "portinuse()", kvm_geterr(kd));
		kvm_close(kd);
		return -1;
	}
	n = kvm_read(kd, list[(proto == IPPROTO_TCP) ? 0 : 1].n_value, &table, sizeof(table));
	if (n < 0) {
		LOG(LOG_ERR, "%s: kvm_read(): %s", "portinuse()", kvm_geterr(kd));
		kvm_close(kd);
		return -1;
	}
	...
	while (next != NULL) {
		...
		if (n < 0) {
			LOG(LOG_ERR, "kvm_read(): %s", kvm_geterr(kd));
			break;
		}
		...
		if ((inpcb.inp_flags & INP_IPV6) != 0)
			continue;
#ifdef DEBUG
		DEBUG("%08lx:%hu %08lx:%hu", (u_long)inpcb.inp_laddr.s_addr, ntohs(inpcb.inp_lport),
	...
	switch (proto) {
		...
		case IPPROTO_UDP:
			...
			break;
		default:
			LOG(LOG_ERR, "port_in_use() unknown proto=%d", proto);
			return -1;
	}

	if (sysctlbyname(varname, NULL, &len, NULL, 0) < 0) {
		LOG(LOG_ERR, "sysctlbyname(%s, NULL, ...): %m", varname);
		return -1;
	}
	buf = malloc(len);
	if (buf == NULL) {
		LOG(LOG_ERR, "malloc(%u) failed", (unsigned)len);
		return -1;
	}
	if (sysctlbyname(varname, buf, &len, NULL, 0) < 0) {
		LOG(LOG_ERR, "sysctlbyname(%s, buf, ...): %m", varname);
		free(buf);
		return -1;
	}
	...
	for (so_begin = buf, so_end = (uint8_t *)so_begin + len;
		...
		switch (proto) {
			case IPPROTO_TCP:
				...
				if (xtp->xt_len != sizeof *xtp) {
					LOG(LOG_WARNING, "struct xtcpcb size mismatch; %ld vs %ld", (long)xtp->xt_len, sizeof *xtp);
					free(buf);
					return -1;
				}
				inp = &xtp->xt_inp;
			...
			case IPPROTO_UDP:
				...
				if (xip->xi_len != sizeof *xip) {
					LOG(LOG_WARNING, "struct xinpcb size mismatch : %ld vs %ld",
		...
		if ((inp->inp_vflag & INP_IPV6) != 0)
			continue;
		DEBUG("%08lx:%hu %08lx:%hu <=> %hu %08lx:%hu", (u_long)inp->inp_laddr.s_addr, ntohs(inp->inp_lport),
				(u_long)inp->inp_faddr.s_addr, ntohs(inp->inp_fport), eport, (u_long)ip_addr.s_addr, iport);
	...
	if (xig->xig_len != sizeof *xig) {
		LOG(LOG_WARNING, "struct xinpgen size mismatch; %ld vs %ld",
	...
	while (1) {
		...
		switch (proto) {
			...
			case IPPROTO_UDP:
				...
				if (xip->xi_len != sizeof *xip) {
					LOG(LOG_WARNING, "struct xinpcb size mismatch : %ld vs %ld", (long)xip->xi_len, sizeof *xip);
					free(buf);
					return -1;
				}
				inp = &xip->xi_inp;
	...
	if (!found) {
		...
		for (i = 0; chains_to_check[i]; i++) {
			if (get_nat_redirect_rule(chains_to_check[i], if_name, eport, proto,
						iaddr_old, sizeof(iaddr_old), &iport_old, 0, 0, 0, 0, 0, 0, 0) == 0) {
				DEBUG("port_in_use check port %d on nat chain %s redirected to %s port %d", eport,
