		...
		int proto, const char *desc, unsigned int timestamp)
{
	...
	if (fd==NULL) {
		LOG(LOG_ERR, "could not open lease file: %s", lease_file);
		return -1;
	}
...
static int lease_file_remove(unsigned short eport, int proto)
{
	...
	if (strlen(lease_file) + 7 > sizeof(tmpfilename)) {
		LOG(LOG_ERR, "Lease filename is too long");
		return -1;
	}
	...
	if (tmp==-1) {
		fclose(fd);
		LOG(LOG_ERR, "could not open temporary lease file");
		return -1;
	}
	...
	if (rename(tmpfilename, lease_file) < 0) {
		LOG(LOG_ERR, "could not rename temporary lease file to %s", lease_file);
		remove(tmpfilename);
	}
...
int reload_from_lease_file()
{
	...
	if (unlink(lease_file) < 0) {
		LOG(LOG_WARNING, "could not unlink file %s : %m", lease_file);
	}

	current_time = getuptime(NULL);
	while (fgets(line, sizeof(line), fd)) {
		DEBUG("parsing lease file line '%s'", line);
		proto = line;
		p = strchr(line, ':');
		if (!p) {
			LOG(LOG_ERR, "unrecognized data in lease file");
			continue;
		}
		*(p++) = '\0';
		iaddr = strchr(p, ':');
		...
		*(desc++) = '\0';
		...
		if (timestamp > 0) {
			if (timestamp <= (unsigned int)current_time) {
				LOG(LOG_NOTICE, "already expired lease in lease file");
				continue;
			} else {
		...
		if (r == -1) {
			LOG(LOG_ERR, "Failed to redirect %hu -> %s:%hu protocol %s", eport, iaddr, iport, proto);
		} else if (r == -2) {
			/* Add the redirection again to the lease file */
			lease_file_add(eport, iaddr, iport, proto_atoi(proto), desc, timestamp);
		}
	}
	fclose(fd);
 ...
 */
int
upnp_redirect(const char *rhost, unsigned short eport, const char *iaddr, unsigned short iport,
		const char *protocol, const char *desc, unsigned int leaseduration)
{
	int proto, r;
	char iaddr_old[32];
	...
	if (inet_aton(iaddr, &address) <= 0) {
		LOG(LOG_ERR, "inet_aton(%s) FAILED", iaddr);
		return -1;
	}

	if (!check_upnp_rule_against_permissions(upnppermlist, num_upnpperm, eport, address, iport)) {
		LOG(LOG_INFO, "redirection permission check failed for " "%hu->%s:%hu %s", eport, iaddr, iport, protocol);
		return -3;
	}
	r = get_redirect_rule(ext_if_name, eport, proto, iaddr_old, sizeof(iaddr_old), &iport_old, 0, 0,
	                      0, 0, &timestamp, 0, 0);
	if (r == 0) {
		...
		if (strcmp(iaddr, iaddr_old)==0 && iport==iport_old) {
			LOG(LOG_INFO, "ignoring redirect request as it matches existing redirect");
		} else {
			LOG(LOG_INFO, "port %hu protocol %s already redirected to %s:%hu",
					eport, protocol, iaddr_old, iport_old);
			return -2;
		}
#ifdef CHECK_PORTINUSE
	} else if (port_in_use(ext_if_name, eport, proto, iaddr, iport) > 0) {
		LOG(LOG_INFO, "port %hu protocol %s already in use",
				eport, protocol);
		return -2;
#endif /* CHECK_PORTINUSE */
	} else {
		timestamp = (leaseduration > 0) ? getuptime(NULL) + leaseduration : 0;
		LOG(LOG_INFO, "redirecting port %hu to %s:%hu protocol %s for: %s",
...
int upnp_redirect_internal(const char *rhost, unsigned short eport, const char *iaddr,
		unsigned short iport, int proto, const char *desc, unsigned int timestamp)
{
	/*LOG(LOG_INFO, "redirecting port %hu to %s:%hu protocol %s for: %s",
		eport, iaddr, iport, protocol, desc);			*/
	if (add_redirect_rule2(ext_if_name, rhost, eport, iaddr, iport, proto, desc, timestamp) < 0) {
		return -1;
	}
...
#endif
/*	LOG(LOG_INFO, "creating pass rule to %s:%hu protocol %s for: %s",
...
int upnp_get_redirection_infos(unsigned short eport, const char *protocol, unsigned short *iport, char *iaddr,
		int iaddrlen, char *desc, int desclen, char *rhost, int rhostlen, unsigned int *leaseduration)
{
	int r;
	unsigned int timestamp;
	time_t current_time;
	...
	r = get_redirect_rule(ext_if_name, eport, proto_atoi(protocol),
			...
			rhost, rhostlen, &timestamp, 0, 0);
	if (r == 0 && timestamp > 0 && timestamp > (unsigned int)(current_time = getuptime(NULL))) {
		*leaseduration = timestamp - current_time;
	} else {
		*leaseduration = 0;
	}
...
int upnp_get_redirection_infos_by_index(int index, unsigned short *eport, char *protocol, unsigned short *iport,
		char *iaddr, int iaddrlen, char *desc, int desclen, char *rhost, int rhostlen, unsigned int *leaseduration)
{
	...
	if (rhost && (rhostlen > 0))
		rhost[0] = '\0';
	if (get_redirect_rule_by_index(index, 0/*ifname*/, eport, iaddr, iaddrlen, iport, &proto, desc, desclen,
				rhost, rhostlen, &timestamp, 0, 0) < 0)
		return -1;
	else {
		current_time = getuptime(NULL);
		*leaseduration = (timestamp > (unsigned int)current_time) ? (timestamp - current_time) : 0;
...
{
	LOG(LOG_INFO, "removing redirect rule port %hu %s", eport, protocol);
...
struct rule_state *get_upnp_rules_state_list(int max_rules_number_target)
{
	...
	while (get_redirect_rule_by_index(i, /*ifname*/0, &tmp->eport, 0, 0,
				&iport, &proto, 0, 0, 0,0, &timestamp, &tmp->packets, &tmp->bytes) >= 0) {
		tmp->to_remove = 0;
		if (timestamp > 0) {
	...
	while (get_peer_rule_by_index(i, /*ifname*/0, &tmp->eport, 0, 0,
				&iport, &proto, 0, 0, 0,0,0, &timestamp, &tmp->packets, &tmp->bytes) >= 0) {
		tmp->to_remove = 0;
		if (timestamp > 0) {
			/* need to remove this port mapping ? */
			if (timestamp <= (unsigned int)current_time)
	...
	for (p = &list, tmp = list; tmp; tmp = *p) {
		if (tmp->to_remove) {
			LOG(LOG_NOTICE, "remove port mapping %hu %s because it has expired",
...
void remove_unused_rules(struct rule_state *list)
{
	...
	while (list) {
		...
		if (get_redirect_rule(ifname, list->eport, list->proto, 0, 0, &iport,
					0, 0, 0, 0, &timestamp, &packets, &bytes) >= 0) {
	...
	if (n > 0)
		LOG(LOG_NOTICE, "removed %d unused rules", n);
}
...
unsigned short *upnp_get_portmappings_in_range(unsigned short startport,
		unsigned short endport, const char *protocol, unsigned int *number)
{
	int proto;
	proto = proto_atoi(protocol);
	if (!number)
...
#ifdef USE_MINIUPNPDCTL
void write_ruleset_details(int s)
{
	int proto = 0;
	unsigned short eport, iport;
	char desc[64];
	...
	while (get_redirect_rule_by_index(i, 0/*ifname*/, &eport, iaddr, sizeof(iaddr),
				&iport, &proto, desc, sizeof(desc), rhost, sizeof(rhost), &timestamp, &packets, &bytes) >= 0) {
