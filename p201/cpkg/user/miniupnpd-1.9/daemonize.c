...
int writepidfile(const char *fname, int pid)
{
	...
	if ((pidfile = open(fname, O_WRONLY|O_CREAT, 0644)) < 0) {
		LOG(LOG_ERR, "Unable to open pidfile for writing %s: %m", fname);
		return -1;
	}

	pidstringlen = snprintf(pidstring, sizeof(pidstring), "%d\n", pid);
	if (pidstringlen <= 0) {
		LOG(LOG_ERR, "Unable to write to pidfile %s: snprintf(): FAILED", fname);
		close(pidfile);
		return -1;
	} else {
		if (write(pidfile, pidstring, pidstringlen) < 0)
			LOG(LOG_ERR, "Unable to write to pidfile %s: %m", fname);
	}
