          ...
          struct in_addr *addr, struct in_addr *mask)
{
	...
	if (s < 0) {
		LOG(LOG_ERR, "socket(PF_INET, SOCK_DGRAM): %m");
		return -1;
	}
	strncpy(ifr.ifr_name, ifname, IFNAMSIZ);
	if (ioctl(s, SIOCGIFFLAGS, &ifr, &ifrlen) < 0) {
		LOG(LOG_DEBUG, "ioctl(s, SIOCGIFFLAGS, ...): %m");
		close(s);
		return -1;
	}
	if ((ifr.ifr_flags & IFF_UP) == 0) {
		LOG(LOG_DEBUG, "network interface %s is down", ifname);
		close(s);
		return -1;
	}
	strncpy(ifr.ifr_name, ifname, IFNAMSIZ);
	if (ioctl(s, SIOCGIFADDR, &ifr, &ifrlen) < 0) {
		LOG(LOG_ERR, "ioctl(s, SIOCGIFADDR, ...): %m");
		close(s);
		return -1;
	}
	...
	if (mask) {
		...
		if (ioctl(s, SIOCGIFNETMASK, &ifr, &ifrlen) < 0) {
			LOG(LOG_ERR, "ioctl(s, SIOCGIFNETMASK, ...): %m");
			close(s);
			return -1;
		}
#ifdef ifr_netmask
	...
	for (ife = ifap; ife; ife = ife->ifa_next) {
		...
		switch (ife->ifa_addr->sa_family) {
			case AF_INET:
				...
				if (addr)
					*addr = ((struct sockaddr_in *)ife->ifa_addr)->sin_addr;
				if (mask)
					*mask = ((struct sockaddr_in *)ife->ifa_netmask)->sin_addr;
				break;
/*
		   case AF_INET6:
		 	  inet_ntop(ife->ifa_addr->sa_family, &((struct sockaddr_in6 *)ife->ifa_addr)->sin6_addr, buf, len
*/
		}
	}
	freeifaddrs(ifap);
		...
		switch (ife->ifa_addr->sa_family) {
			case AF_INET:
				...
				memcpy(addr->s6_addr + 12,
						&(((struct sockaddr_in *)ife->ifa_addr)->sin_addr.s_addr), 4);
				found = 1;
				break;
			...
			case AF_INET6:
				...
				if (!IN6_IS_ADDR_LOOPBACK(&tmpaddr->sin6_addr)
					...
					memcpy(addr->s6_addr, &tmpaddr->sin6_addr, 16);
					found = 1;
				}
				break;
#endif /* ENABLE_IPV6 */
