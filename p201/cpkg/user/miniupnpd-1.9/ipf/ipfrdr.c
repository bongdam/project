...
int init_redirect(void)
{
	...
	if (dev < 0) {
		LOG(LOG_ERR, "open(\"%s\"): %m", IPNAT_NAME);
		return -1;
	}
	dev_ipl = open(IPL_NAME, O_RDWR);
	if (dev_ipl < 0) {
		LOG(LOG_ERR, "open(\"%s\"): %m", IPL_NAME);
		return -1;
	}
	return 0;
}
    ...
    int proto, const char * desc, unsigned int timestamp)
{
	...
	if (dev < 0) {
		LOG(LOG_ERR, "%s not open", IPNAT_NAME);
		return -1;
	}
    ...
    u_int64_t * packets, u_int64_t * bytes)
{
	...
	do {
		if (ioctl(dev, SIOCGENITER, &obj) == -1) {
			LOG(LOG_ERR, "ioctl(dev, SIOCGENITER): %m");
			break;
		}
#if IPFILTER_VERSION >= 5000000
    ...
    u_int64_t * packets, u_int64_t * bytes)
{
	...
	do {
		if (ioctl(dev, SIOCGENITER, &obj) == -1) {
			LOG(LOG_ERR, "%s:ioctl(SIOCGENITER): %m",
...
real_delete_redirect_rule(const char * ifname, unsigned short eport, int proto)
{
	...
	do {
...
#endif
		{
			...
			if (r == -1)
				LOG(LOG_ERR, "%s:ioctl(SIOCRMNAT): %m",
				    "delete_redirect_rule");
    ...
    int proto, const char * desc)
{
	...
	if (dev_ipl < 0) {
		LOG(LOG_ERR, "%s not open", IPL_NAME);
		return -1;
	}
...
delete_filter_rule(const char * ifname, unsigned short eport, int proto)
{
	...
	do {
		...
		if (ioctl(dev_ipl, SIOCIPFITER, &wobj) == -1) {
			LOG(LOG_ERR, "ioctl(SIOCIPFITER): %m");
			break;
		}
	...
	do {
...
#endif
		{
			if(*number >= capacity)
			{
				...
				if(!array)
				{
					LOG(LOG_ERR, "get_portmappings_in_range() : realloc(%lu) error", sizeof(unsigned short)*capacity);
					*number = 0;
					return NULL;
				}
			}
