 ...
 */
static int AddMulticastMembership(int s, in_addr_t ifaddr)
{
	...
	if (setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&imr, sizeof(struct ip_mreq)) < 0) {
		LOG(LOG_ERR, "setsockopt(udp, IP_ADD_MEMBERSHIP): %m");
		return -1;
	}
...
#ifdef ENABLE_IPV6
static int AddMulticastMembershipIPv6(int s, unsigned int ifindex)
{
	struct ipv6_mreq mr;
...
#define IPV6_ADD_MEMBERSHIP IPV6_JOIN_GROUP
	...
	if (setsockopt(s, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mr, sizeof(struct ipv6_mreq)) < 0) {
		LOG(LOG_ERR, "setsockopt(udp, IPV6_ADD_MEMBERSHIP): %m");
		return -1;
	}
...
int OpenAndConfSSDPReceiveSocket(int ipv6)
{
	...
	if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &j, sizeof(j)) < 0) {
		LOG(LOG_WARNING, "setsockopt(udp, SO_REUSEADDR): %m");
	}

	if (!set_non_blocking(s)) {
		LOG(LOG_WARNING, "%s: set_non_blocking(): %m",
				"OpenAndConfSSDPReceiveSocket");
	}

	if (bind(s, (struct sockaddr *)&sockname, sockname_len) < 0) {
		LOG(LOG_ERR, "%s: bind(udp%s): %m",
...
static int OpenAndConfSSDPNotifySocket(in_addr_t addr)
{
	int s;
	...
	if ((s = socket(PF_INET, SOCK_DGRAM, 0)) < 0) {
		LOG(LOG_ERR, "socket(udp_notify): %m");
		return -1;
	}
	...
	if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_LOOP, (char *)&loopchar, sizeof(loopchar)) < 0) {
		LOG(LOG_ERR, "setsockopt(udp_notify, IP_MULTICAST_LOOP): %m");
		close(s);
		return -1;
	}

	if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_IF, (char *)&mc_if, sizeof(mc_if)) < 0) {
		LOG(LOG_ERR, "setsockopt(udp_notify, IP_MULTICAST_IF): %m");
		close(s);
		return -1;
	}

	if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl)) < 0) {
		LOG(LOG_WARNING, "setsockopt(udp_notify, IP_MULTICAST_TTL,): %m");
	}

	if (setsockopt(s, SOL_SOCKET, SO_BROADCAST, &bcast, sizeof(bcast)) < 0) {
		LOG(LOG_ERR, "setsockopt(udp_notify, SO_BROADCAST): %m");
		close(s);
		return -1;
	}
...
static int OpenAndConfSSDPNotifySocketIPv6(unsigned int if_index)
{
	int s;
	unsigned int loop = 0;
	...
	if (s < 0) {
		LOG(LOG_ERR, "socket(udp_notify IPv6): %m");
		return -1;
	}
	if (setsockopt(s, IPPROTO_IPV6, IPV6_MULTICAST_IF, &if_index, sizeof(if_index)) < 0) {
		LOG(LOG_ERR, "setsockopt(udp_notify IPv6, IPV6_MULTICAST_IF, %u): %m", if_index);
		close(s);
		return -1;
	}
	if (setsockopt(s, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &loop, sizeof(loop)) < 0) {
		LOG(LOG_ERR, "setsockopt(udp_notify, IPV6_MULTICAST_LOOP): %m");
		close(s);
		return -1;
	}
	...
	if (bind(s, (struct sockaddr *)&sockname, sizeof(sockname)) < 0) {
		LOG(LOG_ERR, "bind(udp_notify IPv6): %m");
		close(s);
		return -1;
	}
                             ...
                             struct lan_addr_s *lan_addr, int n_lan_addr)*/
{
	...
	for (i=0, lan_addr = lan_addrs.lh_first;
			lan_addr != NULL; lan_addr = lan_addr->list.le_next) {
 ...
 */
static void SendSSDPResponse(int s, const struct sockaddr *addr,
		...
		const char *uuidvalue, unsigned int delay)
{
	...
	st_is_uuid = ((st_len == (int)strlen(uuidvalue)) && (memcmp(uuidvalue, st, st_len) == 0));
	/*
	...
	if (l < 0) {
		LOG(LOG_ERR, "%s: snprintf failed %m",
				"SendSSDPResponse()");
		return;
	} else if ((unsigned)l>=sizeof(buf)) {
		LOG(LOG_WARNING, "%s: truncated output (%u>=%u)",
	...
	DEBUG("%s: %d bytes to %s ST: %.*s", "SendSSDPResponse()",
			n, addr_str, l, buf);
	if (n < 0) {
		LOG(LOG_ERR, "%s: sendto(udp): %m", "SendSSDPResponse()");
	}
}
...
static void SendSSDPNotify(int s, const struct sockaddr *dest, socklen_t dest_len,
		const char *dest_str, const char *host, unsigned short http_port,
#ifdef ENABLE_HTTPS
		unsigned short https_port,
#endif
		const char *nt, const char *suffix,
		const char *usn1, const char *usn2, const char *usn3, unsigned int lifetime)
{
	char bufr[SSDP_PACKET_MAX_LEN];
	int n, l;

	l = snprintf(bufr, sizeof(bufr), "NOTIFY * HTTP/1.1\r\n"
		"Host: %s:%d\r\n"
	...
	if (l < 0) {
		LOG(LOG_ERR, "%s: snprintf error", "SendSSDPNotify()");
		return;
	...
	if (n < 0) {
		LOG(LOG_ERR, "sendto(udp_notify=%d, %s): %m", s,
		       host ? host : "NULL");
	} else if (n != l) {
		LOG(LOG_NOTICE, "sendto() sent %d out of %d bytes", n, l);
	}
...
#ifdef ENABLE_HTTPS
static void SendSSDPNotifies(int s, const char *host, unsigned short http_port,
		unsigned short https_port,
...
#endif
{
	...
	for (j = 0; (mcast_addrs[j].p1 != 0 && ipv6) || j < 1; j++) {
		...
		for (i = 0; known_service_types[i].s; i++) {
			...
			if (!memcmp(known_service_types[i].s,
...
#endif
{
	...
	n = recvfrom(s, bufr, sizeof(bufr), 0, (struct sockaddr *)&sendername, &len_r);
	if (n < 0) {
		/* EAGAIN, EWOULDBLOCK, EINTR : silently ignore (try again next time)
		 * other errors : log to LOG_ERR */
		if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) {
			LOG(LOG_ERR, "recvfrom(udp): %m");
		}
		return;
	}
#ifdef ENABLE_HTTPS
	ProcessSSDPData(s, bufr, n, (struct sockaddr *)&sendername, http_port, https_port);
#else
	ProcessSSDPData(s, bufr, n, (struct sockaddr *)&sendername, http_port);
#endif
...
#endif
{
	...
	if (lan_addr == NULL) {
		LOG(LOG_WARNING, "SSDP packet sender %s not from a LAN, ignoring", sender_str);
		return;
	}
	...
	if (memcmp(bufr, "NOTIFY", 6) == 0) {
		...
		while (i < n) {
			...
			if ((i < n - 3) && (strncasecmp(bufr+i, "st:", 3) == 0)) {
				...
				while (l > 0 && st[l-1] != ':')
					l--;
				st_ver = atoi(st+l);
				DEBUG("ST: %.*s (ver=%d)", st_len, st, st_ver);
				/*j = 0;*/
				/*while(bufr[i+j]!='\r') j++;*/
				/*LOG(LOG_INFO, "%.*s", j, bufr+i);*/
			}
#if defined(UPNP_STRICT) || defined(DELAY_MSEARCH_RESPONSE)
				...
				while (mx[mx_len]!='\r' && mx[mx_len]!='\n' && (mx + mx_len < bufr + n))
					mx_len++;
				mx_value = atoi(mx);
				DEBUG("MX: %.*s (value=%d)", mx_len, mx, mx_value);
			}
#endif
		}
#ifdef UPNP_STRICT
		...
		if (mx_value < 0) {
			LOG(LOG_INFO, "ignoring SSDP packet missing MX: header");
			return;
		} else if (mx_value > 5) {
...
#endif
		/*LOG(LOG_INFO, "SSDP M-SEARCH packet received from %s",
	           sender_str );*/
		if (st && (st_len > 0)) {
			LOG(LOG_INFO, "SSDP M-SEARCH from %s ST: %.*s",
			...
			if (sender->sa_family == AF_INET) {
				...
				if (get_src_for_route_to (sender, &addr6, &addr6_len, &index) < 0) {
					LOG(LOG_WARNING, "get_src_for_route_to() failed, using %s", ipv6_addr_for_http_with_brackets);
					announced_host = ipv6_addr_for_http_with_brackets;
				} else {
					if (inet_ntop(AF_INET6, &addr6, announced_host_buf + 1, sizeof(announced_host_buf) - 2)) {
						...
						if (i < (int)sizeof(announced_host_buf) - 1) {
							...
							LOG(LOG_NOTICE, "cannot suffix %s with ']'",
									announced_host_buf);
						}
						announced_host = announced_host_buf;
					} else {
						LOG(LOG_NOTICE, "inet_ntop() failed %m");
						announced_host = ipv6_addr_for_http_with_brackets;
					}
				}
#else
			...
			for (i = 0; known_service_types[i].s; i++) {
				   ...
				   )
				{
					...
					if (i == 0)
						...
						snprintf(ver_str, sizeof(ver_str), "%d", known_service_types[i].version);
#endif
					LOG(LOG_INFO, "Single search found");
#ifdef DELAY_MSEARCH_RESPONSE
					delay = random() / (1 + RAND_MAX / (1000 * mx_value));
#ifdef DEBUG
					DEBUG("mx=%dsec delay=%ums", mx_value, delay);
#endif
#endif
			...
			if (l == st_len) {
				...
				delay = random() / (1 + RAND_MAX / (1000 * mx_value));
#endif
				if (!memcmp(st, uuidvalue_igd, l)) {
					LOG(LOG_INFO, "ssdp:uuid (IGD) found");
...
#endif
							uuidvalue_igd, delay);
				} else if (!memcmp(st, uuidvalue_wan, l)) {
					LOG(LOG_INFO, "ssdp:uuid (WAN) found");
					SendSSDPResponse(s, sender, st, st_len, "",
							announced_host, http_port,
#ifdef ENABLE_HTTPS
							https_port,
#endif
							uuidvalue_wan, delay);
				} else if (!memcmp(st, uuidvalue_wcd, l)) {
					LOG(LOG_INFO, "ssdp:uuid (WCD) found");
					SendSSDPResponse(s, sender, st, st_len, "",
							announced_host, http_port,
#ifdef ENABLE_HTTPS
							https_port,
			...
			LOG(LOG_INFO, "Invalid SSDP M-SEARCH from %s", sender_str);
		}
	} else {
		LOG(LOG_NOTICE, "Unknown udp packet received from %s", sender_str);
	}
}

static int SendSSDPbyebye(int s, const struct sockaddr *dest, socklen_t destlen,
		const char *dest_str, const char *nt, const char *suffix,
		const char *usn1, const char *usn2, const char *usn3)
{
	...
	if (l < 0) {
		LOG(LOG_ERR, "%s: snprintf error", "SendSSDPbyebye()");
		return -1;
	} else if ((unsigned int)l >= sizeof(bufr)) {
		LOG(LOG_WARNING, "%s: truncated output (%u>=%u)",
	...
	if (n < 0) {
		LOG(LOG_ERR, "sendto(udp_shutdown=%d): %m", s);
		return -1;
	} else if (n != l) {
		LOG(LOG_NOTICE, "sendto() sent %d out of %d bytes", n, l);
		return -1;
...
int SubmitServicesToMiniSSDPD(const char *host, unsigned short port) {
	...
	if (s < 0) {
		LOG(LOG_ERR, "socket(unix): %m");
		return -1;
	}
	addr.sun_family = AF_UNIX;
	strncpy(addr.sun_path, minissdpdsocketpath, sizeof(addr.sun_path));
	if (connect(s, (struct sockaddr *)&addr, sizeof(struct sockaddr_un)) < 0) {
		LOG(LOG_ERR, "connect(\"%s\"): %m", minissdpdsocketpath);
		close(s);
		return -1;
	}
	for (i = 0; known_service_types[i].s; i++) {
		...
		if (l < 0) {
			LOG(LOG_WARNING, "SubmitServicesToMiniSSDPD: snprintf %m");
			continue;
