...
static const struct serviceDesc scpdWANIPCn = {WANIPCnActions, WANIPCnVars};
...
static const struct serviceDesc scpd6FC = {IPv6FCActions, IPv6FCVars};
#endif
...
static const struct serviceDesc scpdDP = {DPActions, DPVars};
#endif
	...
	while (acts[i].name) {
		...
		if (args) {
			...
			while (args[j].dir) {
				...
				} else if (!memcmp(p, "PortMapping", 11)
						&& 0 != memcmp(p + 11, "Description", 11)) {
					if (!memcmp(p + 11, "NumberOfEntries", 15)) {
						/* PortMappingNumberOfEntries */
#ifdef IGD_V2
						if (!memcmp(acts[i].name, "GetListOfPortMappings", 22)) {
...
#ifdef IGD_V2
				} else if (!memcmp(p, "A_ARG_TYPE_", 11)) {
					str = strcat_str(str, len, &tmplen, p + 11);
				} else if (!memcmp(p, "ExternalPort", 13)
						&& args[j].dir == 2
				...
				str = strcat_str(str, len, &tmplen, "</direction><relatedStateVariable>");
				str = strcat_str(str, len, &tmplen, p);
				str = strcat_str(str, len, &tmplen, "</relatedStateVariable></argument>");
				j++;
			}
			str = strcat_str(str, len, &tmplen, "</argumentList>");
		}
...
#ifdef ENABLE_EVENTS
static char *genEventVars(int *len, const struct serviceDesc *s)
{
	char tmp[16];
...
{
	return genEventVars(l, &scpdWANIPCn);
}

char *getVarsWANCfg(int *l)
{
	return genEventVars(l, &scpdWANCfg);
}

#ifdef ENABLE_L3F_SERVICE
char *getVarsL3F(int *l)
{
	return genEventVars(l, &scpdL3F);
}
#endif
