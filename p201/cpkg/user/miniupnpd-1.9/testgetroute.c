...
main(int argc, char ** argv)
{
	...
	OPEN_LOG("testgetroute", LOG_CONS|LOG_PERROR, LOG_USER);
	r = inet_pton (AF_INET, argv[1], &dst4.sin_addr);
	if(r < 0) {
		LOG(LOG_ERR, "inet_pton(AF_INET, %s) : %m", argv[1]);
		CLOSE_LOG();
		return 2;
	}
	if (r == 0) {
		...
		if(r < 0) {
			LOG(LOG_ERR, "inet_pton(AF_INET6, %s) : %m", argv[1]);
			CLOSE_LOG();
			return 2;
		} else if(r > 0) {
			...
			/* r == 0 */
			LOG(LOG_ERR, "%s is not a correct IPv4 or IPv6 address", argv[1]);
			CLOSE_LOG();
			return 1;
		}
	} else {
	...
	if (dst) {
		DEBUG("calling get_src_for_route_to(%p, %p, %p(%u), %p)",
		       dst, src, &src_len, (unsigned)src_len, &index);
		r = get_src_for_route_to (dst, src, &src_len, &index);
		DEBUG("get_src_for_route_to() returned %d", r);
		if(r >= 0) {
			char src_str[128];
