...
#define bb_show_usage() exit(EXIT_FAILURE)

static int xopen(const char *pathname, int flags)
{
	int ret = open(pathname, flags, 0666);
	if (ret < 0)
		error_msg_and_die("can't open '%s': %m", pathname);
	return ret;
}
...
static int ioctl_or_perror_and_die(int fd, unsigned request, void *argp, const char *fmt,...)
{
	int ret;
	va_list p;
...
static unsigned xatou(const char *numstr)
{
	unsigned long v;
	if (safe_strtoul(numstr, &v, 0))
		exit(EXIT_FAILURE);
	return (unsigned)v;
}

static unsigned xatoul_range(const char *numstr, long low, long high)
{
	long v;
	if (safe_strtoul(numstr, (unsigned long *)&v, 0) || v < low || v > high)
		exit(EXIT_FAILURE);
	return (unsigned)v;
}
...
static int ethctl_lock(int lock_fd)
{
	struct flock lock;
	lock.l_type = F_WRLCK;
	lock.l_start = 0;
	...
	return TEMP_FAILURE_RETRY(fcntl(lock_fd, F_SETLKW, &lock));
}

#define ethctl_unlock(lock_fd) close(lock_fd)
#endif
...
static int vconfig_main(int argc, char **argv)
{
	...
	if (!strcmp(argv[1], "addsmux") || !strcmp(argv[1], "remsmux")) {
		...
		if (fd < 0) {
			fprintf(stderr, "Error!Socket create fail in vconfig.\n");
#if defined(RTL_FILE_LOCK_SUPPORT)
			ethctl_unlock(lock_fd);
#endif
			return 1;
		}
		...
		while (idx < argc) {
			if (!strcmp(argv[idx], "napt"))
				...
				if (sifr.vid <= 9) {
					fprintf(stderr, "vid 0-9 are reserved.\n");
					goto arg_err_rtn;
				}
			}
			idx++;
		...
		if (ioctl(fd, SIOCSIFSMUX, &sifr) < 0) {
			fprintf(stderr, "Error ioctl(SIOCSIFSMUX) in vconfig\n");
			close(fd);
#if defined(RTL_FILE_LOCK_SUPPORT)
			ethctl_unlock(lock_fd);
#endif
	...
	if (!strcmp(argv[1], "port_mapping")) {
		...
		lock_fd = open("/tmp/ethctl_lock", O_RDWR | O_CREAT | O_TRUNC);
		if (lock_fd < 0) {
			fprintf(stderr, "create ethctl file lock erro\n");
		}
		ethctl_lock(lock_fd);
#endif
		...
		if (ioctl(fd, SIOCSITFGROUP, &ifr) < 0) {
			fprintf(stderr, "Error set port mapping in vconfig\n");
			close(fd);
#if defined(RTL_FILE_LOCK_SUPPORT)
			ethctl_unlock(lock_fd);
#endif
