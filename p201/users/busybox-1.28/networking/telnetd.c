...
enum {
	OPT_WATCHCHILD = (1 << 3), /* -K */
	OPT_INETD      = (1 << 4) * ENABLE_FEATURE_TELNETD_STANDALONE, /* -i */
...
int telnetd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	unsigned opt, now = now;
	int count;
	struct tsession *ts, *oldest;
#if ENABLE_FEATURE_TELNETD_STANDALONE
#define IS_INETD (opt & OPT_INETD)
	...
	opt = getopt32(argv, "^"
			...
			&G.issuefile, &G.loginpath, &G.timeout
...
*/
 again:
	...
	while (ts) {
		...
		if (ts->shell_pid == -1 || ((now - ts->tstamp) > G.timeout)) {
			...
			if (!oldest || ((signed)ts->tstamp - (signed)oldest->tstamp) < 0)
				oldest = ts;
		}
		ts = next;
	}
...
#endif
		if (G.timeout != -1U && oldest) {
			tv.tv_sec = G.timeout - (now - oldest->tstamp);
			tv_ptr = &tv;
		}
