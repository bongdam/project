...
char strgetregpg[] = "wlan0     get_reg_pg:\n";
...
{
	system("iptables -D INPUT -p udp --dport " __tostring(MYPORT) " -j ACCEPT");
}
...
int main(void)
{
	...
	atexit(cleanup);
	system("iptables -D INPUT -p udp --dport " __tostring(MYPORT) " -j ACCEPT 2>/dev/null");
	system("iptables -I INPUT -p udp --dport " __tostring(MYPORT) " -j ACCEPT");
	...
	while (1) {
		...
		rwHW = 0;
		if ((numbytes = TEMP_FAILURE_RETRY(recvfrom(sockfd, buf, BUFLEN, 0,
		...
		if (!memcmp(buf, "iwpriv wlan0 mp_tssi", 20)) {
				...
				if (fp) {
					...
					buf[BUFLEN - 1] = '\0';
					{
						...
						if (strncmp(buf, strreadde, strlen(strreadde)) == 0)
							get_read_reg_value(fp, buf, BUFLEN);
						if (strncmp(buf, strgetregpg, strlen(strgetregpg)) == 0)
							get_read_reg_value(fp, buf, BUFLEN);
			...
			if (!memcmp(buf, "iwpriv wlan0 mp_brx stop", 24)) {
			...
			} else if (!memcmp(buf, "iwpriv wlan0 get_reg_pg", 23)) {
				strcat(buf, " > /tmp/MP.txt");
				system_if(buf);
				...
				strcat(buf, " > /tmp/MP.txt");
				system_if(buf);
			} else if (!memcmp(buf, "iwpriv wlan1 get_reg_pg", 23)) {
				strcat(buf, " > /tmp/MP.txt");
				system_if(buf);
