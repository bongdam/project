...
#ifdef __DAVO__
static int wait_for_flag(int fd, unsigned int mask, int timeout);
#endif
#if 0
...
int main(int argc, char *argv[])
{
	...
	{  	  
		...
		wisp_wan_connected();
#ifdef __DAVO__
		rc = wait_for_flag(fd, DF_RSTASSERTED, 3000);
		switch (rc) {
		default:
			if (last_status <= 0)
				break;
...
static inline int test_any_flag_set(unsigned int mask, unsigned int old, unsigned int new)
{
...
static int wait_for_flag(int fd, unsigned int mask, int timeout)
{
	...
	if (test_any_flag_set(mask, ~flag, flag))
		return 1;
	old = flag;
	pfd.events = POLLIN;
	if (timeout > 0)
	...
	for (;;) {
		int n = TEMP_FAILURE_RETRY(poll(&pfd, 1, timeout));
		if (n <= 0)
			return n;
		read(pfd.fd, (void *)&flag, sizeof(flag));
		if (test_any_flag_set(mask, old, flag))
			return 1;
		old = flag;
		if (timeout > 0) {
