...
static void	wl_connect_limit_count_set(void)
{
	int i, j, val;
	char name[32] = {0,}, path[32] = {0,};

	for (i = 0; i < NUM_WLAN_INTERFACE; i++) {
		for (j = 0; j <= NUM_VWLAN; j++) {
			if (j == 0) {
				snprintf(name, sizeof(name), "x_wlan%d_max_conn", i);
				snprintf(path, sizeof(path), "/proc/wlan%d/dv_max_count", i);
			} else {
				snprintf(name, sizeof(name), "x_wlan%d_vap%d_max_conn", i, j - 1);
