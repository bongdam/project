...
#endif
int avoid_confliction_ip(char *wanIp, char *wanMask, const char *wan_iface)
{
...
//------------restart miniigd-------
		...
		start_upnp_igd(wan_mode_igd, opmode, wisp_wan_id, br_interface, wan_iface);
...
//		entry[1].type = 0; //Unknown
...
#endif
	{
		...
		ret = avoid_confliction_ip(wanip,mask,interface);
#endif
...
//#ifdef TR181_V6_SUPPORT
	...
	start_upnp_igd(wan_type, op_mode, tmp_wisp_id, "br0", interface);
...
#ifdef __DAVO__
		if (op_mode == 0) {
			if (!yfcat("/var/run/udhcpd.pid", "%d", &pid))
				pid = 0;
			if (pid > 0) {
				kill(pid, SIGTERM);
				sleep(1);
...
#endif
#endif
#ifdef __CONFIG_APP_VPN_SERVER__
	system("vpnconf init");
#endif
}
...
void start_upnp_igd(int wantype, int sys_opmode, int wisp_id, char *lan_interface, ...)
{
	...
	va_start(ap, lan_interface);
	wan_iface = va_arg(ap, char *);
	va_end(ap);
	...
	if ((atoi(tmp) == 1) && (sys_opmode != BRIDGE_MODE) && if_nametoindex(wan_iface)) {
...
#endif
		nvram_get_r_def("HW_SERIAL_NO", serial, sizeof(serial), "1234567890");
...
void start_ntp(void)
{
	...
	if (ntp_onoff == 1) {
		...
		buffer[0] = 0;
		nvram_get_r_def("x_ntp_server_ip1", ntp_server1, sizeof(ntp_server1), "pool.ntp.org");
		nvram_get_r_def("x_ntp_server_ip2", ntp_server2, sizeof(ntp_server2), "cn.pool.ntp.org");

		nvram_get_r_def("DAYLIGHT_SAVE", buffer, sizeof(buffer), "0");
		daylight_save = atoi(buffer);
		snprintf(daylight_save_str, sizeof(daylight_save_str), "%u", daylight_save);
		...
		if (strcmp(time_zone, "3 1") == 0 ||
			...
			snprintf(command, sizeof(command), "GMT%s:30%s", str_tz1, str_datnight);
		} else {
			yecho("/var/TZ", "GMT%s%s\n", str_tz1, str_datnight);
			snprintf(command, sizeof(command), "GMT%s%s", str_tz1, str_datnight);
		}
		RunSystemCmd(NULL_FILE, "ntp_inet", "-x", ntp_server1, command,	daylight_save_str, ntp_server2, NULL_STR);
	}
#else
	unsigned int ntp_onoff=0;
...
//	start_upnp_igd(DHCP_DISABLED, sys_op, wisp_id, lan_iface, wan_iface);
	setFirewallIptablesRules(0, NULL);
...
//	start_upnp_igd(DHCP_CLIENT, sys_op, wisp_id, lan_iface, wan_iface);
...
*/
		...
		//start_upnp_igd(PPPOE, sys_op, wisp_id, lan_iface, wan_iface);
}
#ifdef SUPPORT_ZIONCOM_RUSSIA
...
//#endif
	...
	//start_upnp_igd(PPTP, sys_op, wisp_id, lan_iface, wan_iface);
}
...
//	write_line_to_file(PPP_OPTIONS_FILE,2, line_buffer);
	...
	if(connect_type==1){
		...
		sprintf(tmp_args1, "%d", connect_type);/*connect type*/
		RunSystemCmd(NULL_FILE, "ppp_inet", "-t", tmp_args,  "-c", tmp_args1, "-x", NULL_STR);
		//start_upnp_igd(L2TP, sys_op, wisp_id, lan_iface, wan_iface);
}
