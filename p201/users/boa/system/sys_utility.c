...
void update_dns_resolv_file()
{
	...
	if (isFileExist(RESOLV_V6_CONF))
		n += snprintf(buf + n, sizeof(buf) - n , " %s", RESOLV_V6_CONF);
#endif
	if (isFileExist(RESOLV_V4_CONF))
		n += snprintf(buf + n, sizeof(buf) - n , " %s", RESOLV_V4_CONF);

	if (n >= sizeof("cat")) {
		snprintf(buf + n, sizeof(buf) - n , " >%s", RESOLV_CONF);
		system(buf);
	} else
		unlink(RESOLV_CONF);
}
...
#ifdef __DAVO__
		for (fd = STDERR_FILENO + 1; fd < 10; fd++)
			close(fd);
#endif
