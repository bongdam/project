...
#ifdef __DAVO__
			if (ystrargs(strIp, args, _countof(args), ".", 0) > 3) {
				// to apply local_ip_addr setting to VPN seting
				// TODO: we assume that subnet prefix is 24 only
				snprintf(vpn_subnet, sizeof(vpn_subnet),"%s.%s.%s.0/%d", args[0], args[1], args[2], 24);
				nvram_set("vpn_pptp_local_subnet", vpn_subnet);
			}
#endif
		}
...
void formTcpipSetup(request *wp, char *path, char *query)
{
	char tmpBuf[100];
	char buffer[200];
	char *strDhcp, *strIp, *strMask, *strStartIp, *strEndIp;
	DHCP_T dhcp;
#ifdef MIB_TLV
	char pmib_num[10]={0};
...
#if defined(CONFIG_DOMAIN_NAME_QUERY_SUPPORT)
	apmib_get(MIB_OP_MODE,(void *)&opmode);f
...
#ifdef __DAVO__
		strVal = req_get_cstream_var(wp, ("telnetEnable"), "");
		if (strVal[0])
			nvram_set("telnet_enable", strVal);

		strVal = req_get_cstream_var(wp, ("upnpEnable"), "");
		if (strVal[0]) {
			if (!strcmp(strVal, "ON"))
				nvram_set("UPNP_ENABLED", "1");
			else
				nvram_set("UPNP_ENABLED", "0");
		}

		strVal = req_get_cstream_var(wp, ("dnsBlockEnable"), "");
		if (strVal[0]) {
			if ( !strcmp(strVal, "ON"))
				nvram_set("x_dns_block_enable", "1");
			else
				nvram_set("x_dns_block_enable", "0");
		}
#endif
//Brad add end
...
{
	char *strStp, *strIp, *strHostName, *strAddRsvIP, *strDelRsvIP, *strDelAllRsvIP, *strVal;
	char tmpBuf[100];
	char buffer[100];
	int entryNum, i, stp;
...
setac_ret:
	...
	if (is_delete) {
        countDownTime = 30;
        snprintf(lastUrl, 100, "%s","/tcpip_staticdhcp.htm");
        send_redirect_perm(wp, COUNTDOWN_PAGE);
	} else
		OK_MSG("/tcpip_staticdhcp.htm");
  	return;
setErr_rsv:
	ERR_MSG(tmpBuf);
}
...
int dhcpClientList(request *wp, int argc, char **argv)
{
	...
	while (1) {
		...
		if (ret == 0)
			continue;
#ifdef __DAVO__
		if (argc == 1 && strcmp(argv[0], "macSelect") == 0) {
			nBytesSent += req_format_write(wp,
								("<tr class=\"tbl_body\">\n"
									"<td align=\"center\"><font size=2>%s</td>\n"
									"<td align=\"center\"><font size=2>%s</td>\n"
									"<td align=\"center\"><input type='radio' name='macChecked' value='%s' onClick=macInput('%s')></td>\n"
								"</tr>"),
								ipAddr, macAddr, macAddr, macAddr);
		} else  {
#endif
			nBytesSent += req_format_write(wp,
...
int IPSecTableHeadLine(request *wp)
{
	int nBytesSent = 0;

	nBytesSent = req_format_write(wp,
		("<tr class=\"tbl_head\">"
			"<td align=\"center\" width=\"10%%\" ><font size=\"2\"><b><script>dw(tcpip_ipsec_select)</script></b></font></td>\n"
			"<td align=\"center\" width=\"12%%\" ><font size=\"2\"><b><script>dw(tcpip_ipsec_mode)</script></b></font></td>\n"
			"<td align=\"center\" width=\"20%%\" ><font size=\"2\"><b><script>dw(tcpip_ipsec_remote_host)</script></b></font></td>\n"
			"<td align=\"center\" width=\"20%%\" ><font size=\"2\"><b><script>dw(tcpip_ipsec_local_host_subnet)</script></b></font></td>\n"
			"<td align=\"center\" width=\"23%%\" ><font size=\"2\"><b><script>dw(tcpip_ipsec_remote_host_subnet)</script></b></font></td>\n"
			"<td align=\"center\" width=\"15%%\" ><font size=\"2\"><b><script>dw(tcpip_ipsec_status)</script></b></font></td>\n"
		"</tr>\n"));
	return;
}

int IPSecTableList(request *wp, int argc, char **argv)
{
	int tbllist = 0;
	int index = 0;
	char buffer[64] = {0,};
	char value[32] = {0,};
	char mode[12] = {0,}, remote_host[32] = {0,}, local_subnet[32] = {0,}, remote_subnet[32] = {0,}, status[12] = {0,};
	int nBytesSent = 0;

	nvram_get_r_def("ipsec_tbl_num", buffer, sizeof(buffer), "0");
	tbllist = strtol(buffer, NULL, 10);

	if (tbllist == 0)
		return -1;

	for (index = 0; index < tbllist; index++) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_server_mode", index);
		nvram_get_r_def(buffer, value, sizeof(value), "1");
		if (value[0] == '1')
			snprintf(mode, sizeof(mode), "%s", "Server");
		else
			snprintf(mode, sizeof(mode), "%s", "Client");

		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_host", index);
		nvram_get_r_def(buffer, value, sizeof(value), "");
		if (value[0])
			snprintf(remote_host, sizeof(remote_host), "%s", value);

		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_local_subnet", index);
		nvram_get_r_def(buffer, value, sizeof(value), "");
		if (value[0]);
			snprintf(local_subnet, sizeof(local_subnet), "%s", value);

		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_subnet", index);
		nvram_get_r_def(buffer, value, sizeof(value), "");
		if (value[0])
			snprintf(remote_subnet, sizeof(remote_subnet), "%s", value);

		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_active", index);
		nvram_get_r_def(buffer, value, sizeof(value), "1");
		if (value[0] == '1')
			snprintf(status, sizeof(status), "%s", "Activate");
		else
			snprintf(status, sizeof(status), "%s", "Inactivate");

		nBytesSent += req_format_write(wp,
			("<tr class=\"tbl_head\">"
				"<td align=\"center\" width=\"10%%\" ><input type=\"checkbox\" name=\"select%d\" value=\"ON\"></td>\n"
				"<td align=\"center\" width=\"12%%\" ><font size=\"2\">%s</font></td>\n"
				"<td align=\"center\" width=\"20%%\" ><font size=\"2\">%s</font></td>\n"
				"<td align=\"center\" width=\"20%%\" ><font size=\"2\">%s</font></td>\n"
				"<td align=\"center\" width=\"23%%\" ><font size=\"2\">%s</font></td>\n"
				"<td align=\"center\" width=\"15%%\" ><font size=\"2\">%s</font></td>\n"
			"</tr>\n"),
			index, mode, remote_host, local_subnet, remote_subnet, status);
	}

	return nBytesSent;
}

int IPSecVPNList(request *wp, int argc, char **argv)
{
	int nBytesSent = 0;

	nBytesSent = IPSecTableHeadLine(wp);
	nBytesSent += IPSecTableList(wp, argc, argv);

	return nBytesSent;
}

void formIpsecVpnEnable(request *wp, char *path, char *query)
{
	char *submitUrl;
	char *stream = NULL;
	char value[12] = {0,};
	...
	submitUrl = req_get_cstream_var(wp, "submit-url", "/tcpip_ipsec_setting.htm");   // hidden page

	stream = req_get_cstream_var(wp, "ID_IPSEC_ENABLE", "");
	if (stream[0]) {
		valid_check = strtol(stream, NULL, 10);
		if (valid_check != 0 && valid_check != 1)
			valid_check = 0;

		snprintf(value, sizeof(value), "%d", valid_check);
		nvram_set("vpn_ipsec_enable", value);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_INTERVAL", "");
	if (stream[0]) {
		valid_check = strtol(stream, NULL, 10);
		if (valid_check < 5 || valid_check > 604800)
			valid_check = 600;

		snprintf(value, sizeof(value), "%d", valid_check);
		nvram_set("vpn_ipsec_chk_interval", value);
	}
...
void formIpsecVpnSetting(request *wp, char *path, char *query)
{
	char *submitUrl;
	char *stream = NULL;
	char buffer[64] = {0,};
	...
	submitUrl = req_get_cstream_var(wp, "submit-url", "/tcpip_ipsec_setting.htm");   // hidden page
	stream = req_get_cstream_var(wp, "x_ipsec_tbl", "");
	if (stream[0]) {
		index = strtol(stream, NULL, 10);
		snprintf(buffer, sizeof(buffer), "%d", index + 1);
		nvram_set("ipsec_tbl_num", buffer);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_NAME", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_conn_name", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_MODE", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_server_mode", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_R_HOST", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_host", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "x_ipsec_local_subnet", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_local_subnet", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "x_ipsec_remote_subnet", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_subnet", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "x_ipsec_local_idtype", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_local_fqdn", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_LOCAL_ID", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_local_id", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "x_ipsec_remote_idtype", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_fqdn", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_REMOTE_ID", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_id", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_POLICY", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_ike", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "x_ipsec_protocol", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_protocol", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_PROPOSAL", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_esp", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_KEY", "");
	if (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_psk", index);
		nvram_set(buffer, stream);
	}

	stream = req_get_cstream_var(wp, "ID_IPSEC_STATUS", "");
	if  (stream[0]) {
		snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_active", index);
		nvram_set(buffer, stream);
	}
...
static void rearrange_ipsecVpnTbl(void)
{
	int index, new_index = 0;
	char buffer[64] = {0,}, rewrite[64] = {0,};
	char value[32] = {0,};
	...
	for (index = 0; index < 5; index++) {
		...
		if (value[0]) {
			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_active", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_active", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_conn_name", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_conn_name", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_esp", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_esp", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_ike", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_ike", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_local_fqdn", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_local_fqdn", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_local_id", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_local_id", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_local_subnet", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_local_subnet", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_protocol", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_protocol", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_psk", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_psk", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_fqdn", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_remote_fqdn", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_host", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_remote_host", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_id", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_remote_id", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_remote_subnet", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_remote_subnet", new_index);
			nvram_set(rewrite, value);

			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_server_mode", index);
			nvram_get_r_def(buffer, value, sizeof(value), "");
			snprintf(rewrite, sizeof(rewrite), "vpn_ipsec%d_server_mode", new_index);
			nvram_set(rewrite, value);
			new_index++;
...
void formIpsecVpnDelete(request *wp, char *path, char *query)
{
	char *submitUrl;
	char *stream = NULL;
	char buffer[64] = {0,};
	int index = 0, tbllist = 0;
	int count = 0;
	...
	for (index = 0; index < tbllist; index++) {
		snprintf(buffer, sizeof(buffer), "select%d", index);
		stream = req_get_cstream_var(wp, buffer, "");
		if (stream[0]) {
			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_active", index);
			nvram_unset(buffer);
			snprintf(buffer, sizeof(buffer), "vpn_ipsec%d_conn_name", index);
	...
	tbllist = tbllist - count;
	snprintf(buffer, sizeof(buffer), "%d", tbllist);
	nvram_set("ipsec_tbl_num", buffer);
	rearrange_ipsecVpnTbl();
...
void formL2tpVpnEnable(request *wp, char *path, char *query)
{
	char *submitUrl;
	char *stream;
	int valid_check = 0;
	...
	submitUrl = req_get_cstream_var(wp, "submit-url", "/tcpip_l2tp_setting.htm");   // hidden page
	stream = req_get_cstream_var(wp, "ID_L2TP_ENABLE", "");
	if (stream[0]) {
		valid_check = strtol(stream, NULL, 10);
		if (valid_check != 0 && valid_check != 1)
			valid_check = 0;
		...
		snprintf(value, sizeof(value), "%d", valid_check);
		nvram_set("vpn_pptp_enable", value);
	}

	stream = req_get_cstream_var(wp, "ID_L2TP_INTERVAL", "");
	if (stream[0]) {
		valid_check = strtol(stream, NULL, 10);
		if (valid_check < 5 || valid_check > 604800)
			valid_check = 600;
		...
		snprintf(value, sizeof(value), "%d", valid_check);
		nvram_set("vpn_pptp_chk_interval", value);
	}
...
void formL2tpVpnSetting(request *wp, char *path, char *query)
{
	char *submitUrl;
	char *stream;

	submitUrl = req_get_cstream_var(wp, "submit-url", "/tcpip_l2tp_setting.htm");   // hidden page
	stream = req_get_cstream_var(wp, "ID_L2TP_PRIMARY", "");
	if (stream[0])
		nvram_set("vpn_pptp_dns1", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_SECONDARY", "");
	if (stream[0])
		nvram_set("vpn_pptp_dns2", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_PROTOCOL", "");
	if (stream[0])
		nvram_set("vpn_pptp_protocol", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_MODE", "");
	if (stream[0])
		nvram_set("vpn_pptp_mode", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_ACCOUNT", "");
	if (stream[0])
		nvram_set("vpn_pptp_account", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_PASSWORD", "");
	if (stream[0])
		nvram_set("vpn_pptp_password", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_TUNNEL", "");
	if (stream[0])
		nvram_set("vpn_pptp_lan2lan", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_MAX_CONN", "");
	if (stream[0])
		nvram_set("vpn_pptp_max_conn", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_REMOTE_HOST", "");
	if (stream[0])
		nvram_set("vpn_pptp_remote_host", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_ENCRYPTION", "");
	if (stream[0])
		nvram_set("vpn_pptp_encryption", stream);

	stream = req_get_cstream_var(wp, "ID_L2TP_SHARED_KEY", "");
	if (stream[0])
		nvram_set("vpn_pptp_psk", stream);

	stream = req_get_cstream_var(wp, "x_pptp_ippool", "");
	if (stream[0])
		nvram_set("vpn_pptp_ippool", stream);

	stream = req_get_cstream_var(wp, "x_pptp_subnet", "");
	if (stream[0])
		nvram_set("vpn_pptp_remote_subnet", stream);
...
#ifdef __DAVO__
void formTcpipPort(request *wp, char *path, char *query)
{
	char *strVal;
	int port;
	int Nego = 0, Speed = 0, Power = 0;
	char buffer[64] = {0,}, string[32];

	strVal = req_get_cstream_var(wp, "wanNego", "");
	if (strVal[0]) {
	...
	strVal = req_get_cstream_var(wp, "wanSpeed", "");
	if (strVal[0]) {
		if (strcmp(strVal, "1000") == 0)
			Speed = 1000;
		else if (strcmp(strVal, "100") == 0)
			Speed = 100;
		else
			Speed = 10;
	}

	strVal = req_get_cstream_var(wp, "wanPower", "");
	if (strVal[0]) {
	...
	if (Nego)
		snprintf(buffer, sizeof(buffer), "%s_duplex_full_speed_%d_rxpause_txpause", (Power)? "up" : "down", Speed);
	else
		snprintf(buffer, sizeof(buffer), "%s_auto_rxpause_txpause", (Power)? "up" : "down");
	nvram_set("x_port_0_config", buffer);

	for (port = 1; port < 5; port++) {
		snprintf(string, sizeof(string), "lan%dNego", port);
		strVal = req_get_cstream_var(wp, string, "");
		if (strVal[0]) {
			if (strcmp(strVal, "AUTO") == 0)
				Nego = 0;
			else
		...
		snprintf(string, sizeof(string), "lan%dSpeed", port);
		strVal = req_get_cstream_var(wp, string, "");
		if (strVal[0]) {
			if (strcmp(strVal, "100") == 0)
				Speed = 100;
			else
				Speed = 10;
		}

		snprintf(string, sizeof(string), "lan%dPower", port);
		strVal = req_get_cstream_var(wp, string, "");
		if (strVal[0]) {
			if (strcmp(strVal, "OFF") == 0)
				Power = 0;
		...
		if (Nego)
			snprintf(buffer, sizeof(buffer), "%s_duplex_full_speed_%d_-rxpause_txpause", (Power)? "up" : "down", Speed);
		else
			snprintf(buffer, sizeof(buffer), "%s_auto_rxpause_txpause", (Power)? "up" : "down");

		snprintf(string, sizeof(string), "x_port_%d_config", port);
		nvram_set(string, buffer);
	}

	nvram_commit();

	yexecl(NULL, "setphy down");
	yexecl(NULL, "setphy up");

	countDownTime = 5;
	snprintf(lastUrl, 100, "%s", "/tcpipport.htm");
	send_redirect_perm(wp, COUNTDOWN_PAGE);
	return;
}
#endif
