...
#ifdef __DAVO__
/* crypt encrypts plain-text with 5000 rounds by default
 * which causes performance degradation.
 */
static char *crypt_cache(const char *key, const char *salt)
{
	static char **__pw = NULL;
	static struct timespec __mtim;
	struct stat sb;
	int n, i = 0;
	char *q, *p = NULL;

	if (!stat(_PATH_PASSWD, &sb) && memcmp(&sb.st_mtim, &__mtim, sizeof(__mtim))) {
		for (n = 0; __pw && __pw[n]; n++)
			free(__pw[n]);
		free(__pw);
		__pw = NULL;
		__mtim = sb.st_mtim;
	...
	if (__pw) {
		for (; (p = __pw[i]); i++) {
			if (!strcmp(key, p)) {
				char *e = p + strlen(p) + 1;
				n = 3;
				if (!strncmp(salt + 3, "rounds=", sizeof("rounds=") - 1))
					n = (strchrnul(e + 3, '$') - e) + 1;
				if (!strncmp(e, salt, n))
					return e;
				break;
	...
	asprintf(&q, "%s%c%s%c", key, '\0', crypt(key, salt), '\0');

	if (!p) {
		__pw = realloc(__pw, sizeof(char *) * (i + 2));
		__pw[i] = q;
		__pw[i + 1] = NULL;
	} else {
		free(__pw[i]);
			...
			if (req->userName) {
				if (!strcmp(req->userName, user_name)){
					if (req->password==NULL || req->password[0]==0) {
						...
						if (pwd && !strcmp(pwd->pw_passwd, crypt_cache(req->password, pwd->pw_passwd)))
#else
						if (!strcmp(req->password, user_password))
#endif
							req->auth_flag = 1;
