...
#ifdef __DAVO__
	snprintf(varName, sizeof(varName), "x_wlan%d_max_conn", wlan_id);
	strVal = req_get_cstream_var(wp, varName, "");
	if (strVal[0])
...
{
   	char  *wepKey;
   	char *strKeyLen, *strFormat, *strAuth, *strEnabled;
	char key[30];
	...
	for (i = 1; i < 5; i++) {
		snprintf(varName, sizeof(varName), "key%d", i);
...
#ifdef __DAVO__
					yecho("/tmp/wlacMac_err", "1\n");
					send_redirect_perm(wp, "/wlactrl.htm");
					return;
#endif
					goto setErr_ac;
				}
...
#ifdef __DAVO__
		snprintf(varName, sizeof(varName), "x_wlan%d_vap%d_max_conn", wlan_idx, idx - 1);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0])
			nvram_set(varName, strVal);
#endif
...
#if defined(CONFIG_RTL_ULINKER)
					if(wlan_mode != CLIENT_MODE && wlan_mode != WDS_MODE && rptEnabled == 1)d
						;
					else
#endif
...
int wlSiteSurveyTbl(request *wp, int argc, char **argv)
{
...
#endif
	BssDscr *pBss;
	char tmpBuf[MAX_MSG_BUFFER_SIZE], ssidbuf[64], tmp1Buf[10], tmp2Buf[20], wpa_tkip_aes[20],wpa2_tkip_aes[20], tmp3Buf[128];
	WLAN_MODE_T mode;
	bss_info bss;
#ifdef __DAVO__
...

	{
		...
		if ( (mode == CLIENT_MODE )
			||( ((mode == AP_MODE) || mode == AP_WDS_MODE || mode == AP_MESH_MODE || mode == MESH_MODE) && (rptEnabled == 1)) || repeater_search) {
		...
		for (i = 0; i < pStatus->number && pStatus->number!=0xff; i++) {
			...
			if (repeater_search) {
				...
				tmp3Buf, tmp3Buf, tmp2Buf, wpa_tkip_aes, wpa2_tkip_aes);
			} else {
				if ( (mode == CLIENT_MODE )
		...
		{
			if (( mode == CLIENT_MODE )
			||( (mode == AP_MODE  || mode == AP_WDS_MODE) && (rptEnabled == 1)) || repeater_search) {
				nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
...
#ifdef __DAVO__
void formWps(request *wp, char *path, char *query)
{
	char *strVal;
	char cmd[128] = {0,};
	...
	strVal = req_get_cstream_var(wp, ("regenPIN"), "");
	if (strVal[0]) {
		system("flash gen-pin");
		run_init_script_flag = 1;
		run_init_script("all");
		snprintf(lastUrl, 100, "%s","/wlwps.htm");
		countDownTime = 15;
		send_redirect_perm(wp, COUNTDOWN_PAGE);
		return;
	}

	strVal = req_get_cstream_var(wp, ("startPBC"), "");
	if (strVal[0]) {
		sprintf(cmd, "%s -sig_pbc wlan%d", _WSC_DAEMON_PROG, wlan_idx);
		system(cmd);
	...
	strVal = req_get_cstream_var(wp, ("startPIN"), "");
	if (strVal[0]) {
		strVal = req_get_cstream_var(wp, ("cli_pin"), "");
		if (strVal[0]) {
			idx2 = 0;
			for (idx = 0; idx < strlen(strVal); idx++) {
				if (strVal[idx] >= '0' && strVal[idx] <= '9') {
			...
			snprintf(cmd, sizeof(cmd), "iwpriv %s set_mib pin=%s", WLAN_IF, pincodestr_b);
			system(cmd);
			send_redirect_perm(wp, "/wlwps_connect.htm");
		}
	...
	strVal = req_get_cstream_var(wp, ("stopWps"), "");
	if (strVal[0]) {
		snprintf(cmd, sizeof(cmd), "echo wlan%d > /tmp/wscd_cancel ", wlan_idx);
		system(cmd);
		send_redirect_perm(wp, "/wlwps.htm");
		return;
	}
}

void formWpsStatus(request *wp, char *path, char *query)
{
	char *strVal;
	int intVal;
	char ifname[24] = {0,};

	strVal = req_get_cstream_var(wp, "wps_use", "");
	if (strVal[0]) {
