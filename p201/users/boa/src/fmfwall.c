...
{
	char *strAddPort, *strDelPort, *strVal, *strDelAllPort;
	...
	if (strAddPort[0]) {
		...
		strRmtIp = req_get_cstream_var(wp, ("rmtip"), "0.0.0.0");
		strComment = req_get_cstream_var(wp, ("comment"), "");

		if (!strIp[0] && !strFrom[0] && !strTo[0] && !strExternelFrom[0] && !strExternelTo[0] && !strComment[0])
			goto setOk_portfw;
		...
		for (j = 1; j <= entryNum; j++) {
			...
			*((char *)&checkEntry) = (char)j;
			...
			if ( ( (entry.externelFromPort <= checkEntry.externelFromPort && entry.externelToPort >= checkEntry.externelFromPort) ||
			       (entry.externelFromPort >= checkEntry.externelFromPort && entry.externelFromPort <= checkEntry.externelToPort))&&
			       (entry.protoType & checkEntry.protoType)) {
				strcpy(tmpBuf, "<script>dw(portfw_range_overlapped)</script>");
				yecho("/tmp/portfwerr", "1\n");
				send_redirect_perm(wp, "/portfw.htm");
				return;
			}
		}
...
#ifdef __DAVO__
	countDownTime = 5;
	snprintf(lastUrl, 100, "%s","/portfw.htm");
...
void formFilter(request *wp, char *path, char *query)
{
	...
	char *strDelAllPort, *strDelAllIp, *strDelAllMac, *strVal, *submitUrl, *strComment, *user_custom;
	char *strFrom, *strTo;
#ifdef CONFIG_IPV6
	char *strIP6;
	char *ipVer;
	...
	/* Add IP filter */
	if (strAddIp[0]) {
#ifdef MIB_IPFILTER_POLICY
		strVal = req_get_cstream_var(wp, ("protocol"), "");
		if (strVal[0]) {
			if (strcmp(strVal, "specify") == 0) {
				user_custom = req_get_cstream_var(wp, ("protoNum"), "");
				if (strtol(user_custom, NULL, 10) < 1 ||  strtol(user_custom, NULL, 10) > 255) {
					strcpy(tmpBuf, ("protocol value (1 ~ 255)!"));
					goto setErr_filter;
				}
				ipEntry.proto = strtol(user_custom, NULL, 10);
			} else
				ipEntry.proto = strtol(strVal, NULL, 10);
		}

		strVal = req_get_cstream_var(wp, ("s_ipaddr"), "");
		if (strVal[0]) {
			inet_aton(strVal, (struct in_addr *)&ipEntry.sip);
			getInAddr(BRIDGE_IF, IP_ADDR, (void *)&curIpAddr);
			getInAddr(BRIDGE_IF, SUBNET_MASK, (void *)&curSubnet);

			v1 = *((unsigned long *)ipEntry.sip);
			v2 = *((unsigned long *)&curIpAddr);
			v3 = *((unsigned long *)&curSubnet);
		...
		strVal = req_get_cstream_var(wp, ("s_mask"), "");
		if (strVal[0])
			ipEntry.smask = strtol(strVal, NULL, 10);

		strVal = req_get_cstream_var(wp, ("d_ipaddr"), "");
		if (strVal[0])
			inet_aton(strVal, (struct in_addr *)&ipEntry.dip);

		strVal = req_get_cstream_var(wp, ("d_mask"), "");
		if (strVal[0])
			ipEntry.dmask = strtol(strVal, NULL, 10);

		strVal = req_get_cstream_var(wp, ("s_port"), "");
		if (strVal[0]) {
			if (strcmp(strVal, "specify") == 0) {
				user_custom = req_get_cstream_var(wp, ("sportNum"), "");
				if (strtol(user_custom, NULL, 10) < 1 ||  strtol(user_custom, NULL, 10) > 65535) {
					strcpy(tmpBuf, ("Source Port value (1 ~ 65535)!"));
					goto setErr_filter;
				}
				ipEntry.sport = strtol(user_custom, NULL, 10);
			} else
				ipEntry.sport = strtol(strVal, NULL, 10);
		}

		strVal = req_get_cstream_var(wp, ("d_port"), "");
		if (strVal[0]) {
			if (strcmp(strVal, "specify") == 0) {
				user_custom = req_get_cstream_var(wp, ("dportNum"), "");
				if (strtol(user_custom, NULL, 10) < 1 ||  strtol(user_custom, NULL, 10) > 65535) {
					strcpy(tmpBuf, ("Destination Port value (1 ~ 65535)!"));
					goto setErr_filter;
				}
				ipEntry.dport = strtol(user_custom, NULL, 10);
			} else
				ipEntry.dport = strtol(strVal, NULL, 10);
		}

		strVal = req_get_cstream_var(wp, ("interface"), "");
		if (strVal[0])
			strcpy((char *)ipEntry.netIface, strVal);


		strVal = req_get_cstream_var(wp, ("policy"), "");
		if (strVal[0])
			ipEntry.policy = strtol(strVal, NULL, 10);
#else
		...
		for(j=1;j<=entryNum;j++)
		{
			...
			*((char *)&macEntrytmp) = (char)j;
			if ( apmib_get(MIB_MACFILTER_TBL, (void *)&macEntrytmp))
			{
				...
				{
					yecho("/tmp/macfilter_err", "1\n");
					send_redirect_perm(wp, "/macfilter.htm");
					return;
				}
			...
			*((char *)&urlEntrytmp) = (char)j;
			if ( apmib_get(MIB_URLFILTER_TBL, (void *)&urlEntrytmp))
			{
				...
				if (strlen(urlEntry.urlAddr) == strlen(urlEntrytmp.urlAddr))
				{
					...
					{
						yecho("/tmp/urlfilter_err", "1\n");
						send_redirect_perm(wp, "/urlfilter.htm");
						return;
					}
				}
			}
...
#ifdef __DAVO__
	countDownTime = 5;
	snprintf(lastUrl, 100, "%s", submitUrl);
	send_redirect_perm(wp, COUNTDOWN_PAGE);
#else
	if (submitUrl[0])
...
void formDMZ(request *wp, char *path, char *query)
{
	...
	strSave = req_get_cstream_var(wp, ("setDmzFlag"), "");
	if (strSave[0]) {
		strVal = req_get_cstream_var(wp, ("enabled"), "");
		if ( !strcmp(strVal, "ON"))
			intVal = 1;
...
#ifdef __DAVO__
	countDownTime = 5;
	snprintf(lastUrl, 100, "%s","/dmz.htm");
	send_redirect_perm(wp, COUNTDOWN_PAGE);
#endif
  	return;
...
void pFwTableHeadLine(request *wp){
	req_format_write(wp,
			("<tr class=\"tbl_head\">"
				"<td align=\"center\" width=\"20%%\" ><font size=\"2\"><b><script>dw(portfw_tbl_local_ipaddr)</script></b></font></td>\n"
				"<td align=\"center\" width=\"15%%\" ><font size=\"2\"><b><script>dw(portfw_tbl_proto)</script></b></font></td>\n"
				"<td align=\"center\" width=\"20%%\" ><font size=\"2\"><b><script>dw(portfw_tbl_local_portrange)</script></b></font></td>\n"
				"<td align=\"center\" width=\"20%%\" ><font size=\"2\"><b><script>dw(portfw_tbl_remote_portrange)</script></b></font></td>\n"
				"<td align=\"center\" width=\"20%%\" ><font size=\"2\"><b><script>dw(portfw_tbl_remote_comm)</script></b></font></td>\n"
				"<td align=\"center\" width=\"5%%\" ><font size=\"2\"><b><script>dw(portfw_tbl_select)</script></b></font></td></tr>\n"));
		...
		nBytesSent += req_format_write(wp,
				("<tr class=\"tbl_body\">"
					"<td align=center width=\"20%%\" ><font size=\"2\">%s</td>\n"
     				...
     				"<td align=center width=\"20%%\" ><font size=\"2\">%s</td>\n"
					"<td align=center width=\"20%%\" ><font size=\"2\">%s</td>\n"
      				"<td align=center width=\"5%%\" ><input type=\"checkbox\" name=\"select%d\" value=\"ON\"></td></tr>\n"),
					ip_buff, type, portRange, extPortRange, entry.comment, i);
	}
	return nBytesSent;
}
...
int ipFilterList(request *wp, int argc, char **argv)
{
	...
	IPFILTER_T entry;
	char *policy, *intf;
	char *interface, *query, *buffer;
	char *protoType, *s_port, *d_port;
	char *sip, *dip;
	int k, j;
...
#ifdef MIB_IPFILTER_POLICY
		intf = entry.netIface;
		interface = (char *)calloc(64, sizeof(char));
		query = (char *)calloc(32, sizeof(char));
		buffer = (char *)calloc(32, sizeof(char));
		protoType = (char *)calloc(128, sizeof(char));
		s_port = (char *)calloc(128, sizeof(char));
		d_port = (char *)calloc(128, sizeof(char));
		sip = (char *)calloc(32, sizeof(char));
		dip = (char *)calloc(32, sizeof(char));
		policy = (char *)calloc(128, sizeof(char));

		for (k = 0; k < 2; k++) {
			for (j = 0; j < 5; j++) {
				if (j == 0)
					snprintf(buffer, 32, "wlan%d", k);
				else
					snprintf(buffer, 32, "wlan%d-va%d", k, j - 1);

				if (!strcmp(intf, buffer)) {
					if (j == 0)
						snprintf(query, 32, "WLAN%d_SSID", k);
					else
						snprintf(query, 32, "WLAN%d_VAP%d_SSID", k, j - 1);

					nvram_get_r_def(query, interface, 64, "");
				}
			}
		}

		if (!interface[0]) {
			if (!strcmp(entry.netIface, "eth1"))
				snprintf(interface, 64, "%s", "WAN");
			else if (!strcmp(entry.netIface, "br0"))
				snprintf(interface, 64, "%s", "LAN");
			else
				snprintf(interface, 64, "%s", entry.netIface);
		}

		snprintf(sip, 32, "%s", inet_ntoa(*((struct in_addr *)entry.sip)));
		snprintf(dip, 32, "%s", inet_ntoa(*((struct in_addr *)entry.dip)));

		switch (entry.proto) {
			case 0:
				snprintf(protoType, 128, "%s", "<script>dw(ipfilter_protocol_all)</script>");
				break;
			case 1:
				snprintf(protoType, 128, "%s", "<script>dw(ipfilter_protocol_icmp)</script>");
				break;
			case 6:
				snprintf(protoType, 128, "%s", "<script>dw(ipfilter_protocol_tcp)</script>");
				break;
			case 17:
				snprintf(protoType, 128, "%s", "<script>dw(ipfilter_protocol_udp)</script>");
				break;
			default:
				snprintf(protoType, 128, "%d", entry.proto);
				break;
		}

		if (entry.sport)
			snprintf(s_port, 128, "%hu", entry.sport);
		else
			snprintf(s_port, 128, "%s", "<script>dw(ipfilter_port_all)</script>");

		if (entry.dport)
			snprintf(d_port, 128, "%hu", entry.dport);
		else
			snprintf(d_port, 128, "%s", "<script>dw(ipfilter_port_all)</script>");

		if (entry.policy)
			snprintf(policy, 128, "%s", "<script>dw(ipfilter_policy_discard)</script>");
		else
			snprintf(policy, 128, "%s", "<script>dw(ipfilter_policy_pass)</script>");

		nBytesSent += req_format_write(wp, (
					"<tr class=\"tbl_head\">\n"
						"<td align=\"center\" width=\"10%%\" ><font size=\"2\">%s</td>\n"
      					"<td align=\"center\" width=\"10%%\" ><font size=\"2\">%s</td>\n"
      					"<td align=\"center\" width=\"18%%\" ><font size=\"2\">%s/%d</td>\n"
      					"<td align=\"center\" width=\"18%%\" ><font size=\"2\">%s/%d</td>\n"
      					"<td align=\"center\" width=\"14%%\" ><font size=\"2\">%s</td>\n"
      					"<td align=\"center\" width=\"18%%\" ><font size=\"2\">%s</td>\n"
      					"<td align=\"center\" width=\"7%%\" ><font size=\"2\">%s</td>\n"
      					"<td align=\"center\" width=\"5%%\" ><input type=\"checkbox\" name=\"select%d\" value=\"ON\"></td>"
      				"</tr>\n"),
      				interface, protoType, sip, entry.smask, dip, entry.dmask, s_port, d_port, policy, i);
...
#ifdef __DAVO__
void formPassthru(request *wp, char *path, char *query)
{
	char *l2tp_enable, *pptp_enable, *ipsec_enable;
	char tmpBuf[100] = {0,};
	int intVal, pid;

	l2tp_enable = req_get_cstream_var(wp, ("l2tp_passthru_enabled"), "");
	if (l2tp_enable[0]) {
		if ( !strcmp(l2tp_enable, "ON"))
			intVal = 1;
		else
			intVal = 0;
		...
		if ( !apmib_set(MIB_VPN_PASSTHRU_L2TP_ENABLED, (void *)&intVal)) {
			...
			goto setErr_passthru;
		}
	}

	pptp_enable = req_get_cstream_var(wp, ("pptp_passthru_enabled"), "");
	if (pptp_enable[0]) {
		if ( !strcmp(pptp_enable, "ON"))
			intVal = 1;
		else
			intVal = 0;
	...
	ipsec_enable = req_get_cstream_var(wp, ("ipsec_passthru_enabled"), "");
	if (ipsec_enable[0]) {
		if ( !strcmp(ipsec_enable, "ON"))
			intVal = 1;
		else
			intVal = 0;
	...
	countDownTime = 5;
	snprintf(lastUrl, 100, "%s", "/vpn_passthru.htm");
	send_redirect_perm(wp, COUNTDOWN_PAGE);
	return;
