...
int write_body(request * req)
{
	...
	if (req->upload_data) {
#ifdef __CONFIG_LIB_FURL__
		if ((req->upload_len + bytes_to_write + sizeof(struct fwstat)) > MAX_UPLOAD_SIZE)
#else
		if (req->upload_len+bytes_to_write > MAX_UPLOAD_SIZE)
#endif
		{		
#ifdef __CONFIG_LIB_FURL__
			req->upload_data = realloc(req->upload_data, req->upload_len + bytes_to_write + sizeof(struct fwstat));
#else
