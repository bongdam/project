...
void formUpload(request *wp, char *path, char *query)
{
	...
	mg_parse_multipart((char *)wp->upload_data, wp->upload_len,
	                   ...
	                   (int *)&fbuf->rcvlen);
#ifdef __CONFIG_SEC_CONTAINER__
	ct_unroll(fbuf->fmem, &fbuf->rcvlen);
#endif
	...
	n = sprintf(msg, "<br><br><b>Upload in success</b><br><br>Firmware update in progress...");
	sprintf(&msg[n], "<br><b><font color=\"red\" size=\"2\">Do not disrupt the AP during this time.</font></b><br>");
...
{
	char *newPassword, *confPassword;
	char *originPassword;
	char userPasswd[64] = {0,};

	originPassword = req_get_cstream_var(wp, ("ID_ORIGIN_PASSWORD"), "");
	newPassword = req_get_cstream_var(wp, ("ID_NEW_PASSWD"), "");
	confPassword = req_get_cstream_var(wp, ("ID_CONF_PASSWD"), "");

	nvram_aes_cbc_get("USER_PASSWORD", userPasswd, sizeof(userPasswd));
	if (originPassword[0]) {
		if (strcmp(userPasswd, originPassword)) {
			yecho("/tmp/passwd_error", "1");
			goto setErr_pass;
		}
	}

	if (newPassword[0] && confPassword[0]) {
		if (strcmp(newPassword, confPassword) == 0) {
			nvram_aes_cbc_set("USER_PASSWORD", newPassword);
			yexecl(NULL, "chpwd admin USER_PASSWORD");
		}
	}

	free_from_login_list(wp);
	send_redirect_perm(wp, ("/login.htm"));
setErr_pass:
	send_redirect_perm(wp, ("/password.htm"));
}
...
//Brad add end
	...
	if (enabled) {
		...
		if (tmpStr[0]) {
#ifdef __DAVO__
			nvram_set("x_ntp_server_ip1", tmpStr);
#else
		...
		if (tmpStr[0]) {
#ifdef __DAVO__
			nvram_set("x_ntp_server_ip2", tmpStr);
#else
...
#ifdef __DAVO__
	countDownTime = 30;
	snprintf(lastUrl, 100, "%s","/ntp.htm");
	send_redirect_perm(wp, COUNTDOWN_PAGE);
#else
	OK_MSG(submitUrl);
#endif
...
void formLogin(request *wp, char *path, char *query)
{
	...
	char DecodePassword[65] = {0,};
	char userName[65] = {0,}, userPasswd[65] = {0,};
	...
	lang = req_get_cstream_var(wp, ("language"), "");
	if (lang[0]) {
		lang = req_get_cstream_var(wp, ("language_js"), "");
		if (lang[0] == '0' || lang[0] == '1') {
			nvram_set("WEB_LANGUAGE", lang);
			nvram_commit();
		}
		send_redirect_perm(wp, ("/login.htm"));
		return;
	...
	nvram_get_r_def("USER_NAME", userName, sizeof(userName), "");
	nvram_aes_cbc_get("USER_PASSWORD", userPasswd, sizeof(userPasswd));

	if (strcmp(strUser, userName) || strcmp(DecodePassword, userPasswd))
		goto login_err;
	...
	snprintf(pUser_info->remote_ip_addr, sizeof(pUser_info->remote_ip_addr), "%s", wp->remote_ip_addr);
	peer.s_addr = inet_addr(wp->remote_ip_addr);
	cookie = creat_cookie(strUser, strPassword, peer, buf, sizeof(buf));
	if (cookie)
		snprintf(pUser_info->uniq_cookie, sizeof(pUser_info->uniq_cookie), "%s", cookie);
	pUser_info->directory = strdup("/home.htm");
...
#ifdef __DAVO__
int check_dvflag_wan(int flag)
{
	int fd;
	int read_flag;
	int ret = 0;

	if ((fd = open("/proc/dvflag", O_RDONLY)) < 0)
		return 0;

	if (read(fd, (void *)&read_flag, sizeof(read_flag)) > 0) {
		if (read_flag & flag)
			ret = 1;
	}
...
static void lets_go_fota_check(request *wp)
{
	yecho("/tmp/fota_button", "1\n");

	if (check_dvflag_wan(DF_WANLINK) <= 0 || check_dvflag_wan(DF_WANBOUND) <= 0)
		return;

	system("fota_check > /tmp/fota_upgrade");
}

static int lets_go_fota_upgrade(request *wp)
{
	char server[64] = {0,};
	char cmd[128] = {0,}, firmware[64] = {0,};
	int higher_ver = 0;
	...
	system("fota_check > /tmp/fota_upgrade");
	yfcat("/tmp/fota_upgrade", "%d %s", &higher_ver, firmware);
	unlink("/tmp/fota_upgrade");

	if (higher_ver > 0) {
		nvram_get_r_def("fota_url", server, sizeof(server), "");
		snprintf(cmd, sizeof(cmd), "fota_upgrade -r %s/%s", server, firmware);
		yexecl(NULL, cmd);
	}
...
void formFotaUpload(request *wp, char *path, char *query)
{
	char *fota_check = NULL, *fota_upgrade = NULL;
	int n;
	char msg[256] = {0,};

	fota_check = req_get_cstream_var(wp, ("FOTA_CHECK"), "");
	if (fota_check[0]) {
		lets_go_fota_check(wp);
		send_redirect_perm(wp, "/upload.htm");
		return;
	}

	fota_upgrade = req_get_cstream_var(wp, ("FOTA_UPGRADE"), "");
	if (fota_upgrade[0]) {
		if (lets_go_fota_upgrade(wp) <= 0) {
			send_redirect_perm(wp, "/upload.htm");
			return;
		}
	}
	...
	Reboot_Wait = 70;
	snprintf(msg, sizeof(msg), "%s", "<script>dw(upload_fota_info)</script>");
	sprintf(lastUrl, "%s", "/home.htm");
	strcpy(okMsg, msg);
	countDownTime = Reboot_Wait;
	send_redirect_perm(wp, "/progress_setting.htm");
}

void formFotaConfig(request *wp, char *path, char *query)
{
	char *stream;

	stream = req_get_cstream_var(wp, ("ID_FOTA_URL"), "");
	if (stream[0])
		nvram_set("fota_url", stream);

	stream = req_get_cstream_var(wp, ("ID_FOTA_CFG"), "");
	if (stream[0])
		nvram_set("fota_cfgfile", stream);

	send_redirect_perm(wp, "/upload.htm");
}

void formRebootShd(request *wp, char *path, char *query)
{
	char *strVal;

	strVal = req_get_cstream_var(wp, ("rebootschdEnable"), "");
	if (strVal[0]) {
		if (strcmp(strVal, "ON") == 0) {
			nvram_set("x_reboot_sched_enable", "1");
		} else {
			nvram_set("x_reboot_sched_enable", "0");
			goto setok_reboot;
		}
	}

	strVal = req_get_cstream_var(wp, ("rebootschdWeek"), "");
	if (strVal[0])
		nvram_set("x_reboot_sched_week", strVal);

	strVal = req_get_cstream_var(wp, ("rebootschdHour"), "");
	if (strVal[0])
		nvram_set("x_reboot_sched_hour", strVal);

	strVal = req_get_cstream_var(wp, ("rebootschdMin"), "");
	if (strVal[0])
		nvram_set("x_reboot_sched_min", strVal);

setok_reboot:
	nvram_commit();

	killall(SIGKILL, "reboot_schd");
	system("reboot_schd &");

	countDownTime = 10;
	snprintf(lastUrl, 100, "%s","/reboot_schedule.htm");
	send_redirect_perm(wp, COUNTDOWN_PAGE);
}
...
{
	char *strVal, *ssid, *strEncrypt;
	char *strAuth;
	int mode = 0, wispId, repeaterOpmode = 1, repeaterId = 0;
	int changed = 0;
	char buffer[64] = {0,}, repeaterSSID[64] = {0,};
	char vap4_ssid[32] = {0,}, r_ssid[32] = {0,};
	char varName[40] = {0,};
	...
	strVal = req_get_cstream_var(wp, ("operation_mode"), "");
	if (strVal[0]) {
		mode = strtol(strVal, NULL, 10);
		switch (mode) {
			case 0:
			case 2:
				dhcp = DHCP_SERVER;
		...
		if (changed) {
			...
			if (mode == 3)	//repeater
				apmib_set(MIB_OP_MODE, (void *)&repeaterOpmode);
			else
				apmib_set(MIB_OP_MODE, (void *)&mode);

			if (mode == 2 || mode == 3) {	/* wisp mode & repeater mode */
				strVal = req_get_cstream_var(wp, ("wispWanId"), "");
				if (strVal[0]) {
					wispId = atoi(strVal);
					if (mode == 2)
						apmib_set(MIB_WISP_WAN_ID, (void *)&wispId);
					else
						apmib_set(MIB_WISP_WAN_ID, (void *)&repeaterId);

					ssid = req_get_cstream_var(wp, ("repeater_ssid"), "");
					if (!ssid[0])
						goto setErr_operation;

					snprintf(r_ssid, sizeof(r_ssid), "REPEATER_SSID%d", wispId + 1);
					snprintf(vap4_ssid, sizeof(vap4_ssid), "WLAN%d_VAP4_SSID", wispId);
					...
					snprintf(varName, sizeof(snprintf), "method%d", wlan_idx);
					strEncrypt = req_get_cstream_var(wp, varName, "");
					encrypt = (ENCRYPT_T) strEncrypt[0] - '0';
					if (encrypt == ENCRYPT_WEP) {
						snprintf(varName, sizeof(snprintf), "authType%d", wlan_idx);
					...
					if (wispId == 1)	{	// 2.4G repeater mode setting
						nvram_set("REPEATER_ENABLED2", "1");
						nvram_set("WLAN1_VAP4_WLAN_DISABLED", "0");
						nvram_set("WLAN1_VAP4_MODE", "1");
						nvram_set("REPEATER_ENABLED1", "0");
						nvram_set("WLAN0_VAP4_WLAN_DISABLED", "1");
						nvram_set("WLAN0_VAP4_MODE", "0");
					} else {				// 5G repeater mode setting
						nvram_set("REPEATER_ENABLED1", "1");
						nvram_set("WLAN0_VAP4_WLAN_DISABLED", "0");
						nvram_set("WLAN0_VAP4_MODE", "1");
						nvram_set("REPEATER_ENABLED2", "0");
						nvram_set("WLAN1_VAP4_WLAN_DISABLED", "1");
						nvram_set("WLAN1_VAP4_MODE", "0");
					}

					nvram_set(r_ssid, ssid);
					nvram_set(vap4_ssid, ssid);
				}
			} else {
				nvram_set("REPEATER_ENABLED1", "0");
				nvram_set("WLAN0_VAP4_WLAN_DISABLED", "1");
...
setErr_operation:
	send_redirect_perm(wp, "/operation_mode.htm");
}
#endif
