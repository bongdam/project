...
int getInfo(request *wp, int argc, char **argv)
{
	...
	// P2P_SUPPORT ; need modify
   	if ( !strcmp(name, "device_name") ) {
	...
	else if ( !strcmp(name, "ipsec_vpn_enable")) {
		nvram_get_r_def("vpn_ipsec_enable", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "ipsec_vpn_interval")) {
		nvram_get_r_def("vpn_ipsec_chk_interval", buffer, sizeof(buffer), "600");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "ipsec_local_addr")) {
		nvram_get_r_def("IP_ADDR", buffer, sizeof(buffer), "192.168.0.1");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "ipsec_local_subnet")) {
		nvram_get_r_def("SUBNET_MASK", buffer, sizeof(buffer), "255.255.255.0");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_enable")) {
		nvram_get_r_def("vpn_pptp_enable", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_interval")) {
		nvram_get_r_def("vpn_pptp_chk_interval", buffer, sizeof(buffer), "600");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_pri_dns")) {
		nvram_get_r_def("vpn_pptp_dns1", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_sec_dns")) {
		nvram_get_r_def("vpn_pptp_dns2", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_protocol")) {
		nvram_get_r_def("vpn_pptp_protocol", buffer, sizeof(buffer), "pptp");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_encrypt")) {
		nvram_get_r_def("vpn_pptp_encryption", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_tunnel")) {
		nvram_get_r_def("vpn_pptp_lan2lan", buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_mode")) {
		nvram_get_r_def("vpn_pptp_mode", buffer, sizeof(buffer), "client");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_account")) {
		nvram_get_r_def("vpn_pptp_account", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_acc_passwd")) {
		nvram_get_r_def("vpn_pptp_password", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_max_conn")) {
		nvram_get_r_def("vpn_pptp_max_conn", buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_remote_host")) {
		nvram_get_r_def("vpn_pptp_remote_host", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_psk")) {
		nvram_get_r_def("vpn_pptp_psk", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "pptp_vpn_start_ip")) {
		nvram_get_r_def("vpn_pptp_max_conn", buffer, sizeof(buffer), "1");
		max = strtol(buffer, NULL, 10);

		nvram_get_r_def("vpn_pptp_ippool", buffer, sizeof(buffer), "192.168.0.2");
		if (ystrargs(buffer, arg, _countof(arg), ".", 0) == 4) {
			start = strtol(arg[3], NULL, 10);

			if (start > 0 && start + max - 1 < 255)
				snprintf(start_ip, sizeof(start_ip), "%s.%s.%s.%s", arg[0], arg[1], arg[2], arg[3]);
		}
		return req_format_write(wp, "%s", start_ip);
	}
	else if ( !strcmp(name, "pptp_vpn_end_ip")) {
		nvram_get_r_def("vpn_pptp_max_conn", buffer, sizeof(buffer), "1");
		max = strtol(buffer, NULL, 10);
		...
		if (ystrargs(buffer, arg, _countof(arg), ".", 0) == 4) {
			...
			if (start > 0 && start + max - 1 < 255)
				snprintf(end_ip, sizeof(end_ip), "%s.%s.%s.%d", arg[0], arg[1], arg[2], start + max - 1);
		}
		return req_format_write(wp, "%s", end_ip);
	}
	else if ( !strcmp(name, "pptp_vpn_subnet")) {
		nvram_get_r_def("vpn_pptp_remote_subnet", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "origin_passwd_error")) {
		if (access("/tmp/passwd_error", F_OK) == 0) {
			snprintf(buffer, sizeof(buffer), "%s", "<script>dw(password_origin_unmatched)</script>");
			unlink("/tmp/passwd_error");
		}
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "upload_fota_check_result")) {
		int fota_upgrade = 0;

		if (!yfcat("/tmp/fota_upgrade", "%d", &fota_upgrade))
			fota_upgrade = 0;

		unlink("/tmp/fota_upgrade");

		if (fota_upgrade == 0)
			snprintf(buffer, sizeof(buffer), "%s", "<script>dw(upload_fota_result_latest)</script>");
		else
			snprintf(buffer, sizeof(buffer), "%s", "<font color =\"red\"><script>dw(upload_fota_result_upgrade)</script></font>");

		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "dhcplanServer")) {
		nvram_get_r_def("DHCP", buffer, sizeof(buffer), "2");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "x_fota_url")) {
		nvram_get_r_def("fota_url", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "x_fota_cfg")) {
		nvram_get_r_def("fota_cfgfile", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "reboot_hour")) {
		nvram_get_r_def("x_reboot_sched_hour", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "reboot_min")) {
		nvram_get_r_def("x_reboot_sched_min", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "reboot_week")) {
		nvram_get_r_def("x_reboot_sched_week", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "wlan_max_conn")) {
		char name[32] = {0,};
		snprintf(name, sizeof(name), "x_wlan%d_max_conn", wlan_idx);
		nvram_get_r_def(name, buffer, sizeof(buffer), "10");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "wlan_vap_max_conn")) {
		char name[32] = {0,};
		snprintf(name, sizeof(name), "x_wlan%d_vap%d_max_conn", wlan_idx, vwlan_idx - 1);
		nvram_get_r_def(name, buffer, sizeof(buffer), "10");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "REPEATER_SSID1")) {
		nvram_get_r_def("REPEATER_SSID1", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "REPEATER_SSID2")) {
		nvram_get_r_def("REPEATER_SSID2", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
		...
		snprintf(psk_name, sizeof(psk_name), "WLAN%d_VAP4_WPA_PSK", atoi(argv[1]));
		nvram_get_r_def(psk_name, buffer, sizeof(buffer), "");
		return req_format_write(wp, "%d", strlen(buffer));
	}
	else if (!strcmp(name, "repeater_psk")) {
		char psk_name[32] = {0,};
		...
		char wep_key_name[64] = {0,};

		if (argc < 4)
			return 0;

		snprintf(wep_key_name, sizeof(wep_key_name), "WLAN%d_VAP4_WEP%d_KEY%d", atoi(argv[1]), atoi(argv[2]), atoi(argv[3]));
		nvram_get_r_def(wep_key_name, buffer, sizeof(buffer), "");
		return req_format_write(wp, "%d", strlen(buffer));
	}
	else if (!strcmp(name, "repeater_wep_key")) {
		char wep_key_name[64] = {0,};
	...
	else if (!strcmp(name, "p201_macaddr")) {
		nvram_get_r_def("HW_NIC0_ADDR", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "p201_serial")) {
		nvram_get_r_def("HW_SERIAL_NO", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
#ifdef RTK_MULTI_AP
...
//printf("\r\n buffer[%s],__[%s-%u]\r\n",buffer,__FILE__,__LINE__);
...
#ifdef __DAVO__
		nvram_get_r_def("x_ntp_server_ip1", buffer, sizeof(buffer), "pool.ntp.org");
#else
...
#ifdef __DAVO__
		nvram_get_r_def("x_ntp_server_ip2", buffer, sizeof(buffer), "cn.pool.ntp.org");
#else
	...
	*/
		...
		if ( !apmib_get(MIB_WAN_MAC_ADDR,  (void *)buffer) )
			return -1;
#ifdef __DAVO__
		if ( !memcmp(buffer, "\x00\x00\x00\x00\x00\x00", 6)) {
	...
	/* Advance setting stuffs */
		...
		if (!yfcat("/etc/version", "%*s %s", buffer))
			snprintf(buffer, sizeof(buffer), "%s", "1.00.00");
   		return req_format_write(wp, buffer);
	}
	// added by rock /////////////////////////////////////////
...
//		req_format_write(wp,"<tr><hr size=1 noshade align=top></tr>");
	...
	else if(!strcmp(argv[0],"onoff_dmdphy_comment_end"))
	{
		...
		if(intVal == 1)
			...
			req_format_write(wp, "%s","ulinker_opmode.htm");g
#else
		req_format_write(wp, "%s","/status.htm");
#endif
...
int getIndex(request *wp, int argc, char **argv)
{
	...
	if (strcmp(name, "ipsec_table_num") == 0) {
		nvram_get_r_def("ipsec_tbl_num", buffer, sizeof(buffer), "0");
		req_format_write(wp, "%d", strtol(buffer, NULL, 10));
		return 0;
   	}
   	else if (strcmp(name, "upload_fota_upgrade") == 0) {
   		int fota_upgrade = 0;

		if (!yfcat("/tmp/fota_upgrade", "%d", &fota_upgrade))
			fota_upgrade = 0;

		req_format_write(wp, "%d", fota_upgrade);
		return 0;
   	}
   	else if (strcmp(name, "upload_fota_button") == 0) {
		int fota_button = 0;

		if (!yfcat("/tmp/fota_button", "%d", &fota_button))
			fota_button = 0;

		unlink("/tmp/fota_button");
		req_format_write(wp, "%d", fota_button);
		return 0;
   	}
   	else if (strcmp(name, "login_fail_flag") == 0) {
   		int login_fail = 0;

		if (!yfcat("/tmp/login_fail", "%d", &login_fail))
			login_fail = 0;

		unlink("/tmp/login_fail");
		req_format_write(wp, "%d", login_fail);
   		return 0;
   	}
   	else if (strstr(name, "_link_status")) {
		unsigned int phy_status;
		int mask = 0, portno;

		if (!strncmp(name, "wan", 3))
			portno = PRTNR_WAN0;
		else if (!strncmp(name, "lan1", 4))
			portno = PRTNR_LAN1;
		else if (!strncmp(name, "lan2", 4))
			portno = PRTNR_LAN2;
		else if (!strncmp(name, "lan3", 4))
			portno = PRTNR_LAN3;
		else
			portno = PRTNR_LAN4;
		...
		req_format_write(wp, "%d", (strstr(name, "link_status_linkUp")) ? (mask & 1) : mask);
		return 0;
   	}
   	else if (strcmp(name, "saveconf_validation") == 0) {
   		int config_fail = 0;

		if (!yfcat("/tmp/nvram_restore_fail", "%d", &config_fail))
			config_fail = 0;

		unlink("/tmp/nvram_restore_fail");
		req_format_write(wp, "%d", config_fail);
   		return 0;
   	}
   	else if (strcmp(name, "portfwerr") == 0) {
   		int portfw_err = 0;

		if (!yfcat("/tmp/portfwerr", "%d", &portfw_err))
			portfw_err = 0;

		unlink("/tmp/portfwerr");
		req_format_write(wp, "%d", portfw_err);
   		return 0;
   	}
   	else if (strcmp(name, "macfilterErr") == 0) {
   		int macft_err = 0;

   		if (!yfcat("/tmp/macfilter_err", "%d", &macft_err))
   			macft_err = 0;

   		unlink("/tmp/macfilter_err");
   		req_format_write(wp, "%d", macft_err);
   		return 0;
   	}
   	else if (strcmp(name, "urlfilterErr") == 0) {
   		int urlft_err = 0;

   		if (!yfcat("/tmp/urlfilter_err", "%d", &urlft_err))
   			urlft_err = 0;

   		unlink("/tmp/urlfilter_err");
   		req_format_write(wp, "%d", urlft_err);
   		return 0;
   	}
   	else if (strcmp(name, "telnet_Enable") == 0) {
		nvram_get_r_def("telnet_enable", buffer, sizeof(buffer), "0");
		req_format_write(wp, "%ld", strtol(buffer, NULL, 10));
		return 0;
   	}
   	else if (strcmp(name, "wanPortOn") == 0 || strcmp(name, "wanPortNego") == 0 || strcmp(name, "wanPortSpeed") == 0) {
   		int up = 0;
   		int force = 1;
   		int speed = 1000;
   		nvram_get_r_def("x_port_0_config", buffer, sizeof(buffer), "up_auto_rxpause_txpause");
   		if (strstr(buffer, "up"))
   			up = 1;
		...
		if (strcmp(name, "wanPortOn") == 0)
			req_format_write(wp, "%d", up);
		else if (strcmp(name, "wanPortNego") == 0)
			req_format_write(wp, "%d", force);
		else
			req_format_write(wp, "%d", speed);
		return 0;
   	}
   	else if (strcmp(name, "lan1PortOn") == 0 || strcmp(name, "lan1PortNego") == 0 || strcmp(name, "lan1PortSpeed") == 0) {
   		int up = 0;
   		int force = 1;
   		int speed = 100;
   		nvram_get_r_def("x_port_1_config", buffer, sizeof(buffer), "up_auto_-rxpause_txpause");
   		if (strstr(buffer, "up"))
   			up = 1;
		...
		if (strcmp(name, "lan1PortOn") == 0)
			req_format_write(wp, "%d", up);
		else if (strcmp(name, "lan1PortNego") == 0)
			req_format_write(wp, "%d", force);
		else
			req_format_write(wp, "%d", speed);
		return 0;
   	}
   	else if (strcmp(name, "lan2PortOn") == 0 || strcmp(name, "lan2PortNego") == 0 || strcmp(name, "lan2PortSpeed") == 0) {
   		int up = 0;
   		int force = 1;
   		int speed = 100;
   		nvram_get_r_def("x_port_2_config", buffer, sizeof(buffer), "up_auto_-rxpause_txpause");
   		if (strstr(buffer, "up"))
   			up = 1;
		...
		if (strcmp(name, "lan2PortOn") == 0)
			req_format_write(wp, "%d", up);
		else if (strcmp(name, "lan2PortNego") == 0)
			req_format_write(wp, "%d", force);
		else
			req_format_write(wp, "%d", speed);
		return 0;
   	}
   	else if (strcmp(name, "lan3PortOn") == 0 || strcmp(name, "lan3PortNego") == 0 || strcmp(name, "lan3PortSpeed") == 0) {
   		int up = 0;
   		int force = 1;
   		int speed = 100;
   		nvram_get_r_def("x_port_3_config", buffer, sizeof(buffer), "up_auto_-rxpause_txpause");
   		if (strstr(buffer, "up"))
   			up = 1;
		...
		if (strcmp(name, "lan3PortOn") == 0)
			req_format_write(wp, "%d", up);
		else if (strcmp(name, "lan3PortNego") == 0)
			req_format_write(wp, "%d", force);
		else
			req_format_write(wp, "%d", speed);
		return 0;
   	}
   	else if (strcmp(name, "lan4PortOn") == 0 || strcmp(name, "lan4PortNego") == 0 || strcmp(name, "lan4PortSpeed") == 0) {
   		int up = 0;
   		int force = 1;
   		int speed = 100;
   		nvram_get_r_def("x_port_4_config", buffer, sizeof(buffer), "up_auto_-rxpause_txpause");
   		if (strstr(buffer, "up"))
   			up = 1;
		...
		if (strcmp(name, "lan4PortOn") == 0)
			req_format_write(wp, "%d", up);
		else if (strcmp(name, "lan4PortNego") == 0)
			req_format_write(wp, "%d", force);
		else
			req_format_write(wp, "%d", speed);
		return 0;
   	}
   	else if (strcmp(name, "dns_block_enable") == 0) {
		nvram_get_r_def("x_dns_block_enable", buffer, sizeof(buffer), "0");
		req_format_write(wp, "%ld", strtol(buffer, NULL, 10));
		return 0;
   	}
   	else if (strcmp(name, "reboot_enable") == 0) {
		nvram_get_r_def("x_reboot_sched_enable", buffer, sizeof(buffer), "0");
		req_format_write(wp, "%ld", strtol(buffer, NULL, 10));
   		return 0;
   	}
   	else if (strcmp(name, "wlacMacErr") == 0) {
   		int wlacMac_err = 0;

   		if (!yfcat("/tmp/wlacMac_err", "%d", &wlacMac_err))
   			wlacMac_err = 0;

   		unlink("/tmp/wlacMac_err");
   		req_format_write(wp, "%d", wlacMac_err);
   		return 0;
   	}
   	else if (strcmp(name, "DHCP_RSVD_IP_NUM") == 0) {
   		nvram_get_r_def("DHCPRSVDIP_TBL_NUM", buffer, sizeof(buffer), "0");
   		req_format_write(wp, "%ld", strtol(buffer, NULL, 10));
   		return 0;
   	}
   	else if (strcmp(name, "repeater_enable_2g") == 0) {
   		nvram_get_r_def("REPEATER_ENABLED2", buffer, sizeof(buffer), "0");
   		req_format_write(wp, "%ld", strtol(buffer, NULL, 10));
   		return 0;
   	}
   	else if (strcmp(name, "repeater_enable_5g") == 0) {
   		nvram_get_r_def("REPEATER_ENABLED1", buffer, sizeof(buffer), "0");
   		req_format_write(wp, "%ld", strtol(buffer, NULL, 10));
   		return 0;
   	}
   	else if (strcmp(name, "web_language") == 0) {
   		nvram_get_r_def("WEB_LANGUAGE", buffer, sizeof(buffer), "0");
   		req_format_write(wp, "%ld", strtol(buffer, NULL, 10));
   		return 0;
   	}
	if(!strcmp(name, "ipv6")){
...
#ifdef __DAVO__
int dv_wlaninfo(request *wp, int argc, char **argv)
{
	int wlan_index = 0;
	int vap_index = 0;
	char query[64] = {0,};
	...
	if (argc == 2) {	/* main ssid */
		wlan_index = strtol(argv[1], NULL, 10);
		if (strcmp(name, "wlanMode") == 0) {
			snprintf(query, sizeof(query), "WLAN%d_MODE", wlan_index);
			nvram_get_r_def(query, buffer, sizeof(buffer), "0");
		}
		else if (strcmp(name, "wlanDisabled") == 0) {
			snprintf(query, sizeof(query), "WLAN%d_WLAN_DISABLED", wlan_index);
			nvram_get_r_def(query, buffer, sizeof(buffer), "0");
		}
		else if (strcmp(name, "ssid") == 0) {
			snprintf(query, sizeof(query), "WLAN%d_SSID", wlan_index);
			nvram_get_r_def(query, buffer, sizeof(buffer), "");
		}
	} else {			/* virtual ssid */
		wlan_index = strtol(argv[1], NULL, 10);
		vap_index = strtol(argv[2], NULL, 10);
		if (strcmp(name, "wlanDisabled") == 0) {
			snprintf(query, sizeof(query), "WLAN%d_VAP%d_WLAN_DISABLED", wlan_index, vap_index - 1);
			nvram_get_r_def(query, buffer, sizeof(buffer), "0");
		}
		else if (strcmp(name, "vap_ssid") == 0) {
			snprintf(query, sizeof(query), "WLAN%d_VAP%d_SSID", wlan_index, vap_index - 1);
			nvram_get_r_def(query, buffer, sizeof(buffer), "");
		}
	}
