...
void send_redirect_perm(request * req, const char *url)
{
...
#ifdef __DAVO__
	{
		...
		usr = req_get_cstream_var(req, "username", "");
		pwd = req_get_cstream_var(req, "password", "");

		if (usr[0] && pwd[0] && strcmp(req->request_uri, "/boafrm/formLogin") == 0) {
			peer.s_addr = inet_addr(req->remote_ip_addr);
			creat_cookie(usr, pwd, peer, buf, sizeof(buf));
			req_format_write(req, "Set-Cookie: acookie=%s; path=/ \r\n", buf);
		} else if (!strcmp(req->request_uri, "/boafrm/formLogout")) {
			req_format_write(req, "Set-Cookie: acookie=; expires=Thu, 01-Jan-1970 00:00:01 GMT; path=/ \r\n");
			req_format_write(req, "Cache-Control: no-store, no-cache, must-revalidate \r\n");
		}
	}
#endif
...
#ifdef __DAVO__
void send_r_request_nvram_ok(request * req)
{
    req->response_status = R_REQUEST_OK;
    if (req->http_version == HTTP09)
        return;
	...
	req_write(req, http_ver_string(req->http_version));
	req_write(req, " 200 OK" CRLF);
	req_write(req, "Content-Type: application/octet-stream" CRLF);
	req_write(req, "Content-Disposition: attachment; filename=P201.cgi" CRLF);
	req_write(req, "Content-Transfer-Encoding: binary" CRLF);
	req_write(req, "Pragma: no-cache" CRLF);
