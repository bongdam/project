...
#ifdef __DAVO__
	if (strstr(req->request_uri, "/boafrm/formNvramSave"))
        return 1;
	if (strstr(req->request_uri, "/boafrm/formNvramRestore"))
        return 1;
#endif
...
//	req->last_modified = statbuf.st_mtime;
	...
	if (!req->cookie || !pUser_info || (strcmp(pUser_info->uniq_cookie, req->cookie))) {
		if (strcmp(req->request_uri, "/util_gw.js") &&
			strcmp(req->request_uri, "/language_en.js") &&
			strcmp(req->request_uri, "/language_sc.js") &&
			strcmp(req->request_uri, "/login.htm") &&
			strcmp(req->request_uri, "/favicon.ico") &&
			strcmp(req->request_uri, "/formLogin")) {
			if (pUser_info)
				free_from_login_list(req);
			send_redirect_perm(req, "/login.htm");
			return 0;
		}
	}
