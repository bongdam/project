...
#endif
	{"IPSecVPNList", IPSecVPNList},
	{"dv_wlaninfo", dv_wlaninfo},
	{NULL, NULL}
};
...
#endif
	{"formIpsecVpnEnable", formIpsecVpnEnable},
	{"formIpsecVpnSetting", formIpsecVpnSetting},
	{"formIpsecVpnDelete", formIpsecVpnDelete},
	{"formL2tpVpnEnable", formL2tpVpnEnable},
	{"formL2tpVpnSetting", formL2tpVpnSetting},
	{"formLogin", formLogin},
	{"formFotaUpload", formFotaUpload},
	{"formFotaConfig", formFotaConfig},
	{"formNvramSave", formNvramSave},
	{"formNvramRestore", formNvramRestore},
	{"formSaveConfigReset", formSaveConfigReset},
	{"formPassthru", formPassthru},
	{"formTcpipPort", formTcpipPort},
	{"formRebootShd", formRebootShd},
	{"formIpv6PassThru", formIpv6PassThru},
	{"formSetOperation", formSetOperation},
	{"formWps", formWps},
	{"formWpsStatus", formWpsStatus},
	{NULL, NULL}
};
...
#endif
#ifdef __DAVO__
			// header generation will be done at the form handler
			if ((strcmp(now_form->name, "formNvramSave") == 0)) {
				printf("header change %s\n", now_form->name);
			} else if ((strcmp(now_form->name, "formNvramRestore") == 0)) {
				printf("header change %s\n", now_form->name);
			} else {
#endif
...
#ifdef __DAVO__
int req_write_binary(request * req, unsigned char *buf, int buflen)
{
    char *p;
    int bavail;
    ...
    if (buflen >= bavail) {
        p = realloc(req->buffer, req->max_buffer_size << 3);
        if (p != NULL) {
            req->buffer = p;
            req->max_buffer_size <<= 3;
        } else
            return 0; //len = bavail - 1;
    }
    memcpy(req->buffer + req->buffer_end, buf, buflen);

    req->buffer_end += buflen;
    middle_segment = middle_segment + buflen;      //brad add for update exact length of asp file
