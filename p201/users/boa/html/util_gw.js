...
function saveChanges_basic(form, wlan_id)
{
  ...
  // P2P_SUPPORT
	...
	var MaxConn = form.elements["x_wlan" + wlan_id + "_max_conn"];
	if (validateNum(MaxConn.value) == 0) {
		alert(util_gw_max_conn_limit);
		MaxConn.focus();
		return false;
	}
	if (MaxConn.value == "") {
		alert(util_gw_max_conn);
		MaxConn.focus();
		return false;
	} else if (MaxConn.value < 1 || MaxConn.value > 10) {
		alert(util_gw_max_conn_limit);
		MaxConn.focus();
		return false;
	}

   	if (!form.elements["wlanDisabled"+wlan_id].selectedIndex == 0)
	{
...
function enableWLAN(form, wlan_id)
{
	...
	enableTextField(form.elements["wlanwmm"+wlan_id]);
...
//ac2g
	...
	if(form ==document.wlanSetup){
		enableTextField(form.elements["txRate"+wlan_id]);
  		...
  		enableTextField(form.elements["x_wlan"+wlan_id+"_max_conn"]);
	}
}
function disableWLAN(form, wlan_id)
{
	...
	disableTextField(form.elements["controlsideband"+wlan_id]);
	disableTextField(form.elements["wlanwmm"+wlan_id]);
	if (form == document.wlanSetup) {
		disableTextField(form.elements["hiddenSSID"+wlan_id]);
		disableTextField(form.elements["txRate"+wlan_id]);
		disableButton(form.elements["multipleAP"+wlan_id]);
		disableTextField(form.elements["x_wlan"+wlan_id+"_max_conn"]);
	}
	disableCheckBox(form.elements["wlanMacClone"+wlan_id]);
...
function setDefaultKeyValue(form, wlan_id)
{
	if (form.elements["length"+wlan_id].selectedIndex == 0) {
		if ( form.elements["format"+wlan_id].selectedIndex == 0) {
			form.elements["key1"].maxLength = 5;
			form.elements["key2"].maxLength = 5;
			form.elements["key3"].maxLength = 5;
			form.elements["key4"].maxLength = 5;
			form.elements["key1"].value = "*****";
			form.elements["key2"].value = "*****";
			form.elements["key3"].value = "*****";
			form.elements["key4"].value = "*****";
		} else {
			form.elements["key1"].maxLength = 10;
			form.elements["key2"].maxLength = 10;
			form.elements["key3"].maxLength = 10;
			form.elements["key4"].maxLength = 10;
			form.elements["key1"].value = "**********";
			form.elements["key2"].value = "**********";
			form.elements["key3"].value = "**********";
			form.elements["key4"].value = "**********";
		...
		if ( form.elements["format"+wlan_id].selectedIndex == 0) {
			form.elements["key1"].maxLength = 13;
			form.elements["key2"].maxLength = 13;
			form.elements["key3"].maxLength = 13;
			form.elements["key4"].maxLength = 13;
			form.elements["key1"].value = "*************";
			form.elements["key2"].value = "*************";
			form.elements["key3"].value = "*************";
			form.elements["key4"].value = "*************";
		} else {
			form.elements["key1"].maxLength = 26;
			form.elements["key2"].maxLength = 26;
			form.elements["key3"].maxLength = 26;
			form.elements["key4"].maxLength = 26;
			form.elements["key1"].value ="**************************";
			form.elements["key2"].value ="**************************";
			form.elements["key3"].value ="**************************";
			form.elements["key4"].value ="**************************";
...
function CheckIPPart(num)
{
	if (isNaN(num) ||
			num == "" ||
			Number(num) < 0 ||
			Number(num) > 255) return -1;
	return 0;
}
...
function CheckIP(addr)
{
	...
	if (addrs.length != 4) return -1;
	for(var i = 0 ; i < 4 ; i++)
	{
		if (CheckIPPart(addrs[i]) != 0) return -1;
	}
	return 0;
}
...
function reboot_entry(name, value) {
	this.name = name ;
	this.value = value ;
}

var reboot_week_array = new Array(7);
reboot_week_array[0] = new reboot_entry("<script>dw(rebootschd_sun)</script>", "0");
reboot_week_array[1] = new reboot_entry("<script>dw(rebootschd_mon)</script>", "1");
reboot_week_array[2] = new reboot_entry("<script>dw(rebootschd_tue)</script>", "2");
reboot_week_array[3] = new reboot_entry("<script>dw(rebootschd_wed)</script>", "3");
reboot_week_array[4] = new reboot_entry("<script>dw(rebootschd_thu)</script>", "4");
reboot_week_array[5] = new reboot_entry("<script>dw(rebootschd_fri)</script>", "5");
reboot_week_array[6] = new reboot_entry("<script>dw(rebootschd_sat)</script>", "6");

var reboot_hour_array = new Array(24);
reboot_hour_array[0] = new reboot_entry("<script>dw(rebootschd_hour0)</script>", "0");
reboot_hour_array[1] = new reboot_entry("<script>dw(rebootschd_hour1)</script>", "1");
reboot_hour_array[2] = new reboot_entry("<script>dw(rebootschd_hour2)</script>", "2");
reboot_hour_array[3] = new reboot_entry("<script>dw(rebootschd_hour3)</script>", "3");
reboot_hour_array[4] = new reboot_entry("<script>dw(rebootschd_hour4)</script>", "4");
reboot_hour_array[5] = new reboot_entry("<script>dw(rebootschd_hour5)</script>", "5");
reboot_hour_array[6] = new reboot_entry("<script>dw(rebootschd_hour6)</script>", "6");
reboot_hour_array[7] = new reboot_entry("<script>dw(rebootschd_hour7)</script>", "7");
reboot_hour_array[8] = new reboot_entry("<script>dw(rebootschd_hour8)</script>", "8");
reboot_hour_array[9] = new reboot_entry("<script>dw(rebootschd_hour9)</script>", "9");
reboot_hour_array[10] = new reboot_entry("<script>dw(rebootschd_hour10)</script>", "10");
reboot_hour_array[11] = new reboot_entry("<script>dw(rebootschd_hour11)</script>", "11");
reboot_hour_array[12] = new reboot_entry("<script>dw(rebootschd_hour12)</script>", "12");
reboot_hour_array[13] = new reboot_entry("<script>dw(rebootschd_hour13)</script>", "13");
reboot_hour_array[14] = new reboot_entry("<script>dw(rebootschd_hour14)</script>", "14");
reboot_hour_array[15] = new reboot_entry("<script>dw(rebootschd_hour15)</script>", "15");
reboot_hour_array[16] = new reboot_entry("<script>dw(rebootschd_hour16)</script>", "16");
reboot_hour_array[17] = new reboot_entry("<script>dw(rebootschd_hour17)</script>", "17");
reboot_hour_array[18] = new reboot_entry("<script>dw(rebootschd_hour18)</script>", "18");
reboot_hour_array[19] = new reboot_entry("<script>dw(rebootschd_hour19)</script>", "19");
reboot_hour_array[20] = new reboot_entry("<script>dw(rebootschd_hour20)</script>", "20");
reboot_hour_array[21] = new reboot_entry("<script>dw(rebootschd_hour21)</script>", "21");
reboot_hour_array[22] = new reboot_entry("<script>dw(rebootschd_hour22)</script>", "22");
reboot_hour_array[23] = new reboot_entry("<script>dw(rebootschd_hour23)</script>", "23");

var reboot_min_array = new Array(60);
reboot_min_array[0] = new reboot_entry("<script>dw(rebootschd_min0)</script>", "0");
reboot_min_array[1] = new reboot_entry("<script>dw(rebootschd_min1)</script>", "1");
reboot_min_array[2] = new reboot_entry("<script>dw(rebootschd_min2)</script>", "2");
reboot_min_array[3] = new reboot_entry("<script>dw(rebootschd_min3)</script>", "3");
reboot_min_array[4] = new reboot_entry("<script>dw(rebootschd_min4)</script>", "4");
reboot_min_array[5] = new reboot_entry("<script>dw(rebootschd_min5)</script>", "5");
reboot_min_array[6] = new reboot_entry("<script>dw(rebootschd_min6)</script>", "6");
reboot_min_array[7] = new reboot_entry("<script>dw(rebootschd_min7)</script>", "7");
reboot_min_array[8] = new reboot_entry("<script>dw(rebootschd_min8)</script>", "8");
reboot_min_array[9] = new reboot_entry("<script>dw(rebootschd_min9)</script>", "9");
reboot_min_array[10] = new reboot_entry("<script>dw(rebootschd_min10)</script>", "10");
reboot_min_array[11] = new reboot_entry("<script>dw(rebootschd_min11)</script>", "11");
reboot_min_array[12] = new reboot_entry("<script>dw(rebootschd_min12)</script>", "12");
reboot_min_array[13] = new reboot_entry("<script>dw(rebootschd_min13)</script>", "13");
reboot_min_array[14] = new reboot_entry("<script>dw(rebootschd_min14)</script>", "14");
reboot_min_array[15] = new reboot_entry("<script>dw(rebootschd_min15)</script>", "15");
reboot_min_array[16] = new reboot_entry("<script>dw(rebootschd_min16)</script>", "16");
reboot_min_array[17] = new reboot_entry("<script>dw(rebootschd_min17)</script>", "17");
reboot_min_array[18] = new reboot_entry("<script>dw(rebootschd_min18)</script>", "18");
reboot_min_array[19] = new reboot_entry("<script>dw(rebootschd_min19)</script>", "19");
reboot_min_array[20] = new reboot_entry("<script>dw(rebootschd_min20)</script>", "20");
reboot_min_array[21] = new reboot_entry("<script>dw(rebootschd_min21)</script>", "21");
reboot_min_array[22] = new reboot_entry("<script>dw(rebootschd_min22)</script>", "22");
reboot_min_array[23] = new reboot_entry("<script>dw(rebootschd_min23)</script>", "23");
reboot_min_array[24] = new reboot_entry("<script>dw(rebootschd_min24)</script>", "24");
reboot_min_array[25] = new reboot_entry("<script>dw(rebootschd_min25)</script>", "25");
reboot_min_array[26] = new reboot_entry("<script>dw(rebootschd_min26)</script>", "26");
reboot_min_array[27] = new reboot_entry("<script>dw(rebootschd_min27)</script>", "27");
reboot_min_array[28] = new reboot_entry("<script>dw(rebootschd_min28)</script>", "28");
reboot_min_array[29] = new reboot_entry("<script>dw(rebootschd_min29)</script>", "29");
reboot_min_array[30] = new reboot_entry("<script>dw(rebootschd_min30)</script>", "30");
reboot_min_array[31] = new reboot_entry("<script>dw(rebootschd_min31)</script>", "31");
reboot_min_array[32] = new reboot_entry("<script>dw(rebootschd_min32)</script>", "32");
reboot_min_array[33] = new reboot_entry("<script>dw(rebootschd_min33)</script>", "33");
reboot_min_array[34] = new reboot_entry("<script>dw(rebootschd_min34)</script>", "34");
reboot_min_array[35] = new reboot_entry("<script>dw(rebootschd_min35)</script>", "35");
reboot_min_array[36] = new reboot_entry("<script>dw(rebootschd_min36)</script>", "36");
reboot_min_array[37] = new reboot_entry("<script>dw(rebootschd_min37)</script>", "37");
reboot_min_array[38] = new reboot_entry("<script>dw(rebootschd_min38)</script>", "38");
reboot_min_array[39] = new reboot_entry("<script>dw(rebootschd_min39)</script>", "39");
reboot_min_array[40] = new reboot_entry("<script>dw(rebootschd_min40)</script>", "40");
reboot_min_array[41] = new reboot_entry("<script>dw(rebootschd_min41)</script>", "41");
reboot_min_array[42] = new reboot_entry("<script>dw(rebootschd_min42)</script>", "42");
reboot_min_array[43] = new reboot_entry("<script>dw(rebootschd_min43)</script>", "43");
reboot_min_array[44] = new reboot_entry("<script>dw(rebootschd_min44)</script>", "44");
reboot_min_array[45] = new reboot_entry("<script>dw(rebootschd_min45)</script>", "45");
reboot_min_array[46] = new reboot_entry("<script>dw(rebootschd_min46)</script>", "46");
reboot_min_array[47] = new reboot_entry("<script>dw(rebootschd_min47)</script>", "47");
reboot_min_array[48] = new reboot_entry("<script>dw(rebootschd_min48)</script>", "48");
reboot_min_array[49] = new reboot_entry("<script>dw(rebootschd_min49)</script>", "49");
reboot_min_array[50] = new reboot_entry("<script>dw(rebootschd_min50)</script>", "50");
reboot_min_array[51] = new reboot_entry("<script>dw(rebootschd_min51)</script>", "51");
reboot_min_array[52] = new reboot_entry("<script>dw(rebootschd_min52)</script>", "52");
reboot_min_array[53] = new reboot_entry("<script>dw(rebootschd_min53)</script>", "53");
reboot_min_array[54] = new reboot_entry("<script>dw(rebootschd_min54)</script>", "54");
reboot_min_array[55] = new reboot_entry("<script>dw(rebootschd_min55)</script>", "55");
reboot_min_array[56] = new reboot_entry("<script>dw(rebootschd_min56)</script>", "56");
reboot_min_array[57] = new reboot_entry("<script>dw(rebootschd_min57)</script>", "57");
reboot_min_array[58] = new reboot_entry("<script>dw(rebootschd_min58)</script>", "58");
reboot_min_array[59] = new reboot_entry("<script>dw(rebootschd_min59)</script>", "59");
