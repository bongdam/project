...
{
	char name[sizeof(((mib_table_entry_T *)0)->name) << 1];
	int pos;
...
{
	char name[sizeof(((mib_table_entry_T *)0)->name) << 1], buf[sizeof(t->comment) << 2];
	int pos;
...
{
	char name[sizeof(((mib_table_entry_T *)0)->name) << 1], buf[sizeof(t->hostName) << 2];
	int pos;
...
#define OPT_ARGC	0
	...
	if (argumentize(p, args, MIN_ARGC)) {
		do {
			if (!inet_aton(args[0], (struct in_addr *)&t->sip[0]))
				break;
			if (!inet_aton(args[1], (struct in_addr *)&t->dip[0]))
				break;
			t->proto = strtoul(args[2], NULL, 0);
			t->smask = strtoul(args[3], NULL, 0);
			t->dmask = strtoul(args[4], NULL, 0);
			t->policy = strtoul(args[5], NULL, 0);
			t->sport = strtoul(args[6], NULL, 0);
			t->dport = strtoul(args[7], NULL, 0);
			ystrunescape((char *)t->netIface, sizeof(t->netIface), (args[8]) ? : "");
			status = TRUE;
		} while (0);
	}
	free(p);
...
{
	char name[sizeof(((mib_table_entry_T *)0)->name) << 1], buf[sizeof(t->netIface) << 2];
	int pos;
	...
	ynvram_name(name, sizeof(name), mib->name, mib->section);
	ynvram_put("%s%d=" IPFMT "," IPFMT ",%u,%u,%u,%u,%u,%u,%s",
		name, pos + 1,
		t->sip[0], t->sip[1], t->sip[2], t->sip[3],
		t->dip[0], t->dip[1], t->dip[2], t->dip[3],
		t->proto, t->smask, t->dmask, t->policy, t->sport, t->dport,
...
{
	char name[sizeof(((mib_table_entry_T *)0)->name) << 1], url[128];
	int pos;
...
#define OPT_ARGC	0
	...
	if (argumentize(p, args, MIN_ARGC)) {
		do {
			...
			t->ipversion = strtol(args[24], NULL, 0);
#endif
			status = TRUE;
		} while (0);
		if (status != TRUE)
...
{
	char name[sizeof(((mib_table_entry_T *)0)->name) << 1], buf[sizeof(t->entry_name) << 2];
	int pos;
	...
	ynvram_put("%s%d=%d,%02x%02x%02x%02x%02x%02x,%d," IPFMT "," IPFMT ",%u,%u,%s"
		...
		t->priority, t->weight, t->phyport, t->remark_dscp, t->remark_vlan_pri, t->ipversion
#endif
		);
...
/*-----------------------------------------------------------------------------
   VLAN_ARRAY_T : VLAN_CONFIG_T
	unsigned char VlanType ;
	unsigned short VlanId ;
	unsigned char VlanPriority ;
	unsigned int MemberPortMask ;
	unsigned int TaggedPortMask ;
 */
static int
get_vlancfg_array_t(VLAN_CONFIG_T *t, const char *s, const struct mib *mib)
{
# define MIN_ARGC	4
#define OPT_ARGC	0
	char *args[MIN_ARGC + OPT_ARGC + 1];
	...
	if (p == NULL)
		return FALSE;
	if ((argc = argumentize(p, args, MIN_ARGC))) {
		t->VlanType = strtol(args[0], NULL, 0);
		t->VlanId = strtol(args[1], NULL, 0);
		t->VlanPriority = strtol(args[2], NULL, 0);
		if (ystrargs(args[3], sub_args, _countof(sub_args), "/", 0) == 2) {
			t->MemberPortMask = strtol(sub_args[0], NULL, 0);
			t->TaggedPortMask = strtol(sub_args[1], NULL, 0);
			status = TRUE;
		}
	}
	free(p);
...
static int
add_vlancfg_array_t(VLAN_CONFIG_T *t, const struct mib *mib, const struct mib *nib)
{
	char name[sizeof(((mib_table_entry_T *)0)->name) << 1];
	int pos;
	...
	ynvram_name(name, sizeof(name), mib->name, mib->section);
	ynvram_put("%s%d=%u,%u,%u,%x/%x", name, pos + 1,
		t->VlanType,
		t->VlanId,
		t->VlanPriority,
		t->MemberPortMask,
...
static int
del_vlancfg_array_t(VLAN_CONFIG_T *t, const struct mib *mib, const struct mib *nib)
{
	VLAN_CONFIG_T vconf;
	return del_template(t, sizeof(vconf), mib, nib, (void *)get_vlancfg_array_t, (void *)&vconf);
}
#endif
...
#endif
#ifdef CONFIG_8021Q_VLAN_SUPPORTED
	{ VLAN_ARRAY_T,		(void *)get_vlancfg_array_t,	(void *)add_vlancfg_array_t,		(void *)del_vlancfg_array_t },
#endif
#ifdef WLAN_PROFILE
