...
#ifdef __DAVO__
extern void sysled_blinker(long token);
#else
#define sysled_blinker(n) do {} while (0)
#endif
...
int spi_flw_image(unsigned int chip, unsigned int flash_addr_offset ,unsigned char *image_addr, unsigned int image_size)
{
	...
	if (CONFIG_FLASH_NUMBER > 1) {
		if (flash_addr_offset >= g_m25p_0.mtd.size) {
			...
			end = (flash_addr_offset+image_size) / g_m25p_1.mtd.erasesize;

			er_info.addr = begin * g_m25p_1.mtd.erasesize;
			er_info.len = (end - begin + 1) * g_m25p_1.mtd.erasesize;

			ret = g_m25p_1.mtd._erase(&g_m25p_1.mtd, &er_info);
			if (0 == ret)
				ret = g_m25p_1.mtd._write(&g_m25p_1.mtd, flash_addr_offset, image_size, &retlen, image_addr);

		} else if (flash_addr_offset+image_size <= g_m25p_0.mtd.size) {
			begin = flash_addr_offset / g_m25p_0.mtd.erasesize;
			end = (flash_addr_offset+image_size) / g_m25p_0.mtd.erasesize;

			er_info.addr = begin * g_m25p_0.mtd.erasesize;
			er_info.len = (end - begin + 1) * g_m25p_0.mtd.erasesize;

			ret = g_m25p_0.mtd._erase(&g_m25p_0.mtd, &er_info);
			if (0 == ret)
				ret = g_m25p_0.mtd._write(&g_m25p_0.mtd, flash_addr_offset, image_size, &retlen, image_addr);

		} else if ((flash_addr_offset < g_m25p_0.mtd.size)&&(flash_addr_offset+image_size > g_m25p_0.mtd.size)) {
			begin = flash_addr_offset / g_m25p_0.mtd.erasesize;
			end = ((unsigned int)(g_m25p_0.mtd.size)) / g_m25p_0.mtd.erasesize;

			er_info.addr = begin * g_m25p_0.mtd.erasesize;
			er_info.len = (end - begin + 1) * g_m25p_0.mtd.erasesize;

			begin_1 = 0;
			end_1 = ((flash_addr_offset+image_size-(unsigned int)(g_m25p_0.mtd.size))) / g_m25p_1.mtd.erasesize;

			er_info_1.addr = begin_1 * g_m25p_1.mtd.erasesize;
			er_info_1.len = (end_1 - begin_1 + 1) * g_m25p_1.mtd.erasesize;

			ret = g_m25p_0.mtd._erase(&g_m25p_0.mtd, &er_info);
			if (0 == ret) {
				ret = g_m25p_0.mtd._write(&g_m25p_0.mtd, flash_addr_offset, \
				((unsigned int)(g_m25p_0.mtd.size))-flash_addr_offset, &retlen, image_addr);
			}
			if (0 == ret)
				ret = g_m25p_1.mtd._erase(&g_m25p_1.mtd, &er_info_1);
			if (0 == ret) {
				ret = g_m25p_1.mtd._write(&g_m25p_1.mtd, 0, \
				((flash_addr_offset+image_size-(unsigned int)(g_m25p_0.mtd.size))), \
				&retlen, image_addr + (((unsigned int)(g_m25p_0.mtd.size))-flash_addr_offset));
			}
		}
	} else {
		begin = flash_addr_offset / g_m25p_0.mtd.erasesize;
