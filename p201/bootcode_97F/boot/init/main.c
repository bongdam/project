...
//-------------------------------------------------------
	...
	*/
	...
	RTL_W32(BSP_SYS_LED_PIN_MUX_SEL, 
			(RTL_R32(BSP_SYS_LED_PIN_MUX_SEL) 
			& ~(BSP_SYS_LED_PIN_MASK) | (BSP_SYS_LED_PIN_MUX))); 
	/* turn on gpio pin */
	RTL_W32(BSP_GPIO_CNR_REG(BSP_SYS_LED_PIN), 
			(RTL_R32(BSP_GPIO_CNR_REG(BSP_SYS_LED_PIN)) 
			& (~(1 << BSP_GPIO_BIT(BSP_SYS_LED_PIN)))));
	/* set gpio as out */
	RTL_W32(BSP_GPIO_DIR_REG(BSP_SYS_LED_PIN), 
			(RTL_R32(BSP_GPIO_DIR_REG(BSP_SYS_LED_PIN)) 
			| (1 << BSP_GPIO_BIT(BSP_SYS_LED_PIN))));
	REG32(BSP_GPIO_DAT_REG(BSP_SYS_LED_PIN)) = REG32(BSP_GPIO_DAT_REG(BSP_SYS_LED_PIN)) & ~(1 << BSP_GPIO_BIT(BSP_SYS_LED_PIN));
#endif
	#if 0
	DDR_short_ZQ();
	#endif
...
void sysled_blinker(long token)
{
#ifdef BSP_SYS_LED_PIN
	static long sys_led_blink_jiffy;
	if (++sys_led_blink_jiffy >= token) {
		sys_led_blink_jiffy = 0;
