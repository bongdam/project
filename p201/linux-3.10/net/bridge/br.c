...
static int br_wlanblockread_proc(struct seq_file *s, void *v)
#else
...
static ssize_t rlx_br_wlanblock_single_open(struct inode *inode, struct file *file)
{
	return(single_open(file, br_wlanblockread_proc,NULL));
}

static ssize_t rlx_br_wlanblock_single_write(struct file * file, const char __user * userbuf,
			size_t count, loff_t * off)
{
	return br_wlanblockwrite_proc(file, userbuf,count, off);
}


struct file_operations rlx_br_wlanblock_proc_fops = {
		.open           = rlx_br_wlanblock_single_open,
        .write		    = rlx_br_wlanblock_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#if defined(CONFIG_PROC_FS) && defined (CONFIG_RTL_WLAN_BLOCK_RELAY)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("br_wlanblock",0,&proc_root,&rlx_br_wlanblock_proc_fops,NULL);
#else
