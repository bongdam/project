...
#ifdef __DAVO__
static int erase_sector64(struct m25p *flash, u32 offset)
{
	pr_debug("%s: %s %dKiB at 0x%08x\n", dev_name(&flash->spi->dev),
		 __func__, flash->mtd.erasesize / 1024, offset);
...
static int m25p80_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if (len == flash->mtd.size) {
		...
		while (len) {
#ifdef __DAVO__
			if (!(addr & (0x10000 - 1)) && (len >= 0x10000)) {
				if (!erase_sector64(flash, addr)) {
					addr += 0x10000;
					len -= 0x10000;
					continue;
				} else {
...
static const struct spi_device_id m25p_ids[] = {
	...
	{ "en25qh64a",  
	INFO(0x1c7017, 0, 64 * 1024,  128, 0, 75000000, 100000000, 
		FLASH_VERIFIED) },
	{ "en25qh128a",  
	INFO(0x1c7018, 0, 64 * 1024,  256, 0, 75000000, 100000000, 
		FLASH_VERIFIED) },
	{"en25qh256",
	...
	{"mx25l3205d",
	 INFO(0xc22016, 0, 64 * 1024, 64, 0, 29000000, 41000000,
	      FLASH_VERIFIED)},
	{"mx25l3235d",
	 INFO(0xc25e16, 0, 64 * 1024, 64, SECT_4K, 29000000, 100000000, 
		FLASH_VERIFIED)},
	{"mx25l3255e",
...
static const struct spi_device_id *jedec_probe(struct spi_device *spi)
{
	...
	for (tmp = 0; tmp < ARRAY_SIZE(m25p_ids) - 1; tmp++) {
		...
		if (info->jedec_id == default_jedec){
			info->n_sectors = (1<<(jedec & 0xff))/(info->sector_size);
			return &m25p_ids[tmp];
		}
	}
