...
#endif
{
	...
	memcpy(ssid, priv->pmib->dot11StationConfigEntry.dot11DesiredSSID, priv->pmib->dot11StationConfigEntry.dot11DesiredSSIDLen);

	seq_printf(s, "%s cur:%d max:%u\n", ssid, priv->assoc_num, priv->max_count);
	return pos;
}

static int rtl8192cd_proc_dv_max_count_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	struct net_device *dev = (struct net_device *)data;
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	char tmp[4];
