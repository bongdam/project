...
#ifdef __DAVO__
        || priv->pmib->dot11OperationEntry.ssid_separator
#endif
        )
        goto shouldnot_rxsc;
...
#ifdef __DAVO__
				if (priv->pmib->dot11OperationEntry.ssid_separator && (br_port = GET_BR_PORT(priv->dev))) {
					extern struct net_device *br_fdb_match(struct net_bridge *, const unsigned char *, u16);
					struct net_device *dst = br_fdb_match(br_port->br, da, 0);
					if (dst && ((dst->priv_flags & 0x1C000000) == 0x4000000)) {
						priv->ext_stats.rx_data_drops++;
						DEBUG_ERR("RX DROP: ssid separator\n");
						return FAIL;
					}
				}
#endif /* __DAVO__ */
