...
static void set_sw_LED0(struct rtl8192cd_priv *priv, int flag)
{
    ...
    if ((GET_CHIP_VER(priv) == VERSION_8188E)||(GET_CHIP_VER(priv) == VERSION_8192E)||(GET_CHIP_VER(priv) == VERSION_8192F)) {
        ...
        if(flag) /* led off*/
            gpio_set_value((GET_CHIP_VER(priv) == VERSION_8197G) ? GPIO2ENUM(WL1_GREEN_LED) : GPIO2ENUM(WL0_GREEN_LED), 0);
        else  /*led on*/
            gpio_set_value((GET_CHIP_VER(priv) == VERSION_8197G) ? GPIO2ENUM(WL1_GREEN_LED) : GPIO2ENUM(WL0_GREEN_LED), 1);
    }
    #endif
...
# endif
	  ...
	  int err;
        ...
        err = gpio_request_one((GET_CHIP_VER(priv) == VERSION_8197G) ? GPIO2ENUM(WL1_GREEN_LED) : GPIO2ENUM(WL0_GREEN_LED), GPIOF_OUT_INIT_HIGH | GPIOF_EXPORT_DIR_FIXED, (priv->pshare->curr_band == BAND_2G) ? "2G Wifi" : "5G Wifi");
	  #else
		if((err = gpio_request_one((GET_CHIP_VER(priv) == VERSION_8197G) ? GPIO2ENUM(WL1_GREEN_LED) : GPIO2ENUM(WL0_GREEN_LED), GPIOF_OUT_INIT_HIGH, (priv->pshare->curr_band == BAND_2G) ? "2G Wifi" : "5G Wifi")) == 0)
			err = gpio_export((GET_CHIP_VER(priv) == VERSION_8197G) ? GPIO2ENUM(WL1_GREEN_LED) : GPIO2ENUM(WL0_GREEN_LED), false);
	  #endif
        if(err) {
