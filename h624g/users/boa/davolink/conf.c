...
extern int set_opt_dns(FILE *f);
/* super dmz feature is excluded from the specification - young 2015-06-23
...
#define dns3	arg[3]
	...
	char buf[32];
	FILE *f;
	char repeater1[8], repeater2[8];
	int repeater_mode=0;

	nvram_get_r_def("REPEATER_ENABLED1", repeater1, sizeof(repeater1), "0");
	nvram_get_r_def("REPEATER_ENABLED2", repeater2, sizeof(repeater2), "0");
	repeater_mode |= atoi(repeater1);
	repeater_mode |= atoi(repeater2);
	...
	//if repeatermode fail or yet connect : lease time 30 sec
	fprintf(f, "opt lease %d\n", (repeater_mode == 1)?30:n);
		...
		if (mode != 1) {
			...
			if (dns_mode == 0) {
				...
				if ((dns_fail = set_opt_dns(f))) {
					fprintf(f, "opt dns 210.220.163.82\n");
					fprintf(f, "opt dns 219.250.36.130\n");
				}
			}
		}
#endif
	...
	if ((mode == 1) || (mode == 2 && dns_mode == 1)) {
		...
		nvram_get_r_def("secret_davo_dns1", buf, sizeof(buf), "210.220.163.82");
		inet_aton(buf, (struct in_addr *)&dns1);
#else
