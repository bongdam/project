...
void set_dhcp6s()
{
	...
	/*start daemon */
/*start_dhcp6s: unuse value 150610*/
...
void set_dnsv6()
{
	...
	char addr[256];
	char cmdBuffer[256];
	char dns[64];		/*dnstmp[64]; unuse value 150610 */

	char def_wan_ifname[] = "eth1";

//      char hostfile[]="/var/dnsmasq_hostfile";
	//FILE *fp=NULL; unuse value 150610
...
//      system("rm -f /var/dnsmasq_hostfile 2> /dev/null");
	...
	if (!isFileExist(DNSV6_CONF_FILE)) {
		...
		sprintf(cmdBuffer, "%02x%02x%02x%02x%02x%02x", (unsigned char)tmpBuff[0], (unsigned char)tmpBuff[1],
			(unsigned char)tmpBuff[2], (unsigned char)tmpBuff[3], (unsigned char)tmpBuff[4],
		...
		sprintf(addr, "Fe80::%c%c%c%c:%c%cFF:FE%c%c:%c%c%c%c",
			...
			cmdBuffer[6], cmdBuffer[7], cmdBuffer[8], cmdBuffer[9], cmdBuffer[10], cmdBuffer[11]
		    );
...
void set_radvd()
{
	...
	/*int dnsMode; unuse value 150610 */
	/*FILE *fp=NULL; unuse value 150610 */
...
//              printf("%s-%d: create configure file: %s.\n", __FUNCTION__, __LINE__, RADVD_CONF_FILE);
...
#endif
			{
				memcpy(tmpNum, radvdCfgParam.interface.prefix[0].Prefix,
				       sizeof(radvdCfgParam.interface.prefix[0].Prefix));
				sprintf(tmpBuf, "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x", tmpNum[0], tmpNum[1], tmpNum[2],
					tmpNum[3], tmpNum[4], tmpNum[5], tmpNum[6], tmpNum[7]);
				strcat(tmpBuf, "\0");
		...
		if (radvdCfgParam.interface.prefix[1].enabled > 0) {
			memcpy(tmpNum, radvdCfgParam.interface.prefix[1].Prefix,
			       sizeof(radvdCfgParam.interface.prefix[1].Prefix));
			sprintf(tmpBuf, "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x", tmpNum[0], tmpNum[1], tmpNum[2], tmpNum[3],
				tmpNum[4], tmpNum[5], tmpNum[6], tmpNum[7]);
			strcat(tmpBuf, "\0");
...
/*start_radvd: unuse value 150610*/
...
#ifdef __DAVO__
static int set_manual_dns6(void)
{
# define MAN_DNS6_MAX	2
	int i, count;
	struct nameserver_addr ns_addrs[8];
	char buf[64], name[32];

	for (i = count = 0; i < MAN_DNS6_MAX; i++) {
		sprintf(name, "x_ipv6_manual_dns%d", i + 1);
		_nvram_get_r(name, buf, sizeof(buf), "");
		if (inet_pton(AF_INET6, buf, &ns_addrs[count].na_addr6) != 1)
			continue;
		ns_addrs[count++].na_family = AF_INET6;
	}

	return commit_nameserver(RESOLV_CONF, ns_addrs, count, AF_INET6);
}

int in6_addrconf(const char *ifname, int method)
{
	FILE *f;
	const char *conf_name = "/var/lib/dhcp6/dhcp6c.conf";
	char buf[128], tmp[sizeof(struct in6_addr)];
	int prefix_len;

	if (nvram_atoi("x_ipv6_dns_method", 0))	/* 0 - Provisioned by DHCP server */
		set_manual_dns6();

	symlink("/bin/sysconf", "/var/lib/dhcp6/dhcp6c.script");
	switch (method) {
	case 0:		/* SLAAC */
		f = fopen(conf_name, "w");
		if (!f)
		...
		fprintf(f, "  information-only;\n");
		fprintf(f, "  request domain-name-servers;\n");
		fprintf(f, "  request domain-name;\n");
		fprintf(f, "  script \"/var/lib/dhcp6/dhcp6c.script\";\n");
		fprintf(f, "};\n");
		fclose(f);
		return yexecl(NULL, "dhcp6c -c %s -p /var/lib/dhcp6/dhcp6c.pid %s", conf_name, ifname);

	case 1:		/* Manual */
		/* delete global */
		yexecl(NULL, "ip -6 addr flush dev %s scope global", ifname);
		_nvram_get_r("x_ipv6_manual_addr", buf, sizeof(buf), "");
		if (buf[0] == '\0' || inet_pton(AF_INET6, buf, tmp) != 1)
			break;
		prefix_len = nvram_atoi("x_ipv6_manual_prefix_len", 64);
		return yexecl(NULL, "ip -6 addr add %s/%d dev %s", buf, prefix_len, ifname);

	default:		/* DHCP Client */
		sprintf(buf, "/proc/sys/net/ipv6/conf/%s/forwarding", ifname);
		yecho(buf, "0");
		...
		fprintf(f, "  send ia-na 1;\n");
		fprintf(f, "  script \"/var/lib/dhcp6/dhcp6c.script\";\n");
		fprintf(f, "};\n\n");
		fprintf(f, "id-assoc na 1 { };\n");
		fclose(f);
		return yexecl(NULL, "dhcp6c -c %s -p /var/lib/dhcp6/dhcp6c.pid %s", conf_name, ifname);
	}
...
void set_basicv6()
{
	...
	if (addrIPv6CfgParam.enabled == 1) {
		...
		sprintf(tmpStr, "/bin/ifconfig br0 %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
			addrIPv6CfgParam.addrIPv6[0][0], addrIPv6CfgParam.addrIPv6[0][1], addrIPv6CfgParam.addrIPv6[0][2],
			addrIPv6CfgParam.addrIPv6[0][3], addrIPv6CfgParam.addrIPv6[0][4], addrIPv6CfgParam.addrIPv6[0][5],
			addrIPv6CfgParam.addrIPv6[0][6], addrIPv6CfgParam.addrIPv6[0][7], addrIPv6CfgParam.prefix_len[0]);
		...
		sprintf(tmpStr, "/bin/ifconfig eth1 %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
			addrIPv6CfgParam.addrIPv6[1][0], addrIPv6CfgParam.addrIPv6[1][1], addrIPv6CfgParam.addrIPv6[1][2],
			addrIPv6CfgParam.addrIPv6[1][3], addrIPv6CfgParam.addrIPv6[1][4], addrIPv6CfgParam.addrIPv6[1][5],
			addrIPv6CfgParam.addrIPv6[1][6], addrIPv6CfgParam.addrIPv6[1][7], addrIPv6CfgParam.prefix_len[1]);
		//RunSystemCmd(NULL_FILE, tmpStr, NULL_STR);
		system(tmpStr);
		//printf("the cmd for ipv6 is %s\n", tmpStr);
	}
...
#ifdef __CONFIG_APP_RADVD__
static unsigned long do_ioctl_get_ipaddress(char *dev)
{
	struct ifreq ifr;
	int fd;
	unsigned long ip;
...
static int get_gateway(in_addr_t * p)
{
	FILE *fp;
...
void set_dslite()
{
	...
	sprintf(remote_addr6, "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
		ipaddr6.addrIPv6[0], ipaddr6.addrIPv6[1], ipaddr6.addrIPv6[2],
		ipaddr6.addrIPv6[3], ipaddr6.addrIPv6[4], ipaddr6.addrIPv6[5], ipaddr6.addrIPv6[6], ipaddr6.addrIPv6[7]);
	remote_addr6[39] = '\0';
...
void set_ipv6()
{
	...
	printf("Start setting IPv6\n");
	apmib_get(MIB_IPV6_WAN_ENABLE, &val);
	if (val == 0)
		return;

# ifdef CONFIG_DSLITE_SUPPORT
	apmib_get(MIB_WAN_DHCP, &val);
	if (val == 17)		// DS-Lite mode
		RunSystemCmd("proc/dslite_hw_fw", "echo", "1", NULL_STR);
# endif
#ifndef __DAVO__
	yecho("/proc/sys/net/ipv6/conf/all/forwarding", "1");
