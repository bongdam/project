...
void err_quit(int status, const char *fmt, ...)
{
	va_list p;
...
int DoCmd(char *const argv[], char *path)
{
	pid_t pid;
	int status;
	int i, fd;
	...
	case 0:		/* child */
		for (i = 0; i < (_NSIG - 1); i++)
			signal(i, SIG_DFL);
		/* Use an arbitrarily chosen number for the presumably highest one */
...
#ifndef SYS_DEBUG
		else if ((fd = open("/dev/null", O_RDWR, 0)) > -1)
			dup2(fd, 2);
#endif
		if (fd > 2)
			close(fd);
...
void Create_script(char *script_path, char *iface, int network, char *ipaddr, char *mask, char *gateway)
{
	...
	if (network == WAN_NETWORK) {
		...
		fprintf(fp, "dft_gw=`cat /var/gateway`\n");
		fprintf(fp, "if [ $dft_gw != '' ] && [ \"$dft_gw\" != \"0.0.0.0\" ] ; then\n");
		fprintf(fp, "\troute del default gw $dft_gw\n"
					"fi\n");
	...
	if ((fp = fopen(webPid, "r")) != NULL) {
		...
		if (sscanf(line, "%d", &pid)) {
			if (pid > 1) {
				...
				nvram_get_r("OP_MODE", opmode, sizeof(opmode));
				nvram_get_r("IP_ADDR", lan_nip, sizeof(lan_nip));
				/* APACRTL-84  smlee 20151029 */
				if (get_repeater_mode()) {
					yexecl(NULL, "boa -p 80");
				} else {
					if (!strcmp(opmode, "0")) // NAT
						yexecl(NULL, "boa -p 80 -i %s", lan_nip);
					else // Bridge
						yexecl(NULL, "boa -p 8080 -i %s", lan_nip);
				}
...
#ifdef __DAVO__
char *get_wlan_name(char *buf, int wlan_idx, int ssid_num, char *name)
{
	if (ssid_num == 0) {
