...
void set_br_interface(char *brif)
{

	int opmode=-1;
	char tmpBuff[512]={ [0] = 0 };
...
#error NOT HERE
	...
	#if !defined(__DAVO__) /* Including eth1 to br0 causes packet loop. */
...
int up_mib_value()
{
	...
	new_ver = old_ver;
	 ...
	 upgrade_boot();
        ...
        {
        		apmib_set(new_mib[i].id,strToValue((char *)new_mib[i].value,new_mib[i].type));
        ...
        {
        	apmib_set(update_mib[i].id,strToValue((char *)update_mib[i].value,update_mib[i].type));
...
/* new version of set_log in services.c */
#ifndef __DAVO__
void set_log(void)
{
...
static int kill_and_unlink_pid(const char *pidfile, int sig)
{
...
void clean_process(int sys_opmode, int wan_dhcp_mode, int gateway, int enable_wan,
		   char *lanInterface, char *wlanInterface, char *wanInterface)
{
	int pid;
...
/*clean the process before take new setting*/
	...
	//killall(SIGKILL, "pptp.sh");
	//killall(SIGKILL, "pppoe.sh");
	//killall(SIGKILL, "l2tp.sh");
	...
	killall(SIGKILL, "timelycheck");	//LZQ

#if defined(CONFIG_APP_FWD)
	killall(SIGKILL, "fwd");	//LZQ
#endif
#ifdef HOME_GATEWAY
#ifdef MULTI_PPPOE
	wan_disconnect("all", "");
	...
	kill_and_unlink_pid(L2TPD_PID_FILE, SIGKILL);
	/* kill dhcp client */
	killDhcpcDaemons();
	kill_and_unlink_pid(DNRD_PID_FILE, SIGKILL);
	if (kill_and_unlink_pid(IGMPPROXY_PID_FILE, SIGKILL) > -1)
		yecho(PROC_BR_MCASTFASTFWD, "1,1\n");
	kill_and_unlink_pid(LLTD_PID_FILE, SIGKILL);
	pid = kill_and_unlink_pid(DHCPD_PID_FILE, SIGUSR1);
	if (pid > 0) {
		sleep(1);
		kill(pid, SIGKILL);
	}
#ifdef CONFIG_IPV6
	kill_and_unlink_pid(DHCP6S_PID_FILE, SIGKILL);
	pid = kill_and_unlink_pid(DHCP6C_PID_FILE, SIGUSR1);
	if (pid > 0) {
		sleep(1);
		kill(pid, SIGKILL);
	}
	kill_and_unlink_pid(DNSV6_PID_FILE, SIGKILL);
	kill_and_unlink_pid(RADVD_PID_FILE, SIGKILL);
	kill_and_unlink_pid(ECMH_PID_FILE, SIGKILL);
	/*MLD proxy */
	kill_and_unlink_pid(MLDPROXY_PID_FILE, SIGKILL);

	/* add for rm the configure file */
	unlink(DHCP6S_CONF_FILE);
	unlink(DHCP6C_CONF_FILE);
	unlink(DNSV6_CONF_FILE);
	unlink(RADVD_CONF_FILE);
	unlink(DHCP6PD_CONF_FILE);
	unlink(DNSV6_ADDR_FILE);
	/* end add */
#endif
/*end of clean the process before take new setting*/
	...
	yexecl("2>/dev/null", "iptables -F");
	yexecl("2>/dev/null", "iptables -F -t nat");
	yexecl("2>/dev/null", "iptables -F -t mangle");
	yexecl(NULL, "iptables -A INPUT -i %s -j ACCEPT", lanInterface);
	if (sys_opmode == 1) {
		yexecl(NULL, "iptables -P INPUT ACCEPT");
		yexecl(NULL, "iptables -P FORWARD ACCEPT");
	}
#endif

#if defined(CONFIG_APP_SIMPLE_CONFIG)
	killall(SIGTERM, "simple_config");
#endif
#ifdef SHRINK_INIT_TIME
	cleanWlan_Applications(wlanInterface);
#endif
#if defined(CONFIG_APP_IGMPQUERIER)
	/* jihyun@davo 150614 jcode#1 */
	killall(SIGKILL, "igmpquerier");
#endif
}
...
void start_mount()
{
	...
	yecho("/proc/sys/vm/min_free_kbytes", "2048");
	yecho("/proc/sys/net/core/rmem_max", "1048576");
	yecho("/proc/sys/net/core/wmem_max", "1048576");
	yecho("/proc/sys/net/ipv4/tcp_rmem", "4096 108544 4194304");
	yecho("/proc/sys/net/ipv4/tcp_wmem", "4096 108544 4194304");
	yecho("/proc/sys/net/ipv4/tcp_moderate_rcvbuf", "0");
#else
	/*config linux parameter for improving samba performance*/
	yecho("/proc/sys/vm/min_free_kbytes", "1024");
	yecho("/proc/sys/net/core/netdev_max_backlog", "8192");
	yecho("/proc/sys/net/core/optmem_max", "131072");
	yecho("/proc/sys/net/core/rmem_default", "524288");
	yecho("/proc/sys/net/core/rmem_max", "524288");
	yecho("/proc/sys/net/core/wmem_default", "524288");
	yecho("/proc/sys/net/core/wmem_max", "524288");
	yecho("/proc/sys/net/ipv4/tcp_rmem", "131072 262144 393216");
	yecho("/proc/sys/net/ipv4/tcp_wmem", "131072 262144 393216");
	yecho("/proc/sys/net/ipv4/tcp_mem", "768 1024 1380");
#endif
	/*config hot plug and auto-mount*/
	yecho("/proc/sys/kernel/hotplug", "/usr/hotplug\n");
	mkdir("/tmp/usb/", 0666);

	/*force kernel to write data to disk, don't cache in memory for a long time*/
	yecho("/proc/sys/vm/vfs_cache_pressure", "10000");
	yecho("/proc/sys/vm/dirty_background_ratio", "5");
	yecho("/proc/sys/vm/dirty_writeback_centisecs", "100");
	/*automatically mount partions listed in /proc/partitions*/
	autoMountOnBootUp();

	killall(SIGTERM, "usbStorageAppController");
	yexecl(NULL, "sh -c \"usbStorageAppController &\"");
...
static void rtl_configRps(void)
{
	...
	for (i = 0; i <= 4; i++) {
		sprintf(buf, "/sys/class/net/%s%d/queues/rx-0/rps_cpus", "eth", i);
		if (!stat(buf, &statbuf))
			yecho(buf, "2\n");
		sprintf(buf, "/sys/class/net/%s%d/queues/rx-0/rps_flow_cnt", "eth", i);
		if (!stat(buf, &statbuf))
			yecho(buf, "4096\n");
	}
	yecho("/sys/class/net/wlan0/queues/rx-0/rps_cpus", "2\n");
	yecho("/sys/class/net/wlan1/queues/rx-0/rps_cpus", "2\n");
	yecho("/proc/sys/net/core/rps_sock_flow_entries", "4096\n");
}
#endif

#ifdef __DAVO__
static void set_phy(const char *cmd)
{
	struct nmpipe *namedp;
	char buf[32];

	sprintf(buf, "setphy %s", cmd);
	namedp = prequest(buf);
	if (namedp) {
		presponse(namedp, buf, sizeof(buf));
		prelease(namedp);
	} else
		cprintf("Unsupported setphy %s command\n", cmd);
}
#endif
...
#endif
#ifdef __DAVO__
	yecho("/proc/sys/kernel/printk", "%d", nvram_atoi("x_console_loglevel", 1));
	set_phy("down");
#endif
...
#ifdef CONFIG_IPV6
	yecho("/proc/sys/net/ipv6/conf/eth1/disable_ipv6", "1");
#endif
#ifdef MULTI_PPPOE
...
//			RunSystemCmd(NULL_FILE, "rm", "-f", "/web/*.cpudat", NULL_STR);
	...
	if(isFileExist(REINIT_FILE)==0){
		...
		reinit = 1;
#ifdef __DAVO__
	killall(SIGKILL, "rtnetlnk");
	killall(SIGKILL, "arprobe");
	killall(SIGTERM, "dhcpr");
	killall(SIGTERM, "arp_defender");
#if defined(CONFIG_IPV6)
	killall(SIGKILL, "dadd");
	killall(SIGKILL, "dhcp6c");
	unlink("/var/lib/dhcp6/dhcp6c.pid");
#endif	/* CONFIG_IPV6 */
#endif	/* __DAVO__ */
	...
	if (get_repeater_mode()) {
		lan_dhcp_mode=DHCP_LAN_SERVER;
	}

	if (opmode == GATEWAY_MODE)
		yecho("/proc/sys/private/neigh_unsolicited_add", "1");
#endif
	if(opmode==GATEWAY_MODE)
		yecho("/var/sys_op", "0\n");
...
//printf("\r\n wan_interface=[%s],__[%s-%u]\r\n",wan_interface,__FILE__,__LINE__);
	...
	*/
#ifdef __DAVO__
	start_rtnetlnk(opmode, wan_dhcp_mode);
#endif
...
#ifdef __DAVO__
		if (access("/proc/br_wlanblock", F_OK) == 0)
			yecho("/proc/br_wlanblock", "%d\n", (br_wlan_block == NUM_WLAN_INTERFACE));
#else
...
#ifdef __DAVO__
	yexecl(NULL, "aclwrite del br0 -d in -a cpu -r sfilter -p 67:67 -P 0xf/0x1f -o 7");
	if (nvram_atoi("x_dhcpd_protect", 1))
		yexecl(NULL, "aclwrite add br0 -d in -a cpu -r sfilter -p 67:67 -P 0xf/0x1f -o 7");
#endif
...
//printf("\r\n br_interface=[%s],__[%s-%u]\r\n",tmpBuff,__FILE__,__LINE__);
			...
			if(lan_dhcp_mode==DHCP_LAN_NONE){		/*DHCP disabled*/
				...
				sprintf(Ip, "%s", inet_ntoa(ipaddr));
				apmib_get( MIB_SUBNET_MASK,  (void *)&ipaddr);
				sprintf(Mask, "%s", inet_ntoa(ipaddr));
				apmib_get(MIB_DEFAULT_GATEWAY,  (void *)&ipaddr);

				if (ipaddr.s_addr == 0)
					memset(Gateway, 0x00, sizeof(Gateway));
				else
					sprintf(Gateway, "%s", inet_ntoa(ipaddr));
...
//printf("\r\n wan_interface=[%s],__[%s-%u]\r\n",wan_interface,__FILE__,__LINE__);
...
#if defined(__DAVO__)
			start_wan(wan_dhcp_mode, opmode, br_interface, br_interface, wisp_wan_id, 1);
			set_jumbo_frm();
#endif
		}
#ifndef __DAVO__
	...
	yecho("/proc/custom_Passthru", "%d", !!(intValue & 0x1));
	if (intValue == 0)
	{
	...
	{
		/* forbid any ipv6 packet with peth0 hardware address
		   from being transmitted to WAN
		 */
		yecho("/proc/sys/net/ipv6/conf/peth0/disable_ipv6", "1");
	...
	yecho(PROC_BR_IGMPVERSION, "3\n");
#else
	yecho(PROC_BR_IGMPVERSION, "2\n");
#endif
	yecho(PROC_BR_IGMPSNOOP, "1\n");
	yecho(PROC_BR_IGMPQUERY, "1\n");
#if defined (CONFIG_RTL_MLD_SNOOPING)
	yecho(PROC_BR_MLDSNOOP, "1\n");
	yecho(PROC_BR_MLDQUERY, "1\n");
#endif
	...
	if(intValue)
	{
		yecho(PROC_BR_IGMPSNOOP, "fastleave 0 2\n");
	}
	else
	{
		yecho(PROC_BR_IGMPSNOOP, "fastleave 1 0\n");
	}
#ifdef CONFIG_IPV6
	if (nvram_atoi("x_ipv6_autoconfig_method", 0))
		in6_addrconf("br0", 1);
#endif
	...
	yecho("/proc/sys/net/ipv4/conf/eth1/arp_ignore", "1\n");
	/* increase routing cache rebuild count from 4 to 2048 */
	if (!access(RT_CACHE_REBUILD_COUNT, F_OK))
		yecho(RT_CACHE_REBUILD_COUNT, "2048\n");

// rssi_threshold move to set_wlanapp.c

#ifdef SHRINK_INIT_TIME
	if(opmode != BRIDGE_MODE)
#endif
	yexecl(NULL, "sh -c \"timelycheck &\"");
	yexecl("2>/dev/null", "cp /etc/passwd_orig /var/passwd");
	yexecl("2>/dev/null", "cp /etc/group_orig /var/group");
#ifdef SAMBA_WEB_SUPPORT
	...
	/* Unless QoS has been applied according to configuration
	 * recover ingress bandwidth to default.
	 */
	f = locked_fopen("/var/.qoslock", "a", 0);
	if (f) {
		fseek(f, 0, SEEK_END);
		if (ftell(f) <= 0)
			yexecl(NULL, "dvqos --throttle inrate 0");
		locked_fclose(f);
	}
	//
	yexecl(NULL, "/bin/broadcast_storm.sh");

	start_arprobe(opmode, wan_dhcp_mode);
...
/* APACRTL-524 */
	...
	if (nvram_atoi("x_autoreboot_success", 0))
		yexecl(NULL, "sh -c \"resetTrap &\"");

	yexecl(NULL, "sh -c \"apscanTrap &\"");
	yexecl(NULL, "/bin/mirror apply");
#endif
	return 0;
}
