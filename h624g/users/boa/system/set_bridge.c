...
void start_lan_dhcpd(char *interface)
{
	...
	create_dhcpd_conf(DHCPD_CONF_FILE, interface, 2);

	apmib_get(MIB_IP_ADDR, (void *)&nip);
	sprintf(ip, "%u.%u.%u.%u", NIPQUAD(nip));
	apmib_get(MIB_SUBNET_MASK, (void *)&nip);
	sprintf(mask, "%u.%u.%u.%u", NIPQUAD(nip));

	ifconfig(interface, IFUP, ip, mask);

	yexecl(NULL, "udhcpd "DHCPD_CONF_FILE);
}
...
void set_lan_dhcpd(char *interface, int mode)
{
	...
	create_dhcpd_conf(DHCPD_CONF_FILE, interface, mode);
	...
	ifconfig(interface, IFUP, ip, mask);
	yexecl(NULL, "udhcpd "DHCPD_CONF_FILE);
#if defined(USB_UWIFI_HOST_SUPPORTED)
	start_nmbserver();
	start_uWiFid();
#endif
}
...
void set_lan_dhcpc(char *iface)
{
    ...
    apmib_get(MIB_IP_ADDR, (void *)&(tmp_addr_t));
	sprintf(&Ip[0], "%s", inet_ntoa(tmp_addr_t));

	apmib_get(MIB_SUBNET_MASK, (void *)&(tmp_addr_t));
	sprintf(Mask, "%s", inet_ntoa(tmp_addr_t));

	apmib_get(MIB_DEFAULT_GATEWAY,  (void *)&(tmp_addr_t));
	sprintf(Gateway, "%s", inet_ntoa(tmp_addr_t));
...
//delete interface of bridge
...
#if 0 // TODO CONFIG_RTL_STP
	for (j=0; j<5; j++) {
		sprintf(tmpBuff, "port%d", j);
		yexecl((br_stp_enabled == 1) ? NULL:"2>/dev/null", "brctl %s br0 %s",
				(br_stp_enabled == 1) ? "addif":"delif", tmpBuff);
		ifconfig(tmpBuff, (br_stp_enabled == 1) ? IFUP:0, NULL, NULL);
	}
#endif
	if (br_stp_enabled == 1) {
		...
		*((char *)&vlan_entry) = (char)i;
		...
		if (isFileExist(tmpBuff)) {
			...
			if (strncmp(vlan_entry.netIface, "eth1", strlen("eth1")) == 0) {
				...
				yfecho("/proc/eth7/mib_vlan", O_WRONLY|O_TRUNC, 0644, "1 %d %d %d %d %d %d %d",
						VlanisLan, vlan_entry.enabled, vlan_entry.tagged, vlan_entry.vlanId,
						vlan_entry.priority, vlan_entry.cfi, vlan_entry.forwarding_rule);
#if 0
				sprintf(cmdBuffer, "echo \"1 %d %d %d %d %d %d %d\" > /proc/eth7/mib_vlan", VlanisLan,
						vlan_entry.enabled, vlan_entry.tagged, vlan_entry.vlanId, vlan_entry.priority,
						vlan_entry.cfi, vlan_entry.forwarding_rule);
				system(cmdBuffer);
#endif
				...
				if (vlan_enabled == 1) {
					if (opmode == GATEWAY_MODE) {
						yfecho("/proc/peth0/mib_vlan", O_WRONLY|O_TRUNC, 0644, "1 %d %d %d %d %d %d %d",
								VlanisLan, vlan_entry.enabled, vlan_entry.tagged,
								vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
								vlan_entry.forwarding_rule);
#if 0
						sprintf(cmdBuffer, "echo \"1 %d %d %d %d %d %d %d\" > /proc/peth0/mib_vlan",
								VlanisLan, vlan_entry.enabled, vlan_entry.tagged,
								vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
								vlan_entry.forwarding_rule);
						system(cmdBuffer);
				...
				{
					if (opmode == GATEWAY_MODE) {
						yfecho("/proc/peth0/mib_vlan", O_WRONLY|O_TRUNC, 0644, "0 %d %d %d %d %d %d %d",
								VlanisLan,vlan_entry.enabled, vlan_entry.tagged,
								vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
								vlan_entry.forwarding_rule);
#if 0
						sprintf(cmdBuffer, "echo \"0 %d %d %d %d %d %d %d\" > /proc/peth0/mib_vlan",
								VlanisLan,vlan_entry.enabled, vlan_entry.tagged,
								vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
								vlan_entry.forwarding_rule);
						system(cmdBuffer);
			...
			if (vlan_enabled == 1) {	//global_vlan, is_lan,vlan, tag, id, pri, cfi, forwarding_rule
				yfecho(tmpBuff, O_WRONLY|O_TRUNC, 0644, "1 %d %d %d %d %d %d %d",
						VlanisLan, vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
						vlan_entry.forwarding_rule);
#if 0
				sprintf(cmdBuffer, "echo \"1 %d %d %d %d %d %d %d\" > %s",
						VlanisLan,vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
						vlan_entry.forwarding_rule ,tmpBuff);
				system(cmdBuffer);
#endif
			} else
#endif
			{
				yfecho(tmpBuff, O_WRONLY|O_TRUNC, 0644, "0 %d %d %d %d %d %d %d",
						VlanisLan, vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
						vlan_entry.forwarding_rule);
#if 0
				sprintf(cmdBuffer, "echo \"0 %d %d %d %d %d %d %d\" > %s",
						VlanisLan,vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi,
						vlan_entry.forwarding_rule, tmpBuff);
				system(cmdBuffer);
			...
			if (vlan_enabled==1){		//global_vlan, is_lan,vlan, tag, id, pri, cfi
				yfecho(tmpBuff, O_WRONLY|O_TRUNC, 0644, "1 %d %d %d %d %d %d",
						VlanisLan, vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi);
#if 0
				sprintf(cmdBuffer, "echo \"1 %d %d %d %d %d %d\" > %s",
						VlanisLan,vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi, tmpBuff);
				system(cmdBuffer);
#endif
			} else
#endif
			{
				yfecho(tmpBuff, O_WRONLY|O_TRUNC, 0644, "0 %d %d %d %d %d %d",
						VlanisLan, vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi);
#if 0
				sprintf(cmdBuffer,"echo \"0 %d %d %d %d %d %d\" > %s",
						VlanisLan,vlan_entry.enabled, vlan_entry.tagged,
						vlan_entry.vlanId, vlan_entry.priority, vlan_entry.cfi, tmpBuff);
				system(cmdBuffer);
#endif
	...
	yfecho(tmpBuff, O_WRONLY|O_TRUNC, 0644, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
	...
	if (opmode == GATEWAY_MODE) {
		...
		yfecho("/proc/rtk_vlan_wan_tag", O_WRONLY|O_TRUNC, 0644, "%d %d %d %d %d %d %d",
				vlan_wan_enable, vlan_wan_tag, vlan_wan_bridge_enable,
				vlan_wan_bridge_tag, vlan_wan_bridge_port,
				vlan_wan_bridge_multicast_enable, vlan_wan_bridge_multicast_tag);
#if 0
		...
		yfecho("/proc/rtk_vlan_wan_tag_cpu", O_WRONLY|O_TRUNC, 0644, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
...
//				}
				...
				if (intVal == 1) {
					...
					ifconfig(iface_name, IFUP, "0.0.0.0", NULL);
			...
			if (strncmp(iface_name, "wlan", 4) == 0) {	//wlan iface
				...
				if (wlan_disabled==0) { //wlan if is enabled
					...
					if ((iswlan_va==1) || (opmode != WISP_MODE) || (strcmp(wlan_wan_iface, iface_name) != 0)) {	//do not add wlan wan  iface to br0
						...
						ifconfig(iface_name, IFUP, NULL, NULL);
					}
					...
					if (SetWlan_idx(iface_name)) {
						...
						if (wlan_wds_enabled != 0 && wlan_wds_num != 0 && (wlan_mode == WDS_MODE || wlan_mode == AP_WDS_MODE)) {	//add wds inface to br0
							for(j=0; j<wlan_wds_num; j++) {
								...
								ifconfig(tmp_iface, IFUP, "0.0.0.0", NULL);
								if (bridge_iface[0]) {
...
//printf("\r\n bridge_iface=[%s],__[%s-%u]\r\n",bridge_iface,__FILE__,__LINE__);
...
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
	if (opmode == GATEWAY_MODE && vlan_wan_bridge_enable) {
		yfecho(BR_IFACE_FILE2, O_WRONLY|O_CREAT|O_TRUNC, 0644, "%s", bridge_iface2);
	...
	if (isFileExist(BR_INIT_FILE) == 0) {	//bridge init file is not exist
		...
		yfecho(BR_INIT_FILE, O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", 1);
	}
	...
	if (dhcp_mode==DHCP_DISABLED || dhcp_mode==DHCP_SERVER //dhcp disabled or server mode or auto
		...
		apmib_get(MIB_IP_ADDR, (void *)&(tmp_addr_t.s_addr));
		sprintf(lanIp, "%s", inet_ntoa(tmp_addr_t));

		apmib_get(MIB_SUBNET_MASK, (void *)&(tmp_addr_t.s_addr));
		sprintf(lanMask, "%s", inet_ntoa(tmp_addr_t));

		apmib_get(MIB_DEFAULT_GATEWAY, (void *)&(tmp_addr_t.s_addr));
		sprintf(lanGateway, "%s", inet_ntoa(tmp_addr_t));
		ifconfig("br0", IFUP, lanIp, lanMask);
		...
		lan_addr &= lan_mask;
        tmp_addr_t.s_addr = lan_addr;
		sprintf(lanIp, "%s", inet_ntoa(tmp_addr_t));
		route_del(NULL, 0, lanIp, NULL, lanMask);
		route_add("br0", 0, lanIp, NULL, lanMask);
		//end hyking added

		if (strcmp(lanGateway, "0.0.0.0")) {
			route_del("br0", 0, "0.0.0.0", NULL, "0.0.0.0");
			route_add("br0", 0, "0.0.0.0", lanGateway, "0.0.0.0");
		}
		/* jihyun@davo150602 jcode#0 */
		yecho("/var/bridge_done", "done");
...
//		if(br_stp_enabled==1){		/* Comment here because br0 entering forwarding state always need some time especially booting up.*/
	...
	if (opmode == BRIDGE_MODE) {
		if (nvram_match("OPTION82", "checked")) {
			int verbose = nvram_atoi("x_dhcpr_debug", 0);
			yexecl(NULL, "sh -c \"dhcpr -i br0 %s&\"",
			       (verbose > 1) ? "-dd" : (verbose ? "-d" : ""));
		}
	}
	return 0;
}
...
#if defined(CONFIG_APP_SIMPLE_CONFIG)
	yfecho("/var/sc_ip_status", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", 2);
#endif
}
...
void check_l2tp_status()
{
	...
	yfcat("/var/dhcpc_route.conf", "%s", routebuf);
#else
	FILE *fp=NULL;
...
//	system("l2tpd &");
	yfecho("/var/run/l2tp-control", O_WRONLY|O_CREAT|O_TRUNC, 0644, "c client");
//	system("echo c client > /var/run/l2tp-control 2>/dev/null");

#if 1
	yfcat("/var/dnrd_cmd_line.conf", "%s", cmdbuf);
	system(cmdbuf);
#else
