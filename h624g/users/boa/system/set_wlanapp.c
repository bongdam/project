...
void wl_rate_limit_set(void);
#endif // defined(DAVO_ENABLE_RATELIMIT)
void wl_connect_limit_count_set(void);
void wl_rssi_threshold_set(void);
static void wl_retry_count_set(void);
...
static int start_wsc_deamon(char * wlan_interface, int mode, int WSC_UPNP_Enabled, char * bridge_iface) {
    ...
    token = strtok_r(&arg_buff[0]," ", &savestr1);
    if(token)
...
int setWlan_Applications(char *action, char *argv)
{
	...
	int wlan_wsc_disabled_root=0, wlan_network_type_root=0;
	int wlan_1x_enabled_root=0, wlan_encrypt_root=0, wlan_mac_auth_enabled_root=0;
	...
	int isWLANEnabled=0, isAP=0;
	char bridge_iface[30]={0};
	char *token=NULL, *savestr1=NULL;	
	int WSC=1, WSC_UPNP_Enabled=0;
	int check_cnt = 0;
	int widx, use_autobonding;
	
    //char wlan_wapi_asipaddr[100];
	//int wlan_wapi_cert_sel;
	//int wlan0_mode=1, wlan1_mode=1;
    //int wlan0_wsc_disabled_vxd=1, wlan1_wsc_disabled_vxd=1;
    //int wlan_wapi_auth=0;
    //int intValue=0;
	//char *cmd_opt[16]={0};
	//int cmd_cnt = 0;
...
#if defined(__DAVO__)
	/* jihyun@davo 150701 jcode#7 */
	int dfs_enable = nvram_atoi("x_dfs_enable", 1);
#endif
					...
					if(_enable_1x !=0 && wlan_disabled==0){
						...
						if(wlan_mode != 2 && vap_not_in_pure_ap_mode==0){
							...
							if(wlan_wpa_auth != 2 || _use_rs !=0 ){
								deamon_created=1;
/* check /tmp/auth */
							if(access("/tmp/auth",0)<0) {
								RunSystemCmd(NULL_FILE, "auth", iface_name, bridge_iface, wlan_role, tmpBuff, NULL_STR); 
							} else {
								RunSystemCmd(NULL_FILE, "/tmp/auth", iface_name, bridge_iface, wlan_role, tmpBuff, NULL_STR); 
							}
...
#if defined(__DAVO__)
	/* jihyun@davo 150701 jcode#7 */
	yexecl(NULL, "iwpriv wlan0 set_mib disable_DFS=%d", ((dfs_enable)? 0:1) );
	
	/* 	rewrite 808 reg, 0xff -> 0x66 for receiving sensitivity by netami
		jihyun@davo 150701 jcode#6 	*/
	yexecl(NULL, "ob 808 66");
	
#if defined(DAVO_ENABLE_RATELIMIT)
	wl_rate_limit_set();
#endif // defined(DAVO_ENABLE_RATELIMIT)

	wl_connect_limit_count_set();
	wl_rssi_threshold_set();
...
// wmmmap initialize
...

	{
		...
		p = nvram_get("x_bs_enable");
		val = p ? atoi(p):0;
		yfecho("/proc/sys/private/bs/enable", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", val);
		yfecho("/proc/wlan0-va3/bsteer_enable", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", val);

		p = nvram_get("x_bs_rssi_th");
		val = p ? atoi(p):-80;
		if ((val < 0) && (val > -100))
			yfecho("/proc/sys/private/bs/rssi_threshold", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", val);
    
        p = nvram_get("x_bs_probe_deny_rssi_th");
		val = p ? atoi(p):-70;
		if ((val < 0) && (val > -100))
			yfecho("/proc/sys/private/bs/probe_deny_rssi", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", val);
    
        p = nvram_get("x_bs_tcp_pps_check_time");
		val = p ? atoi(p):20;
        if ((val >= 0) && (val <= 100))
			yfecho("/proc/sys/private/bs/pps_checking_time", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", val);
        
        p = nvram_get("x_bs_handover_delay_time");
		val = p ? atoi(p):3600;
        if (val >= 0)
			yfecho("/proc/sys/private/bs/time_delay_bsteer", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", val);
        
        p = nvram_get("x_bs_tcp_pkts_threshold");
		val = p ? atoi(p):2;
        if ((val > 0) && (val <= 100))
			yfecho("/proc/sys/private/bs/tcp_pkts_threshold", O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", val);

	}
	for ( widx = 0; widx < 2; widx++) {
		tmpBuff[0] = tmp_iface[0] = 0;
		sprintf(&tmpBuff[0], "/proc/dv_wlan%d/auto_bonding", widx);
		sprintf(&tmp_iface[0], "x_wlan%d_auto_bonding", widx);
		use_autobonding = nvram_atoi(tmp_iface, ((widx)? 1: 0));

		yfecho(tmpBuff, O_WRONLY|O_CREAT|O_TRUNC, 0644, "%d", use_autobonding);
	}
	
	wl_retry_count_set();

#if 1 // 임시 SSID 변경 (정식 release 시 해당 코드 삭제)
	{
		int c = 0;
		unsigned char mac[32];
		char wifissid[32], gigassid[32];
		char ssid[32];

		apmib_get(MIB_HW_NIC1_ADDR, mac);

		snprintf(wifissid, sizeof(wifissid), "SK_WiFi%02X%02X", mac[4], mac[5]);
		snprintf(gigassid, sizeof(gigassid), "SK_GIGA%02X%02X", mac[4], mac[5]);
		if (!strncmp(nvram_safe_get("WLAN1_SSID"), wifissid, 9) || !strncmp(nvram_safe_get("WLAN1_SSID"), gigassid, 9)) {
			snprintf(ssid, sizeof(ssid), "SK_WiFiGIGA%02X%02X", mac[4], mac[5]);
			nvram_set("WLAN1_SSID", ssid);
			nvram_set("WLAN0_VAP3_SSID", ssid);
			c++;
		}

		snprintf(wifissid, sizeof(wifissid), "SK_WiFi%02X%02X_5G", mac[4], mac[5]);
		snprintf(gigassid, sizeof(gigassid), "SK_GIGA%02X%02X_5G", mac[4], mac[5]);
		if (!strncmp(nvram_safe_get("WLAN0_SSID"), wifissid, 12) || !strncmp(nvram_safe_get("WLAN0_SSID"), gigassid, 12)) {
			snprintf(ssid, sizeof(ssid), "SK_WiFiGIGA%02X%02X_5G", mac[4], mac[5]);
...
void wl_rate_limit_set(void)
{
	...
	struct iwreq wrq;
	char set_info[80];
	char nv_prefix[16], wl_name[16];
	char nv_name[64], *nv_val;

	int tx_restrict, rx_restrict;
	...
	for (wl_idx=0; wl_idx < 2; wl_idx++) {
		strncpy(wrq.ifr_name, (wl_idx==0)?"wlan0":"wlan1", sizeof(wrq.ifr_name));
		for ( i =0 ; i < 5; i++) {
			if (i == 0) {
				snprintf(nv_prefix, sizeof(nv_prefix), "WLAN%d", wl_idx);
				snprintf(wl_name, sizeof(wl_name), "wlan%d", wl_idx);
			} else {
				snprintf(nv_prefix, sizeof(nv_prefix), "WLAN%d_VAP%d", wl_idx, i-1);
				snprintf(wl_name, sizeof(wl_name), "wlan%d-vap%d", wl_idx, i-1);
			}
			tx_restrict = rx_restrict = 0;

			sprintf(&set_info[0], "clear %s 0 50", wl_name);
			wrq.u.data.pointer = (caddr_t)set_info;
			wrq.u.data.length = sizeof(set_info);
			wrq.u.data.flags = RTL8192CD_IOCTL_RATE_LIMIT_SET;
			ioctl(skfd, SIOCIWCUSTOM, &wrq);

			snprintf(nv_name, sizeof(nv_name), "%s_TX_RESTRICT", nv_prefix);
			if ((nv_val = nvram_get(nv_name))) {
				tx_restrict = atoi(nv_val);
			}
			snprintf(nv_name, sizeof(nv_name), "%s_RX_RESTRICT", nv_prefix);
			if ((nv_val = nvram_get(nv_name))) {
				rx_restrict = atoi(nv_val);
			}

			if (tx_restrict>0) {
				sprintf(&set_info[0], "to %s %dm 50", wl_name, tx_restrict);
				wrq.u.data.flags = RTL8192CD_IOCTL_RATE_LIMIT_SET;
				ioctl(skfd, SIOCIWCUSTOM, &wrq);
			} 
			if (rx_restrict>0) {
				sprintf(&set_info[0], "from %s %dm 50", wl_name, rx_restrict);
				wrq.u.data.flags = RTL8192CD_IOCTL_RATE_LIMIT_SET;
				ioctl(skfd, SIOCIWCUSTOM, &wrq);
			}
		}
...
void wl_connect_limit_count_set(void)
{
	...
	for (wl_idx = 0; wl_idx < 2; wl_idx++) {
		len = 0;
		for (i = 0 ; i < 5; i++) {
			get_wlan_name(nv_name, wl_idx, i, "max_conn");
			max_connect = 0;
            nv_val = nvram_get(nv_name);
			if (nv_val)
				max_connect = atoi(nv_val);

			len += snprintf((char *)&buf[len], sizeof(buf)-len, "%d ", max_connect);
		}
		strncpy(wrq.ifr_name, (wl_idx==0)?"wlan0":"wlan1", sizeof(wrq.ifr_name));
		wrq.u.data.pointer = (caddr_t)set_info;
		wrq.u.data.length = sizeof(set_info);
		wrq.u.data.flags = RTL8192CD_IOCTL_CONNECT_LIMITCOUNT_SET;
		sprintf(&set_info[0], "%s", (char *)buf);
		ioctl(skfd, SIOCIWCUSTOM, &wrq);
	}
...
void wl_rssi_threshold_set(void)
{
	...
	for (i = 0; i < 2; i++) { // DUAL BAND
		for (j = 0; j < 5; j++) {
			...
			get_wlan_name(dv_name, i, j, "rssi_threshold");
			if (j == 0) // Main SSID
				sprintf(wl_name, "wlan%d", i);
			else
				sprintf(wl_name, "wlan%d-va%d", i, j-1);
			if (!(p = nvram_get(dv_name))) {
				if (j == 2) // T wifi home
					p = "75";
			}
			if (p) {
				strncpy(wrq.ifr_name, wl_name, sizeof(wrq.ifr_name));
				wrq.u.data.pointer = (caddr_t)rssi_info;
				wrq.u.data.length = sizeof(rssi_info);
				wrq.u.data.flags = RTL8192CD_IOCTL_RSSI_THRESHOLD_SET;
				sprintf(&rssi_info[0], "%s", p);
				ioctl(skfd, SIOCIWCUSTOM, &wrq);
			}
		}
	}
...
static void wl_retry_count_set(void)
{
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < 5; j++) {
			...
			yexecl(NULL, "iwpriv %s set_mib longretry=7", wl_name);
			yexecl(NULL, "iwpriv %s set_mib shortretry=7", wl_name);
		}
	}
}
#endif
