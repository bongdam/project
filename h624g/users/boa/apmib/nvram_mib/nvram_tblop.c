	...
	if (p != NULL) {
		if (argumentize(p, args, MIN_ARGC) == MIN_ARGC) {
			if (yxatoi(t->macAddr, args[0], sizeof(t->macAddr) << 1)) {
				ystrncpy((char *)t->comment, args[1], sizeof(t->comment));
				status = TRUE;
			}
		}
		free(p);
...
add_wlac_array_t(MACFILTER_T *t, const struct mib *mib, const struct mib *nib)
{
	...
	ynvram_put("%s%d=%02x%02x%02x%02x%02x%02x,%s", name, pos + 1,
		...
		t->macAddr[3], t->macAddr[4], t->macAddr[5], t->comment);
	...
	if (argumentize(p, args, MIN_ARGC)) {
		do {
			...
			if (!inet_aton(*argp++, (struct in_addr *)&t->ipAddr[0]))
				break;
			ystrncpy((char *)t->hostName, *argp ? : "", sizeof(t->hostName));
			status = TRUE;
		} while (0);
	}
	free(p);
...
add_dhcprsvdip_arry_t(DHCPRSVDIP_T *t, const struct mib *mib, const struct mib *nib)
{
	...
	ynvram_put("%s%d=%02x%02x%02x%02x%02x%02x," IPFMT ",%s", name, pos + 1,
		...
		t->ipAddr[0], t->ipAddr[1], t->ipAddr[2], t->ipAddr[3], t->hostName);
...
#define OPT_ARGC	1
	...
	if (argumentize(p, args, MIN_ARGC)) {
		if (inet_aton(args[0], (struct in_addr *)&t->ipAddr[0])) {
			...
			ystrncpy((char *)t->comment, (args[4]) ? : "", sizeof(t->comment));
			status = TRUE;
		}
	}
	free(p);
...
add_portfw_array_t(PORTFW_T *t, const struct mib *mib, const struct mib *nib)
{
	...
	ynvram_name(name, sizeof(name), mib->name, mib->section);
	ynvram_put("%s%d=" IPFMT ",%u,%u,%u,%s", name, pos + 1,
...
# define OPT_ARGC	1
	...
	if (argumentize(p, args, MIN_ARGC)) {
		if (inet_aton(args[0], (struct in_addr *)&t->ipAddr[0])) {
			...
			ystrncpy((char *)t->comment, (args[2]) ? : "", sizeof(t->comment));
#ifdef CONFIG_IPV6
...
#define OPT_ARGC	1
	...
	if (argumentize(p, args, MIN_ARGC)) {
		...
		ystrncpy((char *)t->comment, (args[3]) ? : "", sizeof(t->comment));
		status = TRUE;
	}
	free(p);
#undef MIN_ARGC
...
add_portfilter_array_t(PORTFILTER_T *t, const struct mib *mib, const struct mib *nib)
{
	...
	ynvram_put("%s%d=%u,%u,%u,%s", name, pos + 1,
		t->fromPort, t->toPort, t->protoType, t->comment);
...
#define OPT_ARGC	1
	...
	if (argumentize(p, args, MIN_ARGC)) {
		...
		ystrncpy((char *)t->comment, (args[6]) ? : "", sizeof(t->comment));
		status = TRUE;
	}
	free(p);
#undef MIN_ARGC
...
#define OPT_ARGC	0
	...
	if (argumentize(p, args, MIN_ARGC)) {
		do {
			...
			ystrncpy((char *)t->entry_name, args[7], sizeof(t->entry_name));
			status = TRUE;
		} while (0);
		if (status != TRUE)
			nm_errno = ENM_INVAL;
...
add_qos_array_t(IPQOS_T *t, const struct mib *mib, const struct mib *nib)
{
	...
	ynvram_put("%s%d=%d,%02x%02x%02x%02x%02x%02x,%d," IPFMT "," IPFMT ",%u,%u,%s",
		...
		t->bandwidth, t->bandwidth_downlink, t->entry_name);
	...
	if (argumentize(p, args, MIN_ARGC)) {
		do {
			...
			if (t->tTime >= 1440)
				break;
#ifndef NEW_SCHEDULE_SUPPORT
			ystrncpy((char *)t->text, args[4] ? : "", sizeof(t->text));
#endif
			status = TRUE;
		} while (0);
		if (status != TRUE)
...
#define OPT_ARGC	0
	...
	if ((argc = argumentize(p, args, MIN_ARGC))) {
		ystrncpy((char *)t->netIface, args[0], sizeof(t->netIface));
...
#define OPT_ARGC	5
	...
	memset(t, 0, sizeof(*t));
	if ((argc = argumentize(p, args, MIN_ARGC))) {
		ystrncpy((char *)t->ssid, args[0], sizeof(t->ssid));
		...
		case 4:
			...
			ystrncpy((char *)t->wpaPSK, args[4], sizeof(t->wpaPSK));
			status = TRUE;
			break;
		default:
			break;
