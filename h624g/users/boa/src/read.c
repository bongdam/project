		...
		if (req->status == ONE_LF) {
			*req->header_end = '\0';
			...
			if (retval && req->method == M_POST) {
				...
				if ( strstr(&req->request_uri[0], "formMfgTest") )
					return 0;
#endif
		...
		if (bytes < 0) {
			...
			if (req->kacount <= ka_max && !req->logline && req->client_stream_pos == 0) {
				...
				fprintf(stderr, "%s -- request \"%s\" (\"%s\"): \n",
					req->remote_ip_addr,
					(req->logline ? req->logline : "(null)"), (req->pathname ? req->pathname : "(null)"));
#endif
			}
			req->response_status = 400;
			return 0;
...
#if defined(__DAVO__)
	if ( strstr(&req->request_uri[0], "formMfgTest") ) {
		log_boaform("formMfgTest");
		return init_form(req);
	}
#endif
...
# else
	if (req->upload_data) {
# ifdef __DAVO__
		if ((req->upload_len + bytes_to_write) > (MAX_FWSIZE - sizeof(struct fwstat))) {
			boa_perror(req, "upload image too big!");
			return -2;
		}
# else
...
# endif
	...
	DEBUG(DEBUG_CGI_INPUT) {
		log_error_time();
		{
			...
			fprintf(stderr,
				"%s:%d - wrote %lld bytes (%s). %llu of %llu\n",
				__FILE__, __LINE__, bytes_written, req->header_line, req->filepos, req->filesize);
