...
#endif
#ifdef __DAVO__
    {"autoreboot_status", autoreboot_status},
    {"show_ipv6_information", show_ipv6_information},
    {"show_ipv4_information", show_ipv4_information},
    {"staticMappingTbl", staticMappingTbl},
    {"getInfo_formDaa", getInfo_formDaa},
    {"wlDetectAvoid", wlDetectAvoid},
    {"LanRestrictList", LanRestrictList},
	{"get_ping_info", get_ping_info},
	{"igmp_stb_info", igmp_stb_info},
	{"get_wlan_traffic", get_wlan_traffic},
	{"wlAcMACList", wlAcMACList},
	{"wlAcCommentList", wlAcCommentList},
	{"print_multipleApMenu", print_multipleApMenu},
	{"show_acltbl", show_acltbl},
	{"igmpBlockStatus", igmpBlockStatus},
	{"igmp_snoop_table", igmp_snoop_table},
	{"print_wme_dscp", print_wme_dscp},
	{"print_wme_1p", print_wme_1p},
	{"get_connection_info", get_connection_info},
	{"get_IP_connection_info", get_IP_connection_info},
	{"netconn_viewer", netconn_viewer},
	{"captcha_img", captcha_img},
	{"macList", macList},
	{"sta_protection_list", sta_protection_list},
#endif
    {NULL, NULL}
};
...
/*#if defined(VLAN_CONFIG_SUPPORTED)
    ...
    {"formStats", formStats},
    {"formIpConnection", formIpConnection},
    ...
    {"formLdap", formLdap},
    {"formAutoReboot", formAutoReboot},
    {"formPortSetup", formPortSetup},
    {"formWanIpRenewal", formWanIpRenewal},
    {"formMfgTest", formMfgTest},
    {"formAutoup", formAutoup},
    {"formRedirWlEncrypt", formRedirWlEncrypt},
    {"formStMapping", formStMapping},
    {"formTftpUpload", formTftpUpload},
    {"formWlReset", formWlReset},
    {"formQosQue", formQosQue},
    {"formDaa", formDaa},
    {"formJumbo", formJumbo},
    {"formLanRestrict", formLanRestrict},
	...
	{"formIgmpSet", formIgmpSet},
	{"formWlwmm", formWlwmm},
	{"formSetOperation", formSetOperation},
	{"formHolepunchSet", formHolepunchSet},
	{"formwlprotection", formwlprotection},
	{"formwlprodelete", formwlprodelete},
#endif
    {"formLogin", formLogin},
    {NULL, NULL}
};
...
#define EXPIRE_TIME					LOGIN_AUTOEXIT_TIME // in sec
...
void log_boaform(char *form)
{
    ...
    for (i = 0; i < MAX_TBL_SIZE; i++) {
        if (!security_tbl[i].valid ||
            ...
            if (security_tbl[i].valid) {
                if (oldest_entry == -1
                    || security_tbl[i].time < oldest_time) {
                    oldest_entry = i;
...
static int is_valid_boaform(char *form)
{
	...
	if (valid == 0) {
		if(!strcmp(form, "formSysLog") || !strcmp(form, "formLogin"))
			valid = 1;
	}
...
static inline int xtoi(char c)
{
    if (isdigit(c))
        return (int)(c - '0');
    else if (isxdigit(c))
        return (int)(c + 10 - (islower(c) ? 'a' : 'A'));
    return -1;
}
...
int getcgiparam(char *dst, char *query_string, char *param, int maxlen)
{
    ...
    while (*query_string && query_string <= end_str) {
        if ((len = (int)(end_str - query_string)) > plen) {
            if (!strncmp(query_string, param, plen)) {
                if (query_string[plen] == '=') {        //copy parameter
                    query_string += plen + 1;
                    y = 0;
                    while ((c = *query_string) && (c != '&')) {
                        switch (c) {
                        case '%':
                            if ((h = xtoi(query_string[1])) > -1 &&
                                (l = xtoi(query_string[2])) > -1) {
                                c = (h << 4 | l);
                                query_string += 2;
                            }
                            break;
...
char *rtl_mime_get_name(char *query_string, char *param,
...
int mime_get_var(char *dst, char *query_string, int query_string_len,
                 char *param, int maxlen)
{
    int len, plen;
    int i, reval;
    ...
    while (query_string_len > 0) {
        ...
        if ((pname = rtl_mime_get_name(cur_boundry, param, len)) != NULL) {
            ...
            while ((pname[0]) != 0x0d || (pname[1] != 0x0a)
                   || (pname[2] != 0x0d) || (pname[3] != 0x0a))
                pname++;
...
char *req_get_cstream_var_in_mime(request *req, char *var,
                                  char *defaultGetValue, int *data_len)
{
    char *buf;
    int ret;
    ...
    if (buf != NULL) {
        ret =  mime_get_var(query_temp_var, buf, req->upload_len, var,
                           MAX_QUERY_TEMP_VAL_SIZE);
    }
...
#endif
					send_redirect_perm(req, "/skb_login.htm");
...
extern char inner_req_buff[1536];
extern int middle_segment;      //Brad add for update content length

#if defined(__DAVO__) && defined(SUPPORT_ASP)
int req_format_write(request * req, char *fmt, ...)
{
    ...
    if (req == NULL || fmt == NULL)
        return 0;

    if (req == &inner_req) {
        bavail = (int)sizeof(inner_req_buff);
        va_start(ap, fmt);
        len = vsnprintf(inner_req_buff, bavail, fmt, ap);
        va_end(ap);
        return (len < bavail) ? len : (bavail - 1);
    }

    bavail = req->max_buffer_size - req->buffer_end;
    va_start(ap, fmt);
    len = vsnprintf(req->buffer + req->buffer_end, bavail, fmt, ap);
    if (len >= bavail) {
        p = realloc(req->buffer, req->max_buffer_size << 1);
        if (p != NULL) {
            req->buffer = p;
            req->max_buffer_size <<= 1;
            va_copy(aq, ap);
            len = vsnprintf(req->buffer + req->buffer_end, req->max_buffer_size, fmt, aq);
            va_end(aq);
        } else
            len = bavail - 1;
    }
    va_end(ap);
    req->buffer_end += len;
    middle_segment = middle_segment + len;      //brad add for update exact length of asp file
...
int websStrArgs(char *line, char *ag[], unsigned agsz, const char *delim)
{
    char *q, *p = line;
    unsigned i, ac = 0;
