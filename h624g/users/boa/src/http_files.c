...
"{__send_requestIE(\"/skb_upload_st.htm?MKey=\"+Magic_Key);}\n"
...
void formHttpFilePathRedirect(request *req,char* http_C,char* http_O)
{
	...
	send_redirect_perm(req,"skb_http_files_dir.htm");
}
...
int generate_directory_page(request *req, char *dest_filename)
{
	...
	bytes += req_format_write(req, "\nfunction __update_state(filename){var cur_dir=document.getElementById(\"current_directory\").innerHTML;if(BrowserDetect.browser==\"Explorer\"){__send_request(\"/skb_usb_fileinfo.htm?file=\"+encodeURIComponent(cur_dir)+\"*\"+encodeURIComponent(filename));}else{__send_request(\"/skb_usb_fileinfo.htm?file=%s*\"+filename);}}\n",req->request_uri);
