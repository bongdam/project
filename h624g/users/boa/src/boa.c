...
#ifdef __DAVO__
unsigned int g_daa_hist;
unsigned int g_daa_status;
struct in_addr g_lan_ip, g_lan_mask;
static unsigned int g_port = 80;
#endif
...
#ifdef __DAVO__
int dv_reboot_system;
int dv_script_reboot;
int need_reboot;
...
//#if defined(CONFIG_H514G) || defined(CONFIG_H524G)
unsigned int search_login_session_num(void)
{
	struct user_info *pUser_info;
	...
	return login_session_num;
}

//#endif /* [H514G] Web Login Session */
...
int free_from_login_list(request * req)
{
	...
	for (pUser_info = user_login_list; pUser_info; bUser_info = pUser_info, pUser_info = pUser_info->next) {
		if (!strcmp(pUser_info->remote_ip_addr, req->remote_ip_addr)) {
			memset(pUser_info->uniq_cookie, 0, sizeof(pUser_info->uniq_cookie));
			break;
		}
	}
...
int main(int argc, char *argv[])
{
	...
	apmib_get(MIB_IP_ADDR, (void *)&g_lan_ip);
	apmib_get(MIB_SUBNET_MASK, (void *)&g_lan_mask);
	...
	if (do_ssl) {
		if (InitSSLStuff() != 1) {
			...
			if (do_sock == 0) {
				printf("    normal sockets disabled, so exiting");
				fflush(NULL);
				return 0;
			} else {
				printf("    supporting normal (unencrypted) sockets only");
				fflush(NULL);
				do_sock = 2;
			}
		}
...
static void parse_commandline(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "c:dl:f:r:p:i:sg")) != -1) {
		switch (c) {
		case 'c':
			if (server_root)
				free(server_root);
...
int InitSSLStuff(void)
{
	...
	if (bind(server_ssl, (struct sockaddr *)&server_sockaddr, sizeof(server_sockaddr)) == -1) {
...
/*	SSL_load_error_strings();*/
	...
	if (SSL_CTX_use_certificate_file(ctx, SSL_CERTF, SSL_FILETYPE_PEM) <= 0) {
		printf("Failure reading SSL certificate file: %s", SSL_CERTF);
		fflush(NULL);
		close(server_ssl);
		return 0;
	}
	...
	if (SSL_CTX_use_PrivateKey_file(ctx, SSL_KEYF, SSL_FILETYPE_PEM) <= 0) {
		printf("Failure reading private key file: %s", SSL_KEYF);
		fflush(NULL);
		close(server_ssl);
		return 0;
	}
	printf("Opened private key file: %s", SSL_KEYF);
	fflush(NULL);
