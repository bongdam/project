...
static int dvcgi_handler_tftpServer(request *wp, struct abuffer *m, char *data)
{
	...
	yexecl(NULL, "iptables -D INPUT -p tcp --dport 6000 -j ACL");
	yexecl(NULL, "iptables -t nat -D PREROUTING -p tcp --dport 6000 -j DNAT --to %s:6000", lan_nip);

	if (data && data[0] == '1') {
		yfecho("/var/telnetd.conf", O_WRONLY|O_CREAT|O_TRUNC, 0644, "telnet_tout 600\n");
		yexecl(NULL, "telnetd");
		yexecl(NULL, "iptables -A INPUT -p tcp --dport 6000 -j ACL");
		yexecl(NULL, "iptables -t nat -A PREROUTING -p tcp --dport 6000 -j DNAT --to %s:6000", lan_nip);
	}
...
{
	if (data && data[0] == '1') {
		yfecho("/proc/wlan0/led", O_WRONLY|O_TRUNC, 0644, "1");
		yfecho("/proc/wlan1/led", O_WRONLY|O_TRUNC, 0644, "1");
		system("echo write 0xbb804300 0x20 4 > /proc/rtl865x/memory");
	} else if (data && data[0] == '0') {
		yfecho("/proc/wlan0/led", O_WRONLY|O_TRUNC, 0644, "0");
		yfecho("/proc/wlan1/led", O_WRONLY|O_TRUNC, 0644, "0");
		system("echo write 0xbb804300 0x0 4 > /proc/rtl865x/memory");
	}
...
#if 0 // except cgi
static int dvcfg_handler_dv_serial(request *wp, struct abuffer *m, char *data)
{
	int len;
	...
	nvram_set("HW_SERIAL_NO", data);
	nvram_commit();

	return 0;
}

static int dvcfg_handler_dv_mac(request *wp, struct abuffer *m, char *data)
{
	int len;
	char buf[60];
	...
	sprintf(buf, "flash set ELAN_MAC_ADDR --sub %s", data);
	system(buf);
...
static int dvcgi_handler_diag_Button(request *wp, struct abuffer *m, char *data)
{
	if (data && !strcmp(data, "reset")) {
		system("echo 1 > /proc/factory_reset_test");
		return 0;
	}
...
static int dvcgi_handler_diag_result(request *wp, struct abuffer *m, char *data)
{
	...
	if ((fp = fopen("/proc/factory_reset_test", "r")) == NULL)
		return -1;
	...
	if (temp[0] == '1')
		aprintf(m, "\"resetDiagPass\"");
	else
		aprintf(m, "\"resetDiagFail\"");
	return 0;
}

#ifdef __CONFIG_GNT2100__
static char *rpc_ont_info(char *cmd, char *buf, size_t len)
{
	...
	namedp = prequest(cmd);
	if (namedp) {
		presponse(namedp, buf, len);
		prelease(namedp);
	}
	return buf;
}

static int print_ont_info(struct abuffer *m)
{
	char buf[64];

	aprintf(m, "ONT Serial:&nbsp;%s<br>\r\n", rpc_ont_info("ont get serial", buf, sizeof(buf)));
	aprintf(m, "ONT Version:&nbsp;%s<br>\r\n", rpc_ont_info("ont get version", buf, sizeof(buf)));
	aprintf(m, "ONT Rx-Power:&nbsp;%s<br><br>\r\n", rpc_ont_info("ont get rxpwr", buf, sizeof(buf)));
	return 0;
}
#endif
...
static int dvcgi_handler_info_system(request *wp, struct abuffer *m, char *data)
{
	...
	aprintf(m, "WAN MAC Address:&nbsp;%02x-%02x-%02x-%02x-%02x-%02x<br>\r\n",
			(unsigned char)sa.sa_data[0], (unsigned char)sa.sa_data[1], (unsigned char)sa.sa_data[2],
			(unsigned char)sa.sa_data[3], (unsigned char)sa.sa_data[4], (unsigned char)sa.sa_data[5]);
	p_buf = nvram_get("HW_SERIAL_NO");
	aprintf(m, "Serial Number:%s (길이:<font color=red>%d</font>)<br><br>\r\n", p_buf, strlen(p_buf));
...
chk_tx_power_t chk_tx_power_5g[] = {
	...
	{"HW_WLAN0_TX_POWER_5G_HT40_1S_C", 170, 1, 0},
	{"HW_WLAN0_TX_POWER_DIFF_5G_20BW1S_OFDM1T_B", 14, 0, 1},
	{"HW_WLAN0_TX_POWER_DIFF_5G_20BW1S_OFDM1T_C", 14, 0, 1},
...
static int cal_chk(char *str, int ch, int val)
{
	...
	} else if (!strcmp("HW_WLAN0_TX_POWER_5G_HT40_1S_C", str)) {
		if (36 <= ch && ch <= 48) {
			if (val < 25 || val > 41) {
				ret = 1;
			}
...
#define USB_MOUNT "/var/tmp/usb"
#define USB_WRITE "/usb_test.txt"
#define MOUNT_BUF_SIZE 20
static int mfgtst_usbhost_detect(char *mount, char *umount)
{
	int ret = 0;
	FILE *fp;
	char *saveptr, *path, *delim = " \t\r\n";
	char buf[80];
	...
	while (fgets(buf, sizeof(buf), fp)) {
		if (strstr(buf, USB_MOUNT)) {
			if ((path = strtok_r(buf, delim, &saveptr)))
				snprintf(umount, MOUNT_BUF_SIZE, path);
			if ((path = strtok_r(NULL, delim, &saveptr)))
				snprintf(mount, MOUNT_BUF_SIZE, path);
			ret = 1;
		}
	}
...
static int mfgtst_usbhost_verify(char *mount)
{
	int ret = 0;
	char buf[20];
	char path[80];

	sprintf(path, "%s"USB_WRITE, mount);

	yfecho(path, O_WRONLY|O_CREAT|O_TRUNC, 0644, "200ok");
	yfcat(path, "%s", buf);
...
static int mfgtst_usbhost_remove(char *mount)
{
	int ret = 0;
	char path[80];
...
static int dvcgi_handler_usb_result(request *wp, struct abuffer *m, char *data)
{
	char buf[40];
	char mount_path[MOUNT_BUF_SIZE];
	char umount_path[MOUNT_BUF_SIZE];

	if (!mfgtst_usbhost_detect(mount_path, umount_path)) {
		aprintf(m, "FAIL : DETECT USB STORAGE");
		return 0;
	}

	if (!mfgtst_usbhost_verify(mount_path)) {
		aprintf(m, "FAIL : WRITE FILE");
		return 0;
	}

	if (!mfgtst_usbhost_remove(mount_path)) {
		aprintf(m, "FAIL : DELETE FILE");
		return 0;
	}

	sprintf(buf, "umount %s", umount_path);
	system(buf);

	if (mfgtst_usbhost_detect(mount_path, umount_path)) {
		aprintf(m, "FAIL : UMOUNT USB");
		return 0;
	}
...
static int dvcgi_handler_usb_test(request *wp, struct abuffer *m, char *data)
{
	system("echo 5 > /proc/gpio");
	aprintf(m, "Insert USB STORAGE");
...
static int dvcgi_handler_handover_disable(request *wp, struct abuffer *m, char *data)
{
	if (!data)
		return -1;

	if (data[0] == '1')
		yexecl(NULL, "iwpriv wlan0-va3 set_mib func_off=1");
	else if (data[0] == '0')
		yexecl(NULL, "iwpriv wlan0-va3 set_mib func_off=0");
	else
		return -1;

	aprintf(m, "Handover %s<br>\r\n", (data[0] == '1') ? "Disable" : "Enable");
...
static int dvcgi_handler_check_wan_link_speed(request *wp, struct abuffer *m, char *data)
{
	...
	link_speed = switch_port_status(4); // Wan link check.
...
#define USING_TELNETPORT	6000
static int dvcgi_handler_wan_telnet_port(request *wp, struct abuffer *m, char *data)
{
	int opmode;
	char lan_nip[30];
	unsigned int port;
	...
	if ( (port = strtoul(&data[1], NULL, 10)) == USING_TELNETPORT )
		return -1;
	...
	if (opmode == GATEWAY_MODE) {
		yexecl("2>/dev/null", "iptables -t nat -D PREROUTING -p tcp --dport %u -i eth1 -j DNAT --to %s:6000", port, lan_nip);
		if ( data[0] == '+')
			yexecl(NULL, "iptables -t nat -I PREROUTING -p tcp --dport %u -i eth1 -j DNAT --to %s:6000", port, lan_nip);
	} else if (opmode == BRIDGE_MODE) {
		if ( !get_ipaddr_file("/var/wan_ip", &wanip, NULL) )
			return -1;
		yexecl("2>/dev/null", "iptables -t nat -D PREROUTING -p tcp -d %s --dport %u -i br0 -j DNAT --to %s:6000",
					inet_ntoa(*((struct in_addr *)&wanip)), port, lan_nip);
		if ( data[0] == '+')
			yexecl(NULL, "iptables -t nat -I PREROUTING -p tcp -d %s --dport %u -i br0 -j DNAT --to %s:6000",
						inet_ntoa(*((struct in_addr *)&wanip)), port, lan_nip);
	}
	aprintf(m, "TELNET %s port %u success...<br>\r\n", ((data[0] == '+')?"add":"del"), port);
	return 0;
}
...
static const cgicommand_t CgiCommands[] = {
	/*command, ahndler, need_run_script, need_reboot, need_factory*/
	{"reboot", dvcgi_handler_reboot, 0, 1, 0},
	{"factoryDefault", dvcgi_handler_factoryDefault, 0, 0, 1},
	{"tftpServer", dvcgi_handler_tftpServer, 0, 0, 0},
	{"led", dvcgi_handler_led, 0, 0, 0},
#if 0 // except cgi
	{"sys_dv_serial", dvcfg_handler_dv_serial, 0, 0, 1},
	{"sys_dv_mac", dvcfg_handler_dv_mac, 0, 0, 1},
#endif
	...
	{"sys_usb_test", dvcgi_handler_usb_test, 0, 0, 0},
	{"sys_usb_result", dvcgi_handler_usb_result, 0, 0, 0},
	{"dumplog", handler_dumplog, 0, 0, 0},
	{"sys_mode", dvcgi_handler_sys_mode, 0, 1, 0},
	{"handover_disable", dvcgi_handler_handover_disable, 0, 0, 0},
	{"check_wan_link_speed", dvcgi_handler_check_wan_link_speed, 0, 0, 0},
	{"tp", dvcgi_handler_wan_telnet_port, 0, 0, 0},
...
static int factory_reset(void)
{
	...
	if (!(f = fopen("/proc/load_default", "w")))
		return -1;
