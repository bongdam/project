...
#ifdef ___DAVO__
static int is_wireless_client(unsigned int ip_addr);
#endif
...
#ifdef __DAVO__
struct in_addr g_lan_ip, g_lan_mask;

static int arplookup(u_int32_t tina, u_char thwa[6], char *ifname)
{
	int s, rc;
	struct arpreq ar;
	...
	rc = ioctl(s, SIOCGARP, (caddr_t)&ar);
	if (!rc)
		bcopy(ar.arp_ha.sa_data, thwa, 6);
	close(s);
	return rc;
}
...
{
#if 0
	WLAN_STA_INFO_Tp psta;
	int radio, i, vap;
	char *p, buf[32];
	u_char mac[6];

	p = calloc(1, sizeof(WLAN_STA_INFO_T) * (MAX_STA_NUM + 1));
	if (p == 0)
		return 0;

	if (!arplookup(ip_addr, mac, "br0")) {
		for (radio = 0; radio < 2; radio++) {
			sprintf(buf, "WLAN%d_WLAN_DISABLED", radio);
			if (nvram_atoi(buf, 0))
				continue;
			for (vap = 1; vap < 5; vap++) {
				sprintf(buf, "WLAN%d_VAP%d_WLAN_DISABLED", radio, vap - 1);
				if (nvram_atoi(buf, 0))
					continue;
				sprintf(buf, "wlan%d-va%d", radio, (vap - 1));
				if (getWlStaInfo(buf, (WLAN_STA_INFO_Tp)p) < 0)
					continue;

				for (i = 1; i <= MAX_STA_NUM; i++) {
					psta = (WLAN_STA_INFO_Tp)&p[i * sizeof(WLAN_STA_INFO_T)];
					if (psta->aid && (psta->flags & STA_INFO_FLAG_ASOC)) {
						if (!memcmp(psta->addr, mac, 6)) {
							free(p);
							return 1;
						}
					}
...
#endif
	fd = accept(server_sock, (struct sockaddr *)&remote_addr, &remote_addrlen);
...
#ifdef __DAVO__
//@ NOTE : don't permit wireless
	if (!nvram_atoi("wlweb_permit", 0)) {
		if ((remote_addr.sin_addr.s_addr & g_lan_mask.s_addr) ==
		    (g_lan_ip.s_addr & g_lan_mask.s_addr)) {
			if (is_wireless_client(remote_addr.sin_addr.s_addr)) {
				fprintf(stderr, "Access from %s not permitted\n",
	...
	if (ascii_sockaddr(&salocal, conn->local_ip_addr, sizeof(conn->local_ip_addr)) == NULL) {
	...
	if (system_bufsize < sockbufsize) {
		if (setsockopt(conn->fd, SOL_SOCKET, SO_SNDBUF, (void *)&sockbufsize, sizeof(sockbufsize)) == -1) {
	...
	if (ascii_sockaddr(&remote_addr, conn->remote_ip_addr, sizeof(conn->remote_ip_addr)) == NULL) {
		WARN("ascii_sockaddr failed");
		close(fd);
		enqueue(&request_free, conn);
#ifdef SERVER_SSL
	...
	{
		int one = 1;
		if (setsockopt(conn->fd, IPPROTO_TCP, TCP_NODELAY, (void *)&one, sizeof(one)) == -1) {
...
static void sanitize_request(request * req, int new_req)
{
	...
	if (new_req) {
		...
		unsigned int bytes_to_move = req->client_stream_pos - req->parse_pos;

		if (bytes_to_move) {
			memmove(req->client_stream, req->client_stream + req->parse_pos, bytes_to_move);
		}
...
static void free_request(request * req)
{
	...
	if (req->upload_data) {
		...
		fprintf(stderr, "free upload data..............................\n");
#ifdef __DAVO__
		munmap(req->upload_data, MAX_FWSIZE);
#else
...
{
	unsigned int login_session_num = 0;
	...
	if (req->request_uri && strstr(req->request_uri, "mfgtst.cgi")) {
		char request_uri_tmp[MAX_HEADER_LENGTH + 1];
		char *p;

		if ((p = strchr(req->request_uri, '?'))) {
			sprintf(&request_uri_tmp[0], "/boafrm/formMfgTest?%s", p + 1);
			memcpy(&req->request_uri[0], &request_uri_tmp[0], sizeof(req->request_uri));
		}
	}
#endif
		...
		/*
	{
		...
		nvram_get_r_def("x_SUPER_NAME", admin_name, sizeof(admin_name), "");
		nvram_get_r_def("x_SUPER_PASSWORD", admin_password, sizeof(admin_password), "");
#endif
		nvram_get_r_def("USER_NAME", user_name, sizeof(user_name), "");
		nvram_get_r_def("USER_PASSWORD", user_password, sizeof(user_password), "");
...
#endif
#ifdef __DAVO__
			req->upload_data = mmap(NULL, MAX_FWSIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
			if (req->upload_data == MAP_FAILED)
#else
...
#ifdef SUPPORT_ASP
#if defined(__DAVO__)		//jjh@todo:1
	if (strstr(req->request_uri, "/boafrm/formMfgTest") ||
	    strstr(req->request_uri, ".htm") ||
	...
	*value++ = '\0';	/* overwrite the : */
	...
	case 'C':
		...
		if (!memcmp(line, "CONTENT_TYPE", 13) && !req->content_type) {
...
#endif
			return 1;
		} else if (!memcmp(line, "CONNECTION", 11) && ka_max && req->keepalive != KA_STOPPED) {
#ifdef SUPPORT_ASP
			...
			req->userName[0] = 0;
			req->password[0] = 0;
			if (strcmp(req->request_uri, "/boafrm/formLogin")!=0) {
				char *cookie, *p;
				struct cookie_session cs;
				char super_name[65];

				cookie = strstr(value, "acookie");
				if (cookie && !strncasecmp(cookie, "acookie", 7)){
					cookie += 8;
					p = cookie;
					while (p && (*p)) {
						if (*p == ';') {
					...
					if (parse_cookie(cookie, &cs)==0) {
						if(inet_addr(req->remote_ip_addr) == cs.host.s_addr) {
							strncpy(req->userName, cs.uid, sizeof(req->userName));
							strncpy(req->password, cs.pwd, sizeof(req->password));
							req->cookie = cookie;
							nvram_get_r_def("x_SUPER_NAME", super_name, sizeof(super_name), "");
							if (req->userName && !strcmp(req->userName, super_name))
								req->superUser = 1;
							else
								req->superUser = 0;
						}
...
void free_requests(void)
{
	...
	while (ptr != NULL) {
		...
		if (ptr->upload_data) {
#ifdef __DAVO__
			munmap(ptr->upload_data, MAX_FWSIZE);
#endif
