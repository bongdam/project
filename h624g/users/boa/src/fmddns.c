...
void formDdns(request *wp, char *path, char *query)
{
	...
	if (enabled) {
		...
		if (tmpStr[0]) {
			b64_decode(tmpStr, DecodePassword, sizeof(DecodePassword));
			if (apmib_set(MIB_DDNS_PASSWORD, (void *)DecodePassword) == 0) {
	...
	web_config_trace(5, 7);		/* management/ddns */
