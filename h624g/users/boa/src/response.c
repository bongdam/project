...
void send_r_request_ok(request * req)
{
    ...
    if (!req->cgi_type) {
        ...
        if (req->pathname != NULL
            && !strncmp(req->pathname, "/var/tmp/usb", strlen("/var/tmp/usb")))
...
void send_redirect_perm(request * req, const char *url)
{
    ...
    req_write(req, "Content-Type: " HTML CRLF);
	{
		...
		password = req_get_cstream_var(req, "password", "");
		memset(DecodePassword, 0, sizeof(DecodePassword));
		b64_decode(password, DecodePassword, sizeof(DecodePassword));
		cal_sha256(username, sha256_user);
		cal_sha256(DecodePassword, sha256_pass);

		if (username[0] && password[0] && strcmp(req->request_uri, "/boafrm/formLogin")==0) {
			peer.s_addr = inet_addr(req->remote_ip_addr);
			creat_cookie(sha256_user, sha256_pass, peer, buf, sizeof(buf));
			snprintf(cookie, sizeof(cookie), "Set-Cookie: acookie=%s; path=/ \r\n", buf);
			req_write(req, cookie);
		}
		else if (!strcmp(req->request_uri, "/boafrm/formLogout")) {
			req_write(req, "Set-Cookie: acookie=; expires=Thu, 01-Jan-1970 00:00:01 GMT; path=/ \r\n");
...
/* R_MOVED_TEMP: 302 */
void send_r_moved_temp(request * req, const char *url,
                       const char *more_hdr)
{
    SQUASH_KA(req);
