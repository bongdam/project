...
#define WEB_PAGE_LOGIN	"/skb_index.html"
//#define LOGIN_URL	"/skb_login.htm"
...
#define APPLY_REPEATER_REBOOT_COUNTDOWN_TIME 90
#endif
#define APPLY_OK_MSG "<h4>설정 변경 성공<BR>"
#define COUNTDOWN_PAGE "/skb_countDownPage.htm"
...
#define FAIL_TO_LOGIN(msg) { \
	sprintf(okMsg, "%s", msg); \
	send_redirect_perm(wp, "/skb_fail_login.htm"); \
}

#define ERR_MSG(msg) { \
	update_form_hander_name(wp); \
	sprintf(okMsg, "%s", msg); \
	send_redirect_perm(wp, "/skb_err_msg.htm"); \
}
...
#define REBOOT_WAIT(url) { \
	...
	if( need_reboot==1 ) { \
		if (!strcmp(url, "/skb_opeate_mode.htm")) \
			countDownTime = APPLY_REPEATER_REBOOT_COUNTDOWN_TIME; \
		else \
			countDownTime = APPLY_REBOOT_COUNTDOWN_TIME; \
	} else { countDownTime = APPLY_COUNTDOWN_TIME;} \
	send_redirect_perm(wp, COUNTDOWN_PAGE); \
}

#ifdef __DAVO__
#define DO_APPLY_WAIT(url) { \
		sprintf(lastUrl,"%s",url); \
		sprintf(okMsg,"%s","<br><br><b>설정 항목을 적용하고 있습니다. 잠시 기다려주시기 바랍니다.</b>"); \
		countDownTime = 3;\
		send_redirect_perm(wp, COUNTDOWN_PAGE); \
}

#define DO_NTP_WAIT(url) { \
		sprintf(lastUrl,"%s",url); \
		sprintf(okMsg,"%s","<br><br><b>NTP 동기화를 진행 중 입니다. 잠시 기다려주시기 바랍니다.</b>"); \
		countDownTime = 3;\
		send_redirect_perm(wp, COUNTDOWN_PAGE); \
}
#endif
...
#define OK_MSG(url) { \
	needReboot = 1; \
	if(strlen(url) == 0) \
		strcpy(url, "/skb_home.htm"); \
	sprintf(lastUrl, "%s", url); \
	send_redirect_perm(wp, "/skb_okmsg.htm"); \
}
...
#define OK_MSG_FW(msg, url, c, ip) { \
	...
	req_format_write(wp, "if(count == 0) {parent.location.href='http://%s/skb_home.htm?t='+new Date().getTime(); return false;}\n", ip);\
...
#define CONFIRM_MSG(url) { \
	req_format_write(wp, "<html><head>");\
	getIncludeCss(wp);\
   	req_format_write(wp, "</head>\n"); \
	req_format_write(wp, "<meta http-equiv=\"Content-Type\" content=\"text/html\" charset=\"utf-8\"><body><blockquote><h4>변경된 설정이 저장되었습니다. </h4>\n"); \
	req_format_write(wp, "<form><input type=\"button\" OnClick=window.location.replace(\"%s\") \" value=\"&nbsp;&nbsp;OK&nbsp;&nbsp\" name=\"OK\"></form></blockquote></body>",url); \
	req_format_write(wp, "</html>"); \
}
...
#if defined(__DAVO__) //jjh@todo:1
extern void formLdap(request *wp, char *path, char *query);
extern void formRedirWlEncrypt(request *wp, char *path, char *query);
extern void formAutoReboot(request *wp, char *path, char *query);
extern void formWlReset(request *wp, char *path, char *query);
extern int getInfo_formDaa(request *wp, int argc, char **argv);
extern void formDaa(request *wp, char *path, char *query);
extern int wlDetectAvoid(request *wp, char *path, char *query);
extern void formMfgTest(request *wp, char *path, char *query);
extern int autoreboot_status(request *wp, int argc, char **argv);
extern int igmpBlockStatus(request *wp, int argc, char **argv);
extern int igmp_snoop_table(request *wp, int argc, char **argv);
extern int show_acltbl(request *wp, int argc, char **argv);
extern int show_ipv6_information(request *wp, int argc, char **argv);
extern int show_ipv4_information(request *wp, int argc, char **argv);
extern int staticMappingTbl(request *wp, int argc, char **argv);
extern void formStMapping(request *wp, char *path, char *query);
extern void formAutoup(request *wp, char *path, char *query);
extern void formTftpUpload(request *wp, char * path, char * query);
extern void formQosQue(request *wp, char * path, char * query);
extern void formJumbo(request *wp, char *path, char *query);
extern int LanRestrictList(request *wp, int argc, char **argv);
extern void formLanRestrict(request *wp, char *path, char *query);
extern void formBroadcastStormCtrl(request *wp, char *path, char *query);
extern int get_ping_info(request *wp, int argc, char **argv);
extern int igmp_stb_info(request *wp, int argc, char **argv);
extern void formPortMirror(request *wp, char *path, char *query);
extern int get_wlan_traffic(request *wp, int argc, char **argv);
extern void formSNMP(request *wp, char *path, char *query);
extern int wlAcMACList(request *wp, int argc, char **argv);
extern int wlAcCommentList(request *wp, int argc, char **argv);
extern int print_multipleApMenu(request *wp, int argc, char **argv);
extern void formRemark(request *wp, char *path, char *query);
extern void formAclSetup(request *wp, char *path, char *query);
extern void formIgmpSet(request *wp, char *path, char *query);
extern int print_wme_dscp(request *wp, int argc, char **argv);
extern int print_wme_1p(request *wp, int argc, char **argv);
extern void formWlwmm(request *wp, char *path, char *query);
extern void formSetOperation(request *wp, char *path, char *query);
extern int get_connection_info(request *wp, int argc, char **argv);
extern int get_IP_connection_info(request *wp, int argc, char **argv);
extern void formIpConnection(request *wp, char *path, char *query);
extern int netconn_viewer(request *wp, int argc, char **argv);
extern int captcha_img(request *wp, int argc, char **argv);
extern void formwlprodelete(request *wp, char *path, char *query);
extern void formwlprotection(request *wp, char *path, char *query);
extern int macList(request *wp, int argc, char **argv);
extern int sta_protection_list(request *wp, int argc, char **argv);
#endif
...
#ifdef __DAVO__
extern void formHolepunchSet(request *wp, char *path, char *query);
...
extern int getOneDhcpClient(char **ppStart, unsigned long *size, char *ip, char *mac, char *liveTime, char *name);
#endif
