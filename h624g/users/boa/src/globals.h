...
struct user_info{
//	int login_status; //0:logout;1:login;2:forbidden
	char * directory;
	struct user_info * next;
#ifdef ONE_USER_LIMITED
	struct account_status *paccount;
#endif
	long last_time;
	char remote_ip_addr[20];
	char uniq_cookie[1024];
};
#endif
...
extern int log_disabled, log_stderr;
void ylog(int priority, const char *fmt, ...);

#define LOG(prio, fmt, args...) \
	do { \
		if (!log_disabled) \
			ylog(prio, fmt, ## args); \
	} while (0)
#else	/* !DISABLE_WEBLOG_DEBUG */
#define LOG	syslog
