...
static int snmp_wlscan_ontime(int *expect_hour)
{
	time_t t;
	struct tm *now;
...
static void do_wlscan(void)
{
	int i;
	char ifname[16], path[30];
	int status;

	for (i = 0; i < 2; i++ ) {
		ifname[0] = 0;
		sprintf(ifname, "wlan%d", i);
		getWlSiteSurveyRequest(ifname, &status);
		if ( status >= 0 ) {
			sprintf(&path[0], "/var/web_wlscan%d", i);
			yecho(path, "1");
		}
	}
}
...
void loop(int server_s)
{
	...
	time_t snmp_t;
	struct tm *snmp_now;
	int expect_scan_hour = -1;

	snmp_enable = nvram_match("x_SNMP_ENABLE", "1");
#endif
    ...
    while (1) {
#ifdef USE_LOGINWEB_OF_SERVER
        for (pUser_info = user_login_list; pUser_info;
             bUser_info = pUser_info, pUser_info = pUser_info->next) {
...
#endif
            {
                LOG(LOG_INFO, "웹 접속 %s 사용자의 세션 시간이 만료됨",
                       pUser_info->remote_ip_addr);
...
#ifndef __DAVO__
		if (snmp_enable) {
			if ( snmp_wlscan_ontime(&expect_scan_hour) ) {
				if (time(&snmp_t)) {
					snmp_now = localtime(&snmp_t);
					if ( snmp_now->tm_hour == expect_scan_hour) {
						if ( ready_trigger++ == 0) {
							do_wlscan();
							syslog(LOG_INFO, "주기적(%02dH) 무선 스캔 실행", expect_scan_hour );
        ...
        if (dv_reboot_system) {
            ...
            if (confirm_last_req == 3) {
                system("killall -TERM igmpproxy 2>/dev/null");
                //system("killall -USR1 auth 2>/dev/null");
                system("killall -TERM snmp 2>/dev/null");
                system("killall -USR2 udhcpc 2>/dev/null");
            } else if (confirm_last_req > 3) {
                LOG(LOG_INFO, "웹 %s으로 시스템 재부팅함",
                    (dv_reboot_system == 2) ? "명령" : "설정 변경");
                dv_reboot_system = 0;
                confirm_last_req = 0;
                system("reboot");
            }
        ...
        if (dv_script_reboot) {
            confirm_last_script++;
            if (confirm_last_script == 3) {
                run_init_script("all");
                dv_script_reboot = 0;
                confirm_last_script = 0;
            }
        }
#endif
        if (sigterm_flag) {
        ...
        if (max_fd) {
            ...
            if (select(max_fd + 1, BOA_READ, BOA_WRITE, NULL,
      ...
      ToUpgrade:
        ...
        if (isFWUPGRADE != 0 && isREBOOTASP == 1) {
            ...
            if (isFWUpgrade == 1)
                FirmwareUpgrade(FW_Data, FW_Data_Size, 0, buffer);
            else
        ...
        if (current->buffer_end &&      /* there is data to write */
            ...
            case BODY_WRITE:
                ...
                if (FD_ISSET(current->post_data_fd, BOA_WRITE))
                    ...
                    BOA_FD_SET(current, current->post_data_fd, BOA_WRITE);
                }
#else
                ready_request(current);
#endif
            ...
            case PIPE_READ:
                if (FD_ISSET(current->data_fd, BOA_READ))
                    ...
                    BOA_FD_SET(current, current->data_fd, BOA_READ);
                }
                break;
            case DONE:
                if (FD_ISSET(current->fd, BOA_WRITE))
