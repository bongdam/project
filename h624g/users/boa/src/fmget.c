...
extern unsigned long connection_init_time;
...
int getInfo(request *wp, int argc, char **argv)
{
	...
	else if (!strcmp(name, "code_asp")) {
		if (wp->superUser == 1) {
			snprintf(buffer, sizeof(buffer), "skb_navigation.js");
		} else {
			snprintf(buffer, sizeof(buffer), "skb_navigation_admin.js");
		}
		return req_format_write(wp, "%s", buffer);
	}
	...
	else if(!strcmp(name, "wlEnableProfile"))
	{
   	...
   	else if ( !strcmp(name, "login_page_ssid")) {
		if (argc < 2)
			return -1;
...
//printf("\r\n buffer[%s],__[%s-%u]\r\n",buffer,__FILE__,__LINE__);
 ...
 /*skbb dns1 hardcording*/
 	else if ( !strcmp(name, "davo-dns1")) {
		nvram_get_r_def("secret_davo_dns1", buffer, sizeof(buffer), "210.220.163.82");
   		return req_format_write(wp, "%s", buffer);
	}
	...
	else if ( !strcmp(name, "running_dnsmod")) {
		...
		nvram_get_r_def("x_ntp_server_ip1", buffer, sizeof(buffer), "time1.skbroadband.com");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
        }
	else if ( !strcmp(name, "ntpServerIp2")) { // sc_yang
		nvram_get_r_def("x_ntp_server_ip2", buffer, sizeof(buffer), "time2.skbroadband.com");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
        }
	else if ( !strcmp(name, "ntpServerIp3")) { // sc_yang
		nvram_get_r_def("x_ntp_server_ip3", buffer, sizeof(buffer), "time.bora.net");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
        }
	...
	*/
#ifdef __DAVO__
	else if (!strcmp(name, "wan_vlan_pvid")) {
		char tmpBuf[100];
		int  index = 0;
	...
	else if (!strcmp(name, "wan_link_status") ||
		 !strcmp(name, "lan1_link_status") ||
		 !strcmp(name, "lan2_link_status") ||
		 !strcmp(name, "lan3_link_status") ||
		 !strcmp(name, "lan4_link_status") ||
		 !strcmp(name, "wan_linkUp") ||
		 !strcmp(name, "lan1_linkUp") ||
		 !strcmp(name, "lan2_linkUp") ||
		 !strcmp(name, "lan3_linkUp") ||
		 !strcmp(name, "lan4_linkUp"))
	{
		char var[32];
	...
	else if (!strcmp(name, "wan_rx_pause") ||
		 !strcmp(name, "wan_tx_pause") ||
		 !strcmp(name, "lan1_rx_pause") ||
		 !strcmp(name, "lan1_tx_pause") ||
		 !strcmp(name, "lan2_rx_pause") ||
		 !strcmp(name, "lan2_tx_pause") ||
		 !strcmp(name, "lan3_rx_pause") ||
		 !strcmp(name, "lan3_tx_pause") ||
		 !strcmp(name, "lan4_rx_pause") ||
		 !strcmp(name, "lan4_tx_pause"))
	{
		return req_format_write(wp, "%d",
				 pause_status((!strncmp(name, "wan", 3)) ? 4 : (name[3] - '0')-1,
							  !!strstr(name, "tx")));
	}
	else if (!strcmp(name, "wan_link_duplex") ||
			!strcmp(name, "lan1_link_duplex") ||
			!strcmp(name, "lan2_link_duplex") ||
			!strcmp(name, "lan3_link_duplex") ||
			!strcmp(name, "lan4_link_duplex"))
	{
		int duplex = 0;
		int portno = (!strncmp(name, "wan", 3)) ? 4 : (name[3] - '0')-1;
		unsigned int phy_status = switch_port_status(portno);
	...
	else if (!strcmp(name, "igmp_expire_time")){
		nvram_get_r_def("x_igmp_expire_time", buffer, sizeof(buffer), "180");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "igmp_query_interval")){
		nvram_get_r_def("x_igmp_query_interval", buffer, sizeof(buffer), "125");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "igmp_query_res_interval")){
		nvram_get_r_def("x_igmp_query_res_interval", buffer, sizeof(buffer), "5");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "detect_offer") ) { /*  KDH, internet line connection error view added for status page */
		int port = -1;
	...
	else if(!strcmp(name, "cfg_format")) {
		memset(buffer, 0, sizeof(buffer));
		nvram_get_r("FORMAT_VERSION", buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "cfg_file")) {
		memset(buffer, 0, sizeof(buffer));
		nvram_get_r("cfg_filename", buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "ipv6_manual_addr")) {
		nvram_get_r_def("x_ipv6_manual_addr", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "ipv6_manual_prefix_len")) {
		nvram_get_r_def("x_ipv6_manual_prefix_len", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "ipv6_manual_gateway")) {
		nvram_get_r_def("x_ipv6_manual_gateway", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "ipv6_manual_dns1")) {
		nvram_get_r_def("x_ipv6_manual_dns1", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "ipv6_manual_dns2")) {
		nvram_get_r_def("x_ipv6_manual_dns2", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "ip_conflict")) {
		int ip_conflict = 0;

		sprintf(buffer, "<font size='2' color='green'>없음</font>");
		fp = fopen("/proc/net/ip_conflict", "r");
		if (fp) {
			fscanf(fp, "%d\n", &ip_conflict);
			if (ip_conflict == 1) {
	...
	else if ( !strcmp(name, "PING_TEST_RESULT")) {
		nvram_get_r_def("x_PING_TEST_RESULT", buffer, sizeof(buffer), "");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "igmp_querier_auto")) {
		nvram_get_r_def("x_igmp_querier_auto", buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}
	...
	else if ( !strcmp(name, "igmp_querier_interval") ) {
		nvram_get_r_def("x_igmp_querier_interval", buffer, sizeof(buffer), "125");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "dv_igmp_limite_lan1") ) {
		nvram_get_r_def("x_igmp_limite_lan1", buffer, sizeof(buffer), "32");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "dv_igmp_limite_lan2") ) {
		nvram_get_r_def("x_igmp_limite_lan2", buffer, sizeof(buffer), "32");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "dv_igmp_limite_lan3") ) {
		nvram_get_r_def("x_igmp_limite_lan3", buffer, sizeof(buffer), "32");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "dv_igmp_limite_lan4") ) {
		nvram_get_r_def("x_igmp_limite_lan4", buffer, sizeof(buffer), "32");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "dv_igmp_limite_sys") ) {
		nvram_get_r_def("x_igmp_limite_sys", buffer, sizeof(buffer), "128");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "igmp_block_enabled") ) {
		if( handle_igmp_block_table(0, name) )
		...
		sprintf(buffer, "%d", handle_igmp_block_table(1, name));
		return req_format_write(wp, buffer);
	}
	else if ( !strcmp(name, "igmp_block_period_value") ) {
		sprintf(buffer, "%d", handle_igmp_block_table(1, name));
	...
	else if ( !strcmp(name, "igmp_grpmem_interval")) {
		nvram_get_r_def("x_igmp_grpmem_interval", buffer, sizeof(buffer), "60");
		return req_format_write(wp, "%s", buffer);
	}
#endif
	...
	/* Advance setting stuffs */
		...
		get_performance(buffer, 0);
   		return req_format_write(wp, buffer);
	}
	else if ( !strcmp(name, "ram_usage")) {
		get_performance(buffer, 1);
   		return req_format_write(wp, buffer);
	}
	// added by rock /////////////////////////////////////////
	...
	else if ( !strcmp(name, "lanTxDataBytes")) {
#ifdef __DAVO__
		char str_byte[128];
		int i;
		unsigned long long lan_tx_bytes = 0, total_tx_bytes = 0;

		for(i = 0; i < 4; i++) {
			if ( getPortStats(i, str_byte)) {
				sscanf(str_byte, "%*s %*s %llu %*s", &lan_tx_bytes);
				total_tx_bytes += lan_tx_bytes;
			}
		}

		return req_format_write(wp, "%u, %u", (unsigned int)(total_tx_bytes >> 32), (unsigned int)total_tx_bytes);
#else
	...
	else if ( !strcmp(name, "lanRxDataBytes")) {
#ifdef __DAVO__
		char str_byte[128];
		int i;
		unsigned long long lan_rx_bytes = 0, total_rx_bytes = 0;

		for(i = 0; i < 4; i++) {
			if ( getPortStats(i, str_byte)) {
				sscanf(str_byte, "%*s %*s %*s %llu", &lan_rx_bytes);
				total_rx_bytes += lan_rx_bytes;
			}
		}

		return req_format_write(wp, "%u, %u", (unsigned int)(total_rx_bytes >> 32), (unsigned int)total_rx_bytes);
#else
		if ( getStats(ELAN_IF, &stats) < 0)
			stats.rx_bytes = 0;
	...
	else if ( !strcmp(name, "lan2TxDataBytes")) {
	...
	else if ( !strcmp(name, "lan2RxDataBytes")) {
#if defined(VLAN_CONFIG_SUPPORTED)
		if ( getStats(ELAN2_IF, &stats) < 0)
			stats.rx_bytes = 0;
		return req_format_write(wp, "%u, %u", (unsigned int)(stats.rx_bytes >> 32), (unsigned int)stats.rx_bytes);
#else
		return req_format_write(wp, "0, 0");
#endif
	}
	else if ( !strcmp(name, "lan3TxDataBytes")) {
#if defined(VLAN_CONFIG_SUPPORTED)
	...
	else if ( !strcmp(name, "lan3RxDataBytes")) {
#if defined(VLAN_CONFIG_SUPPORTED)
		if ( getStats(ELAN3_IF, &stats) < 0)
			stats.rx_bytes = 0;
		return req_format_write(wp, "%u, %u", (unsigned int)(stats.rx_bytes >> 32), (unsigned int)stats.rx_bytes);
	...
	else if ( !strcmp(name, "lan4TxDataBytes")) {
#if defined(VLAN_CONFIG_SUPPORTED)
	...
	else if ( !strcmp(name, "lan4RxDataBytes")) {
#if defined(VLAN_CONFIG_SUPPORTED)
		if ( getStats(ELAN4_IF, &stats) < 0)
			stats.rx_bytes = 0;
		return req_format_write(wp, "%u, %u", (unsigned int)(stats.rx_bytes >> 32), (unsigned int)stats.rx_bytes);
	...
	else if ( !strcmp(name, "wlanTxPacketBytes")) {
	...
	else if(!strncmp(name,"USB3G",strlen("USB3G")))
	{
		...
		return req_format_write(wp,"");
#elif defined(CONFIG_RTL_ULINKER)
		return req_format_write(wp, "menu.addItem('ULinker Operation Mode', 'skb_ulinker_opmode.htm', '', 'ULinker Operation Mode');" );
#else
		return req_format_write(wp, "menu.addItem('Operation Mode', 'skb_opmode.htm', '', 'Operation Mode');" );
#endif
	}
	else if(!strcmp(name, "qos_root_menu")) {

#if defined(GW_QOS_ENGINE) && !defined(VOIP_SUPPORT)
		return req_format_write(wp, "menu.addItem('QoS', 'skb_qos.htm', '', 'Setup QoS');" );
#elif defined(QOS_BY_BANDWIDTH) && !defined(VOIP_SUPPORT)
	#ifdef CONFIG_IPV6
		return req_format_write(wp, "menu.addItem('QoS', 'skb_ip6_qos.htm', '', 'Setup QoS');" );
	#else
		return req_format_write(wp, "menu.addItem('QoS', 'skb_ip_qos.htm', '', 'Setup QoS');" );
	#endif
#else
		return req_format_write(wp,"");
#endif
	...
	else if(!strcmp(name, "route_menu_onoff"))
	{
		...
		return req_format_write(wp, "menu.addItem(\"Route Setup\", \"skb_route.htm\", \"\", \"Route Setup\");");
#else
		return req_format_write(wp,"");
#endif
	}
	else if(!strcmp(name, "ethdot1x_menu_onoff"))
	{
		...
		return req_format_write(wp, "menu.addItem(\"Ethernet 802.1x Setup\", \"skb_eth_dot1x.htm\", \"\", \"Ethernet 802.1x Setup\");");
#else
		return req_format_write(wp,"");
#endif
	}
	else if(!strcmp(name,"snmp_menu"))
	{
		...
		return req_format_write(wp, "menu.addItem(\"SNMP\", \"skb_snmp.htm\", \"\", \"SNMP Setup\");");
#else
		return req_format_write(wp,"");
#endif
	}
	else if(!strcmp(name,"ddns_menu"))
	{
		...
		return req_format_write(wp, "manage.addItem(\"DDNS\", \"skb_ddns.htm\", \"\", \"Setup Dynamic DNS\");");
#else
		return req_format_write(wp,"");
#endif
	}
	...
	if (repeater1==1 || repeater2==1) {
		...
		i = open("/proc/dvflag", O_RDONLY);
		if (i > -1) {
			read(i, (void *)&intVal, sizeof(intVal));
			close(i);
		}
		wlclnt_up = test_all_bits(DF_WLCLNT_UP, intVal);

		if (!wlclnt_up) {
			return req_format_write(wp, "Repeater 모드 : 연결 실패");
		} else {
			return req_format_write(wp, "Repeater 모드 : 사용 성공");
		}
	}
	if (opmode == BRIDGE_MODE)
	...
	else if ( !strcmp(name, "wan-ip"))
  {
	...
	else if ( !strcmp(name, "wanTxDataBytes")) {
#ifdef __DAVO__
		char str_byte[128];
		unsigned long long lan_tx_bytes = 0, total_tx_bytes = 0;

		if ( getPortStats(4, str_byte)) {
			sscanf(str_byte, "%*s %*s %llu %*s", &lan_tx_bytes);
			total_tx_bytes += lan_tx_bytes;
		}
	...
	else if ( !strcmp(name, "wanRxDataBytes")) {
#ifdef __DAVO__
		char str_byte[128];
		unsigned long long lan_rx_bytes = 0, total_rx_bytes = 0;
	...
	else if(!strcmp(argv[0],"rsCertInstall"))
	{
		...
		req_format_write(wp,"wlan0.addItem(\"802.1x Cert Install\", get_form(\"skb_rsCertInstall.htm\",i), \"\", \"Install 802.1x certificates\");");
#endif
		return 0;
	}
...
//		int i;
	...
	else if(!strcmp(name, "ethernet802dot1xCert_menu"))
	{
		...
		return req_format_write(wp,"manage.addItem(\"Ethernet 802.1x Cert Install\", get_form(\"skb_ethdot1xCertInstall.htm\",i), \"\", \"Install Ethernet 802.1x certificates\");");
#else
		return req_format_write(wp,"");
#endif
	}
	...
	{
#ifdef CONFIG_IPV6
        return req_format_write(wp, "firewall.addItem('IP Filtering', 'skb_ip6filter.htm', '', 'Setup IP filering');" );
#else
		return req_format_write(wp, "firewall.addItem('IP Filtering', 'skb_ipfilter.htm', '', 'Setup IP filering');" );
#endif
    }
	else if(!strcmp(name, "port_filter"))
	{
#ifdef CONFIG_IPV6
        return req_format_write(wp, "firewall.addItem('Port Filtering', 'skb_portfilter6.htm', '', 'Setup port filer');" );
#else
		return req_format_write(wp, "firewall.addItem('Port Filtering', 'skb_portfilter.htm', '', 'Setup port filer');" );
#endif
    }
	...
	{
#if defined(VLAN_CONFIG_SUPPORTED)
		return req_format_write(wp, "firewall.addItem('VLAN', 'skb_vlan.htm', '', 'Setup VLAN');" );
#else
		return req_format_write(wp,"");
#endif
}
    else if(!strcmp(name, "vlan_wan_menu_onoff"))
    {
        ...
        return req_format_write(wp, "firewall.addItem('VLAN_WAN', 'skb_vlan_wan.htm', '', 'Setup VLAN WAN TAG');" );
#else
        return req_format_write(wp,"");
#endif
    }
	...
	else if(!strcmp(name, "vlan_wan_wifi_vap3_pri"))
	{
	  ...
	  sprintf(buffer, "%d", intVal );
		...
		{
			return req_format_write(wp, "menu.addItem('VLAN', 'skb_vlan.htm', '', 'Setup VLAN');" );
		}
		else if(!strcmp(name, "maxWebVlanNum"))
		{
#if defined(CONFIG_RTL_8198_AP_ROOT) && defined(GMII_ENABLED)
	...
	else if (!strcmp(name, "OPTION82")) {
		char *opt82=NULL;
		opt82 = nvram_get("OPTION82");
		if (opt82 == NULL)
			return -1;
		return req_format_write(wp, opt82);
	}
#if defined(CONFIG_SNMP)
...
/*+++++added by Jack for TR-069 configuration+++++*/
		...
		return req_format_write(wp,
				"menu.addItem('TR-069');" \
				"tr069 = new MTMenu();" \
				"tr069.addItem('TR-069 config', 'skb_tr069config.htm', '', 'Setup TR-069 configuration');" \
				"menu.makeLastSubmenu(tr069);");
#else
		return req_format_write(wp, "manage.addItem('TR-069 config', 'skb_tr069config.htm', '', 'Setup TR-069 configuration');" );
#endif
...
//		req_format_write(wp,"<tr><hr size=1 noshade align=top></tr>");
	...
	else if(!strcmp(argv[0],"btfiles"))
	{
	 ...
	 {
	 	req_format_write(wp, "%s","manage.addItem(\"BT Client\", \"skb_bt.htm\", \"\", \"BT Client\");");
		return 0;
	 }
	...
	{
		req_format_write(wp, "%s","manage.addItem(\"BT Client\", \"skb_transmission.htm\", \"\", \"BT Client\");");
		return 0;
	}
#else
	else if(!strcmp(argv[0],"rtl_trans_bt_menu"))
	...
	else if ( !strcmp(name, "wlan_bandMode_menu_onoff"))
	{
		...
		return req_format_write(wp, "wlan.addItem('BandMode', 'skb_wlbandmode.htm', '', 'Setup WLAN Band Mode');" );
#else
		return req_format_write(wp,"");
#endif
	}
	...
	else if(!strcmp(argv[0],"onoff_dmdphy_comment_end"))
	{
		...
		req_format_write(wp, "skb_http_files.htm");
#else
		req_format_write(wp, "skb_home.htm");
#endif
		return 0;
	}
...
#ifdef __DAVO__
	else if(!strcmp(argv[0],"auto_upgrade_info"))
	{
		intVal = 0;
		i = open("/proc/dvflag", O_RDONLY);
		if (i > -1) {
		...
		req_format_write(wp,"%d", test_all_bits(DF_UPLOADING, intVal));
		return 0;
	}
	else if ( !strcmp(argv[0], "local_connection") ) {
		long WanIP_long=0;
		char wan_ip[32];
...
// add for ds-lite
...
#endif
	else if (!strcmp(name, "local_linux_time")) {
		time_t now;
		now = time(NULL);
		return req_format_write(wp, "%ld000", now);
	} else if (!strcmp(name, "devname")) {
		get_modelname(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_auto_reboot_on_idle"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_auto_wan_port_idle"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_auto_uptime"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "7d");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_auto_bw_kbps"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "1000");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_auto_hour_range"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "03:00-04:00");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_autoreboot_userforce"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_autoreboot_week"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "4-4");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_auto_reboot_enable"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_telnet_enable"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_holepunch_enabled"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_holepunch_server"))
	{
		nvram_get_r_def("x_holepunch_cserver", buffer, sizeof(buffer), "aphp.skbroadband.com");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_holepunch_port"))
	{
		nvram_get_r_def("x_holepunch_cport", buffer, sizeof(buffer), "10219");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_mac_clone_enable"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "wlan_max_conn"))
	{
	...
	else if(!strcmp(name, "wlan_va0_max_conn"))
	{
		get_wlan_name(wlan_name, 1, "max_conn");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "127");

		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "wlan_va1_max_conn"))
	{
		get_wlan_name(wlan_name, 2, "max_conn");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "127");

		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "wlan_va2_max_conn"))
	{
		get_wlan_name(wlan_name, 3, "max_conn");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "127");

		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "wlan_va3_max_conn"))
	{
		get_wlan_name(wlan_name, 4, "max_conn");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "127");
	...
	else if(!strcmp(name, "wlan_va1_rssi_threshold"))
	{
		get_wlan_name(wlan_name, 2, "rssi_threshold");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "75");

		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "wlan_va2_rssi_threshold"))
	{
		get_wlan_name(wlan_name, 3, "rssi_threshold");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "0");

		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "wlan_va3_rssi_threshold"))
	{
		get_wlan_name(wlan_name, 4, "rssi_threshold");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "0");

		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_MACFILTER_TBL_NUM"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_ldap_enabled"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_ldap_autoup_enabled"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_autoup_auth_svr"))
	{
		nvram_get_r(name, buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_ldap_upgrade_server"))
	{
		nvram_get_r("x_ldap_autoup_domain", buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_ldap_autoup_file"))
	{
		memset(buffer, 0, sizeof(buffer));
		nvram_get_r_def(name, buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "prefix_used"))
	{
		int prefix_use;
	...
	else if(!strcmp(name, "ldap_prefix"))
	{
		memset(buffer, 0, sizeof(buffer));
		nvram_get_r("x_ldap_autoup_prefix", buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	...
	else if(!strcmp(name, "x_WLS_REDIR_HOST"))
	{
	...
	else if(!strcmp(name, "x_WLS_REDIR_ALLOW0"))
	{
		get_wlan_name(wlan_name, 0, "WLS_REDIR_ALLOW0");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_WLS_REDIR_ALLOW1"))
	{
		get_wlan_name(wlan_name, 0, "WLS_REDIR_ALLOW1");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_WLS_REDIR_ALLOW2"))
	{
		get_wlan_name(wlan_name, 0, "WLS_REDIR_ALLOW2");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_WLS_REDIR_ALLOW3"))
	{
		get_wlan_name(wlan_name, 0, "WLS_REDIR_ALLOW3");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_WLS_REDIR_ALLOW4"))
	{
		get_wlan_name(wlan_name, 0, "WLS_REDIR_ALLOW4");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}

	else if(!strcmp(name, "wl_reset_enable"))
	{
		nvram_get_r_def("x_wlan_reset_enable", buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
	}

	else if(!strcmp(name, "wl_reset_interval_day"))
	{
		nvram_get_r_def("x_wlan_reset_interval_day", buffer, sizeof(buffer), "8");
		return req_format_write(wp, "%s", buffer);
	}

	else if(!strcmp(name, "wl_reset_bw_kbps"))
	{
		nvram_get_r_def("x_wlan_reset_bw_kbps", buffer, sizeof(buffer), "500");
		return req_format_write(wp, "%s", buffer);
	}

	else if(!strcmp(name, "wl_reset_triger_time"))
	{
		nvram_get_r_def("x_wlan_reset_triger_time", buffer, sizeof(buffer), "03:10-05:00");
		return req_format_write(wp, "%s", buffer);
	}
...
#ifdef __DAVO__
	else if (!strcmp(name, "vap3_disabled"))
	{
		nvram_get_r("WLAN0_VAP3_WLAN_DISABLED", buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_autoup_domain"))
	{
		nvram_get_r(name, buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_autoup_file"))
	{
		nvram_get_r(name, buffer, sizeof(buffer));
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_autoup_prefix"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
	else if ( !strcmp(name, "sdmzHost")) {
		nvram_get_r_def("x_SDMZ_HOST", buffer, sizeof(buffer), "");
		return req_format_write(wp, "%s", buffer);
	}
		...
		for (i=0;i<5;i++) {
			sprintf(name, "x_QOS_ENABLE_%d", i);
			nvram_get_r_def(name, buffer, sizeof(buffer), "0");
			req_format_write(wp, "q_enable[%d]=%s;\n", i, buffer);
			sprintf(name, "x_QOS_RATE_ENABLE_%d", i);
			nvram_get_r_def(name, buffer, sizeof(buffer), "0");
			req_format_write(wp, "r_enable[%d]=%s;\n", i, buffer);
			sprintf(name, "x_QOS_RATE_I_%d", i);
			nvram_get_r_def(name, buffer, sizeof(buffer), "0");
			req_format_write(wp, "q_inrate[%d]=%s;\n", i, buffer);
			sprintf(name, "x_QOS_RATE_O_%d", i);
			nvram_get_r_def(name, buffer, sizeof(buffer), "0");
			req_format_write(wp, "q_outrate[%d]=%s;\n", i, buffer);
			for (j=0;j<4;j++) {
				char *p;
				sprintf(name, "x_QOS_Q_%d_%d", i, j);
				nvram_get_r_def(name, buffer, sizeof(buffer), "S_0_1");
	...
	else if(!strcmp(name, "x_bs_rssi_th"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "80");
		return req_format_write(wp, "%d", atoi(buffer)*(-1));
	}
    else if(!strcmp(name, "x_bs_probe_deny_rssi_th"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "70");
		return req_format_write(wp, "%d", atoi(buffer)*(-1));
	}
    else if(!strcmp(name, "x_bs_tcp_pps_check_time"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "20");
		return req_format_write(wp, "%d", atoi(buffer));
	}
    else if(!strcmp(name, "x_bs_tcp_pkts_threshold"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "2");
		return req_format_write(wp, "%d", atoi(buffer));
	}


/*
	else if(!strcmp(name, "x_wlan_cca_mode"))
	{
		get_wlan_name(wlan_name, 0, "CCA_MODE");
		nvram_get_r_def(wlan_name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
*/
	else if(!strcmp(name, "jumbo_enable"))
	{
		nvram_get_r_def("x_jumbo_enable", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "jumbo_size"))
	{
		nvram_get_r_def("x_jumbo_size", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "repeater_interface"))
	{
		nvram_get_r_def("REPEATER_ENABLED2", buffer, sizeof(buffer), "0");
		if (atoi(buffer) == 1) {
	...
	else if(!strcmp(name, "REPEATER_ENABLED1"))
	{
		nvram_get_r_def("REPEATER_ENABLED1", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "REPEATER_ENABLED2"))
	{
		nvram_get_r_def("REPEATER_ENABLED2", buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "REPEATER_SSID1"))
	{
		nvram_get_r_def("REPEATER_SSID1", buffer, sizeof(buffer), "0");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "REPEATER_SSID2"))
	{
		nvram_get_r_def("REPEATER_SSID2", buffer, sizeof(buffer), "0");
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_BCSTORM_CTRL_ENABLE"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
   }
	else if(!strcmp(name, "x_BCSTORM_CTRL_PERCENT"))
   {
		nvram_get_r_def(name, buffer, sizeof(buffer), "1");
		return req_format_write(wp, "%s", buffer);
   }
	else if(!strcmp(name, "x_BCSTORM_CTRL_BPS"))
   {
		nvram_get_r_def(name, buffer, sizeof(buffer), "3036");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_BCSTORM_PORT0_ENABLE"))
	{
	nvram_get_r_def(name, buffer, sizeof(buffer), "0");
	return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_BCSTORM_PORT1_ENABLE"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_BCSTORM_PORT2_ENABLE"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_BCSTORM_PORT3_ENABLE"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name, "x_BCSTORM_PORT4_ENABLE"))
	{
		nvram_get_r_def(name, buffer, sizeof(buffer), "0");
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strncmp(name, "dvport_", 7))
	{
		buffer[0]=0;
		return handle_dvport_mirror(wp, buffer, name);
	}
    else if ( !strcmp(name, ("pingSecCount"))) {
		nvram_get_r_def("x_icmp_reply_rate", buffer, sizeof(buffer), 0);
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "snmp_input_rate"))
	{
		nvram_get_r_def("x_snmp_input_rate", buffer, sizeof(buffer), 0);
		return req_format_write(wp, "%s", buffer);
	}
	else if (!strcmp(name, "snmp_com1_check"))
	{
	...
	else if (!strcmp(name, "SNMP_GET_COMMUNITY"))
	{
		nvram_get_r_def("x_SNMP_GET_COMMUNITY", buffer, sizeof(buffer), "iptvshro^_");
		return req_format_write(wp, buffer);
	}
	else if (!strcmp(name, "SNMP_SET_COMMUNITY"))
	{
		nvram_get_r_def("x_SNMP_SET_COMMUNITY", buffer, sizeof(buffer), "iptvshrw^_");
		return req_format_write(wp, buffer);
	}
	else if (!strcmp(name, "snmp_trap_server"))
	{
		nvram_get_r_def("x_SNMP_TRAP_SERVER", buffer, sizeof(buffer), "iptvsh-trap.skbroadband.com");
		return req_format_write(wp, buffer);
	}
	else if (!strcmp(name, "snmp_trap_server2"))
	{
		nvram_get_r_def("x_WIFI_TRAP_SERVER", buffer, sizeof(buffer), "iptvap-trap.skbroadband.com");
		return req_format_write(wp, buffer);
	}
	else if (!strcmp(name, "SNMP_TRP_COMMUNITY"))
	{
		nvram_get_r_def("x_SNMP_TRAP_COMMUNITY", buffer, sizeof(buffer), "iptvshrw^_");
		return req_format_write(wp, buffer);
	}
	else if (!strcmp(name, "dv_port_mirror"))
	{
	...
	else if (!strcmp(name, "qos_remark_js"))
	{
		char *p;
		int pbs, i;
	...
	else if (!strcmp(name, "connection_init_time"))
	{
		struct sysinfo info ;
		...
		req_format_write(wp, "%lu", sec-connection_init_time);
	}
	else if ( !strcmp(name, "get_wan_mtp"))
	{
		unsigned long long wan_mtp = 0;
		unsigned long wan_mtp_low = 0;
		unsigned long wan_mtp_high = 0;
		...
		if (strcmp(argv[1], "wan_rx") == 0) {
			sprintf(path, "/proc/rx_hist_peak");
		} else if (strcmp(argv[1], "wan_tx") == 0) {
			sprintf(path, "/proc/tx_hist_peak");
		}

		if (access(path, F_OK)!=0)
			return req_format_write(wp, "0,0");

		yfcat(path, "%*s %*s %*s %*s %llu", &wan_mtp);
		wan_mtp = (wan_mtp*8);

		wan_mtp_high = (unsigned long)(wan_mtp >> 32);
		wan_mtp_low = (unsigned long)wan_mtp;

		return req_format_write(wp, "%lu,%lu", wan_mtp_high, wan_mtp_low);
	}
	else if ( !strcmp(name, "repeater_fail"))
	{
		int repeater_enable1=0, repeater_enable2=0;
		int repeater_fail=0;
		int	intVal = 0;
		int i;
		int wan_bound=0;
...
/* 16:not wan bound (wpa, disabled mode) */
		...
		wan_bound = test_all_bits(DF_WANBOUND, intVal);

		nvram_get_r_def("REPEATER_ENABLED1", buffer, sizeof(buffer), "0");
		repeater_enable1 = atoi(buffer);
		nvram_get_r_def("REPEATER_ENABLED2", buffer, sizeof(buffer), "0");
		repeater_enable2 = atoi(buffer);


		char str_repeater_fail[64];
		char str_encrypt[32];

		sprintf(str_repeater_fail, "/proc/wlan%d-vxd/repeater_state", (repeater_enable2==1)?1:0);
		sprintf(str_encrypt, "WLAN%d_VAP4_ENCRYPT", (repeater_enable2==1)?1:0);

		yfcat(str_repeater_fail, "%d", &repeater_fail);

		if (repeater_fail>0 || !wan_bound) {

			if (repeater_fail==0) {
				nvram_get_r_def(str_encrypt, buffer, sizeof(buffer), "0");
				encrypt = atoi(buffer);
				if (encrypt == 1) {		//wep mode
					repeater_fail = 8;
				} else {
					repeater_fail = 16;	//wpa mode-not get dhcp ip
				}
			}
			return req_format_write(wp, "%d", repeater_fail);
		} else {
			return req_format_write(wp, "0");
		}

	}
	else if ( !strcmp(name, "repeater_wep_key_len")) {

		char wep_key_name[64];

		if (argc < 4)
			return 0;

		sprintf(wep_key_name, "WLAN%d_VAP4_WEP%d_KEY%d", atoi(argv[1]), atoi(argv[2]), atoi(argv[3]));

		nvram_get_r_def(wep_key_name, buffer, sizeof(buffer), "");
		return req_format_write(wp, "%d", strlen(buffer));

	}
	else if ( !strcmp(name, "repeater_psk_len")) {
		char psk_name[64];

		if (argc < 2)
			return 0;

		sprintf(psk_name, "WLAN%d_VAP4_WPA_PSK", atoi(argv[1]));

		nvram_get_r_def(psk_name, buffer, sizeof(buffer), "");
		return req_format_write(wp, "%d", strlen(buffer));
		...
		nvram_get_r_def(argv[1], buffer, sizeof(buffer), "");
		if (strlen(buffer)==0) {
			return 0;
		}
	...
	else if ( !strncmp(name, "crc_lan", strlen("crc_lan")) ) {
		int port = name[7]-'0';

		if ( get_asicinfo(RX_ASIC, port, FCS_PAUSE_ASIC, buffer, sizeof(buffer)) )
			return req_format_write(wp, buffer);
		return req_format_write(wp, "0");
	}
#endif
...
*/
	...
	else if ( !strcmp(name, "handoverDisabled")) {
		nvram_get_r_def("WLAN0_VAP3_WLAN_DISABLED", buffer, sizeof(buffer), "0");
		req_format_write(wp, buffer);
		return 0;
	}
...
//		char tmpStr[10];
    ...
    else if ( !strcmp(name, "ARPspoofEnabled")) {
        nvram_get_r_def("x_ARP_DEFENDER_ENABLE", buffer, sizeof(buffer), "0");
		req_format_write(wp, buffer);
		return 0;
	}
    else if ( !strcmp(name, "TraceRtEnabled")) {
        nvram_get_r_def("x_noreply_tracert", buffer, sizeof(buffer), "0");
		req_format_write(wp, buffer);
		return 0;
	}
    else if ( !strcmp(name, "pingSecEnabled")) {
        nvram_get_r_def("x_pingSecEnabled", buffer, sizeof(buffer), "0");
		req_format_write(wp, buffer);
		return 0;
	}
    else if ( !strcmp(name, "NTPDefEnabled")) {
        nvram_get_r_def("x_NTPDefEnabled", buffer, sizeof(buffer), "0");
		req_format_write(wp, buffer);
		return 0;
	}
    else if ( !strcmp(name, "DNSRelayEnabled")) {
        nvram_get_r_def("x_DNSRelayEnabled", buffer, sizeof(buffer), "0");
		req_format_write(wp, buffer);
		return 0;
	}
#endif
...
//for 11N
		...
		sprintf(tmpStr, "x_wlan%d_auto_bonding", wlan_idx);
		wl_autowidth = nvram_atoi(tmpStr, (wlan_idx)?1: 0);
		val = ((wl_autowidth)?(val|0x8):(val));
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "get_bandwidth")) {
		val = autochan_get_bandwidth();
		sprintf(buffer, "%d", val);
	...
	else if (!strcmp(name, "get_controlsideband")) {
		val = get_controlsideband();
		sprintf(buffer, "%d", val);
...
//### end
    ...
    else if(!strcmp(argv[0], "dsf_enable"))
    {
			...
			nvram_get_r_def("x_dfs_enable", buffer, sizeof(buffer), "0");
#else
			sprintf(buffer, "%d", 0);
#endif
...
//			printf("Warning: cannot open /proc/cpuinfo\n");
...
#endif
#ifdef __DAVO__
	else if(!strcmp(name, "swms_enable"))
	{
		char *autoup=NULL;
		sprintf(buffer, "%d", 1);
	...
	else if (!strcmp(name, "staticDhcpNum")) {
	...
	else if ( !strcmp(name, "ipv6_autoconfig_method")) {
		char ipv6_auto_method[12];
		nvram_get_r_def("x_ipv6_autoconfig_method", ipv6_auto_method, sizeof(ipv6_auto_method), "0");
		sprintf(buffer, "%d", atoi(ipv6_auto_method));
		req_format_write(wp, buffer);
		return 0;
	}
	else if ( !strcmp(name, "ipv6_dns_method")) {
		char ipv6_dns_method[12];
		nvram_get_r_def("x_ipv6_dns_method", ipv6_dns_method, sizeof(ipv6_dns_method), "0");
		sprintf(buffer, "%d", atoi(ipv6_dns_method));
		req_format_write(wp, buffer);
		return 0;
	}
	else if ( !strcmp(name, "stMappingNum") ) {
		char staticMapcnt[12];
		nvram_get_r_def("x_STATICMAP_TBL_NUM", staticMapcnt, sizeof(staticMapcnt), "0");
		sprintf(buffer, "%d", atoi(staticMapcnt));
		req_format_write(wp, buffer);
		return 0;
	}
	else if ( !strcmp(name, "portFwNum") ) {
	...
	else if ( !strcmp(name, "dvnv_fast_igmp") ) {
		int intValue =0;
		apmib_get(MIB_IGMP_FAST_LEAVE_DISABLED, (void *)&intValue);
		if(intValue)
			sprintf(buffer, "%d", 0);
...
#endif
#ifdef __DAVO__
	else if (!strcmp(name, "macfil_active_port")) {
		sprintf(buffer, "%d", get_macfilter_active_port());
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "isAdmin")) {
		char superName[65];
	...
	else if( !strcmp(name, "sdmzEnabled")) {
		int sdmz_val;

		sdmz_val = atoi(nvram_get("x_SDMZ_ENABLED"));
		sprintf(buffer, "%d", sdmz_val);
		req_format_write(wp, buffer);
	}
	else if ( !strcmp(name, "UseAutoup") ) {
	...
	else if ( !strcmp(name, ("input_policy_accept"))) {
		val = atoi(nvram_get("x_input_policy_accept"));
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
	}
	else if ( !strcmp(name, ("snmp_trap_enable"))) {
	...
	else if ( !strcmp(name, "wlprotection_num")) {
		nvram_get_r_def("sta_protection_num", buffer, sizeof(buffer), "0");
		req_format_write(wp, buffer);
		return 0;
	}
	else if ( !strcmp(name, "ntp_fail_flag")) {
		int ntp_sync = 0;
		...
		req_format_write(wp, "%d", ntp_sync);
		return 0;
	}
#endif
	else
...
int get_wlan_traffic(request *wp, int argc, char **argv)
{
	...
	char buf[80], *args[4];
	char *p = "0", *q = "0";
	unsigned long bytes, bytes_high;
		...
		if (f != NULL) {
			while (fgets(buf, sizeof(buf), f)) {
				if (ystrargs(buf, args, _countof(args), ":", 0) &&
					...
					ystrargs(buf, args, _countof(args), ":", 0);
					q = args[1] ? : "0";
					bytes_high = strtoul(q, NULL, 0);
					break;
				}
			}
			fclose(f);
	...
	return req_format_write(wp, "%lu, %lu", bytes_high, bytes);
}

/* APACRTL-92 smlee  */
#define CONNTRACK_INFO	"/tmp/conntrack_info"
int get_connection_info(request *wp, int argc, char **argv)
{
	FILE *f, *fp;
	char buf[512], buf2[256];
	char *proto, *p;
	int all_connection = 0;
	int tcp_connection = 0;
	int udp_connection = 0;
	int icmp_connection = 0;
	int other_connection = 0;
	...
	unsigned long long init_tx_p=0, init_rx_p=0, init_tx_bytes=0, init_rx_bytes=0;
	char str_traffic[128];

	if (access(CONNTRACK_INFO, F_OK) == 0)
		unlink(CONNTRACK_INFO);

	system("cat /proc/net/ip_conntrack > " CONNTRACK_INFO);
	f = fopen(CONNTRACK_INFO, "r");
	...
	if (getPortStats(4, str_traffic))
	{
		sscanf(str_traffic, "%llu %llu %llu %llu", &tx_p, &rx_p, &tx_bytes, &rx_bytes);
	}

	fp = fopen(ALL_CONNECTION_INFO, "r");
	if (fp !=NULL) {
		fgets(buf2, sizeof(buf2), fp);
		sscanf(buf2, "%llu %llu %llu %llu", &init_tx_p, &init_rx_p, &init_tx_bytes, &init_rx_bytes);
	...
	tx_bytes_only_high = (unsigned long)(tx_bytes >> 32);
	tx_bytes_only = (unsigned long)tx_bytes;
	rx_bytes_only_high = (unsigned long)(rx_bytes >> 32);
	rx_bytes_only = (unsigned long)rx_bytes;


	return req_format_write(wp, "<tr>\n"
									"<td align=center>%d</td>\n"
									"<td align=center>%d</td>\n"
									"<td align=center>%d</td>\n"
									"<td align=center>%d</td>\n"
									"<td align=center>%d</td>\n"
									"<td align=center><script>var packet=%llu;document.write(packet.toLocaleString());</script></td>\n"
									"<td align=center><script>var packet=%llu;document.write(packet.toLocaleString());</script></td>\n"
									"<td align=center><script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n"
									"<td align=center><script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n"
								"</tr>\n",
			all_connection, tcp_connection, udp_connection, icmp_connection, other_connection,
			tx_p, rx_p, tx_bytes_only_high, tx_bytes_only, rx_bytes_only_high, rx_bytes_only);
}
/* APACRTL-92 smlee  */
int get_IP_HostName(char *client_ip, char *hostname)
{
	...
	while (1) {
		...
		entry = *((struct dhcpOfferedAddr *)ptr);
		ptr = ptr + sizeof(entry);
		fileSize = fileSize - sizeof(entry);
		...
		snprintf(hostname, 64, "%s", entry.hostname);
		break;
	}
	if (buf)
		free(buf);
...
/* APACRTL-92 smlee  */
static int get_ip_wlan_traffic(char *mac_str, char *wlan_if_name, char *ip_traffic_info)
{
	...
	char client_mac[20];
	FILE *fp2;
	char init_mac_str[18]={0};
	char init_buffer[512]={0};
	unsigned long tx_p=0, rx_p=0, tx_bytes_high=0, tx_bytes=0, rx_bytes_high=0, rx_bytes=0;
	unsigned long  init_tx_p=0, init_rx_p=0, init_tx_bytes_high=0, init_tx_bytes=0, init_rx_bytes_high=0, init_rx_bytes=0;
		...
		for(vwlan_if=0; vwlan_if<5; vwlan_if++) {
			...
			for (i=1; i<=MAX_STA_NUM; i++) {
				...
				if (pInfo->aid && (pInfo->flags & STA_INFO_FLAG_ASOC)) {
					...
					fp2 = fopen(IP_CONNECTION_INFO, "r");
					if (fp2) {
						while(fgets(init_buffer, sizeof(init_buffer), fp2)) {
							sscanf(init_buffer,"%s %lu %lu %lu %lu %lu %lu",
									init_mac_str, &init_rx_p, &init_tx_p, &init_rx_bytes_high, &init_rx_bytes, &init_tx_bytes_high, &init_tx_bytes);
							if (strcmp(init_mac_str, mac_str) == 0) {
								rx_p = init_rx_p;
								tx_p = init_tx_p;
								rx_bytes_high = init_rx_bytes_high;
								rx_bytes = init_rx_bytes;
								tx_bytes_high = init_tx_bytes_high;
								tx_bytes = init_tx_bytes;
								break;
							}
						}
					...
					sprintf(ip_traffic_info ,
							"<td align=center valign=middle><script>var packet=%lu;document.write(packet.toLocaleString());</script></td>\n"
							"<td align=center valign=middle><script>var packet=%lu;document.write(packet.toLocaleString());</script></td>\n"
							"<td align=center valign=middle><script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n"
							"<td align=center valign=middle><script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n",
							pInfo->rx_only_data_packets-rx_p,
							pInfo->tx_only_data_packets-tx_p,
							pInfo->rx_only_data_bytes_high-rx_bytes_high,
							pInfo->rx_only_data_bytes-rx_bytes,
							pInfo->tx_only_data_bytes_high-tx_bytes_high,
							pInfo->tx_only_data_bytes-tx_bytes
							);
					sprintf(wlan_if_name, "%s",(wlan_if==0)?"5G":"2G");
					if (buff)
						free(buff);
					return 1;
...
/* APACRTL-92 smlee  */
#define TEMP_ARP	"/tmp/tmp_arp"
int get_IP_connection_info(request *wp, int argc, char **argv)
{
	...
	char hostname[64];
	int connection_cnt;
	int lan1_row=0, lan2_row=0, lan3_row=0, lan4_row=0;
	unsigned long long tx_p=0, rx_p=0, tx_bytes=0, rx_bytes=0;
	char str_traffic[128];
 	//struct user_net_device_stats stats;
	int lan_cnt=0, lan_num, ret=0;
	char ip_traffic_info[1024];
	char wlan_if[4];
	char init_mac_str[18]={0};
	char init_buffer[512]={0};
	unsigned long long init_tx_p=0, init_rx_p=0, init_tx_bytes=0, init_rx_bytes=0;
	unsigned long tx_bytes_only_high=0, tx_bytes_only=0, rx_bytes_only_high=0, rx_bytes_only=0;
	struct in_addr arp_ip, lan_ip, lan_mask;
	char buffer[100];

	if (access(TEMP_ARP, F_OK)==0)
		unlink(TEMP_ARP);

	system("cat /proc/net/arp >" TEMP_ARP);

	/*lan ip connection info*/
	for (lan_num=0; lan_num<4; lan_num++) {

		if((fp=fopen(TEMP_ARP, "r"))==NULL)
			return 0;
		...
		while(fgets(line_buffer, sizeof(line_buffer), fp))
		{
			...
			inet_aton(ip_str, &arp_ip);

			apmib_get( MIB_IP_ADDR,  (void *)buffer); //save the orig lan subnet
			memcpy((void *)&lan_ip, buffer, 4);

			apmib_get( MIB_SUBNET_MASK,  (void *)buffer); //save the orig lan mask
			memcpy((void *)&lan_mask, buffer, 4);

			//check DHCP client range (invalid ip does not display)
			if ((lan_ip.s_addr & lan_mask.s_addr) != (arp_ip.s_addr & lan_mask.s_addr)) {
				continue;
			}
			...
			lan_port = check_lan(tmp_mac_str);	// lan port check : LAN(0 1 2 3 )
			if (lan_port != lan_num)
				continue;

			get_IP_HostName(ip_str, hostname);	//get host name

			connection_cnt = get_ip_conn_cnt(ip_str);	//each ip connection count
			...
			if (lan_cnt ==0) {
				...
				if (fp2) {
					while(fgets(init_buffer, sizeof(init_buffer), fp2)) {
						sscanf(init_buffer,"%s %llu %llu %llu %llu", init_mac_str, &init_rx_p, &init_tx_p, &init_rx_bytes, &init_tx_bytes);
						if (strcmp(init_mac_str, tmp_mac_str) == 0) {
							rx_p -= init_rx_p;
							tx_p -= init_tx_p;
							rx_bytes -= init_rx_bytes;
							tx_bytes -= init_tx_bytes;
				...
				lan_cnt += req_format_write(wp, "<tr>\n"
						"<td align=center>%s</td>"
						"<td align=center>%s</td>"
						"<td align=center>LAN-%d</td>"
						"<td align=center>%d</td>\n"
						"<td id=\"lan%d_rx_p\" align=center valign=middle>"
						"<script>var packet=%llu;document.write(packet.toLocaleString());</script></td>\n"
						"<td id=\"lan%d_tx_p\" align=center valign=middle>"
						"<script>var packet=%llu;document.write(packet.toLocaleString());</script></td>\n"
						"<td id=\"lan%d_rx_b\" align=center valign=middle>"
						"<script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n"
						"<td id=\"lan%d_tx_b\" align=center valign=middle>"
						"<script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n"
						"</tr>\n",
						ip_str, hostname, lan_port+1, connection_cnt, lan_port+1, rx_p, lan_port+1,
						tx_p, lan_port+1, rx_bytes_only_high, rx_bytes_only, lan_port+1, tx_bytes_only_high, tx_bytes_only);
			} else {

				lan_cnt += req_format_write(wp, "<tr>\n"
						"<td align=center>%s</td>\n<td align=center>%s</td>\n<td align=center>LAN-%d</td>\n<td align=center>%d</td>\n</tr>\n",
						ip_str, hostname, lan_port+1, connection_cnt );
			}
	...
	if (lan1_row > 1) {
		req_format_write(wp, "<script>document.getElementById('lan1_tx_p').rowSpan=%d;"
							"document.getElementById('lan1_rx_p').rowSpan=%d;"
							"document.getElementById('lan1_tx_b').rowSpan=%d;"
							"document.getElementById('lan1_rx_b').rowSpan=%d;</script>",
							lan1_row, lan1_row, lan1_row, lan1_row);
	} else if (lan2_row > 1) {
		req_format_write(wp, "<script>document.getElementById('lan2_tx_p').rowSpan=%d;"
							"document.getElementById('lan2_rx_p').rowSpan=%d;"
							"document.getElementById('lan2_tx_b').rowSpan=%d;"
							"document.getElementById('lan2_rx_b').rowSpan=%d;</script>",
							lan2_row, lan2_row, lan2_row, lan2_row);
	} else if (lan3_row > 1) {
		req_format_write(wp, "<script>document.getElementById('lan3_tx_p').rowSpan=%d;"
							"document.getElementById('lan3_rx_p').rowSpan=%d;"
							"document.getElementById('lan3_tx_b').rowSpan=%d;"
							"document.getElementById('lan3_rx_b').rowSpan=%d;</script>",
							lan3_row, lan3_row, lan3_row, lan3_row);
	} else if (lan4_row > 1) {
		req_format_write(wp, "<script>document.getElementById('lan4_tx_p').rowSpan=%d;"
							"document.getElementById('lan4_rx_p').rowSpan=%d;"
							"document.getElementById('lan4_tx_b').rowSpan=%d;"
							"document.getElementById('lan4_rx_b').rowSpan=%d;</script>",
							lan4_row, lan4_row, lan4_row, lan4_row);
	}
	...
	while(fgets(line_buffer, sizeof(line_buffer), fp))
	{
		...
		ret = get_ip_wlan_traffic(tmp_mac_str, wlan_if, ip_traffic_info);
		...
		if (ret) {
			req_format_write(wp, "<tr>\n"
					...
					"%s\n"
					"</tr>\n",
					ip_str, hostname, wlan_if, connection_cnt, ip_traffic_info);
		}
	}
	fclose(fp);
	return 0;
