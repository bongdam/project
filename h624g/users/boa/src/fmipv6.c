...
#ifdef __DAVO__
#define IPV6_LEN_MAX	120

typedef struct {
	const char *v6pam;
	char backup[IPV6_LEN_MAX];
	int (* v6valid)(char*, char*, char*, char *);
	char *tname;
	int setflag;
} ipv6_variable;

static int ipv6_validate(char *errmsg_, char *addrS, char *target, char *buf)
{
	...
	if ((inet_pton(AF_INET6, addrS, &addr) <= 0)) {
		sprintf(errmsg_, "입력하신 %s주소가 IPv6 형식이 아닙니다.", target);
		return 0;
	}
	inet_ntop(AF_INET6, &addr, buf, IPV6_LEN_MAX);
	return 1;
}

static ipv6_variable v6_c[] = {
	{"x_ipv6_autoconfig_method", 	{0,},NULL, "", 0},
	{"x_ipv6_manual_addr", 	   		{0,},ipv6_validate, "", 0},
	{"x_ipv6_manual_prefix_len", 	{0,},NULL, "", 0},
	{"x_ipv6_manual_gateway",    	{0,},ipv6_validate, "게이트웨이 ", 0},
	{"x_ipv6_dns_method", 	   		{0,},NULL, "", 0},
	{"x_ipv6_manual_dns1", 	   		{0,},ipv6_validate, "DNS1 ", 0},
	{"x_ipv6_manual_dns2", 	   		{0,},ipv6_validate, "DNS2 ", 0},
	{ NULL, NULL}
};
...
void formIpv6Setup(request *wp, char *path, char *query)
{
	...
	ipv6_variable *p;
	int dns1_empty = 0;
	char *submitUrl = req_get_cstream_var(wp, "submit-url", "");	// hidden page

	/* APACRTL-85 */
	apmib_set_hist_clear();
	for (p = v6_c; p->v6pam; p++)
		p->setflag = 0;

	for (c = 0; c < 2; c++) {
		for (i = 0, p = v6_c; p->v6pam; p++) {
			if (c == 0) {
				if ((strVal = req_get_cstream_var(wp, p->v6pam, ""))) {
					ydespaces(strVal);
					if (p->v6valid) {
						if (p->v6valid(tmpBuf, strVal, p->tname, p->backup))
							p->setflag = 1;
						else
							goto setErr_end;
					} else {
						p->setflag = 1;
						sprintf(p->backup, "%s", strVal);
					}
				}
				if (!dns1_empty && !strcmp(p->v6pam, "x_ipv6_manual_dns1") && p->backup[0] == 0)
					dns1_empty = 1;
			} else {
				if (p->setflag) {
					if (dns1_empty && !strcmp(p->v6pam, "x_ipv6_manual_dns2")) {
						apmib_nvram_set("x_ipv6_manual_dns1", p->backup);
						nvram_unset("x_ipv6_manual_dns2");
						continue;
					}
					apmib_nvram_set(p->v6pam, p->backup);
				}
			}
		}
	}
	...
	if (!apmib_set(MIB_CUSTOM_PASSTHRU_ENABLED, (void *)&intVal)) {
		strcpy(tmpBuf, ("custom passthru 설정 오류!"));
		goto setErr_end;
	}
	web_config_trace(1, 2);		/* internet/ipv6 */

	nvram_commit();
	need_reboot = 1;
	OK_MSG("/skb_ipv6tcpipwan.htm");
...
//////MENU//////////////////////////////////////////////////
	if(!strcmp(name,"IPv6_Menu"))
	{
		...
		req_format_write(wp,"ipv6.addItem(\"IPv6 Wan Setting\", \"skb_ipv6_wan.htm\", \"\", \"Configure IPv6 Wan Setting\");\n");
		req_format_write(wp,"ipv6.addItem(\"IPv6 Lan Setting\", \"skb_dhcp6s.htm\", \"\", \"Setup IPv6 Lan\");\n");
		req_format_write(wp,"ipv6.addItem(\"Router Advertisement Daemon\", \"skb_radvd.htm\", \"\", \"Setup Radvd Daemon\");\n");
		//req_format_write(wp,"ipv6.addItem(\"DNS Proxy Daemon\", \"dnsv6.htm\", \"\", \"Setup Dnsmasq Daemon\");\n");
		req_format_write(wp,"ipv6.addItem(\"Tunnel (6 over 4)\", \"skb_tunnel6.htm\", \"\", \"Tunnel (6to4)\");\n");
	...
	if(!strcmp(name,"IPv6_nojs_Menu"))
	{
		...
		req_format_write(wp,"<tr><td><a href=\"skb_ipv6_wan.htm\" target=\"view\">IPv6 Wan Setting</a></td></tr>");
		req_format_write(wp,"<tr><td><a href=\"skb_dhcp6s.htm\" target=\"view\">IPv6 LAN Setting</a></td></tr>");
		req_format_write(wp,"<tr><td><a href=\"skb_radvd.htm\" target=\"view\">Router Advertisement Daemon</a></td></tr>");
		req_format_write(wp,"<tr><td><a href=\"skb_dnsv6.htm\" target=\"view\">DNS Proxy Daemon</a></td></tr>");
		req_format_write(wp,"<tr><td><a href=\"skb_tunnel6.htm\" target=\"view\">Tunnel 6over4</a></td></tr>");
		return 0;
	}
//////////radvd///////////////////////////////////////////////////////////////
