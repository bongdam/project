...
void killSomeDaemon(void)
{
	...
	killall(SIGKILL, "routed");
//	killall(SIGKILL, "pppoe");
//	killall(SIGKILL, "pppd");
//	killall(SIGKILL, "pptp");
	killall(SIGKILL, "dnrd");
	killall(SIGKILL, "ntpclient");
//	killall(SIGKILL, "miniigd");	//comment for miniigd iptables rule recovery
	killall(SIGKILL, "lld2d");
//	killall(SIGKILL, "l2tpd");
//	killall(SIGKILL, "udhcpc");
//	killall(SIGKILL, "udhcpd");
	killall(SIGKILL, "reload");
	killall(SIGKILL, "iapp");
	killall(SIGKILL, "wscd");
	killall(SIGKILL, "mini_upnpd");
	killall(SIGKILL, "iwcontrol");
	killall(SIGKILL, "auth");
	killall(SIGKILL, "disc_server");
	killall(SIGKILL, "igmpproxy");
	yecho("/proc/br_mCastFastFwd", "1,1\n");
	system("echo 1,1 > /proc/br_mCastFastFwd");
//	killall(SIGKILL, "syslogd");
//	killall(SIGKILL, "klogd");

	killall(SIGKILL, "ppp_inet");
#ifdef WLAN_HS2_CONFIG
	killall(SIGKILL, "hs2");
#endif
#ifdef CONFIG_IPV6
	killall(SIGKILL, "dhcp6c");
	killall(SIGKILL, "dhcp6s");
	killall(SIGKILL, "radvd");
	killall(SIGKILL, "ecmh");
	//kill mldproxy
	killall(SIGKILL, "mldproxy");
#endif
#ifdef CONFIG_SNMP
	killall(SIGKILL, "snmpd");
	unlink("/var/run/snmpd.pid");
#endif
}
...
#ifdef RTK_USB3G
		killall(SIGKILL, "mnet");
		killall(SIGKILL, "hub-ctrl");
		killall(SIGKILL, "usb_modeswitch");
		killall(SIGKILL, "ppp_inet");
		killall(SIGKILL, "pppd");
		unlink("/etc/ppp/connectfile");
...
#ifdef CONFIG_IPV6
		yexecl("2>/dev/null", "ip tunnel del tun");
#ifdef CONFIG_DSLITE_SUPPORT
		yexecl("2>/dev/null", "ip -6 tunnel del ds-lite");	//delete ds-lite tunnel
		unlink("/var/ds-lite.script");
#endif
#ifdef CONFIG_SIXRD_SUPPORT
		yexecl("2>/dev/null", "ip tunnel del tun6rd");
#endif
#endif
#ifdef __DAVO__
		int snmp_pid;
		yfcat("/var/run/snmp_agentd.pid", "%d", &snmp_pid);
		if (snmp_pid > 0) {
			kill(snmp_pid, SIGUSR1);
			usleep(500000);
...
int getScheduleInfo(request *wp, int argc, char **argv)
{
	...
	if ( !strcmp(name_arg, ("wlan_state")) ) {
		...
		return req_format_write(wp,"%s","menu.addItem(\"System Time\", \"skb_time.htm\", \"\", \"Setup System Time\");");
		#endif
	}
...
int addWlProfileHandler(request *wp, char *tmpBuf, int wlan_id)
{
	...
	if(add_to_profile == 1)
	{
		...
		strcpy((char *)entry.ssid, strSSID);
		...
		if(encryptVal == ENCRYPT_WEP)
		{
			...
			if(wepType == WEP64)
			{
				...
				strcpy((char *)entry.wepKey1, key);

				memset(key, 0x00, sizeof(key));
				apmib_get(MIB_WLAN_WEP64_KEY2, (void *)key);
				strcpy((char *)entry.wepKey2, key);

				memset(key, 0x00, sizeof(key));
				apmib_get(MIB_WLAN_WEP64_KEY3, (void *)key);
				strcpy((char *)entry.wepKey3, key);

				memset(key, 0x00, sizeof(key));
				apmib_get(MIB_WLAN_WEP64_KEY4, (void *)key);
				strcpy((char *)entry.wepKey4, key);
			}
			else
			{
		...
		else if(encryptVal > ENCRYPT_WEP)
		{
			...
			strcpy((char *)entry.wpaPSK, wpaPsk);
		...
		for(roop=0 ; roop<entryNum; roop++)
		{
			if(!memcmp((oriEntry[roop]).ssid, entry.ssid, strlen((char *)entry.ssid)))
			{
...
int wlanHandler(request *wp, char *tmpBuf, int *mode, int wlan_id)
{
	...
	int band_no = 0;
	int cur_band = 0;
	char tmp_wlan_name[80];
	char handover_disabled[8];
#ifdef  CONFIG_RTL_P2P_SUPPORT
	...
	if (wlan_id == 1) {
		if ( nvram_match("x_handover_enable", "1") ) {
			sprintf(handover_disabled, "%d", disabled);
			nvram_set("WLAN0_VAP3_WLAN_DISABLED", handover_disabled);
		}
	}
	...
	strDisabled = req_get_cstream_var(wp, "handOverdisable", "");
	if (!strcmp(strDisabled, "ON"))
		nvram_set("WLAN0_VAP3_WLAN_DISABLED", "1");
	else
		nvram_set("WLAN0_VAP3_WLAN_DISABLED", "0");
		...
		if (*mode == CLIENT_MODE) {
			...
			if (encrypt &  ENCRYPT_WPA2_MIXED) {
				...
				if (format & 1) { // radius
					...
					//Support wlan client mode with Enterprise (RADIUS)
#else
					strcpy(tmpBuf, ("You cannot set client mode with Enterprise (RADIUS) !"
							"<br><br>Please change the encryption method in security page first."));
					goto setErr_wlan;
#endif
				}
			} else if (encrypt == ENCRYPT_WEP || encrypt == 0) {
				...
				if (use1x & 1) {
					...
					//Support wlan client mode with Enterprise (RADIUS)
#else
					strcpy(tmpBuf, ("You cannot set client mode with 802.1x enabled!"
							"<br><br>Please change the encryption method in security page first."));
					goto setErr_wlan;
#endif
				}
...
#ifdef __DAVO__
		if (wlan_id == 1) {
			nvram_set("WLAN0_VAP3_SSID", strSSID);
		}
#endif
	} else if (*mode == 1 && !strSSID[0]) { // client and NULL SSID
		if (apmib_set(MIB_WLAN_SSID, (void *)strSSID) == 0) {
	...
	if (strChan[0]) {
		...
		if (errno) {
#ifdef __DAVO__
			strcpy(tmpBuf, ("채널 번호가 올바르지 않습니다!"));
#else
	...
	if (strVal[0]) {
		...
		if (wlan_onoff_tkip == 0) { //Wifi request
			...
			if (*mode != CLIENT_MODE &&
			    (band_no == 7 || band_no == 9 || band_no == 10 ||
			     band_no == 11 || band_no == 63 || band_no == 71 || band_no == 75)) { //7:n; 9:gn; 10:bgn 11:5g_an
		...
		if ((wlan_id == 0) && (vwlan_idx == 0)) {
			sprintf(strtmp, "%d", val);
			nvram_set("WLAN0_VAP3_BAND", strtmp);
		}
		...
		for (vap_idx = 1; vap_idx <= 4; vap_idx++) {
			...
			if (!vap_val) { //ON
				...
				if (((vap_val & 1) && (!(root_ap_band & 1))) ||
				    ((vap_val & 2) && (!(root_ap_band & 2))) ||
				    ((vap_val & 8) && (!(root_ap_band & 8))) ||
				    ((vap_val & 4) && (!(root_ap_band & 4))) ||
				    ((vap_val & 64) && (!(root_ap_band & 64)))) {
					...
					if (root_ap_band & 8) { //11N, no TKIP
						...
						apmib_get(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&wpaCipher);
						apmib_get(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&wpa2Cipher);
						if (wlan_encrypt == ENCRYPT_WPA ||
						    wlan_encrypt == ENCRYPT_WPA2 ||
						    wlan_encrypt == ENCRYPT_WPA2_MIXED) {
							wpaCipher &= ~WPA_CIPHER_TKIP;
							if (wpaCipher == 0)
								wpaCipher =  WPA_CIPHER_AES;
							//apmib_set(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&wpaCipher);
...
/*
	@note: The value of "WLANx_FIX_RATE"' shall be marked 32bit(signed bit) for express ac tx-rate.
	So force using the save value by nvram_set.
*/
			sprintf(varName, "WLAN%d_FIX_RATE", wlan_id);
			sprintf(strtmp, "%d", val);
			nvram_set(varName, strtmp);
			//apmib_set(MIB_WLAN_FIX_RATE, &val);
...
#ifdef __DAVO__
		if (wlan_id == 1) {
			nvram_set("WLAN0_VAP3_HIDDEN_SSID", strVal);
		}
#endif
	}
...
// for 11N
	...
	if (strVal[0]) {
		...
		sprintf(strtmp, "x_wlan%d_auto_bonding", wlan_idx);
		if ( val == 5) {
			nvram_set(strtmp, "1");
		} else {
			nvram_set(strtmp, "0");
	...
	sprintf(varName, "channelbound_manual%d", wlan_id);
	strVal = req_get_cstream_var(wp, varName, "");
	if (strVal[0]) {
		if (strVal[0] == '0')
			val = 0;
...
//
#ifdef __DAVO__
	sprintf(varName, "wlan%d_max_conn", wlan_id);
	strVal = req_get_cstream_var(wp, varName, "");
	if (strVal[0]) {
		get_wlan_name(tmp_wlan_name, 0, "max_conn");
		val = atoi(strVal);
		if (val < 1 || val > 127) {
			strcpy(tmpBuf, ("Error! Invalid Max Connect."));
			goto setErr_wlan;
		}
		apmib_nvram_set(tmp_wlan_name, strVal);
		if (wlan_id == 1) {
			nvram_set("x_wlan0_va3_max_conn", strVal);
		}
	}
#endif
	sprintf(varName, "basicrates%d", wlan_id);
...
//displayPostDate(wp->post_data);
...
#endif //#if defined(WLAN_PROFILE)
	vwlan_idx = 0;
	web_config_trace(2, 1);		/* wireless/basic */
	...
	if (warn) {
		OK_MSG1(tmpBuf, "/skb_wlbasic.htm");
	} else {
		OK_MSG("/skb_wlbasic.htm");
	}
	need_reboot = 1;
...
# endif
	...
	sprintf(varName, "wepEnabled%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
	...
	if (enabled) {
		sprintf(varName, "length%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
	...
	if (apmib_set(MIB_WLAN_ENCRYPT, (void *)&encrypt) == 0) {
		strcpy(tmpBuf, ("WLAN ENCRYPT MIB 설정 실패!"));
		goto setErr_wep;
	}
	...
	sprintf(varName, "useMacAuth%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
	strVal = req_get_cstream_var(wp, varName, "");
	if (!strcmp(strVal, "ON")) {
		intVal = 1;
		enableRS = 1;
	...
	if (apmib_set(MIB_WLAN_MAC_AUTH_ENABLED, (void *)&intVal) == 0) {
		strcpy(tmpBuf, ("WLAN MAC AUTH MIB 설정 실패!"));
		goto setErr_wep;
	}

	sprintf(varName, "authType%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
	...
	sprintf(varName, "format%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
	strFormat = req_get_cstream_var(wp, varName, "");
	if (!strFormat[0]) {
		strcpy(tmpBuf, ("Key type 은 존재해야 합니다!"));
		goto setErr_wep;
	}

	if (strFormat[0] != '1' && strFormat[0] != '2') {
		strcpy(tmpBuf, ("key type이 올바르지 않습니다!"));
		goto setErr_wep;
	}

	i = strFormat[0] - '0' - 1;
	if (apmib_set(MIB_WLAN_WEP_KEY_TYPE, (void *)&i) == 0) {
		strcpy(tmpBuf, ("WEP key type 설정 실패!"));
		goto setErr_wep;
	}
			...
			if (!isAllStar(wepKey)) {
				if (strFormat[0] == '1')	// ascii
					...
					if (!string_to_hex(wepKey, key, keyLen)) {
						strcpy(tmpBuf, ("wep-key가 올바르지 않습니다!"));
						goto setErr_wep;
					}
				}

				if (strFormat[0] == '1')	// ascii
					yitoxa(keyValue, (unsigned char *)key, strlen(key));
				else	//hex
					sprintf(keyValue, "%s", wepKey);

				if (vwlan_idx == 0)
					snprintf(varName, sizeof(varName), "WLAN%d_WEP%d_KEY%d", wlan_idx, (wep == WEP64) ? 64 : 128, i);
				else
					snprintf(varName, sizeof(varName), "WLAN%d_VAP%d_WEP%d_KEY%d", wlan_idx, vwlan_idx - 1, (wep == WEP64) ? 64 : 128, i);
				ret = nvram_match(varName, keyValue);
				/* To keep leading or trailing white spaces */
				yexecl(NULL, "nvram set %s=\"%s\"", varName, keyValue);
				if (wlan_id == 1 && vwlan_idx == 0)
					yexecl(NULL, "nvram set WLAN0_VAP3_WEP%d_KEY%d=\"%s\"",
					       (wep == WEP64) ? 64 : 128, i, strVal);	//5G handover
				if (ret == 0) {
					if (wep == WEP64)
						apmib_set_hist_put(MIB_WLAN_WEP64_KEY1 + i - 1);
					else
						apmib_set_hist_put(MIB_WLAN_WEP128_KEY1 + i - 1);
				}
			}
		}
	}

	strFormat = req_get_cstream_var(wp, "key_index", "");
	i = strFormat[0] - '0';
	if (!apmib_set(MIB_WLAN_WEP_DEFAULT_KEY, (void *)&i)) {
		strcpy(tmpBuf, ("기본 송신 key id 설정 오류!"));
		goto setErr_wep;
	}
#if 0
...
# endif
	{
		sprintf(varName, "wps_clear_configure_by_reg%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
...
void formWep(request * wp, char *path, char *query)
{
	...
	if (!(log_control & 1))
		web_config_trace(2, 3);	/* wireless/security */
...
int wpaHandler(request *wp, char *tmpBuf, int wlan_id)
{
	char *strEncrypt, *strVal;
	ENCRYPT_T encrypt;
	int enableRS = 0, intVal, getPSK = 0, len, val, i;
	...
	sprintf(varName, "method%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
	strEncrypt = req_get_cstream_var(wp, varName, "");
	if (!strEncrypt[0]) {
		strcpy(tmpBuf, ("WLAN 암호화 mib 설정 오류!"));
		goto setErr_encrypt;
	}
	encrypt = (ENCRYPT_T)strEncrypt[0] - '0';
	if (encrypt != ENCRYPT_DISABLED && encrypt != ENCRYPT_WEP && encrypt != ENCRYPT_WPA
		...
		strcpy(tmpBuf, ("encryption method가 올바르지 않습니다!"));
		goto setErr_encrypt;
	}
#ifdef WIFI_SIMPLE_CONFIG
# ifdef MBSSID
...
# endif
	...
	if (encrypt == ENCRYPT_DISABLED || encrypt == ENCRYPT_WEP) {
		sprintf(varName, "use1x%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strcmp(strVal, "ON")) {
...
//                              goto setErr_encrypt;
		...
		if (encrypt == ENCRYPT_WEP) {
			...
			if (!apmib_get(MIB_WLAN_WEP, (void *)&wep)) {
				strcpy(tmpBuf, ("WLAN WEP MIB 설정오류!"));
				goto setErr_encrypt;
			}
			if (wep == WEP_DISABLED) {
				wep = WEP64;
				if (apmib_set(MIB_WLAN_WEP, (void *)&wep) == 0) {
					strcpy(tmpBuf, ("WEP MIB 설정오류!"));
					goto setErr_encrypt;
				}
			}
		} else {
		...
		if (strVal[0]) {
			if (!strcmp(strVal, ("eap"))) {
				apmib_get(MIB_WLAN_MODE, (void *)&intVal);
				if (intVal != AP_MODE && intVal != AP_WDS_MODE) {	// not AP mode
					strcpy(tmpBuf, ("오류! 클라이언트 모드에서는 WPA-RADIUS 설정 불가합니다."));
					goto setErr_encrypt;
				}
				...
				strcpy(tmpBuf, ("오류! wapi 인증이 올바르지 않습니다."));
				goto setErr_encrypt;
			}

			if (apmib_set(MIB_WLAN_WAPI_AUTH, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("WLAN 인증 종류 설정 오류!"));
				goto setErr_encrypt;
			}
		}
		// pre-shared key
		if (getPSK) {
			...
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("오류! psk 종류 없음."));
				goto setErr_encrypt;
			}
			intVal = strVal[0] - '0';
			if (intVal != 0 && intVal != 1) {
				strcpy(tmpBuf, ("오류! psk 종류가 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			// remember current psk format and length to compare to default case "****"
			...
			if (apmib_set(MIB_WLAN_WAPI_PSK_FORMAT, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("WLAN WPA PSK 종류 설정 실패!"));
				goto setErr_encrypt;
			}

			if (intVal == 1) {	// hex
				if ( /*len!=MAX_PSK_LEN || */ !string_to_hex(strVal, tmpBuf, MAX_PSK_LEN)) {
					strcpy(tmpBuf, ("오류! psk가 올바르지 않습니다!"));
					goto setErr_encrypt;
				}
				if (0 == (len % 2)) {
					len = len / 2;
					...
					strcpy(tmpBuf, ("오류! psk 길이가 올바르지 않습니다."));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_WAPI_PSKLEN, (void *)&len)) {
					strcpy(tmpBuf, ("오류! wapi key 길이가 잘못 설정되어 있습니다"));
				}
			} else {	// passphras
			...
			if (!apmib_set(MIB_WLAN_WAPI_PSK, (void *)strVal)) {
				strcpy(tmpBuf, ("WLAN WPA PSK 설정 오류!"));
				goto setErr_encrypt;
			}
		}
 wapi_end:
		...
		if (1 == enableAS) {
			...
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("WAPI AS 주소 없음!"));
				goto setErr_encrypt;
			}
			if (!inet_aton(strVal, &inIp)) {
				strcpy(tmpBuf, ("AS IP 주소가 올바르지 않습니다!"));
				goto setErr_encrypt;
			}
			if (!apmib_set(MIB_WLAN_WAPI_ASIPADDR, (void *)&inIp)) {
				strcpy(tmpBuf, ("RS IP 주소 설정 오류!"));
				goto setErr_encrypt;
			}
# if 0
...
# endif
		...
		sprintf(varName, "nonWpaSupp%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		...
		if (apmib_set(MIB_WLAN_ENABLE_SUPP_NONWPA, (void *)&intVal) == 0) {
			strcpy(tmpBuf, ("WLAN SUPP NONWPA mib 설정 오류!"));
			goto setErr_encrypt;
		}
		if (intVal) {
			suppNonWPA = SUPP_NONWPA_NONE;
			sprintf(varName, "nonWpaWep%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strcmp(strVal, "ON"))
				suppNonWPA |= SUPP_NONWPA_WEP;

			sprintf(varName, "nonWpa1x%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strcmp(strVal, "ON")) {
		...
		// WPA authentication
		sprintf(varName, "wpaAuth%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if (!strcmp(strVal, ("eap"))) {
				apmib_get(MIB_WLAN_MODE, (void *)&intVal);
...
# else
					strcpy(tmpBuf, ("오류! 클라이언트 모드에서는 WPA-RADIUS 사용 불가합니다."));
					goto setErr_encrypt;
# endif
				}
#endif
				...
				strcpy(tmpBuf, ("오류! wpa 인증이 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			if (apmib_set(MIB_WLAN_WPA_AUTH, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("WLAN 인증 종류 설정 실패!"));
				goto setErr_encrypt;
			}
		}
		// cipher suite
...
// get cipher suite from user setting, for wpa-aes -------------------
		...
		if ((encrypt == ENCRYPT_WPA) || (encrypt == ENCRYPT_WPA2_MIXED)) {
			intVal = 0;
			for (i = 1; i <= 2; i++) {
				sprintf(varName, "ciphersuite%d_%d", i, (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
					...
					if (strstr(strVal, ("aes")))
						intVal |= WPA_CIPHER_AES;
					if (intVal == 0) {
						strcpy(tmpBuf, ("cipher suite가 올바르지 않습니다!"));
						goto setErr_encrypt;
					}
				}
			}
			...
			if (val == CLIENT_MODE) {
				...
				if (val == INFRASTRUCTURE && intVal == WPA_CIPHER_MIXED) {
					strcpy(tmpBuf, ("오류! 클라이언트 모드에서는 cipher를 TKIP + AES로 사용 불가합니다."));
					goto setErr_encrypt;
				}
			}	// david+2006-1-11

			if (apmib_set(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("WLAN WPA CIPHER SUITE 설정 실패!"));
				goto setErr_encrypt;
			}
		...
		if ((encrypt == ENCRYPT_WPA2) || (encrypt == ENCRYPT_WPA2_MIXED)) {
			intVal = 0;
			for (i = 1; i <= 2; i++) {
				sprintf(varName, "wpa2ciphersuite%d_%d", i, (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
					if (strstr(strVal, ("tkip")))
						intVal |= WPA_CIPHER_TKIP;
					if (strstr(strVal, ("aes")))
						intVal |= WPA_CIPHER_AES;
					if (intVal == 0) {
						strcpy(tmpBuf, ("wpa2 cipher suite가 올바르지 않습니다!"));
						goto setErr_encrypt;
					}
				}
			}
			...
			if (apmib_set(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("WLAN WPA2 CIPHER SUITE 설정 실패!"));
				goto setErr_encrypt;
			}
		}
//-------------------------------------------------- david, 2005-8-03

		if (((encrypt == ENCRYPT_WPA2) || (encrypt == ENCRYPT_WPA2_MIXED)) && enableRS == 1) {
			sprintf(varName, "preAuth%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strcmp(strVal, "ON"))
				intVal = 1;
			else
		...
		if (getPSK) {
			...
			sprintf(varName, "pskFormat%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("오류! psk format이 없습니다."));
				goto setErr_encrypt;
			}
			intVal = strVal[0] - '0';
			if (intVal != 0 && intVal != 1) {
				strcpy(tmpBuf, ("오류! psk format이 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			// remember current psk format and length to compare to default case "****"
			...
			sprintf(varName, "pskValue%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			len = strlen(strVal);
			...
			if (!isAllStar(strVal)) {
				...
				if (vwlan_idx == 0)
					snprintf(varName, sizeof(varName), "WLAN%d_WPA_PSK", wlan_idx);
				else
					snprintf(varName, sizeof(varName), "WLAN%d_VAP%d_WPA_PSK", wlan_idx, vwlan_idx - 1);
				i = nvram_match(varName, strVal);
				/* To keep leading or trailing white spaces */
				yexecl(NULL, "nvram set %s=\"%s\"", varName, strVal);
				if (wlan_id == 1 && vwlan_idx == 0)
					yexecl(NULL, "nvram set WLAN0_VAP3_WPA_PSK=\"%s\"", strVal);	//5G handover
				if (i == 0)
					apmib_set_hist_put(MIB_WLAN_WPA_PSK);
				/* if (!apmib_set(MIB_WLAN_WPA_PSK, (void *)strVal)) {
					strcpy(tmpBuf, ("WLAN WPA PSK가 올바르지 않습니다!"));
					goto setErr_encrypt;
				} */
			}
		}
 rekey_time:
		// group key rekey time
		reKeyTime = 0;
		sprintf(varName, "groupKeyTimeDay%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if (!string_to_dec(strVal, &intVal)) {
				strcpy(tmpBuf, ("오류! rekey day가 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			reKeyTime += intVal * 86400;
		}
		sprintf(varName, "groupKeyTimeHr%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if (!string_to_dec(strVal, &intVal)) {
				strcpy(tmpBuf, ("오류! of rekey hr이 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			reKeyTime += intVal * 3600;
		}
		sprintf(varName, "groupKeyTimeMin%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if (!string_to_dec(strVal, &intVal)) {
				strcpy(tmpBuf, ("오류! rekey min이 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			reKeyTime += intVal * 60;
		}

		sprintf(varName, "groupKeyTimeSec%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if (!string_to_dec(strVal, &intVal)) {
				strcpy(tmpBuf, ("오류! rekey sec가 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			reKeyTime += intVal;
		}
		if (reKeyTime) {
			if (!apmib_set(MIB_WLAN_WPA_GROUP_REKEY_TIME, (void *)&reKeyTime)) {
				strcpy(tmpBuf, ("WLAN WPA 그룹 REKEY TIME 설정 실패!"));
				goto setErr_encrypt;
			}
		}
	}
	...
	if (enableRS == 1) {	// if 1x enabled, get RADIUS server info
		...
		if (wlan_mode == CLIENT_MODE) {	// wlan client mode
			...
			sprintf(varName, "eapType%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if (!string_to_dec(strVal, &intVal)) {
			...
			if (intVal == EAP_MD5) {
				sprintf(varName, "eapUserId%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
				...
				sprintf(varName, "radiusUserName%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
					...
					if (!apmib_set(MIB_WLAN_RS_USER_NAME, (void *)strVal)) {
						strcpy(tmpBuf, ("WLAN_RS_USER_NAME MIB 설정 오류!"));
						goto setErr_encrypt;
					}
				} else {
				...
				sprintf(varName, "radiusUserPass%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
					if (!isAllStar(strVal)) {
						if (strlen(strVal) > MAX_RS_USER_PASS_LEN) {
							strcpy(tmpBuf, ("RADIUS user password 길이가 올바르지 않습니다!"));
							goto setErr_encrypt;
						}
						if (!apmib_set(MIB_WLAN_RS_USER_PASSWD, (void *)strVal)) {
							strcpy(tmpBuf, ("WLAN_RS_USER_PASSWD 설정 실패!"));
							goto setErr_encrypt;
						}
					}
				} else {
				...
				sprintf(varName, "radiusUserCertPass%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
				...
				sprintf(varName, "eapInsideType%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
				...
				if (intVal2 == INSIDE_MSCHAPV2) {
					...
					if (strVal[0]) {
						...
						if (!apmib_set(MIB_WLAN_RS_USER_NAME, (void *)strVal)) {
							strcpy(tmpBuf, ("WLAN_RS_USER_NAME 설정 오류!"));
							goto setErr_encrypt;
						}
					} else {
						strcpy(tmpBuf, ("802.1x RADIUS User Name이 올바르지 않습니다!"));
						goto setErr_encrypt;
					}
					...
					if (strVal[0]) {
						if (!isAllStar(strVal)) {
							if (strlen(strVal) > MAX_RS_USER_PASS_LEN) {
								strcpy(tmpBuf, ("RADIUS user password 사이즈가 올바르지 않습니다!"));
								goto setErr_encrypt;
							}
							if (!apmib_set(MIB_WLAN_RS_USER_PASSWD, (void *)strVal)) {
								strcpy(tmpBuf, ("WLAN_RS_USER_PASSWD MIB 설정 오류!"));
								goto setErr_encrypt;
							}
						}
					} else {
						strcpy(tmpBuf, ("802.1x RADIUS User Password가 올바르지 않습니다!"));
						goto setErr_encrypt;
					}

//                                      if(isFileExist(RS_USER_CERT) == 1){
					sprintf(varName, "radiusUserCertPass%d",
						(wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
					strVal = req_get_cstream_var(wp, varName, "");
					if (strVal[0]) {
						if (strlen(strVal) > MAX_RS_USER_CERT_PASS_LEN) {
							strcpy(tmpBuf, ("RADIUS user cert password 사이즈가 올바르지 않습니다!"));
							goto setErr_encrypt;
						}
						if (!apmib_set(MIB_WLAN_RS_USER_CERT_PASSWD, (void *)strVal)) {
							strcpy(tmpBuf, ("WLAN_RS_USER_CERT_PASSWD MIB 설정 오류!"));
							goto setErr_encrypt;
						}
					} else {
						if (!apmib_set(MIB_WLAN_RS_USER_CERT_PASSWD, (void *)strVal)) {
							strcpy(tmpBuf, ("[1] WLAN_RS_USER_CERT_PASSWD MIB Clear 오류!"));
							goto setErr_encrypt;
						}
						//strcpy(tmpBuf, ("No 802.1x RADIUS user cert password!"));
						//goto setErr_encrypt;
...
//                                      }
					...
					strcpy(tmpBuf, ("802.1x inside tunnel type 은 지원되지 않습니다!"));
					goto setErr_encrypt;
				}
			} else {
				strcpy(tmpBuf, ("802.1x EAP type 은 지원되지 않습니다!"));
				goto setErr_encrypt;
			}
		} else
#endif
		{
			sprintf(varName, "radiusPort%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("RS 포트 번호 없음!"));
				goto setErr_encrypt;
			}
			if (!string_to_dec(strVal, &intVal) || intVal <= 0 || intVal > 65535) {
				strcpy(tmpBuf, ("오류! RS 포트 번호가 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			if (!apmib_set(MIB_WLAN_RS_PORT, (void *)&intVal)) {
				strcpy(tmpBuf, ("RS 포트 설정 실패!"));
				goto setErr_encrypt;
			}
			sprintf(varName, "radiusIP%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("RS IP 주소 없음!"));
				goto setErr_encrypt;
			}
			if (!inet_aton(strVal, &inIp)) {
				strcpy(tmpBuf, ("RS IP주소가 올바르지 않습니다!"));
				goto setErr_encrypt;
			}
			...
			sprintf(varName, "radiusPass%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!isAllStar(strVal)) {
				if (strlen(strVal) > (MAX_RS_PASS_LEN - 1)) {
					strcpy(tmpBuf, ("RS 비밀번호 길이가 너무 깁니다!"));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_RS_PASSWORD, (void *)strVal)) {
					strcpy(tmpBuf, ("RS 비밀번호 설정 오류!"));
					goto setErr_encrypt;
				}
			}
			sprintf(varName, "radiusRetry%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if (!string_to_dec(strVal, &intVal)) {
					strcpy(tmpBuf, ("RS retry가 올바르지 않습니다!"));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_RS_MAXRETRY, (void *)&intVal)) {
					strcpy(tmpBuf, ("WLAN RS RETRY 설정 실패!"));
					goto setErr_encrypt;
				}
			}
			sprintf(varName, "radiusTime%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if (!string_to_dec(strVal, &intVal)) {
					strcpy(tmpBuf, ("RS time가 올바르지 않습니다!"));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_RS_INTERVAL_TIME, (void *)&intVal)) {
					strcpy(tmpBuf, ("WLAN RS 재시도 간격 설정 오류!"));
					goto setErr_encrypt;
				}
			}
			sprintf(varName, "useAccount%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strcmp(strVal, "ON"))
				intVal = 1;
			else
			...
			if (apmib_set(MIB_WLAN_ACCOUNT_RS_ENABLED, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("WLAN ACCOUNT RS mib 사용 설정 오류!"));
				goto setErr_encrypt;
			}
			if (intVal == 0)
				goto get_wepkey;

			sprintf(varName, "accountPort%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("account RS 포트 번호 없음!"));
				goto setErr_encrypt;
			}
			if (!string_to_dec(strVal, &intVal) || intVal <= 0 || intVal > 65535) {
				strcpy(tmpBuf, ("오류! account RS 포트 번호가 올바르지 않습니다."));
				goto setErr_encrypt;
			}
			if (!apmib_set(MIB_WLAN_ACCOUNT_RS_PORT, (void *)&intVal)) {
				strcpy(tmpBuf, ("account RS 포트 설정 오류!"));
				goto setErr_encrypt;
			}
			sprintf(varName, "accountIP%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("account RS IP 주소 없음!"));
				goto setErr_encrypt;
			}
			if (!inet_aton(strVal, &inIp)) {
				strcpy(tmpBuf, ("account RS IP 주소가 올바르지 않습니다!"));
				goto setErr_encrypt;
			}
			if (!apmib_set(MIB_WLAN_ACCOUNT_RS_IP, (void *)&inIp)) {
				strcpy(tmpBuf, ("account RS IP 주소 설정 실패!"));
				goto setErr_encrypt;
			}
			sprintf(varName, "accountPass%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!isAllStar(strVal)) {
				if (strlen(strVal) > (MAX_RS_PASS_LEN - 1)) {
					strcpy(tmpBuf, ("Account RS 비밀번호 길이가 너무 깁니다!"));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_ACCOUNT_RS_PASSWORD, (void *)strVal)) {
					strcpy(tmpBuf, ("account RS 비밀번호 설정 오류!"));
					goto setErr_encrypt;
				}
			}
			sprintf(varName, "accountRetry%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if (!string_to_dec(strVal, &intVal)) {
					strcpy(tmpBuf, ("account RS retry가 올바르지 않습니다!"));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_ACCOUNT_RS_MAXRETRY, (void *)&intVal)) {
					strcpy(tmpBuf, ("WLAN ACCOUNT RS RETRY 설정 오류!"));
					goto setErr_encrypt;
				}
			}
			sprintf(varName, "accountTime%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if (!string_to_dec(strVal, &intVal)) {
					strcpy(tmpBuf, ("account RS time가 올바르지 않습니다!"));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_ACCOUNT_RS_INTERVAL_TIME, (void *)&intVal)) {
					strcpy(tmpBuf, ("WLAN ACCOUNT RS 재시도 간격 설정 오류!"));
					goto setErr_encrypt;
				}
			}
			sprintf(varName, "accountUpdateEnabled%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strcmp(strVal, "ON"))
				intVal = 1;
			else
			...
			if (apmib_set(MIB_WLAN_ACCOUNT_RS_UPDATE_ENABLED, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("WLAN ACCOUNT UPDATE mib 사용 설정 오류!"));
				goto setErr_encrypt;
			}
			sprintf(varName, "accountUpdateTime%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if (!string_to_dec(strVal, &intVal)) {
					strcpy(tmpBuf, ("오류! 계정 갱신 설정이 올바르지 않습니다"));
					goto setErr_encrypt;
				}
				if (!apmib_set(MIB_WLAN_ACCOUNT_RS_UPDATE_DELAY, (void *)&intVal)) {
					strcpy(tmpBuf, ("WLAN ACCOUNT 갱신 지연시간 mib 설정 오류!"));
					goto setErr_encrypt;
				}
			}
 ...
 get_wepkey:
			...
			sprintf(varName, "wepKeyLen%d", (wlan_id == 0 && vwlan_idx == 4) ? 1 : wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if (!strcmp(strVal, ("wep64")))
					...
					strcpy(tmpBuf, ("오류! wep key 길이가 올바르지 않습니다."));
					goto setErr_encrypt;
				}
				if (apmib_set(MIB_WLAN_WEP, (void *)&intVal) == 0) {
					strcpy(tmpBuf, ("WLAN WEP 설정 실패!"));
					goto setErr_encrypt;
				}
			}
		}
...
//printf("\r\n encrypt[%d],__[%s-%u]\r\n",encrypt,__FILE__,__LINE__);
	...
	if (encrypt==ENCRYPT_WEP)
	{
		...
		if  (wepKey[0]) {
			...
			if ( !isAllStar(wepKey) ) {
				...
				if (entry.encryption == WEP64){
					strncpy((char *)entry.wepKey1, key, 10);
					strncpy((char *)entry.wepKey2, key, 10);
					strncpy((char *)entry.wepKey3, key, 10);
					strncpy((char *)entry.wepKey4, key, 10);
				}else{
					strncpy((char *)entry.wepKey1, key, 26);
					strncpy((char *)entry.wepKey2, key, 26);
					strncpy((char *)entry.wepKey3, key, 26);
					strncpy((char *)entry.wepKey4, key, 26);
				}
			}
		}
	...
	else if (encrypt > ENCRYPT_WEP)
	{
		...
		if ( getPSK ) {
			...
			strcpy((char *)entry.wpaPSK, strPskValue);
	...
	else
	{
		...
		strcpy(oldSsid, (char *)entry.ssid);
		memset(&entry, 0x00, sizeof(WLAN_PROFILE_T));
		strcpy((char *)entry.ssid, oldSsid);
		entry.wpa_cipher = 8; //WPA_CIPHER_AES
...
# endif				//#if defined(WLAN_PROFILE)
		...
		if (encrypt == ENCRYPT_WEP) {
			...
			if (strAuth[0]) {	// new UI
				...
				sprintf(str_vap3_auth, "%d", authType);
				nvram_set("WLAN0_VAP3_AUTH_TYPE", str_vap3_auth);
				...
				if (strVal[0] && strcmp(strVal, "ON")) {
					...
					nvram_set("WLAN0_VAP3_ENABLE_1X", "0");
					log_control = 2;
...
void formWlAc(request * wp, char *path, char *query)
{
	...
	int j = 0;
	int ssid_idx = 0, old_idx;
	int wlan_intf = 0, old_wlan;
	char name[64];

	apmib_set_hist_clear();		/* APACRTL-85 */

	strVal = req_get_cstream_var(wp, "Wlanintf", "");
	if (strVal[0]) {
		wlan_intf = atoi(strVal);
	} else {
		printf("Invald wlan interface!\n");
		return;
	}

	strVal = req_get_cstream_var(wp, "wlan_ssid_id", "");
	if (strVal[0]) {
		ssid_idx = atoi(strVal);
		if (ssid_idx > NUM_VWLAN_INTERFACE) {
			printf("Invald ssid_id!\n");
	...
	web_log_wlname(name, sizeof(name));
	if (strAddMac[0]) {
		/*if ( !strcmp(strEnabled, "ON"))
		   enabled = 1;
		   else
		...
		if (apmib_set(MIB_WLAN_MACAC_ENABLED, (void *)&enabled) == 0) {
			strcpy(tmpBuf, ("사용 설정 오류!"));
			goto setErr_ac;
		} else if (apmib_set_hist_search(MIB_WLAN_MACAC_ENABLED) > -1)
			LOG(LOG_INFO, "%s 무선 접근 제어를 %s",
			    name, !enabled ? "사용 안 함" : (enabled == 1 ? "허용 규칙으로 설정함" : "차단 규칙으로 설정함"));
...
//                      strcpy(tmpBuf, ("Error! No mac address to set."));
		...
		if (apmib_set(MIB_WLAN_AC_ADDR_ADD, (void *)&macEntry) == 0) {
			...
			LOG(LOG_INFO, "%s 무선 접근 제어에 %s에 대해 %s함",
			    name, apmib_btoa(ETH_ADDR, macEntry.macAddr), (enabled == 1) ? "허용" : "차단");
		//handover ssid(wlan1_vap3) : same setting
		if (wlan_idx == 1 && vwlan_idx == 0) {
			wlan_idx = 0;
			vwlan_idx = 4;
			...
			if (!strcmp(strVal, "ON")) {
				...
				*((char *)&macEntry) = (char)i;
				...
				if (!apmib_set(MIB_WLAN_AC_ADDR_DEL, (void *)&macEntry)) {
					...
					LOG(LOG_INFO, "%s 무선 접근 제어에서 %s을 삭제함",
					    name, apmib_btoa(ETH_ADDR, macEntry.macAddr));
				//handover ssid(wlan0_vap3) : same setting
				if (wlan_idx == 1 && vwlan_idx == 0) {
					wlan_idx = 0;
					vwlan_idx = 4;
					apmib_set(MIB_WLAN_AC_ADDR_DEL, (void *)&macEntry);
	...
	if (strDelAllMac[0]) {
		if (!apmib_set(MIB_WLAN_AC_ADDR_DELALL, (void *)&macEntry)) {
			...
			LOG(LOG_INFO, "%s 무선 접근 제어 전체 항목을 삭제함", name);
		//handover ssid(wlan0_vap3) : same setting
		if (wlan_idx == 1 && vwlan_idx == 0) {
			wlan_idx = 0;
			vwlan_idx = 4;
...
#endif
	char *rssi_threshold;
	char dv_name[80], rssi_name[40];
	char *strhandover_rssi_th;
	char *strhandover_pps_time;
	char *strhandover_tcp_pkts_cnt;
    char *strhandover_probe_deny_rssi_th;
	char handover_rssi_th[8];
	char handover_probe_deny_rssi_th[8];
    char handover_pps_time[4];
	char handover_tcp_pkts_cnt[4];
	char *strCca_mode;
	char cca_name[40];
...
#if 0
		if (!string_to_dec(strPower, &val) || val < 10 || val > 100) {
			strcpy(tmpBuf, ("Error! Invalid RF output power value."));
			goto setErr_advance;
		}
#endif
...
// for 11N
	...
	strValue = req_get_cstream_var(wp, ("dsf_enable_"), "");
	if (strValue[0]) {
		if (!strcmp(strValue, ("enable")))
			val = 1;
		else
			val = 0;
		apmib_nvram_set("x_dfs_enable", val ? "1" : "0");
	}
	...
	if (wp->superUser == 1) {
		strhandover_rssi_th = req_get_cstream_var(wp, ("x_bs_rssi_th"), "");
		if (strhandover_rssi_th[0]) {
			val = atoi(strhandover_rssi_th);
			if (val < -100 || val > 100) {
				strcpy(tmpBuf, ("Error! Invalid Handover SSID RSSI Threshold value."));
				goto setErr_advance;
			}
			sprintf(handover_rssi_th, "%d", (val < 0) ? val : val * (-1));
			yfecho("/proc/sys/private/bs/rssi_threshold", O_WRONLY | O_CREAT | O_TRUNC, 0644, "%d", (val < 0) ? val : val * (-1));
			apmib_nvram_set("x_bs_rssi_th", handover_rssi_th);
		}
	}

    strhandover_probe_deny_rssi_th = req_get_cstream_var(wp, ("x_bs_probe_deny_rssi_th"), "");
	if (strhandover_probe_deny_rssi_th[0]) {
		val = atoi(strhandover_probe_deny_rssi_th);
		if (val < -100 || val > 100) {
			strcpy(tmpBuf, ("Error! Invalid Handover SSID RSSI Threshold value."));
			goto setErr_advance;
		}
		sprintf(handover_probe_deny_rssi_th, "%d", (val < 0) ? val : val * (-1));
		yfecho("/proc/sys/private/bs/probe_deny_rssi", O_WRONLY | O_CREAT | O_TRUNC, 0644, "%d", (val < 0) ? val : val * (-1));
		apmib_nvram_set("x_bs_probe_deny_rssi_th", handover_probe_deny_rssi_th);
	}

	if (wp->superUser == 1) {
		strhandover_pps_time = req_get_cstream_var(wp, ("x_bs_tcp_pps_check_time"), "");
		if (strhandover_pps_time[0]) {
			val = atoi(strhandover_pps_time);
			if (val < 0 || val > 100) {
				strcpy(tmpBuf, ("Error! Invalid Handover PPS checking time."));
				goto setErr_advance;
			}
			sprintf(handover_pps_time, "%d", val);

			yfecho("/proc/sys/private/bs/pps_checking_time",
		    	   O_WRONLY | O_CREAT | O_TRUNC, 0644, "%d", val);

			apmib_nvram_set("x_bs_tcp_pps_check_time", handover_pps_time);
		}
	}


	if (wp->superUser == 1) {
		strhandover_tcp_pkts_cnt = req_get_cstream_var(wp, ("x_bs_tcp_pkts_threshold"), "");
		if (strhandover_tcp_pkts_cnt[0]) {
			val = atoi(strhandover_tcp_pkts_cnt);
			if (val <= 0 || val > 100) {
				strcpy(tmpBuf, ("Error! Invalid Handover PPS checking time."));
				goto setErr_advance;
			}
			sprintf(handover_tcp_pkts_cnt, "%d", val);

			yfecho("/proc/sys/private/bs/tcp_pkts_threshold",
		       		O_WRONLY | O_CREAT | O_TRUNC, 0644, "%d", val);

			apmib_nvram_set("x_bs_tcp_pkts_threshold", handover_tcp_pkts_cnt);
		}
	}

	/*
	strCca_mode = req_get_cstream_var(wp, ("x_wlan_cca_mode"), "");
	if (strCca_mode[0]) {
		get_wlan_name(cca_name, i, "CCA_MODE");
		apmib_nvram_set(cca_namem, strCca_mode);
	}*/

	if (wp->superUser == 1) {
		for (i = 0; i < 5; i++) {
			if (i == 1 || i == 4) // VoIP SSID, wlan_vap3
				continue;
			sprintf(dv_name, "rssiThresh%d", i);
			get_wlan_name(rssi_name, i, "rssi_threshold");
			rssi_threshold = req_get_cstream_var(wp, dv_name, "");
			apmib_nvram_set(rssi_name, rssi_threshold);

			if (wlan_idx == 1 && i == 0)
				nvram_set("x_wlan0_va3_rssi_threshold", rssi_threshold);
		}
	}
#endif
#ifdef WIFI_SIMPLE_CONFIG
...
int wlAcMACList(request *wp, int argc, char **argv)
{
...
int wlAcCommentList(request *wp, int argc, char **argv)
{
	int nBytesSent=0, entryNum, i, old;
	MACFILTER_T entry;
	char tmpBuf[100];
	...
	if ( !apmib_get(MIB_WLAN_MACAC_NUM, (void *)&entryNum)) {
		...
		for (i=1; i<=entryNum; i++) {
			*((char *)&entry) = (char)i;
			if ( !apmib_get(MIB_WLAN_MACAC_ADDR, (void *)&entry))
				break;
			escape_special(tmpBuf, entry.comment, sizeof(tmpBuf));
...
void formAdvanceSetup(request *wp, char *path, char *query)
{
	...
	web_config_trace(2, 2);		/* wireless/advanced */
...
{
#ifdef __DAVO__
	int nBytesSent=0, i, found=0, link_time_len, disClientFlag=0, macSelectFlag=0;
	WLAN_STA_INFO_Tp pInfo;
	char *buff;
	char mode_buf[20], link_time_buf[32];
	char txrate[20];
	int rateid=0;
	unsigned long m, n;
	int virtual_index;
	...
	if( argc>=1 && strcmp(argv[0],"disClient")==0 )
		disClientFlag=1;
	else
		disClientFlag=0;

	if( argc>=1 && strcmp(argv[0],"wlMacSelect")==0 )
		macSelectFlag=1;
	else
		macSelectFlag=0;
	...
	if (argc == 2) {
		...
		if(virtual_index == 9) { /*skb_status_htm fucntion add*/
			sprintf(WLAN_IF, "wlan0");
			sprintf(Root_WLAN_IF, WLAN_IF);
			virtual_index = 3;   /*5G_vap3*/
		}
#ifdef CONFIG_RTL8196B_GW_8M
		if (virtual_index > 0) {
			strcpy(WLAN_IF, tmp_if);
			if (buff)
				free(buff);
			return 0;
		}
	...
	if (disClientFlag==1) { // display to connect to auth(radius)
		char auth_pid_path[32];
		sprintf (auth_pid_path, "/var/run/auth-%s.pid", WLAN_IF);
		if (access(auth_pid_path, F_OK)) { // not radius
#ifdef MBSSID
			if (argc == 2)
	...
	for (i=1; i<=MAX_STA_NUM; i++) {
		...
		link_time_len=0;
		m = pInfo->link_time / 86400;
		n = pInfo->link_time % 86400;
		if(m)
			link_time_len += sprintf(link_time_buf, "%lud ", m);
		m = n / 3600;
		n = n % 3600;
		if(m)
			link_time_len += sprintf(link_time_buf + link_time_len, "%luh ", m);
		m = n / 60;
		n = n % 60;
		if(m)
			link_time_len += sprintf(link_time_buf + link_time_len, "%lum ", m);
		link_time_len += sprintf(link_time_buf + link_time_len, "%lus", n);

		if(disClientFlag){
			nBytesSent += req_format_write(wp,
			("<tr bgcolor=#f0f0f0 align=\"center\"><td align=\"center\" ><font size=2>%02x:%02x:%02x:%02x:%02x:%02x</td>\n"
			"<td align=\"center\"><font size=2>%s</td>\n"
			"<td align=\"center\"><font size=2><script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n"
			"<td align=\"center\"><font size=2><script>document.write(get_data_size_converter(%lu, %lu));</script></td>\n"
			"<td align=\"center\"><font size=2>%s</td>\n"
			"<td align=\"center\"><font size=2>%s</td>\n"
			"<td align=\"center\"><font size=2>%ds</td>\n"
			"<td align=\"center\"><font size=2>%s</td>\n"
			"<td align=\"center\"><font size=2> <input type='submit' value='차단' name='disc_sta' onClick=\"return selectMac('%02x:%02x:%02x:%02x:%02x:%02x')\"></td>\n"
			"</tr>"),
			pInfo->addr[0],pInfo->addr[1],pInfo->addr[2],pInfo->addr[3],pInfo->addr[4],pInfo->addr[5],
			mode_buf,
			pInfo->tx_only_data_bytes_high, pInfo->tx_only_data_bytes, // change information (tx_packets -> tx_only_data_bytes) (byte -> kbyte)
			pInfo->rx_only_data_bytes_high, pInfo->rx_only_data_bytes,
			txrate,
			( (pInfo->flags & STA_INFO_FLAG_ASLEEP) ? "예" : "아니오"),
			pInfo->expired_time/100,
			link_time_buf, // add client link time
			pInfo->addr[0],pInfo->addr[1],pInfo->addr[2],pInfo->addr[3],pInfo->addr[4],pInfo->addr[5]
			);
		}
		else {
			if(macSelectFlag){
				nBytesSent += req_format_write(wp,
				("<tr bgcolor=#f0f0f0 align=\"center\"><td><font size=2>%02x:%02x:%02x:%02x:%02x:%02x</td>"
				"<td><font size=2>%s</td>"
				"<td><font size=2><script>document.write(get_data_size_converter(%lu, %lu));</script></td>"
				"<td><font size=2><script>document.write(get_data_size_converter(%lu, %lu));</script></td>"
				"<td><font size=2>%s</td>"
				"<td><font size=2>%ds</td>"
				"<td><font size=2>%s</td>"
				"<td><font size=2><input type='radio' name='macChecked' value='%02x:%02x:%02x:%02x:%02x:%02x' onClick=macInput('%02x:%02x:%02x:%02x:%02x:%02x')></td></tr>"),
				pInfo->addr[0],pInfo->addr[1],pInfo->addr[2],pInfo->addr[3],pInfo->addr[4],pInfo->addr[5],
				mode_buf,
				pInfo->tx_only_data_bytes_high, pInfo->tx_only_data_bytes, // change information (tx_packets -> tx_only_data_bytes) (byte -> kbyte)
				pInfo->rx_only_data_bytes_high, pInfo->rx_only_data_bytes,
				...
				calc_use_data(tx_data_buf, pInfo->tx_only_data_bytes_high, pInfo->tx_only_data_bytes);
				calc_use_data(rx_data_buf, pInfo->rx_only_data_bytes_high, pInfo->rx_only_data_bytes);
				nBytesSent += req_format_write(wp,
				("<tr align=\"center\" class=\"tbl_body\"><td><font size=2>%02x:%02x:%02x:%02x:%02x:%02x</td>"
				 "<td><font size=2>%s</td>"
				 "<td><font size=2>%s</td>"
				 "<td><font size=2>%s</td>"
				 "<td><font size=2>%s</td>"
				...
				pInfo->addr[0],pInfo->addr[1],pInfo->addr[2],pInfo->addr[3],pInfo->addr[4],pInfo->addr[5],
				mode_buf,
				tx_data_buf, rx_data_buf,
				txrate,
				CONV_TO_RSSI(pInfo->rssi),
	...
	if (found == 0) {
		if(!disClientFlag && !macSelectFlag){
			nBytesSent += req_format_write(wp,
	   		("<tr align=\"center\" class=\"tbl_body\"><td><font size=2>없음</td>"
			"<td><font size=2>---</td>"
		    "<td><font size=2>---</td>"
			"<td><font size=2>---</td>"
			"<td><font size=2>---</td>"
	...
	return nBytesSent;
#else
	int nBytesSent=0, i, found=0, link_time_len;
	WLAN_STA_INFO_Tp pInfo;
	char *buff;
	char mode_buf[20], link_time_buf[32];
	char txrate[20];
...
/////////////////////////////////////////////////////////////////////////////
const char * statbl_list[] = {
	"/skb_wlstatbl.htm",
	"/skb_wlstatbl_vap.htm",
	NULL
};
...
void formWirelessTbl(request *wp, char *path, char *query)
{
	...
	submitUrl = req_get_cstream_var(wp, ("submit-url"), "");
	if (submitUrl[0]) {
		for (i = 0; statbl_list[i] != NULL; i++) {
			if (strstr(submitUrl, statbl_list[i])) {
				white = 1;
				break;
			}
		}
		send_redirect_perm(wp, white ? submitUrl : "/skb_home.htm");
	}
}
...
void formWlanMultipleAP(request *wp, char *path, char *query)
{
	...
	for (idx = 1; idx <= 4; idx++) {
		if (idx == 1)
			continue;
		/* user setting only vwaln_idx 4 */
		if (wp->superUser != 1 && idx != 4)
			continue;

		/* prevent handover setting */
		if (wlan_idx == 0 && idx == 4)
			continue;
		...
		if (strVal[0]) {
			...
			if (wlan_onoff_tkip == 0) { //Wifi request
				...
				if (band_no == 7 || band_no == 9 || band_no == 10 || band_no == 11 ||
				    band_no == 63 || band_no == 71 || band_no == 75) { //7:n; 9:gn; 10:bgn 11:5g_an
					if (wlan_encrypt == ENCRYPT_WPA || wlan_encrypt == ENCRYPT_WPA2) {
						wpaCipher &= ~WPA_CIPHER_TKIP;
						if (wpaCipher == 0)
							wpaCipher =  WPA_CIPHER_AES;
		...
		sprintf(varName, "x_wlan%d_va%d_max_conn", wlan_idx, idx - 1);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			apmib_nvram_set(varName, strVal);
			nvram_commit();
		}
	...
	OK_MSG("/skb_wlmultipleap.htm");
	need_reboot = 1;
	return;

	//submitUrl = req_get_cstream_var(wp, ("submit-url"), "");
	submitUrl = "skb_wlmultipleap.htm";
...
*/
		...
		if (submitUrl[0])
			send_redirect_perm(wp, "/skb_pocket_sitesurvey.htm");
...
int wlSiteSurveyTbl(request *wp, int argc, char **argv)
{
	...
	if (argc == 1 && argv[0] != NULL) {
		if (strncmp(argv[0], "search_ap", strlen("search_ap"))==0) {
			repeater_search = 1;
		}
	}

	if (repeater_search == 1) {
		while(1)	{
...
//#ifdef CONFIG_RTK_MESH
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
	"<td align=center width=\"%d%%\" ><font size=\"2\"><b>SSID</b></font></td>\n"
	...
	if (repeater_search == 1) {
		nBytesSent += req_format_write(wp, ("<td align=center width=\"5%%\" ><font size=\"2\"><b>선택</b></font></td>\n"));
	}
	...
	for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) {
...
#endif
		{
			...
			((pBss->bdCap & cIBSS) ? "Ad hoc" : "AP"), tmp2Buf, CONV_TO_RSSI(pBss->rssi));

			if (repeater_search == 1) {
				nBytesSent += req_format_write(wp, ("<td align=center width=\"10%%\" >"
				"<input type='radio' name='checkSSID' value='%s' onclick=check_ssid('%s','%s','%s','%s')></td>\n"),
				ssidbuf, ssidbuf, tmp2Buf, wpa_tkip_aes, wpa2_tkip_aes);
			}
		}
	...
	if( pStatus->number == 0 )
	{
		...
		)
		{
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
					...
					"<td align=center width=\"10%%\" ><font size=\"2\"></td>\n"));
			if (repeater_search == 1) {
				nBytesSent += req_format_write(wp, ("<td align=center width=\"10%%\" ><font size=\"2\"></td>\n"));
			}
			nBytesSent += req_format_write(wp, ("</tr>\n"));
		}
		else
...
#ifdef __DAVO__
void formWlReset(request * wp, char *path, char *query)
{
	char *strVal;
	char *submitUrl;
	...
	apmib_set_hist_clear();		/* APACRTL-85 */
	strVal = req_get_cstream_var(wp, "wl_reset_enable_", "");
	if (strVal[0]) {
		apmib_nvram_set("x_wlan_reset_enable", strVal);
	}

	strVal = req_get_cstream_var(wp, "wl_reset_interval_", "");
	if (strVal[0]) {
		apmib_nvram_set("x_wlan_reset_interval_day", strVal);
	}

	strVal = req_get_cstream_var(wp, "wl_reset_bw_kbps_", "");
	if (strVal[0]) {
		apmib_nvram_set("x_wlan_reset_bw_kbps", strVal);
	}

	strVal = req_get_cstream_var(wp, "wl_reset_range", "");
	if (strVal[0]) {
		apmib_nvram_set("x_wlan_reset_triger_time", strVal);
	}

	web_config_trace(2, 8);		/* wireless/periodic_reset */
	nvram_commit();
#ifndef NO_ACTION
	run_init_script("all");
#endif
	...
	need_reboot = 1;
	submitUrl = req_get_cstream_var(wp, ("submit-url"), "");	// hidden page
	OK_MSG("/skb_wlreset.htm");
}

extern unsigned int g_daa_hist;
extern unsigned int g_daa_status;
void formDaa(request *wp, char *path, char *query)
{
    char *submitUrl;
    char tmpBuf[100]="지원되지 않는 명령어입니다!\n";
	char *strVal;
	int mode;
	char cmd[80];
	char filesrt[80];
	char str_rfi_test_mode[80];

	if (wp->superUser != 1)
		return;

	submitUrl = req_get_cstream_var(wp, "submit-url", "/skb_daa.htm");

	if ( (strVal = req_get_cstream_var(wp, "run_do_it", "")) && strVal[0]!=0 ) {
		if ( (strVal = req_get_cstream_var(wp, "wl_detect_mode", "")) )
			mode = strtoul(strVal, NULL, 10);
		if ( mode >= 0 && mode <= 5) {
			sprintf(str_rfi_test_mode, "/proc/wlan%d/rfi_test_mode", wlan_idx);
			yfecho(str_rfi_test_mode, O_WRONLY|O_TRUNC, 0644, "%d", mode);
			//vfecho(str_rfi_test_mode, "%d", mode);

			DO_APPLY_WAIT("/skb_daa.htm");
			return;
		}
	}

	if ( (strVal = req_get_cstream_var(wp, "mirror_daa_hist", "")) && strVal[0]!= 0 ) {
		g_daa_hist = strtoul(strVal, NULL, 10);
		send_redirect_perm(wp, "/skb_daa.htm");
		return;
	}

	if ( (strVal = req_get_cstream_var(wp, "mirror_daa_status", "")) && strVal[0]!= 0 ) {
		g_daa_status = strtoul(strVal, NULL, 10);
		send_redirect_perm(wp, "/skb_daa.htm");
		return;
	}
	ERR_MSG(tmpBuf);
...
static const char *get_decting_mode(int mode)
{
	switch (mode) {
	case 0:
		return "동작 정지 상태";
	...
	case 1:
		return "<font color=blue><b>무선 환경 검사 진행 중...</font>";

	case 2:
		return "<font color=blue><b>연속 무선 환경 검사 진행 중...</font>";

	case 3:
		return "<font color=blue><b>무선 간섭 회피[1] 진행 중...</font>";

	case 4:
		return "<font color=blue><b>무선 간섭 회피[2] 진행 중...</font>";

	case 5:
		return "<font color=blue><b>무선 간섭 회피[3] 진행 중...</font>";
	default:
		return "알수없음";
	}
}

int wlDetectAvoid(request *wp, char *path, char *query)
{
	FILE *fp;
	char *n, *v, *p;
	char rfi_itf_score[80];
	int nBytesSent = 0;
	int rfi_test_mode = 0;
	int num, i = 0;
	char rtime[30], buf[128];
	unsigned int dur, fa, cca, mac, rfi;
	char str_tmp[80];

	sprintf(str_tmp, "/proc/wlan%d/rfi_test_mode", wlan_idx);
	if ( !(fp=fopen(str_tmp, "r")) )
		return -1;
	...
	while(fgets(buf, sizeof(buf), fp)) {
		if ( i < 2) {
			...
			ydespaces(n);
			ydespaces(v);
			if ( i == 0 && !strcasecmp(n, "mode")) {
				rfi_test_mode = strtoul(v, NULL, 10);

				nBytesSent += req_format_write(wp, "<tr bgcolor=\"#FFFFFF\">");
				nBytesSent += req_format_write(wp, "<td width=\"25%%\" align=\"left\" nowrap bgcolor=\"#FFFFFF\" >\n");
				nBytesSent += req_format_write(wp, "현재 진행 모드: </td>\n");
				nBytesSent += req_format_write(wp, "<td align=\"left\">\n");
				nBytesSent += req_format_write(wp, "%s</td>\n", get_decting_mode(rfi_test_mode));
				nBytesSent += req_format_write(wp, "</tr>\n");
			}
			else if ( i == 1 && strstr(n, "interference")) {
				sprintf(&rfi_itf_score[0], "%s", v);

				nBytesSent += req_format_write(wp, "<tr bgcolor=\"#FFFFFF\">");
				nBytesSent += req_format_write(wp, "<td width=\"25%%\" align=\"left\" nowrap bgcolor=\"#FFFFFF\" >\n");
				nBytesSent += req_format_write(wp, "RF interference score: </td>\n");
				nBytesSent += req_format_write(wp, "<td align=\"left\">\n");
				nBytesSent += req_format_write(wp, "%s</td>\n", rfi_itf_score);
				nBytesSent += req_format_write(wp, "</tr></table>\n");
			}
		}

		if ( i > 3) {
			if (i == 4) {
				nBytesSent += req_format_write(wp, "<table width=\"650\" cellpadding=\"3\" cellspacing=\"1\" border=\"1\">\n");
				nBytesSent += req_format_write(wp, "<tr bgcolor=\"#FFFFFF\">\n");
				nBytesSent += req_format_write(wp, "<td width=\"25%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >초당 FA count</td>\n");
		        nBytesSent += req_format_write(wp, "<td align=\"left\"><b>0 ~ 150:</b> 간섭이 없거나 성능에 지장이 없는 수준 임<br>\n");
		        nBytesSent += req_format_write(wp, "<b>150 ~ 500:</b> 약한 간섭 환경으로 성능에 약간 영향이 있으며,<br>특히 약전계에서 영향 있음.<br>\n");
		        nBytesSent += req_format_write(wp, "<b>500 ~ 2000:</b> 강한 간섭 환경으로 성능에 영향이 있음.<br>아주 근거리가 아니면 성능 저하가 심하게 발생함.<br>\n");
				nBytesSent += req_format_write(wp, "<b>2000 이상:</b> 아주 강한 간섭 환경으로 강전계에서도 심각한 무선 성능 저하발생.</td></tr>\n");
				nBytesSent += req_format_write(wp, "<tr bgcolor=\"#FFFFFF\">\n");
		        nBytesSent += req_format_write(wp, "<td width=\"25%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >CCA count</td>\n");
		        nBytesSent += req_format_write(wp, "<td align=\"left\">FA count와 MAC event count에 양의 상관관계 있음.<br>따라서 CCA count는 참고용으로만 사용.</td></tr>\n");

		        nBytesSent += req_format_write(wp, "<tr bgcolor=\"#FFFFFF\">\n");
		        nBytesSent += req_format_write(wp, "<td width=\"25%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >초당 MAC event count</td>\n");
		        nBytesSent += req_format_write(wp, "<td align=\"left\"><b>0 ~ 100:</b> 간섭이 없거나 성능에 지장이 없는 수준임.<br>\n");
		        nBytesSent += req_format_write(wp, "<b>100 ~ 400:</b> 약한 간섭 환경으로 간헐 적으로 속도 저하를 느낄 수 있음.<br>\n");
		        nBytesSent += req_format_write(wp, "<b>500 ~ 2000:</b> 강한 간섭 환경으로 성능에 영향이 있음. <br>아주 근거리가 아니면 성능 저하가 심하게 발생함.<br>\n");
		        nBytesSent += req_format_write(wp, "<b>2000 이상:</b> 타 무선 장비가 동일 채널을 50%%이상 점유하고 있음.</td></tr>\n");
		        nBytesSent += req_format_write(wp, "</table>\n");

				nBytesSent += req_format_write(wp, "<table width=\"650\" cellpadding=\"3\" cellspacing=\"1\" border=\"1\"><tr>\n"
		        "<td width=\"10%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >시간</td>\n"
		        "<td width=\"18%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >측정시간(초)</td>\n"
		        "<td width=\"18%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >\n"
		        "<abbr title=\"전송이 수행되지 않음에도 CCA 검사 시 채널이 사용 중으로 판단되는 경우 증가한다. 주로 인접채널 간섭 또는 주파수 대역을 공유하는 다른 무선 기기의 간섭에 의해 증가한다.\">FA</abbr></td>\n"
		        "<td width=\"18%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >\n"
		        "<abbr title=\"Clear Channel Assessment count로 Channel 상태가 Busy channel에서 clear channel로 전환될 때 증가한다.\"> CCA</abbr></td>\n"
		        "<td width=\"18%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >\n"
		        "<abbr title=\"유효한 802.11 frame이 송/수신될 경우 증가한다. 동일 채널 간섭을 검출하는 척도로 사용된다.\">MAC-RX</abbr></td>\n");

		        nBytesSent += req_format_write(wp, "<td width=\"18%%\" align=\"center\" nowrap bgcolor=\"#C1C1C1\" >\n"
		        							"<abbr title=\"해당 시간동안의 RFI score임.\">RFI_LV</abbr></td>\n");
			    nBytesSent += req_format_write(wp, "</tr>\n");
			    nBytesSent += req_format_write(wp, "<tr>"
			            "<td colspan=\"7\" style=\"height:1px;line-height:1px;font-size:1px;background-color:#C1C1C1;padding:0px;\" ></td></tr>\n");
			}
			if ( (num=sscanf(buf, "%s %u %u %u %u %u", rtime, &dur, &fa, &cca, &mac, &rfi)) == 6) {
				nBytesSent += req_format_write(wp, "<tr>"
	            "<td align=\"center\" width=\"10%%\" nowrap bgcolor=\"#FFFFFF\" >%s</td>\n"
	            "<td align=\"center\" width=\"18%%\" nowrap bgcolor=\"#FFFFFF\" >%u</td>\n"
	            "<td align=\"center\" width=\"18%%\" nowrap bgcolor=\"#FFFFFF\" >%u</td>\n"
	            "<td align=\"center\" width=\"18%%\" nowrap bgcolor=\"#FFFFFF\" >%u</td>\n"
	            "<td align=\"center\" width=\"18%%\" nowrap bgcolor=\"#FFFFFF\" >%u</td>\n"
	            "<td align=\"center\" width=\"18%%\" nowrap bgcolor=\"#FFFFFF\" >%u</td>\n",
	            rtime, dur, fa, cca, mac, rfi);

				nBytesSent += req_format_write(wp, "</tr>\n");

				nBytesSent += req_format_write(wp, "<tr>"
				"<td colspan=\"7\" style=\"height:1px;line-height:1px;font-size:1px;background-color:#E1E1E1;padding:0px;\" ></td></tr>\n");
			}
		}
		i++;
	}
...
const char * security_weblist[] = {
	"/skb_wlsecurity_redirect.htm",
	"/skb_wlsecurity_disclient.htm",
	NULL
};

void formRedirWlEncrypt(request * wp, char *path, char *query)
{
	char *submitUrl;
	char tmpBuf[100];
	char wlan_name[80];
	...
	submitUrl = req_get_cstream_var(wp, ("submit-url"), "");	// hidden page
	if (submitUrl[0]) {
		for (i = 0; security_weblist[i] != NULL; i++) {
			if (strcmp(submitUrl, security_weblist[i]) == 0) {
				white = 1;
				break;
			}
		}
		if (white == 0)
			submitUrl = "/skb_home.htm";
	}

	apmib_set_hist_clear();		/* APACRTL-85 */
	strVal = req_get_cstream_var(wp, "disc_all", "");
	if (strVal[0]) {
		//disconnect button pressed
		system("echo all > /var/tmp/disc_sta");
		system("killall -USR2 auth");	// kick-off all auth-ed STAs without reboot
		send_redirect_perm(wp, submitUrl);
		return;
	}
	strVal = req_get_cstream_var(wp, "disc_sta", "");
	if (strVal[0]) {
		unsigned char mac[6];
		//disconnect button pressed
		strVal = req_get_cstream_var(wp, "disc_sta_mac", "");
		if (strVal[0] && get_mac_addr(strVal, mac)) {
			sprintf(tmpBuf, "echo %02x:%02x:%02x:%02x:%02x:%02x > /var/tmp/disc_sta",
				mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
			system(tmpBuf);
			system("killall -USR2 auth");	// kick-off all auth-ed STAs without reboot
			send_redirect_perm(wp, submitUrl);
			...
			strcpy(tmpBuf, "클라이언트 MAC 주소가 올바르지 않습니다.");
			goto setErr_end;
		}
	}

	strVal = req_get_cstream_var(wp, "redirect_enable", "");
	get_wlan_name(wlan_name, 0, "WLS_REDIR_ENABLE");
	if (strVal[0] && (strcmp(strVal, "ON") == 0))
		apmib_nvram_set(wlan_name, "1");
	else
		apmib_nvram_unset(wlan_name);

	strVal = req_get_cstream_var(wp, "redirect_host", "");
	strVal1 = NULL;
	if (strVal[0])
		strVal1 = strtok(strVal, " \t");

	get_wlan_name(wlan_name, 0, "WLS_REDIR_HOST");
	if (strVal1)
		apmib_nvram_set(wlan_name, strVal1);
	else
		apmib_nvram_unset(wlan_name);

	init_abuffer(&m, 160);
	for (i = 0; i < 5; i++) {
		sprintf(varnam, "allow_list_%d", i);
		strVal = req_get_cstream_var(wp, varnam, "");
		if (strVal && strVal[0])
			strVal = strtok(strVal, " \t");
		sprintf(varnam, "WLS_REDIR_ALLOW%d", i);
		get_wlan_name(wlan_name, 0, varnam);
		aprintf(&m, "%s ", nvram_safe_get(wlan_name));
		if (!strVal || !strVal[0])
			apmib_nvram_unset(wlan_name);
		else
			apmib_nvram_set(wlan_name, strVal);
	}

	web_config_trace(2, 4, m.buf);		/* wireless/redirection */
	fini_abuffer(&m);
...
#ifdef __DAVO__
			strcpy(txrate, getwlratebyid(tx_fixed_rate, entry.fixedTxRate) ? : "");
#else
...
struct fregex {
	...
	regex_t regex;
	char buffer[1024];
	const char *rexpr, *path;
};

static char *apmib_nvram_fgets(struct fregex *arg, int unused)
{
	char *p;

	if (arg->f == NULL) {
		arg->f = fopen(arg->path, "r");
		if (arg->f == NULL)
			return NULL;
		if (regcomp(&arg->regex, arg->rexpr, REG_EXTENDED | REG_NEWLINE)) {
			fclose(arg->f);
			arg->f = NULL;
			return NULL;
		}
	...
	p = fgets(arg->buffer, sizeof(arg->buffer), arg->f);
	if (p)
		ydespaces(p);
	return p;
}

static int apmib_nvram_rmatch(const char *string, struct fregex *arg)
{
	regmatch_t match[6];
	return !regexec(&arg->regex, string, _countof(match), match, 0);
}

static int apmib_nvram_rset_dfl(const char *regex, const char *path)
{
	struct fregex arg = { .f = NULL, .path = path, .rexpr = regex };
	struct mib_iterator it;
	char **pp, *eq;
	int i;

	it.priv_data = (void *)&arg;
	it._fetch = (void *)apmib_nvram_fgets;
	it._match = (void *)apmib_nvram_rmatch;

	if ((pp = apmib_iterate(&it))) {
		for (i = 0; pp[i]; i++) {
			eq = strchr(pp[i], '=');
			if (eq)
				ynvram_put(pp[i]);
...
void apmib_reset_wlan_to_default(char *wlanif_name)
{
	char expr[32];

	SetWlan_idx(wlanif_name);
	if (vwlan_idx == 0)
		sprintf(expr, "^WLAN%d_[^V][^A][^P]", wlan_idx);
	else
		sprintf(expr, "^WLAN%d_VAP%d_", wlan_idx, vwlan_idx);
	apmib_nvram_rset_dfl(expr, apmib_file_dfl(RUN_DFL));
}
#else
void apmib_reset_wlan_to_default(char *wlanif_name)
{
...
#ifdef CONFIG_NVRAM_APMIB
		wlanDisabled[0] = atoi(nvram_safe_get("WLAN0_WLAN_DISABLED"));
		wlanDisabled[1] = atoi(nvram_safe_get("WLAN1_WLAN_DISABLED"));
		wlanMode[0] = atoi(nvram_safe_get("WLAN0_MODE"));
		wlanMode[1] = atoi(nvram_safe_get("WLAN1_MODE"));
		wlanMacPhy[0] = atoi(nvram_safe_get("WLAN0_MAC_PHY_MODE"));
		wlanMacPhy[1] = atoi(nvram_safe_get("WLAN1_MAC_PHY_MODE"));
#else
...
#ifdef CONFIG_NVRAM_APMIB
			ynvram_put("WLAN0_WLAN_DISABLED=%d", wlanDisabled[0]);
			ynvram_put("WLAN0_MAC_PHY_MODE=%d", wlanMacPhy[0]);
#else
...
#ifdef CONFIG_NVRAM_APMIB
			ynvram_put("WLAN1_WLAN_DISABLED=%d", wlanDisabled[1]);
			ynvram_put("WLAN1_MAC_PHY_MODE=%d", wlanMacPhy[1]);
#else
	...
	if (strVal[0]) {
		...
		if (intVal) {
			...
			if (local_pin_changed) {
				...
				/* support  special MAC , 2011-0505 ;WPS2DOTX*/
				if(targetAPMac[0]){
					char targetAPMacFilter[20];
					int idx = 0;
					int idx2 = 0;
					//printf("before ,mac =%s len=%d \n",targetAPMac , strlen(targetAPMac));
					for(;idx<strlen(targetAPMac);idx++){
...
static const char *white_list[] = {
	...
	"skb_wlsurvey.htm",
	"skb_wlactrl.htm",
	"skb_repeater_search_ap.htm",
	"skb_wlsecurity_redirect.htm",
	"skb_daa.htm",
	"skb_sub_menu_wlan.htm",
	"skb_wlmultipleap.htm",
	NULL
};
...
void formWlanRedirect(request *wp, char *path, char *query)
{
	...
	for (i = 0; white_list[i]; i++) {
		if (strcmp(white_list[i], redirectUrl) == 0) {
			white = 1;
			break;
		}
	}
	...
	if (white == 0) {
		send_redirect_perm(wp, "/skb_home.htm");
		return;
	}
...
void formWlanRedirect2(request *wp, char *redirectUrl, char *strWlanId)
{
	...
	if (white == 0) {
		send_redirect_perm(wp, "/skb_sub_menu_wlan.htm");
		return;
	}
...
#ifdef __DAVO__
int print_multipleApMenu(request *wp, int argc, char **argv)
{
	int nBytesSent = 0;

	/* only 2.4G */
	if (wlan_idx == 1) {
		nBytesSent += req_format_write(wp, ("\t<tr>\n<td width=\"26%%\"><font size=2><b>Multi SSID:</b></td>\n"
								"\t\t<td width=\"74%%\"><font size=2>\n"
								"\t\t\t<input type=\"button\" value=\"설정\" name=\"multipleAP%d\" onClick=\"showMultipleAP(document.wlanSetup, '/skb_wlmultipleap.htm')\">\n"
								"\t\t</td></tr>\n"), wlan_idx);
	}
	return nBytesSent;
}
...
int print_wme_dscp(request *wp, int argc, char **argv)
{
	...
	char buf[128], *ptr;
	int  pri[8];
	char cmd[80], fpath[80];

	sprintf(fpath, "/var/wme_dscp%d", wlan_idx);
	sprintf(cmd, "cat /proc/dv_wlan%d/wme_dscp > %s", wlan_idx, fpath);
	system(cmd);
	fp = fopen(fpath, "r");
...
int print_wme_1p(request *wp, int argc, char **argv)
{
    int nBytesSent = 0;
    FILE *fp;
    int  i, j;
	...
	sprintf(fpath, "/var/web/wme_1p%d", wlan_idx);
    sprintf(cmd, "cat /proc/dv_wlan%d/wme_1p > %s", wlan_idx, fpath);
    system(cmd);
    ...
    if ( (fp = fopen(fpath, "r")) ) {
        ...
        while (fgets(buf, sizeof(buf), fp)) {
            nBytesSent += req_format_write(wp, "<tr align=center class=\"content\" height=\"20\" bgcolor=#DDDDDD>\n");
			nBytesSent += req_format_write(wp, "<td width=250>PRI[0 - 7]</td>");
            ptr = strstr(buf, ": ");
            if (ptr != NULL && strlen(ptr) > 2) {
                ptr += 2;
                memset(pri, 0, sizeof(pri));
                ...
                //@NOTE: revision wmm mapping table-R/W
                for (i = 0 ; i < 8; i++) {
	                nBytesSent += req_format_write(wp, "<td width=30><select name=\"wmm_1p_%d\">"
	                            "<option value=\"0\" %s>0"
	                            "<option value=\"1\" %s>1"
	                            "<option value=\"2\" %s>2"
	                            "<option value=\"3\" %s>3"
...
void formWlwmm(request *wp, char *path, char *query)
{
	int dscpIdx, n;
	char *p, *pCmd;
	char tmpBuf[512], param[10], setup_path[80];
	char wme_1p_path[32], wmm_name[32], str_wmm_mode[8];
	int wmm_mode;
	FILE *fp;
	...
	if ( (p=req_get_cstream_var(wp, "saveApply", ""))&& strlen(p) > 0 ) {
		tmpBuf[0] = 0;
		pCmd = &tmpBuf[0];
		p=req_get_cstream_var(wp, "wmm_mode", "2");
		wmm_mode = strtoul(p, NULL, 10);

		if ( wmm_mode != 1 && wmm_mode != 2) {
			sprintf(tmpBuf, "wmm mode: wrong value(%d)\n", wmm_mode);
			goto setErr_end;
		}

		if ( wmm_mode == 1) {
			pCmd[0]=0;
			sprintf(wme_1p_path, "/var/web_wme_1p%d", wlan_idx);
	        if ( (fp=fopen(wme_1p_path, "w")) ) {
		        for (dscpIdx = 0; dscpIdx < DAVO_WME_RULE_SIZE_802_1P; dscpIdx++) {
		            sprintf(param, "wmm_1p_%d", dscpIdx);
		            p=req_get_cstream_var(wp, param, "");
		            if (strlen(p) == 0) {
		                sprintf(tmpBuf, "DSCP setting value is invalid");
		                goto setErr_end;
		            }
		            fprintf(fp, "%d:%d\n", dscpIdx, atoi(p));
		        }
		    ...
		    sprintf(pCmd, "wmmmap -m %d -p -i %d -o -c %s", wmm_mode, wlan_idx, wme_1p_path);
	        system(pCmd);
		} else {
			...
			sprintf(pCmd, "wmmmap -m %d -d -i %d -o -c %s", wmm_mode, wlan_idx, setup_path);
			system(pCmd);
		}
		sprintf(wmm_name, "x_wlan%d_wme_mode", wlan_idx);
		sprintf(str_wmm_mode, "%d", wmm_mode);
		nvram_set(wmm_name, str_wmm_mode);
		nvram_commit();
	}

	send_redirect_perm(wp, "/skb_wlwmm.htm");
...
//////MENU//////////////////////////////////////////////////
	...
	{
		sprintf(buffer,"%s", (char *)entry.ssid);
...
void formwlprodelete(request *wp, char *path, char *query)
{
	char *submitUrl, *strVal;
	char buffer[128], buffer2[128], list[128];
	char value[128];
	int entryNum, i, j = 1, count = 0;
	int delete_flag = 0;
	char *args[12] = {NULL,};

	nvram_get_r_def("sta_protection_num", buffer, sizeof(buffer), "0");
	entryNum = strtol(buffer, NULL, 10);

	//sta_protection_list1=000852112255,127,0,0,0,10,2,test,0
	for (i = 0; i < entryNum; i ++) {
		snprintf(buffer, sizeof(buffer), "cpeuse%d", i);
		strVal = req_get_cstream_var(wp, buffer, "");
		snprintf(buffer, sizeof(buffer), "sta_protection_list%d", (i + 1));
		nvram_get_r_def(buffer, value, sizeof(value), "");
		if (strVal[0]) {
			if (value[0]) {
				value[strlen(value) - 1] = '1';
				nvram_set(buffer, value);
			}
		} else {
			if (value[0]) {
				value[strlen(value) - 1] = '0';
				nvram_set(buffer, value);
				if (ystrargs(value, args, _countof(args), ",", 0) > 7)
					syslog(LOG_INFO, "[Child_Guard] Station %s Allow!.", args[0]);
			}
		}
	}

	for (i = 0; i < entryNum; i ++) {
		snprintf(buffer, sizeof(buffer), "select%d", i);
		strVal = req_get_cstream_var(wp, buffer, "");
		if (strVal[0]) {
			count++;
			snprintf(buffer, sizeof(buffer), "sta_protection_list%d", (i + 1));
	...
	if (delete_flag) {
		/* rearrange sta_protection list */
		for (i = 0; i < entryNum; i ++) {
			snprintf(buffer, sizeof(buffer), "sta_protection_list%d", (i + 1));
			if (nvram_get_r(buffer, list, sizeof(list))) {
				snprintf(buffer2, sizeof(buffer2), "sta_protection_list%d", j);
				nvram_set(buffer2, list);
				j++;
				if ((i + 1) != (j - 1))
					nvram_unset(buffer);
			}
		...
		if (entryNum == 0) {
			...
			syslog(LOG_INFO, "[Child_Guard] All Station Delete!.");
		} else {
			snprintf(buffer, sizeof(buffer), "%d", entryNum);
			nvram_set("sta_protection_num", buffer);
		}
	}
	...
	submitUrl = req_get_cstream_var(wp, "submit-url", "");	// hidden page
	if (submitUrl[0])
		DO_APPLY_WAIT("/skb_sta_protection.htm");
	return;
}

#define NTPTMP_FILE "/tmp/ntp_tmp"
void formwlprotection(request *wp, char *path, char *query)
{
	unsigned int ntp_onoff = 0;
	char buffer[256], list[256], tmpBuf[MAX_MSG_BUFFER_SIZE] = {0};
	...
	int day = 0, i, entryNum;
	char *strVal, *submitUrl, *strMac, *strName;
	char *s_hour, *e_hour, *rule;
	char *s_min, *e_min;
	char *argv[9];

	strVal = req_get_cstream_var(wp, "ntp_update", "");

	if (strVal[0]) {
		killall(SIGKILL, "ntp_inet");
		unlink(NTPTMP_FILE);

		nvram_get_r_def("NTP_ENABLED", buffer, sizeof(buffer), "1");
		...
		if (ntp_onoff == 1)
		{
			...
			nvram_get_r_def("x_ntp_server_ip2", ntp_server3, sizeof(ntp_server3), "time.bora.net");
			...
			if (ntp_server_id == 0)
				yexecl(NULL, "sh -c \"ntp_inet -x %s %s %s %s %s&\"", ntp_server1, command, daylight_save_str, ntp_server2, ntp_server3);
			else if (ntp_server_id == 1)
				yexecl(NULL, "sh -c \"ntp_inet -x %s %s %s %s %s&\"", ntp_server2, command, daylight_save_str, ntp_server3, ntp_server1);
			else
				yexecl(NULL, "sh -c \"ntp_inet -x %s %s %s %s %s&\"", ntp_server3, command, daylight_save_str, ntp_server1, ntp_server2);
		}

		DO_NTP_WAIT("/skb_sta_protection.htm");
		return;
	}

	strMac = req_get_cstream_var(wp, "mac_area", "");
	strName = req_get_cstream_var(wp, "sta_name", "");
	rule = req_get_cstream_var(wp, "rule", "");
	s_hour = req_get_cstream_var(wp, "start_hour", "");
	s_min = req_get_cstream_var(wp, "start_min", "");
	e_hour = req_get_cstream_var(wp, "end_hour", "");
	e_min = req_get_cstream_var(wp, "end_min", "");

	for (i = 0; i < 8; i++) {
		snprintf(buffer, sizeof(buffer), "week%d", i);
		strVal = req_get_cstream_var(wp, buffer, "");
		if (strVal[0])
			day += strtol(strVal, NULL, 10);
	}

	if (strMac[0] && rule[0] && s_hour[0] && s_min[0] && e_hour[0] && e_min[0]) {
		nvram_get_r_def("sta_protection_num", list, sizeof(list), "0");
		entryNum = strtol(list, NULL, 10);

		if ((entryNum + 1) > MAX_WLAN_AC_NUM) {
			strcpy(tmpBuf, ("테이블이 모두 차서 더이상 추가할 수 없습니다!"));
			goto setErr_sta_protection;
		}

		for (i = 0; i < entryNum; i++) {
			snprintf(buffer, sizeof(buffer), "sta_protection_list%d", (i + 1));
			if (nvram_get_r(buffer, list, sizeof(list))) {
				parse_line(list, argv, 9, " ,\r\n");
				if (argv[0]) {
					if (strcmp(argv[0], strMac) == 0) {				// exist mac list
						if (argv[6]) {								// check to same rule
							if (atoi(argv[6]) != atoi(rule)) {
								strcpy(tmpBuf, ("일관된 규칙만 설정 할 수 있습니다!"));
								goto setErr_sta_protection;
							}
						}

						if (argv[1] && (atoi(argv[1]) & day)) {							// check to day
							if (argv[2] && argv[3] && argv[4] && argv[5]) {		// start_h start_m end_h end_m
								if ((atoi(argv[2]) < atoi(s_hour) && atoi(argv[4]) > atoi(s_hour)) ||
									(atoi(argv[2]) < atoi(e_hour) && atoi(argv[4]) > atoi(e_hour))) {
									strcpy(tmpBuf, ("시간 설정이 중복됩니다!"));
									goto setErr_sta_protection;
								}

								if ((atoi(argv[2]) == atoi(s_hour) && atoi(argv[3]) <= atoi(s_min)) ||
									(atoi(argv[4]) == atoi(s_hour) && atoi(argv[5]) >= atoi(s_min))) {
									strcpy(tmpBuf, ("시간 설정이 중복됩니다!"));
									goto setErr_sta_protection;
								}

								if ((atoi(argv[2]) == atoi(e_hour) && atoi(argv[3]) <= atoi(e_min)) ||
									(atoi(argv[4]) == atoi(e_hour) && atoi(argv[5]) >= atoi(e_min))) {
									strcpy(tmpBuf, ("시간 설정이 중복됩니다!"));
									goto setErr_sta_protection;
								}
							}
		...
		snprintf(buffer, sizeof(buffer), "%d", entryNum + 1);
		nvram_set("sta_protection_num", buffer);

		snprintf(list, sizeof(list), "sta_protection_list%d", entryNum + 1);
		snprintf(buffer, sizeof(buffer), "%s,%d,%s,%s,%s,%s,%s,%s,0", strMac, day, s_hour, s_min, e_hour, e_min, rule, strName);
		nvram_set(list, buffer);
...
int macList(request *wp, int argc, char **argv)
{
	FILE *fp = NULL;
	int nBytesSent = 0;
	int element = 0, ret;
	char *buf = NULL, *ptr, tmpBuf[100];
	char ipAddr[40], macAddr[40], liveTime[80], hostName[64] = {0,};
	struct stat status;
	int pid;
	unsigned long fileSize = 0;

	nBytesSent += req_format_write(wp, ("<option value=\"\"></option>\n"));
	...
	while (1) {
		...
		ydespaces(macAddr);
		ydespaces(hostName);
		nBytesSent += req_format_write(wp, ("<option value=\"%s\">%s %s</option>\n"), macAddr, hostName, macAddr);
	}
...
int sta_protection_list(request *wp, int argc, char **argv)
{
	int	nBytesSent = 0, i, entryNum, num;
	int day, j, n, len;
	char *sta_argv[9], *d_day[] = {"일", "월", "화", "수", "목", "금", "토"};
	char buf[256], list[256], buffer[60];

	nvram_get_r_def("sta_protection_num", buf, sizeof(buf), "0");
	entryNum = strtol(buf, NULL, 10);

	for (i = 0; i < entryNum; i ++) {
		n = 0;
		len = sizeof(buffer);
		snprintf(list, sizeof(list), "sta_protection_list%d", (i + 1));
		if (nvram_get_r(list, buf, sizeof(buf))) {
			if ((num = parse_line(buf, sta_argv, 9, " ,\r\n")) >= 8) {
				day = strtol(sta_argv[1], NULL, 10);
				for (j = 0; j < 7; j ++) {
					if (day & (1 << j))
						n += snprintf(&buffer[n], len-n, "%s ", d_day[j]);
				}
				n += snprintf(&buffer[n], len-n, "%s시:%s분 - %s시:%s분", sta_argv[2], sta_argv[3], sta_argv[4], sta_argv[5]);

				nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
													"<td><font size=\"2\">%d</td>\n"
													"<td><input type=\"checkbox\" name=\"cpeuse%d\" id=\"cpeuse%d\" value=\"ON\" onClick=\"updateDel(%d);\" %s></td>\n"
    												"<td><font size=\"2\">%s</td>\n"
    												"<td><font size=\"2\">%s</td>\n"
    												"<td><font size=\"2\">%s</td>\n"
    												"<td><font size=\"2\">%s</td>\n"
      												"<td><input type=\"checkbox\" name=\"select%d\" id=\"select%d\" value=\"ON\" onClick=\"updateSet(%d);\" ></td></tr>\n"),
													(i + 1), i, i, i, (atoi(sta_argv[8]))? "checked" : "", (sta_argv[7])? translate_control_code(sta_argv[7]) : "", (sta_argv[0])? : "", buffer, (sta_argv[6][0] == '1')? "허용":"차단", i, i, i);
			}
		}
	}
