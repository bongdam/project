...
extern void nls_convert(unsigned char *buffer, unsigned char *table,
                        long count);
...
//      req->status = CLOSE;
#ifdef __DAVO__
	if (!req->cookie && strcmp(req->request_uri, "/boafrm/formLogin") && strcmp(req->request_uri, "/boafrm/formMfgTest")) {
...
int init_get2(request *req)
{
    ...
    {                           /* if countDownPage.htm had been sent to browser, don't allow browser to get other page(br0 down) */
        ...
        if (isFWUPGRADE == 2) {
            if (strstr(req->pathname, "skb_countDownPage.htm")) {
                isCountDown = 1;
...
//                   req->remote_ip_addr, strerror(errno_save));
			...
			if (errno == ENOENT) {
				if (req->cookie) {
					send_redirect_perm(req, "/skb_home.htm");
				} else {
					send_redirect_perm(req, "/skb_login.htm");
				}
    ...
    if (S_ISDIR(statbuf.st_mode)) {
        ...
        if (req->pathname[strlen(req->pathname) - 1] != '/') {
            ...
            if (server_port != 80)
                sprintf(buffer, "http://%s:%d%s/",
                        req->host ? req->host : server_name, server_port,
                        req->request_uri);
            else
                sprintf(buffer, "http://%s%s/",
                        req->host ? req->host : server_name,
                        req->request_uri);
...
//      req->last_modified = statbuf.st_mtime;
	...
	if (!req->cookie || (!pUser_info) || (strcmp(pUser_info->uniq_cookie, req->cookie))) {
		if (strcmp(req->request_uri,"/skb_login.htm") && strcmp(req->request_uri, "/formLogin") &&
			strcmp(req->request_uri, "/skb_util_gw.js") && strcmp(req->request_uri,"/skb_fail_login.htm")) {
			send_redirect_perm(req, "/skb_login.htm");
			return 0;
		}
	}
	...
	if (req->cookie && req->superUser == 0) {		// is user
		for (i = 0; user_deny_weblist[i] != NULL; i++) {
			if (strstr(req->request_uri, user_deny_weblist[i])) {
				send_redirect_perm(req, "/skb_home.htm");
				return 0;
			}
		}
...
//Brad add begin for update content length
    ...
    content_length_orig1 =
        content_length_orig1 + strlen("Content-Length: ");
    ...
    if (orig_char_length == exact_char_length) {
        ...
        memcpy(content_length_orig1, simple_itoa(exact_size),
               exact_char_length);
        ...
        memmove((content_length_orig2 + byte_shift), content_length_orig2,
                total_length_shift);
        memcpy(content_length_orig1, simple_itoa(exact_size),
               exact_char_length);
...
//fprintf(stderr, "###[%s %d] req->host=%s###\n", __FUNCTION__, __LINE__, req->host);
    ...
    if (strstr(req->request_uri, "formWlanRedirect")) {
        ...
        extern void formWlanRedirect2(request *wp, char *redirectUrl,
                                      char *strWlanId);
    ...
    if (data_fd == -1 && errno == ENOENT) {
        ...
        if (data_fd != -1) {
            ...
            if (!req->pathname) {
                boa_perror(req, "strdup req->pathname for gzipped filename "
...
#endif
        {
            bytes_written = write(req->fd, req->data_mem + req->ranges->start, bytes_to_write);
        }
#ifdef SERVER_SSL
        else {
...
int get_dir(request *req, struct stat *statbuf)
{
    ...
    if (directory_index) {      /* look for index.html first?? */
        ...
        if (data_fd != -1) {    /* user's index file */
            ...
            if (!req->pathname) {
                boa_perror(req, "strdup of pathname_with_index for .gz files "
                           __FILE__ ":" STR(__LINE__));
                return 0;
            }
            if (req->http_version != HTTP09) {
