...
#ifdef __DAVO__
char *get_wlan_name(char *buf, int ssid_num, char *name)
{
	if (ssid_num == 0) {
		sprintf(buf, "x_wlan%d_%s", wlan_idx, name);
	} else {
		sprintf(buf, "x_wlan%d_va%d_%s", wlan_idx, ssid_num - 1, name);
	}

	return buf;
}

void rearrange_portno(int *portno)
{
	if(*portno == 0)
		*portno = 4;
	else if(*portno == 1)
...
void arrange_portno(int *portno)
{
	if(*portno == 4)
		*portno = 0;
	else
...
int get_mac_addr(char *_str, unsigned char *mac)
{
	char *str;
	char t[20];
	int a[6];
	...
	if (strlen(str) == 12) {
		sprintf(t, "%c%c:%c%c:%c%c:%c%c:%c%c:%c%c",
	...
	mac[0] = (unsigned char)a[0];
	mac[1] = (unsigned char)a[1];
	mac[2] = (unsigned char)a[2];
	mac[3] = (unsigned char)a[3];
	mac[4] = (unsigned char)a[4];
	mac[5] = (unsigned char)a[5];

	return 1;
}

char *INET6_getaddrs(const char *ifname)
{
	FILE *f;
...
#define IPV6_ADDR_MAPPED        0x1000U
	...
	while (fscanf(f, "%4s%4s%4s%4s%4s%4s%4s%4s %08x %02x %02x %02x %20s\n",
		...
		if (ifname == NULL || !strcmp(devname, ifname)) {
			sprintf(addr6, "%s:%s:%s:%s:%s:%s:%s:%s",
				addr6p[0], addr6p[1], addr6p[2], addr6p[3],
				addr6p[4], addr6p[5], addr6p[6], addr6p[7]);
			inet_pton(AF_INET6, addr6, (struct sockaddr *)&sin6_addr);
			inet_ntop(AF_INET6, &sin6_addr, addr6p[0], INET6_ADDRSTRLEN);
			aprintf(&b, "%s/%d ", addr6p[0], plen);
...
char *INET6_getdroutes(void)
{
 ...
 ERROR:
		...
		if ((iflags & (RTF_UP | RTF_GATEWAY)) == (RTF_UP | RTF_GATEWAY)) {
			...
			inet_pton(AF_INET6, addr6x + 40, (struct sockaddr *)&sin6_addr);
...
char *INET6_getdns(void)
{
	struct nameserver_addr dns[2];
	char str[INET6_ADDRSTRLEN];
	struct abuffer b;
	int i, n;
	...
	n = sort_nameserver("/etc/resolv.conf", dns, 2, AF_INET6);

	for (i = 0; i < n; i++) {
		inet_ntop(AF_INET6, &dns[i].na_addr6, str, INET6_ADDRSTRLEN);
		aprintf(&b, "%s ", str);
		b.buf[b.count - 1] = '\0';
	}
...
int INET6_getflags(const char *ifname, char *buf)
{
	FILE *f;
	char addr6[40], devname[20];
	int plen, scope, if_idx, ifa_flags, dad_status = 0;
	char addr6p[8][5];
	struct in6_addr sin6_addr;
	...
	while (fscanf(f, "%4s%4s%4s%4s%4s%4s%4s%4s %08x %02x %02x %02x %20s\n",
		      addr6p[0], addr6p[1], addr6p[2], addr6p[3], addr6p[4],
		      addr6p[5], addr6p[6], addr6p[7], &if_idx, &plen, &scope,
		      &ifa_flags, devname) != EOF) {
		if (!strcmp(devname, ifname)) {
			if (ifa_flags & 0x08) {
				dad_status = ifa_flags;
				sprintf(addr6, "%s:%s:%s:%s:%s:%s:%s:%s",
					addr6p[0], addr6p[1], addr6p[2], addr6p[3],
					addr6p[4], addr6p[5], addr6p[6], addr6p[7]);
				inet_pton(AF_INET6, addr6, (struct sockaddr *)&sin6_addr);
...
int ping(struct sockaddr_in *addr, int pid)
{
	int i, sd;
	icmp_packet_t pkt;
	...
	if ((sd = socket(PF_INET, SOCK_RAW, 1 /*icmp */ )) < 0)
		return -1;
...
#ifdef __DAVO__
static int getsvrip_dnsquery(char *tst_ip, char *req_ip, int len)
{
	struct addrinfo *_addrinfo, *_res;
	struct sockaddr_in *sin;
	int errcode = 0, ipaddr = 0;
	int query = 0;

	if (!tst_ip || !tst_ip[0])
		return query;

	if ((ipaddr = inet_addr(tst_ip)) == INADDR_NONE) {
		errcode = getaddrinfo(tst_ip, NULL, NULL, &_addrinfo);
		if (errcode) {
			return query;
		}
		for(_res = _addrinfo; _res != NULL; _res = _res->ai_next) {

			sin = (void *)_res->ai_addr;
			inet_ntop(_res->ai_family, &sin->sin_addr, req_ip, len);
		}
		freeaddrinfo(_addrinfo);
	} else
		snprintf(req_ip, len, "%s", inet_ntoa(*((struct in_addr *)&ipaddr)));

	query = 1;
	return query;
}

int send_ping_test(unsigned char *test_ip)
{
	char ping_cmd[128] = {0,};
	FILE *pp = NULL;
	int result = 0;
	int query = 0;
	char buf[64] = {0,};
	char request_ip[64] = {0,};
	struct in_addr ipAddr;
	int val = 0;

	query = getsvrip_dnsquery(test_ip, request_ip, sizeof(request_ip));
	if (!query)
		return result;
	...
	if (val == 0) {
		...
		if (strcmp(request_ip, inet_ntoa(ipAddr)) == 0)
			return result;
	}

	snprintf(ping_cmd, sizeof(ping_cmd), "mping -q -p -c 1 -w 1000 %s", request_ip);
	pp = popen(ping_cmd, "r");
...
int getWlanwanlink(char *infName)
{
	...
	skfd = socket(AF_INET, SOCK_DGRAM, 0);
	/* Get wireless name */
	if (iw_get_ext(skfd, infName, SIOCGIWNAME, &wrq) < 0) {
		close(skfd);
		/* If no wireless name : no wireless extensions */
		return -1;
	}
	...
	if (iw_get_ext(skfd, infName, SIOCGIWRTLJOINREQSTATUS, &wrq) < 0)
		return -1;
...
unsigned int get_network_info(char *name, char *value)
{
	int i = 0;
	char buf[80], buf2[80];
	char targetfile[80];
	...
	if ((fp = fopen(targetfile, "r"))) {
		while (fgets(buf, sizeof(buf), fp) && buf[0] != 0) {
			if (i == 0) {
				n += sprintf(value, "%s", buf);
			} else {
				sprintf(buf2, "%s", value);
				n += sprintf(&value[n], "|%s", buf);
			}
			i++;
		}
		n = 0;
	...
	if (buf[0] != 0)
		n = inet_aton(((i == 1) ? value : &buf2[0]), &in);

	return ((n > 0) ? in.s_addr : 0);
}

int sdmz_enable(void)
{
	int opmode;
	char buf[32];
	...
	apmib_get(MIB_OP_MODE, (void *)&opmode);
	if (opmode == 0) {
		nvram_get_r_def("x_SDMZ_ENABLED", buf, sizeof(buf), "0");
		if (atoi(buf) == 1) {
			nvram_get_r_def("x_SDMZ_HOST", buf, sizeof(buf), "");
			if (buf[0])
				return 1;
		}
	}
...
int fngets(const char *path, int nth, char *buf, int len)
{
	int i, res = -1;
	FILE *fp;

     	if (path == NULL || path[0] == 0 || buf == NULL || len <= 0)
         	return -1;
     	...
     	if ((fp = fopen(path, "r")) != NULL) {
         	...
         	if (i == nth && fgets(buf, len, fp) != NULL)
             		res = 0;
...
int get_ipaddr_file(char *file, long *ipaddr, char *strIpaddr)
{
	char buf[32];
	struct in_addr addr;

	if (fngets(file, 0, buf, sizeof(buf)))
		return 0;
	//despaces(buf);
	ydespaces(buf);
	if (inet_aton(buf, &addr)) {
		*ipaddr = addr.s_addr;
		if (strIpaddr)
			strcpy(strIpaddr, inet_ntoa(addr));
		return 1;
	}
	return 0;
}
...
int increase_entropy(void)
{
	...
	if (fd < 0) {
		fprintf(stderr, "Cannot open urandom for writing\n");
		return 0;
	}

	f = fopen("/proc/interrupts", "r");
	if (!f) {
		fprintf(stderr, "Cannot open interrupts for reading\n");
		close(fd);
		return 0;
	}
...
char *INETx_getdns(int af)
{
	...
	n = sort_nameserver("/etc/resolv.conf", dns, 2, af);

	for (i = 0; i < n; i++) {
		inet_ntop(af, &dns[i].na_addr6, str, INET6_ADDRSTRLEN);
		aprintf(&b, "%s ", str);
		b.buf[b.count - 1] = '\0';
	}
...
/* APACRTL-92 smlee  */
#define L2_INFO "/tmp/l2_info"
int check_lan(char *mac_addr)
{
	FILE *fp = NULL;
	char buf[256];
	int lan_port = -1;
	char *tmp, *port_num, *mbr_str;

	system("cat /proc/rtl865x/l2 > " L2_INFO);

	if ((fp = fopen(L2_INFO, "r")) == NULL)
		return lan_port;

	while (fgets(buf, sizeof(buf), fp)) {
		if (strstr(buf, mac_addr) != 0) {
			mbr_str = &buf[0];
			tmp = strsep(&mbr_str, "(");
			port_num = strsep(&mbr_str, " ");

			if ('0' <= port_num[0] && port_num[0] <= '3') {
				ydespaces(port_num);
				lan_port = atoi(port_num);
			}
			break;
		}
	}
...
int get_controlsideband(void)
{
	int skfd = 0;
	char ctrlsideband = 0;	//0:Dont CARE   1:BELOW   2:ABOVE
	struct iwreq wrq;
	...
	wrq.u.data.pointer = (caddr_t)&ctrlsideband;
	wrq.u.data.length = sizeof(ctrlsideband);
	strncpy(wrq.ifr_name, "wlan1", IFNAMSIZ);
	wrq.u.data.flags = RTL8192CD_IOCTL_CONTROLSIDEBAND_GET;
	...
	return (int)ctrlsideband;
}
/* APACRTL-92 smlee  */
#define TMP_ASICCOUNTER "/tmp/tmp_asicCounter"
int getPortStats(int port_num, char *str_traffic)
{
 	FILE *fh;
  	char buf[512];
	char str_port[10];
	...
	system("cat /proc/asicCounter > " TMP_ASICCOUNTER);
	...
	sprintf(str_traffic, "%llu %llu %llu %llu", tx_p, rx_p, tx_b, rx_b);
	fclose(fh);
...
/* APACRTL-100 */
int check_wlan_exist(char *macaddr)
{
	int i;
	int wlan_id, vwlan_id;
	char *buff;
	char wlan_name[20];
	char get_mac[20];
	...
	for (wlan_id = 0; wlan_id < 2; wlan_id++) {
		for (vwlan_id = 0; vwlan_id < 5; vwlan_id++) {
			if (vwlan_id == 0)
				snprintf(wlan_name, sizeof(wlan_name), "wlan%d", wlan_id);
			else
				snprintf(wlan_name, sizeof(wlan_name), "wlan%d-va%d", wlan_id, vwlan_id - 1);
			...
			for (i = 1; i < MAX_STA_NUM; i++) {
				...
				if (pInfo->aid && (pInfo->flags & STA_INFO_FLAG_ASOC)) {
					snprintf(get_mac, sizeof(get_mac), "%02x:%02x:%02x:%02x:%02x:%02x",
							pInfo->addr[0],pInfo->addr[1],pInfo->addr[2],pInfo->addr[3],pInfo->addr[4],pInfo->addr[5]);
					if (strcmp(macaddr, get_mac) == 0) {
						free(buff);
						return 1;
					}
				}
...
static long get_cpu_utiliz(void)
{
	time_t t;
	char buf[512];
	double cpu_idle_load=0, cpu_usage=0;
	struct nmpipe *named_pipe = NULL;
...
static long get_ram_utiliz(void)
{
...
char *get_performance(char *msg, int type)
{
	if ( !msg )
		return;
	...
	sprintf(&msg[0], "%s: %ld",
				((type)? "RAM":"CPU"),
				((type)? get_ram_utiliz():get_cpu_utiliz()));
	return msg;
}

char *get_asicinfo(int dir, int port, int packet_kind, char *data, int data_len)
{
	FILE *fp;
	char buf[256];
				...
				for ( i = 0; i < argc; i++ ) {
					...
					if ( find_dir >= 0 ) {
						if ( dir == find_dir && (argv[0][0]-'0') == port ) {
							snprintf(data, data_len, "%s", argv[packet_kind]);
							break;
						}
					}
				}
...
void hash_sha256_captcha(unsigned char *captcha, unsigned char *hash)
{
	uint8 dgst[80];
	sha256_context ctx;

	snprintf((char *)dgst, sizeof(dgst), "%s%s", (char *)captcha, CAPTCHA_INIT_STR);
	sha256_starts(&ctx);
	sha256_update(&ctx, dgst, strlen((char *)dgst));
	sha256_finish(&ctx, dgst);
	yitoxa((char *)hash, dgst, 32);
}

int check_saved_passwd(char *new_pass)
{
	int number_cnt = 0, english_cnt = 0, special_cnt = 0;
	int i;
	...
	for (i = 0; i < strlen(new_pass); i++) {
		if ((new_pass[i] >= 0x41 && new_pass[i] <= 0x5A) || (new_pass[i] >= 0x61 && new_pass[i] <= 0x7A)) {	/* english check */
			english_cnt = 1;
		} else if (new_pass[i] >= 0x30 && new_pass[i] <= 0x39) { /* number check  */
			number_cnt = 1;
		} else if ((new_pass[i] >= 0x20 && new_pass[i] <= 0x21) || (new_pass[i] >= 0x23 && new_pass[i] <= 0x26) || 	/* special */
				(new_pass[i] >= 0x28 && new_pass[i] <= 0x2b) || (new_pass[i] == 0x2d) || (new_pass[i] == 0x3d) || (new_pass[i] == 0x40) ||
				(new_pass[i] >= 0x5e && new_pass[i] <= 0x5f ) || (new_pass[i] == 0x60) || (new_pass[i] == 0x7e)) {
			special_cnt = 1;
		}
	}

	if (english_cnt && number_cnt && special_cnt)
		return 1;
...
#ifndef DISABLE_WEBLOG_DEBUG
#include <syslog.h>

int log_disabled, log_stderr;

void ylog(int priority, const char *fmt, ...)
{
	va_list ap, aq;
