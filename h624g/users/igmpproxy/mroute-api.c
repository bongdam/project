...
*/
{
  ...
  log( LOG_DEBUG, 0, "adding VIF, idx=%d Fl flags=0x%x IP=%s %s", 
...
*/
{
 ...
 // CtlReq.mfcc_subscriber = Dp->SubsAdr;
  ...
  if( setsockopt( MRouterFD, IPPROTO_IP, MRT_ADD_MFC, (void *)&CtlReq, sizeof( CtlReq ) ) ) {
        //log( LOG_WARNING, errno, "MRT_ADD_MFC" );
        LOG(LOG_INFO, "IGMP failed MRT_ADD_MFC(%s) InpVIf: %d",
            fmtInAdr( FmtBuM, CtlReq.mfcc_mcastgrp ),
            CtlReq.mfcc_parent == ALL_VIFS ? -1 : CtlReq.mfcc_parent);
	}
...
#ifndef __DAVO__
  {
    log( LOG_DEBUG, 0, "removing MFC: %s -> %s, InpVIf: %d", 
	    fmtInAdr( FmtBuO, CtlReq.mfcc_origin ), 
	    fmtInAdr( FmtBuM, CtlReq.mfcc_mcastgrp ),
	    CtlReq.mfcc_parent == ALL_VIFS ? -1 : CtlReq.mfcc_parent
	    );
...
#endif
  if( setsockopt( MRouterFD, IPPROTO_IP, MRT_DEL_MFC, (void *)&CtlReq, sizeof( CtlReq ) ) )  {
      //log( LOG_WARNING, errno, "MRT_DEL_MFC" );
      LOG(LOG_INFO, "IGMP failed MRT_DEL_MFC(%s) InpVIf: %d",
    		fmtInAdr( FmtBuM, CtlReq.mfcc_mcastgrp ),
    		CtlReq.mfcc_parent == ALL_VIFS ? -1 : CtlReq.mfcc_parent);
  }
}
