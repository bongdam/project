		...
		for( IfPt = IfVc; IfPt < IfEp; IfPt++ ) {
			...
			*/
			...
			if (ioctl(Sock, SIOCGIFNETMASK, &IfReq) < 0)
				log(LOG_ERR, errno, "ioctl SIOCGIFNETMASK");

			IfDescEp->InNetMask = ((struct sockaddr_in *)&IfReq.ifr_netmask)->sin_addr;
#endif
			/* get if flags
			**
			** typical flags:
			...
			*/			
			...
			log( LOG_DEBUG, 0, "buildIfVc: Interface %s Addr: %s, Flags: 0x%04x", 
						IfDescEp->Name, fmtInAdr( FmtBu, IfDescEp->InAdr ), IfDescEp->Flags );
			IfDescEp++;
		}
	}
	close( Sock );
