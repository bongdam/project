...
#undef POLL_RF_SWITCH
#else
#define POLL_RF_SWITCH 1
#endif
...
#if defined(CONFIG_RTL_8196B) || defined(POLL_RF_SWITCH)
static int test_gpio(void)
{
	FILE *fp;
	int i;
...
static int wait_for_flags(unsigned int mask, int bwaitall, int timeout)
{
	struct pollfd pfd;
	unsigned int flag;
	int status, expiry;
							...
							while (fgets(line, 200, fp)) {
								...
								if (strlen(line) != 0) {
									...
									DEBUG_PRINT(" idx=[%u],date=[%u],fTime=[%u],tTime=[%u]__[%s-%u]\r\n", idx,
										    ks[i][idx].date, ks[i][idx].fTime, ks[i][idx].tTime, __FILE__, __LINE__);
									wlan_schk = 1;
									wlan_schkmulti[i] = 1;
								}
							}
...
# endif
	...
	while (1) {
		...
		wisp_wan_connected();
#ifdef __DAVO__
		rc = wait_for_flags(DF_RSTASSERTED, 1, 3000);
		switch (rc) {
		default:
			if (last_status <= 0)
				break;
