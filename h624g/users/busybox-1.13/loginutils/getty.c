...
#if DV_CONSOLE_PROTECT
static inline int magic_string_ok(char *n)
{
	const char *p="$1$UUM.c3Sq$Jl2Pn/hEcCJ70qOLYSfAO1";	// same as console password
	int good;

	good = strcmp(p, crypt(n, p)) ? 0 : 1;
...
#if DV_CONSOLE_PROTECT
#define CONS_PROT_DECL_VAR char name[40]; int i=0; memset(name, 0, sizeof(name));
#define CONS_PROT_UPDATE_STR if (i<(sizeof(name)-1)) name[i++]=ch;
#define CONS_PROT_POST_STR \
	if (i<=0) {	exit(EXIT_SUCCESS); } \
	else if (name[i-1]=='\n' || name[i-1]=='\r') { name[i-1] = 0; }
#define CONS_PROT_CHECK_STR if (!magic_string_ok(name)) { exit(EXIT_SUCCESS); }
#else
#define CONS_PROT_DECL_VAR
#define CONS_PROT_UPDATE_STR
#define CONS_PROT_POST_STR
#define CONS_PROT_CHECK_STR
#endif

    {
        CONS_PROT_DECL_VAR;
    	/* Optionally wait for CR or LF before writing /etc/issue */
		if (DV_CONSOLE_PROTECT || (options.flags & F_WAITCRLF)) {
			char ch;
