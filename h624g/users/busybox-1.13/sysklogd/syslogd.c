...
USE_FEATURE_VOLATILE_LOG( \
	const char *tempLogPath;		\
)
...
static const struct init_globals init_data = {
	...
	.SMwdn = { {0, 0}, {1, 0}, {1, +1} },
#endif
#if ENABLE_FEATURE_VOLATILE_LOG
	.tempLogPath = "/var/tmp/messages",
#endif
};
 ...
 reopen:
	...
	if (G.logFileSize && G.isRegular && G.curFileSize > G.logFileSize) {
		if (G.logFileRotate) { /* always 0..99 */
			...
			close(G.logFD);
#if COMPRESS_SYSLOG
			gzip_append("/var/log/messages.gz", newFile);
#endif
			goto reopen;
		}
		ftruncate(G.logFD, 0);
...
#ifdef __DAVO__
	if (!(ntp_sync & DF_NTPSYNC)) {
		if (!(ntpstate() & DF_NTPSYNC)) {
			syscall(__NR_clock_gettime, CLOCK_MONOTONIC, &ts);
			n = sprintf(tmp, "%lu.%03ld", ts.tv_sec, ts.tv_nsec / 1000000);
			sprintf(timestamp, "%.*s%s", 15 - n, "**********", tmp);
		}
	}
#endif
	timestamp[15] = '\0';
...
#ifdef __DAVO__
	volatile_log_locally(G.tempLogPath, G.logFileSize << 1, G.printbuf);
#endif
}
...
static int try_to_resolve_remote(void)
{
	if (!G.remoteAddr) {
		...
		G.remoteAddr = host2sockaddr(G.remoteAddrStr, 10614);
		if (!G.remoteAddr)
			return -1;
	}
...
#ifdef __DAVO__
static ssize_t rsyslog(int sockfd, const void *buf, size_t len, int flags,
                       const struct sockaddr *dest_addr, socklen_t addrlen)
{
	char p[strlen(G.hostname) + 4 + len];
	char *q;
	size_t n;

	q = strchr((const char *)buf, '>');
	if (q) {
		for (++q; *q && isspace(*q); )
			++q;
		q = strchr(q + 15, ':');
		if (q && ((size_t)(q - (const char *)buf) < len)) {
			for (++q; *q && isspace(*q); )
				++q;
			n = (size_t)(q - (const char *)buf);
			len = sprintf(p, "%.*s[%s] %.*s", n, (const char *)buf, G.hostname, len - n, q);
			return sendto(sockfd, p, len, flags, dest_addr, addrlen);
		}
	}
...
{
#if COMPRESS_SYSLOG
	fd_set allset, sndset, fds;
	int listenfd, fd, maxfd, i, prev;
	socklen_t len;
	struct sockaddr_un sunx;
	struct stat statf;
#endif
...
#define recvbuf (G.recvbuf)
	...
	if (listenfd < 0)
		bb_error_msg_and_die("serv_listen");
	FD_SET(listenfd, &allset);
	FD_SET(sock_fd, &allset);
	maxfd = (listenfd > sock_fd) ? listenfd : sock_fd;
#endif
#ifdef __DAVO__
	(void)ntpstate();

	if ((lstat(G.logFilePath, &statf) == 0 && S_ISREG(statf.st_mode)) &&
	    (lstat(G.tempLogPath, &statf) || !S_ISREG(statf.st_mode))) {
		fd = open(G.tempLogPath, O_WRONLY|O_CREAT|O_TRUNC, 0666);
		if (fd > -1) {
			Pwrite((void *)fd, G.logFilePath,
				fopen, fclose, (void *)clone_write);
			close(fd);
		}
	}
#else
 ...
 read_again:
		...
		if (select(maxfd + 1, &fds, NULL, NULL, NULL) < 0) {
			if (errno == EINTR)
				continue;
			bb_error_msg_and_die("select error");
		}

		prev = -1;
		for (i = 0; i <= maxfd; i++) {
			if (FD_ISSET(i, &sndset) && FD_ISSET(i, &fds)) {
				recvfd_n_writepage(i, "/var/log/messages.gz", G.logFilePath);
				FD_CLR(i, &allset);
				FD_CLR(i, &sndset);
				if (i == maxfd)
					maxfd = prev;
			} else if (FD_ISSET(i, &allset))
				prev = i;
		}

		if (FD_ISSET(listenfd, &fds)) {
			len = sizeof(sunx);
			fd = accept(listenfd, (struct sockaddr *)&sunx, &len);
			if (fd < 0)
				perror("accept");
			else {
				FD_SET(fd, &allset);
...
#ifdef __DAVO__
			rsyslog(G.remoteFD, recvbuf, sz+1, MSG_DONTWAIT,
