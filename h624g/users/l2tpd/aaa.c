...
int handle_challenge (struct tunnel *t, struct challenge *chal)
{
    ...
    if (!get_secret (us, them, (char *)chal->secret, sizeof (chal->secret)))
    {
    ...
    MD5Update (&chal->md5, chal->secret, (unsigned int)strlen((const char *)chal->secret));
...
void encrypt_avp (struct buffer *buf, _u16 len, struct tunnel *t)
{
    ...
    previous_segment = (unsigned char *)ptr;
    while (ptr < end)
    {
#if DEBUG_HIDDEN
