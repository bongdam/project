...
#if defined(RTL_RADIUS_2SET) || defined(__DAVO__)
...
typedef struct Global_Params_tag
{
	...
	//End Added WPA
#if defined(__DAVO__)
	int dv_idle_time; // youngho DAVO
	u_char redirect_host; // youngho DAVO
	u_char dv_drop; // youngho DAVO
	u_char wait_eap_start; // youngho DAVO
	u_char acct_sid_reuse; // youngho DAVO
#define TIMER_FIND_IP_UPDATE 300	// sec,  youngho DAVO
#define TIMER_FIND_IP_1ST	 10		// sec,  youngho DAVO
	int tm_find_ip; // youngho DAVO
	u_int ipaddr; // youngho DAVO

	u_int is_reauthenticating; // youngho DAVO
#endif
} Global_Params;
...
#endif
#if defined(CONFIG_RTL8196C_AP_HCM) || defined(__DAVO__)
...
extern char dev_supp[];	// david+2006-03-31, for add event to syslog
