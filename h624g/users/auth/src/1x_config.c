...
static int gain_wlan_index(char *name)
{
...
int lib1x_load_config(Dot1x_Authenticator * auth, char *confFileName)
{
	...
	for(tagIndex=0 ; tagIndex< ulNumOfTag; tagIndex++)
	{
		...
		if ( ! (retResult = lib1x_config_parse(confFileName, authTag, authVal)) ) {
			lib1x_message(MESS_DBG_CONFIG, "%s = %s", authTag, authVal);
		...
		{
			//sc_yang
			extern char lib1x_nas_id[MAX_NAS_ID_LEN] ;
...
#endif
#if defined(__DAVO__)
		else if(!strcmp(ConfigTag[tagIndex], "rsDomain"))
		{
			memset(auth->svrdomain, 0, sizeof(auth->svrdomain));
			strncpy(auth->svrdomain, authVal, sizeof(auth->svrdomain));
			lib1x_message(MESS_DBG_CONFIG, "svrdomain = %s\n", auth->svrdomain);
		}
		else if(!strcmp(ConfigTag[tagIndex], "accountRsDomain"))
		{
			memset(auth->acctdomain, 0, sizeof(auth->acctdomain));
			strncpy(auth->acctdomain, authVal, sizeof(auth->acctdomain));
			lib1x_message(MESS_DBG_CONFIG, "acctdomain = %s\n", auth->acctdomain);
		}
#ifdef RTL_RADIUS_2SET
		else if(!strcmp(ConfigTag[tagIndex], "rs2Domain"))
		{
			memset(auth->svrdomain2, 0, sizeof(auth->svrdomain2));
			strncpy(auth->svrdomain2, authVal, sizeof(auth->svrdomain2));
			lib1x_message(MESS_DBG_CONFIG, "svrdomain2 = %s\n", auth->svrdomain2);
		}
		else if(!strcmp(ConfigTag[tagIndex], "accountRs2Domain"))
		{
			memset(auth->acctdomain2, 0, sizeof(auth->acctdomain2));
			strncpy(auth->acctdomain2, authVal, sizeof(auth->acctdomain2));
			lib1x_message(MESS_DBG_CONFIG, "acctdomain2 = %s\n", auth->acctdomain2);
		}
#endif
		else if(!strcmp(ConfigTag[tagIndex], "redirect"))
		{
			auth->redirect = atoi(authVal);
			lib1x_message(MESS_DBG_CONFIG, "redirect = %d\n", auth->redirect);
		}
		else if(!strcmp(ConfigTag[tagIndex], "redir_host"))
		{
			memset(auth->redir_host, 0, sizeof(auth->redir_host));
			strncpy(auth->redir_host, authVal, sizeof(auth->redir_host));
			lib1x_message(MESS_DBG_CONFIG, "redir_host = %s\n", auth->redir_host);
		}
		else if(!strcmp(ConfigTag[tagIndex], "wlist_file"))
		{
			memset(auth->wlist_file, 0, sizeof(auth->wlist_file));
			strncpy(auth->wlist_file, authVal, sizeof(auth->wlist_file));
			lib1x_message(MESS_DBG_CONFIG, "wlist_file = %s\n", auth->wlist_file);
		}
#endif
			...
			else if(pAuth->AlgoTable[DOT11_AuthKeyType_RSNPSK].Enabled)
			{
				...
				if(ulUsePassphrase)
				{
					PasswordHash((char *)auth->RSNVariable.PassPhrase, strlen((char *)auth->RSNVariable.PassPhrase),
						(unsigned char *)auth->RSNVariable.ssid, strlen((char *)auth->RSNVariable.ssid), auth->RSNVariable.PassPhraseKey);
