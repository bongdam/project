...
int lib1x_control_QuerySTA(Global_Params * global)
{
        ...
        else
	{
		...
		if(pQuery_Stat->IsSuccess)
		{
			...
			global->theAuthenticator->acct_sm->tx_bytes_h = pQuery_Stat->tx_only_data_bytes_high;         // == transmited bytes youngho DAVO
			global->theAuthenticator->acct_sm->rx_bytes_h = pQuery_Stat->rx_only_data_bytes_high;         // == received bytes youngho DAVO
#else
