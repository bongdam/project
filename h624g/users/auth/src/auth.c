...
extern void toggle_debug(void);

int _redirect_used;

static void sighup_handler(int no);
static void sigusr1_handler(int no);
static void sigusr2_handler(int no);
static void sigterm_handler(int no);
static int redirect_setup(int set, int enable);
#endif
...
*/
char svrip[IP_ADDRSIZE+1];
char dev_svr[LIB1X_MAXDEVLEN];
char dev_supp[LIB1X_MAXDEVLEN];
u_short udp_svrport;
#ifdef RTL_RADIUS_2SET
char svrip2[IP_ADDRSIZE+1];
u_short udp_svrport2;
#endif

char *g_wlan_name=NULL;
...
{
	int i;
	char tmp_ourip[IP_ADDRSIZE+1];
	struct in_addr tmpip;
	char errbuf[LIBNET_ERRBUF_SIZE];

	memset(tmp_ourip, 0, sizeof(tmp_ourip));
/* get NAS-IP-Address */
	i = yfcat("/var/wan_ip", "%s", tmp_ourip);
	if (i!=1) {
		strcpy(tmp_ourip, "0.0.0.0");
	}
	ydespaces(tmp_ourip);

	if (strcmp(tmp_ourip, "0.0.0.0")==0) {
		...
		if (tmpip.s_addr == -1 || tmpip.s_addr == 0) {
			lib1x_message(MESS_ERROR_FATAL,"open1x: Could not obtain IP for interface %s", dev_svr );
			return 0;	// david
		} else {
			strcpy( tmp_ourip, inet_ntoa(tmpip));
		}
	}

	if (strcmp(tmp_ourip, "0.0.0.0")) {
		strcpy(ourip, tmp_ourip);
		return 1;
	}
	return 0;	// david
}
...
/* get NAS-IP-Address */
	...
	libnet_close_link_interface(l); // david
	DVDBG_PRINT("%s():%d dev_supp=%s get mac from libnet_get_hwaddr usrsupp_addr=%s\n", __FUNCTION__, __LINE__, dev_supp, oursupp_addr);
...
#endif
{
	...
	memset(auth, 0, sizeof(Dot1x_Authenticator));
...
*/
...
#if defined(__DAVO__)
	RTLAuthenticator.GlobalTxRx = (TxRx_Params *)lib1x_init_driver_interface(auth, dev_supp);

	lib1x_init_authRSNConfig(auth);
	lib1x_init_authGlobal(auth);
#endif
	...
	_redirect_used = redirect_setup(0, auth->redirect);

	i = yfcat("/var/wan_intf", "%s", dev_svr);
	if (i!=1) {
		strcpy(dev_svr, "eth1");
	}
...
#if defined(__DAVO__)
	if (auth->redirect) {
		_redirect_used = redirect_setup(1, auth->redirect);
	}
#endif
...
#endif
#if defined(__DAVO__)
			auth->svrdomain, auth->acctdomain, 
#ifdef RTL_RADIUS_2SET
			auth->svrdomain2, auth->acctdomain2, 
#endif
#endif
			dev_svr, dev_supp);
...
#if defined(__DAVO__)
static void svr_reconnect(void)
{
	int ret = 0;
	Dot1x_Authenticator * auth = &RTLAuthenticator;
	...
	if (!auth->GlobalTxRx || ret==0) {
		DVDBG_PRINT("auth->GlobalTxRx is %s, get wan_ip %s\n", auth->GlobalTxRx?"TRUE":"FALSE", (ret==0)?"Fail":"Success");
		return;
	}

	dev_txrx = auth->GlobalTxRx;
	if ( inet_aton(ourip, & dev_txrx->ourip_inaddr ) == 0 ) {
		lib1x_message(MESS_ERROR_FATAL, "Invalid OUR IP addr ");
	}

	lib1x_nal_reconnect(auth);
}
...
{
	// admin reboot
	RTLAuthenticator.dv_reboot = 1;	// reboot (restart wlan), after sending EAP failure, dv_reboot goes to 2.
}

static int get_mac_addr(char *_str, unsigned char *mac)
{
	char *str;
	char t[20];
	int  a[6];
	...
	mac[0] = (unsigned char)a[0]; mac[1] = (unsigned char)a[1]; mac[2] = (unsigned char)a[2];
	mac[3] = (unsigned char)a[3]; mac[4] = (unsigned char)a[4]; mac[5] = (unsigned char)a[5];
...
static int get_cmd_from_file(char *fname, unsigned char *m, int *all)
{
	FILE *fp;
	char str[40];
	char *p;
	...
	*all = 0;
	...
	if (strcmp(p, "svr_reconnect")==0) {
		return CMD_SVR_RECON;
	}
...
static void sigusr2_handler(int no)
{
	...
	snprintf(fname, sizeof(fname), "/var/tmp/disc_sta");

	cmd = get_cmd_from_file(fname, m, &all); 
	if (cmd==CMD_UNKNOWN) {
		return;
	}
	if (cmd==CMD_SVR_RECON) {
		svr_reconnect();
		return;
	}

	for(i = 0; i < MAX_SUPPLICANT; i++) {
		if (RTLAuthenticator.Supp[i] && RTLAuthenticator.Supp[i]->global) {
			if (RTLAuthenticator.Supp[i]->isEnable) {
				if (all) {
					RTLAuthenticator.Supp[i]->global->dv_drop = 1;
					f=1;
				} else {
					auth_pae = RTLAuthenticator.Supp[i]->global->theAuthenticator;
/*
printf("auth: test %x:%x:%x:%x:%x:%x\n",
 auth_pae->supp_addr[0],
 auth_pae->supp_addr[1],
 auth_pae->supp_addr[2],
 auth_pae->supp_addr[3],
 auth_pae->supp_addr[4],
 auth_pae->supp_addr[5]);
*/
					if (auth_pae &&  memcmp(auth_pae->supp_addr, m, 6)==0) {
						RTLAuthenticator.Supp[i]->global->dv_drop = 1;
						f=1;
						break;
					}
	...
	if (f)
		RTLAuthenticator.dv_reboot = 3;	// drop sta. after drop, dv_reboot return to 0
}
...
APNRTL-273
*/
    if (RTLAuthenticator.RSNVariable.Dot1xEnabled == FALSE
        && RTLAuthenticator.RSNVariable.MacAuthEnabled == FALSE) {
        lib1x_remove_fifo();
	...
	// admin reboot
	sigusr1_handler(no);
	RTLAuthenticator.dv_sigterm = 1;	// reboot (restart wlan), after sending EAP failure, dv_reboot goes to 2.
}

static int redirect_setup(int set, int enable) // youngho DAVO
{
	if (set && enable == 1)
		return REDIR_RESOLV;
	else
