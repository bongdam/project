...
//--------------------------------------------------
struct lib1x_nal_intfdesc *  lib1x_nal_initialize( char * intdev , u_char * ouraddr, u_char inttype)
{
...
#if defined(__DAVO__)
						if (suppid == -1) {
							// db mismatch. disconnect sta.
							struct lib1x_ethernet * eth;
							eth = ( struct lib1x_ethernet * ) descSupp->packet.data;
#if 0
							{
								unsigned char *p;
								int a;
...
#endif
							// youngho DAVO
							DVDBG_PRINT("db_mismatch:disconnect:%x:%x:%x:%x:%x:%x\n", eth->ether_shost[0], eth->ether_shost[1], eth->ether_shost[2], eth->ether_shost[3], eth->ether_shost[4], eth->ether_shost[5]);
							lib1x_control_AuthDisconnect(auth, eth->ether_shost, auth_not_valid);
						}
#endif
					}
				}
...
//  that here.
//--------------------------------------------------
BOOLEAN lib1x_nal_send( struct lib1x_nal_intfdesc * desc,  u_char * packet , int size)
{
	int num_sent = 0;
...
void lib1x_nal_reconnect(Dot1x_Authenticator * auth)
{
