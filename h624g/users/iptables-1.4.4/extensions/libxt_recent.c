...
static const struct option recent_opts[] = {
	...
	{ .name = "blockoff", .has_arg = 1, .val = 211 },
	{ .name = NULL }
};
...
"    --blockoff seconds          Blockoff source address during specified seconds if matched.\n"
                         ...
                         int numeric)
{
	...
	if (info->side == XT_RECENT_DEST)
		printf("side: dest");
	if(info->blockoff) printf("blockoff: %d ",info->blockoff);
}
