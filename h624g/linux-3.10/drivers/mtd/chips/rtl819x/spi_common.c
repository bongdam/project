...
static inline void rtk_udelay(int us)
{
	if (us >= 1000)
		...
		udelay(us);
		/* BUG: sleeping function called from invalid context - young
		cond_resched();
		*/
	}
}
...
static struct spi_flash_known spi_flash_registed[] = {
/****************************************** Micronix Flash ******************************************/
//#define MX25L1605D            0x00C22015
...
//8MB*4=32MB
	0x00C22019, 0x00, SIZE2N_08MB, SIZE_064K, SIZE_004K, SIZE_256B, "MX25L25735F"
#else
...
//#define W25Q256			0x00EF4019
	{0x00EF4019, 0x00, SIZE2N_08MB, SIZE_064K, SIZE_004K, SIZE_256B, "W25Q256"
#if (SPI_DRIVER_MODE == 1)
...
static unsigned int ComSrlCmd_RDJEDEC(unsigned char ucChip, unsigned int addr, unsigned int uiLen);
#endif
...
void spi_regist(unsigned char ucChip)
{
...
#if DAVO_4B_ONLY_SUPPORT
	chip_spi_4b_only = 0;
	if (ui == 0xc22019) {	// if flash is MX25L25635 or MX25L25735
		unsigned int t1;
		t1 = ComSrlCmd_RDJEDEC(ucChip, 0, 4);
		if ((t1 & 0x00ffffff)==0x00534644) {	// SFDP supported (Serial Flash Discoverable Parameter)
			t1 = ComSrlCmd_RDJEDEC(ucChip, 0x32, 4);
			//printk("ComSrlCmd_RDJEDEC(0x32) : 0x%08x\n", t1);
			t1 = (t1 >> 16) & 0x000000ff;
			if (((t1>>1)&0x03)==2) {
				chip_spi_4b_only = 1;
			}
	...
	if (i == uiCount) {
		...
		set_flash_info(ucChip, ui, i, SIZE_064K, SIZE_004K, SIZE_256B, "UNKNOWN", ComSrlCmd_SE, SpiRead_11110B,
			       ComSrlCmd_NoneQeBit, PageWrite_111002, 40);
		...
		set_flash_info(ucChip, ui, spi_flash_registed[i].uiCapacityId, spi_flash_registed[i].uiBlockSize,
			       spi_flash_registed[i].uiSectorSize, spi_flash_registed[i].uiPageSize,
			       chip_spi_4b_only ? "MX25L25735F" : spi_flash_registed[i].pcChipName,
			       spi_flash_registed[i].pfErase, spi_flash_registed[i].pfRead,
			       spi_flash_registed[i].pfQeBit, spi_flash_registed[i].pfPageWrite,
			       spi_flash_registed[i].chipClock);
	}
	spi_flash_info[ucChip].pfQeBit(ucChip);
#if DAVO_4B_ONLY_SUPPORT
	spi_flash_info[ucChip].chip_spi_4b_only = chip_spi_4b_only;
#endif
...
void set_flash_info(unsigned char ucChip, unsigned int chip_id, unsigned int device_cap, unsigned int block_size,
		    unsigned int sector_size, unsigned int page_size, char *chip_name, FUNC_ERASE pfErase, FUNC_READ pfRead,
		    FUNC_SETQEBIT pfQeBit, FUNC_PAGEWRITE pfPageWrite, unsigned int chipClock)
{
	...
	case 0xef4019:
	case 0xc22018:		/* MX25L12845E */
	case 0xc22019:		/* */
		spi_flash_info[ucChip].pfBErase = ComSrlCmd_BE;
		break;
	default:
		spi_flash_info[ucChip].pfBErase = NULL;
		break;
	}
#endif
...
//      SPI_REG_LOAD(SFCR, SFCR_SPI_CLK_DIV((ui-2)/2) | SFCR_RBO(uiRBO) | SFCR_WBO(uiWBO) | SFCR_SPI_TCS(uiTCS));
	...
	LDEBUG("setFSCR:uiClkMhz=%d, uiRBO=%d, uiWBO=%d, uiTCS=%d, resMhz=%d, vale=%8x\n", uiClkMhz, uiRBO, uiWBO, uiTCS,
	       spi_flash_info[ucChip].chip_clk, SPI_REG_READ(SFCR));
#endif
}

// Calculate write address group
void calAddr(unsigned int uiStart, unsigned int uiLenth, unsigned int uiSectorSize, unsigned int *uiStartAddr,
	     unsigned int *uiStartLen, unsigned int *uiSectorAddr, unsigned int *uiSectorCount, unsigned int *uiEndAddr,
	     unsigned int *uiEndLen)
{
	unsigned int ui;
	// only one sector
	if ((uiStart + uiLenth) < ((uiStart / uiSectorSize + 1) * uiSectorSize)) {	// start
		*uiStartAddr = uiStart;
		*uiStartLen = uiLenth;
		//middle
		*uiSectorAddr = 0x00;
...
void prnFlashInfo(unsigned char ucChip, struct spi_flash_type sftInfo)
{
	...
	NDEBUG("|%2d %6xh %2xh %7xh %6xh %6xh %7xh %4d %4d %17s|\n", ucChip, sftInfo.chip_id, sftInfo.size_shift,
	       sftInfo.chip_size, sftInfo.block_size, sftInfo.sector_size, sftInfo.page_size, sftInfo.chipClock,
	       sftInfo.chip_clk, sftInfo.chip_name);
...
static void __always_inline noooop(void)
{
#if 1
...
void SFCSR_CS_L(unsigned char ucChip, unsigned char ucLen, unsigned char ucIOWidth)
{
	...
	*((volatile unsigned int *)(SFCSR)) =
	    SFCSR_SPI_CSB(1 + (ucChip)) | SFCSR_LEN(ucLen) | SFCSR_SPI_RDY(1) | SFCSR_IO_WIDTH(ucIOWidth);
}
#endif
// set cs low
#ifdef CONFIG_RTL_8198C
...
void SFCSR_CS_H(unsigned char ucChip, unsigned char ucLen, unsigned char ucIOWidth)
{
	...
	//*((volatile unsigned int *)(SFCSR)) = SFCSR_SPI_CSB(3) | SFCSR_LEN(uiLen) | SFCSR_SPI_RDY(1) |  SFCSR_IO_WIDTH(ucIOWidth) | SFCSR_CHIP_SEL(0) | SFCSR_CMD_BYTE(0);
	*((volatile unsigned int *)(SFCSR)) =
	    SFCSR_SPI_CSB(3) | SFCSR_LEN(ucLen) | SFCSR_SPI_RDY(1) | SFCSR_IO_WIDTH(ucIOWidth);
}
#endif
...
static unsigned int ComSrlCmd_RDJEDEC(unsigned char ucChip, unsigned int addr, unsigned int uiLen)
{
	...
	cmd = 0x5a000000 | addr;
	rstSPIFlash(ucChip);
	SFCSR_CS_L(ucChip, 3, IOWIDTH_SINGLE);
	SPI_REG_LOAD(SFDR, cmd);
	SFCSR_CS_L(ucChip, (uiLen - 1), IOWIDTH_SINGLE);
	ui = SPI_REG_READ(SFDR);
	SFCSR_CS_H(ucChip, 0, IOWIDTH_SINGLE);
	LDEBUG("ComSrlCmd_RDID: ucChip=%x; uiLen=%x; returnValue=%x; SPICMD_RDID=%x;\n",
...
unsigned int ComSrlCmd_EX4B(unsigned char ucChip, unsigned int uiLen)
{
	...
	//LDEBUG("ComSrlCmd_RDID: ucChip=%x; uiLen=%x; returnValue=%x; SPICMD_RDID=%x;\n", ucChip, uiLen, ui, SPICMD_RDID);
	pr_debug("ComSrlCmd_EX4B: ucChip=%x; uiLen=%x; returnValue=%x; SPICMD_EX4B=%x;\n", ucChip, uiLen, ui, SPICMD_EX4B);
...
void SeqCmd_Write(unsigned char ucChip, unsigned char ucIOWidth, unsigned int uiCmd, unsigned int uiValue,
			    ...
			    unsigned char ucIOWidth, unsigned char ucDummyCount)
{
	...
	LDEBUG("ComSrlCmd_InputCommand: ucChip=%x; uiAddr=%x; uiCmd=%x; uiIsfast=%x; ucIOWidth=%x; ucDummyCount=%x\n", ucChip,
	       uiAddr, uiCmd, ucIsFast, ucIOWidth, ucDummyCount);
	// input command
# if 0
...
unsigned int SetSFCR2(unsigned int uiCmd, unsigned char ucIsFast, unsigned char ucIOWidth, unsigned char ucDummyCount)
{
	...
	LDEBUG("SetSFCR2: uiCmd=%x; ucIsFast=%; ucIOWidth=%x; ucDummyCount=%x; ucSFCR2=%x; SFCR2=%x\n;", uiCmd, ucIsFast,
	       ucIOWidth, ucDummyCount, ucSFCR2, ui);
	return ui;
}

// read template
unsigned int ComSrlCmd_ComRead(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer,
			       unsigned int uiCmd, unsigned char ucIsFast, unsigned char ucIOWidth, unsigned char ucDummyCount)
{
...
// write template
unsigned int ComSrlCmd_ComWrite(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer,
				unsigned int uiCmd, unsigned char ucIsFast, unsigned char ucIOWidth, unsigned char ucDummyCount)
{
	unsigned int ui, uiCount, i;
	unsigned char *puc = pucBuffer;
...
unsigned int ComSrlCmd_ComWriteSector(unsigned char ucChip, unsigned int uiAddr, unsigned char *pucBuffer)
{
	...
	LDEBUG("ComSrlCmd_ComWriteSector: ucChip=%x; uiAddr=%x; pucBuffer=%x; returnValue=%x;\n", ucChip, uiAddr,
	       (unsigned int)pucBuffer, spi_flash_info[ucChip].sector_size);
	//prnDispAddr(uiAddr);
	NDEBUG(".");
...
unsigned int ComSrlCmd_ComWriteData(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	...
	calAddr(uiAddr, uiLen, spi_flash_info[ucChip].sector_size, &uiStartAddr, &uiStartLen, &uiSectorAddr, &uiSectorCount,
		&uiEndAddr, &uiEndLen);
...
unsigned int SpiRead_11110B(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG(" SpiRead_11110B: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_FASTREAD=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_FASTREAD);
...
unsigned int PageWrite_111002(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG(" PageWrite_111002: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_PP=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_PP);
...
unsigned int mxic_spi_setQEBit(unsigned char ucChip)
{
	...
	KDEBUG("MxicSetQEBit: ucChip=%d; statusRegister=%x; returnValue=%x\n", ucChip,
	       SeqCmd_Read(ucChip, IOWIDTH_SINGLE, SPICMD_RDSR, 1), ui);
	return ui;
}
...
unsigned int SpiRead_1221BB(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG(" SpiRead_1221BB: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_2READ=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_2READ);
...
unsigned int SpiRead_1443EB(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG(" SpiRead_1443EB: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_4READ=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_4READ);
...
unsigned int PageWrite_144038(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG(" PageWrite_144038: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_4PP=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_4PP);
...
unsigned int span_spi_setQEBit(unsigned char ucChip)
{
	...
	KDEBUG("SpanSetQEBit: ucChip=%d; statusRegister=%x; returnValue=%x\n", ucChip,
	       SeqCmd_Read(ucChip, IOWIDTH_SINGLE, SPICMD_RDSR, 2), ui);
	return spiFlashReady(ucChip);
}
#endif
...
unsigned int PageWrite_114032(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG("PageWrite_114032: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_WB_QPP=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_WB_QPP);
...
unsigned int gd_spi_setQEBit(unsigned char ucChip)
{
	...
	KDEBUG("gd_spi_setQEBit: ucChip=%d; statusRegister=%x; returnValue=%x\n", ucChip,
	       SeqCmd_Read(ucChip, IOWIDTH_SINGLE, SPICMD_RDSR, 2));
	GdCmdSrl_HPM(ucChip);
	return ui;
}
...
unsigned int SpiRead_11213B(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG("SpiRead_11213B: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_AT_READ2=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_AT_READ2);
...
unsigned int PageWrite_1120A2(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char *pucBuffer)
{
	KDEBUG("PageWrite_1120A2: ucChip=%x; uiAddr=%x; uiLen=%x; pucBuffer=%x; SPICMD_AT_PP2=%x;\n", ucChip, uiAddr, uiLen,
	       (unsigned int)pucBuffer, SPICMD_AT_PP2);
...
unsigned int test_spi_flash(unsigned char ucChip)
{
	...
	NDEBUG("\nTest SPI flash: offset->0x%x, step->0x%x\n", (uiOffset - 0x80),
	       (spi_flash_info[ucChip].sector_size * uiStep));
	uiCount = 1;		// Delete
