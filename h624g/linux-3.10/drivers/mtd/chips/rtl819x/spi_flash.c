...
#ifdef __DAVO__
static int do_spi_berase(unsigned int addr, unsigned int uiChip)
{
	unsigned int uiRet;
	uiRet = spi_flash_info[uiChip].pfBErase(uiChip, addr);
	KDEBUG("do_spi_berase: addr=%x;\n", addr);
	return 0;
}
#endif
...
#ifdef __DAVO__
		chip_info->berase = spi_flash_info[chip_select].pfBErase ? do_spi_berase : NULL;
#endif
#if DAVO_4B_ONLY_SUPPORT
		chip_info->spi_4b_only = spi_flash_info[chip_select].chip_spi_4b_only;
		//printk("%s():%d 4b_only(%d)\n", __FUNCTION__, __LINE__, spi_flash_info[chip_select].chip_spi_4b_only);
#endif
