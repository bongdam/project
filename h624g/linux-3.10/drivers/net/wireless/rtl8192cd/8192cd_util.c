...
extern int rtl_sync_l2_wlsta(const u8 *addr);
#endif
...
#if defined(__DAVO__)
	int max_conn = davo_wlan_get_max_conn(priv);
#endif

	SAVE_INT_AND_CLI(flags);

#if defined(__DAVO__)
	if (priv->assoc_num >= max_conn) {
		printk("+현재 접속 단말수:%d 최대 접속 단말수:%d 연결 거부됨[%02x:%02x:%02x:%02x:%02x:%02x]\n",
				priv->assoc_num, max_conn, hwaddr[0], hwaddr[1], hwaddr[2], hwaddr[3], hwaddr[4], hwaddr[5]);
		davo_wlan_inc_drop_count(priv);
		RESTORE_INT(flags);
...
/* APACRTL-94 */
			if (OPMODE & WIFI_STATION_STATE)
				set_repeater_state(priv, pstat, 0);
#endif
...
#ifdef __DAVO__
	/* Test */		{{36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,149,153,157,161}, 19},
#else
...
#ifdef __DAVO__
	/* Test */		{{36,40,44,48,149,153,157,161}, 8},
#else
...
#ifndef __DAVO__
	// commented out by youngho, if below code included, ch52~140 are always excluded at scan, autochan, when DFS is disabled
...
void _txrpt_handler(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct tx_rpt *report)
{
	...
	if (report->txfail != 65535 && report->txfail != 40385) {	//__DAVO__ not to count incorrect reporting
	...
	if (dv_wl_log_enable(priv, WL_LOG_FLAG_FROM_TXS, report->macid)) {
		char log[100];
		snprintf(log, sizeof(log), "[%d] report txok=%u txfail=%u tx_rate=0x%x",
				report->macid, report->txok, report->txfail, report->initil_tx_rate);
		dv_wl_log_update(priv, WL_LOG_FLAG_FROM_TXS, log);
	}
#endif
