...
//#ifndef RTL_WPA2
            ...
            //APACRTL-102 : Check STA is connected completely because of removing unnessary STA informaion.
            pstat->conn_ok = 1;
...
#if 1 // KRACK_PATCH
		if (error_code == _RSON_MIC_FAILURE_) {
			priv->repeater_mic_fail = 1;
		}
#endif
	}
...
#ifdef __DAVO__
	if (OPMODE & WIFI_STATION_STATE) {
		if (pStaInfo->clientHndshkDone == 1 && pStaInfo->clientHndshkProcessing == 1) {
			set_repeater_state(priv, pstat, 1);
		} else  if (pStaInfo->clientHndshkDone == 0 && pStaInfo->clientHndshkProcessing == 1) {
			priv->repeater_mic_fail = 1;
		}
	}
#endif // __DAVO__
