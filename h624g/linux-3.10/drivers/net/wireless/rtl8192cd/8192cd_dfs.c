...
void rtl8192cd_DFS_TXPAUSE_timer(unsigned long task_priv)
{
	...
	pr_debug("rtl8192cd_DFS_TXPAUSE_timer\n");
	...
	if (priv->available_chnl_num != 0) {
		...
		pr_debug("rtl8192cd_DFS_TXPAUSE_timer PATH2\n");
		...
		pr_debug("rtl8192cd_DFS_TXPAUSE_timer,dfsSwitchChannel=%d\n",priv->pshare->dfsSwitchChannel);
#ifdef MBSSID
		...
		pr_debug("rtl8192cd_DFS_TXPAUSE_timer PATH3\n");
...
void rtl8192cd_DFS_timer(unsigned long task_priv)
{
	...
	if (!priv->pmib->dot11DFSEntry.disable_DFS && priv->pshare->rf_ft_var.dfsdbgmode 
		...
		if (GET_CHIP_VER(priv) == VERSION_8192D) {
			...
			if (priv->pshare->rf_ft_var.dfs_det_print1)
				pr_debug("DFS dbg mode count %d, Radar type %s(%x) is detected %08x %d(ms)!\n", priv->pshare->rf_ft_var.dfsdbgcnt,
					(radar_type==0)?"short":"long", radar_type, PHY_QueryBBReg(priv, 0xf98, 0xffffffff), (unsigned int)RTL_JIFFIES_TO_MILISECONDS(jiffies));
	...
	if (!priv->pmib->dot11DFSEntry.disable_DFS && priv->pmib->dot11DFSEntry.DFS_detected) {
		...
		if (GET_CHIP_VER(priv) == VERSION_8192D) {
			...
			pr_debug("Radar is detected as %x %08x (%d)!\n",
		...
		if (timer_pending(&priv->dfs_chk_timer)) {
			...
			if (GET_CHIP_VER(priv) == VERSION_8192D)
				PHY_SetBBReg(priv, 0xcdc, BIT(8)|BIT(9), 1);
			pr_debug("DFS CP2. Switch channel!\n");
		} else {
			if (dfs_chk){
...
void DFS_SwitchChannel(struct rtl8192cd_priv *priv)
{
	...
	pr_debug("2. Swiching channel to %d! bw %d.\n", priv->pmib->dot11RFEntry.dot11channel, priv->pshare->CurrentChannelBW);
