...
#ifdef __DAVO__
	{ SIOCIWCUSTOM, IW_PRIV_TYPE_CHAR | 128, 0, "das" },
#endif
...
int del_sta(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if (!list_empty(&pstat->asoc_list))
	{
		if (IEEE8021X_FUN)
		{
			...
			send_trap_userspace(priv, pstat, DISCONNECT_STA, ETC_STA);
#endif
			event_indicate(priv, macaddr, 2);
#endif
		}
...
#if defined(__DAVO__)
			pInfo->tx_only_data_bytes = pstat->tx_only_data_bytes;	// KDH
			pInfo->tx_only_data_bytes_high = pstat->tx_only_data_bytes_high;
			pInfo->rx_only_data_bytes = pstat->rx_only_data_bytes;	// KDH
			pInfo->rx_only_data_bytes_high = pstat->rx_only_data_bytes_high;
			pInfo->tx_only_data_packets = pstat->tx_only_data_packets;	// KDH
			pInfo->rx_only_data_packets = pstat->rx_only_data_packets;	// KDH
#endif
...
static int iwpriv_atoi(struct rtl8192cd_priv *priv, unsigned char *data, unsigned char *buf, int len)
{
	...
	if ((len - 1) == MACADDRLEN) {	//user send 6 byte mac address
		if (ioctl_copy_from_user(buf, (void *)data, len))
			return -1;
		return 0;
	} else if (((len - 1) == MACADDRLEN * 2) || ((len - 1) == (MACADDRLEN * 2) + 2)) { //user send 12 byte mac string
		if (len == 15) // user send 12 byte mac string + opt(1:allow|2:deny)
			len = 13;
...
static int acl_add_cmd(struct rtl8192cd_priv *priv, unsigned char *data, int len)
{
	...
	if (len == 6)
		...
		paclnode->mode = (unsigned char)opt;
	else
...
static unsigned int get_aclmode(unsigned char *data)
{
	unsigned int mode;
...
#ifdef __DAVO__
static int copy_arg_from_user(struct rtl8192cd_priv *priv,
		void *to, size_t tolen, void *from, size_t fromlen)
{
	if (to == NULL || from == NULL || tolen < fromlen)
		return -1;
	return ioctl_copy_from_user(to, from, fromlen);
}
#endif
...
/* reboot or restart */
	...
	case RTL8192CD_IOCTL_ADD_ACL_TABLE:
	{
        ...
        /* if wlan interface down, ioctl not operation. */
        if(!IS_DRV_OPEN(priv)) {
            printk("%s: this interface is not open\n", __FUNCTION__);
            break;
        }
		...
		if (wrq->u.data.length == 15)
			aclmode = get_aclmode((unsigned char *)(wrq->u.data.pointer));
		else
...
case SIOC_SET_ROUTING_INFO:
	...
	case SIOCIWCUSTOM:
		...
		case RTL8192CD_IOCTL_WLIST_SET:
			if (!copy_arg_from_user(priv, tmpbuf, sizeof_tmpbuf,
				(void *)wrq->u.data.pointer, wrq->u.data.length)) {
				wauth_set_wlist(tmpbuf, wrq->u.data.length, &(priv->wlist));
				ret = 0;
			}
			break;
		case RTL8192CD_IOCTL_WLIST_RDHOST_SET:
			if (!copy_arg_from_user(priv, tmpbuf, sizeof_tmpbuf,
				(void *)wrq->u.data.pointer, wrq->u.data.length)) {
				if ((strncasecmp(tmpbuf, "http://", 7) == 0) ||
				    (strncasecmp(tmpbuf, "https://", 8) == 0))
					snprintf(priv->dv_webrd_host_url,
						 sizeof(priv->dv_webrd_host_url), "%s", tmpbuf);
				else
					snprintf(priv->dv_webrd_host_url,
						 sizeof(priv->dv_webrd_host_url), "http://%s", tmpbuf);
				ret = 0;
			}
			break;
#if defined(DAVO_ENABLE_RATELIMIT)
		case RTL8192CD_IOCTL_RATE_LIMIT_SET:
			if (!copy_arg_from_user(priv, tmpbuf, sizeof_tmpbuf,
				(void *)wrq->u.data.pointer, wrq->u.data.length)) {
				dv_set_rate_limit_info(priv, tmpbuf);
				ret = 0;
			}
			break;
#endif	/* DAVO_ENABLE_RATELIMIT */
		case RTL8192CD_IOCTL_CONNECT_LIMITCOUNT_SET:
			if (!copy_arg_from_user(priv, tmpbuf, sizeof_tmpbuf,
				(void *)wrq->u.data.pointer, wrq->u.data.length)) {
				dv_count_limit_set(priv, tmpbuf);
				ret = 0;
			}
			break;
		case RTL8192CD_IOCTL_RSSI_THRESHOLD_SET:
			if (!copy_arg_from_user(priv, tmpbuf, sizeof_tmpbuf,
				(void *)wrq->u.data.pointer, wrq->u.data.length)) {
				sscanf(tmpbuf, "%d", &rssi_th);
			...
			break;
		case RTL8192CD_IOCTL_WLAN_STATS:
			/* jihyun@davo150617 jcode#2 */
			if (wrq->u.data.pointer) {
				wts.tx_only_data_packets = priv->tx_only_data_packets;
				wts.tx_only_data_bytes = priv->tx_only_data_bytes;
				wts.tx_only_data_bytes_high = priv->tx_only_data_bytes_high;
				wts.rx_only_data_packets = priv->rx_only_data_packets;
				wts.rx_only_data_bytes = priv->rx_only_data_bytes;
				wts.rx_only_data_bytes_high = priv->rx_only_data_bytes_high;
				if (ioctl_copy_to_user((void *)wrq->u.data.pointer, &wts, sizeof(wts)))
					break;
				wrq->u.data.length = sizeof(wts);
				ret = 0;
			}
			break;
		case RTL8192CD_IOCTL_ALL_DEL_STA:
			if (!copy_arg_from_user(priv, tmpbuf, sizeof_tmpbuf,
				(void *)wrq->u.data.pointer, wrq->u.data.length))
				ret = del_allsta(priv, tmpbuf);
			break;
		case RTL8192CD_IOCTL_BANDWIDTH_GET:
			ioctl_copy_to_user((void *)wrq->u.data.pointer, (void *)&priv->pshare->is_40m_bw, wrq->u.data.length);
			ret = 0;
			break;
		case RTL8192CD_IOCTL_CONTROLSIDEBAND_GET:
			ioctl_copy_to_user((void *)wrq->u.data.pointer, (void *)&priv->pshare->offset_2nd_chan, wrq->u.data.length);
			ret = 0;
			break;

		default:
			printk("Unknown sub-command(%u) of SIOCIWCUSTOM\n", wrq->u.data.flags);
			break;
		}
		break;
