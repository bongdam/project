...
#endif	
#if defined(__DAVO__)
			// 20161003 to fix wrong RSS calcuation by 11b rates.
			// The RSSI of 11b rate is not reliable at current S/W, 3.4.9.xx
			if (pfrinfo->RX_RATE <= 3) { // 11b rate (1,2,5,11Mbps)
				if ((priv->up_time - pstat->rx_ofdm_rate_time) > 3) {
#if 0
					pstat->rssi = cal_rssi_avg(pstat->rssi, pfrinfo->rssi/3);
#else
					if ( (pstat->rssi < 35) && (pfrinfo->rssi < 40) ) {
						pstat->rssi = cal_rssi_avg(pstat->rssi, pfrinfo->rssi/3);
					} else {
				...
				pstat->rx_ofdm_rate_time = priv->up_time;
				pstat->rssi = cal_rssi_avg(pstat->rssi, pfrinfo->rssi);
			}
#else
			pstat->rssi             = cal_rssi_avg(pstat->rssi, pfrinfo->rssi);
