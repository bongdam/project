...
#endif
#if defined(__DAVO__)
//davo@jihyun add wlan statistics
	unsigned int		tx_only_data_packets;
	unsigned int		rx_only_data_packets;
	unsigned int		tx_only_data_bytes;
	unsigned int		tx_only_data_bytes_high;
	unsigned int		rx_only_data_bytes;
	unsigned int		rx_only_data_bytes_high;
	unsigned int		tx_bytes_h;
	unsigned int		rx_bytes_h;
///
...
#ifdef __DV_BSTEER__
    //APACRTL-91
    unsigned long bs_ip_pkt_cnt_list[100];
    unsigned char bs_ip_pkt_cnt_list_cur_index;
    unsigned char progress_in_bsteer;
// APACRTL-502
    unsigned char have_bsteered;
    unsigned char pps_check_time;
    unsigned long bs_last_ip_packets;
	struct _bs_rssi_hist bs_rssi;
#endif
#if defined(__DAVO__)
	unsigned long rx_ofdm_rate_time;
	unsigned long assoc_sec;
#endif
};
...
//	unsigned char		mp_dig_enable;
	...
	unsigned char		cfo_tracking;
	unsigned char		afb_ft;
	unsigned char		afb_dbg;
	unsigned char		afb_force;
	unsigned char		rx_pwr_7;
	unsigned char		rx_pwr_6;
	unsigned char		rx_pwr_5;
	unsigned char		rx_pwr_4;
	unsigned char		rx_pwr_3;
	unsigned char		rx_pwr_2;
	unsigned char		rx_pwr_1;
	unsigned char		rx_pwr_0;
	unsigned char		tp_dbg;
	unsigned char		rts_force;
	unsigned char		norts_h;
	unsigned char		norts_l;
	unsigned char		no_sgi;
	unsigned int		vhtmask;
	unsigned char		addba_bufsz;
	unsigned char		ampdu_buf;
	unsigned char		ampdu_den;
	unsigned char		ampdu_den_vht;
	unsigned char		maxAggNum;
...
#ifdef __DAVO__
	unsigned short			LED_Continuous_Nodata;
#endif
...
#endif
#ifdef __DAVO__
	/* The declaration position MUST be moved up here for our proprietary fields.
	 * Because rtl8192cd_open function filled zero from net_stats field, which resulted
	 * in unexpected disappearance of our proprietary fields.
	 * young@davolink.co.kr 2013-12-16
	 */
	//davo@jihyun add wlan statistics
	unsigned long   rx_only_data_bytes;
	unsigned long   rx_only_data_bytes_high;
	unsigned long   tx_only_data_bytes;
	unsigned long   tx_only_data_bytes_high;
	unsigned long   rx_only_data_packets;
	unsigned long   tx_only_data_packets;
	void 		*wlist;
	char dv_webrd_host_url[256];
	/* davo@jihyun rssi thresh-hold */
	int		establish_rssi_threshold;
	unsigned char dv_priv_mssid_index;
...
/* APACRTL-94 */
	unsigned int repeater_probe;
	unsigned char repeater_connect;
	unsigned char repeater_mic_fail;
	unsigned char encrypt_diff;
#endif
#ifdef __DV_BSTEER__
	int		bsteer_enable;
#endif
...
//	unsigned char			*pBeaconErp;			// ptr of ERP field in beacon buf
	...
	unsigned char			chnl_ss_fa_lv[MAX_CHANNEL_NUM];	// record fa_lv for dumping
	unsigned char			chnl_ss_cca_lv[MAX_CHANNEL_NUM];	// record cca_lv for dumping
#endif
...
#define for_each_stat_info(psta, head, member) \
	if ((head)->next != NULL) \
		list_for_each_entry(psta, head, member)
...
#define SIOCIWCUSTOM	0x8BFA

enum {
	RTL8192CD_IOCTL_WLIST_SET = 1024,
	RTL8192CD_IOCTL_WLIST_RDHOST_SET,
	RTL8192CD_IOCTL_CONNECT_LIMITCOUNT_SET,
	RTL8192CD_IOCTL_RATE_LIMIT_SET,
	RTL8192CD_IOCTL_RSSI_THRESHOLD_SET,
	/* jihyun@davo150617 jcode#2 */
	RTL8192CD_IOCTL_WLAN_STATS,
	RTL8192CD_IOCTL_ALL_DEL_STA,
	RTL8192CD_IOCTL_BANDWIDTH_GET,
	RTL8192CD_IOCTL_CONTROLSIDEBAND_GET
};

static inline int CONV_TO_RSSI(int percent)
{
...
#define REPEATER_CONNECT		0x0
#define REPEATER_SSID_NOTFOUND	0x1
#define REPEATER_ENCRYPT_DIFF	0x2
#define REPEATER_PASSWORD_FAIL	0x4

/* jihyun@davo160202 jcode#7 -*/

#define CONNECT_STA				0x1
#define DISCONNECT_STA			0x2
#define CONNECT_FAIL_STA		0x4
#define TRY_HANDOVER_STA		(DISCONNECT_STA|0x80)

#define NONE					0
#define LEAVED_STA				8
#define MANAGER_FORCE			68
#define EXPIRED_STA				69
#define HANDOVER_STA			72
#define ETC_STA					1

#define MICFIAL_KEY				14
#define TIMEOUT_KEY				15
#define ETC_KEY					1

struct monitor_sta_t {
	unsigned short aid;
	unsigned char mac[6];
	unsigned char rssi;
	unsigned long link_time;
