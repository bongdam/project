...
#ifdef __DAVO__
static inline bool isEqualMACAddr(const u8 *addr1, const u8 *addr2)
{
...
static __inline__ int wifi_mac_hash(unsigned char *mac)
{
	...
	x = mac[3] ^ mac[4] ^ mac[5];
#endif
#if ((NUM_STAT & (NUM_STAT - 1)) == 0)		/* NUM_STAT is a power of 2 */
	return x & (NUM_STAT - 1);
#elif ((NUM_STAT & (NUM_STAT + 1)) == 0)	/* NUM_STAT is 1 less than a power of 2 */
	x &= NUM_STAT;
	return (x < NUM_STAT) ? x : 0;
#else
	return x % NUM_STAT;
#endif
}
...
#else
static __inline__ void tx_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, int pktlen, unsigned char *pframe)
#endif
{
	...
	if (pframe && (GetFrameType(pframe) == WIFI_DATA_TYPE) && 
	    (((subtype= GetFrameSubType(pframe)) == WIFI_DATA) || (subtype == WIFI_QOS_DATA))) {
		only_data = 1;
	}
...
#if defined(__DAVO__)
		/* KDH, add only data bytes and packets */
		/* APNRTL-245:  change means of only data: data or qos data in subtype*/
		if (only_data) {
#if 1
			if (priv->tx_only_data_bytes + pktlen < priv->tx_only_data_bytes) 
				priv->tx_only_data_bytes_high++;
			priv->tx_only_data_bytes += pktlen;
			priv->tx_only_data_packets++;
#endif
		}
		/* END DAVO */
#endif
	...
	if (pstat) {
#if defined(__DAVO__)
		unsigned int bytes;		// youngho DAVO
#endif
#if defined(TXREPORT)
#ifdef CONFIG_RTL_88E_SUPPORT
...
#if defined(__DAVO__)
		bytes=pstat->tx_bytes;		// youngho DAVO
		/* KDH, add only data bytes and packets */
		/* APNRTL-245:  change means of only data: data or qos data in subtype*/
		if (only_data) {
			if (pstat->tx_only_data_bytes + pktlen < pstat->tx_only_data_bytes)
				pstat->tx_only_data_bytes_high++;
			pstat->tx_only_data_bytes += pktlen;
			pstat->tx_only_data_packets++;
		}
		/* END DAVO */

		if (bytes > pstat->tx_bytes) {	// overflow		// youngho DAVO
			pstat->tx_bytes_h++;
		}
#endif
	}
...
static __inline__ void rx_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, int pktlen, int retry, unsigned char *pframe)
{
	struct net_device_stats *pnet_stats;
	unsigned int subtype = 0;
	int only_data = 0;

	if (GetFrameType(pframe) == WIFI_DATA_TYPE && (((subtype= GetFrameSubType(pframe)) == WIFI_DATA) || (subtype == WIFI_QOS_DATA))){
		only_data = 1;
	}
	...
	if (priv) {
		...
		/* APNRTL-245:  change means of only data: data or qos data in subtype*/
		if (only_data) {
			if (priv->rx_only_data_bytes + pktlen < priv->rx_only_data_bytes) 
				priv->rx_only_data_bytes_high++;
			priv->rx_only_data_bytes += pktlen;
			priv->rx_only_data_packets++;
		}
		/* END DAVO */
#endif
		if (retry)
	...
	if (pstat) {
#if defined(__DAVO__)
		unsigned int bytes;		// youngho DAVO
		bytes=pstat->rx_bytes;		// youngho DAVO
#endif
		pstat->rx_pkts++;
		...
		/* APNRTL-245:  change means of only data: data or qos data in subtype*/
		if (only_data) {
			if (pstat->rx_only_data_bytes + pktlen < pstat->rx_only_data_bytes)
				pstat->rx_only_data_bytes_high++;
			pstat->rx_only_data_bytes += pktlen;
			pstat->rx_only_data_packets++;
		}
		/* END DAVO */

		if (bytes > pstat->rx_bytes) {	// overflow		// youngho DAVO
			pstat->rx_bytes_h++;
		}
#endif
