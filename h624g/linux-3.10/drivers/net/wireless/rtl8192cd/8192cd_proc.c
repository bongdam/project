...
#ifdef __DAVO_SKT_AUTOCHAN__
extern void dv_get_bss_bw_offset(struct bss_desc *pBss, int *bw, int *offset);

#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_SSR_cur_read(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_SSR_cur_read(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif
{
	struct net_device *dev = PROC_GET_DEV();
	...
	if(priv->ss_req_ongoing)	{
		...
		PRINT_ONE(" SiteSurvey result (cur) : ", "%s", 1);
		PRINT_ONE("BSSID,SSID,Channel,BW,2nd_SB,RSSI,TYPE", "%s", 1);
		PRINT_ONE("====================", "%s", 1);
		if(priv->site_survey->count==0){
			...
			for(i1=0; i1<priv->site_survey->count;i1++){
				base = pos;
				dv_get_bss_bw_offset(&priv->site_survey->bss[i1], &bw, &offset);
				memcpy(tmp,priv->site_survey->bss[i1].bssid,MACADDRLEN);
				PRINT_ARRAY(tmp, "%02x", MACADDRLEN, 0);
				PRINT_ONE(priv->site_survey->bss[i1].channel, ",%d", 0);
				PRINT_ONE(priv->site_survey->bss[i1].ssid, ",%s", 0);
				PRINT_ONE((bw==HT_CHANNEL_WIDTH_80)?80:((bw==HT_CHANNEL_WIDTH_20_40)?40:20), ",%d", 0);
				PRINT_ONE((offset==HT_2NDCH_OFFSET_BELOW) ? "B":((offset==HT_2NDCH_OFFSET_ABOVE)?"A":"0"), ",%s", 0);
				PRINT_ONE(priv->site_survey->bss[i1].rssi, ",%d", 0);
				PRINT_ONE(((priv->site_survey->bss[i1].bsstype == 16) ? "AP" : "Ad-Hoc"), ",%s", 1);
...
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_dv_autochan_res(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_dv_autochan_res(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif
{
	struct net_device *dev = PROC_GET_DEV();
...
#define SZ_TMP_BUF 2048
	buf = kmalloc(SZ_TMP_BUF, GFP_ATOMIC);
	if (buf) {
		csel_dump(is_5g, buf, SZ_TMP_BUF);
		PRINT_ONE(buf, "%s", 1);
		kfree(buf);
	} else {
		PRINT_ONE("kmalloc failed", "%s", 1);
	}
	len = pos;
#undef SZ_TMP_BUF
...
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_dv_chanutil_dump(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_dv_chanutil_dump(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif
{
	struct net_device *dev = PROC_GET_DEV();
	...
	PRINT_ONE("chan,load,fa_cnt,cca_count,fa_lv,cca_lv", "%s", 1);

	for (i=0;i<priv->available_chnl_num;i++) {
		PRINT_ONE(priv->available_chnl[i],"%d", 0);
		PRINT_ONE(priv->chnl_ss_load[i],",%d", 0);
		PRINT_ONE(priv->chnl_ss_fa_count[i],",%d", 0);
		PRINT_ONE(priv->chnl_ss_cca_count[i],",%d", 0);
		PRINT_ONE(priv->chnl_ss_fa_lv[i],",%d", 0);
		PRINT_ONE(priv->chnl_ss_cca_lv[i],",%d", 1);
	}
...
#endif
{
	...
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    tx_bytes_h: ", pstat->tx_bytes_h, "%u");	// youngho DAVO
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    tx_only_data_bytes: ", pstat->tx_only_data_bytes, "%u"); /* KDH, add only data bytes transmitted */
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    tx_only_data_bytes_high: ", pstat->tx_only_data_bytes_high, "%u"); /* KDH, add tx_only_data_bytes high order */
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    tx_pkts: ", pstat->tx_pkts, "%u");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    tx_only_data_packets: ", pstat->tx_only_data_packets, "%u"); /* KDH, add only data packets transmitted */
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    rx_bytes: ", pstat->rx_bytes, "%u");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    rx_bytes_h: ", pstat->rx_bytes_h, "%u");	// youngho DAVO
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    rx_only_data_bytes: ", pstat->rx_only_data_bytes, "%u"); /* KDH, add only data bytes received */
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    rx_only_data_bytes_high: ", pstat->rx_only_data_bytes_high, "%u"); /* KDH, add rx_only_data_bytes high order */
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    rx_pkts: ", pstat->rx_pkts, "%u");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    rx_only_data_packets: ", pstat->rx_only_data_packets, "%u"); /* KDH, add only data packets received */
	CHECK_LEN_B;
...
#endif
{
	...
	PRINT_SINGL_ARG("    tx_only_data_packets: ", priv->tx_only_data_packets, "%lu"); /* KDH, add only data packets transmitted  */
	PRINT_SINGL_ARG("    tx_bytes:      ", priv->net_stats.tx_bytes, "%lu");
	PRINT_SINGL_ARG("    tx_only_data_bytes: ", priv->tx_only_data_bytes, "%lu"); /* KDH, add only data bytes transmitted  */
	PRINT_SINGL_ARG("    tx_only_data_bytes_high: ", priv->tx_only_data_bytes_high, "%lu"); /* KDH, add tx_only_data_bytes high order */
	...
	PRINT_SINGL_ARG("    rx_only_data_packets: ", priv->rx_only_data_packets, "%lu"); /* KDH, add only data packets received */
	PRINT_SINGL_ARG("    rx_bytes:      ", priv->net_stats.rx_bytes, "%lu");
	PRINT_SINGL_ARG("    rx_only_data_bytes: ", priv->rx_only_data_bytes, "%lu"); /* KDH, add only data bytes received */
	PRINT_SINGL_ARG("    rx_only_data_bytes_high: ", priv->rx_only_data_bytes_high, "%lu"); /* KDH, add rx_only_data_bytes high order */
...
#if defined(__DAVO__) && defined(CONFIG_RTL_PROC_NEW)
static int rtl8192cd_proc_led_read(struct seq_file *s, void *data)
{
	struct net_device *dev = PROC_GET_DEV();
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);

	seq_printf(s, "Interval msec   : %u\n", jiffies_to_msecs(priv->pshare->LED_Interval));
	seq_printf(s, "Toggle start    : %s\n", (priv->pshare->LED_ToggleStart == LED_OFF) ? "Off" : "On");
	seq_printf(s, "Toggle current  : %s\n", (priv->pshare->LED_Toggle == LED_OFF) ? "On" : "Off");
	seq_printf(s, "Quiescent period: %u\n", priv->pshare->LED_Continuous_Nodata);
	seq_printf(s, "Blinking freq   : %u\n", priv->pshare->rf_ft_var.ledBlinkingFreq);
	return 0;
}
#endif
...
#else
	if (strtrim_from_user(tmpbuf, sizeof(tmpbuf), buffer, count) > 0) {
				...
				unsigned long count, void *data)
{
    ...
    if (strtrim_from_user(tmp, sizeof(tmp), buffer, count) > 0) { //read cmd from proc file
		/*
		if (NULL == phydm_msg) {
			phydm_msg = rtw_zmalloc(PHYDM_MSG_LEN);
			if (NULL == phydm_msg)
...
#ifdef __DV_BSTEER__
static int dv_proc_read_bsteer_enable(struct seq_file *s, void *v)
{
	struct net_device *dev = PROC_GET_DEV();
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	int pos = 0;

	PRINT_ONE(priv->bsteer_enable, "%d", 1);

	return pos;
}

static int dv_proc_write_bsteer_enable(struct file *file, const char *buffer, unsigned long count, void *data)
{
	struct net_device *dev = (struct net_device *)data;
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	char tmp[128];
...
#ifdef __DAVO__
static int dv_proc_read_establish_rssi_threshhold(struct seq_file *s, void *v)
{
	struct net_device *dev = PROC_GET_DEV();
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	int pos = 0;

	PRINT_ONE(priv->establish_rssi_threshold, "%d", 1);

	return pos;
}

static int dv_proc_write_establish_rssi_threshhold(struct file *file, const char *buffer, unsigned long count, void *data)
{
	struct net_device *dev = (struct net_device *)data;
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	char tmp[128];
	...
	if (strtrim_from_user(tmp, sizeof(tmp), buffer, count) > 0) {
		sscanf(tmp, "%d", &rssi_th);

		if (rssi_th >= 20 && rssi_th <= 80)
			priv->establish_rssi_threshold = 100 - rssi_th;
		else
			priv->establish_rssi_threshold = 0;
	}

	return count;
}

static char *str_rfi_level(int level)
{
	if (level >= RFI_VERY_HIGH) return "very high";
	else if (level >= RFI_HIGH) return "high";
	else if (level >= RFI_MEDIUM) return "medium";
	else if (level >= RFI_LOW) return "low";

	return "none";
}

static int dv_proc_read_rfi_test_mode(struct seq_file *s, void *v)
{
	struct net_device *dev = PROC_GET_DEV();
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	int pos = 0, i, ret;
	dv_rf_chk_t rfi_score[3];

	if (IS_ROOT_INTERFACE(priv)) {
		PRINT_SINGL_ARG("mode:", priv->dv_rf.mode, "%d");

		ret = dv_get_rfi_score(priv, rfi_score);
		if (ret > 0) {
			char *str[] = { "3min", "15min", "1hour" };
			PRINT_ONE("RF interference score:", "%s", 0);
			PRINT_ONE(priv->dv_rf.avg_rfi_level, "%d", 0);
			PRINT_ONE(str_rfi_level(priv->dv_rf.avg_rfi_level), "(%s)", 1);

			PRINT_ONE("time    dur     fa      cca    mac-rx  rfi_lv", "%s", 1);
			PRINT_ONE("-----------------------------------------------", "%s", 1);
			for (i=0; i<ret; i++) {
				PRINT_ONE(str[i], "%-8s", 0);
				PRINT_ONE(rfi_score[i].dur, "%-8d", 0);
				PRINT_ONE(rfi_score[i].fa,  "%-8u", 0);
				PRINT_ONE(rfi_score[i].cca, "%-8u", 0);
				PRINT_ONE(rfi_score[i].mac_rx, "%-8u", 0);
				PRINT_ONE(rfi_score[i].rfi_level, "%-8d", 1);
			}
		}
	} else {
		PRINT_ONE("N/A", "%s", 1);
...
static int dv_proc_write_rfi_test_mode(struct file *file, const char *buffer, unsigned long count, void *data)
{
	struct net_device *dev = (struct net_device *)data;
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	char tmp[128];
	...
	if (strtrim_from_user(tmp, sizeof(tmp), buffer, count) > 0) {
		if (sscanf(tmp, "%d", &mode)) {
			if (mode >= DV_RF_MODE_NONE && mode <= DV_RF_CHK_ACT3)
				dv_set_rf_env_mode(priv, mode);
		}
	}
...
static char *calc_mbps(unsigned long bytes, char *buf, int sz)
{
    unsigned long quotient, remainder;

    quotient = bytes/125000;
    remainder = (bytes%125000)*8;

    if (quotient<9) {
        remainder /= 1000;
        snprintf(buf, sz, "%lu.%03lu", quotient, remainder);
    } else if (quotient<99) {
        remainder /= 10000;
        snprintf(buf, sz, "%lu.%02lu", quotient, remainder);
    } else {
        if (quotient>=1000) quotient = 999;	// over 1G
        remainder /= 100000;
        snprintf(buf, sz, "%lu.%lu", quotient, remainder);
    }
    return (buf);
}

static int dv_proc_read_rfi_hist(struct seq_file *s, void *v)
{
	struct net_device *dev = PROC_GET_DEV();
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	dv_rf_env_t	rf_env;
	...
	if (dv_get_rf_env_log(priv, &rf_env)) {
		PRINT_ONE("idx      fa     cca  mac-rx  tx-Mbps  rx-Mbps", "%s", 1);
		PRINT_ONE("-------------------------------------------", "%s", 1);
		i=rf_env.index;
		for (j=0; j<MAX_LOG_HIST; j++) {
			i = (MAX_LOG_HIST+i-1)%MAX_LOG_HIST;
			PRINT_ONE(j, "%3d", 0);
			PRINT_ONE(rf_env.cnt[i].fa, "%8u", 0);
			PRINT_ONE(rf_env.cnt[i].cca, "%8u", 0);
			PRINT_ONE(rf_env.cnt[i].mac_rx, "%8u", 0);
			PRINT_ONE(calc_mbps(rf_env.cnt[i].tx_bytes, buff, sizeof(buff)), "%8s", 0);
			PRINT_ONE(calc_mbps(rf_env.cnt[i].rx_bytes, buff, sizeof(buff)), "%8s", 1);
		}
	} else {
		PRINT_ONE("N/A", "%s", 1);
	}
...
static int dv_proc_read_rfi_status(struct seq_file *s, void *v)
{
	struct net_device *dev = PROC_GET_DEV();
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	int pos = 0, i, j, first;
	...
	if (IS_ROOT_INTERFACE(priv)) {
		PRINT_ONE("mode :", "%s", 0);
		PRINT_ONE(priv->dv_rf.mode, "%d", 1);
		PRINT_ONE("avg rfi :", "%s", 0);
		PRINT_ONE(priv->dv_rf.avg_rfi_level, "%d", 0);
		PRINT_ONE(str_rfi_level(priv->dv_rf.avg_rfi_level), "(%s)", 1);
		i=priv->dv_rf.index_min;
		first = 1;
		for (j=0; j<MAX_CHK_MINUTES; j++) {
			if (priv->dv_rf.chk_mins[i].dur > 0) {
				if (first) {
					first = 0;
					PRINT_SINGL_ARG("minutes index :", priv->dv_rf.index_min, "%d");
					PRINT_ONE("dur     fa      cca     mac-rx  rfi_lv", "%s", 1);
					PRINT_ONE("---------------------------------------", "%s", 1);
				}
				PRINT_ONE(priv->dv_rf.chk_mins[i].dur, "%-8d", 0);
				PRINT_ONE(priv->dv_rf.chk_mins[i].fa,  "%-8lu", 0);
				PRINT_ONE(priv->dv_rf.chk_mins[i].cca, "%-8lu", 0);
				PRINT_ONE(priv->dv_rf.chk_mins[i].mac_rx, "%-8lu", 0);
				PRINT_ONE(priv->dv_rf.chk_mins[i].rfi_level, "%-8d", 1);
			}
			i = (i+1)%MAX_CHK_MINUTES;
		}
		i=priv->dv_rf.index_hour;
		first = 1;
		for (j=0; j<MAX_CHK_HOURS; j++) {
			if (priv->dv_rf.chk_hours[i].dur > 0) {
				if (first) {
					first = 0;
					PRINT_SINGL_ARG("hours index :", priv->dv_rf.index_hour, "%d");
					PRINT_ONE("dur     fa      cca     mac-rx  rfi_lv", "%s", 1);
					PRINT_ONE("---------------------------------------", "%s", 1);
				}
				PRINT_ONE(priv->dv_rf.chk_hours[i].dur, 	"%-8d", 0);
				PRINT_ONE(priv->dv_rf.chk_hours[i].fa, 		"%-8lu", 0);
				PRINT_ONE(priv->dv_rf.chk_hours[i].cca, 	"%-8lu", 0);
				PRINT_ONE(priv->dv_rf.chk_hours[i].mac_rx,  "%-8lu", 0);
				PRINT_ONE(priv->dv_rf.chk_hours[i].rfi_level, "%-8d", 1);
			}
			i = (i+1)%MAX_CHK_HOURS;
		}
	} else {
...
#ifdef CLIENT_MODE
/* APACRTL-94 */
static int dv_proc_read_repeater_state(struct seq_file *s, void *v)
{
	struct net_device *dev = PROC_GET_DEV();
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	int pos = 0;
...
#if 0
	printk("repeater connect:%d 4way:%d mic_fail:%d probe:%d diff:%d\n",
			priv->repeater_connect, priv->dot114WayStatus, priv->repeater_mic_fail,
			priv->repeater_probe, priv->encrypt_diff);
#endif
	if (priv->repeater_connect == 1) {
		PRINT_ONE(REPEATER_CONNECT, "%d", 1);
	} else if (priv->encrypt_diff == 0 && priv->repeater_probe == 0) {
		PRINT_ONE(REPEATER_ENCRYPT_DIFF, "%d", 1);
	} else if (priv->dot114WayStatus > 0 || priv->repeater_mic_fail > 0) {
		PRINT_ONE(REPEATER_PASSWORD_FAIL, "%d", 1);
	} else {
		PRINT_ONE(REPEATER_SSID_NOTFOUND, "%d", 1);
	}
...
#endif
#if defined(__DAVO__) && defined(CONFIG_RTL_PROC_NEW)
	RTK_DECLARE_READ_WRITE_PROC_FOPS(rtl8192cd_proc_led_read, rtl8192cd_proc_led);
#else
...
#endif
#ifdef __DAVO_SKT_AUTOCHAN__
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_SSR_cur_read);
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_dv_autochan_res);
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_dv_chanutil_dump);
#endif
#if defined(RTLWIFINIC_GPIO_CONTROL)
...
#endif
#ifdef __DV_BSTEER__
	RTK_DECLARE_READ_WRITE_PROC_FOPS(dv_proc_read_bsteer_enable, dv_proc_write_bsteer_enable);
#endif
#ifdef __DAVO__
	RTK_DECLARE_READ_WRITE_PROC_FOPS(dv_proc_read_establish_rssi_threshhold, dv_proc_write_establish_rssi_threshhold);

	RTK_DECLARE_READ_WRITE_PROC_FOPS(dv_proc_read_rfi_test_mode, dv_proc_write_rfi_test_mode);
	RTK_DECLARE_READ_PROC_FOPS(dv_proc_read_rfi_hist);
	RTK_DECLARE_READ_PROC_FOPS(dv_proc_read_rfi_status);
#ifdef CLIENT_MODE
	RTK_DECLARE_READ_PROC_FOPS(dv_proc_read_repeater_state);
#endif
#endif
...
void MDL_DEVINIT rtl8192cd_proc_init(struct net_device *dev)
{
...
#endif
    {
...
#if defined(__DAVO__) && defined(CONFIG_RTL_PROC_NEW)
		RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "led", rtl8192cd_proc_led_read, rtl8192cd_proc_led);
#else
...
#endif
#ifdef __DAVO_SKT_AUTOCHAN__
		RTK_CREATE_PROC_READ_ENTRY(p, "SS_Result_cur", rtl8192cd_proc_SSR_cur_read);
		RTK_CREATE_PROC_READ_ENTRY(p, "dv_autochan_res", rtl8192cd_proc_dv_autochan_res);
		RTK_CREATE_PROC_READ_ENTRY(p, "dv_chanutil_dump", rtl8192cd_proc_dv_chanutil_dump);
#endif
        ...
        p->size = 0x10;
        #endif
	/* Restore this line without which LED(wlan1) could not be turned on
	 * young 2015-06-17
	 */
...
#endif
#ifdef __DV_BSTEER__
	RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "bsteer_enable",
			dv_proc_read_bsteer_enable,
			dv_proc_write_bsteer_enable);
#endif
#ifdef __DAVO__
	RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "rssi_threshold",
			dv_proc_read_establish_rssi_threshhold,
			dv_proc_write_establish_rssi_threshhold);

	RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "rfi_test_mode",
			dv_proc_read_rfi_test_mode,
			dv_proc_write_rfi_test_mode);
	RTK_CREATE_PROC_READ_ENTRY(p, "rfi_hist", dv_proc_read_rfi_hist);
	RTK_CREATE_PROC_READ_ENTRY(p, "rfi_status", dv_proc_read_rfi_status);
#ifdef CLIENT_MODE
	RTK_CREATE_PROC_READ_ENTRY(p, "repeater_state", dv_proc_read_repeater_state);
#endif
#endif
...
#endif
#ifdef __DAVO_SKT_AUTOCHAN__
            remove_proc_entry( "SS_Result_cur", rtl8192cd_proc_root );
            remove_proc_entry( "dv_autochan_res", rtl8192cd_proc_root );
            remove_proc_entry( "dv_chanutil_dump", rtl8192cd_proc_root );
#endif
...
#endif
#ifdef __DAVO__
        remove_proc_entry( "rssi_threshold", rtl8192cd_proc_root );
#endif
...
#endif
#ifdef __DAVO_SKT_AUTOCHAN__
	{"SS_Result_cur",		rtl8192cd_proc_SSR_cur_read},
	{"dv_autochan_res",		rtl8192cd_proc_dv_autochan_res},
	{"dv_chanutil_dump",	rtl8192cd_proc_dv_chanutil_dump},
#endif
...
#endif
#ifdef __DV_BSTEER__
	{"bsteer_enable", dv_proc_read_bsteer_enable},
#endif
#ifdef __DAVO__
	{"rssi_threshold", dv_proc_read_establish_rssi_threshhold},
#ifdef CLIENT_MODE
	{"repeater_state", dv_proc_read_repeater_state},
#endif
#endif
