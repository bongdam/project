...
#if defined(__DAVO__)
struct net_device *br_fdb_locate(struct net_bridge *br, u8 *addr);
#endif
...
#ifdef __DAVO__
	struct pf_hook_ops *h;
	struct webrd_private wprv = { .pstat = pstat,
				      .wlist = priv->wlist,
				      .priv = priv };
#endif
	int disable_brsc = priv->pmib->dot11OperationEntry.disable_brsc;
#ifdef CONFIG_RTL_DNS_TRAP
...
#ifdef __DAVO__
	list_for_each_entry(h, &pf_rx_chains, list) {
		if (h->hook(h, pskb, (void *)&wprv) == PF_DISCARD) {
			priv->ext_stats.rx_data_drops++;
			rtl_kfree_skb(priv, pskb, _SKB_RX_);
			return;
		}
	}
	if (snoop_dhcp && !snoop_dhcp_packet(pskb))
		disable_brsc = 1;
#endif
	...
	if(rtl_IsMcastIP(pskb)==0)
	{
		...
		if(*(unsigned short *)(pskb->data+ETH_ALEN*2) != htons(0x88d9))
		{
			...
			if ((OPMODE & WIFI_AP_STATE) && pskb->dev) {
				if (!(pskb->data[0] & 0x01) && !disable_brsc && priv->reperater_idx
...
#if defined(__DAVO__)	/* APACRTL-213, for rx frame statistics */
	if (frame_type == WIFI_BEACON) {
		dv_inc_rxpkt_cnt(priv, beacon);
	} else if (frame_type == WIFI_PROBEREQ) {
		dv_inc_rxpkt_cnt(priv, prbreq);
	} else if (frame_type==WIFI_ASSOCREQ || frame_type==WIFI_REASSOCREQ) {
		dv_inc_rxpkt_cnt(priv, assoc);
	} else if (frame_type==WIFI_AUTH) {
		dv_inc_rxpkt_cnt(priv, auth);
	} else if (frame_type==WIFI_DEAUTH || frame_type==WIFI_DISASSOC) {
		dv_inc_rxpkt_cnt(priv, deauth);
	}
#endif
...
//		printk("pfrinfo->hdr_len == 0\n");
	...
	{
#if defined(__DAVO__) && defined(MBSSID)	// jmchoi@davolink.co.kr 2015.06.26
		if (vap_idx >= 0) {
			if (priv->pvap_priv[vap_idx]->pmib->dot11OperationEntry.opmode & WIFI_AP_STATE) {
				if ((pfrinfo->to_fr_ds == 0x01) || // ToDs=0, FromDs=1, from another AP
					(!IS_BSSID(priv->pvap_priv[vap_idx], GetAddr1Ptr(pframe))) ) {
#if defined(__DAVO__)	/* APACRTL-213, for rx frame statistics */
					dv_inc_rxpkt_cnt(priv, obss);
#endif
					return 1;	// return reuse
				}
			}
		} else {
			if (opmode & WIFI_AP_STATE) {
				if ((pfrinfo->to_fr_ds == 0x01) || // ToDs=0, FromDs=1, from another AP
					(!IS_BSSID(priv, GetAddr1Ptr(pframe))) ) {
#if defined(__DAVO__)	/* APACRTL-213, for rx frame statistics */
					dv_inc_rxpkt_cnt(priv, obss);
#endif
					return 1;	// return reuse
...
#endif	/*#if defined(__DAVO__) && defined(MBSSID)*/
...
#if defined(__DAVO__)	/* APACRTL-213, for rx frame statistics */
			dv_inc_rxpkt_cnt(priv, mgnt);
			if (IS_BSSID(priv, GetAddr1Ptr(pframe))
#ifdef MBSSID
				|| vap_idx >= 0
#endif
			   ) {
			   	dv_inc_rxpkt_cnt(priv, ibss);
			} else {
				dv_inc_rxpkt_cnt(priv, obss);
			}
#endif
...
#if defined(__DAVO__)	/* APACRTL-213, for rx frame statistics */
			dv_inc_rxpkt_cnt(priv, data);
			if (IS_BSSID(priv, GetAddr3Ptr(pframe))
#ifdef MBSSID
				|| vap_idx >= 0
#endif
			   ) {
...
#endif	/* APACRTL-213, for rx frame statistics */#endif
...
#if defined(__DAVO__)
			// we can use this rate information later. It doesnot need notation conversion.
			pfrinfo->RX_RATE = (unsigned char)rx_desc_status.RX_RATE;
#endif
			if (!pfrinfo->physt) {
				pfrinfo->rssi = 0;
			} else {
...
#if defined(__DAVO__)
/**
 * If destination is permitted to access, returns 0. otherwise returns none zero.
 */
static inline int test_guest_access(struct rtl8192cd_priv *priv, u8 *to_dst)
{
    struct net_device *dev;
	struct net_bridge_port *br_port=NULL;

	br_port = GET_BR_PORT(priv->dev);
    switch (priv->pmib->dot11OperationEntry.guest_access) {
    case 1: /*NAT*/
        return compare_ether_addr(to_dst, br_port->br->dev->dev_addr);

    case 2: /*BRIDGE*/
        dev = br_fdb_locate(br_port->br, to_dst);
        if (dev == NULL || *(unsigned int *)dev->name != 0x776c616e/*wlan*/)
            return 0;
    default:
        return -1;
    }
