...
#define MAX_INTF 6 /* wlan0, wlan0-vax, wlan0-vxx */
#else
...
/*****************************************************************************
 *
 * To restrict bandwidth for 802.11b STA when compitions exist wit 11g/n
 * It's strict requirements of SKBB 2009.
 *
 * ***************************************************************************
*/
#define DAVO_8190N_ENABLE_RESTRICT_11B_BW       1
...
/* We should adjust tx bandwidth gab of ethernet header size of SKB and
 *   802.11 header size counted at wifi drivers.
 */
#define BW11B_EXTRA_BYTES_PER_HEADER  (18)        /* in case of using tkip  */

struct davo_bw11b_ctrl {
    int     num_11b_sta;
    int     num_11gn_sta;
    UINT32  now;
    UINT32  trig_bw_all;
    ...
    UINT32  next_update_time;
    UINT32  curbw_all;
    UINT32  last_update_time;
    UINT32  last_tx_packets;
    UINT32  last_tx_bytes;
};

#if DAVO_8190N_ENABLE_RESTRICT_11B_BW
  #ifndef _8190N_DAVO_MISC_C_
    extern int g_davo_restrict_11b_bw;
  #endif
  #define davo_11b_bw_is_restrict_enabled() (g_davo_restrict_11b_bw != 0)
  #define davo_11b_bw_is_11b_sta(pstat)     (((pstat)!=NULL) && (pstat)->f_11b_sta != 0)
  int davo_11b_bw_check_11b_bw(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *skb);
  void davo_11b_bw_restrict_sta_update(struct rtl8192cd_priv *priv);
#else
  #define davo_11b_bw_is_restrict_enabled()             (0)
  #define davo_11b_bw_is_11b_sta(pstat)                  (0)
  #define davo_11b_bw_check_11b_bw(priv, pstat, pktlen)  (0)
  #define davo_11b_bw_restrict_sta_update(priv)
#endif
...
#endif
	int restrict_11b_bw;
	dv_ratelimit_ctrl_t bw11b;
	int my_acl_count[2];
/* MSSID */
	struct dv_priv_mssid_t mssid[MAX_INTF];
#ifdef DV_RXDESC_CHECK_WATCHDOG /* APACRTL-182, WR for no rx auth from sta */
	struct {
		unsigned int rx_count;		/* increased by rx_isr */
		unsigned int chk_count;		/* compared with rx_count */
		unsigned int guard_time;	/* check guard time */
		unsigned int recover_cnt;	/* recover time */
	} rxbd_check;
#endif
...
void dv_set_webrd_host(char *url);
...
#ifdef DV_RXDESC_CHECK_WATCHDOG	/* APACRTL-182, WR for no rx auth from sta */
#define dv_rxdesc_check_update(priv) \
	do { \
		struct dv_priv_t *dv_priv = (struct dv_priv_t *)((priv)->dv_priv); \
		if (dv_priv != NULL) \
			dv_priv->rxbd_check.rx_count++; \
	} while (0)
