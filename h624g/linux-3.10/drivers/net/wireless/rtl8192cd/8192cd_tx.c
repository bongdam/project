...
unsigned int get_tx_rate(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
    if (pstat->sta_in_firmware == 1) {
		...
		if (dv_wl_force_tx_rate(priv)) {
			return (dv_wl_get_tx_rate(priv, pstat->current_tx_rate));
		}
#endif
...
# if defined(__DAVO__) && defined(DAVO_ENABLE_WME_OVERRIDE)
	struct dv_priv_t *dv_priv = (struct dv_priv_t *)priv->dv_priv;

	if (QOS_ENABLE && davo_wme_override_enabled(dv_priv))
		parsing = 1;
#else // defined(__DAVO__) && defined(DAVO_ENABLE_WME_OVERRIDE)
#ifdef WIFI_WMM
    if (QOS_ENABLE) {
        parsing = 1;
#ifdef CONFIG_PCI_HCI
	...
	if (parsing) {
...
#endif
		{
			...
			if (davo_wme_802_1p_enabled(dv_priv)) {
				if (((struct ethhdr *)p)->h_proto == __constant_htons(ETH_P_8021Q))
					pri = davo_wme_get_802_1p_priority(dv_priv, p[14] >> 5);
				else if ((skb->cb[0] > 0) && (skb->cb[0] < 8))
					pri = skb->cb[0];
				else if (skb->vlan_tci)
					pri = davo_wme_get_802_1p_priority(dv_priv, ((skb->vlan_tci - 1) >> 13) & 7);
				else
					pri = davo_wme_get_802_1p_priority(dv_priv, skb->priority & 7);
			} else if (davo_wme_dscp_enabled(dv_priv)) {
				for (i = 2; i > 0; i--) {
					switch (((struct ethhdr *)p)->h_proto) {
					case __constant_htons(ETH_P_8021Q):
						p += VLAN_HLEN;
						/* sanity check of being piggy-backed another vlan header.
						 * 802.1ad specifies a TPID of 0x88a8 for service-provider outer S-TAG
						 */
						continue;
					case __constant_htons(ETH_P_IP):
						pri = davo_wme_get_dscp_priority(dv_priv, p[15] >> 2);
						break;
					case __constant_htons(ETH_P_IPV6):
						pri = davo_wme_get_dscp6_priority(dv_priv, ipv6_get_dsfield((void *)&p[14]) >> 2);
						break;
					default:
						if ((skb->cb[0] > 0) && (skb->cb[0] < 8))
							pri = skb->cb[0];
						break;
					}
					break;
			...
			if ((protocol[0] == 0x08) && (protocol[1] == 0x00))
			{
				...
				pri = 0;
#endif				//defined(__DAVO__) && defined(DAVO_ENABLE_WME_OVERRIDE)
		}
...
int rtl8192cd_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	struct webrd_private wprv = { .priv = priv };
#endif
	...
	if (unlikely(skb->dev == NULL))
		skb->dev = dev;
	/* APNRTL-288 Forwarding between interfaces belonging to bridge */
	if (SNOOP_MARK_ISSET(skb->mark, SNOOP_DROP_ALWAYS) ||
	    SNOOP_MARK_ISSET(skb->mark, SNOOP_DROP_WLAN)) {
		dev_kfree_skb_any(skb);
		RESTORE_INT(x);
		return 0;
	}

	list_for_each_entry(h, &pf_tx_chains, list) {
		if (h->hook(h, skb, (void *)&wprv) == PF_DISCARD) {
			dev_kfree_skb_any(skb);
			RESTORE_INT(x);
			return 0;
		}
...
//      bool bCts2SelfEnable;
...
#ifdef WIFI_11N_2040_COEXIST
			    && !(COEXIST_ENABLE
				 && (((OPMODE & WIFI_AP_STATE) && (priv->bg_ap_timeout || orForce20_Switch20Map(priv)))
#ifdef CLIENT_MODE
				|| ((OPMODE & WIFI_STATION_STATE) && priv->coexist_connection &&
				    (txcfg->pstat->ht_ie_len) && !(txcfg->pstat->ht_ie_buf.info0 & _HTIE_STA_CH_WDTH_))
#endif
	...
	if ((priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) && (pdesc_data->RTSRate) < 4) {
		pdesc_data->RTSRate = 4;
	}
#else
	if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G)
	...
	if (txcfg->need_ack) {
		...
		if (txcfg->q_num == MANAGE_QUE_NUM) {
			...
			if (is_MCS_rate(txcfg->pstat->current_tx_rate) && (txcfg->pstat->IOTPeer == HT_IOT_PEER_INTEL)
			    && (txcfg->pstat->retry_inc)
			    && !(txcfg->pstat->leave) && priv->pshare->intel_rty_lmt) {
				pdesc_data->rtyLmtEn = TRUE;
				pdesc_data->dataRtyLmt = priv->pshare->intel_rty_lmt;
			} else if ((txcfg->pstat->IOTPeer == HT_IOT_PEER_BROADCOM) && (txcfg->pstat->retry_inc)
				   && !(txcfg->pstat->leave)) {
				pdesc_data->rtyLmtEn = TRUE;
				pdesc_data->dataRtyLmt = 0x20;
			}
#ifdef __DV_WLCMD__		/* APACRTL-160 */
			else if (dv_wl_tx_retry_limit(priv)) {
				pdesc_data->rtyLmtEn = TRUE;
				pdesc_data->dataRtyLmt = dv_wl_tx_retry_limit(priv);
				DV_WL_INC_COUNT();
			}
#endif
		}
		...
		if (txcfg->ndpa) {
			...
			if (IsCtrlNDPA(pFrame) || IsMgntNDPA(pFrame)) {
				...
				{
					if (!IS_TEST_CHIP(priv)) {
						if ((txcfg->pstat->WirelessMode & WIRELESS_MODE_AC_5G) &&
						    (txcfg->pstat->IOTPeer != HT_IOT_PEER_BROADCOM))
							pdesc_data->ndpa = 2;
						else
							pdesc_data->ndpa = 1;
...
#else
#if defined(__DAVO__)
			tx_sum_up(priv, pstat, txcfg->fr_len+txcfg->hdr_len+txcfg->iv+txcfg->llc+txcfg->mic+txcfg->icv, txcfg->phdr); // add frame type
#else
...
#else
#if defined(__DAVO__)
	tx_sum_up(priv, txcfg->pstat, txcfg->fr_len+txcfg->hdr_len+txcfg->iv+txcfg->llc+txcfg->mic+txcfg->icv, txcfg->phdr); // add frame type
#else
