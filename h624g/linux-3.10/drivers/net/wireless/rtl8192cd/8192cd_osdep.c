...
extern int gpio_sys_reset(void);
#endif
	...
	if (!(priv->pmib->dot11BssType.net_work_type & WIRELESS_11N)) {
...
#endif
		{
			if (!((priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_2G) &&
					(priv->pmib->dot11RFEntry.used_autochan == 1) && is_auto_bonding(CSEL_2G)))
...
#endif
#if defined(__DAVO__)
    /* regardless auto channel site_survey do it */
    if ((OPMODE & WIFI_AP_STATE)) {
#else
...
#if defined(__DAVO__)   /* jjj 20091025 */
	priv->rx_only_data_bytes = 0;
	priv->rx_only_data_bytes_high = 0;
	priv->tx_only_data_bytes = 0;
	priv->tx_only_data_bytes_high = 0;
	priv->rx_only_data_packets = 0;
	priv->tx_only_data_packets = 0;
#endif
...
#ifdef __DAVO__
	dev->priv_flags |= (IFF_DOMAIN_WLAN|IFF_PASSTHRU);
#endif
#ifdef __KERNEL__
	if (register_netdev(dev)) {
