...
#if defined(__DAVO__)
static void DOT11_Process_Acc_ClearStats(struct net_device *dev, struct rtl8192cd_priv *priv, struct iw_point __user *data);
#endif
...
#endif
#ifndef WITHOUT_ENQUEUE
					put_deauth_info(DOT11_EVENT_DISASSOCIATION_IND, 0, _STATS_OTHER_,
							pstat_del->hwaddr, pstat_del, &Disassociation_Ind);
...
#if defined(RADIUS_ACCOUNTING) || defined(__DAVO__)
...
#if defined(__DAVO__)
	unsigned char haddr[MACADDRLEN];

	if( pStats == NULL ) {
		DEBUG_ERR("%s: NULL POINTER!\n", (char *)__FUNCTION__);
		return;
	}
	put_user(DOT11_EVENT_ACC_QUERY_STATS, &pStats->EventId);
	put_user(0, &pStats->IsMoreEvent);
	copy_from_user(haddr, pStats->MACAddr, MACADDRLEN);
	pstat = get_stainfo(priv, haddr);
	if (pstat) {
		put_user(pstat->tx_pkts, &pStats->tx_packets);
		put_user(pstat->rx_pkts, &pStats->rx_packets);
		put_user(pstat->tx_bytes, &pStats->tx_bytes);
		put_user(pstat->rx_bytes, &pStats->rx_bytes);
		put_user(pstat->tx_bytes_h, &pStats->tx_bytes_h);	// youngho DAVO
		put_user(pstat->rx_bytes_h, &pStats->rx_bytes_h);	// youngho DAVO
		put_user(pstat->tx_only_data_packets, &pStats->tx_only_data_packets);
		put_user(pstat->rx_only_data_packets, &pStats->rx_only_data_packets);
		put_user(pstat->tx_only_data_bytes, &pStats->tx_only_data_bytes);
		put_user(pstat->tx_only_data_bytes_high, &pStats->tx_only_data_bytes_high);
		put_user(pstat->rx_only_data_bytes, &pStats->rx_only_data_bytes);
		put_user(pstat->rx_only_data_bytes_high, &pStats->rx_only_data_bytes_high);
		put_user(TRUE, &pStats->IsSuccess);
		//DEBUG_INFO("%s: Get wlan_sta_tbl[%d].stats!\n", (char *)__FUNCTION__, sta_tbl_idx);

	} else {
		put_user(0, &pStats->tx_packets);
		put_user(0, &pStats->rx_packets);
		put_user(0, &pStats->tx_bytes);
		put_user(0, &pStats->rx_bytes);
		put_user(0, &pStats->tx_bytes_h);	// youngho DAVO
		put_user(0, &pStats->rx_bytes_h);	// youngho DAVO
		put_user(0, &pStats->tx_only_data_packets);
		put_user(0, &pStats->rx_only_data_packets);
		put_user(0, &pStats->tx_only_data_bytes);
		put_user(0, &pStats->tx_only_data_bytes_high);
		put_user(0, &pStats->rx_only_data_bytes);
		put_user(0, &pStats->rx_only_data_bytes_high);
		put_user(FALSE, &pStats->IsSuccess);
...
#if defined(__DAVO__)
		case DOT11_EVENT_ACC_CLEAR_STATS:	// youngho DAVO
			DOT11_Process_Acc_ClearStats(dev, priv, data);		// youngho DAVO
			break;
#endif
...
#endif
#if defined(__DAVO__)
//@NOTE: mark webrd in sta_info
		case DOT11_EVENT_SET_PORT_WEBRD:
			{
				struct stat_info	*pstat;
				struct wifi_mib		*pmib = priv->pmib;
				DOT11_SET_PORT		*Set_Port = (DOT11_SET_PORT *)data->pointer;

				DEBUG_INFO("Set webrd %pM Status %X\n",	Set_Port->MACAddr, Set_Port->PortStatus);
				// if driver is not opened, return immediately, david
				if (!netif_running(priv->dev))
					break;
				...
				pstat->webrd = Set_Port->PortStatus;
			}
			break;
#endif
		default:
...
static void DOT11_Process_Acc_ClearStats(struct net_device *dev, struct rtl8192cd_priv *priv, struct iw_point __user *data)
{
	...
	put_user(DOT11_EVENT_ACC_CLEAR_STATS, &pStats->EventId);
	put_user(0, &pStats->IsMoreEvent);
	copy_from_user(haddr, pStats->MACAddr, MACADDRLEN);
	pstat = get_stainfo(priv, haddr);
	if (pstat) {
