...
#ifdef __DAVO__
extern unsigned int mirror_to_bits, mirror_from_bits;
#endif
...
#if defined(__DAVO__)
			mirror_to_bits = port_mask;
			mirror_from_bits = tx_mask;
#endif
...
#ifdef __DAVO__
int rtl_l2_search(const u8 *addr, void *asicarg)
{
	rtl865x_tblAsicDrv_l2Param_t l2;
	rtl865x_tblAsicDrv_l2Param_t *p = (rtl865x_tblAsicDrv_l2Param_t *)asicarg;
	int i, row;
	...
	for (i = 0; i < RTL8651_L2TBL_COLUMN; i++) {
		if (rtl8651_getAsicL2Table(row, i, p) == SUCCESS &&
		    !memcmp(addr, p->macAddr.octet, 6))
		    	return 0;
	}

	return -1;
}
EXPORT_SYMBOL(rtl_l2_search);

static long l2_proc_ioctl(struct file *file, unsigned int command, unsigned long arg)
{
	rtl865x_tblAsicDrv_l2Param_t l2;
	u8 addr[6];
	int resp[2];
...
static int
rtl_ll_lookup(const u8 *addr, rtl865x_tblAsicDrv_l2Param_t *l2, struct rtl_matrix *mat)
{
	int i, row;
	...
	for (i = 0; i < RTL8651_L2TBL_COLUMN; i++) {
		if (rtl8651_getAsicL2Table(row, i, l2) == SUCCESS &&
		    !memcmp(addr, l2->macAddr.octet, 6)) {
		    	if (mat) {
				mat->r = row;
				mat->c = i;
			}
...
{
	struct rtl_matrix mat;
	rtl865x_tblAsicDrv_l2Param_t l2;

	if (rtl_ll_lookup(addr, &l2, &mat))
		return -1;
	return rtl8651_delAsicL2Table(mat.r, mat.c);
}
EXPORT_SYMBOL(rtl_sync_l2_wlsta);

/* APACRTL-87 */
int rtl865x_isEthArp_r(ipaddr_t ip)
{
	rtl865x_arpMapping_entry_t arp;
	rtl865x_tblAsicDrv_l2Param_t l2;

	if (rtl865x_getArpMapping(ip, &arp) != SUCCESS)
		return FALSE;
	if (rtl_ll_lookup(arp.mac.octet, &l2, NULL))
		return FALSE;
	return (l2.memberPortMask & 0x1f) ? TRUE : FALSE;
}
EXPORT_SYMBOL(rtl865x_isEthArp_r);
#endif	/* __DAVO__ */
...
static int32 rtl865x_proc_hw_mcast_read(struct seq_file *s, void *v)
{
	...
	for(entry=0; entry<RTL8651_MULTICASTTBL_SIZE; entry++)
	{
			...
			{
				seq_printf(s, "\t[%d]  (OK)dip:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
					entry,
				...
				seq_printf(s, "\t          sip:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
				...
				seq_printf(s, "\t          spa:%d, age:%d, cpu:%d\n", asic6.port, asic6.age, asic6.cpu);
			}
	}
...
#if defined(CONFIG_RTL_LAYERED_DRIVER_L4)
static int32 napt_show(struct seq_file *s, void *v)
{
	rtl865x_tblAsicDrv_naptTcpUdpParam_t parm;
	int i, nr_of_entry = 0;
	char buffer[32];

	seq_printf(s, "                                                     Tcp SelE SelE-     Pri-\n");
	seq_printf(s, "Indx Local address         V D C K S T  Age   Offset Flg Idx  IPIdx Pri Valid\n");

	for (i = 0; i < RTL8651_TCPUDPTBL_SIZE; i++) {
		if (rtl8651_getAsicNaptTcpUdpTable(i, &parm) == FAILED)
			continue;

		if (parm.isValid == 1 || parm.isDedicated == 1) {
			sprintf(buffer, "%u.%u.%u.%u:%u", NIPQUAD(parm.insideLocalIpAddr), parm.insideLocalPort);
			seq_printf(s, "%4d %-21s %d %d %d %d %d %d %-6d %-6d %-3d %-4d %-5d %-3d %d\n",
				   i, buffer,
				   parm.isValid,
				   parm.isDedicated,
				   parm.isCollision,
				   ...
				   parm.isTcp,
				   parm.ageSec,
				   parm.offset << 10, parm.tcpFlag, parm.selEIdx, parm.selExtIPIdx, parm.priority,
				   parm.priValid);
	...
	seq_printf(s, "\tC: Collision\n");
	seq_printf(s, "\tK: Collision2\n");
	seq_printf(s, "\tV: Valid\n");
	seq_printf(s, "\tD: Dedicated\n");
	seq_printf(s, "\tS: Static\n");
	seq_printf(s, "\tT: TCP\n");
...
#ifdef DAVO_WDT_SPI_3B4B_PATCH
extern unsigned long dv_cnt_en4b;
#endif
...
#else
extern struct file_operations acl_proc_fops;
#endif
