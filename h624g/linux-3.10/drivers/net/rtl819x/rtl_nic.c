...
#include "davolink/rtl_davo_vlan.h"
...
#ifdef CONFIG_SMP
#ifdef __DAVO__
struct stats_ether rx_stats_per_port[5];
struct stats_ether tx_stats_per_dev[2];
/* jihyun@davo 150614 jcode#1 */
extern int igmp_query_received;
extern int igmp_query_only_to_lan;
...
static struct task_struct *p2tsk;
static int enqueue_igmp_worker_queue(struct sk_buff *skb);
static int igmp_worker_thread(void *dummy);
#endif
int lock_owner = -1;
int lock_owner_rx = -1;
int lock_owner_tx = -1;
...
#if DAVO_VLAN_ENABLED
# if defined(CONFIG_RTL_MULTI_LAN_DEV) || defined(CONFIG_RTK_VLAN_SUPPORT)
# error DAVO_VLAN_ENABLED can not be used with CONFIG_RTK_VLAN_SUPPORT!
# endif
#endif
...
#if defined(__DAVO__)
unsigned int mirror_to_bits, mirror_from_bits;
#endif
...
#   ifdef CONFIG_RTK_VLAN_NEW_FEATURE
	{	RTL_DRV_LAN_P7_NETIF_NAME,	0,	 IF_ETHER,	RTL_LANVLANID,	RTL_LAN_FID,	0, 	0, 	1500,	{ { 0x00, 0x12, 0x34, 0x56, 0x78, 0x97 } }, 0	},
#   endif
#  elif defined(DAVO_VLAN_ENABLED)
    {   RTL_DRV_LAN_P1_NETIF_NAME,	0,	 IF_NONE, 0, 0, 0, 0, 1500, {{0x00, 0x12, 0x34, 0x56, 0x78, 0x92}}, 0},
    {   RTL_DRV_LAN_P2_NETIF_NAME,	0,	 IF_NONE, 0, 0, 0, 0, 1500, {{0x00, 0x12, 0x34, 0x56, 0x78, 0x93}}, 0},
    {   RTL_DRV_LAN_P3_NETIF_NAME,	0,	 IF_NONE, 0, 0, 0, 0, 1500, {{0x00, 0x12, 0x34, 0x56, 0x78, 0x94}}, 0},
#  endif
# endif
#else	/*CONFIG_BRIDGE*/
# if defined (CONFIG_RTL_MULTI_LAN_DEV)
 ...
 void  re865x_accumulate_port_stats(uint32 portnum, struct net_device_stats *net_stats)
{
	...
	/* rx_pkt = rx_unicast +rx_multicast + rx_broadcast */
#ifdef __DAVO__
	net_stats->rx_packets += ((portnum < 5) ?
			get_stats22(portnum, 0, 'u') : rtl8651_returnAsicCounter( OFFSET_IFINUCASTPKTS_P0 + addrOffset_fromP0 ));
	net_stats->multicast += ((portnum < 5) ?
			get_stats22(portnum, 0, 'm') : rtl8651_returnAsicCounter( OFFSET_ETHERSTATSMULTICASTPKTS_P0 + addrOffset_fromP0 ));
	net_stats->rx_packets += net_stats->multicast;
	net_stats->rx_packets += ((portnum < 5) ?
			get_stats22(portnum, 0, 'b') : rtl8651_returnAsicCounter( OFFSET_ETHERSTATSBROADCASTPKTS_P0 + addrOffset_fromP0 ));
	/* tx_pkt = tx_unicast +tx_multicast + tx_broadcast*/
	net_stats->tx_packets += ((portnum < 5) ?
			get_stats22(portnum, 1, 'u') : rtl8651_returnAsicCounter( OFFSET_IFOUTUCASTPKTS_P0 + addrOffset_fromP0 ));
	net_stats->tx_packets += ((portnum < 5) ?
			get_stats22(portnum, 1, 'm') : rtl8651_returnAsicCounter( OFFSET_IFOUTMULTICASTPKTS_P0 + addrOffset_fromP0 ));
	net_stats->tx_packets += ((portnum < 5) ?
			get_stats22(portnum, 1, 'b') : rtl8651_returnAsicCounter( OFFSET_IFOUTBROADCASTPKTS_P0 + addrOffset_fromP0 ));
#else
...
#if !defined(CONFIG_RTL_8196C) && !defined(__DAVO__)
...
#ifdef __DAVO__
			({ SNOOP_MARK_SET(skb->mark, SNOOP_DROP_ETH); 1; }) &&
#endif
			(skb->data[2]!=0xff))
		{
...
#if defined(__DAVO__)
				if (nicTx.portlist & mirror_from_bits)
					nicTx.portlist |= mirror_to_bits;
#endif
...
#ifdef __DAVO__
static void igmp_shrink_mrt(struct iphdr *ih, struct igmphdr *igmph, int mrt)
{
	if (igmph->code > mrt) {
		igmph->code = mrt;
		igmph->csum = 0;
		igmph->csum = ip_compute_csum((void *)igmph,
			(int)(ih->tot_len - (ih->ihl << 2)));
	}
}

static int igmp_packet_spoof(struct iphdr *ih, int pid)
{
	struct igmphdr *igmph = (struct igmphdr *)((__u32 *)ih + ih->ihl);

	if (ih->saddr == INADDR_ANY && igmph->type == IGMP_HOST_MEMBERSHIP_QUERY)
		return 0;	/* stop further processing. toss into igmp subsystem of linux */
	else if ((1 << pid) & RTL_WANPORT_MASK) {
		/* It should be happened to be bridge mode only */
		if (igmph->type == IGMP_HOST_MEMBERSHIP_QUERY) {
			igmp_query_received = 1;
			/* spoof max-response-code being greater than 1sec */
			igmp_shrink_mrt(ih, igmph, 10);
		}
	}
	return -1;
}
...
#ifdef __DAVO__	/* APACRTL-536 */
#define REAL_LANPORT_MASK (RTL_LANPORT_MASK & ((1 << RTL8651_MAC_NUMBER) - 1))
#endif

static uint32 igmp_flood_decision(struct iphdr *ih, int pid)
{
	if (rtl865x_curOpMode != BRIDGE_MODE)
		return 0;

	switch (((struct igmphdr *)((__u32 *)ih + ih->ihl))->type) {
	case IGMP_HOST_MEMBERSHIP_QUERY:
		return ((1 << pid) & RTL_WANPORT_MASK) ? REAL_LANPORT_MASK : 0;

	case IGMP_HOST_MEMBERSHIP_REPORT:
	case IGMPV2_HOST_MEMBERSHIP_REPORT:
	case IGMP_HOST_LEAVE_MESSAGE:
		return ((1 << pid) & RTL_WANPORT_MASK) ? 0 : RTL_WANPORT_MASK;

	default:
		return ((1 << pid) & RTL_WANPORT_MASK) ? REAL_LANPORT_MASK : RTL_WANPORT_MASK;
	}
}
#endif /* __DAVO__ */
...
int rtl_MulticastRxCheck(struct sk_buff *skb, rtl_nicRx_info * info, int mCastFlag)
{
	...
	vportmask = rtl865x_getVlanPortMask(vid);
	vlanRelayPortMask = vportmask & (~(1 << pid)) & ((1 << RTL8651_MAC_NUMBER) - 1);
...
#  ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
				if (vlan_bridge_tag && !strcmp(cp_this->dev->name, RTL_PS_ETH_NAME_ETH2)) {
					if (igmpsnoopenabled && (nicIgmpModuleIndex_2 != 0xFFFFFFFF)) {
						...
						ret =
						    rtl_getMulticastDataFwdInfo(nicIgmpModuleIndex_2, &multicastDataInfo,
										&nicMCastFwdInfo);
						vlanRelayPortMask =
						    vportmask & (~(1 << pid)) & nicMCastFwdInfo.fwdPortMask & ((1 << RTL8651_MAC_NUMBER) - 1);
						if (ret == SUCCESS) {
						} else {
							ret = rtl_getMulticastDataFwdInfo(brIgmpModuleIndex_2, &multicastDataInfo,
											  &br0MCastFwdInfo);
							if (ret == SUCCESS) {
...
#ifdef __DAVO__	/* APACRTL-536 */
					if ((REAL_LANPORT_MASK & (1 << pid)) && ip_mc_reserved(iph->daddr)) {
						//vlanRelayPortMask = 0;
						dev_kfree_skb_any(skb);
						return -100;
					} else {
...
#  else
					...
					multicastDataInfo.groupAddr[0] = (uint32) (iph->daddr);
#  endif
					ret = rtl_getMulticastDataFwdInfo(nicIgmpModuleIndex, &multicastDataInfo,
									  &nicMCastFwdInfo);
#  ifndef CONFIG_RTL_VLAN_8021Q
					vlanRelayPortMask =
					    vportmask & (~(1 << pid)) & nicMCastFwdInfo.fwdPortMask & cp_this->portmask & ((1 << RTL8651_MAC_NUMBER) - 1);
#  else
					vlanRelayPortMask =
					    vportmask & (~(1 << pid)) & nicMCastFwdInfo.fwdPortMask & ((1 << RTL8651_MAC_NUMBER) - 1);
#  endif
#  if defined(__DAVO__)
					/* jihyun@davo 150614 jcode#1 */
					vlanRelayPortMask &= dp->portmask;
#  endif
					if (ret == SUCCESS) {

					} else {
						ret =
						    rtl_getMulticastDataFwdInfo(brIgmpModuleIndex, &multicastDataInfo,
										&br0MCastFwdInfo);
						if (ret == SUCCESS) {
							/*there is wireless client,can not flooding in vlan */
							vlanRelayPortMask = 0;
...
#  ifdef CONFIG_RTL_VLAN_8021Q
				if (linux_vlan_enable && linux_vlan_hw_support_enable) {
					if (is_nat_wan_vlan(vid) || (vid != info->priv->id)) {
						rtl865x_nic_MulticastHardwareAccelerate(nicMCastFwdInfo.fwdPortMask, pid, vid,
...
# if defined(__DAVO__)
				if (igmp_packet_spoof(iph, pid) == 0)
					return RTL_ACCEPT;
# endif
# ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
				if (vlan_bridge_tag && !strcmp(cp_this->dev->name, RTL_PS_ETH_NAME_ETH2)) {
					if (igmpsnoopenabled && (nicIgmpModuleIndex_2 != 0xFFFFFFFF)) {
						...
						//just flooding
						vlanRelayPortMask =
						    vportmask & (~(1 << pid)) & ((1 << RTL8651_MAC_NUMBER) - 1);
					}
				} else {
# endif
					if (igmpsnoopenabled && (nicIgmpModuleIndex != 0xFFFFFFFF)) {
#ifdef __DAVO__
						if (!enqueue_igmp_worker_queue(skb))
							return -100;
#endif	/* __DAVO__ */
						/*igmp packet */
# if defined (CONFIG_NETFILTER)
...
# endif
						...
						vlanRelayPortMask = vportmask & vlanRelayPortMask & ((1 << RTL8651_MAC_NUMBER) - 1);
					}
# ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
				}
# endif
# ifdef __DAVO__
				vlanRelayPortMask = vportmask & igmp_flood_decision(iph, pid);
# endif
				re865x_relayTrappedMCast(skb, vid, vlanRelayPortMask, TRUE);
			} else {
# if defined (CONFIG_RTL_HARDWARE_MULTICAST)
...
# endif
					if (ret == SUCCESS) {
						...
						ret = rtl_getMulticastDataFwdInfo(brIgmpModuleIndex, &multicastDataInfo,
										  &br0MCastFwdInfo);
						if (ret == SUCCESS) {
							/*there is wireless client,can not flooding in vlan */
							vlanRelayPortMask = 0;
...
#ifdef __DAVO__
				if (!(oldStatus & IP6_PASSTHRU_MASK) || mCastFlag != IPV6_MCAST_PACKET)
#endif
				re865x_relayTrappedMCast(skb, vid, vlanRelayPortMask, TRUE);
				...
				if (igmpsnoopenabled && (nicIgmpModuleIndex != 0xFFFFFFFF)) {
					rtl_igmpMldProcess(nicIgmpModuleIndex, skb->data, pid, &vlanRelayPortMask);
					vlanRelayPortMask =
					    vportmask & vlanRelayPortMask & ((1 << RTL8651_MAC_NUMBER) - 1);
				}
#ifdef __DAVO__
				if (!(oldStatus & IP6_PASSTHRU_MASK) || mCastFlag != IPV6_MCAST_PACKET)
#endif
...
# else
#  if defined(DAVO_VLAN_ENABLED)
    return (cp->id == wan_vlan_id);
#  endif
#  ifndef CONFIG_RTL_VLAN_8021Q
...
static inline int32 rtl_isPassthruFrame(uint8 *data)
{
	struct vlan_ethhdr *veh = (struct vlan_ethhdr *)data;

	if (oldStatus & IP6_PASSTHRU_MASK) {
		switch (veh->h_vlan_proto) {
		case __constant_htons(ETH_P_8021Q):
			if (veh->h_vlan_encapsulated_proto !=
...
static inline void rtl_processRxFrame(rtl_nicRx_info *info)
{
	...
	//skb->dev=cp_this->dev;
#ifdef __DAVO__
	if (pid <= ARRAY_SIZE(rx_stats_per_port)) {
		BR_INPUT_SKB_CB(skb)->source_port = 1 << (pid & 7);
		rx_stats_per_port[pid].rx_bytes += (info->len - ETH_HLEN);
		rx_stats_per_port[pid].rx_packets += 1;
	}
#endif
#ifdef CONFIG_RTK_VOIP_ETHERNET_DSP
	//merged from r8627 may conflict later
    ...
    defined(CONFIG_RTL_FAST_FILTER) ||\
    defined(CONFIG_RTL_QOS_PATCH) ||\
    defined(CONFIG_RTK_VOIP_QOS) ||\
    defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) ||\
    defined(CONFIG_RTL_MAC_FILTER_CARE_INPORT) ||\
    defined(CONFIG_RTL_HW_QOS_SUPPORT_WLAN) ||\
    defined(__DAVO__)
...
Reserve_tag:
...
#ifdef __DAVO__
	if (snoop_dhcp)
		snoop_status = snoop_dhcp_packet(skb);
#endif
	#if defined(CONFIG_RTL_ETH_802DOT1X_SUPPORT)
	...
	{
#ifdef __DAVO__ /* APNRTL-288 NEVER take bridge shorcut  */
		if (snoop_status == 0)
			goto RxToPs;
#endif
...

	{
		...
		if (rtl_MulticastRxCheck(skb, info,mCastFlag) == -100)
			return;
		#endif	/*end of CONFIG_RTL865X_IGMP_SNOOPING*/
		/*	multicast process end	*/
	}
	#endif
#if defined(CONFIG_RTL_STP) || defined(__DAVO__)
RxToPs:
#endif
...
static unsigned int reset_hw_mib_counter(struct net_device *dev)
{
	int i, port;
...
static void one_sec_timer(unsigned long task_priv)
{
	unsigned long		flags=0;
	struct dev_priv	*cp;
#if defined (CONFIG_RTL_REINIT_SWITCH_CORE) || defined(__DAVO__)
	int i;
#endif
...
#ifdef CONFIG_RTL_HARDWARE_NAT
	portmask = reset_hw_mib_counter(dev);
#ifdef __DAVO__
	for (i = 0; i < 5; i++)
		if ((1 << i) & portmask)
			reset_stats22(i);
...
/* jihyun@davo 150614 jcode#1 */
				...
				if (igmp_query_only_to_lan) {
					struct igmphdr *igmph = (struct igmphdr *)((u32 *)iph + iph->ihl);
					if (igmph->type == IGMP_HOST_MEMBERSHIP_QUERY)
...
static inline int rtl_process_passthru_tx(rtl_nicTx_info *txInfo)
{
	...
	if (!(skb->dev->priv_flags & IFF_PASSTHRU))
		return SUCCESS;

	cp = (struct dev_priv *)netdev_priv(skb->dev);
	if (likely(!rtl_isPassthruFrame(skb->data))) {
		if ((*(u16 *)&skb->data[0]) == 0x3333 &&
		    ((struct dev_priv *)netdev_priv(cp->dev))->portmask == RTL_WANPORT_MASK &&
		    ((*(u32 *)&skb->data[2]) == __constant_htonl(0x0000000c) ||
		     (*(u32 *)&skb->data[2]) == __constant_htonl(0x000000fb) ||
		     (*(u32 *)&skb->data[2]) == __constant_htonl(0x00010003)))
			goto dispose;
		/*
		 * Replace with a real device for transmitting
		 */
		skb->dev = cp->dev;
		return SUCCESS;
	}
...
{
#ifdef __DAVO__
	int i = rtl_isWanDev(cp);
	tx_stats_per_dev[i].rx_bytes += (len - ETH_HLEN);
	tx_stats_per_dev[i].rx_packets += 1;
#endif
#if !defined(CONFIG_RTL_NIC_HWSTATS)
...
static inline int rtl_fill_txInfo(rtl_nicTx_info *txInfo)
{
	...
	struct sk_buff *pskb = NULL;
    #endif
#ifdef __DAVO__	/* APNRTL-288 */
	unsigned int snoop_port = SNOOP_MARK_GET(skb->mark);
	...
	txInfo->tagport = 0 ;
	#endif
#ifdef __DAVO__	/* APNRTL-288 Take the intended port(s), if specified */
	if (snoop_port) {
		if (snoop_port & ((1 << SNOOP_DROP_ALWAYS) | (1 << SNOOP_DROP_ETH)))
			txInfo->portlist = 0;
		else
			rtl_direct_txInfo((snoop_port & SNOOP_MASK_ETH), txInfo);
	} else
#endif
	if((skb->data[0]&0x01)==0)
	{
...
#if defined(__DAVO__)
	if (txInfo->portlist & mirror_from_bits)
		txInfo->portlist |= mirror_to_bits;
#endif
...
/*2007-12-19*/
	...
	rtlglue_printf("Probing RTL819X NIC-kenel stack size order[%d]\n", THREAD_SIZE_ORDER);
    	REG32(CPUIIMR) = 0x00;
...
#ifdef __DAVO__
		for (i = 0; i < 5; i++)
			rtl8651_setAsicEthernetPHYPowerDown(i, TRUE);
#endif
#ifdef CONFIG_RTL_LAYERED_ASIC_DRIVER_L3
...
#if DAVO_VLAN_ENABLED
	vport_read_conf();
	/* MUST precede rtl865x_config */
	vport_organize_vconf_table(vlanconfig, totalVlans);
#endif
...
#endif
#ifdef __DAVO__
	if (IS_ERR_OR_NULL(p2tsk))
		p2tsk = kthread_run(igmp_worker_thread, NULL, "p2worker");
#endif
...
#endif
#if DAVO_VLAN_ENABLED
            if (vlanconfig[i].memPort != vlanconfig[i].untagSet) {
                rtl865x_setVlanPortTag(vlanconfig[i].vid,
                        (vlanconfig[i].memPort & ~vlanconfig[i].untagSet), TRUE);
            }
#endif
...
static int re865x_reInitIgmpSetting(int mode)
{
	...
	for(i=0; i<totalVlans; i++)
	{
		...
		{
			rtl_setIgmpSnoopingModuleStaticRouterPortMask(nicIgmpModuleIndex,
#ifdef __DAVO__
				RTL_WANPORT_MASK
#else
				0x01
...
#  if DAVO_VLAN_ENABLED
    	...
    	vlanconfig[1].fid = RTL_WAN_FID;

		for (i = 2; vlanconfig[i].ifname[0] != '\0'; i++) {
			if (vlanconfig[i].if_type == IF_ETHER) {
				vlanconfig[i].vid = 0;
				vlanconfig[i].memPort = 0;
				vlanconfig[i].untagSet = 0;
			}
		}
		...
		if (mode == BRIDGE_MODE) {
			...
			vlanconfig[1].memPort = 1 << 6;     /* Extension Port */
			vlanconfig[1].untagSet = 1 << 6;
			vlanconfig[1].isWan = 0;
			vlanconfig[1].fid = RTL_LAN_FID;
		}
...
#if DAVO_VLAN_ENABLED
    vport_read_conf();
    vport_organize_vconf_table(vlanconfig, (sizeof(vlanconfig) / sizeof(vlanconfig[0])) - 1);
#endif
...
#endif
#if DAVO_VLAN_ENABLED
            if (pvlanconfig[i].memPort != pvlanconfig[i].untagSet) {
                rtl865x_setVlanPortTag(pvlanconfig[i].vid,
                        (pvlanconfig[i].memPort & ~pvlanconfig[i].untagSet), TRUE);
            }
#endif
...
{
	char tmpbuf[32];
	int32 size = JUMBO_FRAME_SIZE_NORMAL;
	uint32 regData;

	if (strtrim_from_user(tmpbuf, sizeof(tmpbuf), buff, len) > 0) {
...
#ifdef __DAVO__
/* configure Maximum Acceptable Packet Length to wan port(port 4) */
/* WAN ===> LAN */
		regData = READ_MEM32(PCRP4);
		regData &= (~AcptMaxLen_MASK);
		if (size == JUMBO_FRAME_SIZE_NORMAL) {
			regData |= AcptMaxLen_1536;
		} else if (size==JUMBO_FRAME_SIZE_9K) {
			regData |= AcptMaxLen_9K;
		} else if (size==JUMBO_FRAME_SIZE_16K) {
			regData |= AcptMaxLen_16K;
		}
		WRITE_MEM32(PCRP4, regData);
#endif
	}
...
static int custom_createPseudoDevForPassThru(void)
{
		...
		dev->irq = 0;
#ifdef __DAVO__
		dev->priv_flags |= (IFF_PASSTHRU | IFF_ETHERNET);
#endif
		rc = register_netdev(dev);
		if(!rc){
			SMP_LOCK_ETH(flags);
		      ...
		      unsigned long count, void *data)
{
	...
	if (strtrim_from_user(tmp, sizeof(tmp), buffer, count) > 0) {
		flag = (int)simple_strtol(tmp, NULL, 0);
		if(flag ^ oldStatus)
		{
			//IPv6 PassThru
		      ...
		      unsigned long count, void *data)
{
    ...
    char tmpBuf[32];
    int stormCtrlType = 0;
    int enableStormCtrl = -1;
    int port = -1, limit_bps = -1;
    uint32 regData1, regData2;

    if (strtrim_from_user(tmpBuf, sizeof(tmpBuf), buffer, count) > 0) {
        sscanf(tmpBuf, "%d %d %d %d", &port, &enableStormCtrl,
                &limit_bps, &stormCtrlType);

        if (port < 0 || port > 4 || enableStormCtrl < 0)
            return -EFAULT;

        regData1 = READ_MEM32(PCRP0 + port * 4);
        if (enableStormCtrl) {
            if (limit_bps < 0)
                return -EFAULT;
            if (stormCtrlType > 3)
            ...
            if (stormCtrlType & 0x01)
                regData1 |= BCSC_ENBROADCAST;

            if (stormCtrlType & 0x02)
                regData1 |= BCSC_ENMULTICAST;

            regData1 |= ENBCSC;
            WRITE_MEM32(PCRP0 + port * 4, regData1);

            regData2 = READ_MEM32(BSCR);
            regData2 = limit_bps;
            WRITE_MEM32(BSCR, regData2);
            regData2 = READ_MEM32(BSCR);
        } else {
            regData1 &= ~(ENBCSC);
            WRITE_MEM32(PCRP0 + port * 4, regData1);
...
#ifdef __DAVO__
static struct sk_buff_head igmp_pkt_queue;
static struct semaphore igmp_pkt_sem;

static int enqueue_igmp_worker_queue(struct sk_buff *skb)
{
	if (igmp_pkt_queue.next == NULL)
		return -1;

	skb_queue_tail(&igmp_pkt_queue, skb);
	up(&igmp_pkt_sem);
	return 0;
}
...
{
	struct sk_buff *skb;
	unsigned int fwdmask, vlan_pmask, pid;

	set_user_nice(current, 10);
	sema_init(&igmp_pkt_sem, 0);
	skb_queue_head_init(&igmp_pkt_queue);

	while (1) {
		schedule_timeout_uninterruptible(2);
		if (down_interruptible(&igmp_pkt_sem))
			continue;
		skb = skb_dequeue(&igmp_pkt_queue);
		if (skb != NULL) {
#ifdef CONFIG_NETFILTER
			if (rtl_MulticastRxFilterOff(skb, 4) != true) {
#endif
				vlan_pmask = rtl865x_getVlanPortMask(skb->srcVlanId);
				pid = skb->srcPort;
				fwdmask =  vlan_pmask & ~(1 << pid) & ((1 << RTL8651_MAC_NUMBER) - 1);
				rtl_igmpMldProcess(nicIgmpModuleIndex, skb->data, pid, &fwdmask);
				fwdmask = vlan_pmask & igmp_flood_decision(re865x_getIpv4Header(skb->data), pid);
				if (fwdmask)
					re865x_relayTrappedMCast(skb, skb->srcVlanId, fwdmask, TRUE);
#ifdef CONFIG_NETFILTER
			}
#endif
			skb->protocol = eth_type_trans(skb, skb->dev);
