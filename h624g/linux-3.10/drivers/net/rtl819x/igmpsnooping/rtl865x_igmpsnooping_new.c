...
#endif
#ifdef __DAVO__
static DEFINE_SPINLOCK(mldprocess_lock);
#endif
...
#endif
{
	...
	reportEventContext.moduleIndex=moduleIndex;
#ifdef __DAVO__
	spin_lock_irqsave(&mldprocess_lock, flags);
#endif
	switch(pktBuf[0])
	{
		case IGMP_QUERY:
...
#ifdef __DAVO__
	spin_unlock_irqrestore(&mldprocess_lock, flags);
#endif
	return fwdPortMask;
...
static void _rtl865x_configIgmpSnoopingExpire(int32 disableExpire)
{
	...
	if((rtl_mCastTimerParas.disableExpire==TRUE) && (disableExpire==FALSE) )
	{
		...
		struct timespec ts;
		ktime_get_ts(&ts);
		currentTimeVector.tv_sec = ts.tv_sec;
#else
...
#ifdef __DAVO__	/* APACRTL-536 */
			|| (multicastDataInfo->groupAddr[0]==0xE0000141)
			|| (multicastDataInfo->groupAddr[0]==0xE000014c)
			|| (multicastDataInfo->groupAddr[0]==0xE00001b2)
#endif
			)
		{
...
static void rtl_multicastSysTimerExpired(uint32 expireDada)
{
	...
	spin_lock_irqsave(&mldprocess_lock, flags);
	rtl_maintainMulticastSnoopingTimerList(ts.tv_sec);
	spin_unlock_irqrestore(&mldprocess_lock, flags);
#else
	struct timeval currentTimeVector;
