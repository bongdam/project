...
/* Blink LEDs in burning flash */
#define BLINK_IN_BURN 1
...
#ifdef BLINK_IN_BURN
static void burn_notice_func(unsigned long cmd);
int gpio_sys_reset(void);
#endif
static int set_led_pin_usage(int pin, int gpio_sel, int data);
static unsigned int factory_test_mode = 0;
static unsigned int rstbtn_pressed = 0;
#endif
...
static ssize_t watchdog_start_single_write(struct file *file, const char __user * userbuf, size_t count, loff_t * off)
{
	char flag[20];
...
static ssize_t watchdog_kick_single_write(struct file *file, const char __user * userbuf, size_t count, loff_t * off)
{
	char flag[20];
	...
	if (probe_state == PROBE_NULL) {
		...
		if (pressed) {
			...
			if (probe_counter < 2) {
				...
				if ( default_flag == '0' )
					printk("+Restore default setting by rst-btn\n");
				//reload default
				default_flag = '1';
#ifdef __DAVO__
...
#ifdef __DAVO__
static int factory_reset_test_write(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	factory_test_mode = 1;
	return count;
}

static int factory_reset_test_read_proc(struct seq_file *s, void *v)
{
	seq_printf(s, "%d\n", rstbtn_pressed);
	return 0;
}

static int factory_reset_test_open(struct inode *inode, struct file *file)
{
	return (single_open(file, factory_reset_test_read_proc, NULL));
}

struct file_operations factory_reset_test_proc_fops = {
	.open = factory_reset_test_open,
	.write = factory_reset_test_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
void set_mux_to_gpio(void)
{
	// pin number ??
	REG32(RTL_GPIO_MUX4) = (REG32(RTL_GPIO_MUX4) & ~(7 << 0)) | (3 << 0);
	REG32(PEFGH_CNR) = REG32(PEFGH_CNR) & ~(1 << (6 + 8));
	REG32(PEFGH_DIR) = REG32(PEFGH_DIR) | (1 << (6 + 8));
}

void write_gpio(int val)
{
	if (val)
		REG32(PEFGH_DAT) = REG32(PEFGH_DAT) | (1 << (6 + 8));
	else
		REG32(PEFGH_DAT) = REG32(PEFGH_DAT) & ~(1 << (6 + 8));
}
...
//Brad add for update flash check 20080711
	...
	len = (count < sizeof(flag)) ? count : sizeof(flag);
	if (buffer && !copy_from_user(&flag, buffer, len)) {
		if (flag[0] == 'E') {
...
//Brad add for update flash check 20080711
			...
			/*DAVO mfgtst usb command 20150529*/
			set_mux_to_gpio();
		} else if (flag[0] == '6') {
			write_gpio(flag[1] - '0');
		}
			...
			/* Reset-FSM & Upgrade-blinker must be mutually exclusive.
			 * 2015-04-14 13:55 young
			 */
			if (del_timer(&probe_timer)) {
				probe_counter = 0;
				probe_state = PROBE_NULL;
			}
			burn_notice_func(1);
			...
			mod_timer(&probe_timer, jiffies + HZ);
			   ...
			   (flag[0] == 'p' || flag[0] == 'P') &&
			   isdigit(flag[1]) && isdigit(flag[2]))
			set_led_pin_usage(flag[1] - '0', (flag[0] == 'p'), flag[2] - '0');
		else if (flag[0] == 'R') {
			gpio_sys_reset();
		} else {
		}
...
static int read_gpio_wscd_pid(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len;
	char flag;
...
static int write_gpio_wscd_pid(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char flag[20];
	char start_count[10], wait[10];
	if (count < 2)
...
__NOMIPS16
#endif
static int default_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	if (count < 2)
		return -EFAULT;
	if (buffer && !copy_from_user(&default_flag, buffer, 1)) {
#ifdef __DAVO__
		dvflag_set((default_flag == '1') ? DF_RSTASSERTED : 0, DF_RSTASSERTED);
#endif
		return count;
	}
	return -EFAULT;
...
static ssize_t gpio_single_write(struct file *file, const char __user * userbuf, size_t count, loff_t * off)
{
...
static ssize_t load_default_single_write(struct file *file, const char __user * userbuf, size_t count, loff_t * off)
{
...
static int __init hostap_init(void)
{
	if (init_net.proc_net != NULL) {
		...
		if (!hostap_proc)
			printk(KERN_WARNING "Failed to mkdir " "/proc/rtl865x\n");
	} else
		hostap_proc = NULL;
...
static ssize_t write_watchdog_reboot(struct file *file, const char __user *buffer,
...
#ifdef BLINK_IN_BURN
static DEFINE_TIMER(burn_timer, burn_notice_func, 0, 0);
static unsigned int old_reg_stack[3];

/* 1 install timer
 * 2 uninstall timer
 * 0 blink action invoked by timer-subsystem
 */
static void burn_notice_func(unsigned long cmd)
{
	const unsigned int led_mask = (1 << (2 + 8)) |
				      (1 << (3 + 8)) |
				      (1 << (4 + 8)) |
				      (1 << (5 + 8)) |
				      (1 << (6 + 8));
	switch (cmd) {
	default:
		REG32(PABCD_DAT) = REG32(PABCD_DAT) ^ led_mask;
		mod_timer(&burn_timer, jiffies + (HZ >> 1));
		break;
	case 2:
		if (del_timer(&burn_timer)) {
			REG32(PABCD_DIR) = old_reg_stack[2];
			REG32(PABCD_CNR) = old_reg_stack[1];
			REG32(RTL_GPIO_MUX3) = old_reg_stack[0];
		}
		break;
	case 1:
		if (del_timer(&burn_timer) == 0) {
			old_reg_stack[0] = REG32(RTL_GPIO_MUX3);
			old_reg_stack[1] = REG32(PABCD_CNR);
			old_reg_stack[2] = REG32(PABCD_DIR);

			REG32(RTL_GPIO_MUX3) = (old_reg_stack[0] & ~(0x7fff)) | 0x36db;
			REG32(PABCD_CNR) = old_reg_stack[1] & ~led_mask;
			REG32(PABCD_DIR) = old_reg_stack[2] | led_mask;
		}
		mod_timer(&burn_timer, jiffies + (HZ >> 1));
		break;
	}
...
static int set_led_pin_usage(int pin, int gpio_sel, int data)
{
	...
	pin -= 2;
	if (gpio_sel) {
		REG32(RTL_GPIO_MUX3) &= ~(7 << (pin * 3));
		REG32(RTL_GPIO_MUX3) |= (3 << (pin * 3));

		REG32(PABCD_CNR) &= ~(1 << (pin + 10));
		/* OUT direction */
		REG32(PABCD_DIR) |= (1 << (pin + 10));
		if (data)
			REG32(PABCD_DAT) |= (1 << (pin + 10));
		else
			REG32(PABCD_DAT) &= ~(1 << (pin + 10));
	} else {
		REG32(PABCD_DIR) &= ~(1 << (pin + 10));
...
{
	/* GPIO_F4 reset */
	REG32(RTL_GPIO_MUX3) = (REG32(RTL_GPIO_MUX3) & ~(7 << 15)) | (3 << 15);
	REG32(PEFGH_CNR) = REG32(PEFGH_CNR) & ~(1 << (4 + 8));
	REG32(PEFGH_DIR) = REG32(PEFGH_DIR) | (1 << (4 + 8));
	REG32(PEFGH_DAT) = REG32(PEFGH_DAT) | (1 << (4 + 8));
	return 0;	
}
EXPORT_SYMBOL(gpio_sys_reset);
...
# else
	...
	proc_create_data("gpio", 0, &proc_root, &gpio_proc_fops, NULL);
...
# endif
	...
	proc_create_data("load_default", 0, &proc_root, &load_default_proc_fops, NULL);
#endif

	proc_create_data("watchdog_reboot", 0, &proc_root, &watchdog_reboot_proc_fops, NULL);

#ifdef CONFIG_RTL_FLASH_DUAL_IMAGE_ENABLE
	proc_create_data("bootbank", 0, &proc_root, &bootbank_proc_fops, NULL);
#endif

#ifdef __DAVO__
	proc_create_data("factory_reset_test", 0, &proc_root, &factory_reset_test_proc_fops, NULL);
#endif
...
#ifdef  CONFIG_RTL_USERSPACE_WTDOG
	proc_create_data("watchdog_start", 0, &proc_root, &watchdog_start_proc_fops, NULL);

	proc_create_data("watchdog_kick", 0, &proc_root, &watchdog_kick_proc_fops, NULL);
#endif
	return 0;
}
