...
struct net_bridge_fdb_entry
{
	...
	__u16				vlan_id;
#ifdef __DAVO__
#define QUOTA_BLK_BIT		7
#define QUOTA_MBR_MASK		0x7F
#define QUOTA_MBR(f)		((f)->quota_packed & QUOTA_MBR_MASK)
	unsigned char			quota_packed;
#endif
};
...
#endif
#ifdef __DAVO__
	int source_port;	/* for STP by young */
#endif
};
...
#ifdef __DAVO__
extern int br_feature_setup(void);

#if !defined(CONFIG_RTL_STP) && !defined(CONFIG_RTL_HW_STP)
extern int pstp_ratelimit(struct net_bridge_port *p, struct sk_buff *skb);
extern int rtl865x_setSpanningEnable(char spanningTreeEnabled);
#else
#define pstp_ratelimit(bport, skb) ({ (void)(bport); (void)(skb); -1; })
#define rtl865x_setSpanningEnable(on) ({ (void)(lock); 0; })
#endif
...
extern int rtl_quota_l2_control(const u8 *addr, int cmd, void *lparm, int wparm);
extern int mbr_port_quota_enabled;
#else
#define mbr_port_quota_enabled	0
#define rtl_quota_l2_control(a, b, c, d)	do {} while (0)
#endif
#endif	/* __DAVO__ */
#endif
