...
# ifdef __DAVO__
			if ((skb->dev->priv_flags & (IFF_PASSTHRU|IFF_ETHERNET)) == (IFF_PASSTHRU|IFF_ETHERNET) &&
			    !ether_addr_equal_u16(dest, 0x3333) &&
			    (p->dev->priv_flags & (IFF_PASSTHRU|IFF_ETHERNET)) == IFF_ETHERNET)
				continue;
# else
			if (!strcmp(skb->dev->name, "peth0") && (!(dest[0] == 0x33 && dest[1] == 0x33))) {
...
#  endif
			{
				if (is_broadcast_ether_addr(dest) ||
				    is_unicast_ether_addr(dest)) {
					if ((rtl_is_lan_dev(skb->dev) == TRUE) &&
					    (rtl_is_lan_dev(p->dev) == TRUE))
						continue;
				}
			}
# endif
