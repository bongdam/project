...
# endif
				...
				if (((proto == IPPROTO_UDP) || (proto == IPPROTO_TCP)) && (reserved == 0)) {
					...
					ret = rtl_getMulticastDataFwdInfo(brIgmpModuleIndex,
...
#  if defined(CONFIG_RTK_VLAN_SUPPORT)
							if (rtk_vlan_support_enable == 0) {
								rtl865x_ipMulticastHardwareAccelerate(br,
									multicastFwdInfo.fwdPortMask, srcPort, srcVlanId,
...
#   if defined(CONFIG_RTK_VLAN_SUPPORT)
							if (rtk_vlan_support_enable == 0) {
								rtl865x_ipv6MulticastHardwareAccelerate(br,
									multicastFwdInfo.fwdPortMask, srcPort,
									srcVlanId, sip, dip);
							}
#   else
							rtl865x_ipv6MulticastHardwareAccelerate(br,
...
#  endif	/* CONFIG_RTL_HARDWARE_MULTICAST && CONFIG_RTL_8198C */
				} else {
					br_flood_deliver(br, skb);
				}
			}
