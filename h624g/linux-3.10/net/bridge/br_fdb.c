...
static void fdb_delete(struct net_bridge *br, struct net_bridge_fdb_entry *f)
{
	...
	if (mbr_port_quota_enabled)
		rtl_quota_l2_control(f->addr.addr, DEL_UNBLK, (void *)f, 0);
#endif
...
#endif
#ifdef __DAVO__
		/* be located after rtl_fdb_create_hooks which undo source blocking */
		fdb->quota_packed = 0;
		if (mbr_port_quota_enabled && !is_local && !(addr[0] & 1))
			rtl_quota_l2_control(addr, TEST_AND_BLK, (void *)fdb, 0);
#endif
		...
		if (unlikely(fdb->is_local)) {
#ifdef __DAVO__
			int phy_no;
			if (skb && br->stp_enabled != BR_NO_STP &&
			    (phy_no = pstp_ratelimit(source, skb)) > -1)
				pr_warning("+%s: port %d(%s) entering blocking(r) state\n",
					   br->dev->name, phy_no, source->dev->name);
#endif
			if (net_ratelimit())
			...
			if (fdb->dst != source) {
				...
				if (!memcmp(fdb->dst->dev->name, "wlan", 4)) {
					// del sta
					sprintf(para, "%02x%02x%02x%02x%02x%02xno",
					...
					if (fdb->dst->dev->netdev_ops->ndo_do_ioctl != NULL)
						fdb->dst->dev->netdev_ops->ndo_do_ioctl(fdb->dst->dev, (struct ifreq *)&wrq,
											0x89f7 /* RTL8185_IOCTL_DEL_STA */ );
				} else
#endif
			...
			if (fdb->quota_packed &&
			    !(addr[0] & 1) &&
			    ((BR_INPUT_SKB_CB(skb)->source_port != QUOTA_MBR(fdb)) ||
			     (((long)jiffies - (long)fdb->updated) >= HZ)))
				rtl_quota_l2_control(addr, SYNC_BLK, (void *)fdb, INT_MAX);
#endif
			fdb->updated = jiffies;
		}
	} else {
