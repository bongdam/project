			     ...
			     int event, unsigned int flags)
{
	...
	pmsg->prefix_pad1 = 0;
#ifdef __DAVO__		/* Utilize padding bytes in order to send if_flags
			 * that denotes Managed and Other bit.
			 */
	pmsg->prefix_pad2 = (unsigned short)idev->if_flags;
#else
	pmsg->prefix_pad2 = 0;
#endif
...
static int addrconf_force_dad_failure(struct ctl_table *table,
				struct in6_addr *addr)
{
	struct inet6_ifaddr *ifp;
	struct inet6_dev *idev;
	...
	if (idev) {
		...
		list_for_each_entry(ifp, &idev->addr_list, if_list) {
			...
			if (ifp->flags & (IFA_F_TENTATIVE | IFA_F_DADFAILED))
				continue;
			if (timer_pending(&ifp->timer))
				continue;
			spin_lock(&ifp->lock);
			if (ifp->state != INET6_IFADDR_STATE_DAD)
				ifp->flags |= (IFA_F_TENTATIVE | IFA_F_DADFAILED);
			spin_unlock(&ifp->lock);
		}
		read_unlock_bh(&idev->lock);
	}
...
static int addrconf_sysctl_force_dad_failure(ctl_table *ctl, int write,
			    void __user *buffer, size_t *lenp, loff_t *ppos)
{
	ctl_table fake_table = *ctl;
	struct in6_addr addr;
	...
	if (write) {
		if (in6_pton(buf, -1, addr.s6_addr, -1, NULL) == 1)
			addrconf_force_dad_failure(ctl, &addr);
	}
	if (ret)
		*ppos = pos;
	return ret;
...
#ifdef __DAVO__
		{
			.procname	= "force_dad_failure",
			.data		= "",
			.maxlen		= 4,
			.mode		= 0200,
			.proc_handler	= addrconf_sysctl_force_dad_failure,
		},
#endif
		{
			/* sentinel */
