...
extern int rtl865x_isEthArp_r(ipaddr_t ip);
#endif
#endif
...
#ifdef __DAVO__		/* APACRTL-87 */
	if (rtl865x_isEthArp_r(lanIp) == TRUE)
#else
...
{
	if (strtrim_from_user(gHwNatSetting, sizeof(gHwNatSetting), buffer, count) > 0) {
		if ((gHwNatSetting[0] == '-') && (gHwNatSetting[1] == '1')) {
			/* hardware NAT disabled, operation mode = gateway */
			gHwNatEnabled = 0;
			rtl865x_nat_reinit();
			rtl865x_reChangeOpMode();
			//rtl8651_setAsicOperationLayer(4);
		} else if (gHwNatSetting[0] == '0') {
			/* hardware NAT disabled, operation mode = gateway */
			gHwNatEnabled = 0;
			rtl865x_nat_reinit();
			...
			rtl8651_setAsicOperationLayer(4);
#endif
		} else if (gHwNatSetting[0] == '1') {
			/* hardware NAT enabled, operation mode = gateway */
			rtl865x_changeOpMode(GATEWAY_MODE);
#if !defined(CONFIG_RTK_VLAN_SUPPORT)
			rtl8651_setAsicOperationLayer(4);
#endif
			gHwNatEnabled = 1;
		} else if (gHwNatSetting[0] == '2') {
			/* hardware NAT enabled, operation mode = bridge mode */
			//rtl865x_delRoute(wdst, wmask);
			//rtl865x_delRoute(ldst, lmask);
			rtl865x_changeOpMode(BRIDGE_MODE);
			//rtl8651_setAsicOperationLayer(3);
			gHwNatEnabled = 2;
		} else if (gHwNatSetting[0] == '3') {
			/* hardware NAT enabled, operation mode = wisp mode */
