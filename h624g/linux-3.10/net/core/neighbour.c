...
#ifdef __DAVO__
int neigh_ipv4_add(struct net_device *dev, __be32 addr, __u8 *lladdr,
		   __u16 state, __u32 request)
{
	...
	for (tbl = neigh_tables; tbl; tbl = tbl->next) {
		...
		neigh = neigh_lookup(tbl, &addr, dev);
		if (neigh == NULL) {
			if (!(request & NLM_F_CREATE)) {
				err = -ENOENT;
				goto out;
			}

			neigh = __neigh_lookup_errno(tbl, &addr, dev);
			if (IS_ERR(neigh)) {
				err = PTR_ERR(neigh);
				goto out;
			}
		} else {
			if (request & NLM_F_EXCL) {
				err = -EEXIST;
				neigh_release(neigh);
				goto out;
			}

			if (!(request & NLM_F_REPLACE))
				flags &= ~NEIGH_UPDATE_F_OVERRIDE;
		}

		err = neigh_update(neigh, lladdr, state, flags);
		neigh_release(neigh);
		goto out;
	}
