	...
	if (!(range->flags & NF_NAT_RANGE_PROTO_RANDOM)) {
		...
		if ((!(range->flags & NF_NAT_RANGE_PROTO_SPECIFIED) ||
			...
			if ((gHwNatEnabled) && (maniptype == NF_NAT_MANIP_SRC) &&
				...
				unsigned int asicNaptHashScore;
				rtl865x_napt_entry pentr;
				struct nf_conntrack_tuple *t;

				t = &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple;
				pentr.protocol = (t->dst.protonum == IPPROTO_TCP) ? 1 : 0;

				pentr.intIp = t->src.u3.ip;
				pentr.remIp = t->dst.u3.ip;
				pentr.extIp = tuple->src.u3.ip;
				pentr.intPort = pentr.protocol ? t->src.u.tcp.port : t->src.u.udp.port;
				pentr.remPort = pentr.protocol ? t->dst.u.tcp.port : t->dst.u.udp.port;
				pentr.extPort = pentr.protocol ? tuple->src.u.tcp.port : tuple->src.u.udp.port;
#ifdef __DAVO__
				asicNaptHashScore = getAsicNaptHashScore(&pentr);
#else
				asicNaptHashScore = 0;
				rtl865x_getAsicNaptHashScore(&pentr, &asicNaptHashScore);
#endif
				if (asicNaptHashScore >= napt_hash_score) {
					rtl865x_preReserveConn(&pentr);
					goto out;
				}
			} else
				goto out;
...
#  endif
#  ifdef __DAVO__
		   ) && (nf_nat_preprocess_packet(ct, hooknum, skb)
#  endif /* __DAVO__ */
		   ))
# endif	/* 1 */
		{
