...
#ifdef CONFIG_NF_NAT_BATTLENET
struct nf_ct_event_notifier sc_ct_notifier;
EXPORT_SYMBOL(sc_ct_notifier);

static int
ctnetlink_conntrack_event_chain(unsigned int events, struct nf_ct_event *item)
{
	if (sc_ct_notifier.fcn)
		sc_ct_notifier.fcn(events, item);
	return ctnetlink_conntrack_event(events, item);
}
#endif /* CONFIG_NF_NAT_BATTLENET */
...
#ifdef CONFIG_NF_NAT_BATTLENET
	.fcn = ctnetlink_conntrack_event_chain,
#else
