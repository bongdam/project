...
static int ifa_cmpr4(struct if_addr_entry *ifa, struct if_addr_entry *ifb)
{
	return (ifa->addr.addr.s_addr != ifb->addr.addr.s_addr &&
		((ifb->addr.addr.s_addr & ifa->mask.s_addr) ==
		 (ifa->addr.addr.s_addr & ifa->mask.s_addr)))
		? 0 : -1;
}
...
int is_ifa_subnet(__be32 address, int ifindex)
{
	...
	ifa = (struct if_addr_entry *)ip_addr_entry_get(&in_addr_locals,
					ifindex ? (void *)ifa_cmpr3 :
					(void *)ifa_cmpr4, (void *)&ifb);
	if (ifa != NULL)
		ip_addr_entry_put(&ifa->addr);
	return (ifa != NULL) ? ifa->ifindex : 0;
}
