...
extern unsigned char hndcrc8(unsigned char *pdata, unsigned int nbytes, unsigned char crc);
...
BCMINITFN(nvram_rehash)(struct nvram_header *header)
{
	...
	if (nvram_space_str)
		nvram_space =  simple_strtoul(nvram_space_str, NULL, 0);
#ifdef __DAVO__
	else if (header->len > DEF_NVRAM_SPACE)
	...
	for (; *name; name = value + strlen(value) + 1) {
		...
		*eq = '\0';
		...
		if ( nvram_full_init ||
			...
			&& (memcmp(name, "dv_", 3)!=0)
			&& (memcmp(name, "wl_", 3)!=0)
			&& (memcmp(name, "nas_", 4)!=0)
			&& (memcmp(name, "vfilter", 7)!=0)) )
#endif
#endif
...
BCMINITFN(_nvram_init)(void)
{
	...
	if (!(header = (struct nvram_header *)malloc(MAX_NVRAM_SPACE))) {
