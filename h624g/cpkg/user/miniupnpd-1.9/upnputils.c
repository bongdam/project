		...
		case AF_LINK:
			{
				...
				n = snprintf(str, size, "index=%hu type=%d %s", sdl->sdl_index, sdl->sdl_type, link_ntoa(sdl));
			}
			break;
#endif	/* AF_LINK */
		default:
...
struct lan_addr_s *get_lan_for_peer(const struct sockaddr * peer)
{
	...
	if (peer->sa_family == AF_INET6) {
		...
		if (IN6_IS_ADDR_V4MAPPED(&peer6->sin6_addr)) {
			...
			for (lan_addr = lan_addrs.lh_first; lan_addr != NULL; lan_addr = lan_addr->list.le_next) {
				if ((peer_addr.s_addr & lan_addr->mask.s_addr) == (lan_addr->addr.s_addr & lan_addr->mask.s_addr))
					break;
			}
		} else {
			int index = -1;
			...
			DEBUG("%s looking for LAN interface index=%d",
					"get_lan_for_peer()", index);
			for (lan_addr = lan_addrs.lh_first; lan_addr != NULL; lan_addr = lan_addr->list.le_next) {
				DEBUG("ifname=%s index=%u str=%s addr=%08x mask=%08x", lan_addr->ifname, lan_addr->index,
						lan_addr->str, ntohl(lan_addr->addr.s_addr), ntohl(lan_addr->mask.s_addr));
	...
	if (lan_addr) {
		DEBUG("%s: %s found in LAN %s %s", "get_lan_for_peer()", dbg_str, lan_addr->ifname, lan_addr->str);
	} else {
		DEBUG("%s: %s not found !", "get_lan_for_peer()", dbg_str);
	}
#endif /* DEBUG */
	return lan_addr;
}
...
void say(const char *fmt, ...)
{
	...
	va_start(va, fmt);
	n = vsnprintf(buffer, sizeof(buffer) - 1, fmt, va);
	if (n < (int)(sizeof(buffer) - 1))
		p = buffer;
	else if ((p = (char *)malloc(n + 2)))
...
unsigned getuptime(struct timeval *tvp)
{
	struct timespec ts;
	ygettime(&ts);
	if (tvp) {
...
int leased_ip(struct in_addr ip)
{
	const char *path = "/var/lib/misc/udhcpd.leases";
	FILE *f;
	...
	if ((f = fopen(path, "r"))) {
		ygettime(&ts);
		while (fread(&lease, sizeof(lease), 1, f)) {
			if (lease.yiaddr == ip.s_addr &&
					memcmp(lease.chaddr, "\x00\x00\x00\x00\x00\x00", 6)) {
				valid = (((long)ts.tv_sec - (long)lease.expires) < 0);
				break;
			}
		}
		fclose(f);
