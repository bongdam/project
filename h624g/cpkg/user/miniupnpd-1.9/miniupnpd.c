...
#endif
{
	...
	if (s < 0 && ipv6 && errno == EAFNOSUPPORT) {
		...
		LOG(LOG_WARNING, "socket(PF_INET6, ...) failed with EAFNOSUPPORT, disabling IPv6");
...
#endif
	if (s < 0) {
		LOG(LOG_ERR, "socket(http): %m");
		return -1;
	}

	if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &i, sizeof(i)) < 0) {
		LOG(LOG_WARNING, "setsockopt(http, SO_REUSEADDR): %m");
	}
#if 0
	...
	if(setsockopt(s, IPPROTO_IPV6, IPV6_V6ONLY, &i, sizeof(i)) < 0) {
		LOG(LOG_WARNING, "setsockopt(http, IPV6_V6ONLY): %m");
	}
#endif

	if (!set_non_blocking(s)) {
		LOG(LOG_WARNING, "set_non_blocking(http): %m");
	}
...
#ifdef ENABLE_IPV6
	if (bind(s, ipv6 ? (struct sockaddr *)&listenname6 : (struct sockaddr *)&listenname4,
				listenname_len) < 0)
#else
	if (bind(s, (struct sockaddr *)&listenname, listenname_len) < 0)
#endif
	{
		LOG(LOG_ERR, "bind(http): %m");
		close(s);
		return -1;
	}

	if (listen(s, 5) < 0) {
		LOG(LOG_ERR, "listen(http): %m");
		close(s);
		return -1;
	}
...
static struct upnphttp *ProcessIncomingHTTP(int shttpl, const char *protocol)
{
	...
	if (shttp < 0) {
		...
		if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)
			LOG(LOG_ERR, "accept(http): %m");
	} else {
		struct upnphttp *tmp = 0;
		char addr_str[64];

		sockaddr_to_string((struct sockaddr *)&clientname, addr_str, sizeof(addr_str));
		LOG(LOG_INFO, "%s connection from %s", protocol, addr_str);
			...
			if (tmp) {
				...
				if (clientname.ss_family == AF_INET) {
					...
					if (IN6_IS_ADDR_V4MAPPED(&addr->sin6_addr)) {
						memcpy(&tmp->clientaddr, &addr->sin6_addr.s6_addr[12], 4);
					} else {
						tmp->ipv6 = 1;
						memcpy(&tmp->clientaddr_v6, &addr->sin6_addr, sizeof(struct in6_addr));
					}
				}
#else
				...
				LOG(LOG_ERR, "New_upnphttp() failed");
				close(shttp);
			}
		}
	}
...
static int OpenAndConfNFqueue(void)
{
	...
	if (!(nfqHandle = nfq_open())) {
		LOG(LOG_ERR, "Error in nfq_open(): %m");
		return -1;
	}
	...
	if ((e = nfq_unbind_pf(nfqHandle, AF_INET)) < 0) {
		LOG(LOG_ERR, "Error in nfq_unbind_pf(): %m");
		return -1;
	}

	/* Bind this handler to process IP packets... */
	if (nfq_bind_pf(nfqHandle, AF_INET) < 0) {
		LOG(LOG_ERR, "Error in nfq_bind_pf(): %m");
		return -1;
	}

	/*      Install a callback on queue -Q */
	if (!(myQueue = nfq_create_queue(nfqHandle,  nfqueue, &nfqueue_cb, NULL))) {
		LOG(LOG_ERR, "Error in nfq_create_queue(): %m");
		return -1;
	}

	/*      Turn on packet copy mode */
	if (nfq_set_mode(myQueue, NFQNL_COPY_PACKET, 0xffff) < 0) {
		LOG(LOG_ERR, "Error setting packet copy mode (): %m");
		return -1;
	}
...
static int nfqueue_cb(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
	...
	if (ph) {
		...
		if ((dport == 1900) && (id_protocol == IPPROTO_UDP)
			...
			for (; i < n_nfqix; i++) {
				if (nfqix[i] == idx) {
					...
					/* printf("pkt found %s\n",dd);*/
					ProcessSSDPData(sudp, dd, size - x, &sendername, (unsigned short) 5555);
				}
			}
		}
		...
		LOG(LOG_ERR,"nfq_get_msg_packet_hdr failed");
		return 1;
...
#ifdef USE_MINIUPNPDCTL
static int OpenAndConfCtlUnixSocket(const char *path)
{
	...
	if (bind(s, (struct sockaddr *)&localun, sizeof(struct sockaddr_un)) < 0) {
		LOG(LOG_ERR, "bind(sctl): %m");
		close(s);
		s = -1;
	} else if(listen(s, 5) < 0) {
		LOG(LOG_ERR, "listen(sctl): %m");
		close(s);
		s = -1;
	}
	return s;
...
static void write_upnphttp_details(int fd, struct upnphttp *e)
{
	char buffer[256];
	int len;
	write(fd, "HTTP :\n", 7);
...
static void write_ctlsockets_list(int fd, struct ctlelem *e)
{
	char buffer[256];
	int len;
	write(fd, "CTL :\n", 6);
...
static void write_option_list(int fd)
{
	char buffer[256];
	int len;
	unsigned int i;
...
static void write_command_line(int fd, int argc, char **argv)
{
	char buffer[256];
	int len;
	int i;
...
static void sigterm(int sig)
{
	 ...
	 * signal(3) */
	LOG(LOG_NOTICE, "received signal %d, good-bye", sig);
#endif
...
static void sigusr1(int sig)
{
	...
	/* calling syslog() is forbidden in signal handler according to
	 * signal(3) */
	LOG(LOG_INFO, "received signal %d, public ip address change", sig);
#endif
...
static void set_startup_time(int sysuptime)
{
	startup_time = getuptime(NULL);
	...
	if (sysuptime) {
		...
		if (fd < 0) {
			LOG(LOG_ERR, "open(\"/proc/uptime\" : %m");
		} else {
			memset(buff, 0, sizeof(buff));
			if (read(fd, buff, sizeof(buff) - 1) < 0) {
				LOG(LOG_ERR, "read(\"/proc/uptime\" : %m");
			} else {
				uptime = atoi(buff);
				LOG(LOG_INFO, "system uptime is %d seconds", uptime);
			}
			close(fd);
			startup_time -= uptime;
		}
		...
		if (kc != NULL) {
			...
			if (ksp && (kstat_read(kc, ksp, NULL) != -1)) {
				...
				if (ptr)
					...
					LOG(LOG_ERR, "cannot find boot_time kstat");
			}
			else
				LOG(LOG_ERR, "cannot open kstats for unix/0/system_misc: %m");
			kstat_close(kc);
		}
#else
		...
		if (sysctl(name, 2, &boottime, &size, NULL, 0) < 0) {
			LOG(LOG_ERR, "sysctl(\"kern.boottime\") failed");
		} else {
...
static int init(int argc, char **argv, struct runtime_vars *v)
{
	...
	if (readoptionsfile(optionsfile) < 0) {
		...
		for (i=0; i<(int)num_options; i++) {
			switch (ary_options[i].id) {
				...
				case UPNPONLYBOOTPC:
					leased_only = atoi(ary_options[i].value);
					break;
#endif
				default:
			...
			case 'a':
				...
				if (i + 1 < argc) {
					...
					for (lan_addr2 = lan_addrs.lh_first; lan_addr2 != NULL; lan_addr2 = lan_addr2->list.le_next) {
						if (!strncmp(lan_addr2->str, lan_addr->str, 15))
							break;
					}
					if (lan_addr2 == NULL)
						LIST_INSERT_HEAD(&lan_addrs, lan_addr, list);
	...
	OPEN_LOG("miniupnpd", openlog_option, LOG_MINIUPNPD);
...
#else
	Lmask = !debug_flag ? 0 : (1 << (LOG_DEBUG + 1)) - 1;
#endif

	if (checkforrunning(pidfilename) < 0) {
		LOG(LOG_ERR, "MiniUPnPd is already running. EXITING");
		return 1;
	}
	...
	if (sigaction(SIGTERM, &sa, NULL) < 0) {
		LOG(LOG_ERR, "Failed to set %s handler. EXITING", "SIGTERM");
		return 1;
	}
	if (sigaction(SIGINT, &sa, NULL) < 0) {
		LOG(LOG_ERR, "Failed to set %s handler. EXITING", "SIGINT");
		return 1;
	}
	sa.sa_handler = SIG_IGN;
	if (sigaction(SIGPIPE, &sa, NULL) < 0) {
		LOG(LOG_ERR, "Failed to ignore SIGPIPE signals");
	}
	sa.sa_handler = sigusr1;
	if (sigaction(SIGUSR1, &sa, NULL) < 0) {
		LOG(LOG_NOTICE, "Failed to set %s handler", "SIGUSR1");
	}
	...
	if (init_redirect() < 0) {
		LOG(LOG_ERR, "Failed to init redirection engine. EXITING");
		return 1;
	}
#ifdef ENABLE_UPNPPINHOLE
#ifdef USE_NETFILTER
...
#ifdef ENABLE_LEASEFILE
	/*remove(lease_file);*/
	LOG(LOG_INFO, "Reloading rules from lease file");
...
int main(int argc, char **argv)
{
	...
	if (
		...
		LOG(LOG_ERR, "Why did you run me anyway?");
		return 0;
	}

	LOG(LOG_INFO, "version " MINIUPNPD_VERSION " starting%s%sext if %s BOOTID=%u",
#ifdef ENABLE_NATPMP
#ifdef ENABLE_PCP
	...
	if (GETFLAG(ENABLEUPNPMASK)) {
...
#endif /* ENABLE_IPV6 */
		if (shttpl < 0) {
			LOG(LOG_ERR, "Failed to open socket for HTTP. EXITING");
			return 1;
		}
		v.port = listen_port;
		LOG(LOG_NOTICE, "HTTP listening on port %d", v.port);
#if defined(V6SOCKETS_ARE_V6ONLY) && defined(ENABLE_IPV6)
		if (!GETFLAG(IPV6DISABLEDMASK)) {
			shttpl_v4 = OpenAndConfHTTPSocket(&listen_port, 0);
			if (shttpl_v4 < 0) {
				LOG(LOG_ERR, "Failed to open socket for HTTP on port %hu (IPv4). EXITING", v.port);
				return 1;
			}
		}
...
#endif /* ENABLE_IPV6 */
		if (shttpl < 0) {
			LOG(LOG_ERR, "Failed to open socket for HTTPS. EXITING");
			return 1;
		}
		v.https_port = listen_port;
		LOG(LOG_NOTICE, "HTTPS listening on port %d", v.https_port);
#if defined(V6SOCKETS_ARE_V6ONLY) && defined(ENABLE_IPV6)
		shttpsl_v4 =  OpenAndConfHTTPSocket(&listen_port, 0);
		if (shttpsl_v4 < 0) {
			LOG(LOG_ERR, "Failed to open socket for HTTPS on port %hu (IPv4). EXITING", v.https_port);
			return 1;
		}
#endif /* V6SOCKETS_ARE_V6ONLY */
#endif /* ENABLE_HTTPS */
		...
		if (find_ipv6_addr(NULL, ipv6_addr_for_http_with_brackets, sizeof(ipv6_addr_for_http_with_brackets)) > 0) {
			LOG(LOG_NOTICE, "HTTP IPv6 address given to control points : %s",
					ipv6_addr_for_http_with_brackets);
		} else {
			memcpy(ipv6_addr_for_http_with_brackets, "[::1]", 6);
			LOG(LOG_WARNING, "no HTTP IPv6 address, disabling IPv6");
			SETFLAG(IPV6DISABLEDMASK);
		}
#endif
		...
		if (sudp < 0) {
			LOG(LOG_NOTICE, "Failed to open socket for receiving SSDP. Trying to use MiniSSDPd");
			if (SubmitServicesToMiniSSDPD(lan_addrs.lh_first->str, v.port) < 0) {
				LOG(LOG_ERR, "Failed to connect to MiniSSDPd. EXITING");
				return 1;
			}
		}
#ifdef ENABLE_IPV6
		if (!GETFLAG(IPV6DISABLEDMASK)) {
			...
			if (sudpv6 < 0) {
				LOG(LOG_WARNING, "Failed to open socket for receiving SSDP (IP v6).");
			}
		}
#endif
		...
		if (OpenAndConfSSDPNotifySockets(snotify) < 0) {
			LOG(LOG_ERR, "Failed to open sockets for sending SSDP notify "
					"messages. EXITING");
			return 1;
		}
		...
		if (sudp >= 0) {
			...
			if (sifacewatcher < 0) {
				LOG(LOG_ERR, "Failed to open socket for receiving network interface notifications");
			}
		}
#endif
	}
...
#ifdef ENABLE_PCP
		{
			LOG(LOG_ERR, "Failed to open sockets for NAT-PMP/PCP.");
		} else {
			LOG(LOG_NOTICE, "Listening for NAT-PMP/PCP traffic on port %u", NATPMP_PORT);
		}
#else
		{
			LOG(LOG_ERR, "Failed to open sockets for NAT PMP.");
		} else {
			LOG(LOG_NOTICE, "Listening for NAT-PMP traffic on port %u", NATPMP_PORT);
		}
#endif
	}
#endif
	...
	if (nfqueue != -1 && n_nfqix > 0) {
		...
		if (nfqh < 0) {
			LOG(LOG_ERR, "Failed to open fd for NFQUEUE.");
			return 1;
		} else {
			LOG(LOG_NOTICE, "Opened NFQUEUE %d",nfqueue);
		}
	}
#endif
	...
	while (!quitting) {
		...
		if ((startup_time<60*60*24) && (getuptime(NULL)>60*60*24)) {
			set_startup_time(GETFLAG(SYSUPTIMEMASK));
		}
		/* send public address change notifications if needed */
		if (should_send_public_address_change_notif) {
			LOG(LOG_INFO, "should send external iface address change notification(s)");
#ifdef ENABLE_NATPMP
			if (GETFLAG(ENABLENATPMPMASK))
		...
		if (timeofday.tv_sec >= (lasttimeofday.tv_sec + v.notify_interval)) {
			if (GETFLAG(ENABLEUPNPMASK))
				SendSSDPNotifies2(snotify, (unsigned short)v.port,
#ifdef ENABLE_HTTPS
		...
		if (nextruletoclean_timestamp
			...
			timeout.tv_usec = 0;
			DEBUG("setting timeout to %u sec",
...
#ifdef DEBUG
		if (i > 1) {
			DEBUG("%d active incoming HTTP connections", i);
		}
#endif
#ifdef ENABLE_NATPMP
		for (i = 0; i<addr_count; i++) {
		...
		if (select(max_fd+1, &readset, &writeset, 0, &timeout) < 0) {
			...
			LOG(LOG_ERR, "select(all): %m");
			LOG(LOG_ERR, "Failed to select open sockets. EXITING");
			return 1;	/* very serious cause of error */
		}
		i = try_sendto(&writeset);
		if (i < 0) {
			LOG(LOG_ERR, "try_sendto failed to send %d packets", -i);
		}
#ifdef USE_MINIUPNPDCTL
		...
		if ((sctl >= 0) && FD_ISSET(sctl, &readset)) {
			...
			s = accept(sctl, (struct sockaddr *)&clientname,
					&clientnamelen);
			DEBUG("sctl! : '%s'", clientname.sun_path);
			tmp = malloc(sizeof(struct ctlelem));
			if (tmp == NULL) {
				LOG(LOG_ERR, "Unable to allocate memory for ctlelem in main()");
				close(s);
			} else {
				tmp->socket = s;
			...
			if ((snatpmp[i] >= 0) && FD_ISSET(snatpmp[i], &readset)) {
				...
				if (msg_buff[0] == 0) {  /* version equals to 0 -> means NAT-PMP */
					...
					if (lan_addr == NULL) {
						...
						LOG(LOG_WARNING, "NAT-PMP packet sender %s not from a LAN, ignoring", sender_str);
						continue;
					}
		...
		if (spcp_v6 >= 0 && FD_ISSET(spcp_v6, &readset)) {
			...
			memset(msg_buff, 0, PCP_MAX_LEN);
			senderaddrlen = sizeof(senderaddr);
			len = ReceiveNATPMPOrPCPPacket(spcp_v6, (struct sockaddr *)&senderaddr,
					&senderaddrlen, &receiveraddr, msg_buff, sizeof(msg_buff));
			if (len >= 1)
		...
		if (sudp >= 0 && FD_ISSET(sudp, &readset)) {
			/*LOG(LOG_INFO, "Received UDP Packet");*/
#ifdef ENABLE_HTTPS
		...
		if (sudpv6 >= 0 && FD_ISSET(sudpv6, &readset)) {
			LOG(LOG_INFO, "Received UDP Packet (IPv6)");
#ifdef ENABLE_HTTPS
		...
		for (e = upnphttphead.lh_first; e != NULL; e = e->entries.le_next) {
			if (e->socket >= 0) {
				if (FD_ISSET(e->socket, &readset) || FD_ISSET(e->socket, &writeset)) {
					Process_upnphttp(e);
				}
			}
		}
		...
		if (nfqh >= 0 && FD_ISSET(nfqh, &readset)) {
			/* LOG(LOG_INFO, "Received NFQUEUE Packet");*/
			ProcessNFQUEUE(nfqh);
		}
#endif
...
shutdown:
	LOG(LOG_NOTICE, "shutting down MiniUPnPd");
	/* send good-bye */
	if (GETFLAG(ENABLEUPNPMASK)) {
#ifndef ENABLE_IPV6
...
#endif
		{
			LOG(LOG_ERR, "Failed to broadcast good-bye notifications");
		}
	}
	...
	if (pidfilename && (unlink(pidfilename) < 0)) {
		LOG(LOG_ERR, "Failed to remove pidfile %s: %m", pidfilename);
	}
