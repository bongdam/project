 ...
 */
static ssize_t send_from_to(int sockfd, const void *buf, size_t len, int flags,
		const struct sockaddr_in6 *src_addr, const struct sockaddr *dest_addr, socklen_t addrlen)
{
#ifdef IPV6_PKTINFO
	if (src_addr) {
		struct iovec iov;
...
/* delay = milli seconds */
ssize_t sendto_schedule2(int sockfd, const void *buf, size_t len, int flags,
		const struct sockaddr *dest_addr, socklen_t addrlen,
		const struct sockaddr_in6 *src_addr, unsigned int delay)
{
	...
	if (elt == NULL) {
		LOG(LOG_ERR, "malloc failed to allocate %u bytes",
				(unsigned)alloc_len);
		return -1;
	}
	elt->state = state;
...
ssize_t sendto_or_schedule(int sockfd, const void *buf, size_t len, int flags,
		const struct sockaddr *dest_addr, socklen_t addrlen)
{
	return sendto_schedule2(sockfd, buf, len, flags, dest_addr, addrlen, NULL, 0);
}

ssize_t sendto_or_schedule2(int sockfd, const void *buf, size_t len, int flags,
		const struct sockaddr *dest_addr, socklen_t addrlen, const struct sockaddr_in6 *src_addr)
{
	...
	for (elt = send_list.lh_first; elt != NULL; elt = elt->entries.le_next) {
		if (elt->state == EWAITREADY) {
			...
			if (elt->sockfd > *max_fd)
				*max_fd = elt->sockfd;
			n++;
		} else if ((elt->ts.tv_sec < now->tv_sec) || (elt->ts.tv_sec == now->tv_sec && elt->ts.tv_usec <= now->tv_usec)) {
...
int try_sendto(fd_set *writefds)
{
	...
	for (elt = send_list.lh_first; elt != NULL; elt = next) {
		...
		if ((elt->state == ESENDNOW) || (elt->state == EWAITREADY && FD_ISSET(elt->sockfd, writefds))) {
#ifdef DEBUG
			DEBUG("%s: %d bytes on socket %d",
			...
			if (n < 0) {
				if (errno == EINTR) {
					...
					if (sockaddr_to_string(elt->dest_addr, addr_str, sizeof(addr_str)) <= 0)
						addr_str[0] = '\0';
					LOG(LOG_ERR, "%s(sock=%d, len=%u, dest=%s): sendto: %m",
							"try_sendto", elt->sockfd, (unsigned)elt->len, addr_str);
					ret--;
				}
			} else if ((int)n != (int)elt->len) {
				LOG(LOG_WARNING, "%s: %d bytes sent out of %d", "try_sendto", (int)n, (int)elt->len);
			}
...
void finalize_sendto(void)
{
	...
	while (send_list.lh_first) {
		...
		for (elt = send_list.lh_first; elt != NULL; elt = next) {
			next = elt->entries.le_next;
			DEBUG("finalize_sendto(): %d bytes on socket %d",
					(int)elt->len, elt->sockfd);
			n = send_from_to(elt->sockfd, elt->buf, elt->len, elt->flags,
					elt->src_addr, elt->dest_addr, elt->addrlen);
			/*n = sendto(elt->sockfd, elt->buf, elt->len, elt->flags, elt->dest_addr, elt->addrlen);*/
			if (n < 0) {
				...
				LOG(LOG_WARNING, "finalize_sendto(): socket=%d sendto: %m", elt->sockfd);
			}
			/* remove from the list */
			LIST_REMOVE(elt, entries);
			free(elt);
