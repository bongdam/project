...
static int addnatrule(int proto, unsigned short eport,
		const char *iaddr, unsigned short iport, const char *rhost);

static int add_filter_rule(int proto, const char *rhost,
		const char *iaddr, unsigned short iport);

static int addpeernatrule(int proto,
		const char *eaddr, unsigned short eport,
		const char *iaddr, unsigned short iport,
		const char *rhost, unsigned short rport);

static int addpeerdscprule(int proto, unsigned char dscp,
		const char *iaddr, unsigned short iport,
		const char *rhost, unsigned short rport);
...
{
	return snprintf(dst, size, "%u.%u.%u.%u",
...
static void get_redirect_desc(unsigned short eport, int proto, char *desc,
		int desclen, unsigned int *timestamp)
{
	struct rdr_desc *p;
...
static int get_redirect_desc_by_index(int index, unsigned short *eport,
		int *proto, char *desc, int desclen, unsigned int *timestamp)
{
	int i = 0;
	struct rdr_desc *p;
	if (!desc || (desclen == 0))
...
{
	return get_nat_redirect_rule(miniupnpd_nat_chain, ifname, eport, proto,
			iaddr, iaddrlen, iport, desc, desclen, rhost, rhostlen,
			timestamp, packets, bytes);
}

int get_nat_redirect_rule(const char *nat_chain_name, const char *ifname, unsigned short eport, int proto,
		char *iaddr, int iaddrlen, unsigned short *iport,
		char *desc, int desclen,
		char *rhost, int rhostlen,
		unsigned int *timestamp,
		u_int64_t *packets, u_int64_t *bytes)
{
	...
	h = iptc_init("nat");
	if (!h) {
		LOG(LOG_ERR, "get_redirect_rule() : "
				"iptc_init() failed : %s",
				iptc_strerror(errno));
		return -1;
	}
	if (!iptc_is_chain(nat_chain_name, h)) {
		LOG(LOG_ERR, "chain %s not found", nat_chain_name);
	} else {
#ifdef IPTABLES_143
		for (e = iptc_first_rule(nat_chain_name, h);
				e; e = iptc_next_rule(e, h))
#else
		for (e = iptc_first_rule(nat_chain_name, &h);
				e; e = iptc_next_rule(e, &h))
#endif
		{
			if (proto == e->ip.proto) {
				...
				if (!strncmp(match->u.user.name, "tcp", IPT_FUNCTION_MAXNAMELEN)) {
		...
		u_int64_t *packets, u_int64_t *bytes)
{
	...
	h = iptc_init("nat");
	if (!h) {
		LOG(LOG_ERR, "get_redirect_rule_by_index() : "
				"iptc_init() failed : %s", iptc_strerror(errno));
		return -1;
	}
	if (!iptc_is_chain(miniupnpd_nat_chain, h)) {
		LOG(LOG_ERR, "chain %s not found", miniupnpd_nat_chain);
	} else {
#ifdef IPTABLES_143
		...
		u_int64_t *packets, u_int64_t *bytes)
{
	...
	h = iptc_init("nat");
	if (!h) {
		LOG(LOG_ERR, "get_peer_rule_by_index() : "
				"iptc_init() failed : %s", iptc_strerror(errno));
		return -1;
	}
	if (!iptc_is_chain(miniupnpd_peer_chain, h)) {
		LOG(LOG_ERR, "chain %s not found", miniupnpd_peer_chain);
	} else {
#ifdef IPTABLES_143
...
static int delete_rule_and_commit(unsigned int index, IPTC_HANDLE h,
		const char *miniupnpd_chain, const char *logcaller)
{
	int r = 0;
#ifdef IPTABLES_143
...
#endif
	{
		LOG(LOG_ERR, "%s() : iptc_delete_num_entry(): %s\n",
...
int delete_redirect_and_filter_rules(unsigned short eport, int proto)
{
	...
	h = iptc_init("nat");
	if (!h) {
		LOG(LOG_ERR, "delete_redirect_and_filter_rules() : "
				"iptc_init() failed : %s",
				iptc_strerror(errno));
		return -1;
	}
...
#endif
	if (r == 0) {
		LOG(LOG_INFO, "Trying to delete nat rule at index %u", index);
		...
		if ((r == 0) && (h = iptc_init("filter"))) {
...
#endif
			{
				if (proto == e->ip.proto) {
					match = (const struct ipt_entry_match *)&e->elems;
					/*LOG(LOG_DEBUG, "filter rule #%u: %s %s",
					...
					if (iaddr != e->ip.dst.s_addr)
						continue;
					index = i;
					LOG(LOG_INFO, "Trying to delete filter rule at index %u", index);
	...
	if ((h = iptc_init("nat"))) {
...
#endif
		{
			if (proto == e->ip.proto) {
				...
				index = i;
				LOG(LOG_INFO, "Trying to delete peer rule at index %u", index);
...
#endif
		{
			if (proto == e->ip.proto) {
				...
				if (iaddr != e->ip.src.s_addr)
					continue;
				index = i;
				LOG(LOG_INFO, "Trying to delete dscp rule at index %u", index);
...
static int iptc_init_verify_and_append(const char *table,
		...
		const char *logcaller)
{
	...
	h = iptc_init(table);
	if (!h) {
		LOG(LOG_ERR, "%s : iptc_init() error : %s\n",
				logcaller, iptc_strerror(errno));
		return -1;
	}
	if (!iptc_is_chain(miniupnpd_chain, h)) {
		LOG(LOG_ERR, "%s : chain %s not found",
...
#endif
	{
		LOG(LOG_ERR, "%s : iptc_append_entry() error : %s\n",
				logcaller, iptc_strerror(errno));
		if (h)
#ifdef IPTABLES_143
			iptc_free(h);
...
#endif
	{
		LOG(LOG_ERR, "%s : iptc_commit() error : %s\n",
				logcaller, iptc_strerror(errno));
		if (h)
#ifdef IPTABLES_143
			iptc_free(h);
		...
		const char *rhost)
{
	...
	if (!e) {
		LOG(LOG_ERR, "%s: calloc(%d) error", "addnatrule",
	...
	if (!tmp) {
		LOG(LOG_ERR, "%s: realloc(%d) error", "addnatrule",
		...
		const char *rhost, unsigned short rport)
{
	...
	e = calloc(1, sizeof(struct ipt_entry));
	if (!e) {
		LOG(LOG_ERR, "%s: calloc(%d) error", "addpeernatrule",
				(int)sizeof(struct ipt_entry));
		return -1;
	}
	e->ip.proto = proto;
	...
	tmp = realloc(e, sizeof(struct ipt_entry)
				   ...
				   + target->u.target_size);
	if (!tmp) {
		LOG(LOG_ERR, "%s: realloc(%d) error", "addpeernatrule",
				(int)(sizeof(struct ipt_entry) + match->u.match_size + target->u.target_size));
		free(e);
		free(match);
		free(target);
		...
		const char *rhost, unsigned short rport)
{
	...
	e = calloc(1, sizeof(struct ipt_entry));
	if (!e) {
		LOG(LOG_ERR, "%s: calloc(%d) error", "addpeerdscprule",
		       (int)sizeof(struct ipt_entry));
		return -1;
	}
	e->ip.proto = proto;
	...
	tmp = realloc(e, sizeof(struct ipt_entry)
				   ...
				   + target->u.target_size);
	if (!tmp) {
		LOG(LOG_ERR, "%s: realloc(%d) error", "addpeerdscprule",
				(int)(sizeof(struct ipt_entry) + match->u.match_size + target->u.target_size));
		free(e);
		free(match);
		free(target);
		...
		const char *iaddr, unsigned short iport)
{
	...
	e = calloc(1, sizeof(struct ipt_entry));
	if (!e) {
		LOG(LOG_ERR, "%s: calloc(%d) error", "add_filter_rule",
		       (int)sizeof(struct ipt_entry));
		return -1;
	}
	e->ip.proto = proto;
	...
	tmp = realloc(e, sizeof(struct ipt_entry)
				   ...
				   + target->u.target_size);
	if (!tmp) {
		LOG(LOG_ERR, "%s: realloc(%d) error", "add_filter_rule",
				(int)(sizeof(struct ipt_entry) + match->u.match_size + target->u.target_size));
		free(e);
		free(match);
		free(target);
...
unsigned short *get_portmappings_in_range(unsigned short startport, unsigned short endport,
		int proto, unsigned int *number)
{
	...
	*number = 0;
	...
	if (!array) {
		LOG(LOG_ERR, "get_portmappings_in_range() : calloc error");
		return NULL;
	}
			...
			if (proto == e->ip.proto) {
				...
				if (startport <= eport && eport <= endport) {
					if (*number >= capacity) {
						...
						if (!tmp) {
							LOG(LOG_ERR, "get_portmappings_in_range() : realloc(%u) error",
...
/* ================================ */
#ifdef DEBUG
static int print_match(const struct ipt_entry_match *match)
{
	printf("match %s\n", match->u.user.name);
	if (!strncmp(match->u.user.name, "tcp", IPT_FUNCTION_MAXNAMELEN)) {
		struct ipt_tcp *tcpinfo;
	...
	} else if (!strncmp(match->u.user.name, "udp", IPT_FUNCTION_MAXNAMELEN)) {
		struct ipt_udp *udpinfo;
		udpinfo = (struct ipt_udp *)match->data;
		printf("srcport = %hu:%hu dstport = %hu:%hu\n",
...
static void print_iface(const char *iface, const unsigned char *mask, int invert)
{
	unsigned i;
	if (mask[0] == 0)
		return;
