...
extern int leased_ip(struct in_addr ip);
#endif
...
static void BuildSendAndCloseSoapResp(struct upnphttp *h, const char *body, int bodylen)
{
	...
	int r = BuildHeader_upnphttp(h, 200, "OK", sizeof(beforebody) - 1 + sizeof(afterbody) - 1 + bodylen);
...
static void GetConnectionTypeInfo(struct upnphttp *h, const char *action)
{
...
static void GetTotalBytesSent(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
...
static void GetTotalBytesReceived(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
...
static void GetTotalPacketsSent(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
...
static void GetTotalPacketsReceived(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
...
static void GetCommonLinkProperties(struct upnphttp *h, const char *action)
{
	...
	bodylen = snprintf(body, sizeof(body), resp, action,
			"urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1",
			wan_access_type, upstream_bitrate, downstream_bitrate, status, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}

static void GetStatusInfo(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	...
	uptime = (getuptime(NULL) - startup_time);
	bodylen = snprintf(body, sizeof(body), resp, action,
			SERVICE_TYPE_WANIPC, status, (long)uptime, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}

static void GetNATRSIPStatus(struct upnphttp *h, const char *action)
{
	static const char resp[] =
...
static void GetExternalIPAddress(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	...
	if (use_ext_ip_addr) {
		...
		LOG(LOG_ERR, "Failed to get ip address for interface %s", ext_if_name);
	...
	for (lan_addr = lan_addrs.lh_first; lan_addr != NULL; lan_addr = lan_addr->list.le_next) {
		if ((h->clientaddr.s_addr & lan_addr->mask.s_addr) == (lan_addr->addr.s_addr & lan_addr->mask.s_addr)) {
			strncpy(ext_ip_addr, lan_addr->ext_ip_str, INET_ADDRSTRLEN);
			break;
		}
	}
#endif
	bodylen = snprintf(body, sizeof(body), resp, action, SERVICE_TYPE_WANIPC, ext_ip_addr, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}

/* AddPortMapping method of WANIPConnection Service
 * Ignored argument : NewEnabled */
static void AddPortMapping(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
	...
	if (inet_pton(AF_INET, int_ip, &result_ip) <= 0) {
		...
		if (hp && hp->h_addrtype == AF_INET) {
			...
			LOG(LOG_ERR, "Failed to convert hostname '%s' to ip address", int_ip);
			ClearNameValueList(&data);
			SoapError(h, 402, "Invalid Args");
			return;
		}
	...
	if (GETFLAG(SECUREMODEMASK)) {
		if (h->clientaddr.s_addr != result_ip.s_addr) {
			LOG(LOG_INFO, "Client %s tried to redirect port to %s",
	...
	LOG(LOG_INFO, "%s: ext port %hu to %s:%hu protocol %s for: %s leaseduration=%u rhost=%s",
			action, eport, int_ip, iport, protocol, desc, leaseduration, r_host ? r_host : "NULL");
	...
	*/
#ifdef LEASE_WRITE_THRU
	if (leased_only && !leased_ip(h->clientaddr))
		r = -3;
	else
#endif
...
static void AddAnyPortMapping(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
	...
	switch (r) {
		case 0:	/* success */
			bodylen = snprintf(body, sizeof(body), resp, action, SERVICE_TYPE_WANIPC, eport, action);
			BuildSendAndCloseSoapResp(h, body, bodylen);
			break;
		case -2:	/* already redirected */
			SoapError(h, 718, "ConflictInMappingEntry");
...
static void GetSpecificPortMappingEntry(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
	 ...
	 * a priority. */
	r = upnp_get_redirection_infos(eport, protocol, &iport, int_ip, sizeof(int_ip),
			desc, sizeof(desc), NULL, 0, &leaseduration);

	if (r < 0) {
		SoapError(h, 714, "NoSuchEntryInArray");
	} else {
		LOG(LOG_INFO, "%s: rhost='%s' %s %s found => %s:%u desc='%s'",
				action, r_host ? r_host : "NULL", ext_port, protocol, int_ip,
				(unsigned int)iport, desc);
		bodylen = snprintf(body, sizeof(body), resp, action, SERVICE_TYPE_WANIPC,
				(unsigned int)iport, int_ip, desc, leaseduration, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
	}
...
static void DeletePortMapping(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
	...
	LOG(LOG_INFO, "%s: external port: %hu, protocol: %s", action, eport, protocol);
...
static void DeletePortMappingRange(struct upnphttp *h, const char *action)
{
	int r = -1;
	static const char resp[] =
	...
	LOG(LOG_INFO, "%s: deleting external ports: %hu-%hu, protocol: %s",
			action, startport, endport, protocol);

	port_list = upnp_get_portmappings_in_range(startport, endport, protocol, &number);
	if (number == 0) {
	...
	for (i = 0; i < number; i++) {
		...
		LOG(LOG_INFO, "%s: deleting external port: %hu, protocol: %s: %s",
...
GetGenericPortMappingEntry(struct upnphttp *h, const char *action)
{
	...
	if ((errno == ERANGE && (index == LONG_MAX || index == LONG_MIN))
		...
		if (m_index == endptr)
			LOG(LOG_WARNING, "%s: no digits were found in <%s>",
					"GetGenericPortMappingEntry", "NewPortMappingIndex");
		else
			LOG(LOG_WARNING, "%s: strtol('%s'): %m",
	...
	LOG(LOG_INFO, "%s: index=%d", action, (int)index);

	rhost[0] = '\0';
	r = upnp_get_redirection_infos_by_index((int)index, &eport, protocol, &iport,
			iaddr, sizeof(iaddr), desc, sizeof(desc), rhost, sizeof(rhost), &leaseduration);
	...
	if (r < 0) {
		...
		int bodylen;
		char body[2048];
		bodylen = snprintf(body, sizeof(body), resp, action, SERVICE_TYPE_WANIPC, rhost,
				(unsigned int)eport, protocol, (unsigned int)iport, iaddr, desc, leaseduration, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
	}
...
static void GetListOfPortMappings(struct upnphttp *h, const char *action)
{
	...
	if (number == 0)
		number = 1000;	/* return up to 1000 mappings by default */
...
*/
	...
	bodylen = snprintf(body, bodyalloc, resp_start, action, SERVICE_TYPE_WANIPC);
	if (bodylen < 0) {
		SoapError(h, 501, "ActionFailed");
		free(body);
		return;
	...
	port_list = upnp_get_portmappings_in_range(startport, endport, protocol, &list_size);
	...
	for (i = 0; number > 0 && i < list_size; i++) {
		...
		if ((unsigned int)bodylen + 1024 > bodyalloc) {
			...
			if (!body) {
				LOG(LOG_CRIT, "realloc(%p, %u) FAILED", body_sav, (unsigned)bodyalloc);
				ClearNameValueList(&data);
				SoapError(h, 501, "ActionFailed");
				free(body_sav);
				free(port_list);
		...
		rhost[0] = '\0';
		r = upnp_get_redirection_infos(port_list[i], protocol, &iport, int_ip, sizeof(int_ip),
				desc, sizeof(desc), rhost, sizeof(rhost), &leaseduration);
		if (r == 0) {
	...
	bodylen += snprintf(body+bodylen, bodyalloc-bodylen, resp_end, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
	free(body);
...
static void SetDefaultConnectionService(struct upnphttp *h, const char *action)
{
	static const char resp[] =
...
#endif
		{
			LOG(LOG_INFO, "%s(%s) : Ignored", action, p);
			BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1);
		}
	} else {
		/* missing argument */
...
static void GetDefaultConnectionService(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"urn:schemas-upnp-org:service:Layer3Forwarding:1\">"
	...
	bodylen = snprintf(body, sizeof(body), resp, action, uuidvalue_wcd, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}
#endif

/* Added for compliance with WANIPConnection v2 */
static void SetConnectionType(struct upnphttp *h, const char *action)
{
#ifdef UPNP_STRICT
...
*/
static void QueryStateVariable(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	...
	/*LOG(LOG_INFO, "QueryStateVariable(%.40s)", var_name); */
	...
	if (!var_name) {
		...
		status = get_wan_connection_status_str(ext_if_name);
		bodylen = snprintf(body, sizeof(body), resp, action, "urn:schemas-upnp-org:control-1-0",
				status, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
	}
#if 0
	/* not usefull */
		...
		char strn[10];
		snprintf(strn, sizeof(strn), "%i", upnp_get_portmapping_number_of_entries());
		bodylen = snprintf(body, sizeof(body), resp, action,
				"urn:schemas-upnp-org:control-1-0", strn, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
	} else {
		LOG(LOG_NOTICE, "%s: Unknown: %s", action, var_name?var_name:"");
...
static void GetFirewallStatus(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
...
static int CheckStatus(struct upnphttp *h)
{
...
static int PinholeVerification(struct upnphttp *h, char *int_ip, unsigned short int_port)
{
	int n;
	...
	LOG(LOG_INFO, "Checking internal IP@ and port (Security policy purpose)");
	...
	if (strcmp(senderAddr, int_ip) != 0)
		if (h->clientaddr_v6.s6_addr != result_ip.s6_addr) {
			LOG(LOG_INFO, "Client %s tried to access pinhole for internal %s and is not authorized to do it",
					senderAddr, int_ip);
			SoapError(h, 606, "Action not authorized");
			return 0;
		}
	...
	if (int_port < 1024) {
		LOG(LOG_INFO, "Client %s tried to access pinhole with port < 1024 and is not authorized to do it",
				senderAddr);
		SoapError(h, 606, "Action not authorized");
		return 0;
	}
...
static void AddPinhole(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
		...
		if (err == 0) {
			...
			for (p = ai; p; p = p->ai_next) {
				if (p->ai_family == AF_INET6) {
					inet_ntop(AF_INET6, &(((struct sockaddr_in6 *)p->ai_addr)->sin6_addr), rem_ip, sizeof(rem_ip));
					LOG(LOG_INFO, "resolved '%s' to '%s'", rem_host, rem_ip);
					rem_host = rem_ip;
					break;
				}
			}
			...
			LOG(LOG_WARNING, "AddPinhole : getaddrinfo(%s) : %s", rem_host, gai_strerror(err));
#if 0
			SoapError(h, 402, "Invalid Args");
			goto clear_and_exit;
#endif
	...
	if (ltime < 1 || ltime > 86400) {
		LOG(LOG_WARNING, "%s: LeaseTime=%d not supported, (ip=%s)", action, ltime, int_ip);
		SoapError(h, 402, "Invalid Args");
		goto clear_and_exit;
	}
	...
	LOG(LOG_INFO, "%s: (inbound) from [%s]:%hu to [%s]:%hu with proto %ld during %d sec",
			action, rem_host?rem_host:"any", rport, int_ip, iport, proto, ltime);
	...
	switch (r) {
		case 1:	        /* success */
			bodylen = snprintf(body, sizeof(body), resp, action,
					"urn:schemas-upnp-org:service:WANIPv6FirewallControl:1", uid, action);
			BuildSendAndCloseSoapResp(h, body, bodylen);
			break;
...
static void UpdatePinhole(struct upnphttp *h, const char *action)
{
	static const char resp[] =
	...
	n = upnp_get_pinhole_info(uid, NULL, 0, NULL, iaddr, sizeof(iaddr), &iport,
			NULL, /* proto */
			NULL, 0, /* desc, desclen */
			NULL, NULL);
	if (n >= 0) {
	...
	LOG(LOG_INFO, "%s: (inbound) updating lease duration to %d for pinhole with ID: %d", action, ltime, uid);
...
static void GetOutboundPinholeTimeout(struct upnphttp *h, const char *action)
{
	int r;
	static const char resp[] =
		"<u:%sResponse "
	...
	LOG(LOG_INFO, "%s: retrieving timeout for outbound pinhole from [%s]:%hu to [%s]:%hu protocol %s", action, int_ip, iport,rem_host, rport, protocol);
	...
	switch (r) {
		case 1:	/* success */
			bodylen = snprintf(body, sizeof(body), resp, action,
					"urn:schemas-upnp-org:service:WANIPv6FirewallControl:1", opt, action);
			BuildSendAndCloseSoapResp(h, body, bodylen);
			break;
...
static void DeletePinhole(struct upnphttp *h, const char *action)
{
	int n;
	static const char resp[] =
	 ...
	 * it doesn't have access to, because of its public access */
	n = upnp_get_pinhole_info(uid, NULL, 0, NULL, iaddr, sizeof(iaddr), &iport,
			&proto, NULL, 0, /* desc, desclen */ &leasetime, NULL);
	if (n >= 0) {
		if (PinholeVerification(h, iaddr, iport) <= 0)
			return;
	} else if (n == -2) {
	...
	if (n < 0) {
		LOG(LOG_INFO, "%s: (inbound) failed to remove pinhole with ID: %d", action, uid);
		SoapError(h, 501, "ActionFailed");
		return;
	}
	LOG(LOG_INFO, "%s: (inbound) pinhole with ID %d successfully removed", action, uid);
	BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1);
}

static void CheckPinholeWorking(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	 ...
	 * it doesn't have access to, because of its public access */
	r = upnp_get_pinhole_info(uid, NULL, 0, NULL, iaddr, sizeof(iaddr), &iport,
			NULL, /* proto */
			NULL, 0, /* desc, desclen */
			NULL, &packets);
	if (r >= 0) {
		...
		bodylen = snprintf(body, sizeof(body), resp, action,
				"urn:schemas-upnp-org:service:WANIPv6FirewallControl:1", 1, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
	} else if (r == -2)
		SoapError(h, 704, "NoSuchEntry");
	else
...
static void GetPinholePackets(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	 ...
	 * it doesn't have access to, because of its public access */
	n = upnp_get_pinhole_info(uid, NULL, 0, NULL, iaddr, sizeof(iaddr), &iport,
			&proto, NULL, 0, /* desc, desclen */&leasetime, &packets);
	if (n >= 0) {
		if (PinholeVerification(h, iaddr, iport)<=0)
			return ;
	}
...
static void SendSetupMessage(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	...
	bodylen = snprintf(body, sizeof(body), resp, action,
			"urn:schemas-upnp-org:service:DeviceProtection:1", OutMessage, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
	ClearNameValueList(&data);
}

static void GetSupportedProtocols(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	...
	bodylen = snprintf(body, sizeof(body), resp, action,
			"urn:schemas-upnp-org:service:DeviceProtection:1", ProtocolList, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}

static void GetAssignedRoles(struct upnphttp *h, const char *action)
{
	static const char resp[] =
		"<u:%sResponse "
		"xmlns:u=\"%s\">"
	...
	bodylen = snprintf(body, sizeof(body), resp, action,
			"urn:schemas-upnp-org:service:DeviceProtection:1", RoleList, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}
#endif
...
#ifdef DEBUG
				DEBUG("Remote Call of SoapMethod '%s'", soapMethods[i].methodName);
#endif /* DEBUG */
				soapMethods[i].methodImpl(h, soapMethods[i].methodName);
				return;
			}
		}
		LOG(LOG_NOTICE, "SoapMethod: Unknown: %.*s", methodlen, p);
	} else {
		LOG(LOG_NOTICE, "cannot parse SoapAction");
	}
...
void SoapError(struct upnphttp *h, int errCode, const char *errDesc)
{
	...
	LOG(LOG_INFO, "Returning UPnPError %d: %s", errCode, errDesc);
