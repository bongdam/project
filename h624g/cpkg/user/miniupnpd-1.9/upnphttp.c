...
syslogsslerr(void)
{
	...
	while ((err = ERR_get_error()) != 0) {
		LOG(LOG_ERR, "%s", ERR_error_string(err, buffer));
	}
}

static int verify_callback(int preverify_ok, X509_STORE_CTX *ctx)
{
	DEBUG("verify_callback(%d, %p)", preverify_ok, ctx);
	return preverify_ok;
}
...
int init_ssl(void)
{
	...
	if (method == NULL) {
		LOG(LOG_ERR, "TLSv1_server_method() failed");
		syslogsslerr();
		return -1;
	}
	ssl_ctx = SSL_CTX_new(method);
	if (ssl_ctx == NULL) {
		LOG(LOG_ERR, "SSL_CTX_new() failed");
		syslogsslerr();
		return -1;
	}
	/* set the local certificate */
	if (!SSL_CTX_use_certificate_file(ssl_ctx, HTTPS_CERTFILE, SSL_FILETYPE_PEM)) {
		LOG(LOG_ERR, "SSL_CTX_use_certificate_file(%s) failed", HTTPS_CERTFILE);
		syslogsslerr();
		return -1;
	}
	/* set the private key */
	if (!SSL_CTX_use_PrivateKey_file(ssl_ctx, HTTPS_KEYFILE, SSL_FILETYPE_PEM)) {
		LOG(LOG_ERR, "SSL_CTX_use_PrivateKey_file(%s) failed", HTTPS_KEYFILE);
		syslogsslerr();
		return -1;
	}
	/* verify private key */
	if (!SSL_CTX_check_private_key(ssl_ctx)) {
		LOG(LOG_ERR, "SSL_CTX_check_private_key() failed");
		syslogsslerr();
		return -1;
	}
	...
	LOG(LOG_INFO, "using %s", SSLeay_version(SSLEAY_VERSION));
	return 0;
}
...
struct upnphttp *New_upnphttp(int s)
{
	...
	if (!set_non_blocking(s))
		LOG(LOG_WARNING, "New_upnphttp::set_non_blocking(): %m");
	return ret;
}

#ifdef ENABLE_HTTPS
void InitSSL_upnphttp(struct upnphttp *h)
{
	int r;
	h->ssl = SSL_new(ssl_ctx);
	if (h->ssl == NULL) {
		LOG(LOG_ERR, "SSL_new() failed");
		syslogsslerr();
		abort();
	}
	if (!SSL_set_fd(h->ssl, h->socket)) {
		LOG(LOG_ERR, "SSL_set_fd() failed");
		syslogsslerr();
		abort();
	}
	...
	if (r < 0) {
		...
		DEBUG("SSL_accept() returned %d, SSL_get_error() %d", r, err);
		if (err != SSL_ERROR_WANT_READ && err != SSL_ERROR_WANT_WRITE) {
			LOG(LOG_ERR, "SSL_accept() failed");
			syslogsslerr();
			abort();
		}
	}
...
void CloseSocket_upnphttp(struct upnphttp *h)
{
	/* SSL_shutdown() ? */
	if (close(h->socket) < 0)
		LOG(LOG_ERR, "CloseSocket_upnphttp: close(%d): %m", h->socket);
	h->socket = -1;
	h->state = EToDelete;
}
		...
		if (colon) {
			if (strncasecmp(line, "Content-Length:", 15) == 0) {
				...
				if (h->req_contentlen < 0) {
					LOG(LOG_WARNING, "ParseHttpHeaders() invalid Content-Length %d", h->req_contentlen);
					h->req_contentlen = 0;
				}
				...
				while (p[n]>=' ')
					n++;
				DEBUG("accept-language HTTP header : '%.*s'", n, p);
				...
				if (strncasecmp(p, "100-continue", 12) == 0) {
					...
					DEBUG("\"Expect: 100-Continue\" header detected");
				}
			}
#ifdef ENABLE_EVENTS
...
static void Send405(struct upnphttp *h)
{
	...
	h->respflags |= FLAG_HTML;
	BuildResp2_upnphttp(h, 405, "Method Not Allowed", body405, sizeof(body405) - 1);
	SendRespAndClose_upnphttp(h);
}
...
static void sendXMLdesc(struct upnphttp *h, char *(f)(int *))
{
	...
	if (!desc) {
		...
		LOG(LOG_ERR, "Failed to generate XML description");
		h->respflags = FLAG_HTML;
		BuildResp2_upnphttp(h, 500, "Internal Server Error", error500, sizeof(error500)-1);
	} else {
...
static void ProcessHTTPPOST_upnphttp(struct upnphttp *h)
{
	if ((h->req_buflen - h->req_contentoff) >= h->req_contentlen) {
		...
		if (h->req_soapActionOff > 0) {
			...
			LOG(LOG_INFO, "SOAPAction: %.*s",
					h->req_soapActionLen, h->req_buf + h->req_soapActionOff);
			ExecuteSoapAction(h, h->req_buf + h->req_soapActionOff, h->req_soapActionLen);
		} else {
			static const char err400str[] =
				"<html><body>Bad request</body></html>";
			LOG(LOG_INFO, "No SOAPAction in HTTP headers");
			h->respflags = FLAG_HTML;
			BuildResp2_upnphttp(h, 400, "Bad Request", err400str, sizeof(err400str) - 1);
			SendRespAndClose_upnphttp(h);
		}
		...
		h->res_buflen = snprintf(h->res_buf, h->res_buf_alloclen, "%s 100 Continue\r\n\r\n", h->HttpVer);
		h->res_sent = 0;
 ...
 * 1 if it is valid.
 */
static int checkCallbackURL(struct upnphttp *h)
{
	char addrstr[48];
	int ipv6;
	const char *p;
...
start_again:
	...
	if (ipv6) {
		...
		if (inet_pton(AF_INET6, addrstr, &addr) <= 0)
			goto invalid;
		if (!h->ipv6 || (0!=memcmp(&addr, &(h->clientaddr_v6), sizeof(struct in6_addr))))
			goto invalid;
#else
		goto invalid;
#endif
...
{
	const char *sid;
	DEBUG("ProcessHTTPSubscribe %s", path);
	DEBUG("Callback '%.*s' Timeout=%d",
			h->req_CallbackLen, h->req_buf + h->req_CallbackOff,
			h->req_Timeout);
	DEBUG("SID '%.*s'", h->req_SIDLen, h->req_buf + h->req_SIDOff);
	if ((h->req_CallbackOff <= 0) && (h->req_SIDOff <= 0)) {
		...
		if (h->req_CallbackOff > 0) {
			...
			if (h->req_SIDOff > 0) {
				LOG(LOG_WARNING, "Both Callback: and SID: in SUBSCRIBE");
			...
			/* "NT: upnp:event" header is mandatory */
				...
				LOG(LOG_WARNING, "Invalid NT in SUBSCRIBE %.*s",
			...
			if (checkCallbackURL(h)) {
				...
				if (sid) {
					DEBUG("generated sid=%s", sid);
					h->respflags |= FLAG_SID;
					h->res_SID = sid;
				}
				BuildResp_upnphttp(h, 0, 0);
			} else {
				LOG(LOG_WARNING, "Invalid Callback in SUBSCRIBE %.*s",
						h->req_CallbackLen, h->req_buf + h->req_CallbackOff);
				BuildResp2_upnphttp(h, 412, "Precondition Failed", 0, 0);
			}
		} else {
...
with HTTP error 412 Precondition Failed. */
			...
			if (h->req_NTOff > 0) {
				LOG(LOG_WARNING, "Both NT: and SID: in SUBSCRIBE");
				BuildResp2_upnphttp(h, 400, "Incompatible header fields", 0, 0);
			} else
#endif
...
{
	DEBUG("ProcessHTTPUnSubscribe %s", path);
	DEBUG("SID '%.*s'", h->req_SIDLen, h->req_buf + h->req_SIDOff);
...
static void ProcessHttpQuery_upnphttp(struct upnphttp *h)
{
	...
	for (i = 0; i<15 && *p != '\r'; i++)
		HttpVer[i] = *(p++);
	HttpVer[i] = '\0';
	LOG(LOG_INFO, "HTTP REQUEST : %s %s (%s)", HttpCommand, HttpUrl, HttpVer);
	ParseHttpHeaders(h);
	if (h->req_HostOff > 0 && h->req_HostLen > 0) {
		DEBUG("Host: %.*s", h->req_HostLen, h->req_buf + h->req_HostOff);
		...
		if (*p == '[') {
			...
			if (*p != ']') {
				LOG(LOG_NOTICE, "DNS rebinding attack suspected (Host: %.*s)", h->req_HostLen, h->req_buf + h->req_HostOff);
				Send404(h);/* 403 */
				return;
			}
			p++;
	...
	if (strcmp("POST", HttpCommand) == 0) {
...
#endif
		LOG(LOG_NOTICE, "%s not found, responding ERROR 404", HttpUrl);
		Send404(h);
	}
#ifdef ENABLE_EVENTS
		...
		LOG(LOG_NOTICE, "SUBSCRIBE not implemented. ENABLE_EVENTS compile option disabled");
		Send501(h);
	}
#endif
	else {
		LOG(LOG_NOTICE, "Unsupported HTTP Command %s", HttpCommand);
		Send501(h);
	}
}
	...
	switch (h->state) {
		case EWaitingForHttpRequest:
			...
			/* if errno is EAGAIN, EWOULDBLOCK or EINTR, try again later */
				...
				LOG(LOG_WARNING, "HTTP Connection closed unexpectedly");
				h->state = EToDelete;
			} else {
				...
				if (h_tmp == NULL) {
					LOG(LOG_WARNING, "Unable to allocate new memory for h->req_buf)");
					h->state = EToDelete;
				} else {
					h->req_buf = h_tmp;
		...
		case EWaitingForHttpContent:
			...
			if (n < 0) {
				...
				LOG(LOG_WARNING, "HTTP Connection closed inexpectedly");
				h->state = EToDelete;
			} else {
				void *tmp = realloc(h->req_buf, n + h->req_buflen);
				if (!tmp) {
					LOG(LOG_ERR, "memory allocation error %m");
					h->state = EToDelete;
				} else {
					h->req_buf = tmp;
					memcpy(h->req_buf + h->req_buflen, buf, n);
		...
		case ESendingAndClosing:
			...
			break;
		default:
			LOG(LOG_WARNING, "Unexpected state: %d", h->state);
	}
}
...
int BuildHeader_upnphttp(struct upnphttp *h, int respcode, const char *respmsg, int bodylen)
{
	...
	if (!h->res_buf || h->res_buf_alloclen < ((int)sizeof(httpresphead) + 256 + bodylen)) {
		if (h->res_buf)
			free(h->res_buf);
		templen = sizeof(httpresphead) + 256 + bodylen;
		h->res_buf = (char *)malloc(templen);
		if (!h->res_buf) {
			LOG(LOG_ERR, "malloc error in BuildHeader_upnphttp()");
			return -1;
		}
...
#ifdef ENABLE_HTTP_DATE
	{
		...
		strftime(http_date, sizeof(http_date), "%a, %d %b %Y %H:%M:%S GMT", &tm);
		h->res_buflen += snprintf(h->res_buf + h->res_buflen,
				h->res_buf_alloclen - h->res_buflen, "Date: %s\r\n", http_date);
	}
#endif
#ifdef ENABLE_EVENTS
	if (h->respflags & FLAG_TIMEOUT) {
		h->res_buflen += snprintf(h->res_buf + h->res_buflen,
				h->res_buf_alloclen - h->res_buflen, "Timeout: Second-");
		if (h->req_Timeout) {
			h->res_buflen += snprintf(h->res_buf + h->res_buflen,
					h->res_buf_alloclen - h->res_buflen, "%d\r\n", h->req_Timeout);
		} else {
			h->res_buflen += snprintf(h->res_buf + h->res_buflen,
					h->res_buf_alloclen - h->res_buflen, "infinite\r\n");
		}
	}
	if (h->respflags & FLAG_SID) {
		h->res_buflen += snprintf(h->res_buf + h->res_buflen,
				h->res_buf_alloclen - h->res_buflen, "SID: %s\r\n", h->res_SID);
	}
#endif
	if (h->respflags & FLAG_ALLOW_POST) {
		h->res_buflen += snprintf(h->res_buf + h->res_buflen,
				h->res_buf_alloclen - h->res_buflen, "Allow: %s\r\n", "POST");
	...
	if (h->accept_language[0] != '\0') {
		/* defaulting to "en" */
		h->res_buflen += snprintf(h->res_buf + h->res_buflen,
				h->res_buf_alloclen - h->res_buflen, "Content-Language: %s\r\n",
	...
	if (h->res_buf_alloclen < (h->res_buflen + bodylen)) {
		...
		if (tmp) {
			...
			LOG(LOG_ERR, "realloc error in BuildHeader_upnphttp()");
			return -1;
		}
	}
	return 0;
...
int SendResp_upnphttp(struct upnphttp *h)
{
	...
	while (h->res_sent < h->res_buflen) {
		...
		if (n < 0) {
			...
			LOG(LOG_ERR, "send(res_buf): %m");
			break; /* avoid infinite loop */
#ifdef ENABLE_HTTPS
			}
#endif
		} else if (n == 0) {
			LOG(LOG_ERR, "send(res_buf): %d bytes sent (out of %d)",
