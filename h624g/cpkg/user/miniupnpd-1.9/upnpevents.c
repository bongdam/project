...
static struct subscriber *newSubscriber(const char *eventurl, const char *callback, int callbacklen)
{
		...
		const char *callback, int callbacklen, int timeout)
{
	...
	if (!tmp)
		return NULL;
	if (timeout)
		tmp->timeout = getuptime(NULL) + timeout;
...
int renewSubscription(const char *sid, int sidlen, int timeout)
{
	...
	for (sub = subscriberlist.lh_first; sub != NULL; sub = sub->entries.le_next) {
		if ((sidlen == 41) && (memcmp(sid, sub->uuid, 41) == 0)) {
			...
			if (sub->timeout && getuptime(NULL) > sub->timeout)
				continue;
#endif
			sub->timeout = (timeout ? getuptime(NULL) + timeout : 0);
			return 0;
		}
	}
	return -1;
...
static void upnp_event_create_notify(struct subscriber *sub)
{
	...
	if (!obj) {
		LOG(LOG_ERR, "%s: calloc(): %m", "upnp_event_create_notify");
		return;
	}
	obj->sub = sub;
	obj->state = ECreated;
#ifdef ENABLE_IPV6
	obj->s = socket((obj->sub->callback[7] == '[') ? PF_INET6 : PF_INET, SOCK_STREAM, 0);
#else
	obj->s = socket(PF_INET, SOCK_STREAM, 0);
#endif
	if (obj->s < 0) {
		LOG(LOG_ERR, "%s: socket(): %m", "upnp_event_create_notify");
		goto error;
	}
	...
	if (setsockopt(obj->s, SOL_SOCKET, SO_RCVTIMEO, &sock_timeout, sizeof(struct timeval)) < 0) {
		LOG(LOG_WARNING, "%s: setsockopt(SO_RCVTIMEO): %m",
		       "upnp_event_create_notify");
	}
	sock_timeout.tv_sec = 3;
	sock_timeout.tv_usec = 0;
	if (setsockopt(obj->s, SOL_SOCKET, SO_SNDTIMEO, &sock_timeout, sizeof(struct timeval)) < 0) {
		LOG(LOG_WARNING, "%s: setsockopt(SO_SNDTIMEO): %m",
		       "upnp_event_create_notify");
	}
#endif
	/* set socket non blocking */
	if (!set_non_blocking(obj->s)) {
		LOG(LOG_ERR, "%s: set_non_blocking(): %m", "upnp_event_create_notify");
		goto error;
	}
	if (sub)
		sub->notify = obj;
...
static void upnp_event_notify_connect(struct upnp_event_notify *obj)
{
	...
	addrlen = sizeof(struct sockaddr_in);
#endif
	DEBUG("%s: '%s' %hu '%s'", "upnp_event_notify_connect",
	       obj->addrstr, port, obj->path);
	obj->state = EConnecting;
	if (connect(obj->s, (struct sockaddr *)&addr, addrlen) < 0) {
		if (errno != EINPROGRESS && errno != EWOULDBLOCK) {
			LOG(LOG_ERR, "%s: connect(%d, %s, %u): %m",
...
static void upnp_event_prepare(struct upnp_event_notify *obj)
{
	...
	if (!obj->buffer) {
		LOG(LOG_ERR, "%s: malloc returned NULL", "upnp_event_prepare");
		if (xml) {
			free(xml);
		}
		obj->state = EError;
...
static void upnp_event_send(struct upnp_event_notify *obj)
{
	...
	DEBUG("%s: sending event notify message to %s%s",
	...
	if (i < 0) {
		if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) {
			LOG(LOG_NOTICE, "%s: send(): %m", "upnp_event_send");
			obj->state = EError;
			return;
		} else {
	...
	if (i != (obj->tosend - obj->sent))
		LOG(LOG_NOTICE, "%s: %d bytes send out of %d",
...
static void upnp_event_recv(struct upnp_event_notify *obj)
{
	...
	if (n<0) {
		if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) {
			LOG(LOG_ERR, "%s: recv(): %m", "upnp_event_recv");
			obj->state = EError;
		}
		return;
	}
	DEBUG("%s: (%dbytes) %.*s", "upnp_event_recv", n, n, obj->buffer);
	...
	switch(obj->state) {
		case EConnecting:
			...
			if (getsockopt(obj->s, SOL_SOCKET, SO_ERROR, &err, &len) < 0) {
				LOG(LOG_ERR, "%s: getsockopt: %m", "upnp_event_process_notify");
				obj->state = EError;
				break;
			}
			if (err != 0) {
				errno = err;
				LOG(LOG_WARNING, "%s: connect(%s%s): %m",
		...
		case EFinished:
			...
			break;
		default:
			LOG(LOG_ERR, "%s: unknown state", "upnp_event_process_notify");
	}
}
...
//		DEBUG("upnpevents_selectfds: %p %d %d", obj, obj->state, obj->s);
		if (obj->s >= 0) {
			switch(obj->state) {
				case ECreated:
...
void upnpevents_processfds(fd_set *readset, fd_set *writeset)
{
	...
	for (obj = notifylist.lh_first; obj != NULL; obj = obj->entries.le_next) {
//		DEBUG("%s: %p %d %d %d %d", "upnpevents_processfds", obj, obj->state, obj->s,
