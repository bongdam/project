...
#define USAGE \
	...
	"  options:\n" \
	"    -d in|out  (direction)\n" \
	"    -a permit|drop|cpu|prio (action type)\n" \
	"    -o pktOpApp\n" \
	"    -r ruleType\n" \
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "d:a:o:r:m:n:p:P:u:i:s:y:v:34t:T:qM:c:bQ6C:L:H:")) != -1) {
		switch (opt) {
		case 'd':
			dir = optarg;
			break;
	...
	case RTL865X_ACL_TCP:
		...
		/* fall thru */
	case RTL865X_ACL_IP:
		R->srcIpAddr_ = sip.ip.ip;
		R->srcIpAddrMask_ = sip.mask.ip;
		R->dstIpAddr_ = dip.ip.ip;
		R->dstIpAddrMask_ = dip.mask.ip;
		R->tos_ = iptos_v;
		R->tosMask_ = iptos_m;
	...
	case RTL865X_ACL_SRCFILTER:
		...
		R->srcFilterIpAddr_ = sip.ip.ip;
		R->srcFilterIpAddrMask_ = sip.mask.ip;
	...
	case RTL865X_ACL_DSTFILTER:
		...
		R->dstFilterIpAddr_ = sip.ip.ip;
		R->dstFilterIpAddrMask_ = sip.mask.ip;
	...
	case RTL865X_ACL_IPV6:
		*((int *)&dvcmd.chain[0]) = RTL865X_ACL_IPV6_USED;
		memcpy(R->un_ty.L3V6._srcIpV6Addr.v6_addr, sip.ip.ip6, sizeof(inv6_addr_t));
		memcpy(R->un_ty.L3V6._srcIpV6AddrMask.v6_addr, sip.mask.ip6, sizeof(inv6_addr_t));
		memcpy(R->un_ty.L3V6._dstIpV6Addr.v6_addr, dip.ip.ip6, sizeof(inv6_addr_t));
		memcpy(R->un_ty.L3V6._dstIpV6AddrMask.v6_addr, dip.mask.ip6, sizeof(inv6_addr_t));
