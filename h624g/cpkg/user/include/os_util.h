...
#ifdef CONFIG_SMP
#define spinlock_enter(owner, lock, flgs, recursive, wait)	\
({								\
	int __curr_cpu, __cpu = smp_processor_id();		\
	bool __ret = false;					\
	do {							\
		__curr_cpu = cmpxchg(owner, -1, __cpu);		\
		if (__curr_cpu == -1) {				\
			spin_lock(lock);		\
			recursive = 1;				\
			__ret = true;				\
		} else if (__curr_cpu == __cpu) {		\
			++recursive;				\
			__ret = true;				\
		}						\
	} while (wait && __ret == false);			\
	__ret;							\
})

#define spinlock_exit(owner, lock, flgs, recursive)		\
({								\
	int __cpu = smp_processor_id();				\
	if (*owner == __cpu) {					\
		if (--recursive <= 0) {				\
			spin_unlock(lock);	\
			cmpxchg(owner, __cpu, -1);		\
		}						\
	} else							\
		pr_err("%s:%d: cpu%d can't release lock unowned\n",	\
			__func__, __LINE__, __cpu);		\
})
#endif	/* CONFIG_SMP */

/* drivers/net/rtl819x/custom/link.c */
extern void (*os_link_notifier)(void);
extern void setup_probe_link(void);
extern void del_probe_link(void);
