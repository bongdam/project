...
requested_ip_assignable(struct dhcpMessage *oldpacket, u_int32_t req_align)
{
	...
	/* if a lease exists with the same HW address buf has a different IP address
...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if (rc > 0)
		LOG(LOG_DEBUG, "DHCPD OFFER " L_SENTFOR " [" NQF "]", NIPQUAD(packet.yiaddr));
	return rc;
}
...
int sendNAK(struct dhcpMessage *oldpacket)
{
	...
	LOG(LOG_DEBUG, "DHCPD NAK " L_SENTFOR " [%s]", ether_ntoa(oldpacket->chaddr));
...
int sendACK(struct dhcpMessage *oldpacket, u_int32_t yiaddr)
{
	...
	LOG(LOG_DEBUG, "DHCPD ACK " L_SENTFOR " %s[" NQF "]",
