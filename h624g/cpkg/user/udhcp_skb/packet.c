	       ...
	       u_int32_t dest_ip, int dest_port, unsigned char *dest_arp, int ifindex)
{
	...
	// addon - option82
	if (client_config.snoop_request) {
		int sk_mark = 0;

		SNOOP_MARK_SET(sk_mark, SNOOP_UPLINK_PORT);
		SNOOP_MARK_SET(sk_mark, SNOOP_DROP_WLAN);
		setsockopt(fd, SOL_SOCKET, SO_MARK, &sk_mark, sizeof(int));
	}
