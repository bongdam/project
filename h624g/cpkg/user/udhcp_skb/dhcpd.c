...
int probe_server_main(const int, int, unsigned);
...
#endif
{
	...
	if (server_config.dhcp_protection > 0) {
		...
		if (probe_pid == 0) {
			...
			probe_server_main((server_config.probe_period < 30) ? \
					  ...
					  signal_pipe[1], server_config.dhcp_protection);
			exit_server(0);
		}
	}
	...
	while (1) {		/* loop until universe collapses */
		...
		switch (state[0]) {
		case DHCPDISCOVER:
			LOG(LOG_DEBUG, "DHCPD DISCOVER " L_RCVED2 " [%s]", ether_ntoa(packet.chaddr));
			sendOffer(&packet);
			break;
		case DHCPREQUEST:
			if (!requested_ip_opt) {
				LOG(LOG_DEBUG, "DHCPD REQUEST " L_RCVED2 " [%s]", ether_ntoa(packet.chaddr));
				...
				LOG(LOG_DEBUG, "DHCPD REQUEST " L_RCVED2 " [" NQF "]", NIPQUAD(requested_nip));
			}
...
// addon
			if (sdmz_host_match(packet.chaddr, packet.hlen)) {
				...
				f = fopen("/proc/sys/net/private/twin_inaddr", "r");
				if (f) {
...
//
			...
			break;
		case DHCPDECLINE:
			LOG(LOG_DEBUG, "DHCPD DECLINE " L_RCVED2 " [%s]", ether_ntoa(packet.chaddr));
			if (lease) {
			...
			break;
		case DHCPRELEASE:
			LOG(LOG_DEBUG, "DHCPD RELEASE " L_RCVED2 " [" NQF "]", NIPQUAD(packet.ciaddr));
			if (lease)
				lease->expires = monotonic_sec();
			break;
		case DHCPINFORM:
			LOG(LOG_DEBUG, "DHCPD INFORM " L_RCVED2 " [%s]", ether_ntoa(packet.chaddr));
			send_inform(&packet);
			break;
		default:
