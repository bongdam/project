...
#include <snoop_dhcp.h>
#include <brdio.h>
#include <libytool.h>

#define DEBUG_DHCPR 1
...
add_option_relay_agent(struct dhcpMessage *packet, u_int8_t *ifarp, int port)
{
	...
	int fd = open("/proc/brdio", O_RDWR);

	pvid = port;
	if (fd != -1) {
		ioctl(fd, PVIDGET, (char *)&pvid);
		close(fd);
	}
	circuit++; // option82 port range 1~4.
	...
	string[6] = (u_int8_t)(pvid >> 8);	//     + Vlan ID
	string[7] = (u_int8_t)pvid;
...
{
	yexecl("2>/dev/null", 
	       "aclwrite %s br0 -d in -a cpu -o 7 -r sfilter -p 67:68 -P 0x1f/0x1f",
	       (enable) ? "add" : "del");
	yexecl(NULL, "ifconfig %s %spromisc", ifname, (enable) ? "" : "-");
	yecho("/proc/sys/private/snoop_dhcp", "%d", !!(enable));
}
			...
			if (!(i = dhcpr_recv(&ll_packet, fd))) {
				...
				case DHCPDECLINE:
					...
					if (!add_relay_host(ll_packet.data.chaddr, port))
						...
						if (!memcmp(ll_packet.ether, ifhaddr, ETH_ALEN))
							memcpy(ll_packet.ether, MAC_BCAST_ADDR, ETH_ALEN);
						dhcpr_send(fd, &ll_packet, if_nametoindex("eth0"), mark);
					}
					break;
				case DHCPOFFER:
				case DHCPACK:
