...
#else				/* CONFIG_FEATURE_TELNETD_INETD */
make_new_session(int sockfd, int timeout)
#endif				/* CONFIG_FEATURE_TELNETD_INETD */
{
	...
	port = login_peer(sockfd, str, sizeof(str));
	...
	if (pid == 0) {
		...
		if ((loginpath = tlogin_main()) == NULL)
			exit(-20);
		if (str[0])
			syslog(LOG_INFO, "\xed\x85\x94\xeb\x84\xb7 "
			       "\xec\xa0\x91\xec\x86\x8d\xeb\x90\xa8 %s:%u",
			       str, ntohs(port));
		...
		if (FD_ISSET(master_fd, &rdfdset)) {
			...
			if ((fd = accept(master_fd, (struct sockaddr *)&sa,
				...
				/* Create a new session and link it into
				   our active list.  */
				struct tsession *new_ts = make_new_session(fd, timeout);
				if (new_ts) {
