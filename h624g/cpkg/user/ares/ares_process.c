...
static const char rcsid[] = "$Id: ares_process.c,v 1.2 2010-07-20 09:27:19 sjyoon Exp $";
...
static void fork_server(ares_channel channel, struct query *query, time_t now);
#ifdef SUPPORT_TCP
 ...
 */
int ares_process(ares_channel channel, fd_set *read_fds, fd_set *write_fds)
{
    int i;
    time_t now;
...
#ifdef LGDACOM_PROVIS   /* jjj, APR-101 additional provisioning requirement of LG-Dacom */
    for (i=0; i<channel->nservers; i++) {
        if (channel->servers[i].f_answer) {
            return (i);
        }
    }
#else
...
#ifdef LGDACOM_PROVIS   /* jjj, APR-101 additional provisioning requirement of LG-Dacom */
    channel->servers[whichserver].f_answer = 1;
	if ( rcode == NOERROR && DNS_HEADER_ANCOUNT(abuf) > 0 )
#endif
...
static void next_server(ares_channel channel, struct query *query, time_t now)
{
#ifdef LGDACOM_PROVIS
    if (channel->query_mode == 1) {
        for (; query->server < channel->nservers; query->server++) {
            for (; ++query->try < channel->tries; ) {
...
void ares__send_query(ares_channel channel, struct query *query, time_t now)
{
...
#endif
    {
        ...
        if (channel->query_mode != 0)
			query->timeout = now + channel->timeout ;
        else
            query->timeout = now
