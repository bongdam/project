...
static int quiet, probe, using_cpe, dev_id;
...
void ping_result(struct ping *pong)
{
	FILE *fp;
	char tmp[80];

	sprintf(tmp, "%s%d", CPEPING_RESULT_PATH, dev_id );
	if ((fp = fopen(tmp, "w"))) {
		fprintf(fp, "minPingTime=%d\n", pong->tmin / 10);
		fprintf(fp, "avgPingTime=%d\n", (pong->tsum / (pong->nreceived + pong->nrepeats))/10 );
		fprintf(fp, "maxPingTime=%d\n", pong->tmax / 10);
		fprintf(fp, "pktTimeoutcnt=%d\n", (pong->ntransmitted - pong->nreceived) );
		fclose(fp);
	}
}
...
int mping_main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "c:s:w:pqg:f:")) != -1) {
		switch (opt) {
		case 'c':
			count = atoi(optarg);
			break;
