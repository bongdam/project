...
extern unsigned int ComSrlCmd_EX4B(unsigned char, unsigned int);
...
#define BSP_WDTCNR          (BSP_TC_BASE + 0x1C)
	...
	local_irq_disable();
#ifdef CONFIG_SPI_3to4BYTES_ADDRESS_SUPPORT
	/* young@davolink.co.kr -
	   Move in here due to being hanged in the course of force_stop_wlan_hw().
	   In detail, reading from 0xb900XXXX which is the ioaddr of WLAN1 is stalled.
	 */
	ComSrlCmd_EX4B(0, 4);
#endif
