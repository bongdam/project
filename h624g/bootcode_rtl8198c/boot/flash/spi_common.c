...
{0x00EF4019, 0x00, SIZEN_08M, SIZE_064K, SIZE_004K, SIZE_256B, "W25Q256", 104
#ifndef CONFIG_SPI_STD_MODE
, ComSrlCmd_SE, wb_cmd_read_q0, wb_spi_setQEBit, wb_cmd_write_q0
#endif
},
...
#ifdef __DAVO__
extern int printScale(char *buf, unsigned int int_part);
#ifdef CONFIG_SPI_3to4BYTES_ADDRESS_SUPPORT
unsigned int ComSrlCmd_EX4B(unsigned char ucChip, unsigned int uiLen);
static unsigned int ComSrlCmd_RDJEDEC(unsigned char, unsigned int, unsigned int);
unsigned int (* spi_cmd_4to3)(unsigned char, unsigned int) = ComSrlCmd_EX4B;
static int spi_cmd_4to3_dummy(unsigned char chip, unsigned int len)
{
	return 0;
}
#endif	/* CONFIG_SPI_3to4BYTES_ADDRESS_SUPPORT */
#endif	/* __DAVO__ */
// spi flash probe
void spi_regist(unsigned char ucChip)
{
	...
	case 0xc22019c2:
		...
		if ((t1 & 0x00ffffff) == 0x00534644) {	// SFDP supported (Serial Flash Discoverable Parameter)
			...
			t1 = (t1 >> 16) & 0x000000ff;
			if (((t1 >> 1) & 0x03) == 2) {
				spi_cmd_4to3 = spi_cmd_4to3_dummy;
				break;
			}
		}
...
void prnFlashInfo(unsigned char ucChip, struct spi_flash_type sftInfo)
{
	...
	p += printScale(p, sftInfo.block_size);
	*p++ = '/';
	p += printScale(p, sftInfo.sector_size);
	*p++ = '/';
	p += printScale(p, sftInfo.page_size);
	*p++ = '/';
	printScale(p, sftInfo.chip_size);
	NDEBUG("\nFlash%d:%s Manu.ID/JEDEC.ID:0x%02x/0x%x Blk/Sect/Page/Size:%s Clk:%d\n",
		ucChip, sftInfo.chip_name,
		sftInfo.mfr_id, sftInfo.chip_id, buf, sftInfo.chip_clk);
#else
#ifndef CONFIG_SPI_STD_MODE
	NDEBUG("\n********************************************************************************\n");
	NDEBUG("*\n");
...
#else
	char buf[32];
	strcpy(buf, sftInfo.chip_name);
	NDEBUG("\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
	NDEBUG("@\n");
	NDEBUG("@ chip__no chip__id mfr___id dev___id cap___id size_sft dev_size chipSize\n");
	NDEBUG("@ %7xh %7xh %7xh %7xh %7xh %7xh %7xh %7xh\n", ucChip, sftInfo.chip_id, sftInfo.mfr_id, sftInfo.dev_id, sftInfo.capacity_id, sftInfo.size_shift, sftInfo.device_size, sftInfo.chip_size);
	NDEBUG("@ blk_size blk__cnt sec_size sec__cnt pageSize page_cnt chip_clk chipName\n");
	NDEBUG("@ %7xh %7xh %7xh %7xh %7xh %7xh %7xh %s\n", sftInfo.block_size, sftInfo.block_cnt, sftInfo.sector_size, sftInfo.sector_cnt, sftInfo.page_size, sftInfo.page_cnt, sftInfo.chip_clk, buf/*sftInfo.chip_name*/);
	NDEBUG("@ \n");
	NDEBUG("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
#endif
#endif
...
unsigned int flashCapacity(void)
{
	return spi_flash_info[0].chip_size;
}
