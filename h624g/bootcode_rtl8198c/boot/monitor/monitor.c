...
extern int rtk_read_ecc_page(unsigned long flash_address, unsigned char *image_addr, unsigned int image_size, char ecc_enable);
extern int rtk_write_ecc_page(unsigned long flash_address, unsigned char *image_addr, unsigned int image_size);
...
__inline__ void __delay(unsigned long loops)
{
...
int CmdCfn(int argc, char *argv[])
{
	...
	if (argc > 0) {
		Address = strtoul(argv[0], &eptr, 16);
		if (argv[0] == eptr || *eptr) {
...
int CmdSFlw(int argc, char *argv[])
{
	...
	printf("Write 0x%x Bytes to SPI flash#%d, offset 0x%x<0x%x>, from RAM 0x%x to 0x%x\n", length, cnt2 + 1,
	       dst_flash_addr_offset, dst_flash_addr_offset + 0xbd000000, src_RAM_addr, end_of_RAM_addr);
	printf("(Y)es, (N)o->");
	if (YesOrNo()) {
		spi_pio_init();
...
static int CmdNvram(int argc, char *argv[])
{
	if (argc < 1)
		return 0;
	if (!strcmp(argv[0], "get")) {
		if (argc > 1) {
			char *p = nvram_get(argv[1]);
			dprintf("%s\n", (p) ? p : "");
		}
...
#ifdef  CONFIG_DRAM_TEST
	{"DRAMTEST", 7, Dram_test,
	 "dramtest <1-R/W> <2-enable_random_delay> <3-PowerManagementMode><4-cache_type><5-bit_type><6-Data_Pattern><7-Random_mode>"},
#endif
