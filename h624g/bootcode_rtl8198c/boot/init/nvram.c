...
#define FLASH_MEM_MAP_ADDR 0x1d000000
...
int nvram_commit(void)
{
	...
	spi_flw_image_mio_8198(0, flashCapacity() - MAX_NVRAM_SPACE, (unsigned char *)header, header->len);
	free(header);
	return 0;
}
...
int nvram_backup(void)
{
	...
	p = (struct nvram_header *)_nvram_memcpy(buf,
			flashCapacity() - MAX_NVRAM_SPACE, sizeof(struct nvram_header));
	if (p->magic != NVRAM_MAGIC)
		return -1;
	b = (struct nvram_header *)_nvram_memcpy(&buf[MAX_NVRAM_SPACE],
			flashCapacity() - (MAX_NVRAM_SPACE << 1), sizeof(struct nvram_header));
	...
	if ((b->magic != NVRAM_MAGIC2) ||
		...
		spi_flw_image_mio_8198(0, flashCapacity() - (MAX_NVRAM_SPACE << 1), buf, p->len);
		rc = 0;
	}
	free(buf);
	return rc;
...
int parse_bootline(unsigned int *kern, unsigned int *fs)
{
	...
	if (p != NULL) {
		...
		if (*q) {
			...
			if (kernel < rootfs && rootfs < flashCapacity()) {
				*kern = kernel;
				*fs = rootfs;
				return 0;
			}
