...
#  else
#    define _SYSTEM_HEAP_SIZE	(1024*1024)	//wei add
#  endif
static char dl_heap[_SYSTEM_HEAP_SIZE];	//wei add
# endif
#endif
...
#   endif
#   ifdef __DAVO__
		if ((RTL_R32(RTL_GPIO_MUX4) & (3 << 23)) != RTL_GPIO_RESET_BTN_MUX) {
			RTL_W32(RTL_GPIO_MUX4, RTL_R32(RTL_GPIO_MUX4) & ~(3 << 23));
			RTL_W32(RTL_GPIO_MUX4, RTL_R32(RTL_GPIO_MUX4) | RTL_GPIO_RESET_BTN_MUX);
		}
		if (RTL_R32(RESET_PIN_IOBASE) & (1 << RESET_BTN_PIN))
			RTL_W32(RESET_PIN_IOBASE, RTL_R32(RESET_PIN_IOBASE) & ~(1 << RESET_BTN_PIN));
		if (RTL_R32(RESET_PIN_DIRBASE) & (1 << RESET_BTN_PIN))
			RTL_W32(RESET_PIN_DIRBASE, RTL_R32(RESET_PIN_DIRBASE) & ~(1 << RESET_BTN_PIN));
#   endif
		if (0 /*Get_GPIO_SW_IN()*/) {	// button pressed
#   if defined(UTILITY_DEBUG)
