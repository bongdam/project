...
//dprintf("send TFTP-RRQ request\n");
	...
	prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_SERVER].node, (Int8 *)&tftp_tx,
		      (Int16)sizeof(struct iphdr) + sizeof(struct udphdr) + length + 2);
}
...
void autoreboot()
{
	...
	ComSrlCmd_EX4B(0, 4);
#else	
	extern unsigned int (* spi_cmd_4to3)(unsigned char, unsigned int);
	spi_cmd_4to3(0, 4);
#endif
#endif
...
#  ifdef SUPPORT_SPI_MIO_8198_8196C
		if (Header.burnAddr + burn_offset + burnLen > spi_flash_info[0].chip_size) {
			if (spi_flw_image_mio_8198(0, Header.burnAddr + burn_offset, srcAddr,
			    ...
			    && spi_flw_image_mio_8198(1, 0, srcAddr + spi_flash_info[0].chip_size - (Header.burnAddr + burn_offset),
...
#  if CHECK_BURN_SERIAL
		...
		if (!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) ||
		    !memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN)) {
			if (memcmp(Header.burnAddr + burn_offset + (unsigned long)FLASH_BASE + mips_io_port_base, srcAddr,
			     burnLen) != 0) {
			...
			if (memcmp(Header.burnAddr + burn_offset + burnLen + (unsigned long)FLASH_BASE + mips_io_port_base,
			     &burn_serial, sizeof(burn_serial)) != 0) {
				printf("[%s]%d: linux image burn fail!\n", __func__, __LINE__);
				memset(Header.signature, 0, SIG_LEN);
				spi_flw_image_mio_8198(0, Header.burnAddr, &Header, sizeof(Header));
				trueorfalse = 0;
			...
			if (padding_len) {
				...
				if (memcmp(Header.burnAddr + burn_offset + burnLen - SIZE_OF_CHECKSUM + (unsigned long)FLASH_BASE +
				     mips_io_port_base, (void *)(&rootfs_padding), sizeof(rootfs_padding)) != 0) {
					printf("[%s]%d: root image burn fail!\n", __func__, __LINE__);
					memset(Header.signature, 0, SIG_LEN);
					spi_flw_image_mio_8198(0, Header.burnAddr, &Header, sizeof(Header));
					trueorfalse = 0;
				...
				BDBG_BSN("[%s:%d] nopad\n", __FUNCTION__, __LINE__);
				if (memcmp(Header.burnAddr + burn_offset + burnLen - SIZE_OF_CHECKSUM + (unsigned long)FLASH_BASE +
				     mips_io_port_base, (void *)(&rootfs_padding) + sizeof(rootfs_padding.zero_pad),
...
void tftpd_send_ack(Int16 number)
{
	...
	if (tftp_client_enabled)
		prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_SERVER].node, (Int8 *)&tftp_tx,
			      (Int16)sizeof(struct iphdr) + sizeof(struct udphdr) + 4);
	else
#endif
		prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_CLIENT].node, (Int8 *)&tftp_tx,
			      (Int16)sizeof(struct iphdr) + sizeof(struct udphdr) + 4);
}
...
void tftpd_send_data(char *filename, Int16 block_number)
{
 ...
 /********************************************/
	...
	prepare_txpkt(0, FRAME_IP, arptable_tftp[TFTP_CLIENT].node, (Int8 *)&tftp_tx,
		      (Int16)sizeof(struct iphdr) + sizeof(struct udphdr) + length + 4);
}
