...
extern unsigned int flashCapacity(void);
...
#endif
	/* GPIO_C0 PWR LED */
	RTL_W32(RTL_GPIO_MUX5, RTL_R32(RTL_GPIO_MUX5) | (3 << 16));
	RTL_W32(PABCD_CNR , RTL_R32(PABCD_CNR) & ~(1 << 16));
	RTL_W32(PABCD_DIR , RTL_R32(PABCD_DIR) | (1 << 16));
	RTL_W32(PABCD_DAT , RTL_R32(PABCD_DAT) | (1 << 16));
...
# endif
	...
	chipSize = flashCapacity();
	if (init_nvram(chipSize))
		dprintf("Could not find NVRAM.\n");
	else if ((p_nvram_offset && (p_nvram_offset != (chipSize - max_nvram_space))) ||
...
#define PATT_READY  0x5555
	...
	prom_printf("---Realtek RTL8198C boot code at %s v%c.%c%c (%dMHz) [C%d running][C%d %s]\n",
		    BOOT_CODE_TIME, BT_MAJOR, BT_MINOR, BT_BUILD,
		    cpu_speed, cpu_num, 1 - cpu_num,
		    (REG32(POLLING_REG) == PATT_SLEEP) ? "sleep" : (REG32(POLLING_REG) == PATT_READY) ? "ready" : "miss");
