...
#define _memctl_debug_printf(arg...)	do {} while (0)
...
//---------------------------------------------------
#if DBG
static void uart_outc_internal(char c)
{
...
{
	if (c == '\n')
		uart_outc_internal('\r');
	uart_outc_internal(c);
}
#endif
//-----------------------------------------------------
...
void memctlc_check_DCR(void)
{
	...
	printf("  Size: %dMB", (dram_total_size/1024/1024));
...
void memctlc_show_reg_info(void)
{
	...
	printf("Freq: %dMHz", board_DRAM_freq_mhz());
	  memctlc_check_DCR();
	_memctl_debug_printf("\tMCR (0x%08x):0x%08x\n", MCR, REG(MCR));
	_memctl_debug_printf("\tDCR (0x%08x):0x%08x\n", DCR, REG(DCR));
	_memctl_debug_printf("\tDTR0(0x%08x):0x%08x\n", DTR0,REG(DTR0));
